...
class AUGovBOMWeatherAmqpProducer:
    """
    Producer class to send messages in the `AU.Gov.BOM.Weather.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        ...
        """
        Send the `AU.Gov.BOM.Weather.amqp.Station` message
        ...
        attributes = {
            ...
            "source":
            "http://reg.bom.gov.au/fwo",
            "subject":
            "{station_wmo}".format(station_wmo=_station_wmo),
        }
        ...
        amqp_msg.subject = "{station_wmo}".format(station_wmo=_station_wmo)
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `AU.Gov.BOM.Weather.amqp.Station` messages
    ...
    def send_weather_observation(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `AU.Gov.BOM.Weather.amqp.WeatherObservation` message
    ...
    def send_weather_observation_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `AU.Gov.BOM.Weather.amqp.WeatherObservation` messages
...
class AUGovBOMWarningAmqpProducer:
    """
    Producer class to send messages in the `AU.Gov.BOM.Warning.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_warning_bulletin(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `AU.Gov.BOM.Warning.amqp.WarningBulletin` message
        ...
        attributes = {
            ...
            "source":
            "https://www.bom.gov.au/rss/",
            "subject":
            "{state}/{severity}/{warning_id}".format(state=_state, severity=_severity, warning_id=_warning_id),
        }
        ...
        if self.content_mode == 'structured':
            ...
            if headers:
                amqp_msg.properties = self._ce_headers_to_amqp_properties(headers)
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "{state}/{severity}/{warning_id}".format(state=_state, severity=_severity, warning_id=_warning_id)
    ...
    def send_warning_bulletin_batch(self,
        data_array: typing.List[WarningBulletin],
        _state: str,
        _severity: str,
        _warning_id: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `AU.Gov.BOM.Warning.amqp.WarningBulletin` messages
        
        Args:
            data_array (typing.List[WarningBulletin]): Array of message data objects
            _state (str): Value for placeholder state in attribute subject
            _severity (str): Value for placeholder severity in attribute subject
            _warning_id (str): Value for placeholder warning_id in attribute subject
            content_type (str): The content type of the message data
        """
        for data in data_array:
            self.send_warning_bulletin(
                data=data,
                _state=_state,
                _severity=_severity,
                _warning_id=_warning_id,
                content_type=content_type)
