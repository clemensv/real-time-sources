...
def get_default_topic_mappings_au_gov_bom_weather_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the AU.Gov.BOM.Weather.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "AU.Gov.BOM.Weather.mqtt.Station": "weather/au/bom/bom-australia/{state}/{station_wmo}/info",
        "AU.Gov.BOM.Weather.mqtt.WeatherObservation": "weather/au/bom/bom-australia/{state}/{station_wmo}/observation",
    }


def get_subscription_topics_au_gov_bom_weather_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_au_gov_bom_weather_mqtt()
    topics = []
...
class AUGovBOMWeatherMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the AU.Gov.BOM.Weather.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_au_gov_bom_weather_mqtt()
        ...
        self.au_gov_bom_weather_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, bom_australia_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.au_gov_bom_weather_mqtt_weather_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, bom_australia_mqtt_producer_data.WeatherObservation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "AU.Gov.BOM.Weather.Station":
            if self.au_gov_bom_weather_mqtt_station_async:
                try:
                    ...
                    data = bom_australia_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "AU.Gov.BOM.Weather.mqtt.Station")
                    await self.au_gov_bom_weather_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in au_gov_bom_weather_mqtt_station handler: {e}")
            return
        
        if event_type == "AU.Gov.BOM.Weather.WeatherObservation":
            if self.au_gov_bom_weather_mqtt_weather_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = bom_australia_mqtt_producer_data.WeatherObservation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "AU.Gov.BOM.Weather.mqtt.WeatherObservation")
                    await self.au_gov_bom_weather_mqtt_weather_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in au_gov_bom_weather_mqtt_weather_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_au_gov_bom_weather_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_au_gov_bom_weather_mqtt_station(self,
        station_wmo: str,
        state: str,
        data: bom_australia_mqtt_producer_data.Station,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'AU.Gov.BOM.Weather.mqtt.Station' event to an MQTT topic.

        Args:
        
            station_wmo: URI template variable for 'station_wmo'
            state: URI template variable for 'state'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/au/bom/bom-australia/{state}/{station_wmo}/info'
        ...
        """
        target_topic = topic if topic is not None else "weather/au/bom/bom-australia/{state}/{station_wmo}/info"
        _topic_template_values: Dict[str, str] = {
            "station_wmo": str(station_wmo),
            "state": str(state),
        }
        ...
        attributes = {
             ...
             "subject":"{station_wmo}".format(station_wmo = station_wmo)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_au_gov_bom_weather_mqtt_weather_observation(self,
        station_wmo: str,
        state: str,
        data: bom_australia_mqtt_producer_data.WeatherObservation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'AU.Gov.BOM.Weather.mqtt.WeatherObservation' event to an MQTT topic.
        ...
        Args:
            ...
            state: URI template variable for 'state'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/au/bom/bom-australia/{state}/{station_wmo}/observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "weather/au/bom/bom-australia/{state}/{station_wmo}/observation"
        _topic_template_values: Dict[str, str] = {
            "station_wmo": str(station_wmo),
            "state": str(state),
        }
...
def get_default_topic_mappings_au_gov_bom_warning_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the AU.Gov.BOM.Warning.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "AU.Gov.BOM.Warning.mqtt.WarningBulletin": "alerts/au/bom/bom-australia/{state}/{severity}/{warning_id}/warning",
    }


def get_subscription_topics_au_gov_bom_warning_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_au_gov_bom_warning_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class AUGovBOMWarningMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the AU.Gov.BOM.Warning.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_au_gov_bom_warning_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        
        # Message handler callbacks (Dispatcher pattern)
        
        self.au_gov_bom_warning_mqtt_warning_bulletin_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, bom_australia_mqtt_producer_data.WarningBulletin, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "AU.Gov.BOM.Warning.WarningBulletin":
            if self.au_gov_bom_warning_mqtt_warning_bulletin_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = bom_australia_mqtt_producer_data.WarningBulletin.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "AU.Gov.BOM.Warning.mqtt.WarningBulletin")
                    await self.au_gov_bom_warning_mqtt_warning_bulletin_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in au_gov_bom_warning_mqtt_warning_bulletin handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_au_gov_bom_warning_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_au_gov_bom_warning_mqtt_warning_bulletin(self,
        state: str,
        severity: str,
        warning_id: str,
        data: bom_australia_mqtt_producer_data.WarningBulletin,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'AU.Gov.BOM.Warning.mqtt.WarningBulletin' event to an MQTT topic.

        Args:
        
            state: URI template variable for 'state'
            severity: URI template variable for 'severity'
            warning_id: URI template variable for 'warning_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'alerts/au/bom/bom-australia/{state}/{severity}/{warning_id}/warning'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "alerts/au/bom/bom-australia/{state}/{severity}/{warning_id}/warning"
        _topic_template_values: Dict[str, str] = {
            "state": str(state),
        ...
        attributes = {
             ...
             "subject":"{state}/{severity}/{warning_id}".format(state = state,severity = severity,warning_id = warning_id)
        }
        attributes["datacontenttype"] = content_type
        byte_data = data.to_byte_array(content_type) if data is not None else b''
        # to_byte_array returns str for text content types (e.g. JSON);
