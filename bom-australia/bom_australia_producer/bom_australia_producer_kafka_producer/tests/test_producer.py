...
def test_au_gov_bom_weather_mqtt_augovbomweathermqttstation(kafka_emulator):
    """Test the AUGovBOMWeatherMqttStation event from the AU.Gov.BOM.Weather.Mqtt message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_au_gov_bom_weather_mqtt_augovbomweathermqttstation',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "AU.Gov.BOM.Weather.mqtt.Station":
                return msg.key().decode('utf-8') if msg.key() else None

    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = AUGovBOMWeatherMqttEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    event_data = Test_Station.create_instance()
...
def test_au_gov_bom_weather_mqtt_augovbomweathermqttweatherobservation(kafka_emulator):
    """Test the AUGovBOMWeatherMqttWeatherObservation event from the AU.Gov.BOM.Weather.Mqtt message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_au_gov_bom_weather_mqtt_augovbomweathermqttweatherobservation',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "AU.Gov.BOM.Weather.mqtt.WeatherObservation":
                return msg.key().decode('utf-8') if msg.key() else None
...
def test_au_gov_bom_weather_amqp_augovbomweatheramqpstation(kafka_emulator):
    """Test the AUGovBOMWeatherAmqpStation event from the AU.Gov.BOM.Weather.Amqp message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_au_gov_bom_weather_amqp_augovbomweatheramqpstation',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "AU.Gov.BOM.Weather.amqp.Station":
                return msg.key().decode('utf-8') if msg.key() else None

    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = AUGovBOMWeatherAmqpEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    event_data = Test_Station.create_instance()
...
def test_au_gov_bom_weather_amqp_augovbomweatheramqpweatherobservation(kafka_emulator):
    """Test the AUGovBOMWeatherAmqpWeatherObservation event from the AU.Gov.BOM.Weather.Amqp message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_au_gov_bom_weather_amqp_augovbomweatheramqpweatherobservation',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "AU.Gov.BOM.Weather.amqp.WeatherObservation":
                return msg.key().decode('utf-8') if msg.key() else None
...
def test_au_gov_bom_warning_mqtt_augovbomwarningmqttwarningbulletin(kafka_emulator):
    """Test the AUGovBOMWarningMqttWarningBulletin event from the AU.Gov.BOM.Warning.Mqtt message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_au_gov_bom_warning_mqtt_augovbomwarningmqttwarningbulletin',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "AU.Gov.BOM.Warning.mqtt.WarningBulletin":
                return msg.key().decode('utf-8') if msg.key() else None

    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = AUGovBOMWarningMqttEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    event_data = Test_WarningBulletin.create_instance()
...
def test_au_gov_bom_warning_amqp_augovbomwarningamqpwarningbulletin(kafka_emulator):
    """Test the AUGovBOMWarningAmqpWarningBulletin event from the AU.Gov.BOM.Warning.Amqp message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_au_gov_bom_warning_amqp_augovbomwarningamqpwarningbulletin',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "AU.Gov.BOM.Warning.amqp.WarningBulletin":
                return msg.key().decode('utf-8') if msg.key() else None

    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = AUGovBOMWarningAmqpEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    event_data = Test_WarningBulletin.create_instance()
