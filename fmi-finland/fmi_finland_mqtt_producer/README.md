# Fmi_finland_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the fmi_finland_mqtt_producer message definitions.

# Fmi_finland_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - FiFmiOpendataAirqualityMqttEventDispatcher
...
from fmi_finland_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = FiFmiOpendataAirqualityMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### FiFmiOpendataAirqualityMqttEventDispatcher



# Publish message`FiFmiOpendataAirqualityMqttEventDispatcher` handles events for the fi.fmi.opendata.airquality.mqtt
message group.
...
from fmi_finland_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = FiFmiOpendataAirqualityMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe FiFmiOpendataAirqualityMqttEventDispatcher defines the following event handler hooks.
...
```##### `fi_fmi_opendata_airquality_mqtt_station_async`



## Test```python

fi_fmi_opendata_airquality_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station],
Awaitable[None]]
...
- `data`: The event data of type `fmi_finland_mqtt_producer_data.Station`.

Example:

```python
async def fi_fmi_opendata_airquality_mqtt_station_event(partition_context: PartitionContext, event: EventData,
...
```##### `fi_fmi_opendata_airquality_mqtt_observation_async`



## Test```python

fi_fmi_opendata_airquality_mqtt_observation_async:  Callable[[PartitionContext, EventData, CloudEvent, Observation],
Awaitable[None]]
...
```Asynchronous handler hook for `fi.fmi.opendata.airquality.mqtt.Observation`: Hourly air quality observation
aggregated per station and timestamp from FMI OGC WFS simple query results.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `fmi_finland_mqtt_producer_data.Observation`.

Example:

```python
async def fi_fmi_opendata_airquality_mqtt_observation_event(partition_context: PartitionContext, event: EventData,
