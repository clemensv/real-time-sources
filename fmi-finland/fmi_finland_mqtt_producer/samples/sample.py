...
import os
import signal
from fmi_finland_mqtt_producer_data import *
from fmi_finland_mqtt_producer_mqtt_client.client import FiFmiOpendataAirqualityMqttProducer, FiFmiOpendataAirqualityMqttDispatcher

async def handle_fi_fmi_opendata_airquality_mqtt_station(mqtt_msg,cloud_event, fi_fmi_opendata_airquality_mqtt_station_data):
    """ Handles the fi.fmi.opendata.airquality.mqtt.Station message """
    print(f"Received fi.fmi.opendata.airquality.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {fi_fmi_opendata_airquality_mqtt_station_data}")

async def handle_fi_fmi_opendata_airquality_mqtt_observation(mqtt_msg,cloud_event, fi_fmi_opendata_airquality_mqtt_observation_data):
    """ Handles the fi.fmi.opendata.airquality.mqtt.Observation message """
    print(f"Received fi.fmi.opendata.airquality.mqtt.Observation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {fi_fmi_opendata_airquality_mqtt_observation_data}")
