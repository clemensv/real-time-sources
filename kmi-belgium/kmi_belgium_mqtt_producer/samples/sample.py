...
import os
import signal
from kmi_belgium_mqtt_producer_data import *
from kmi_belgium_mqtt_producer_mqtt_client.client import BEGovKMIWeatherMqttProducer, BEGovKMIWeatherMqttDispatcher

async def handle_be_gov_kmi_weather_mqtt_station(mqtt_msg,cloud_event, be_gov_kmi_weather_mqtt_station_data):
    """ Handles the BE.Gov.KMI.Weather.mqtt.Station message """
    print(f"Received BE.Gov.KMI.Weather.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {be_gov_kmi_weather_mqtt_station_data}")

async def handle_be_gov_kmi_weather_mqtt_weather_observation(mqtt_msg,cloud_event, be_gov_kmi_weather_mqtt_weather_observation_data):
    """ Handles the BE.Gov.KMI.Weather.mqtt.WeatherObservation message """
    print(f"Received BE.Gov.KMI.Weather.mqtt.WeatherObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {be_gov_kmi_weather_mqtt_weather_observation_data}")
