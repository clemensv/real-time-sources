...
def get_default_topic_mappings_de_uba_airdata_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the de.uba.airdata.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "de.uba.airdata.mqtt.Station": "air-quality/at/uba/uba-airdata/{bundesland}/{station_id}/{component_id}/info",
        "de.uba.airdata.mqtt.Measure": "air-quality/at/uba/uba-airdata/{bundesland}/{station_id}/{component_id}/measure",
    }


def get_subscription_topics_de_uba_airdata_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_de_uba_airdata_mqtt()
    topics = []
...
class DeUbaAirdataMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the de.uba.airdata.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_de_uba_airdata_mqtt()
        ...
        self.de_uba_airdata_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, uba_airdata_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.de_uba_airdata_mqtt_measure_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, uba_airdata_mqtt_producer_data.Measure, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "de.uba.airdata.Station":
            if self.de_uba_airdata_mqtt_station_async:
                try:
                    ...
                    data = uba_airdata_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "de.uba.airdata.mqtt.Station")
                    await self.de_uba_airdata_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_uba_airdata_mqtt_station handler: {e}")
            return
        
        if event_type == "de.uba.airdata.Measure":
            if self.de_uba_airdata_mqtt_measure_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = uba_airdata_mqtt_producer_data.Measure.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "de.uba.airdata.mqtt.Measure")
                    await self.de_uba_airdata_mqtt_measure_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_uba_airdata_mqtt_measure handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_de_uba_airdata_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_de_uba_airdata_mqtt_station(self,
        feedurl: str,
        station_id: str,
        bundesland: str,
        component_id: str,
        data: uba_airdata_mqtt_producer_data.Station,
        ...
        """
        Publish the 'de.uba.airdata.mqtt.Station' event to an MQTT topic.

        Args:
        
            feedurl: URI template variable for 'feedurl'
            station_id: URI template variable for 'station_id'
            bundesland: URI template variable for 'bundesland'
            component_id: URI template variable for 'component_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/at/uba/uba-airdata/{bundesland}/{station_id}/{component_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "air-quality/at/uba/uba-airdata/{bundesland}/{station_id}/{component_id}/info"
    ...
    async def publish_de_uba_airdata_mqtt_measure(self,
        feedurl: str,
        station_id: str,
        bundesland: str,
        component_id: str,
        data: uba_airdata_mqtt_producer_data.Measure,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'de.uba.airdata.mqtt.Measure' event to an MQTT topic.
        ...
        Args:
            ...
            component_id: URI template variable for 'component_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/at/uba/uba-airdata/{bundesland}/{station_id}/{component_id}/measure'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/at/uba/uba-airdata/{bundesland}/{station_id}/{component_id}/measure"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
            "station_id": str(station_id),
            "bundesland": str(bundesland),
...
def get_default_topic_mappings_de_uba_airdata_components_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the de.uba.airdata.components.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "de.uba.airdata.components.mqtt.Component": "air-quality/at/uba/uba-airdata/{bundesland}/{station_id}/{component_id}/component",
    }


def get_subscription_topics_de_uba_airdata_components_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_de_uba_airdata_components_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class DeUbaAirdataComponentsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the de.uba.airdata.components.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_de_uba_airdata_components_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        
        # Message handler callbacks (Dispatcher pattern)
        
        self.de_uba_airdata_components_mqtt_component_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, uba_airdata_mqtt_producer_data.Component, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "de.uba.airdata.components.Component":
            if self.de_uba_airdata_components_mqtt_component_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = uba_airdata_mqtt_producer_data.Component.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "de.uba.airdata.components.mqtt.Component")
                    await self.de_uba_airdata_components_mqtt_component_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_uba_airdata_components_mqtt_component handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_de_uba_airdata_components_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_de_uba_airdata_components_mqtt_component(self,
        feedurl: str,
        component_id: str,
        bundesland: str,
        station_id: str,
        data: uba_airdata_mqtt_producer_data.Component,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'de.uba.airdata.components.mqtt.Component' event to an MQTT topic.
        ...
        Args:
            ...
            station_id: URI template variable for 'station_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/at/uba/uba-airdata/{bundesland}/{station_id}/{component_id}/component'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/at/uba/uba-airdata/{bundesland}/{station_id}/{component_id}/component"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
            "component_id": str(component_id),
            "bundesland": str(bundesland),
        ...
        attributes = {
             ...
             "source":"{feedurl}".format(feedurl = feedurl),
             "subject":"{component_id}".format(component_id = component_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
