# Uba_airdata_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the uba_airdata_mqtt_producer message definitions.

# Uba_airdata_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - DeUbaAirdataMqttEventDispatcher,

    DeUbaAirdataComponentsMqttEventDispatcher
...
from uba_airdata_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = DeUbaAirdataMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### DeUbaAirdataMqttEventDispatcher



# Publish message`DeUbaAirdataMqttEventDispatcher` handles events for the de.uba.airdata.mqtt message group.
...
from uba_airdata_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = DeUbaAirdataMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
client = DeUbaAirdataComponentsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe DeUbaAirdataMqttEventDispatcher defines the following event handler hooks.
...
```##### `de_uba_airdata_mqtt_station_async`



## Test```python

de_uba_airdata_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `uba_airdata_mqtt_producer_data.Station`.

Example:

```python
async def de_uba_airdata_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `de_uba_airdata_mqtt_measure_async`



## Test```python

de_uba_airdata_mqtt_measure_async:  Callable[[PartitionContext, EventData, CloudEvent, Measure], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `de.uba.airdata.mqtt.Measure`: Hourly one-hour-average air quality measurement for a
UBA monitoring station and pollutant component as returned by the air_data/v3 measures endpoint.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `uba_airdata_mqtt_producer_data.Measure`.

Example:

```python
async def de_uba_airdata_mqtt_measure_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Measure) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = DeUbaAirdataComponentsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### DeUbaAirdataComponentsMqttEventDispatcher



# Publish message`DeUbaAirdataComponentsMqttEventDispatcher` handles events for the de.uba.airdata.components.mqtt
message group.
...
## BuildThe DeUbaAirdataComponentsMqttEventDispatcher defines the following event handler hooks.
...
```##### `de_uba_airdata_components_mqtt_component_async`



## Test```python

de_uba_airdata_components_mqtt_component_async:  Callable[[PartitionContext, EventData, CloudEvent, Component],
Awaitable[None]]
...
```Asynchronous handler hook for `de.uba.airdata.components.mqtt.Component`: Reference data for an air pollutant
component published by the UBA air_data/v3 components catalog, including the stable numeric component identifier, code,
symbol, unit, and English display name.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `uba_airdata_mqtt_producer_data.Component`.

Example:

```python
async def de_uba_airdata_components_mqtt_component_event(partition_context: PartitionContext, event: EventData,
