...
import os
import signal
from uba_airdata_mqtt_producer_data import *
from uba_airdata_mqtt_producer_mqtt_client.client import DeUbaAirdataMqttProducer, DeUbaAirdataMqttDispatcher

async def handle_de_uba_airdata_mqtt_station(mqtt_msg,cloud_event, de_uba_airdata_mqtt_station_data):
    """ Handles the de.uba.airdata.mqtt.Station message """
    print(f"Received de.uba.airdata.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {de_uba_airdata_mqtt_station_data}")

async def handle_de_uba_airdata_mqtt_measure(mqtt_msg,cloud_event, de_uba_airdata_mqtt_measure_data):
    """ Handles the de.uba.airdata.mqtt.Measure message """
    print(f"Received de.uba.airdata.mqtt.Measure on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {de_uba_airdata_mqtt_measure_data}")
from uba_airdata_mqtt_producer_mqtt_client.client import DeUbaAirdataComponentsMqttProducer, DeUbaAirdataComponentsMqttDispatcher

async def handle_de_uba_airdata_components_mqtt_component(mqtt_msg,cloud_event, de_uba_airdata_components_mqtt_component_data):
    """ Handles the de.uba.airdata.components.mqtt.Component message """
    print(f"Received de.uba.airdata.components.mqtt.Component on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {de_uba_airdata_components_mqtt_component_data}")
