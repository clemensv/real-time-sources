...
class DeUbaAirdataAmqpProducer:
    """
    Producer class to send messages in the `de.uba.airdata.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        ...
        """
        Send the `de.uba.airdata.amqp.Station` message
        ...
        Args:
            ...
            _bundesland (str): Value for AMQP protocol option placeholder bundesland
            _component_id (str): Value for AMQP protocol option placeholder component_id
        ...
        app_properties = {}
        app_properties["bundesland"] = "{bundesland}".format(bundesland=_bundesland)
        app_properties["component_id"] = "{component_id}".format(component_id=_component_id)
        if app_properties:
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `de.uba.airdata.amqp.Station` messages
        ...
        for data in data_array:
            self.send_station(
                ...
                _time=_time,
                _bundesland=_bundesland,
                _component_id=_component_id,
    ...
    def send_measure(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `de.uba.airdata.amqp.Measure` message
    ...
    def send_measure_batch(self,
        data_array: typing.List[Measure],
        _feedurl: str,
        _station_id: str,
        _bundesland: str,
        _component_id: str,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `de.uba.airdata.amqp.Measure` messages
        
        Args:
            data_array (typing.List[Measure]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _station_id (str): Value for placeholder station_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            _bundesland (str): Value for AMQP protocol option placeholder bundesland
...
class DeUbaAirdataComponentsAmqpProducer:
    """
    Producer class to send messages in the `de.uba.airdata.components.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_component(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `de.uba.airdata.components.amqp.Component` message
        ...
        attributes = {
            ...
            "{feedurl}".format(feedurl=_feedurl),
            "subject":
            "{component_id}".format(component_id=_component_id),
        }
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        ...
        if amqp_creation_time is not None:
            amqp_msg.creation_time = amqp_creation_time
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "{component_id}".format(component_id=_component_id)

        app_properties = {}
        app_properties["bundesland"] = "{bundesland}".format(bundesland=_bundesland)
        app_properties["station_id"] = "{station_id}".format(station_id=_station_id)
        if app_properties:
            if amqp_msg.properties is None:
                amqp_msg.properties = {}
            amqp_msg.properties.update(app_properties)
    ...
    def send_component_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `de.uba.airdata.components.amqp.Component` messages
