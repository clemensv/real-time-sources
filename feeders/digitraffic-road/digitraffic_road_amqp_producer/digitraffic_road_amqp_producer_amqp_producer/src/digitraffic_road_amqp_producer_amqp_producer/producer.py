...
class FiDigitrafficRoadAmqpProducer:
    """
    Producer class to send messages in the `fi.digitraffic.road.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_tms_sensor_data(self,
        ...
        app_properties["sensor_id"] = "{sensor_id}".format(sensor_id=_sensor_id)
        if app_properties:
    ...
    def send_weather_sensor_data(self,
    ...
    def send_traffic_announcement(self,
        ...
        A current transport measurement or status update from Fintraffic Digitraffic. It carries road traffic measurements and status updates when the upstream feed reports a new or refreshed value.
    ...
    def send_maintenance_tracking(self,
        ...
        app_properties = {}
        app_properties["domain"] = "{domain}".format(domain=_domain)
        if app_properties:
            if amqp_msg.properties is None:
                amqp_msg.properties = {}
            amqp_msg.properties.update(app_properties)
    ...
    def send_maintenance_task_type(self,
        ...
        app_properties = {}
        app_properties["task_id"] = "{task_id}".format(task_id=_task_id)
        if app_properties:
            if amqp_msg.properties is None:
                amqp_msg.properties = {}
            amqp_msg.properties.update(app_properties)
