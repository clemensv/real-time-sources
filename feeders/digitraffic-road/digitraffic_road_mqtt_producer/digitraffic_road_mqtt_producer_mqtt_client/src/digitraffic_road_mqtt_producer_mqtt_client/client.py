...
def get_default_topic_mappings_fi_digitraffic_road_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the fi.digitraffic.road.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "fi.digitraffic.road.mqtt.TmsSensorData": "traffic/fi/fintraffic/digitraffic-road/{station_id}/{sensor_id}/tms-sensor-data",
        "fi.digitraffic.road.mqtt.WeatherSensorData": "traffic/fi/fintraffic/digitraffic-road/{station_id}/{sensor_id}/weather-sensor-data",
        "fi.digitraffic.road.mqtt.TrafficAnnouncement": "traffic/fi/fintraffic/digitraffic-road/messages/{situation_id}/traffic-announcement",
        "fi.digitraffic.road.mqtt.RoadWork": "traffic/fi/fintraffic/digitraffic-road/messages/{situation_id}/road-work",
        "fi.digitraffic.road.mqtt.WeightRestriction": "traffic/fi/fintraffic/digitraffic-road/messages/{situation_id}/weight-restriction",
        "fi.digitraffic.road.mqtt.ExemptedTransport": "traffic/fi/fintraffic/digitraffic-road/messages/{situation_id}/exempted-transport",
        "fi.digitraffic.road.mqtt.MaintenanceTracking": "traffic/fi/fintraffic/digitraffic-road/maintenance/{domain}/tracking",
        "fi.digitraffic.road.mqtt.TmsStation": "traffic/fi/fintraffic/digitraffic-road/stations/{station_id}/tms-station",
        "fi.digitraffic.road.mqtt.WeatherStation": "traffic/fi/fintraffic/digitraffic-road/stations/{station_id}/weather-station",
        "fi.digitraffic.road.mqtt.MaintenanceTaskType": "traffic/fi/fintraffic/digitraffic-road/maintenance-tasks/{task_id}/task-type",
    }


def get_subscription_topics_fi_digitraffic_road_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_fi_digitraffic_road_mqtt()
    topics = []
...
class FiDigitrafficRoadMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the fi.digitraffic.road.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_fi_digitraffic_road_mqtt()
        ...
        self.fi_digitraffic_road_mqtt_tms_sensor_data_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_road_mqtt_producer_data.TmsSensorData, Dict[str, str]], Awaitable[None]]] = None
        
        self.fi_digitraffic_road_mqtt_weather_sensor_data_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_road_mqtt_producer_data.WeatherSensorData, Dict[str, str]], Awaitable[None]]] = None
        
        self.fi_digitraffic_road_mqtt_traffic_announcement_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_road_mqtt_producer_data.TrafficMessage, Dict[str, str]], Awaitable[None]]] = None
        
        self.fi_digitraffic_road_mqtt_road_work_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_road_mqtt_producer_data.TrafficMessage, Dict[str, str]], Awaitable[None]]] = None
        
        self.fi_digitraffic_road_mqtt_weight_restriction_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_road_mqtt_producer_data.TrafficMessage, Dict[str, str]], Awaitable[None]]] = None
        
        self.fi_digitraffic_road_mqtt_exempted_transport_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_road_mqtt_producer_data.TrafficMessage, Dict[str, str]], Awaitable[None]]] = None
        
        self.fi_digitraffic_road_mqtt_maintenance_tracking_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_road_mqtt_producer_data.MaintenanceTracking, Dict[str, str]], Awaitable[None]]] = None
        
        self.fi_digitraffic_road_mqtt_tms_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_road_mqtt_producer_data.TmsStation, Dict[str, str]], Awaitable[None]]] = None
        
        self.fi_digitraffic_road_mqtt_weather_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_road_mqtt_producer_data.WeatherStation, Dict[str, str]], Awaitable[None]]] = None
        
        self.fi_digitraffic_road_mqtt_maintenance_task_type_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_road_mqtt_producer_data.MaintenanceTaskType, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "fi.digitraffic.road.sensors.TmsSensorData":
            if self.fi_digitraffic_road_mqtt_tms_sensor_data_async:
                try:
                    ...
                    data = digitraffic_road_mqtt_producer_data.TmsSensorData.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.road.mqtt.TmsSensorData")
                    await self.fi_digitraffic_road_mqtt_tms_sensor_data_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_road_mqtt_tms_sensor_data handler: {e}")
            return
        
        if event_type == "fi.digitraffic.road.sensors.WeatherSensorData":
            if self.fi_digitraffic_road_mqtt_weather_sensor_data_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = digitraffic_road_mqtt_producer_data.WeatherSensorData.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.road.mqtt.WeatherSensorData")
                    await self.fi_digitraffic_road_mqtt_weather_sensor_data_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_road_mqtt_weather_sensor_data handler: {e}")
            return
        
        if event_type == "fi.digitraffic.road.messages.TrafficAnnouncement":
            if self.fi_digitraffic_road_mqtt_traffic_announcement_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = digitraffic_road_mqtt_producer_data.TrafficMessage.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.road.mqtt.TrafficAnnouncement")
                    await self.fi_digitraffic_road_mqtt_traffic_announcement_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_road_mqtt_traffic_announcement handler: {e}")
            return
        
        if event_type == "fi.digitraffic.road.messages.RoadWork":
            if self.fi_digitraffic_road_mqtt_road_work_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = digitraffic_road_mqtt_producer_data.TrafficMessage.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.road.mqtt.RoadWork")
                    await self.fi_digitraffic_road_mqtt_road_work_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_road_mqtt_road_work handler: {e}")
            return
        
        if event_type == "fi.digitraffic.road.messages.WeightRestriction":
            if self.fi_digitraffic_road_mqtt_weight_restriction_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = digitraffic_road_mqtt_producer_data.TrafficMessage.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.road.mqtt.WeightRestriction")
                    await self.fi_digitraffic_road_mqtt_weight_restriction_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_road_mqtt_weight_restriction handler: {e}")
            return
        
        if event_type == "fi.digitraffic.road.messages.ExemptedTransport":
            if self.fi_digitraffic_road_mqtt_exempted_transport_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = digitraffic_road_mqtt_producer_data.TrafficMessage.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.road.mqtt.ExemptedTransport")
                    await self.fi_digitraffic_road_mqtt_exempted_transport_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_road_mqtt_exempted_transport handler: {e}")
            return
        
        if event_type == "fi.digitraffic.road.maintenance.MaintenanceTracking":
            if self.fi_digitraffic_road_mqtt_maintenance_tracking_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = digitraffic_road_mqtt_producer_data.MaintenanceTracking.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.road.mqtt.MaintenanceTracking")
                    await self.fi_digitraffic_road_mqtt_maintenance_tracking_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_road_mqtt_maintenance_tracking handler: {e}")
            return
        
        if event_type == "fi.digitraffic.road.stations.TmsStation":
            if self.fi_digitraffic_road_mqtt_tms_station_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = digitraffic_road_mqtt_producer_data.TmsStation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.road.mqtt.TmsStation")
                    await self.fi_digitraffic_road_mqtt_tms_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_road_mqtt_tms_station handler: {e}")
            return
        
        if event_type == "fi.digitraffic.road.stations.WeatherStation":
            if self.fi_digitraffic_road_mqtt_weather_station_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = digitraffic_road_mqtt_producer_data.WeatherStation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.road.mqtt.WeatherStation")
                    await self.fi_digitraffic_road_mqtt_weather_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_road_mqtt_weather_station handler: {e}")
            return
        
        if event_type == "fi.digitraffic.road.maintenance.tasks.MaintenanceTaskType":
            if self.fi_digitraffic_road_mqtt_maintenance_task_type_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = digitraffic_road_mqtt_producer_data.MaintenanceTaskType.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.road.mqtt.MaintenanceTaskType")
                    await self.fi_digitraffic_road_mqtt_maintenance_task_type_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_road_mqtt_maintenance_task_type handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_fi_digitraffic_road_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_fi_digitraffic_road_mqtt_tms_sensor_data(self,
        station_id: str,
        sensor_id: str,
        data: digitraffic_road_mqtt_producer_data.TmsSensorData,
        ...
        Args:
            ...
            sensor_id: URI template variable for 'sensor_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/fi/fintraffic/digitraffic-road/{station_id}/{sensor_id}/tms-sensor-data'
    ...
    async def publish_fi_digitraffic_road_mqtt_weather_sensor_data(self,
        station_id: str,
        sensor_id: str,
        data: digitraffic_road_mqtt_producer_data.WeatherSensorData,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            sensor_id: URI template variable for 'sensor_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/fi/fintraffic/digitraffic-road/{station_id}/{sensor_id}/weather-sensor-data'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (0).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    async def publish_fi_digitraffic_road_mqtt_traffic_announcement(self,
        situation_id: str,
        data: digitraffic_road_mqtt_producer_data.TrafficMessage,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            situation_id: URI template variable for 'situation_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/fi/fintraffic/digitraffic-road/messages/{situation_id}/traffic-announcement'
                with URI template placeholders substituted from the keyword arguments.
    ...
    async def publish_fi_digitraffic_road_mqtt_road_work(self,
        situation_id: str,
        data: digitraffic_road_mqtt_producer_data.TrafficMessage,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        ...
        Args:
            ...
            situation_id: URI template variable for 'situation_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/fi/fintraffic/digitraffic-road/messages/{situation_id}/road-work'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    async def publish_fi_digitraffic_road_mqtt_weight_restriction(self,
        situation_id: str,
        data: digitraffic_road_mqtt_producer_data.TrafficMessage,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        ...
        Args:
            ...
            situation_id: URI template variable for 'situation_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/fi/fintraffic/digitraffic-road/messages/{situation_id}/weight-restriction'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    async def publish_fi_digitraffic_road_mqtt_exempted_transport(self,
        situation_id: str,
        data: digitraffic_road_mqtt_producer_data.TrafficMessage,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        ...
        Args:
            ...
            situation_id: URI template variable for 'situation_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/fi/fintraffic/digitraffic-road/messages/{situation_id}/exempted-transport'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    async def publish_fi_digitraffic_road_mqtt_maintenance_tracking(self,
        domain: str,
        data: digitraffic_road_mqtt_producer_data.MaintenanceTracking,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            domain: URI template variable for 'domain'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/fi/fintraffic/digitraffic-road/maintenance/{domain}/tracking'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (0).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    async def publish_fi_digitraffic_road_mqtt_tms_station(self,
        station_id: str,
        data: digitraffic_road_mqtt_producer_data.TmsStation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            station_id: URI template variable for 'station_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/fi/fintraffic/digitraffic-road/stations/{station_id}/tms-station'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
    ...
    async def publish_fi_digitraffic_road_mqtt_weather_station(self,
        station_id: str,
        data: digitraffic_road_mqtt_producer_data.WeatherStation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            station_id: URI template variable for 'station_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/fi/fintraffic/digitraffic-road/stations/{station_id}/weather-station'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    async def publish_fi_digitraffic_road_mqtt_maintenance_task_type(self,
        task_id: str,
        data: digitraffic_road_mqtt_producer_data.MaintenanceTaskType,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            task_id: URI template variable for 'task_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/fi/fintraffic/digitraffic-road/maintenance-tasks/{task_id}/task-type'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
