...
async def main(connection_string: Optional[str], producer_config: Optional[str], topic: Optional[str]):
    ...
    if connection_string:
        ...
        nwsalerts_event_producer = NWSAlertsEventProducer.from_connection_string(connection_string, topic, 'binary')
    else:
        # use a Kafka producer configuration provided as JSON text
        kafka_producer = KafkaProducer(json.loads(producer_config))
        nwsalerts_event_producer = NWSAlertsEventProducer(kafka_producer, topic, 'binary')

    # ---- NWS.WeatherAlert ----
    # TODO: Supply event data for the NWS.WeatherAlert event
    _weather_alert = WeatherAlert()

    # sends the 'NWS.WeatherAlert' event to Kafka topic.
    await nwsalerts_event_producer.send_nws_weather_alert(_alert_id = 'TODO: replace me', data = _weather_alert)
    print(f"Sent 'NWS.WeatherAlert' event: {_weather_alert.to_json()}")
    if connection_string:
        # use a connection string obtained for an Event Stream from the Microsoft Fabric portal
        # or an Azure Event Hubs connection string
        nwsalerts_mqtt_event_producer = NWSAlertsMqttEventProducer.from_connection_string(connection_string, topic, 'binary')
    else:
        # use a Kafka producer configuration provided as JSON text
        kafka_producer = KafkaProducer(json.loads(producer_config))
    ...
    if connection_string:
        # use a connection string obtained for an Event Stream from the Microsoft Fabric portal
        # or an Azure Event Hubs connection string
        nwsalerts_amqp_event_producer = NWSAlertsAmqpEventProducer.from_connection_string(connection_string, topic, 'binary')
    else:
        # use a Kafka producer configuration provided as JSON text
        kafka_producer = KafkaProducer(json.loads(producer_config))
