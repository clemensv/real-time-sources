...
def get_default_topic_mappings_microsoft_opendata_us_noaa_nws_alerts_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the Microsoft.OpenData.US.NOAA.NWS.Alerts.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "Microsoft.OpenData.US.NOAA.NWS.Alerts.mqtt.WeatherAlert": "alerts/us/noaa/noaa-nws/{state}/{severity}/{event_type}/{alert_id}/alert",
    }


def get_subscription_topics_microsoft_opendata_us_noaa_nws_alerts_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_microsoft_opendata_us_noaa_nws_alerts_mqtt()
    topics = []
...
class MicrosoftOpenDataUSNOAANWSAlertsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the Microsoft.OpenData.US.NOAA.NWS.Alerts.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_microsoft_opendata_us_noaa_nws_alerts_mqtt()
        ...
        self.microsoft_open_data_us_noaa_nws_alerts_mqtt_weather_alert_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, noaa_nws_mqtt_producer_data.WeatherAlert, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "Microsoft.OpenData.US.NOAA.NWS.WeatherAlert":
            if self.microsoft_open_data_us_noaa_nws_alerts_mqtt_weather_alert_async:
                try:
                    ...
                    data = noaa_nws_mqtt_producer_data.WeatherAlert.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Microsoft.OpenData.US.NOAA.NWS.Alerts.mqtt.WeatherAlert")
                    await self.microsoft_open_data_us_noaa_nws_alerts_mqtt_weather_alert_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in microsoft_open_data_us_noaa_nws_alerts_mqtt_weather_alert handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_microsoft_opendata_us_noaa_nws_alerts_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_microsoft_open_data_us_noaa_nws_alerts_mqtt_weather_alert(self,
        state: str,
        severity: str,
        event_type: str,
        alert_id: str,
        data: noaa_nws_mqtt_producer_data.WeatherAlert,
        ...
        """
        Publish the 'Microsoft.OpenData.US.NOAA.NWS.Alerts.mqtt.WeatherAlert' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'alerts/us/noaa/noaa-nws/{state}/{severity}/{event_type}/{alert_id}/alert'
        ...
        """
        target_topic = topic if topic is not None else "alerts/us/noaa/noaa-nws/{state}/{severity}/{event_type}/{alert_id}/alert"
        ...
        attributes = {
             "type":"Microsoft.OpenData.US.NOAA.NWS.WeatherAlert",
             "source":"https://api.weather.gov",
             "subject":"{state}/{severity}/{event_type}/{alert_id}".format(state = state,severity = severity,event_type = event_type,alert_id = alert_id)
        }
        attributes["datacontenttype"] = content_type
...
def get_default_topic_mappings_microsoft_opendata_us_noaa_nws_observations_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the Microsoft.OpenData.US.NOAA.NWS.Observations.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "Microsoft.OpenData.US.NOAA.NWS.Observations.mqtt.ObservationStation": "weather/us/noaa/noaa-nws/{state}/{zone_id}/{station_id}/info",
        "Microsoft.OpenData.US.NOAA.NWS.Observations.mqtt.WeatherObservation": "weather/us/noaa/noaa-nws/{state}/{zone_id}/{station_id}/observation",
    }


def get_subscription_topics_microsoft_opendata_us_noaa_nws_observations_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_microsoft_opendata_us_noaa_nws_observations_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class MicrosoftOpenDataUSNOAANWSObservationsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the Microsoft.OpenData.US.NOAA.NWS.Observations.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_microsoft_opendata_us_noaa_nws_observations_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        
        # Message handler callbacks (Dispatcher pattern)
        
        self.microsoft_open_data_us_noaa_nws_observations_mqtt_observation_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, noaa_nws_mqtt_producer_data.ObservationStation, Dict[str, str]], Awaitable[None]]] = None
        
        self.microsoft_open_data_us_noaa_nws_observations_mqtt_weather_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, noaa_nws_mqtt_producer_data.WeatherObservation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "Microsoft.OpenData.US.NOAA.NWS.ObservationStation":
            if self.microsoft_open_data_us_noaa_nws_observations_mqtt_observation_station_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = noaa_nws_mqtt_producer_data.ObservationStation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Microsoft.OpenData.US.NOAA.NWS.Observations.mqtt.ObservationStation")
                    await self.microsoft_open_data_us_noaa_nws_observations_mqtt_observation_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in microsoft_open_data_us_noaa_nws_observations_mqtt_observation_station handler: {e}")
            return
        
        if event_type == "Microsoft.OpenData.US.NOAA.NWS.WeatherObservation":
            if self.microsoft_open_data_us_noaa_nws_observations_mqtt_weather_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = noaa_nws_mqtt_producer_data.WeatherObservation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Microsoft.OpenData.US.NOAA.NWS.Observations.mqtt.WeatherObservation")
                    await self.microsoft_open_data_us_noaa_nws_observations_mqtt_weather_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in microsoft_open_data_us_noaa_nws_observations_mqtt_weather_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_microsoft_opendata_us_noaa_nws_observations_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_microsoft_open_data_us_noaa_nws_observations_mqtt_observation_station(self,
        station_id: str,
        state: str,
        zone_id: str,
        data: noaa_nws_mqtt_producer_data.ObservationStation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Microsoft.OpenData.US.NOAA.NWS.Observations.mqtt.ObservationStation' event to an MQTT topic.

        Args:
        
            station_id: URI template variable for 'station_id'
            state: URI template variable for 'state'
            zone_id: URI template variable for 'zone_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/us/noaa/noaa-nws/{state}/{zone_id}/{station_id}/info'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            ...
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "weather/us/noaa/noaa-nws/{state}/{zone_id}/{station_id}/info"
        _topic_template_values: Dict[str, str] = {
        ...
        attributes = {
             "type":"Microsoft.OpenData.US.NOAA.NWS.ObservationStation",
             "source":"https://api.weather.gov",
    ...
    async def publish_microsoft_open_data_us_noaa_nws_observations_mqtt_weather_observation(self,
        station_id: str,
        state: str,
        zone_id: str,
        data: noaa_nws_mqtt_producer_data.WeatherObservation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Microsoft.OpenData.US.NOAA.NWS.Observations.mqtt.WeatherObservation' event to an MQTT topic.
        ...
        Args:
            ...
            zone_id: URI template variable for 'zone_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/us/noaa/noaa-nws/{state}/{zone_id}/{station_id}/observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "weather/us/noaa/noaa-nws/{state}/{zone_id}/{station_id}/observation"
        _topic_template_values: Dict[str, str] = {
            "station_id": str(station_id),
            "state": str(state),
            "zone_id": str(zone_id),
        ...
        attributes = {
             "type":"Microsoft.OpenData.US.NOAA.NWS.WeatherObservation",
             "source":"https://api.weather.gov",
             "subject":"{station_id}".format(station_id = station_id)
        }
        attributes["datacontenttype"] = content_type
