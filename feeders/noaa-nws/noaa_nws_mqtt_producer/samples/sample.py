...
import os
import signal
from noaa_nws_mqtt_producer_data import *
from noaa_nws_mqtt_producer_mqtt_client.client import MicrosoftOpenDataUSNOAANWSAlertsMqttProducer, MicrosoftOpenDataUSNOAANWSAlertsMqttDispatcher

async def handle_microsoft_open_data_us_noaa_nws_alerts_mqtt_weather_alert(mqtt_msg,cloud_event, microsoft_open_data_us_noaa_nws_alerts_mqtt_weather_alert_data):
    """ Handles the Microsoft.OpenData.US.NOAA.NWS.Alerts.mqtt.WeatherAlert message """
    print(f"Received Microsoft.OpenData.US.NOAA.NWS.Alerts.mqtt.WeatherAlert on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {microsoft_open_data_us_noaa_nws_alerts_mqtt_weather_alert_data}")
from noaa_nws_mqtt_producer_mqtt_client.client import MicrosoftOpenDataUSNOAANWSObservationsMqttProducer, MicrosoftOpenDataUSNOAANWSObservationsMqttDispatcher

async def handle_microsoft_open_data_us_noaa_nws_observations_mqtt_observation_station(mqtt_msg,cloud_event, microsoft_open_data_us_noaa_nws_observations_mqtt_observation_station_data):
    """ Handles the Microsoft.OpenData.US.NOAA.NWS.Observations.mqtt.ObservationStation message """
    print(f"Received Microsoft.OpenData.US.NOAA.NWS.Observations.mqtt.ObservationStation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {microsoft_open_data_us_noaa_nws_observations_mqtt_observation_station_data}")

async def handle_microsoft_open_data_us_noaa_nws_observations_mqtt_weather_observation(mqtt_msg,cloud_event, microsoft_open_data_us_noaa_nws_observations_mqtt_weather_observation_data):
    """ Handles the Microsoft.OpenData.US.NOAA.NWS.Observations.mqtt.WeatherObservation message """
    print(f"Received Microsoft.OpenData.US.NOAA.NWS.Observations.mqtt.WeatherObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {microsoft_open_data_us_noaa_nws_observations_mqtt_weather_observation_data}")
