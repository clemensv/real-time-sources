...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../noaa_nws_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../noaa_nws_amqp_producer_amqp_producer/src')))
...
class TestMicrosoftOpenDataUSNOAANWSAlertsAmqpProducer:
    """Test cases for MicrosoftOpenDataUSNOAANWSAlertsAmqpProducer"""
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "Microsoft.OpenData.US.NOAA.NWS.WeatherAlert"
    ...
    def test_send_weather_alert_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'Microsoft.OpenData.US.NOAA.NWS.WeatherAlert'
        assert received.body is not None
        assert received.subject == "{state}/{severity}/{event_type}/{alert_id}".format(state="value", severity="value", event_type="value", alert_id="value")
...
class TestMicrosoftOpenDataUSNOAANWSObservationsAmqpProducer:
    """Test cases for MicrosoftOpenDataUSNOAANWSObservationsAmqpProducer"""
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "Microsoft.OpenData.US.NOAA.NWS.ObservationStation"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_observation_station_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'Microsoft.OpenData.US.NOAA.NWS.ObservationStation'
        assert received.body is not None
        assert received.subject == "{station_id}".format(station_id="value")
        assert properties.get('state') == "{state}".format(state="value")
        assert properties.get('zone_id') == "{zone_id}".format(zone_id="value")
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "Microsoft.OpenData.US.NOAA.NWS.WeatherObservation"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_weather_observation_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'Microsoft.OpenData.US.NOAA.NWS.WeatherObservation'
        assert received.body is not None
        assert received.subject == "{station_id}".format(station_id="value")
        assert properties.get('state') == "{state}".format(state="value")
        assert properties.get('zone_id') == "{zone_id}".format(zone_id="value")
