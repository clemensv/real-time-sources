...
class MicrosoftOpenDataUSNOAANWSAlertsAmqpProducer:
    """
    Producer class to send messages in the `Microsoft.OpenData.US.NOAA.NWS.Alerts.amqp` message group via AMQP 1.0 protocol.
    """
...
class MicrosoftOpenDataUSNOAANWSObservationsAmqpProducer:
    """
    Producer class to send messages in the `Microsoft.OpenData.US.NOAA.NWS.Observations.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_observation_station(self,
        ...
        app_properties["zone_id"] = "{zone_id}".format(zone_id=_zone_id)
        if app_properties:
            if amqp_msg.properties is None:
                amqp_msg.properties = {}
            amqp_msg.properties.update(app_properties)
