...
"""
Tests for eurdep_radiation_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../eurdep_radiation_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../eurdep_radiation_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../eurdep_radiation_amqp_producer_amqp_producer/src')))

from eurdep_radiation_amqp_producer_amqp_producer import *
from eurdep_radiation_amqp_producer_data import Station
from test_station import Test_Station
from eurdep_radiation_amqp_producer_data import DoseRateReading
...
class TestEuJrcEurdepAmqpProducer:
    """Test cases for EuJrcEurdepAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(EuJrcEurdepAmqpProducer)
    ...
    def test_send_dose_rate_reading(self, artemis_container):
        """Send and receive a DoseRateReading message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_DoseRateReading.create_instance()
    ...
    def test_send_dose_rate_reading_single_fresh_connection(self, artemis_container):
        """Send exactly one DoseRateReading message on a fresh producer connection."""
        payload = Test_DoseRateReading.create_instance()
