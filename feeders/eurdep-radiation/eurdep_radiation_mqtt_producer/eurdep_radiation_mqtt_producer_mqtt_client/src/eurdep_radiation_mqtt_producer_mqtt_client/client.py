...
def get_default_topic_mappings_eu_jrc_eurdep_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the eu.jrc.eurdep.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "eu.jrc.eurdep.mqtt.Station": "radiation/intl/eurdep/eurdep-radiation/{country}/{station_id}/info",
        "eu.jrc.eurdep.mqtt.DoseRateReading": "radiation/intl/eurdep/eurdep-radiation/{country}/{station_id}/dose-rate",
    }


def get_subscription_topics_eu_jrc_eurdep_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_eu_jrc_eurdep_mqtt()
    topics = []
...
class EuJrcEurdepMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the eu.jrc.eurdep.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_eu_jrc_eurdep_mqtt()
        ...
        self.eu_jrc_eurdep_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, eurdep_radiation_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.eu_jrc_eurdep_mqtt_dose_rate_reading_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, eurdep_radiation_mqtt_producer_data.DoseRateReading, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "eu.jrc.eurdep.Station":
            if self.eu_jrc_eurdep_mqtt_station_async:
                try:
                    ...
                    data = eurdep_radiation_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "eu.jrc.eurdep.mqtt.Station")
                    await self.eu_jrc_eurdep_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in eu_jrc_eurdep_mqtt_station handler: {e}")
            return
        
        if event_type == "eu.jrc.eurdep.DoseRateReading":
            if self.eu_jrc_eurdep_mqtt_dose_rate_reading_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = eurdep_radiation_mqtt_producer_data.DoseRateReading.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "eu.jrc.eurdep.mqtt.DoseRateReading")
                    await self.eu_jrc_eurdep_mqtt_dose_rate_reading_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in eu_jrc_eurdep_mqtt_dose_rate_reading handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_eu_jrc_eurdep_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_eu_jrc_eurdep_mqtt_station(self,
        feedurl: str,
        station_id: str,
        country: str,
        data: eurdep_radiation_mqtt_producer_data.Station,
        ...
        """
        Publish the 'eu.jrc.eurdep.mqtt.Station' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'radiation/intl/eurdep/eurdep-radiation/{country}/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "radiation/intl/eurdep/eurdep-radiation/{country}/{station_id}/info"
    ...
    async def publish_eu_jrc_eurdep_mqtt_dose_rate_reading(self,
        feedurl: str,
        station_id: str,
        country: str,
        data: eurdep_radiation_mqtt_producer_data.DoseRateReading,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'eu.jrc.eurdep.mqtt.DoseRateReading' event to an MQTT topic.
        ...
        Args:
            ...
            country: URI template variable for 'country'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'radiation/intl/eurdep/eurdep-radiation/{country}/{station_id}/dose-rate'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "radiation/intl/eurdep/eurdep-radiation/{country}/{station_id}/dose-rate"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
            "station_id": str(station_id),
            "country": str(country),
