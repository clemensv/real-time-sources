...
import os
import signal
from eurdep_radiation_mqtt_producer_data import *
from eurdep_radiation_mqtt_producer_mqtt_client.client import EuJrcEurdepMqttProducer, EuJrcEurdepMqttDispatcher

async def handle_eu_jrc_eurdep_mqtt_station(mqtt_msg,cloud_event, eu_jrc_eurdep_mqtt_station_data):
    """ Handles the eu.jrc.eurdep.mqtt.Station message """
    print(f"Received eu.jrc.eurdep.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {eu_jrc_eurdep_mqtt_station_data}")

async def handle_eu_jrc_eurdep_mqtt_dose_rate_reading(mqtt_msg,cloud_event, eu_jrc_eurdep_mqtt_dose_rate_reading_data):
    """ Handles the eu.jrc.eurdep.mqtt.DoseRateReading message """
    print(f"Received eu.jrc.eurdep.mqtt.DoseRateReading on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {eu_jrc_eurdep_mqtt_dose_rate_reading_data}")
