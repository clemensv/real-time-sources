...
class FrGouvTransportBisonFuteTrafficFlowAmqpProducer:
    """
    Producer class to send messages in the `fr.gouv.transport.bison_fute.traffic_flow.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_amqp(self,
        ...
        annotations = {}
        annotation_value = "{site_id}".format(site_id=_site_id)
...
class FrGouvTransportBisonFuteRoadEventAmqpProducer:
    """
    Producer class to send messages in the `fr.gouv.transport.bison_fute.road_event.amqp` message group via AMQP 1.0 protocol.
    """
