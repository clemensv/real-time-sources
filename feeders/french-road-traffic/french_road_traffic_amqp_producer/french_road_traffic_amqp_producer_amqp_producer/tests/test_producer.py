...
"""
Tests for french_road_traffic_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../french_road_traffic_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../french_road_traffic_amqp_producer_amqp_producer/src')))
...
class TestFrGouvTransportBisonFuteTrafficFlowAmqpProducer:
    """Test cases for FrGouvTransportBisonFuteTrafficFlowAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(FrGouvTransportBisonFuteTrafficFlowAmqpProducer)
    ...
    def test_send_amqp(self, artemis_container):
        ...
        payload = Test_TrafficFlowMeasurement.create_instance()
...
class TestFrGouvTransportBisonFuteRoadEventAmqpProducer:
    """Test cases for FrGouvTransportBisonFuteRoadEventAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        fake_connection = FakeConnection()
        producer = object.__new__(FrGouvTransportBisonFuteRoadEventAmqpProducer)
        producer._sender = fake_sender
        producer._connection = fake_connection
        producer._blocking_sender_is_presettled = True
