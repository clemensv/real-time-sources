...
def get_default_topic_mappings_fr_gouv_transport_bison_fute_traffic_flow_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the fr.gouv.transport.bison_fute.traffic_flow.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "fr.gouv.transport.bison_fute.TrafficFlowMeasurement.mqtt": "traffic/fr/fr-traffic/french-road-traffic/{road}/{site_id}/flow",
    }


def get_subscription_topics_fr_gouv_transport_bison_fute_traffic_flow_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_fr_gouv_transport_bison_fute_traffic_flow_mqtt()
    topics = []
...
class FrGouvTransportBisonFuteTrafficFlowMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the fr.gouv.transport.bison_fute.traffic_flow.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_fr_gouv_transport_bison_fute_traffic_flow_mqtt()
        ...
        self.fr_gouv_transport_bison_fute_traffic_flow_measurement_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, french_road_traffic_mqtt_producer_data.TrafficFlowMeasurement, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "fr.gouv.transport.bison_fute.TrafficFlowMeasurement":
            if self.fr_gouv_transport_bison_fute_traffic_flow_measurement_mqtt_async:
                try:
                    ...
                    data = french_road_traffic_mqtt_producer_data.TrafficFlowMeasurement.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fr.gouv.transport.bison_fute.TrafficFlowMeasurement.mqtt")
                    await self.fr_gouv_transport_bison_fute_traffic_flow_measurement_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fr_gouv_transport_bison_fute_traffic_flow_measurement_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_fr_gouv_transport_bison_fute_traffic_flow_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_fr_gouv_transport_bison_fute_traffic_flow_measurement_mqtt(self,
        feedurl: str,
        site_id: str,
        road: str,
        data: french_road_traffic_mqtt_producer_data.TrafficFlowMeasurement,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'fr.gouv.transport.bison_fute.TrafficFlowMeasurement.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'traffic/fr/fr-traffic/french-road-traffic/{road}/{site_id}/flow'
        ...
        """
        target_topic = topic if topic is not None else "traffic/fr/fr-traffic/french-road-traffic/{road}/{site_id}/flow"
...
def get_default_topic_mappings_fr_gouv_transport_bison_fute_road_event_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the fr.gouv.transport.bison_fute.road_event.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "fr.gouv.transport.bison_fute.RoadEvent.mqtt": "traffic/fr/fr-traffic/french-road-traffic/situations/{road}/{situation_id}/event",
    }


def get_subscription_topics_fr_gouv_transport_bison_fute_road_event_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_fr_gouv_transport_bison_fute_road_event_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class FrGouvTransportBisonFuteRoadEventMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the fr.gouv.transport.bison_fute.road_event.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_fr_gouv_transport_bison_fute_road_event_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        
        # Message handler callbacks (Dispatcher pattern)
        
        self.fr_gouv_transport_bison_fute_road_event_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, french_road_traffic_mqtt_producer_data.RoadEvent, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "fr.gouv.transport.bison_fute.RoadEvent":
            if self.fr_gouv_transport_bison_fute_road_event_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = french_road_traffic_mqtt_producer_data.RoadEvent.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fr.gouv.transport.bison_fute.RoadEvent.mqtt")
                    await self.fr_gouv_transport_bison_fute_road_event_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fr_gouv_transport_bison_fute_road_event_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_fr_gouv_transport_bison_fute_road_event_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_fr_gouv_transport_bison_fute_road_event_mqtt(self,
        feedurl: str,
        situation_id: str,
        road: str,
        data: french_road_traffic_mqtt_producer_data.RoadEvent,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'fr.gouv.transport.bison_fute.RoadEvent.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            road: URI template variable for 'road'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/fr/fr-traffic/french-road-traffic/situations/{road}/{situation_id}/event'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "traffic/fr/fr-traffic/french-road-traffic/situations/{road}/{situation_id}/event"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
