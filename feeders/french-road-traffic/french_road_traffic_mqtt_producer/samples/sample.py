...
import os
import signal
from french_road_traffic_mqtt_producer_data import *
from french_road_traffic_mqtt_producer_mqtt_client.client import FrGouvTransportBisonFuteTrafficFlowMqttProducer, FrGouvTransportBisonFuteTrafficFlowMqttDispatcher

async def handle_fr_gouv_transport_bison_fute_traffic_flow_measurement_mqtt(mqtt_msg,cloud_event, fr_gouv_transport_bison_fute_traffic_flow_measurement_mqtt_data):
    """ Handles the fr.gouv.transport.bison_fute.TrafficFlowMeasurement.mqtt message """
    print(f"Received fr.gouv.transport.bison_fute.TrafficFlowMeasurement.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {fr_gouv_transport_bison_fute_traffic_flow_measurement_mqtt_data}")
from french_road_traffic_mqtt_producer_mqtt_client.client import FrGouvTransportBisonFuteRoadEventMqttProducer, FrGouvTransportBisonFuteRoadEventMqttDispatcher

async def handle_fr_gouv_transport_bison_fute_road_event_mqtt(mqtt_msg,cloud_event, fr_gouv_transport_bison_fute_road_event_mqtt_data):
    """ Handles the fr.gouv.transport.bison_fute.RoadEvent.mqtt message """
    print(f"Received fr.gouv.transport.bison_fute.RoadEvent.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {fr_gouv_transport_bison_fute_road_event_mqtt_data}")
