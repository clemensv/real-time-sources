...
class IeGovOpwWaterlevelAmqpProducer:
    """
    Producer class to send messages in the `ie.gov.opw.waterlevel.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        ...
        """
        Send the `ie.gov.opw.waterlevel.amqp.Station` message
        A reference record for one Irish hydrometric station published by Ireland's Office of Public Works (OPW). It fires when the bridge publishes or refreshes the station catalog so consumers can interpret measurement events.
        ...
        Args:
            ...
            _basin (str): Value for AMQP protocol option placeholder basin
        ...
        attributes = {
            ...
            "subject":
            "{station_ref}".format(station_ref=_station_ref),
        }
        ...
        amqp_msg.subject = "{station_ref}".format(station_ref=_station_ref)

        app_properties = {}
        app_properties["basin"] = "{basin}".format(basin=_basin)
        if app_properties:
        ...
        annotations = {}
        annotation_value = "{station_ref}".format(station_ref=_station_ref)
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `ie.gov.opw.waterlevel.amqp.Station` messages
    ...
    def send_water_level_reading(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `ie.gov.opw.waterlevel.amqp.WaterLevelReading` message
        A current measurement from Ireland's Office of Public Works (OPW) for one monitoring site. It carries water level, temperature, and sensor voltage observations when the upstream feed reports a new or refreshed value.
    ...
    def send_water_level_reading_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `ie.gov.opw.waterlevel.amqp.WaterLevelReading` messages
