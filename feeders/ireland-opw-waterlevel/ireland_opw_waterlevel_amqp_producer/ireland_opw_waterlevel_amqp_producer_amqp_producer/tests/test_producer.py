...
"""
Tests for ireland_opw_waterlevel_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../ireland_opw_waterlevel_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../ireland_opw_waterlevel_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../ireland_opw_waterlevel_amqp_producer_amqp_producer/src')))

from ireland_opw_waterlevel_amqp_producer_amqp_producer import *
from ireland_opw_waterlevel_amqp_producer_data import Station
from test_station import Test_Station
from ireland_opw_waterlevel_amqp_producer_data import WaterLevelReading
...
class TestIeGovOpwWaterlevelAmqpProducer:
    """Test cases for IeGovOpwWaterlevelAmqpProducer"""
    
    def test_producer_initialization(self, artemis_container):
        """Test that producer initializes correctly"""
        producer = IeGovOpwWaterlevelAmqpProducer(
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(IeGovOpwWaterlevelAmqpProducer)
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "ie.gov.opw.waterlevel.amqp.Station"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{station_ref}".format(station_ref="value")
                assert properties.get('basin') == "{basin}".format(basin="value")
                assert annotations.get(symbol('x-opt-partition-key')) == str("{station_ref}".format(station_ref="value"))[:128]
        finally:
            producer.close()
    ...
    def test_send_station_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'ie.gov.opw.waterlevel.amqp.Station'
        assert received.body is not None
        assert received.subject == "{station_ref}".format(station_ref="value")
        assert properties.get('basin') == "{basin}".format(basin="value")
        assert annotations.get(symbol('x-opt-partition-key')) == str("{station_ref}".format(station_ref="value"))[:128]
    
    def test_send_water_level_reading(self, artemis_container):
        """Send and receive a WaterLevelReading message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_WaterLevelReading.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "ie.gov.opw.waterlevel.amqp.WaterLevelReading"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_water_level_reading_single_fresh_connection(self, artemis_container):
        """Send exactly one WaterLevelReading message on a fresh producer connection."""
        payload = Test_WaterLevelReading.create_instance()
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'ie.gov.opw.waterlevel.amqp.WaterLevelReading'
        assert received.body is not None
        assert received.subject == "{station_ref}".format(station_ref="value")
        assert properties.get('basin') == "{basin}".format(basin="value")
        assert annotations.get(symbol('x-opt-partition-key')) == str("{station_ref}".format(station_ref="value"))[:128]
