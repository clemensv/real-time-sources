"""Ireland OPW Water Level -> AMQP 1.0 bridge.

Polls the Ireland OPW Water Level REST API and publishes events as CloudEvents over AMQP 1.0.
"""
...
from ireland_opw_waterlevel.ireland_opw_waterlevel import (
    fetch_geojson, extract_stations, extract_readings, dedup_key,
    _load_state, _save_state
)
from ireland_opw_waterlevel_amqp_producer_data import Station, WaterLevelReading
from ireland_opw_waterlevel_amqp_producer_amqp_producer.producer import IeGovOpwWaterlevelAmqpProducer
...
def _build_producer(host, port, address, tls, content_mode, auth_mode, username, password,
    ...
    if auth_mode == 'entra':
        ...
        return IeGovOpwWaterlevelAmqpProducer(
...
USER_AGENT = "ireland-opw-waterlevel-amqp/1.0"


def feed(host, port, address='ireland-opw-waterlevel', username=None, password=None, tls=False,
    ...
    """Poll Ireland OPW waterlevel.ie GeoJSON and emit station + reading events via AMQP."""
    ...
    seen_keys: Set[str] = set(previous.get("seen_keys", []))
    ...
    try:
        while True:
            try:
                ...
                # Emit stations
                for ref, info in stations.items():
                    basin = info.get("basin", "unknown") or "unknown"
                    station = Station(
                        station_ref=ref,
                    ...
                    producer.send_station(data=station, _feedurl=FEED_URL, _station_ref=ref, _basin=basin)
                logger.info("Published %d stations", len(stations))
                ...
                for r in readings:
                    ...
                    if key in seen_keys:
                        continue
                    station_ref = r.get("station_ref", "")
                    basin = stations.get(station_ref, {}).get("basin", "unknown") or "unknown"
                ...
                if len(seen_keys) > 50000:
                    seen_keys = set(sorted(seen_keys)[-25000:])
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(description='Ireland OPW Water Level AMQP 1.0 bridge')
    ...
    fp.add_argument('--address', default=os.getenv('AMQP_ADDRESS', 'ireland-opw-waterlevel'))
    ...
    fp.add_argument('--state-file', default=os.getenv('STATE_FILE', os.path.expanduser('~/.ireland_opw_waterlevel_amqp_state.json')))
