...
def get_default_topic_mappings_ie_gov_opw_waterlevel_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the ie.gov.opw.waterlevel.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "ie.gov.opw.waterlevel.mqtt.Station": "hydro/ie/opw/ireland-opw-waterlevel/{basin}/{station_ref}/info",
        "ie.gov.opw.waterlevel.mqtt.WaterLevelReading": "hydro/ie/opw/ireland-opw-waterlevel/{basin}/{station_ref}/water-level",
    }


def get_subscription_topics_ie_gov_opw_waterlevel_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_ie_gov_opw_waterlevel_mqtt()
    topics = []
...
class IeGovOpwWaterlevelMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the ie.gov.opw.waterlevel.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_ie_gov_opw_waterlevel_mqtt()
        ...
        self.ie_gov_opw_waterlevel_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, ireland_opw_waterlevel_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.ie_gov_opw_waterlevel_mqtt_water_level_reading_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, ireland_opw_waterlevel_mqtt_producer_data.WaterLevelReading, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "ie.gov.opw.waterlevel.Station":
            if self.ie_gov_opw_waterlevel_mqtt_station_async:
                try:
                    ...
                    data = ireland_opw_waterlevel_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "ie.gov.opw.waterlevel.mqtt.Station")
                    await self.ie_gov_opw_waterlevel_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in ie_gov_opw_waterlevel_mqtt_station handler: {e}")
            return
        
        if event_type == "ie.gov.opw.waterlevel.WaterLevelReading":
            if self.ie_gov_opw_waterlevel_mqtt_water_level_reading_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = ireland_opw_waterlevel_mqtt_producer_data.WaterLevelReading.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "ie.gov.opw.waterlevel.mqtt.WaterLevelReading")
                    await self.ie_gov_opw_waterlevel_mqtt_water_level_reading_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in ie_gov_opw_waterlevel_mqtt_water_level_reading handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_ie_gov_opw_waterlevel_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_ie_gov_opw_waterlevel_mqtt_station(self,
        feedurl: str,
        station_ref: str,
        basin: str,
        data: ireland_opw_waterlevel_mqtt_producer_data.Station,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'ie.gov.opw.waterlevel.mqtt.Station' event to an MQTT topic.

        Args:
        
            feedurl: URI template variable for 'feedurl'
            station_ref: URI template variable for 'station_ref'
            basin: URI template variable for 'basin'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/ie/opw/ireland-opw-waterlevel/{basin}/{station_ref}/info'
        ...
        """
        target_topic = topic if topic is not None else "hydro/ie/opw/ireland-opw-waterlevel/{basin}/{station_ref}/info"
        ...
        attributes = {
             ...
             "subject":"{station_ref}".format(station_ref = station_ref)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_ie_gov_opw_waterlevel_mqtt_water_level_reading(self,
        feedurl: str,
        station_ref: str,
        basin: str,
        data: ireland_opw_waterlevel_mqtt_producer_data.WaterLevelReading,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'ie.gov.opw.waterlevel.mqtt.WaterLevelReading' event to an MQTT topic.
        ...
        Args:
            ...
            basin: URI template variable for 'basin'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/ie/opw/ireland-opw-waterlevel/{basin}/{station_ref}/water-level'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "hydro/ie/opw/ireland-opw-waterlevel/{basin}/{station_ref}/water-level"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
            "station_ref": str(station_ref),
            "basin": str(basin),
