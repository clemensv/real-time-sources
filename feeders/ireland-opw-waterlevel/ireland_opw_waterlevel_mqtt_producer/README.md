# Ireland_opw_waterlevel_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the ireland_opw_waterlevel_mqtt_producer message definitions.

# Ireland_opw_waterlevel_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - IeGovOpwWaterlevelMqttEventDispatcher
...
from ireland_opw_waterlevel_mqtt_producer_mqtt_client import *This module defines an event processing framework for
Azure Event Hubs,
...
client = IeGovOpwWaterlevelMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### IeGovOpwWaterlevelMqttEventDispatcher



# Publish message`IeGovOpwWaterlevelMqttEventDispatcher` handles events for the ie.gov.opw.waterlevel.mqtt message
group.
...
from ireland_opw_waterlevel_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = IeGovOpwWaterlevelMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe IeGovOpwWaterlevelMqttEventDispatcher defines the following event handler hooks.
...
```##### `ie_gov_opw_waterlevel_mqtt_station_async`



## Test```python

ie_gov_opw_waterlevel_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `ie.gov.opw.waterlevel.mqtt.Station`: A reference record for one Irish hydrometric
station published by Ireland's Office of Public Works (OPW). It fires when the bridge publishes or refreshes the station
catalog so consumers can interpret measurement events.
...
- `data`: The event data of type `ireland_opw_waterlevel_mqtt_producer_data.Station`.

Example:

```python
async def ie_gov_opw_waterlevel_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `ie_gov_opw_waterlevel_mqtt_water_level_reading_async`



## Test```python

ie_gov_opw_waterlevel_mqtt_water_level_reading_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ireland_opw_waterlevel_mqtt_producer_data.WaterLevelReading`.

Example:

```python
async def ie_gov_opw_waterlevel_mqtt_water_level_reading_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: WaterLevelReading) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
