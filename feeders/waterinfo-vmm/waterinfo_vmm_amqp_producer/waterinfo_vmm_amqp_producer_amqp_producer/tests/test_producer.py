...
"""
Tests for waterinfo_vmm_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../waterinfo_vmm_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../waterinfo_vmm_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../waterinfo_vmm_amqp_producer_amqp_producer/src')))

from waterinfo_vmm_amqp_producer_amqp_producer import *
...
class TestBEVlaanderenWaterinfoVMMAmqpProducer:
    """Test cases for BEVlaanderenWaterinfoVMMAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(BEVlaanderenWaterinfoVMMAmqpProducer)
    ...
    def test_send_station(self, artemis_container):
        ...
        try:
            ...
            for i in range(5):
                ...
                assert annotations.get(symbol('x-opt-partition-key')) == str("{station_no}".format(station_no="value"))[:128]
        finally:
            producer.close()
