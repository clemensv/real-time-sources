    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from waterinfo_vmm_amqp_producer_data import Station
from waterinfo_vmm_amqp_producer_data import WaterLevelReading

class BEVlaanderenWaterinfoVMMAmqpProducer:
    """
    Producer class to send messages in the `BE.Vlaanderen.Waterinfo.VMM.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        ...
        """
        Send the `BE.Vlaanderen.Waterinfo.VMM.amqp.Station` message
        A reference record for one Flemish water monitoring location published by Waterinfo.be and the Flemish Environment Agency (VMM). It fires when the bridge publishes or refreshes the station catalog so consumers can interpret measurement events.
        
        Args:
            _station_no (str): Value for placeholder station_no in attribute subject
            _water_body (str): Value for AMQP protocol option placeholder water_body
        ...
        attributes = {
            ...
            "source":
            "https://waterinfo.vlaanderen.be",
            "subject":
            "{station_no}".format(station_no=_station_no),
        }
        ...
        amqp_msg.subject = "{station_no}".format(station_no=_station_no)

        app_properties = {}
        app_properties["water_body"] = "{water_body}".format(water_body=_water_body)
        if app_properties:
        ...
        annotations = {}
        annotation_value = "{station_no}".format(station_no=_station_no)
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `BE.Vlaanderen.Waterinfo.VMM.amqp.Station` messages
        ...
        for data in data_array:
            self.send_station(
                data=data,
                _station_no=_station_no,
                _time=_time,
                _water_body=_water_body,
                content_type=content_type)
    
    
    def send_water_level_reading(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `BE.Vlaanderen.Waterinfo.VMM.amqp.WaterLevelReading` message
        A current measurement from Waterinfo.be and the Flemish Environment Agency (VMM) for one monitoring site. It carries water level observations when the upstream feed reports a new or refreshed value.
        
        Args:
            _station_no (str): Value for placeholder station_no in attribute subject
            _water_body (str): Value for AMQP protocol option placeholder water_body
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            data (WaterLevelReading): The message data object
            content_type (str): The content type of the message data (default: 'application/json')
        """
        # Build CloudEvent attributes
        attributes = {
    ...
    def send_water_level_reading_batch(self,
        data_array: typing.List[WaterLevelReading],
        _station_no: str,
        _water_body: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `BE.Vlaanderen.Waterinfo.VMM.amqp.WaterLevelReading` messages
        
        Args:
            data_array (typing.List[WaterLevelReading]): Array of message data objects
            _station_no (str): Value for placeholder station_no in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            _water_body (str): Value for AMQP protocol option placeholder water_body
            content_type (str): The content type of the message data
