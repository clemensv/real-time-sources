...
import os
import signal
from waterinfo_vmm_mqtt_producer_data import *
from waterinfo_vmm_mqtt_producer_mqtt_client.client import BEVlaanderenWaterinfoVMMMqttProducer, BEVlaanderenWaterinfoVMMMqttDispatcher

async def handle_be_vlaanderen_waterinfo_vmm_mqtt_station(mqtt_msg,cloud_event, be_vlaanderen_waterinfo_vmm_mqtt_station_data):
    """ Handles the BE.Vlaanderen.Waterinfo.VMM.mqtt.Station message """
    print(f"Received BE.Vlaanderen.Waterinfo.VMM.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {be_vlaanderen_waterinfo_vmm_mqtt_station_data}")

async def handle_be_vlaanderen_waterinfo_vmm_mqtt_water_level_reading(mqtt_msg,cloud_event, be_vlaanderen_waterinfo_vmm_mqtt_water_level_reading_data):
    """ Handles the BE.Vlaanderen.Waterinfo.VMM.mqtt.WaterLevelReading message """
    print(f"Received BE.Vlaanderen.Waterinfo.VMM.mqtt.WaterLevelReading on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {be_vlaanderen_waterinfo_vmm_mqtt_water_level_reading_data}")
