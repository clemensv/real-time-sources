...
import waterinfo_vmm_mqtt_producer_data
from waterinfo_vmm_mqtt_producer_data import Station
from waterinfo_vmm_mqtt_producer_data import WaterLevelReading
...
def get_default_topic_mappings_be_vlaanderen_waterinfo_vmm_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the BE.Vlaanderen.Waterinfo.VMM.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "BE.Vlaanderen.Waterinfo.VMM.mqtt.Station": "hydro/be/vmm/waterinfo-vmm/{water_body}/{station_no}/info",
        "BE.Vlaanderen.Waterinfo.VMM.mqtt.WaterLevelReading": "hydro/be/vmm/waterinfo-vmm/{water_body}/{station_no}/water-level",
    }


def get_subscription_topics_be_vlaanderen_waterinfo_vmm_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_be_vlaanderen_waterinfo_vmm_mqtt()
    topics = []
...
class BEVlaanderenWaterinfoVMMMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the BE.Vlaanderen.Waterinfo.VMM.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_be_vlaanderen_waterinfo_vmm_mqtt()
        ...
        self.be_vlaanderen_waterinfo_vmm_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, waterinfo_vmm_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.be_vlaanderen_waterinfo_vmm_mqtt_water_level_reading_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, waterinfo_vmm_mqtt_producer_data.WaterLevelReading, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "BE.Vlaanderen.Waterinfo.VMM.Station":
            if self.be_vlaanderen_waterinfo_vmm_mqtt_station_async:
                try:
                    ...
                    data = waterinfo_vmm_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "BE.Vlaanderen.Waterinfo.VMM.mqtt.Station")
                    await self.be_vlaanderen_waterinfo_vmm_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in be_vlaanderen_waterinfo_vmm_mqtt_station handler: {e}")
            return
        
        if event_type == "BE.Vlaanderen.Waterinfo.VMM.WaterLevelReading":
            if self.be_vlaanderen_waterinfo_vmm_mqtt_water_level_reading_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = waterinfo_vmm_mqtt_producer_data.WaterLevelReading.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "BE.Vlaanderen.Waterinfo.VMM.mqtt.WaterLevelReading")
                    await self.be_vlaanderen_waterinfo_vmm_mqtt_water_level_reading_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in be_vlaanderen_waterinfo_vmm_mqtt_water_level_reading handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_be_vlaanderen_waterinfo_vmm_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_be_vlaanderen_waterinfo_vmm_mqtt_station(self,
        station_no: str,
        water_body: str,
        data: waterinfo_vmm_mqtt_producer_data.Station,
        ...
        """
        Publish the 'BE.Vlaanderen.Waterinfo.VMM.mqtt.Station' event to an MQTT topic.

        Args:
        
            station_no: URI template variable for 'station_no'
            water_body: URI template variable for 'water_body'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/be/vmm/waterinfo-vmm/{water_body}/{station_no}/info'
        ...
        """
        target_topic = topic if topic is not None else "hydro/be/vmm/waterinfo-vmm/{water_body}/{station_no}/info"
        ...
        attributes = {
             ...
             "subject":"{station_no}".format(station_no = station_no)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_be_vlaanderen_waterinfo_vmm_mqtt_water_level_reading(self,
        station_no: str,
        water_body: str,
        data: waterinfo_vmm_mqtt_producer_data.WaterLevelReading,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'BE.Vlaanderen.Waterinfo.VMM.mqtt.WaterLevelReading' event to an MQTT topic.
        ...
        Args:
            ...
            water_body: URI template variable for 'water_body'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/be/vmm/waterinfo-vmm/{water_body}/{station_no}/water-level'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "hydro/be/vmm/waterinfo-vmm/{water_body}/{station_no}/water-level"
        _topic_template_values: Dict[str, str] = {
            "station_no": str(station_no),
            "water_body": str(water_body),
        }
