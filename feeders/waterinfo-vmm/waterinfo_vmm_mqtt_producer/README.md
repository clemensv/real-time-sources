# Waterinfo_vmm_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the waterinfo_vmm_mqtt_producer message definitions.

# Waterinfo_vmm_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - BEVlaanderenWaterinfoVMMMqttEventDispatcher
...
from waterinfo_vmm_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = BEVlaanderenWaterinfoVMMMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### BEVlaanderenWaterinfoVMMMqttEventDispatcher



# Publish message`BEVlaanderenWaterinfoVMMMqttEventDispatcher` handles events for the BE.Vlaanderen.Waterinfo.VMM.mqtt
message group.
...
from waterinfo_vmm_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = BEVlaanderenWaterinfoVMMMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of
...
## BuildThe BEVlaanderenWaterinfoVMMMqttEventDispatcher defines the following event handler hooks.
...
```##### `be_vlaanderen_waterinfo_vmm_mqtt_station_async`



## Test```python

be_vlaanderen_waterinfo_vmm_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station],
Awaitable[None]]
...
- `data`: The event data of type `waterinfo_vmm_mqtt_producer_data.Station`.

Example:

```python
async def be_vlaanderen_waterinfo_vmm_mqtt_station_event(partition_context: PartitionContext, event: EventData,
...
```##### `be_vlaanderen_waterinfo_vmm_mqtt_water_level_reading_async`



## Test```python

be_vlaanderen_waterinfo_vmm_mqtt_water_level_reading_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `waterinfo_vmm_mqtt_producer_data.WaterLevelReading`.

Example:

```python
async def be_vlaanderen_waterinfo_vmm_mqtt_water_level_reading_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: WaterLevelReading) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
