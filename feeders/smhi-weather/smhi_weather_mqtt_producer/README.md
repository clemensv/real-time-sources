# Smhi_weather_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the smhi_weather_mqtt_producer message definitions.

# Smhi_weather_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - SEGovSMHIWeatherMqttEventDispatcher
...
from smhi_weather_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = SEGovSMHIWeatherMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### SEGovSMHIWeatherMqttEventDispatcher



# Publish message`SEGovSMHIWeatherMqttEventDispatcher` handles events for the SE.Gov.SMHI.Weather.mqtt message group.
...
from smhi_weather_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = SEGovSMHIWeatherMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe SEGovSMHIWeatherMqttEventDispatcher defines the following event handler hooks.
...
```##### `se_gov_smhi_weather_mqtt_station_async`



## Test```python

se_gov_smhi_weather_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `smhi_weather_mqtt_producer_data.Station`.

Example:

```python
async def se_gov_smhi_weather_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `se_gov_smhi_weather_mqtt_weather_observation_async`



## Test```python

se_gov_smhi_weather_mqtt_weather_observation_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `smhi_weather_mqtt_producer_data.WeatherObservation`.

Example:

```python
async def se_gov_smhi_weather_mqtt_weather_observation_event(partition_context: PartitionContext, event: EventData,
