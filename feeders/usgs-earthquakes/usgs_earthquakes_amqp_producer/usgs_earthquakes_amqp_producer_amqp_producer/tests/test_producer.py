...
"""
Tests for usgs_earthquakes_amqp_producer_amqp_producer
"""
import base64
import json
import os
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../usgs_earthquakes_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../usgs_earthquakes_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../usgs_earthquakes_amqp_producer_amqp_producer/src')))

from usgs_earthquakes_amqp_producer_amqp_producer import *
from usgs_earthquakes_amqp_producer_data import Event
from test_usgs_earthquakes_amqp_producer_data_event import Test_Event
...
class TestUSGSEarthquakesAmqpProducer:
    """Test cases for USGSEarthquakesAmqpProducer"""
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "USGS.Earthquakes.amqp.Event"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{net}/{code}".format(net="value", code="value")
        finally:
            producer.close()
