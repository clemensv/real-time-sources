...
import usgs_earthquakes_mqtt_producer_data
from usgs_earthquakes_mqtt_producer_data import Event
...
def get_default_topic_mappings_usgs_earthquakes_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the USGS.Earthquakes.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "USGS.Earthquakes.mqtt.Event": "seismic/intl/usgs/usgs-earthquakes/{net}/{magnitude_bucket}/{code}/quake",
    }


def get_subscription_topics_usgs_earthquakes_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_usgs_earthquakes_mqtt()
    topics = []
...
class USGSEarthquakesMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the USGS.Earthquakes.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_usgs_earthquakes_mqtt()
        ...
        self.usgs_earthquakes_mqtt_event_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, usgs_earthquakes_mqtt_producer_data.Event, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "USGS.Earthquakes.Event":
            if self.usgs_earthquakes_mqtt_event_async:
                try:
                    ...
                    data = usgs_earthquakes_mqtt_producer_data.Event.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "USGS.Earthquakes.mqtt.Event")
                    await self.usgs_earthquakes_mqtt_event_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in usgs_earthquakes_mqtt_event handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_usgs_earthquakes_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_usgs_earthquakes_mqtt_event(self,
        source_uri: str,
        net: str,
        code: str,
        event_time: str,
        magnitude_bucket: str,
        data: usgs_earthquakes_mqtt_producer_data.Event,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'USGS.Earthquakes.mqtt.Event' event to an MQTT topic.

        Args:
        
            source_uri: URI template variable for 'source_uri'
            net: URI template variable for 'net'
            code: URI template variable for 'code'
            ...
            topic: Optional topic override. If not provided, uses default topic 'seismic/intl/usgs/usgs-earthquakes/{net}/{magnitude_bucket}/{code}/quake'
        ...
        """
        target_topic = topic if topic is not None else "seismic/intl/usgs/usgs-earthquakes/{net}/{magnitude_bucket}/{code}/quake"
        ...
        attributes = {
             ...
             "subject":"{net}/{code}".format(net = net,code = code),
