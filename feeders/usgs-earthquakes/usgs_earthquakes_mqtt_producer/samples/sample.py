...
import os
import signal
from usgs_earthquakes_mqtt_producer_data import *
from usgs_earthquakes_mqtt_producer_mqtt_client.client import USGSEarthquakesMqttProducer, USGSEarthquakesMqttDispatcher

async def handle_usgs_earthquakes_mqtt_event(mqtt_msg,cloud_event, usgs_earthquakes_mqtt_event_data):
    """ Handles the USGS.Earthquakes.mqtt.Event message """
    print(f"Received USGS.Earthquakes.mqtt.Event on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {usgs_earthquakes_mqtt_event_data}")
