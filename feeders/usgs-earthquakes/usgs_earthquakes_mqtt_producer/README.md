# Usgs_earthquakes_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the usgs_earthquakes_mqtt_producer message definitions.

# Usgs_earthquakes_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - USGSEarthquakesMqttEventDispatcher
...
from usgs_earthquakes_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = USGSEarthquakesMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### USGSEarthquakesMqttEventDispatcher



# Publish message`USGSEarthquakesMqttEventDispatcher` handles events for the USGS.Earthquakes.mqtt message group.
...
from usgs_earthquakes_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = USGSEarthquakesMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe USGSEarthquakesMqttEventDispatcher defines the following event handler hooks.
...
```##### `usgs_earthquakes_mqtt_event_async`



## Test```python

usgs_earthquakes_mqtt_event_async:  Callable[[PartitionContext, EventData, CloudEvent, Event], Awaitable[None]]
...
- `data`: The event data of type `usgs_earthquakes_mqtt_producer_data.Event`.

Example:

```python
async def usgs_earthquakes_mqtt_event_event(partition_context: PartitionContext, event: EventData, cloud_event:
