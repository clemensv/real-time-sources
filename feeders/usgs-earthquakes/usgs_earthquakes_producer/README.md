...
# Usgs-earthquakes-producer Kafka Producer# Usgs-earthquakes-producer Event Dispatcher for Apache Kafka
...
- USGSEarthquakesProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### USGSEarthquakesEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`USGSEarthquakesEventDispatcher` handles events for the USGS.Earthquakes message group.
...
The USGSEarthquakesEventDispatcher defines the following event handler hooks.
...
producer = USGSEarthquakesProducer(
    ...
    security_protocol='SASL_SSL',##### `usgs_earthquakes_event_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'usgs_earthquakes_event_async:  Callable[[ConsumerRecord, CloudEvent, Event],
Awaitable[None]]
...
Asynchronous handler hook for `USGS.Earthquakes.Event`: USGS earthquake event data from the Earthquake Hazards Program.
...
usgs_earthquakes_dispatcher.usgs_earthquakes_event_async = usgs_earthquakes_event_event
...
##### `send_usgs_earthquakes_event`Dispatchers have the following protected methods:



```python### Methods:

async def send_usgs_earthquakes_event(
...
Send a single `USGS.Earthquakes.Event` message. USGS earthquake event data from the Earthquake Hazards Program.Args:
...
await producer.send_usgs_earthquakes_event(
...
Send multiple `USGS.Earthquakes.Event` messages in a batch.
...
await producer.send_usgs_earthquakes_event_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### USGSEarthquakesMqttEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`USGSEarthquakesMqttEventDispatcher` handles events for the USGS.Earthquakes.mqtt message group.
...
The USGSEarthquakesMqttEventDispatcher defines the following event handler hooks.
...
producer = USGSEarthquakesProducer(
    ...
    security_protocol='SASL_SSL',##### `usgs_earthquakes_mqtt_event_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'usgs_earthquakes_mqtt_event_async:  Callable[[ConsumerRecord, CloudEvent, Event],
Awaitable[None]]
...
Asynchronous handler hook for `USGS.Earthquakes.mqtt.Event`: USGS earthquake event data from the Earthquake Hazards
Program.
...
usgs_earthquakes_dispatcher.usgs_earthquakes_mqtt_event_async = usgs_earthquakes_mqtt_event_event
...
usgs_earthquakes_mqtt_dispatcher.usgs_earthquakes_mqtt_event_async = usgs_earthquakes_mqtt_event_event
...
##### `send_usgs_earthquakes_mqtt_event`Dispatchers have the following protected methods:



```python### Methods:

async def send_usgs_earthquakes_mqtt_event(
...
Send a single `USGS.Earthquakes.mqtt.Event` message. USGS earthquake event data from the Earthquake Hazards
Program.Args:
...
await producer.send_usgs_earthquakes_mqtt_event(
...
Send multiple `USGS.Earthquakes.mqtt.Event` messages in a batch.
