# Open_charge_map_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the open_charge_map_mqtt_producer message definitions.

# Open_charge_map_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - IOOpenChargeMapLocationsMqttEventDispatcher,

    IOOpenChargeMapReferenceMqttEventDispatcher
...
from open_charge_map_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = IOOpenChargeMapLocationsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### IOOpenChargeMapLocationsMqttEventDispatcher



# Publish message`IOOpenChargeMapLocationsMqttEventDispatcher` handles events for the IO.OpenChargeMap.locations.mqtt
message group.
...
from open_charge_map_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = IOOpenChargeMapLocationsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of
...
client = IOOpenChargeMapReferenceMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of
...
## BuildThe IOOpenChargeMapLocationsMqttEventDispatcher defines the following event handler hooks.
...
```##### `io_open_charge_map_mqtt_charging_location_async`



## Test```python

io_open_charge_map_mqtt_charging_location_async:  Callable[[PartitionContext, EventData, CloudEvent, ChargingLocation],
Awaitable[None]]
...
```Asynchronous handler hook for `IO.OpenChargeMap.mqtt.ChargingLocation`: Near-real-time charging-location record for
one Open Charge Map point of interest, keyed by the stable OCM POI id. Emitted at startup for the configured scope and
...
- `data`: The event data of type `open_charge_map_mqtt_producer_data.ChargingLocation`.

Example:

```python
async def io_open_charge_map_mqtt_charging_location_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: ChargingLocation) -> None:
...
client = IOOpenChargeMapReferenceMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### IOOpenChargeMapReferenceMqttEventDispatcher



# Publish message`IOOpenChargeMapReferenceMqttEventDispatcher` handles events for the IO.OpenChargeMap.reference.mqtt
message group.
...
## BuildThe IOOpenChargeMapReferenceMqttEventDispatcher defines the following event handler hooks.
...
```##### `io_open_charge_map_mqtt_operator_async`



## Test```python

io_open_charge_map_mqtt_operator_async:  Callable[[PartitionContext, EventData, CloudEvent, Operator], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `IO.OpenChargeMap.mqtt.Operator`: Open Charge Map Operator reference catalog entity,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `open_charge_map_mqtt_producer_data.Operator`.

Example:

```python
async def io_open_charge_map_mqtt_operator_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Operator) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `io_open_charge_map_mqtt_connection_type_async`



## Test```python

io_open_charge_map_mqtt_connection_type_async:  Callable[[PartitionContext, EventData, CloudEvent, ConnectionType],
Awaitable[None]]
...
```Asynchronous handler hook for `IO.OpenChargeMap.mqtt.ConnectionType`: Open Charge Map ConnectionType reference
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `open_charge_map_mqtt_producer_data.ConnectionType`.

Example:

```python
async def io_open_charge_map_mqtt_connection_type_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: ConnectionType) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `io_open_charge_map_mqtt_current_type_async`



## Test```python

io_open_charge_map_mqtt_current_type_async:  Callable[[PartitionContext, EventData, CloudEvent, CurrentType],
Awaitable[None]]
...
```Asynchronous handler hook for `IO.OpenChargeMap.mqtt.CurrentType`: Open Charge Map CurrentType reference catalog
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `open_charge_map_mqtt_producer_data.CurrentType`.

Example:

```python
async def io_open_charge_map_mqtt_current_type_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: CurrentType) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `io_open_charge_map_mqtt_charger_type_async`



## Test```python

io_open_charge_map_mqtt_charger_type_async:  Callable[[PartitionContext, EventData, CloudEvent, ChargerType],
Awaitable[None]]
...
```Asynchronous handler hook for `IO.OpenChargeMap.mqtt.ChargerType`: Open Charge Map ChargerType reference catalog
entity, keyed by the composite `{reference_type}/{reference_id}` so all lookup entities share one identity shape on the
reference topic. Emitted at startup and on the periodic reference refresh.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `open_charge_map_mqtt_producer_data.ChargerType`.

Example:

```python
async def io_open_charge_map_mqtt_charger_type_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: ChargerType) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `io_open_charge_map_mqtt_country_async`



## Test```python

io_open_charge_map_mqtt_country_async:  Callable[[PartitionContext, EventData, CloudEvent, Country], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `IO.OpenChargeMap.mqtt.Country`: Open Charge Map Country reference catalog entity,
keyed by the composite `{reference_type}/{reference_id}` so all lookup entities share one identity shape on the
reference topic. Emitted at startup and on the periodic reference refresh.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `open_charge_map_mqtt_producer_data.Country`.

Example:

```python
async def io_open_charge_map_mqtt_country_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Country) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `io_open_charge_map_mqtt_data_provider_async`



## Test```python

io_open_charge_map_mqtt_data_provider_async:  Callable[[PartitionContext, EventData, CloudEvent, DataProvider],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `open_charge_map_mqtt_producer_data.DataProvider`.

Example:

```python
async def io_open_charge_map_mqtt_data_provider_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: DataProvider) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `io_open_charge_map_mqtt_status_type_async`



## Test```python

io_open_charge_map_mqtt_status_type_async:  Callable[[PartitionContext, EventData, CloudEvent, StatusType],
Awaitable[None]]
...
```Asynchronous handler hook for `IO.OpenChargeMap.mqtt.StatusType`: Open Charge Map StatusType reference catalog
entity, keyed by the composite `{reference_type}/{reference_id}` so all lookup entities share one identity shape on the
reference topic. Emitted at startup and on the periodic reference refresh.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `open_charge_map_mqtt_producer_data.StatusType`.

Example:

```python
async def io_open_charge_map_mqtt_status_type_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: StatusType) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `io_open_charge_map_mqtt_usage_type_async`



## Test```python

io_open_charge_map_mqtt_usage_type_async:  Callable[[PartitionContext, EventData, CloudEvent, UsageType],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `open_charge_map_mqtt_producer_data.UsageType`.

Example:

```python
async def io_open_charge_map_mqtt_usage_type_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: UsageType) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `io_open_charge_map_mqtt_submission_status_type_async`



## Test```python

io_open_charge_map_mqtt_submission_status_type_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `open_charge_map_mqtt_producer_data.SubmissionStatusType`.

Example:

```python
async def io_open_charge_map_mqtt_submission_status_type_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: SubmissionStatusType) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
