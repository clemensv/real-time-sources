    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from open_charge_map_amqp_producer_data import ChargingLocation
from open_charge_map_amqp_producer_data import Operator
from open_charge_map_amqp_producer_data import ConnectionType
from open_charge_map_amqp_producer_data import CurrentType
from open_charge_map_amqp_producer_data import ChargerType
from open_charge_map_amqp_producer_data import Country
from open_charge_map_amqp_producer_data import DataProvider
from open_charge_map_amqp_producer_data import StatusType
from open_charge_map_amqp_producer_data import UsageType
from open_charge_map_amqp_producer_data import SubmissionStatusType

class IOOpenChargeMapLocationsAmqpProducer:
    """
    Producer class to send messages in the `IO.OpenChargeMap.locations.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_charging_location(self,
        ...
        """
        Send the `IO.OpenChargeMap.amqp.ChargingLocation` message
        Near-real-time charging-location record for one Open Charge Map point of interest, keyed by the stable OCM POI id. Emitted at startup for the configured scope and thereafter whenever the record's `DateLastStatusUpdate` advances.
        ...
        amqp_msg.subject = "{poi_id}".format(poi_id=_poi_id)
        ...
        annotations = {}
        annotation_value = "{poi_id}".format(poi_id=_poi_id)
    ...
    def send_charging_location_batch(self,
        data_array: typing.List[ChargingLocation],
        _feedurl: str,
        _poi_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `IO.OpenChargeMap.amqp.ChargingLocation` messages
        
        Args:
            data_array (typing.List[ChargingLocation]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _poi_id (str): Value for placeholder poi_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
...
class IOOpenChargeMapReferenceAmqpProducer:
    """
    Producer class to send messages in the `IO.OpenChargeMap.reference.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_operator(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `IO.OpenChargeMap.amqp.Operator` message
        Open Charge Map Operator reference catalog entity, keyed by the composite `{reference_type}/{reference_id}` so all lookup entities share one identity shape on the reference topic. Emitted at startup and on the periodic reference refresh.
        ...
        attributes = {
            ...
            "{feedurl}".format(feedurl=_feedurl),
            "subject":
            "{reference_type}/{reference_id}".format(reference_type=_reference_type, reference_id=_reference_id),
        }
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        ...
        if amqp_creation_time is not None:
            amqp_msg.creation_time = amqp_creation_time
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "{reference_type}/{reference_id}".format(reference_type=_reference_type, reference_id=_reference_id)
        ...
        annotations = {}
        annotation_value = "{reference_type}/{reference_id}".format(reference_type=_reference_type, reference_id=_reference_id)
        annotation_value = str(annotation_value)[:128]
        annotations[symbol("x-opt-partition-key")] = annotation_value
        if annotations:
            if amqp_msg.annotations is None:
    ...
    def send_operator_batch(self,
        data_array: typing.List[Operator],
        _feedurl: str,
        _reference_type: str,
        _reference_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `IO.OpenChargeMap.amqp.Operator` messages
        
        Args:
            data_array (typing.List[Operator]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _reference_type (str): Value for placeholder reference_type in attribute subject
            _reference_id (str): Value for placeholder reference_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        for data in data_array:
            self.send_operator(
                data=data,
                _feedurl=_feedurl,
                _reference_type=_reference_type,
                _reference_id=_reference_id,
                _time=_time,
                content_type=content_type)
    
    
    def send_connection_type(self,
        data: ConnectionType,
        _feedurl: str,
        _reference_type: str,
        _reference_id: str,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `IO.OpenChargeMap.amqp.ConnectionType` message
        Open Charge Map ConnectionType reference catalog entity, keyed by the composite `{reference_type}/{reference_id}` so all lookup entities share one identity shape on the reference topic. Emitted at startup and on the periodic reference refresh.
    ...
    def send_connection_type_batch(self,
        data_array: typing.List[ConnectionType],
        _feedurl: str,
        _reference_type: str,
        _reference_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `IO.OpenChargeMap.amqp.ConnectionType` messages
        
        Args:
            data_array (typing.List[ConnectionType]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _reference_type (str): Value for placeholder reference_type in attribute subject
            _reference_id (str): Value for placeholder reference_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    def send_current_type(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `IO.OpenChargeMap.amqp.CurrentType` message
        Open Charge Map CurrentType reference catalog entity, keyed by the composite `{reference_type}/{reference_id}` so all lookup entities share one identity shape on the reference topic. Emitted at startup and on the periodic reference refresh.
    ...
    def send_current_type_batch(self,
        data_array: typing.List[CurrentType],
        _feedurl: str,
        _reference_type: str,
        _reference_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `IO.OpenChargeMap.amqp.CurrentType` messages
        
        Args:
            data_array (typing.List[CurrentType]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _reference_type (str): Value for placeholder reference_type in attribute subject
            _reference_id (str): Value for placeholder reference_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    def send_charger_type(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `IO.OpenChargeMap.amqp.ChargerType` message
        Open Charge Map ChargerType reference catalog entity, keyed by the composite `{reference_type}/{reference_id}` so all lookup entities share one identity shape on the reference topic. Emitted at startup and on the periodic reference refresh.
    ...
    def send_charger_type_batch(self,
        data_array: typing.List[ChargerType],
        _feedurl: str,
        _reference_type: str,
        _reference_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `IO.OpenChargeMap.amqp.ChargerType` messages
        
        Args:
            data_array (typing.List[ChargerType]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _reference_type (str): Value for placeholder reference_type in attribute subject
            _reference_id (str): Value for placeholder reference_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    def send_country(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `IO.OpenChargeMap.amqp.Country` message
        Open Charge Map Country reference catalog entity, keyed by the composite `{reference_type}/{reference_id}` so all lookup entities share one identity shape on the reference topic. Emitted at startup and on the periodic reference refresh.
    ...
    def send_country_batch(self,
        data_array: typing.List[Country],
        _feedurl: str,
        _reference_type: str,
        _reference_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `IO.OpenChargeMap.amqp.Country` messages
        
        Args:
            data_array (typing.List[Country]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _reference_type (str): Value for placeholder reference_type in attribute subject
            _reference_id (str): Value for placeholder reference_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    def send_data_provider(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `IO.OpenChargeMap.amqp.DataProvider` message
        Open Charge Map DataProvider reference catalog entity, keyed by the composite `{reference_type}/{reference_id}` so all lookup entities share one identity shape on the reference topic. Emitted at startup and on the periodic reference refresh.
    ...
    def send_data_provider_batch(self,
        data_array: typing.List[DataProvider],
        _feedurl: str,
        _reference_type: str,
        _reference_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `IO.OpenChargeMap.amqp.DataProvider` messages
        
        Args:
            data_array (typing.List[DataProvider]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _reference_type (str): Value for placeholder reference_type in attribute subject
            _reference_id (str): Value for placeholder reference_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    def send_status_type(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `IO.OpenChargeMap.amqp.StatusType` message
        Open Charge Map StatusType reference catalog entity, keyed by the composite `{reference_type}/{reference_id}` so all lookup entities share one identity shape on the reference topic. Emitted at startup and on the periodic reference refresh.
    ...
    def send_status_type_batch(self,
        data_array: typing.List[StatusType],
        _feedurl: str,
        _reference_type: str,
        _reference_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `IO.OpenChargeMap.amqp.StatusType` messages
        
        Args:
            data_array (typing.List[StatusType]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _reference_type (str): Value for placeholder reference_type in attribute subject
            _reference_id (str): Value for placeholder reference_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    def send_usage_type(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `IO.OpenChargeMap.amqp.UsageType` message
        Open Charge Map UsageType reference catalog entity, keyed by the composite `{reference_type}/{reference_id}` so all lookup entities share one identity shape on the reference topic. Emitted at startup and on the periodic reference refresh.
    ...
    def send_usage_type_batch(self,
        data_array: typing.List[UsageType],
        _feedurl: str,
        _reference_type: str,
        _reference_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `IO.OpenChargeMap.amqp.UsageType` messages
        
        Args:
            data_array (typing.List[UsageType]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _reference_type (str): Value for placeholder reference_type in attribute subject
            _reference_id (str): Value for placeholder reference_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    def send_submission_status_type(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `IO.OpenChargeMap.amqp.SubmissionStatusType` message
        Open Charge Map SubmissionStatusType reference catalog entity, keyed by the composite `{reference_type}/{reference_id}` so all lookup entities share one identity shape on the reference topic. Emitted at startup and on the periodic reference refresh.
    ...
    def send_submission_status_type_batch(self,
        data_array: typing.List[SubmissionStatusType],
        _feedurl: str,
        _reference_type: str,
        _reference_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `IO.OpenChargeMap.amqp.SubmissionStatusType` messages
        
        Args:
            data_array (typing.List[SubmissionStatusType]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _reference_type (str): Value for placeholder reference_type in attribute subject
            _reference_id (str): Value for placeholder reference_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        """
        for data in data_array:
            self.send_submission_status_type(
                data=data,
                _feedurl=_feedurl,
                _reference_type=_reference_type,
                _reference_id=_reference_id,
