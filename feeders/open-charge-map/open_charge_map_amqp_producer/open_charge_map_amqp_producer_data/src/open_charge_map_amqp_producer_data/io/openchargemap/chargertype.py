...
class ChargerType:
    """
    One charging level (charger power class) from the Open Charge Map reference data (`ChargerTypes` table, 3 entries: Level 1 Low, Level 2 Medium, Level 3 High). Open Charge Map calls this table `ChargerTypes` while POI connections reference it through the `LevelID` field; a ChargingLocation connection joins to this event through its `level_id`. Emitted at startup and on the periodic reference refresh.
    ...
    @classmethod
    def from_serializer_dict(cls, data: dict) -> 'ChargerType':
        """
    ...
    @classmethod
    def from_data(cls, data: typing.Any, content_type_string: typing.Optional[str] = None) -> typing.Optional['ChargerType']:
        """
        ...
        if base_content_type == 'application/json':
            if isinstance(data, (bytes, str)):
                ...
                return ChargerType.from_serializer_dict(_record)
            else:
                raise NotImplementedError('Data is not of a supported type for JSON deserialization')
        raise NotImplementedError(f'Unsupported media type {content_type}')

    @classmethod
    def create_instance(cls) -> 'ChargerType':
        """
        ...
        """
        return cls(
            reference_type='pxstlhlhepnxhhdkjmcb',
            reference_id=int(48),
            title='kvcxszsqeezxxetfpeso',
            comments='tcubhazkxbgurdqnisds',
