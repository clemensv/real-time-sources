    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from energy_charts_amqp_producer_data import PublicPower
from energy_charts_amqp_producer_data import SpotPrice
from energy_charts_amqp_producer_data import GridSignal
from energy_charts_amqp_producer_data import Info

class InfoEnergyChartsAmqpProducer:
    """
    Producer class to send messages in the `info.energy_charts.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_public_power(self,
        data: PublicPower,
        _country: str,
        content_type: str = 'application/json') -> None:
        """
        Send the `info.energy_charts.amqp.PublicPower` message
        
        Args:
            _country (str): Value for placeholder country in attribute subject
            data (PublicPower): The message data object
        ...
        attributes = {
            ...
            "subject":
            "{country}".format(country=_country),
        }
        ...
        amqp_msg.subject = "{country}".format(country=_country)
        ...
        annotations = {}
        annotation_value = "{country}".format(country=_country)
    ...
    def send_public_power_batch(self,
        data_array: typing.List[PublicPower],
        _country: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `info.energy_charts.amqp.PublicPower` messages
        
        Args:
            data_array (typing.List[PublicPower]): Array of message data objects
            _country (str): Value for placeholder country in attribute subject
            content_type (str): The content type of the message data
        """
        for data in data_array:
            self.send_public_power(
                data=data,
                _country=_country,
                content_type=content_type)
    
    
    def send_spot_price(self,
        data: SpotPrice,
        _country: str,
        content_type: str = 'application/json') -> None:
        """
        Send the `info.energy_charts.amqp.SpotPrice` message
    ...
    def send_spot_price_batch(self,
        data_array: typing.List[SpotPrice],
        _country: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `info.energy_charts.amqp.SpotPrice` messages
        
        Args:
            data_array (typing.List[SpotPrice]): Array of message data objects
            _country (str): Value for placeholder country in attribute subject
            content_type (str): The content type of the message data
        """
        for data in data_array:
            self.send_spot_price(
                data=data,
                _country=_country,
                content_type=content_type)
    
    
    def send_grid_signal(self,
        data: GridSignal,
        _country: str,
        content_type: str = 'application/json') -> None:
        """
        Send the `info.energy_charts.amqp.GridSignal` message
        
        Args:
            _country (str): Value for placeholder country in attribute subject
            data (GridSignal): The message data object
            content_type (str): The content type of the message data (default: 'application/json')
        """
        # Build CloudEvent attributes
        attributes = {
    ...
    def send_grid_signal_batch(self,
        data_array: typing.List[GridSignal],
        _country: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `info.energy_charts.amqp.GridSignal` messages
        
        Args:
            data_array (typing.List[GridSignal]): Array of message data objects
            _country (str): Value for placeholder country in attribute subject
            content_type (str): The content type of the message data
        """
        for data in data_array:
            self.send_grid_signal(
                data=data,
                _country=_country,
                content_type=content_type)
    ...
    def send_info(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `info.energy_charts.amqp.Info` message
    ...
    def send_info_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `info.energy_charts.amqp.Info` messages
