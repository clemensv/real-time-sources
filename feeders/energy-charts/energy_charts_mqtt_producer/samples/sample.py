...
import os
import signal
from energy_charts_mqtt_producer_data import *
from energy_charts_mqtt_producer_mqtt_client.client import InfoEnergyChartsMqttProducer, InfoEnergyChartsMqttDispatcher

async def handle_info_energy_charts_mqtt_public_power(mqtt_msg,cloud_event, info_energy_charts_mqtt_public_power_data):
    """ Handles the info.energy_charts.mqtt.PublicPower message """
    print(f"Received info.energy_charts.mqtt.PublicPower on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {info_energy_charts_mqtt_public_power_data}")

async def handle_info_energy_charts_mqtt_spot_price(mqtt_msg,cloud_event, info_energy_charts_mqtt_spot_price_data):
    """ Handles the info.energy_charts.mqtt.SpotPrice message """
    print(f"Received info.energy_charts.mqtt.SpotPrice on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {info_energy_charts_mqtt_spot_price_data}")

async def handle_info_energy_charts_mqtt_grid_signal(mqtt_msg,cloud_event, info_energy_charts_mqtt_grid_signal_data):
    """ Handles the info.energy_charts.mqtt.GridSignal message """
    print(f"Received info.energy_charts.mqtt.GridSignal on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {info_energy_charts_mqtt_grid_signal_data}")

async def handle_info_energy_charts_mqtt_info(mqtt_msg,cloud_event, info_energy_charts_mqtt_info_data):
    """ Handles the info.energy_charts.mqtt.Info message """
    print(f"Received info.energy_charts.mqtt.Info on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {info_energy_charts_mqtt_info_data}")
