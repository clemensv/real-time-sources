...
def get_default_topic_mappings_info_energy_charts_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the info.energy_charts.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "info.energy_charts.mqtt.PublicPower": "energy/de/energy-charts/energy-charts/{country}/public-power",
        "info.energy_charts.mqtt.SpotPrice": "energy/de/energy-charts/energy-charts/{country}/spot-price",
        "info.energy_charts.mqtt.GridSignal": "energy/de/energy-charts/energy-charts/{country}/grid-signal",
        "info.energy_charts.mqtt.Info": "energy/de/energy-charts/energy-charts/{country}/info",
    }


def get_subscription_topics_info_energy_charts_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_info_energy_charts_mqtt()
    topics = []
...
class InfoEnergyChartsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the info.energy_charts.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_info_energy_charts_mqtt()
        ...
        self.info_energy_charts_mqtt_public_power_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, energy_charts_mqtt_producer_data.PublicPower, Dict[str, str]], Awaitable[None]]] = None
        
        self.info_energy_charts_mqtt_spot_price_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, energy_charts_mqtt_producer_data.SpotPrice, Dict[str, str]], Awaitable[None]]] = None
        
        self.info_energy_charts_mqtt_grid_signal_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, energy_charts_mqtt_producer_data.GridSignal, Dict[str, str]], Awaitable[None]]] = None
        
        self.info_energy_charts_mqtt_info_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, energy_charts_mqtt_producer_data.Info, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "info.energy_charts.PublicPower":
            if self.info_energy_charts_mqtt_public_power_async:
                try:
                    ...
                    data = energy_charts_mqtt_producer_data.PublicPower.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "info.energy_charts.mqtt.PublicPower")
                    await self.info_energy_charts_mqtt_public_power_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in info_energy_charts_mqtt_public_power handler: {e}")
            return
        
        if event_type == "info.energy_charts.SpotPrice":
            if self.info_energy_charts_mqtt_spot_price_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = energy_charts_mqtt_producer_data.SpotPrice.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "info.energy_charts.mqtt.SpotPrice")
                    await self.info_energy_charts_mqtt_spot_price_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in info_energy_charts_mqtt_spot_price handler: {e}")
            return
        
        if event_type == "info.energy_charts.GridSignal":
            if self.info_energy_charts_mqtt_grid_signal_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = energy_charts_mqtt_producer_data.GridSignal.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "info.energy_charts.mqtt.GridSignal")
                    await self.info_energy_charts_mqtt_grid_signal_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in info_energy_charts_mqtt_grid_signal handler: {e}")
            return
        
        if event_type == "info.energy_charts.Info":
            if self.info_energy_charts_mqtt_info_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = energy_charts_mqtt_producer_data.Info.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "info.energy_charts.mqtt.Info")
                    await self.info_energy_charts_mqtt_info_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in info_energy_charts_mqtt_info handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_info_energy_charts_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_info_energy_charts_mqtt_public_power(self,
        country: str,
        data: energy_charts_mqtt_producer_data.PublicPower,
        ...
        """
        Publish the 'info.energy_charts.mqtt.PublicPower' event to an MQTT topic.

        Args:
        
            country: URI template variable for 'country'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'energy/de/energy-charts/energy-charts/{country}/public-power'
        ...
        """
        target_topic = topic if topic is not None else "energy/de/energy-charts/energy-charts/{country}/public-power"
        ...
        attributes = {
             "type":"info.energy_charts.PublicPower",
             "source":"https://api.energy-charts.info",
             "subject":"{country}".format(country = country)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_info_energy_charts_mqtt_spot_price(self,
        country: str,
        data: energy_charts_mqtt_producer_data.SpotPrice,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'info.energy_charts.mqtt.SpotPrice' event to an MQTT topic.

        Args:
        
            country: URI template variable for 'country'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'energy/de/energy-charts/energy-charts/{country}/spot-price'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "energy/de/energy-charts/energy-charts/{country}/spot-price"
        _topic_template_values: Dict[str, str] = {
            "country": str(country),
        }
        if _topic_template_values:
        ...
        attributes = {
             "type":"info.energy_charts.SpotPrice",
             "source":"https://api.energy-charts.info",
             "subject":"{country}".format(country = country)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_info_energy_charts_mqtt_grid_signal(self,
        country: str,
        data: energy_charts_mqtt_producer_data.GridSignal,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'info.energy_charts.mqtt.GridSignal' event to an MQTT topic.

        Args:
        
            country: URI template variable for 'country'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'energy/de/energy-charts/energy-charts/{country}/grid-signal'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "energy/de/energy-charts/energy-charts/{country}/grid-signal"
        _topic_template_values: Dict[str, str] = {
            "country": str(country),
        }
        if _topic_template_values:
        ...
        attributes = {
             "type":"info.energy_charts.GridSignal",
             "source":"https://api.energy-charts.info",
             "subject":"{country}".format(country = country)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_info_energy_charts_mqtt_info(self,
        country: str,
        data: energy_charts_mqtt_producer_data.Info,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'info.energy_charts.mqtt.Info' event to an MQTT topic.

        Args:
        
            country: URI template variable for 'country'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'energy/de/energy-charts/energy-charts/{country}/info'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "energy/de/energy-charts/energy-charts/{country}/info"
        _topic_template_values: Dict[str, str] = {
            "country": str(country),
        }
        if _topic_template_values:
