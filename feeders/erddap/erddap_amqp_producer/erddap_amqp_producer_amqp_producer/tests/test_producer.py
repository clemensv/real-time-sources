...
"""
Tests for erddap_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../erddap_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../erddap_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../erddap_amqp_producer_amqp_producer/src')))

from erddap_amqp_producer_amqp_producer import *
from erddap_amqp_producer_data import DatasetMetadata
from test_datasetmetadata import Test_DatasetMetadata
from erddap_amqp_producer_data import StationMetadata
from test_stationmetadata import Test_StationMetadata
from erddap_amqp_producer_data import Observation
...
class TestOrgErddapAmqpDatasetProducer:
    """Test cases for OrgErddapAmqpDatasetProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(OrgErddapAmqpDatasetProducer)
    ...
    def test_send_dataset_metadata(self, artemis_container):
        """Send and receive a DatasetMetadata message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_DatasetMetadata.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "org.erddap.DatasetMetadata"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{erddap_id}/{dataset_id}".format(erddap_id="value", dataset_id="value")
                assert properties.get('erddap_id') == "{erddap_id}".format(erddap_id="value")
                assert properties.get('dataset_id') == "{dataset_id}".format(dataset_id="value")
        finally:
            producer.close()

    def test_send_dataset_metadata_single_fresh_connection(self, artemis_container):
        """Send exactly one DatasetMetadata message on a fresh producer connection."""
        payload = Test_DatasetMetadata.create_instance()
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.erddap.DatasetMetadata'
        assert received.body is not None
        assert received.subject == "{erddap_id}/{dataset_id}".format(erddap_id="value", dataset_id="value")
        assert properties.get('erddap_id') == "{erddap_id}".format(erddap_id="value")
        assert properties.get('dataset_id') == "{dataset_id}".format(dataset_id="value")
...
class TestOrgErddapAmqpStationProducer:
    """Test cases for OrgErddapAmqpStationProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        fake_connection = FakeConnection()
        producer = object.__new__(OrgErddapAmqpStationProducer)
        producer._sender = fake_sender
        producer._connection = fake_connection
        producer._blocking_sender_is_presettled = True
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "org.erddap.StationMetadata"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{erddap_id}/{dataset_id}/{station_id}".format(erddap_id="value", dataset_id="value", station_id="value")
                assert properties.get('erddap_id') == "{erddap_id}".format(erddap_id="value")
                assert properties.get('dataset_id') == "{dataset_id}".format(dataset_id="value")
        finally:
            producer.close()
    ...
    def test_send_station_metadata_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.erddap.StationMetadata'
        assert received.body is not None
        assert received.subject == "{erddap_id}/{dataset_id}/{station_id}".format(erddap_id="value", dataset_id="value", station_id="value")
        assert properties.get('erddap_id') == "{erddap_id}".format(erddap_id="value")
        assert properties.get('dataset_id') == "{dataset_id}".format(dataset_id="value")
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "org.erddap.Observation"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_observation_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.erddap.Observation'
        assert received.body is not None
        assert received.subject == "{erddap_id}/{dataset_id}/{station_id}".format(erddap_id="value", dataset_id="value", station_id="value")
        assert properties.get('erddap_id') == "{erddap_id}".format(erddap_id="value")
        assert properties.get('dataset_id') == "{dataset_id}".format(dataset_id="value")
