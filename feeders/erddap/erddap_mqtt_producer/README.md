# Erddap_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the erddap_mqtt_producer message definitions.

# Erddap_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - OrgErddapMqttDatasetEventDispatcher,

    OrgErddapMqttStationEventDispatcher
...
from erddap_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = OrgErddapMqttDatasetMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgErddapMqttDatasetEventDispatcher



# Publish message`OrgErddapMqttDatasetEventDispatcher` handles events for the org.erddap.mqtt.dataset message group.
...
from erddap_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = OrgErddapMqttDatasetMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
client = OrgErddapMqttStationMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe OrgErddapMqttDatasetEventDispatcher defines the following event handler hooks.
...
```##### `org_erddap_mqtt_dataset_metadata_async`



## Test```python

org_erddap_mqtt_dataset_metadata_async:  Callable[[PartitionContext, EventData, CloudEvent, DatasetMetadata],
Awaitable[None]]
...
- `data`: The event data of type `erddap_mqtt_producer_data.DatasetMetadata`.

Example:

```python
async def org_erddap_mqtt_dataset_metadata_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: DatasetMetadata) -> None:
...
client = OrgErddapMqttStationMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgErddapMqttStationEventDispatcher



# Publish message`OrgErddapMqttStationEventDispatcher` handles events for the org.erddap.mqtt.station message group.
...
## BuildThe OrgErddapMqttStationEventDispatcher defines the following event handler hooks.
...
```##### `org_erddap_mqtt_station_metadata_async`



## Test```python

org_erddap_mqtt_station_metadata_async:  Callable[[PartitionContext, EventData, CloudEvent, StationMetadata],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `erddap_mqtt_producer_data.StationMetadata`.

Example:

```python
async def org_erddap_mqtt_station_metadata_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `org_erddap_mqtt_observation_async`



## Test```python

org_erddap_mqtt_observation_async:  Callable[[PartitionContext, EventData, CloudEvent, Observation], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `erddap_mqtt_producer_data.Observation`.

Example:

```python
async def org_erddap_mqtt_observation_event(partition_context: PartitionContext, event: EventData, cloud_event:
