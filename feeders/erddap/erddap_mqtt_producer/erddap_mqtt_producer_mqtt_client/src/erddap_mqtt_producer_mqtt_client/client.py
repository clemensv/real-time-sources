...
import erddap_mqtt_producer_data
from erddap_mqtt_producer_data import DatasetMetadata
from erddap_mqtt_producer_data import StationMetadata
from erddap_mqtt_producer_data import Observation
...
def get_default_topic_mappings_org_erddap_mqtt_dataset() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.erddap.mqtt.dataset message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.erddap.mqtt.DatasetMetadata": "marine/global/erddap/{erddap_id}/{dataset_id}/dataset",
    }


def get_subscription_topics_org_erddap_mqtt_dataset(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_org_erddap_mqtt_dataset()
    topics = []
...
class OrgErddapMqttDatasetMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.erddap.mqtt.dataset message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_erddap_mqtt_dataset()
        ...
        self.org_erddap_mqtt_dataset_metadata_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, erddap_mqtt_producer_data.DatasetMetadata, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.erddap.DatasetMetadata":
            if self.org_erddap_mqtt_dataset_metadata_async:
                try:
                    ...
                    data = erddap_mqtt_producer_data.DatasetMetadata.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.erddap.mqtt.DatasetMetadata")
                    await self.org_erddap_mqtt_dataset_metadata_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_erddap_mqtt_dataset_metadata handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_erddap_mqtt_dataset(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_org_erddap_mqtt_dataset_metadata(self,
        base_url: str,
        erddap_id: str,
        dataset_id: str,
        data: erddap_mqtt_producer_data.DatasetMetadata,
        ...
        """
        Publish the 'org.erddap.mqtt.DatasetMetadata' event to an MQTT topic.

        Args:
        
            base_url: URI template variable for 'base_url'
            erddap_id: URI template variable for 'erddap_id'
            dataset_id: URI template variable for 'dataset_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'marine/global/erddap/{erddap_id}/{dataset_id}/dataset'
        ...
        """
        target_topic = topic if topic is not None else "marine/global/erddap/{erddap_id}/{dataset_id}/dataset"
        _topic_template_values: Dict[str, str] = {
            "base_url": str(base_url),
            "erddap_id": str(erddap_id),
        ...
        attributes = {
             ...
             "source":"{base_url}".format(base_url = base_url),
             "subject":"{erddap_id}/{dataset_id}".format(erddap_id = erddap_id,dataset_id = dataset_id)
        }
        attributes["datacontenttype"] = content_type
...
def get_default_topic_mappings_org_erddap_mqtt_station() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.erddap.mqtt.station message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.erddap.mqtt.StationMetadata": "marine/global/erddap/{erddap_id}/{dataset_id}/{station_id}/station",
        "org.erddap.mqtt.Observation": "marine/global/erddap/{erddap_id}/{dataset_id}/{station_id}/observation",
    }


def get_subscription_topics_org_erddap_mqtt_station(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_org_erddap_mqtt_station()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class OrgErddapMqttStationMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.erddap.mqtt.station message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_erddap_mqtt_station()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.org_erddap_mqtt_station_metadata_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, erddap_mqtt_producer_data.StationMetadata, Dict[str, str]], Awaitable[None]]] = None
        
        self.org_erddap_mqtt_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, erddap_mqtt_producer_data.Observation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.erddap.StationMetadata":
            if self.org_erddap_mqtt_station_metadata_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = erddap_mqtt_producer_data.StationMetadata.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.erddap.mqtt.StationMetadata")
                    await self.org_erddap_mqtt_station_metadata_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_erddap_mqtt_station_metadata handler: {e}")
            return
        
        if event_type == "org.erddap.Observation":
            if self.org_erddap_mqtt_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = erddap_mqtt_producer_data.Observation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.erddap.mqtt.Observation")
                    await self.org_erddap_mqtt_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_erddap_mqtt_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_erddap_mqtt_station(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_org_erddap_mqtt_station_metadata(self,
        base_url: str,
        erddap_id: str,
        dataset_id: str,
        station_id: str,
        data: erddap_mqtt_producer_data.StationMetadata,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.erddap.mqtt.StationMetadata' event to an MQTT topic.
        ...
        Args:
            ...
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'marine/global/erddap/{erddap_id}/{dataset_id}/{station_id}/station'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "marine/global/erddap/{erddap_id}/{dataset_id}/{station_id}/station"
        _topic_template_values: Dict[str, str] = {
            "base_url": str(base_url),
            "erddap_id": str(erddap_id),
            "dataset_id": str(dataset_id),
        ...
        attributes = {
             ...
             "source":"{base_url}".format(base_url = base_url),
             "subject":"{erddap_id}/{dataset_id}/{station_id}".format(erddap_id = erddap_id,dataset_id = dataset_id,station_id = station_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
    ...
    async def publish_org_erddap_mqtt_observation(self,
        base_url: str,
        erddap_id: str,
        dataset_id: str,
        station_id: str,
        data: erddap_mqtt_producer_data.Observation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.erddap.mqtt.Observation' event to an MQTT topic.
        ...
        Args:
            ...
            station_id: URI template variable for 'station_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'marine/global/erddap/{erddap_id}/{dataset_id}/{station_id}/observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            ...
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "marine/global/erddap/{erddap_id}/{dataset_id}/{station_id}/observation"
        _topic_template_values: Dict[str, str] = {
            "base_url": str(base_url),
            "erddap_id": str(erddap_id),
            "dataset_id": str(dataset_id),
