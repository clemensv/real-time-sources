...
import os
import signal
from erddap_mqtt_producer_data import *
from erddap_mqtt_producer_mqtt_client.client import OrgErddapMqttDatasetProducer, OrgErddapMqttDatasetDispatcher

async def handle_org_erddap_mqtt_dataset_metadata(mqtt_msg,cloud_event, org_erddap_mqtt_dataset_metadata_data):
    """ Handles the org.erddap.mqtt.DatasetMetadata message """
    print(f"Received org.erddap.mqtt.DatasetMetadata on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_erddap_mqtt_dataset_metadata_data}")
from erddap_mqtt_producer_mqtt_client.client import OrgErddapMqttStationProducer, OrgErddapMqttStationDispatcher

async def handle_org_erddap_mqtt_station_metadata(mqtt_msg,cloud_event, org_erddap_mqtt_station_metadata_data):
    """ Handles the org.erddap.mqtt.StationMetadata message """
    print(f"Received org.erddap.mqtt.StationMetadata on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_erddap_mqtt_station_metadata_data}")

async def handle_org_erddap_mqtt_observation(mqtt_msg,cloud_event, org_erddap_mqtt_observation_data):
    """ Handles the org.erddap.mqtt.Observation message """
    print(f"Received org.erddap.mqtt.Observation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_erddap_mqtt_observation_data}")
