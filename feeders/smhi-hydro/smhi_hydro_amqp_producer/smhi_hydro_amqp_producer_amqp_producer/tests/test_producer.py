...
"""
Tests for smhi_hydro_amqp_producer_amqp_producer
"""
import base64
import json
import os
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../smhi_hydro_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../smhi_hydro_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../smhi_hydro_amqp_producer_amqp_producer/src')))

from smhi_hydro_amqp_producer_amqp_producer import *
from smhi_hydro_amqp_producer_data import Station
from test_smhi_hydro_amqp_producer_data_station import Test_Station
from smhi_hydro_amqp_producer_data import DischargeObservation
from test_smhi_hydro_amqp_producer_data_dischargeobservation import Test_DischargeObservation
...
class TestSEGovSMHIHydroAmqpProducer:
    """Test cases for SEGovSMHIHydroAmqpProducer"""
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "SE.Gov.SMHI.Hydro.amqp.Station"
                ...
                assert properties.get('catchment_name') == "{catchment_name}".format(catchment_name="value")
        finally:
            producer.close()
    
    def test_send_discharge_observation(self, artemis_container):
        """Send and receive a DischargeObservation message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_DischargeObservation.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "SE.Gov.SMHI.Hydro.amqp.DischargeObservation"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
