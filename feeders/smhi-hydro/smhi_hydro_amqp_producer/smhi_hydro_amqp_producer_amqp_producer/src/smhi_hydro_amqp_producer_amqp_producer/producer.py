    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from smhi_hydro_amqp_producer_data import Station
from smhi_hydro_amqp_producer_data import DischargeObservation

class SEGovSMHIHydroAmqpProducer:
    """
    Producer class to send messages in the `SE.Gov.SMHI.Hydro.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        ...
        """
        Send the `SE.Gov.SMHI.Hydro.amqp.Station` message
        A reference record for one Swedish hydrological monitoring station published by the Swedish Meteorological and Hydrological Institute (SMHI). It fires when the bridge publishes or refreshes the station catalog so consumers can interpret measurement events.
        
        Args:
            _station_id (str): Value for placeholder station_id in attribute subject
            _catchment_name (str): Value for AMQP protocol option placeholder catchment_name
        ...
        app_properties = {}
        app_properties["catchment_name"] = "{catchment_name}".format(catchment_name=_catchment_name)
        if app_properties:
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `SE.Gov.SMHI.Hydro.amqp.Station` messages
        ...
        for data in data_array:
            self.send_station(
                ...
                _catchment_name=_catchment_name,
                content_type=content_type)
    
    
    def send_discharge_observation(self,
        data: DischargeObservation,
        _station_id: str,
        _catchment_name: str,
        content_type: str = 'application/json') -> None:
        """
        Send the `SE.Gov.SMHI.Hydro.amqp.DischargeObservation` message
        A current measurement from the Swedish Meteorological and Hydrological Institute (SMHI) for one monitoring site. It carries river discharge observations when the upstream feed reports a new or refreshed value.
        
        Args:
            _station_id (str): Value for placeholder station_id in attribute subject
            _catchment_name (str): Value for AMQP protocol option placeholder catchment_name
            data (DischargeObservation): The message data object
            content_type (str): The content type of the message data (default: 'application/json')
        """
        # Build CloudEvent attributes
        attributes = {
    ...
    def send_discharge_observation_batch(self,
        data_array: typing.List[DischargeObservation],
        _station_id: str,
        _catchment_name: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `SE.Gov.SMHI.Hydro.amqp.DischargeObservation` messages
        
        Args:
            data_array (typing.List[DischargeObservation]): Array of message data objects
            _station_id (str): Value for placeholder station_id in attribute subject
            _catchment_name (str): Value for AMQP protocol option placeholder catchment_name
            content_type (str): The content type of the message data
        """
        for data in data_array:
            self.send_discharge_observation(
                data=data,
                _station_id=_station_id,
                _catchment_name=_catchment_name,
                content_type=content_type)
