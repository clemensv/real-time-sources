...
Attach a link with `role=receiver` whose **source** is `smhi-hydro`. The source terminus is the broker-side node you consume from; source filters such as selectors, Event Hubs offsets, or subscription filters further select which messages flow. The target is your client-side terminus. Generic brokers use their advertised SASL mechanisms (often PLAIN over TLS, EXTERNAL with mTLS, or ANONYMOUS on trusted links). Azure Service Bus and Event Hubs can use SASL PLAIN for SAS credentials on short-lived connections; CBS `put-token` on `$cbs` installs and refreshes Entra ID JWTs or SAS tokens for long-lived AMQP connections.

```python
from proton.handlers import MessagingHandler
from proton.reactor import Container
class H(MessagingHandler):
    def on_start(self,e): e.container.create_receiver('amqps://user:pass@localhost:5671/smhi-hydro')
...
| `AMQP/1.0` | source address `amqps://localhost:5671/smhi-hydro`, message subject `{station_id}`; application properties catchment_name `{catchment_name}` |
...
- xRegistry manifest: [`xreg/smhi-hydro.xreg.json`](xreg/smhi-hydro.xreg.json)
