...
import smhi_hydro_mqtt_producer_data
from smhi_hydro_mqtt_producer_data import Station
from smhi_hydro_mqtt_producer_data import DischargeObservation
...
def get_default_topic_mappings_se_gov_smhi_hydro_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the SE.Gov.SMHI.Hydro.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "SE.Gov.SMHI.Hydro.mqtt.Station": "hydro/se/smhi/smhi-hydro/{catchment_name}/{station_id}/info",
        "SE.Gov.SMHI.Hydro.mqtt.DischargeObservation": "hydro/se/smhi/smhi-hydro/{catchment_name}/{station_id}/discharge",
    }


def get_subscription_topics_se_gov_smhi_hydro_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_se_gov_smhi_hydro_mqtt()
    topics = []
...
class SEGovSMHIHydroMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the SE.Gov.SMHI.Hydro.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_se_gov_smhi_hydro_mqtt()
        ...
        self.se_gov_smhi_hydro_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, smhi_hydro_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.se_gov_smhi_hydro_mqtt_discharge_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, smhi_hydro_mqtt_producer_data.DischargeObservation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "SE.Gov.SMHI.Hydro.Station":
            if self.se_gov_smhi_hydro_mqtt_station_async:
                try:
                    ...
                    data = smhi_hydro_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "SE.Gov.SMHI.Hydro.mqtt.Station")
                    await self.se_gov_smhi_hydro_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in se_gov_smhi_hydro_mqtt_station handler: {e}")
            return
        
        if event_type == "SE.Gov.SMHI.Hydro.DischargeObservation":
            if self.se_gov_smhi_hydro_mqtt_discharge_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = smhi_hydro_mqtt_producer_data.DischargeObservation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "SE.Gov.SMHI.Hydro.mqtt.DischargeObservation")
                    await self.se_gov_smhi_hydro_mqtt_discharge_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in se_gov_smhi_hydro_mqtt_discharge_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_se_gov_smhi_hydro_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_se_gov_smhi_hydro_mqtt_station(self,
        station_id: str,
        catchment_name: str,
        data: smhi_hydro_mqtt_producer_data.Station,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'SE.Gov.SMHI.Hydro.mqtt.Station' event to an MQTT topic.

        Args:
        
            station_id: URI template variable for 'station_id'
            catchment_name: URI template variable for 'catchment_name'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/se/smhi/smhi-hydro/{catchment_name}/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "hydro/se/smhi/smhi-hydro/{catchment_name}/{station_id}/info"
    ...
    async def publish_se_gov_smhi_hydro_mqtt_discharge_observation(self,
        station_id: str,
        catchment_name: str,
        data: smhi_hydro_mqtt_producer_data.DischargeObservation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'SE.Gov.SMHI.Hydro.mqtt.DischargeObservation' event to an MQTT topic.
        ...
        Args:
            ...
            catchment_name: URI template variable for 'catchment_name'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/se/smhi/smhi-hydro/{catchment_name}/{station_id}/discharge'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "hydro/se/smhi/smhi-hydro/{catchment_name}/{station_id}/discharge"
        _topic_template_values: Dict[str, str] = {
            "station_id": str(station_id),
            "catchment_name": str(catchment_name),
        }
