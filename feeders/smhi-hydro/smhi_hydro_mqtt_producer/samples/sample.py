...
import os
import signal
from smhi_hydro_mqtt_producer_data import *
from smhi_hydro_mqtt_producer_mqtt_client.client import SEGovSMHIHydroMqttProducer, SEGovSMHIHydroMqttDispatcher

async def handle_se_gov_smhi_hydro_mqtt_station(mqtt_msg,cloud_event, se_gov_smhi_hydro_mqtt_station_data):
    """ Handles the SE.Gov.SMHI.Hydro.mqtt.Station message """
    print(f"Received SE.Gov.SMHI.Hydro.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {se_gov_smhi_hydro_mqtt_station_data}")

async def handle_se_gov_smhi_hydro_mqtt_discharge_observation(mqtt_msg,cloud_event, se_gov_smhi_hydro_mqtt_discharge_observation_data):
    """ Handles the SE.Gov.SMHI.Hydro.mqtt.DischargeObservation message """
    print(f"Received SE.Gov.SMHI.Hydro.mqtt.DischargeObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {se_gov_smhi_hydro_mqtt_discharge_observation_data}")
