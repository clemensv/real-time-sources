...
# Bafu_hydro_producer Kafka Producer# Bafu_hydro_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - CHBAFUHydrologyEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    CHBAFUHydrologyMqttEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    CHBAFUHydrologyAmqpEventDispatcher
...
- CHBAFUHydrologyProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- CHBAFUHydrologyMqttProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- CHBAFUHydrologyAmqpProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### CHBAFUHydrologyEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`CHBAFUHydrologyEventDispatcher` handles events for the CH.BAFU.Hydrology message group.
...
from bafu_hydro_producer import CHBAFUHydrologyProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = CHBAFUHydrologyProducer(
...
await producer.send_ch_bafu_hydrology_station(```python
...
The CHBAFUHydrologyEventDispatcher defines the following event handler hooks.
...
producer = CHBAFUHydrologyProducer(
    ...
    security_protocol='SASL_SSL',##### `ch_bafu_hydrology_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'ch_bafu_hydrology_station_async:  Callable[[ConsumerRecord, CloudEvent, Station],
Awaitable[None]]
...
Asynchronous handler hook for `CH.BAFU.Hydrology.Station`: A reference record for one Swiss river, lake, and stream
gauge published by the Swiss Federal Office for the Environment (BAFU/FOEN). It fires when the bridge publishes or
...
### CHBAFUHydrologyProducer- `data`: The event data of type `bafu_hydro_producer_data.Station`.



Producer for `CH.BAFU.Hydrology` message group.Example:



#### Constructor```python

async def ch_bafu_hydrology_station_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Station) -> None:

```python    # Process the event data

CHBAFUHydrologyProducer(    await some_processing_function(record, cloud_event, data)
...
ch_bafu_hydrology_dispatcher.ch_bafu_hydrology_station_async = ch_bafu_hydrology_station_event
...
### CHBAFUHydrologyMqttProducer- `data`: The event data of type `bafu_hydro_producer_data.Station`.



Producer for `CH.BAFU.Hydrology.mqtt` message group.Example:
...
CHBAFUHydrologyMqttProducer(    await some_processing_function(record, cloud_event, data)
...
ch_bafu_hydrology_mqtt_dispatcher.ch_bafu_hydrology_station_async = ch_bafu_hydrology_station_event
...
### CHBAFUHydrologyAmqpProducer- `data`: The event data of type `bafu_hydro_producer_data.Station`.



Producer for `CH.BAFU.Hydrology.amqp` message group.Example:
...
CHBAFUHydrologyAmqpProducer(    await some_processing_function(record, cloud_event, data)
...
ch_bafu_hydrology_amqp_dispatcher.ch_bafu_hydrology_station_async = ch_bafu_hydrology_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `ch_bafu_hydrology_water_level_observation_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'ch_bafu_hydrology_water_level_observation_async:  Callable[[ConsumerRecord, CloudEvent,
WaterLevelObservation], Awaitable[None]]

)```

```

Asynchronous handler hook for `CH.BAFU.Hydrology.WaterLevelObservation`: A current measurement from the Swiss Federal
Office for the Environment (BAFU/FOEN) for one monitoring site. It carries water level, discharge, and water temperature
...
### CHBAFUHydrologyProducer- `data`: The event data of type `bafu_hydro_producer_data.WaterLevelObservation`.
...
async def ch_bafu_hydrology_water_level_observation_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
...
ch_bafu_hydrology_dispatcher.ch_bafu_hydrology_water_level_observation_async =
ch_bafu_hydrology_water_level_observation_event
...
### CHBAFUHydrologyMqttProducer- `data`: The event data of type `bafu_hydro_producer_data.WaterLevelObservation`.
...
ch_bafu_hydrology_mqtt_dispatcher.ch_bafu_hydrology_water_level_observation_async =
ch_bafu_hydrology_water_level_observation_event
...
### CHBAFUHydrologyAmqpProducer- `data`: The event data of type `bafu_hydro_producer_data.WaterLevelObservation`.
...
ch_bafu_hydrology_amqp_dispatcher.ch_bafu_hydrology_water_level_observation_async =
ch_bafu_hydrology_water_level_observation_event
...
##### `send_ch_bafu_hydrology_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_ch_bafu_hydrology_station(
...
Send a single `CH.BAFU.Hydrology.Station` message. A reference record for one Swiss river, lake, and stream gauge
published by the Swiss Federal Office for the Environment (BAFU/FOEN). It fires when the bridge publishes or refreshes
...
await producer.send_ch_bafu_hydrology_station(
...
Send multiple `CH.BAFU.Hydrology.Station` messages in a batch.
...
await producer.send_ch_bafu_hydrology_station_batch(```
...
##### `send_ch_bafu_hydrology_water_level_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_ch_bafu_hydrology_water_level_observation(
...
Send a single `CH.BAFU.Hydrology.WaterLevelObservation` message. A current measurement from the Swiss Federal Office for
the Environment (BAFU/FOEN) for one monitoring site. It carries water level, discharge, and water temperature
observations when the upstream feed reports a new or refreshed value.Args:
...
await producer.send_ch_bafu_hydrology_water_level_observation(
...
Send multiple `CH.BAFU.Hydrology.WaterLevelObservation` messages in a batch.
...
await producer.send_ch_bafu_hydrology_water_level_observation_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### CHBAFUHydrologyMqttEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`CHBAFUHydrologyMqttEventDispatcher` handles events for the CH.BAFU.Hydrology.mqtt message group.
...
The CHBAFUHydrologyMqttEventDispatcher defines the following event handler hooks.
...
producer = CHBAFUHydrologyProducer(
    ...
    security_protocol='SASL_SSL',##### `ch_bafu_hydrology_mqtt_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'ch_bafu_hydrology_mqtt_station_async:  Callable[[ConsumerRecord, CloudEvent, Station],
Awaitable[None]]
...
Asynchronous handler hook for `CH.BAFU.Hydrology.mqtt.Station`: A reference record for one Swiss river, lake, and stream
gauge published by the Swiss Federal Office for the Environment (BAFU/FOEN). It fires when the bridge publishes or
refreshes the station catalog so consumers can interpret measurement events.
...
async def ch_bafu_hydrology_mqtt_station_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Station) -> None:
...
ch_bafu_hydrology_dispatcher.ch_bafu_hydrology_mqtt_station_async = ch_bafu_hydrology_mqtt_station_event
...
ch_bafu_hydrology_mqtt_dispatcher.ch_bafu_hydrology_mqtt_station_async = ch_bafu_hydrology_mqtt_station_event
...
ch_bafu_hydrology_amqp_dispatcher.ch_bafu_hydrology_mqtt_station_async = ch_bafu_hydrology_mqtt_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `ch_bafu_hydrology_mqtt_water_level_observation_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'ch_bafu_hydrology_mqtt_water_level_observation_async:  Callable[[ConsumerRecord,
...
Asynchronous handler hook for `CH.BAFU.Hydrology.mqtt.WaterLevelObservation`: A current measurement from the Swiss
Federal Office for the Environment (BAFU/FOEN) for one monitoring site. It carries water level, discharge, and water
temperature observations when the upstream feed reports a new or refreshed value.
...
async def ch_bafu_hydrology_mqtt_water_level_observation_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
WaterLevelObservation) -> None:
...
ch_bafu_hydrology_dispatcher.ch_bafu_hydrology_mqtt_water_level_observation_async =
ch_bafu_hydrology_mqtt_water_level_observation_event
...
ch_bafu_hydrology_mqtt_dispatcher.ch_bafu_hydrology_mqtt_water_level_observation_async =
ch_bafu_hydrology_mqtt_water_level_observation_event
...
ch_bafu_hydrology_amqp_dispatcher.ch_bafu_hydrology_mqtt_water_level_observation_async =
ch_bafu_hydrology_mqtt_water_level_observation_event
...
##### `send_ch_bafu_hydrology_mqtt_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_ch_bafu_hydrology_mqtt_station(
...
Send a single `CH.BAFU.Hydrology.mqtt.Station` message. A reference record for one Swiss river, lake, and stream gauge
published by the Swiss Federal Office for the Environment (BAFU/FOEN). It fires when the bridge publishes or refreshes
the station catalog so consumers can interpret measurement events.Args:
...
await producer.send_ch_bafu_hydrology_mqtt_station(
...
Send multiple `CH.BAFU.Hydrology.mqtt.Station` messages in a batch.
...
await producer.send_ch_bafu_hydrology_mqtt_station_batch(```
...
##### `send_ch_bafu_hydrology_mqtt_water_level_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_ch_bafu_hydrology_mqtt_water_level_observation(
...
Send a single `CH.BAFU.Hydrology.mqtt.WaterLevelObservation` message. A current measurement from the Swiss Federal
Office for the Environment (BAFU/FOEN) for one monitoring site. It carries water level, discharge, and water temperature
observations when the upstream feed reports a new or refreshed value.Args:
...
await producer.send_ch_bafu_hydrology_mqtt_water_level_observation(
...
Send multiple `CH.BAFU.Hydrology.mqtt.WaterLevelObservation` messages in a batch.
...
await producer.send_ch_bafu_hydrology_mqtt_water_level_observation_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### CHBAFUHydrologyAmqpEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`CHBAFUHydrologyAmqpEventDispatcher` handles events for the CH.BAFU.Hydrology.amqp message group.
...
The CHBAFUHydrologyAmqpEventDispatcher defines the following event handler hooks.
...
producer = CHBAFUHydrologyProducer(
    ...
    security_protocol='SASL_SSL',##### `ch_bafu_hydrology_amqp_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'ch_bafu_hydrology_amqp_station_async:  Callable[[ConsumerRecord, CloudEvent, Station],
Awaitable[None]]
...
Asynchronous handler hook for `CH.BAFU.Hydrology.amqp.Station`: A reference record for one Swiss river, lake, and stream
gauge published by the Swiss Federal Office for the Environment (BAFU/FOEN). It fires when the bridge publishes or
refreshes the station catalog so consumers can interpret measurement events.
...
async def ch_bafu_hydrology_amqp_station_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Station) -> None:
...
ch_bafu_hydrology_dispatcher.ch_bafu_hydrology_amqp_station_async = ch_bafu_hydrology_amqp_station_event
...
ch_bafu_hydrology_mqtt_dispatcher.ch_bafu_hydrology_amqp_station_async = ch_bafu_hydrology_amqp_station_event
...
ch_bafu_hydrology_amqp_dispatcher.ch_bafu_hydrology_amqp_station_async = ch_bafu_hydrology_amqp_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `ch_bafu_hydrology_amqp_water_level_observation_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'ch_bafu_hydrology_amqp_water_level_observation_async:  Callable[[ConsumerRecord,
CloudEvent, WaterLevelObservation], Awaitable[None]]
...
Asynchronous handler hook for `CH.BAFU.Hydrology.amqp.WaterLevelObservation`: A current measurement from the Swiss
Federal Office for the Environment (BAFU/FOEN) for one monitoring site. It carries water level, discharge, and water
temperature observations when the upstream feed reports a new or refreshed value.
...
async def ch_bafu_hydrology_amqp_water_level_observation_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
WaterLevelObservation) -> None:
...
ch_bafu_hydrology_dispatcher.ch_bafu_hydrology_amqp_water_level_observation_async =
ch_bafu_hydrology_amqp_water_level_observation_event
...
ch_bafu_hydrology_mqtt_dispatcher.ch_bafu_hydrology_amqp_water_level_observation_async =
ch_bafu_hydrology_amqp_water_level_observation_event
...
ch_bafu_hydrology_amqp_dispatcher.ch_bafu_hydrology_amqp_water_level_observation_async =
ch_bafu_hydrology_amqp_water_level_observation_event
...
##### `send_ch_bafu_hydrology_amqp_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_ch_bafu_hydrology_amqp_station(
...
Send a single `CH.BAFU.Hydrology.amqp.Station` message. A reference record for one Swiss river, lake, and stream gauge
published by the Swiss Federal Office for the Environment (BAFU/FOEN). It fires when the bridge publishes or refreshes
the station catalog so consumers can interpret measurement events.Args:
...
await producer.send_ch_bafu_hydrology_amqp_station(
...
Send multiple `CH.BAFU.Hydrology.amqp.Station` messages in a batch.
...
await producer.send_ch_bafu_hydrology_amqp_station_batch(```
...
##### `send_ch_bafu_hydrology_amqp_water_level_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_ch_bafu_hydrology_amqp_water_level_observation(
...
Send a single `CH.BAFU.Hydrology.amqp.WaterLevelObservation` message. A current measurement from the Swiss Federal
Office for the Environment (BAFU/FOEN) for one monitoring site. It carries water level, discharge, and water temperature
observations when the upstream feed reports a new or refreshed value.Args:
...
await producer.send_ch_bafu_hydrology_amqp_water_level_observation(
...
Send multiple `CH.BAFU.Hydrology.amqp.WaterLevelObservation` messages in a batch.
...
await producer.send_ch_bafu_hydrology_amqp_water_level_observation_batch(```
