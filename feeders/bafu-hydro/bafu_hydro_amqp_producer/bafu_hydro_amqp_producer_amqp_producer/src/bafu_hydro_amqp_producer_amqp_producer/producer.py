    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from bafu_hydro_amqp_producer_data import Station
from bafu_hydro_amqp_producer_data import WaterLevelObservation

class CHBAFUHydrologyAmqpProducer:
    """
    Producer class to send messages in the `CH.BAFU.Hydrology.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        data: Station,
        _station_id: str,
        _water_body_name: str,
        content_type: str = 'application/json') -> None:
        """
        Send the `CH.BAFU.Hydrology.amqp.Station` message
        A reference record for one Swiss river, lake, and stream gauge published by the Swiss Federal Office for the Environment (BAFU/FOEN). It fires when the bridge publishes or refreshes the station catalog so consumers can interpret measurement events.
        
        Args:
            _station_id (str): Value for placeholder station_id in attribute subject
            _water_body_name (str): Value for AMQP protocol option placeholder water_body_name
        ...
        app_properties = {}
        app_properties["water_body_name"] = "{water_body_name}".format(water_body_name=_water_body_name)
        if app_properties:
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `CH.BAFU.Hydrology.amqp.Station` messages
        ...
        for data in data_array:
            self.send_station(
                ...
                _water_body_name=_water_body_name,
    ...
    def send_water_level_observation(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `CH.BAFU.Hydrology.amqp.WaterLevelObservation` message
        A current measurement from the Swiss Federal Office for the Environment (BAFU/FOEN) for one monitoring site. It carries water level, discharge, and water temperature observations when the upstream feed reports a new or refreshed value.
    ...
    def send_water_level_observation_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `CH.BAFU.Hydrology.amqp.WaterLevelObservation` messages
