"""AMQP feeder application for BAFU Hydrology → Unified Namespace.
...
:class:`CHBAFUHydrologyAmqpProducer`.
...
from bafu_hydro_amqp_producer_data import Station, WaterLevelObservation
from bafu_hydro_amqp_producer_amqp_producer.producer import CHBAFUHydrologyAmqpProducer
...
def _publish_stations(
    producer: CHBAFUHydrologyAmqpProducer,
...
def _publish_mock(producer: CHBAFUHydrologyAmqpProducer) -> None:
    station = Station(station_id="mock-station", name="Mock Station", water_body_name="mock-river", water_body_type="river", latitude=47.0, longitude=8.0)
    observation = WaterLevelObservation(station_id="mock-station", water_body_name="Mock River", water_level=1.23, water_level_unit="m", water_level_timestamp=datetime.now(timezone.utc), discharge=12.3, discharge_unit="m3/s", discharge_timestamp=datetime.now(timezone.utc), water_temperature=9.5, water_temperature_unit="C", water_temperature_timestamp=datetime.now(timezone.utc))
    producer.send_station(data=station, _station_id="mock-station", _water_body_name="mock-river")
    producer.send_water_level_observation(data=observation, _station_id="mock-station", _water_body_name="mock-river")
...
def feed(
    ...
    *,
    ...
    _publish_stations(producer, locations)
    ...
    try:
        while True:
            try:
                ...
                count = _publish_observations(api, producer, locations, previous_readings)
...
def _build_producer(*, host: str, port: int, address: str, use_tls: bool, content_mode: str, auth_mode: str, username: Optional[str], password: Optional[str], entra_audience: str, entra_client_id: Optional[str], sas_key_name: Optional[str], sas_key: Optional[str]) -> CHBAFUHydrologyAmqpProducer:
    if auth_mode == "entra":
        ...
        return CHBAFUHydrologyAmqpProducer(host=host, address=address, port=port, content_mode=content_mode, credential=credential, entra_audience=entra_audience, use_tls=use_tls)  # type: ignore[arg-type]
    if auth_mode == "sas":
        if not sas_key_name or not sas_key:
            raise RuntimeError("auth-mode=sas requires AMQP_SAS_KEY_NAME and AMQP_SAS_KEY")
        return CHBAFUHydrologyAmqpProducer(host=host, address=address, port=port, content_mode=content_mode, sas_key_name=sas_key_name, sas_key=sas_key, use_tls=use_tls)  # type: ignore[arg-type]
    return CHBAFUHydrologyAmqpProducer(host=host, address=address, port=port, username=username, password=password, content_mode=content_mode, use_tls=use_tls)  # type: ignore[arg-type]
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="BAFU Hydrology → AMQP 1.0 bridge.")
    ...
    feed_parser.add_argument("--address", type=str, default=os.getenv("AMQP_ADDRESS", "bafu-hydro"))
    ...
    feed_parser.add_argument("--state-file", type=str,
                             default=os.getenv("STATE_FILE", os.path.expanduser("~/.bafu_hydro_amqp_state.json")))
