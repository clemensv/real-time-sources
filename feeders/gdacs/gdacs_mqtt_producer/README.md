# Gdacs_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the gdacs_mqtt_producer message definitions.

# Gdacs_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - GDACSAlertsMqttEventDispatcher
...
from gdacs_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = GDACSAlertsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### GDACSAlertsMqttEventDispatcher



# Publish message`GDACSAlertsMqttEventDispatcher` handles events for the GDACS.Alerts.mqtt message group.
...
from gdacs_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = GDACSAlertsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe GDACSAlertsMqttEventDispatcher defines the following event handler hooks.
...
```##### `gdacs_alerts_mqtt_disaster_alert_async`



## Test```python

gdacs_alerts_mqtt_disaster_alert_async:  Callable[[PartitionContext, EventData, CloudEvent, DisasterAlert],
Awaitable[None]]
...
- `data`: The event data of type `gdacs_mqtt_producer_data.DisasterAlert`.

Example:

```python
async def gdacs_alerts_mqtt_disaster_alert_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: DisasterAlert) -> None:
