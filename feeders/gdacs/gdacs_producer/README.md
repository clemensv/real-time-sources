...
# Gdacs-producer Kafka Producer# Gdacs-producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - GDACSAlertsEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    GDACSAlertsMqttEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    GDACSAlertsAmqpEventDispatcher
...
- GDACSAlertsProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- GDACSAlertsMqttProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- GDACSAlertsAmqpProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### GDACSAlertsEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`GDACSAlertsEventDispatcher` handles events for the GDACS.Alerts message group.
...
from gdacs-producer import GDACSAlertsProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = GDACSAlertsProducer(
...
await producer.send_gdacs_disaster_alert(```python

    data=DisasterAlert(...),add_consumer(self, consumer: KafkaConsumer)
...
The GDACSAlertsEventDispatcher defines the following event handler hooks.
...
producer = GDACSAlertsProducer(
    ...
    security_protocol='SASL_SSL',##### `gdacs_disaster_alert_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gdacs_disaster_alert_async:  Callable[[ConsumerRecord, CloudEvent, DisasterAlert],
Awaitable[None]]
...
Asynchronous handler hook for `GDACS.DisasterAlert`: A disaster alert from the Global Disaster Alert and Coordination
System (GDACS), covering earthquakes, tropical cyclones, floods, volcanoes, forest fires, and droughts worldwide.
...
### GDACSAlertsProducer- `data`: The event data of type `gdacs_producer_data.DisasterAlert`.



Producer for `GDACS.Alerts` message group.Example:



#### Constructor```python

async def gdacs_disaster_alert_event(record: ConsumerRecord, cloud_event: CloudEvent, data: DisasterAlert) -> None:

```python    # Process the event data

GDACSAlertsProducer(    await some_processing_function(record, cloud_event, data)
...
gdacs_alerts_dispatcher.gdacs_disaster_alert_async = gdacs_disaster_alert_event
...
### GDACSAlertsMqttProducer- `data`: The event data of type `gdacs_producer_data.DisasterAlert`.



Producer for `GDACS.Alerts.mqtt` message group.Example:
...
GDACSAlertsMqttProducer(    await some_processing_function(record, cloud_event, data)
...
gdacs_alerts_mqtt_dispatcher.gdacs_disaster_alert_async = gdacs_disaster_alert_event
...
### GDACSAlertsAmqpProducer- `data`: The event data of type `gdacs_producer_data.DisasterAlert`.



Producer for `GDACS.Alerts.amqp` message group.Example:
...
GDACSAlertsAmqpProducer(    await some_processing_function(record, cloud_event, data)
...
gdacs_alerts_amqp_dispatcher.gdacs_disaster_alert_async = gdacs_disaster_alert_event
...
##### `send_gdacs_disaster_alert`Dispatchers have the following protected methods:



```python### Methods:

async def send_gdacs_disaster_alert(
...
Send a single `GDACS.DisasterAlert` message. A disaster alert from the Global Disaster Alert and Coordination System
(GDACS), covering earthquakes, tropical cyclones, floods, volcanoes, forest fires, and droughts worldwide.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `DisasterAlert`
...
await producer.send_gdacs_disaster_alert(

    data=DisasterAlert(...),Args:
...
Send multiple `GDACS.DisasterAlert` messages in a batch.
...
await producer.send_gdacs_disaster_alert_batch(```

    messages=[

        DisasterAlert(...),Initializes the runner with a Kafka consumer.
...
- **Scales horizontally** across multiple brokers and partitions### GDACSAlertsMqttEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`GDACSAlertsMqttEventDispatcher` handles events for the GDACS.Alerts.mqtt message group.
...
The GDACSAlertsMqttEventDispatcher defines the following event handler hooks.
...
producer = GDACSAlertsProducer(
    ...
    security_protocol='SASL_SSL',##### `gdacs_alerts_mqtt_disaster_alert_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gdacs_alerts_mqtt_disaster_alert_async:  Callable[[ConsumerRecord, CloudEvent,
DisasterAlert], Awaitable[None]]

)```

```

Asynchronous handler hook for `GDACS.Alerts.mqtt.DisasterAlert`: A disaster alert from the Global Disaster Alert and
Coordination System (GDACS), covering earthquakes, tropical cyclones, floods, volcanoes, forest fires, and droughts
worldwide.
...
async def gdacs_alerts_mqtt_disaster_alert_event(record: ConsumerRecord, cloud_event: CloudEvent, data: DisasterAlert)
-> None:
...
gdacs_alerts_dispatcher.gdacs_alerts_mqtt_disaster_alert_async = gdacs_alerts_mqtt_disaster_alert_event
...
gdacs_alerts_mqtt_dispatcher.gdacs_alerts_mqtt_disaster_alert_async = gdacs_alerts_mqtt_disaster_alert_event
...
gdacs_alerts_amqp_dispatcher.gdacs_alerts_mqtt_disaster_alert_async = gdacs_alerts_mqtt_disaster_alert_event
...
##### `send_gdacs_alerts_mqtt_disaster_alert`Dispatchers have the following protected methods:



```python### Methods:

async def send_gdacs_alerts_mqtt_disaster_alert(
...
Send a single `GDACS.Alerts.mqtt.DisasterAlert` message. A disaster alert from the Global Disaster Alert and
Coordination System (GDACS), covering earthquakes, tropical cyclones, floods, volcanoes, forest fires, and droughts
worldwide.Args:
...
await producer.send_gdacs_alerts_mqtt_disaster_alert(
...
Send multiple `GDACS.Alerts.mqtt.DisasterAlert` messages in a batch.
...
await producer.send_gdacs_alerts_mqtt_disaster_alert_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### GDACSAlertsAmqpEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`GDACSAlertsAmqpEventDispatcher` handles events for the GDACS.Alerts.amqp message group.
...
The GDACSAlertsAmqpEventDispatcher defines the following event handler hooks.
...
producer = GDACSAlertsProducer(
    ...
    security_protocol='SASL_SSL',##### `gdacs_alerts_amqp_disaster_alert_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gdacs_alerts_amqp_disaster_alert_async:  Callable[[ConsumerRecord, CloudEvent,
DisasterAlert], Awaitable[None]]
...
Asynchronous handler hook for `GDACS.Alerts.amqp.DisasterAlert`: A disaster alert from the Global Disaster Alert and
Coordination System (GDACS), covering earthquakes, tropical cyclones, floods, volcanoes, forest fires, and droughts
worldwide.
...
async def gdacs_alerts_amqp_disaster_alert_event(record: ConsumerRecord, cloud_event: CloudEvent, data: DisasterAlert)
-> None:
...
gdacs_alerts_dispatcher.gdacs_alerts_amqp_disaster_alert_async = gdacs_alerts_amqp_disaster_alert_event
...
gdacs_alerts_mqtt_dispatcher.gdacs_alerts_amqp_disaster_alert_async = gdacs_alerts_amqp_disaster_alert_event
...
gdacs_alerts_amqp_dispatcher.gdacs_alerts_amqp_disaster_alert_async = gdacs_alerts_amqp_disaster_alert_event
...
##### `send_gdacs_alerts_amqp_disaster_alert`Dispatchers have the following protected methods:



```python### Methods:

async def send_gdacs_alerts_amqp_disaster_alert(
...
Send a single `GDACS.Alerts.amqp.DisasterAlert` message. A disaster alert from the Global Disaster Alert and
Coordination System (GDACS), covering earthquakes, tropical cyclones, floods, volcanoes, forest fires, and droughts
worldwide.Args:
...
await producer.send_gdacs_alerts_amqp_disaster_alert(
...
Send multiple `GDACS.Alerts.amqp.DisasterAlert` messages in a batch.
...
await producer.send_gdacs_alerts_amqp_disaster_alert_batch(```
