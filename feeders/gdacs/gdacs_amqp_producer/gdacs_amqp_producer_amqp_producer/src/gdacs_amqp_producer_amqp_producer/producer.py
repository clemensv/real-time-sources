...
class GDACSAlertsAmqpProducer:
    """
    Producer class to send messages in the `GDACS.Alerts.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_disaster_alert(self,
        ...
        """
        Send the `GDACS.Alerts.amqp.DisasterAlert` message
        ...
        Args:
            ...
            _alert_color (str): Value for AMQP protocol option placeholder alert_color
        ...
        attributes = {
            ...
            "https://www.gdacs.org",
            "subject":
            "{event_type}/{event_id}".format(event_type=_event_type, event_id=_event_id),
        }
        ...
        amqp_msg.subject = "{event_type}/{event_id}".format(event_type=_event_type, event_id=_event_id)

        app_properties = {}
        app_properties["event_type"] = "{event_type}".format(event_type=_event_type)
        app_properties["event_id"] = "{event_id}".format(event_id=_event_id)
        app_properties["alert_color"] = "{alert_color}".format(alert_color=_alert_color)
        ...
        annotations = {}
        annotation_value = "{event_type}/{event_id}".format(event_type=_event_type, event_id=_event_id)
    ...
    def send_disaster_alert_batch(self,
        data_array: typing.List[DisasterAlert],
        _event_type: str,
        _event_id: str,
        _alert_color: str,
        _country: str,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `GDACS.Alerts.amqp.DisasterAlert` messages
        
        Args:
            data_array (typing.List[DisasterAlert]): Array of message data objects
            _event_type (str): Value for placeholder event_type in attribute subject
            _event_id (str): Value for placeholder event_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            _alert_color (str): Value for AMQP protocol option placeholder alert_color
        ...
        for data in data_array:
            self.send_disaster_alert(
                data=data,
                _event_type=_event_type,
                _event_id=_event_id,
                _time=_time,
                _alert_color=_alert_color,
                _country=_country,
