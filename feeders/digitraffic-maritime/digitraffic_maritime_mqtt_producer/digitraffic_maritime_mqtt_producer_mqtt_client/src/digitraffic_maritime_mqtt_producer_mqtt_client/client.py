...
def get_default_topic_mappings_fi_digitraffic_marine_ais_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the fi.digitraffic.marine.ais.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "fi.digitraffic.marine.ais.mqtt.location": "maritime/fi/fintraffic/digitraffic-maritime/vessels/{mmsi}/location",
        "fi.digitraffic.marine.ais.mqtt.metadata": "maritime/fi/fintraffic/digitraffic-maritime/vessels/{mmsi}/metadata",
    }


def get_subscription_topics_fi_digitraffic_marine_ais_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_fi_digitraffic_marine_ais_mqtt()
    topics = []
...
class FiDigitrafficMarineAisMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the fi.digitraffic.marine.ais.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_fi_digitraffic_marine_ais_mqtt()
        ...
        self.fi_digitraffic_marine_ais_mqtt_location_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_maritime_mqtt_producer_data.VesselLocation, Dict[str, str]], Awaitable[None]]] = None
        
        self.fi_digitraffic_marine_ais_mqtt_metadata_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_maritime_mqtt_producer_data.VesselMetadata, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "fi.digitraffic.marine.ais.VesselLocation":
            if self.fi_digitraffic_marine_ais_mqtt_location_async:
                try:
                    ...
                    data = digitraffic_maritime_mqtt_producer_data.VesselLocation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.marine.ais.mqtt.location")
                    await self.fi_digitraffic_marine_ais_mqtt_location_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_marine_ais_mqtt_location handler: {e}")
            return
        
        if event_type == "fi.digitraffic.marine.ais.VesselMetadata":
            if self.fi_digitraffic_marine_ais_mqtt_metadata_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = digitraffic_maritime_mqtt_producer_data.VesselMetadata.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.marine.ais.mqtt.metadata")
                    await self.fi_digitraffic_marine_ais_mqtt_metadata_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_marine_ais_mqtt_metadata handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_fi_digitraffic_marine_ais_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_fi_digitraffic_marine_ais_mqtt_location(self,
        mmsi: str,
        data: digitraffic_maritime_mqtt_producer_data.VesselLocation,
        ...
        """
        Publish the 'fi.digitraffic.marine.ais.mqtt.location' event to an MQTT topic.

        Args:
        
            mmsi: URI template variable for 'mmsi'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'maritime/fi/fintraffic/digitraffic-maritime/vessels/{mmsi}/location'
        ...
        """
        target_topic = topic if topic is not None else "maritime/fi/fintraffic/digitraffic-maritime/vessels/{mmsi}/location"
    ...
    async def publish_fi_digitraffic_marine_ais_mqtt_metadata(self,
        mmsi: str,
        data: digitraffic_maritime_mqtt_producer_data.VesselMetadata,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'fi.digitraffic.marine.ais.mqtt.metadata' event to an MQTT topic.

        Args:
        
            mmsi: URI template variable for 'mmsi'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'maritime/fi/fintraffic/digitraffic-maritime/vessels/{mmsi}/metadata'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "maritime/fi/fintraffic/digitraffic-maritime/vessels/{mmsi}/metadata"
        _topic_template_values: Dict[str, str] = {
            "mmsi": str(mmsi),
        }
        if _topic_template_values:
...
def get_default_topic_mappings_fi_digitraffic_marine_portcall_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the fi.digitraffic.marine.portcall.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "fi.digitraffic.marine.portcall.mqtt.port_call": "maritime/fi/fintraffic/digitraffic-maritime/port-calls/{port_call_id}/port-call",
    }


def get_subscription_topics_fi_digitraffic_marine_portcall_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_fi_digitraffic_marine_portcall_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class FiDigitrafficMarinePortcallMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the fi.digitraffic.marine.portcall.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_fi_digitraffic_marine_portcall_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.fi_digitraffic_marine_portcall_mqtt_port_call_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_maritime_mqtt_producer_data.PortCall, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "fi.digitraffic.marine.portcall.PortCall":
            if self.fi_digitraffic_marine_portcall_mqtt_port_call_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = digitraffic_maritime_mqtt_producer_data.PortCall.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.marine.portcall.mqtt.port_call")
                    await self.fi_digitraffic_marine_portcall_mqtt_port_call_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_marine_portcall_mqtt_port_call handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_fi_digitraffic_marine_portcall_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_fi_digitraffic_marine_portcall_mqtt_port_call(self,
        port_call_id: str,
        data: digitraffic_maritime_mqtt_producer_data.PortCall,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'fi.digitraffic.marine.portcall.mqtt.port_call' event to an MQTT topic.

        Args:
        
            port_call_id: URI template variable for 'port_call_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'maritime/fi/fintraffic/digitraffic-maritime/port-calls/{port_call_id}/port-call'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            ...
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "maritime/fi/fintraffic/digitraffic-maritime/port-calls/{port_call_id}/port-call"
        _topic_template_values: Dict[str, str] = {
            "port_call_id": str(port_call_id),
        }
        if _topic_template_values:
            target_topic = _apply_topic_template(target_topic, _topic_template_values)
        ...
        attributes = {
             ...
             "subject":"{port_call_id}".format(port_call_id = port_call_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
...
def get_default_topic_mappings_fi_digitraffic_marine_portcall_vesseldetails_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the fi.digitraffic.marine.portcall.vesseldetails.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "fi.digitraffic.marine.portcall.vesseldetails.mqtt.vessel_details": "maritime/fi/fintraffic/digitraffic-maritime/vessels/{vessel_id}/vessel-details",
    }


def get_subscription_topics_fi_digitraffic_marine_portcall_vesseldetails_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_fi_digitraffic_marine_portcall_vesseldetails_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class FiDigitrafficMarinePortcallVesseldetailsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the fi.digitraffic.marine.portcall.vesseldetails.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_fi_digitraffic_marine_portcall_vesseldetails_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.fi_digitraffic_marine_portcall_vesseldetails_mqtt_vessel_details_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_maritime_mqtt_producer_data.VesselDetails, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "fi.digitraffic.marine.portcall.VesselDetails":
            if self.fi_digitraffic_marine_portcall_vesseldetails_mqtt_vessel_details_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = digitraffic_maritime_mqtt_producer_data.VesselDetails.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.marine.portcall.vesseldetails.mqtt.vessel_details")
                    await self.fi_digitraffic_marine_portcall_vesseldetails_mqtt_vessel_details_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_marine_portcall_vesseldetails_mqtt_vessel_details handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_fi_digitraffic_marine_portcall_vesseldetails_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_fi_digitraffic_marine_portcall_vesseldetails_mqtt_vessel_details(self,
        vessel_id: str,
        data: digitraffic_maritime_mqtt_producer_data.VesselDetails,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'fi.digitraffic.marine.portcall.vesseldetails.mqtt.vessel_details' event to an MQTT topic.

        Args:
        
            vessel_id: URI template variable for 'vessel_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'maritime/fi/fintraffic/digitraffic-maritime/vessels/{vessel_id}/vessel-details'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "maritime/fi/fintraffic/digitraffic-maritime/vessels/{vessel_id}/vessel-details"
        _topic_template_values: Dict[str, str] = {
...
def get_default_topic_mappings_fi_digitraffic_marine_portcall_portlocation_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the fi.digitraffic.marine.portcall.portlocation.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "fi.digitraffic.marine.portcall.portlocation.mqtt.port_location": "maritime/fi/fintraffic/digitraffic-maritime/ports/{locode}/port-location",
    }


def get_subscription_topics_fi_digitraffic_marine_portcall_portlocation_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_fi_digitraffic_marine_portcall_portlocation_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class FiDigitrafficMarinePortcallPortlocationMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the fi.digitraffic.marine.portcall.portlocation.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_fi_digitraffic_marine_portcall_portlocation_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.fi_digitraffic_marine_portcall_portlocation_mqtt_port_location_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, digitraffic_maritime_mqtt_producer_data.PortLocation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "fi.digitraffic.marine.portcall.PortLocation":
            if self.fi_digitraffic_marine_portcall_portlocation_mqtt_port_location_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = digitraffic_maritime_mqtt_producer_data.PortLocation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.digitraffic.marine.portcall.portlocation.mqtt.port_location")
                    await self.fi_digitraffic_marine_portcall_portlocation_mqtt_port_location_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_digitraffic_marine_portcall_portlocation_mqtt_port_location handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_fi_digitraffic_marine_portcall_portlocation_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_fi_digitraffic_marine_portcall_portlocation_mqtt_port_location(self,
        locode: str,
        data: digitraffic_maritime_mqtt_producer_data.PortLocation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'fi.digitraffic.marine.portcall.portlocation.mqtt.port_location' event to an MQTT topic.

        Args:
        
            locode: URI template variable for 'locode'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'maritime/fi/fintraffic/digitraffic-maritime/ports/{locode}/port-location'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "maritime/fi/fintraffic/digitraffic-maritime/ports/{locode}/port-location"
        _topic_template_values: Dict[str, str] = {
            "locode": str(locode),
        }
        if _topic_template_values:
        ...
        attributes = {
             ...
             "subject":"{locode}".format(locode = locode)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
