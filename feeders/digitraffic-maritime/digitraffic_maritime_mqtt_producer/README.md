# Digitraffic_maritime_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the digitraffic_maritime_mqtt_producer message definitions.

# Digitraffic_maritime_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - FiDigitrafficMarineAisMqttEventDispatcher,

    FiDigitrafficMarinePortcallMqttEventDispatcher,
...
from digitraffic_maritime_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = FiDigitrafficMarineAisMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### FiDigitrafficMarineAisMqttEventDispatcher



# Publish message`FiDigitrafficMarineAisMqttEventDispatcher` handles events for the fi.digitraffic.marine.ais.mqtt
message group.
...
from digitraffic_maritime_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = FiDigitrafficMarineAisMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
client = FiDigitrafficMarinePortcallMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of
the Event Hub.Args:- `consumer_group_name`: The name of the consumer group.- `eventhubs_fully_qualified_namespace`: The
fully qualified namespace of the Event Hub.

client = FiDigitrafficMarinePortcallVesseldetailsMqttMqttClient(mqtt_client, content_mode='structured')-
`eventhub_name`: The name of the Event Hub.Args:- `consumer_group_name`: The name of the consumer group.-
`eventhubs_fully_qualified_namespace`: The fully qualified namespace of the Event Hub.

client = FiDigitrafficMarinePortcallPortlocationMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`:
The name of the Event Hub.- `blob_account_url`: The URL of the Azure Storage account.
...
## BuildThe FiDigitrafficMarineAisMqttEventDispatcher defines the following event handler hooks.
...
```##### `fi_digitraffic_marine_ais_mqtt_location_async`



## Test```python

fi_digitraffic_marine_ais_mqtt_location_async:  Callable[[PartitionContext, EventData, CloudEvent, VesselLocation],
Awaitable[None]]
...
- `data`: The event data of type `digitraffic_maritime_mqtt_producer_data.VesselLocation`.

Example:

```python
async def fi_digitraffic_marine_ais_mqtt_location_event(partition_context: PartitionContext, event: EventData,
...
```##### `fi_digitraffic_marine_ais_mqtt_metadata_async`



## Test```python

fi_digitraffic_marine_ais_mqtt_metadata_async:  Callable[[PartitionContext, EventData, CloudEvent, VesselMetadata],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `digitraffic_maritime_mqtt_producer_data.VesselMetadata`.

Example:

```python
async def fi_digitraffic_marine_ais_mqtt_metadata_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: VesselMetadata) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = FiDigitrafficMarinePortcallMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### FiDigitrafficMarinePortcallMqttEventDispatcher



# Publish message`FiDigitrafficMarinePortcallMqttEventDispatcher` handles events for the
fi.digitraffic.marine.portcall.mqtt message group.
...
## BuildThe FiDigitrafficMarinePortcallMqttEventDispatcher defines the following event handler hooks.
...
```##### `fi_digitraffic_marine_portcall_mqtt_port_call_async`



## Test```python

fi_digitraffic_marine_portcall_mqtt_port_call_async:  Callable[[PartitionContext, EventData, CloudEvent, PortCall],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `digitraffic_maritime_mqtt_producer_data.PortCall`.

Example:

```python
async def fi_digitraffic_marine_portcall_mqtt_port_call_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: PortCall) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
await client.connect("mqtt.example.com", 1883)### FiDigitrafficMarinePortcallVesseldetailsMqttEventDispatcher



# Publish message`FiDigitrafficMarinePortcallVesseldetailsMqttEventDispatcher` handles events for the
...
## BuildThe FiDigitrafficMarinePortcallVesseldetailsMqttEventDispatcher defines the following event handler hooks.
...
```##### `fi_digitraffic_marine_portcall_vesseldetails_mqtt_vessel_details_async`



## Test```python

fi_digitraffic_marine_portcall_vesseldetails_mqtt_vessel_details_async:  Callable[[PartitionContext, EventData,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `digitraffic_maritime_mqtt_producer_data.VesselDetails`.

Example:

```python
async def fi_digitraffic_marine_portcall_vesseldetails_mqtt_vessel_details_event(partition_context: PartitionContext,
event: EventData, cloud_event: CloudEvent, data: VesselDetails) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = FiDigitrafficMarinePortcallPortlocationMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### FiDigitrafficMarinePortcallPortlocationMqttEventDispatcher



# Publish message`FiDigitrafficMarinePortcallPortlocationMqttEventDispatcher` handles events for the
...
## BuildThe FiDigitrafficMarinePortcallPortlocationMqttEventDispatcher defines the following event handler hooks.
...
```##### `fi_digitraffic_marine_portcall_portlocation_mqtt_port_location_async`



## Test```python

fi_digitraffic_marine_portcall_portlocation_mqtt_port_location_async:  Callable[[PartitionContext, EventData,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `digitraffic_maritime_mqtt_producer_data.PortLocation`.

Example:

```python
async def fi_digitraffic_marine_portcall_portlocation_mqtt_port_location_event(partition_context: PartitionContext,
event: EventData, cloud_event: CloudEvent, data: PortLocation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
