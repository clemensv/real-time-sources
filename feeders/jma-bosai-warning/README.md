...
<sub>per-prefecture weather warnings + tsunami alerts · Kafka · MQTT · AMQP · <a href="https://www.jma.go.jp/bosai/map.html?contents=warning">upstream</a> · <a href="https://www.jma.go.jp/bosai/warning/">API docs</a></sub>
...
[🚀 **Deploy to Azure**](https://clemensv.github.io/real-time-sources#jma-bosai-warning) &nbsp;·&nbsp;
[📓 **Fabric Notebook**](https://clemensv.github.io/real-time-sources#jma-bosai-warning/fabric-notebook) &nbsp;·&nbsp;
[🐳 **docker pull**](CONTAINER.md) &nbsp;·&nbsp;
[📑 **Event schemas**](EVENTS.md) &nbsp;·&nbsp;
[🗄️ **KQL schema**](kql/jma-bosai-warning.kql) &nbsp;·&nbsp;
[↗ **Upstream**](https://www.jma.go.jp/bosai/map.html?contents=warning)

</td></tr></table>
<!-- source-hero:end -->

This feeder turns JMA Bosai weather warnings and tsunami alerts into a real-time CloudEvents stream over Apache Kafka, MQTT 5.0 (Unified Namespace), and AMQP 1.0.
...
**JMA Bosai Warning** ships three transport variants from one source bridge:

| Variant | Container image | Transport | Default delivery shape |
|---|---|---|---|
| **Kafka** | `ghcr.io/clemensv/real-time-sources-jma-bosai-warning` | Apache Kafka 2.x compatible (Azure Event Hubs, Fabric Event Streams, Confluent Cloud, plain Kafka) | One topic, JSON CloudEvents (binary mode) |
| **MQTT** | `ghcr.io/clemensv/real-time-sources-jma-bosai-warning-mqtt` | MQTT 5.0 broker (Mosquitto, EMQX, HiveMQ, Azure Event Grid MQTT, Fabric MQTT broker) | Unified-Namespace topic tree defined in `xreg/jma-bosai-warning.xreg.json` |
| **AMQP** | `ghcr.io/clemensv/real-time-sources-jma-bosai-warning-amqp` | AMQP 1.0 (RabbitMQ AMQP 1.0 plugin, Artemis, Qpid Dispatch, Azure Service Bus / Event Hubs) | Single AMQP address, binary CloudEvents |

All three variants share the same source contract in `xreg/jma-bosai-warning.xreg.json`.
...
```text
jma-bosai-warning/
  xreg/jma-bosai-warning.xreg.json
  jma_bosai_warning/
  jma_bosai_warning_mqtt/
  jma_bosai_warning_amqp/
  jma_bosai_warning_producer/
  jma_bosai_warning_mqtt_producer/
  jma_bosai_warning_amqp_producer/
  Dockerfile
  Dockerfile.mqtt
  Dockerfile.amqp
  notebook/
...
docker run --rm \
  -v "$PWD/state:/state" \
  -e STATE_FILE=/state/jma-bosai-warning.json \
  -e CONNECTION_STRING="<event-hubs-or-fabric-connection-string>" \
  ghcr.io/clemensv/real-time-sources-jma-bosai-warning:latest
```
...
docker run --rm \
  -v "$PWD/state:/state" \
  -e JMA_BOSAI_WARNING_MQTT_STATE_FILE=/state/jma-bosai-warning.json \
  -e MQTT_BROKER_URL="mqtts://<broker-host>:8883" \
  -e MQTT_USERNAME="<username>" \
  -e MQTT_PASSWORD="<password>" \
  ghcr.io/clemensv/real-time-sources-jma-bosai-warning-mqtt:latest
```
...
docker run --rm \
  -v "$PWD/state:/state" \
  -e JMA_BOSAI_WARNING_AMQP_STATE_FILE=/state/jma-bosai-warning.json \
  -e AMQP_BROKER_URL="amqp://<user>:<password>@<broker-host>:5672/jma-bosai-warning" \
  ghcr.io/clemensv/real-time-sources-jma-bosai-warning-amqp:latest
```
...
Japan Meteorological Agency Bosai warning and tsunami feeds.
...
JMA Bosai Warning & Tsunami targets Microsoft Fabric end-to-end: events land in a Fabric **Event Stream** (custom endpoint), an attached **Eventhouse / KQL database** materializes the contract from [`kql/`](kql/).

Two hosting models are supported. Use the deploy buttons on the [project portal](https://clemensv.github.io/real-time-sources#jma-bosai-warning) to launch either — both walk you through the same Fabric workspace selection and follow-up steps.

#### Fabric Notebook feeder &nbsp;<sub><i>(recommended for low-volume polling)</i></sub>

A scheduled Fabric Notebook in [`notebook/`](notebook/) runs the poller inside the Fabric workspace itself, against a per-source Fabric **Environment** that bundles the `jma_bosai_warning` package and the generated producer sub-packages. The Event Stream custom-endpoint connection string is looked up at runtime via the public Fabric Topology API using the workspace identity — no secrets in the notebook, no separate container host to manage. Dedupe state lives in OneLake under `/lakehouse/default/Files/feeder-state/jma-bosai-warning/`.
...
[![Deploy Fabric Notebook](https://img.shields.io/badge/Fabric-Notebook%20Feeder-117865?logo=microsoftfabric&logoColor=white)](https://clemensv.github.io/real-time-sources#jma-bosai-warning/fabric-notebook)
...
[![Deploy Fabric ACI](https://img.shields.io/badge/Fabric-Container%20Feeder-117865?logo=microsoftfabric&logoColor=white)](https://clemensv.github.io/real-time-sources#jma-bosai-warning/fabric-aci)
...
#### Kafka — bring your own Event Hubs / Kafka endpoint

Deploy the Kafka container against an existing Azure Event Hubs namespace, Fabric Event Stream custom endpoint, or another Kafka-compatible endpoint by providing the destination connection string at deployment time.

[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fjma-bosai-warning%2Fazure-template.json)

#### Kafka — provision a new Event Hubs namespace

Deploy the Kafka container together with a new Azure Event Hubs namespace and event hub. The template wires the generated connection string into the container automatically.

[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fjma-bosai-warning%2Fazure-template-with-eventhub.json)
...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fjma-bosai-warning%2Fazure-template-mqtt.json)
...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fjma-bosai-warning%2Fazure-template-with-eventgrid-mqtt.json)
...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fjma-bosai-warning%2Fazure-template-with-servicebus.json)
...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fjma-bosai-warning%2Fazure-template-amqp.json)
