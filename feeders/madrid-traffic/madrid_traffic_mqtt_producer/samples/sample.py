...
import os
import signal
from madrid_traffic_mqtt_producer_data import *
from madrid_traffic_mqtt_producer_mqtt_client.client import EsMadridInformoMqttProducer, EsMadridInformoMqttDispatcher

async def handle_es_madrid_informo_measurement_point_mqtt(mqtt_msg,cloud_event, es_madrid_informo_measurement_point_mqtt_data):
    """ Handles the es.madrid.informo.MeasurementPoint.mqtt message """
    print(f"Received es.madrid.informo.MeasurementPoint.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {es_madrid_informo_measurement_point_mqtt_data}")

async def handle_es_madrid_informo_traffic_reading_mqtt(mqtt_msg,cloud_event, es_madrid_informo_traffic_reading_mqtt_data):
    """ Handles the es.madrid.informo.TrafficReading.mqtt message """
    print(f"Received es.madrid.informo.TrafficReading.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {es_madrid_informo_traffic_reading_mqtt_data}")
