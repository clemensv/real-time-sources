...
def get_default_topic_mappings_es_madrid_informo_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the es.madrid.informo.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "es.madrid.informo.MeasurementPoint.mqtt": "traffic/es/madrid/madrid-traffic/{district}/{sensor_id}/info",
        "es.madrid.informo.TrafficReading.mqtt": "traffic/es/madrid/madrid-traffic/{district}/{sensor_id}/reading",
    }


def get_subscription_topics_es_madrid_informo_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_es_madrid_informo_mqtt()
    topics = []
...
class EsMadridInformoMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the es.madrid.informo.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_es_madrid_informo_mqtt()
        ...
        self.es_madrid_informo_measurement_point_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, madrid_traffic_mqtt_producer_data.MeasurementPoint, Dict[str, str]], Awaitable[None]]] = None
        
        self.es_madrid_informo_traffic_reading_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, madrid_traffic_mqtt_producer_data.TrafficReading, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "es.madrid.informo.MeasurementPoint":
            if self.es_madrid_informo_measurement_point_mqtt_async:
                try:
                    ...
                    data = madrid_traffic_mqtt_producer_data.MeasurementPoint.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "es.madrid.informo.MeasurementPoint.mqtt")
                    await self.es_madrid_informo_measurement_point_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in es_madrid_informo_measurement_point_mqtt handler: {e}")
            return
        
        if event_type == "es.madrid.informo.TrafficReading":
            if self.es_madrid_informo_traffic_reading_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = madrid_traffic_mqtt_producer_data.TrafficReading.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "es.madrid.informo.TrafficReading.mqtt")
                    await self.es_madrid_informo_traffic_reading_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in es_madrid_informo_traffic_reading_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_es_madrid_informo_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_es_madrid_informo_measurement_point_mqtt(self,
        sensor_id: str,
        district: str,
        data: madrid_traffic_mqtt_producer_data.MeasurementPoint,
        ...
        """
        Publish the 'es.madrid.informo.MeasurementPoint.mqtt' event to an MQTT topic.

        Args:
        
            sensor_id: URI template variable for 'sensor_id'
            district: URI template variable for 'district'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/es/madrid/madrid-traffic/{district}/{sensor_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "traffic/es/madrid/madrid-traffic/{district}/{sensor_id}/info"
    ...
    async def publish_es_madrid_informo_traffic_reading_mqtt(self,
        sensor_id: str,
        district: str,
        data: madrid_traffic_mqtt_producer_data.TrafficReading,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'es.madrid.informo.TrafficReading.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            district: URI template variable for 'district'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/es/madrid/madrid-traffic/{district}/{sensor_id}/reading'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            ...
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "traffic/es/madrid/madrid-traffic/{district}/{sensor_id}/reading"
        _topic_template_values: Dict[str, str] = {
            "sensor_id": str(sensor_id),
            "district": str(district),
        }
