...
class EsMadridInformoEventProducer:
    ...
    def send_es_madrid_informo_measurement_point(self,_sensor_id : str, data: MeasurementPoint, content_type: str = "application/json", _time: typing.Optional[typing.Union[str, datetime]] = None, flush_producer=True, key_mapper: typing.Callable[[CloudEvent, MeasurementPoint], str]=None) -> None:
        """
        Sends the 'es.madrid.informo.MeasurementPoint' event to the Kafka topic

        Args:
            _sensor_id(str):  Value for placeholder sensor_id in attribute subject
            data: (MeasurementPoint): The event data to be sent
            content_type (str): The content type that the event data shall be sent with
            _time(typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            flush_producer(bool): Whether to flush the producer after sending the event (default: True)
            key_mapper(Callable[[CloudEvent, MeasurementPoint], str]): A function to map the CloudEvent contents to a Kafka key (default: None).
    ...
    def send_es_madrid_informo_traffic_reading(self,_sensor_id : str, data: TrafficReading, content_type: str = "application/json", _time: typing.Optional[typing.Union[str, datetime]] = None, flush_producer=True, key_mapper: typing.Callable[[CloudEvent, TrafficReading], str]=None) -> None:
        """
        Sends the 'es.madrid.informo.TrafficReading' event to the Kafka topic

        Args:
            _sensor_id(str):  Value for placeholder sensor_id in attribute subject
            data: (TrafficReading): The event data to be sent
            content_type (str): The content type that the event data shall be sent with
            _time(typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            flush_producer(bool): Whether to flush the producer after sending the event (default: True)
            key_mapper(Callable[[CloudEvent, TrafficReading], str]): A function to map the CloudEvent contents to a Kafka key (default: None).
                The default key is derived from the xRegistry Kafka key declaration '{sensor_id}'
        """
        kafka_key = "{sensor_id}".format(sensor_id=_sensor_id)
        attributes = {
    ...
    @classmethod
    def from_connection_string(cls, connection_string: str, topic: typing.Optional[str]=None, content_mode: typing.Literal['structured','binary']='structured') -> 'EsMadridInformoEventProducer':
        """
