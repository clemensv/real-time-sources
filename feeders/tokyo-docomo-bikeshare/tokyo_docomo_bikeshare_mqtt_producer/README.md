# Tokyo_docomo_bikeshare_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the tokyo_docomo_bikeshare_mqtt_producer message definitions.

# Tokyo_docomo_bikeshare_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - JPODPTDocomoBikeshareSystemMqttEventDispatcher,

    JPODPTDocomoBikeshareStationsMqttEventDispatcher
...
from tokyo_docomo_bikeshare_mqtt_producer_mqtt_client import *This module defines an event processing framework for
Azure Event Hubs,
...
client = JPODPTDocomoBikeshareSystemMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### JPODPTDocomoBikeshareSystemMqttEventDispatcher



# Publish message`JPODPTDocomoBikeshareSystemMqttEventDispatcher` handles events for the
JP.ODPT.DocomoBikeshare.System.mqtt message group.
...
from tokyo_docomo_bikeshare_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = JPODPTDocomoBikeshareSystemMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of
...
client = JPODPTDocomoBikeshareStationsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name
...
## BuildThe JPODPTDocomoBikeshareSystemMqttEventDispatcher defines the following event handler hooks.
...
```##### `jp_odpt_docomo_bikeshare_bikeshare_system_mqtt_async`



## Test```python

jp_odpt_docomo_bikeshare_bikeshare_system_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
BikeshareSystem], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `JP.ODPT.DocomoBikeshare.BikeshareSystem.mqtt`: A transport update from Docomo Bike
Share open feeds. It carries station status and availability updates for Tokyo bike-share stations.
...
- `data`: The event data of type `tokyo_docomo_bikeshare_mqtt_producer_data.BikeshareSystem`.

Example:

```python
async def jp_odpt_docomo_bikeshare_bikeshare_system_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: BikeshareSystem) -> None:
...
client = JPODPTDocomoBikeshareStationsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### JPODPTDocomoBikeshareStationsMqttEventDispatcher



# Publish message`JPODPTDocomoBikeshareStationsMqttEventDispatcher` handles events for the
JP.ODPT.DocomoBikeshare.Stations.mqtt message group.
...
## BuildThe JPODPTDocomoBikeshareStationsMqttEventDispatcher defines the following event handler hooks.
...
```##### `jp_odpt_docomo_bikeshare_bikeshare_station_mqtt_async`



## Test```python

jp_odpt_docomo_bikeshare_bikeshare_station_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
BikeshareStation], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `JP.ODPT.DocomoBikeshare.BikeshareStation.mqtt`: A reference record from Docomo Bike
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `tokyo_docomo_bikeshare_mqtt_producer_data.BikeshareStation`.

Example:

```python
async def jp_odpt_docomo_bikeshare_bikeshare_station_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: BikeshareStation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `jp_odpt_docomo_bikeshare_bikeshare_station_status_mqtt_async`



## Test```python

jp_odpt_docomo_bikeshare_bikeshare_station_status_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
BikeshareStationStatus], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `JP.ODPT.DocomoBikeshare.BikeshareStationStatus.mqtt`: A reference record from Docomo
Bike Share open feeds for a station, stop, route, site, or other transport resource. It gives consumers stable
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `tokyo_docomo_bikeshare_mqtt_producer_data.BikeshareStationStatus`.

Example:

```python
async def jp_odpt_docomo_bikeshare_bikeshare_station_status_mqtt_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: BikeshareStationStatus) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
