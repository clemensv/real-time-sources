...
def get_default_topic_mappings_jp_odpt_docomobikeshare_system_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the JP.ODPT.DocomoBikeshare.System.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "JP.ODPT.DocomoBikeshare.BikeshareSystem.mqtt": "transit/jp/docomo/tokyo-docomo-bikeshare/{system_id}/{ward}/{station_id}/info",
    }


def get_subscription_topics_jp_odpt_docomobikeshare_system_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_jp_odpt_docomobikeshare_system_mqtt()
    topics = []
...
class JPODPTDocomoBikeshareSystemMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the JP.ODPT.DocomoBikeshare.System.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_jp_odpt_docomobikeshare_system_mqtt()
        ...
        self.jp_odpt_docomo_bikeshare_bikeshare_system_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, tokyo_docomo_bikeshare_mqtt_producer_data.BikeshareSystem, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "JP.ODPT.DocomoBikeshare.BikeshareSystem":
            if self.jp_odpt_docomo_bikeshare_bikeshare_system_mqtt_async:
                try:
                    ...
                    data = tokyo_docomo_bikeshare_mqtt_producer_data.BikeshareSystem.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "JP.ODPT.DocomoBikeshare.BikeshareSystem.mqtt")
                    await self.jp_odpt_docomo_bikeshare_bikeshare_system_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_odpt_docomo_bikeshare_bikeshare_system_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_jp_odpt_docomobikeshare_system_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_jp_odpt_docomo_bikeshare_bikeshare_system_mqtt(self,
        system_id: str,
        ward: str,
        station_id: str,
        data: tokyo_docomo_bikeshare_mqtt_producer_data.BikeshareSystem,
        ...
        """
        Publish the 'JP.ODPT.DocomoBikeshare.BikeshareSystem.mqtt' event to an MQTT topic.

        Args:
        
            system_id: URI template variable for 'system_id'
            ward: URI template variable for 'ward'
            station_id: URI template variable for 'station_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'transit/jp/docomo/tokyo-docomo-bikeshare/{system_id}/{ward}/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "transit/jp/docomo/tokyo-docomo-bikeshare/{system_id}/{ward}/{station_id}/info"
        ...
        attributes = {
             "type":"JP.ODPT.DocomoBikeshare.BikeshareSystem",
             "source":"https://api-public.odpt.org/api/v4/gbfs/docomo-cycle-tokyo",
...
def get_default_topic_mappings_jp_odpt_docomobikeshare_stations_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the JP.ODPT.DocomoBikeshare.Stations.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "JP.ODPT.DocomoBikeshare.BikeshareStation.mqtt": "transit/jp/docomo/tokyo-docomo-bikeshare/{system_id}/{ward}/{station_id}/info",
        "JP.ODPT.DocomoBikeshare.BikeshareStationStatus.mqtt": "transit/jp/docomo/tokyo-docomo-bikeshare/{system_id}/{ward}/{station_id}/status",
    }


def get_subscription_topics_jp_odpt_docomobikeshare_stations_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_jp_odpt_docomobikeshare_stations_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class JPODPTDocomoBikeshareStationsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the JP.ODPT.DocomoBikeshare.Stations.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_jp_odpt_docomobikeshare_stations_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        
        # Message handler callbacks (Dispatcher pattern)
        
        self.jp_odpt_docomo_bikeshare_bikeshare_station_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, tokyo_docomo_bikeshare_mqtt_producer_data.BikeshareStation, Dict[str, str]], Awaitable[None]]] = None
        
        self.jp_odpt_docomo_bikeshare_bikeshare_station_status_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, tokyo_docomo_bikeshare_mqtt_producer_data.BikeshareStationStatus, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "JP.ODPT.DocomoBikeshare.BikeshareStation":
            if self.jp_odpt_docomo_bikeshare_bikeshare_station_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = tokyo_docomo_bikeshare_mqtt_producer_data.BikeshareStation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "JP.ODPT.DocomoBikeshare.BikeshareStation.mqtt")
                    await self.jp_odpt_docomo_bikeshare_bikeshare_station_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_odpt_docomo_bikeshare_bikeshare_station_mqtt handler: {e}")
            return
        
        if event_type == "JP.ODPT.DocomoBikeshare.BikeshareStationStatus":
            if self.jp_odpt_docomo_bikeshare_bikeshare_station_status_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = tokyo_docomo_bikeshare_mqtt_producer_data.BikeshareStationStatus.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "JP.ODPT.DocomoBikeshare.BikeshareStationStatus.mqtt")
                    await self.jp_odpt_docomo_bikeshare_bikeshare_station_status_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_odpt_docomo_bikeshare_bikeshare_station_status_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_jp_odpt_docomobikeshare_stations_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_jp_odpt_docomo_bikeshare_bikeshare_station_mqtt(self,
        system_id: str,
        station_id: str,
        ward: str,
        data: tokyo_docomo_bikeshare_mqtt_producer_data.BikeshareStation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'JP.ODPT.DocomoBikeshare.BikeshareStation.mqtt' event to an MQTT topic.
        ...
        attributes = {
             "type":"JP.ODPT.DocomoBikeshare.BikeshareStation",
             "source":"https://api-public.odpt.org/api/v4/gbfs/docomo-cycle-tokyo",
    ...
    async def publish_jp_odpt_docomo_bikeshare_bikeshare_station_status_mqtt(self,
        system_id: str,
        station_id: str,
        ward: str,
        data: tokyo_docomo_bikeshare_mqtt_producer_data.BikeshareStationStatus,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'JP.ODPT.DocomoBikeshare.BikeshareStationStatus.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            ward: URI template variable for 'ward'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'transit/jp/docomo/tokyo-docomo-bikeshare/{system_id}/{ward}/{station_id}/status'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "transit/jp/docomo/tokyo-docomo-bikeshare/{system_id}/{ward}/{station_id}/status"
        _topic_template_values: Dict[str, str] = {
            "system_id": str(system_id),
            "station_id": str(station_id),
            "ward": str(ward),
        ...
        attributes = {
             "type":"JP.ODPT.DocomoBikeshare.BikeshareStationStatus",
             "source":"https://api-public.odpt.org/api/v4/gbfs/docomo-cycle-tokyo",
             "subject":"{system_id}/{station_id}".format(system_id = system_id,station_id = station_id)
        }
        attributes["datacontenttype"] = content_type
