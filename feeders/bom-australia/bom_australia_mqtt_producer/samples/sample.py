...
from bom_australia_mqtt_producer_mqtt_client.client import AUGovBOMWeatherMqttProducer, AUGovBOMWeatherMqttDispatcher

async def handle_au_gov_bom_weather_mqtt_station(mqtt_msg,cloud_event, au_gov_bom_weather_mqtt_station_data):
    """ Handles the AU.Gov.BOM.Weather.mqtt.Station message """
    print(f"Received AU.Gov.BOM.Weather.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {au_gov_bom_weather_mqtt_station_data}")

async def handle_au_gov_bom_weather_mqtt_weather_observation(mqtt_msg,cloud_event, au_gov_bom_weather_mqtt_weather_observation_data):
    """ Handles the AU.Gov.BOM.Weather.mqtt.WeatherObservation message """
    print(f"Received AU.Gov.BOM.Weather.mqtt.WeatherObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {au_gov_bom_weather_mqtt_weather_observation_data}")
from bom_australia_mqtt_producer_mqtt_client.client import AUGovBOMWarningMqttProducer, AUGovBOMWarningMqttDispatcher

async def handle_au_gov_bom_warning_mqtt_warning_bulletin(mqtt_msg,cloud_event, au_gov_bom_warning_mqtt_warning_bulletin_data):
    """ Handles the AU.Gov.BOM.Warning.mqtt.WarningBulletin message """
    print(f"Received AU.Gov.BOM.Warning.mqtt.WarningBulletin on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {au_gov_bom_warning_mqtt_warning_bulletin_data}")
