# Bom_australia_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the bom_australia_mqtt_producer message definitions.

# Bom_australia_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - AUGovBOMWeatherMqttEventDispatcher,

    AUGovBOMWarningMqttEventDispatcher
...
from bom_australia_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = AUGovBOMWeatherMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### AUGovBOMWeatherMqttEventDispatcher



# Publish message`AUGovBOMWeatherMqttEventDispatcher` handles events for the AU.Gov.BOM.Weather.mqtt message group.
...
from bom_australia_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = AUGovBOMWeatherMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
client = AUGovBOMWarningMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe AUGovBOMWeatherMqttEventDispatcher defines the following event handler hooks.
...
```##### `au_gov_bom_weather_mqtt_station_async`



## Test```python

au_gov_bom_weather_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `bom_australia_mqtt_producer_data.Station`.

Example:

```python
async def au_gov_bom_weather_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `au_gov_bom_weather_mqtt_weather_observation_async`



## Test```python

au_gov_bom_weather_mqtt_weather_observation_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `bom_australia_mqtt_producer_data.WeatherObservation`.

Example:

```python
async def au_gov_bom_weather_mqtt_weather_observation_event(partition_context: PartitionContext, event: EventData,
...
client = AUGovBOMWarningMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### AUGovBOMWarningMqttEventDispatcher



# Publish message`AUGovBOMWarningMqttEventDispatcher` handles events for the AU.Gov.BOM.Warning.mqtt message group.
...
## BuildThe AUGovBOMWarningMqttEventDispatcher defines the following event handler hooks.
...
```##### `au_gov_bom_warning_mqtt_warning_bulletin_async`



## Test```python

au_gov_bom_warning_mqtt_warning_bulletin_async:  Callable[[PartitionContext, EventData, CloudEvent, WarningBulletin],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `bom_australia_mqtt_producer_data.WarningBulletin`.

Example:

```python
async def au_gov_bom_warning_mqtt_warning_bulletin_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: WarningBulletin) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
