...
# Wikimedia-eventstreams-producer Kafka Producer# Wikimedia-eventstreams-producer Event Dispatcher for Apache Kafka
...
- WikimediaEventStreamsProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### WikimediaEventStreamsEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`WikimediaEventStreamsEventDispatcher` handles events for the Wikimedia.EventStreams message group.
...
The WikimediaEventStreamsEventDispatcher defines the following event handler hooks.
...
producer = WikimediaEventStreamsProducer(
    ...
    security_protocol='SASL_SSL',##### `wikimedia_event_streams_recent_change_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'wikimedia_event_streams_recent_change_async:  Callable[[ConsumerRecord, CloudEvent,
RecentChange], Awaitable[None]]

)```

```

Asynchronous handler hook for `Wikimedia.EventStreams.RecentChange`: Public recentchange events from Wikimedia
...
wikimedia_event_streams_dispatcher.wikimedia_event_streams_recent_change_async =
...
##### `send_wikimedia_event_streams_recent_change`Dispatchers have the following protected methods:



```python### Methods:

async def send_wikimedia_event_streams_recent_change(
...
Send a single `Wikimedia.EventStreams.RecentChange` message. Public recentchange events from Wikimedia EventStreams.
Each event describes one edit, page creation, log action, categorization change, or related MediaWiki recentchanges
entry from across Wikimedia projects.Args:
...
await producer.send_wikimedia_event_streams_recent_change(
...
Send multiple `Wikimedia.EventStreams.RecentChange` messages in a batch.
...
await producer.send_wikimedia_event_streams_recent_change_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### WikimediaEventStreamsMqttEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`WikimediaEventStreamsMqttEventDispatcher` handles events for the Wikimedia.EventStreams.mqtt message group.
...
The WikimediaEventStreamsMqttEventDispatcher defines the following event handler hooks.
...
producer = WikimediaEventStreamsProducer(
    ...
    security_protocol='SASL_SSL',##### `wikimedia_event_streams_recent_change_mqtt_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'wikimedia_event_streams_recent_change_mqtt_async:  Callable[[ConsumerRecord,
...
Asynchronous handler hook for `Wikimedia.EventStreams.RecentChange.mqtt`: Public recentchange events from Wikimedia
EventStreams. Each event describes one edit, page creation, log action, categorization change, or related MediaWiki
recentchanges entry from across Wikimedia projects.
...
wikimedia_event_streams_dispatcher.wikimedia_event_streams_recent_change_mqtt_async =
...
wikimedia_event_streams_mqtt_dispatcher.wikimedia_event_streams_recent_change_mqtt_async =
wikimedia_event_streams_recent_change_mqtt_event
...
##### `send_wikimedia_event_streams_recent_change_mqtt`Dispatchers have the following protected methods:



```python### Methods:

async def send_wikimedia_event_streams_recent_change_mqtt(
...
Send a single `Wikimedia.EventStreams.RecentChange.mqtt` message. Public recentchange events from Wikimedia
EventStreams. Each event describes one edit, page creation, log action, categorization change, or related MediaWiki
...
await producer.send_wikimedia_event_streams_recent_change_mqtt(
...
Send multiple `Wikimedia.EventStreams.RecentChange.mqtt` messages in a batch.
