# Wikimedia_eventstreams_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the wikimedia_eventstreams_mqtt_producer message definitions.

# Wikimedia_eventstreams_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - WikimediaEventStreamsMqttEventDispatcher
...
from wikimedia_eventstreams_mqtt_producer_mqtt_client import *This module defines an event processing framework for
Azure Event Hubs,
...
client = WikimediaEventStreamsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### WikimediaEventStreamsMqttEventDispatcher



# Publish message`WikimediaEventStreamsMqttEventDispatcher` handles events for the Wikimedia.EventStreams.mqtt message
group.
...
from wikimedia_eventstreams_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = WikimediaEventStreamsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe WikimediaEventStreamsMqttEventDispatcher defines the following event handler hooks.
...
```##### `wikimedia_event_streams_recent_change_mqtt_async`



## Test```python

wikimedia_event_streams_recent_change_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RecentChange],
Awaitable[None]]
...
- `data`: The event data of type `wikimedia_eventstreams_mqtt_producer_data.RecentChange`.

Example:

```python
async def wikimedia_event_streams_recent_change_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RecentChange) -> None:
