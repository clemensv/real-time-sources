...
def get_default_topic_mappings_wikimedia_eventstreams_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the Wikimedia.EventStreams.mqtt message group.
...
def get_subscription_topics_wikimedia_eventstreams_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_wikimedia_eventstreams_mqtt()
    topics = []
...
class WikimediaEventStreamsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the Wikimedia.EventStreams.mqtt message group."""
    ...
    def __init__(
        ...
        self.wikimedia_event_streams_recent_change_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, wikimedia_eventstreams_mqtt_producer_data.RecentChange, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "Wikimedia.EventStreams.RecentChange":
            if self.wikimedia_event_streams_recent_change_mqtt_async:
                try:
                    ...
                    data = wikimedia_eventstreams_mqtt_producer_data.RecentChange.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Wikimedia.EventStreams.RecentChange.mqtt")
                    await self.wikimedia_event_streams_recent_change_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in wikimedia_event_streams_recent_change_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_wikimedia_eventstreams_mqtt(self._topic_mappings)
        for topic in topics:
