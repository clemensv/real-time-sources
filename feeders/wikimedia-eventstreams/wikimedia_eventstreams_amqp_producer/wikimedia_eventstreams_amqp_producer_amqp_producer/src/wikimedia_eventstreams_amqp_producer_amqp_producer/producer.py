...
class WikimediaEventStreamsAmqpProducer:
    """
    Producer class to send messages in the `Wikimedia.EventStreams.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_recent_change(self,
        data: RecentChange,
        _event_id: str,
        _event_time: str,
        content_type: str = 'application/json') -> None:
        """
        Send the `Wikimedia.EventStreams.amqp.RecentChange` message
    ...
    def send_recent_change_batch(self,
        data_array: typing.List[RecentChange],
        _event_id: str,
        _event_time: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `Wikimedia.EventStreams.amqp.RecentChange` messages
        
        Args:
            data_array (typing.List[RecentChange]): Array of message data objects
            _event_id (str): Value for placeholder event_id in attribute subject
            _event_time (str): Value for placeholder event_time in attribute time
            content_type (str): The content type of the message data
        """
        for data in data_array:
            self.send_recent_change(
                data=data,
                _event_id=_event_id,
