"""AMQP 1.0 companion feeder for cbp-border-wait."""
...
SOURCE_ID = "cbp-border-wait"
PY_MODULE = "cbp_border_wait"
...
async def _run_live(args: argparse.Namespace, producer) -> None:
    ...
    api = CbpBorderWaitAPI(); previous_timestamps = _load_state(args.state_file)
    raw_ports = api.fetch_ports()
    for raw in raw_ports:
        try:
            port = api.parse_port(raw); producer.send_port(data=port, _port_number=port.port_number, _border_slug=port.border_slug)
        except Exception as exc: logger.error('Error sending port %s: %s', raw.get('port_number', '?'), exc)
    while True:
        count = 0; start = time.monotonic(); raw_ports = api.fetch_ports()
        for raw in raw_ports:
