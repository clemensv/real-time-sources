...
# Cbp_border_wait_producer Kafka Producer# Cbp_border_wait_producer Event Dispatcher for Apache Kafka
...
- GovCbpBorderwaitProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### GovCbpBorderwaitEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`GovCbpBorderwaitEventDispatcher` handles events for the gov.cbp.borderwait message group.
...
The GovCbpBorderwaitEventDispatcher defines the following event handler hooks.
...
producer = GovCbpBorderwaitProducer(
    ...
    security_protocol='SASL_SSL',##### `gov_cbp_borderwait_port_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gov_cbp_borderwait_port_async:  Callable[[ConsumerRecord, CloudEvent, Port],
Awaitable[None]]
...
Asynchronous handler hook for `gov.cbp.borderwait.Port`: Reference data for a US Customs and Border Protection land
...
gov_cbp_borderwait_dispatcher.gov_cbp_borderwait_port_async = gov_cbp_borderwait_port_event
...
gov_cbp_borderwait_mqtt_dispatcher.gov_cbp_borderwait_port_async = gov_cbp_borderwait_port_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `gov_cbp_borderwait_wait_time_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gov_cbp_borderwait_wait_time_async:  Callable[[ConsumerRecord, CloudEvent, WaitTime],
Awaitable[None]]
...
Asynchronous handler hook for `gov.cbp.borderwait.WaitTime`: Current wait times at a US land border port of entry,
...
gov_cbp_borderwait_dispatcher.gov_cbp_borderwait_wait_time_async = gov_cbp_borderwait_wait_time_event
...
##### `send_gov_cbp_borderwait_port`Dispatchers have the following protected methods:



```python### Methods:

async def send_gov_cbp_borderwait_port(
...
Send a single `gov.cbp.borderwait.Port` message. Reference data for a US Customs and Border Protection land border port
of entry. The CBP Border Wait Time system covers approximately 81 ports along the US-Canada and US-Mexico borders. Each
port record identifies the crossing name, operating hours, border (Canadian or Mexican), and current operational status.
Emitted at bridge startup and periodically refreshed.Args:
...
await producer.send_gov_cbp_borderwait_port(
...
Send multiple `gov.cbp.borderwait.Port` messages in a batch.
...
await producer.send_gov_cbp_borderwait_port_batch(```
...
##### `send_gov_cbp_borderwait_wait_time`Dispatchers have the following protected methods:



```python### Methods:

async def send_gov_cbp_borderwait_wait_time(
...
Send a single `gov.cbp.borderwait.WaitTime` message. Current wait times at a US land border port of entry, flattened
from the CBP nested lane structure. Reports delay in minutes and number of open lanes for each combination of traveler
category (passenger vehicle, pedestrian, commercial vehicle) and lane type (standard, SENTRI/NEXUS, Ready Lane, FAST).
Wait times are updated approximately every hour by CBP officers at each port.Args:
...
await producer.send_gov_cbp_borderwait_wait_time(
...
Send multiple `gov.cbp.borderwait.WaitTime` messages in a batch.
...
await producer.send_gov_cbp_borderwait_wait_time_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### GovCbpBorderwaitMqttEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`GovCbpBorderwaitMqttEventDispatcher` handles events for the gov.cbp.borderwait.mqtt message group.
...
The GovCbpBorderwaitMqttEventDispatcher defines the following event handler hooks.
...
producer = GovCbpBorderwaitProducer(
    ...
    security_protocol='SASL_SSL',##### `gov_cbp_borderwait_mqtt_port_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gov_cbp_borderwait_mqtt_port_async:  Callable[[ConsumerRecord, CloudEvent, Port],
Awaitable[None]]
...
Asynchronous handler hook for `gov.cbp.borderwait.mqtt.Port`: Reference data for a US Customs and Border Protection land
border port of entry. The CBP Border Wait Time system covers approximately 81 ports along the US-Canada and US-Mexico
borders. Each port record identifies the crossing name, operating hours, border (Canadian or Mexican), and current
operational status. Emitted at bridge startup and periodically refreshed.
...
gov_cbp_borderwait_dispatcher.gov_cbp_borderwait_mqtt_port_async = gov_cbp_borderwait_mqtt_port_event
...
gov_cbp_borderwait_mqtt_dispatcher.gov_cbp_borderwait_mqtt_port_async = gov_cbp_borderwait_mqtt_port_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `gov_cbp_borderwait_mqtt_wait_time_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gov_cbp_borderwait_mqtt_wait_time_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `gov.cbp.borderwait.mqtt.WaitTime`: Current wait times at a US land border port of entry,
flattened from the CBP nested lane structure. Reports delay in minutes and number of open lanes for each combination of
traveler category (passenger vehicle, pedestrian, commercial vehicle) and lane type (standard, SENTRI/NEXUS, Ready Lane,
FAST). Wait times are updated approximately every hour by CBP officers at each port.
...
gov_cbp_borderwait_dispatcher.gov_cbp_borderwait_mqtt_wait_time_async = gov_cbp_borderwait_mqtt_wait_time_event
...
##### `send_gov_cbp_borderwait_mqtt_port`Dispatchers have the following protected methods:



```python### Methods:

async def send_gov_cbp_borderwait_mqtt_port(
...
Send a single `gov.cbp.borderwait.mqtt.Port` message. Reference data for a US Customs and Border Protection land border
...
await producer.send_gov_cbp_borderwait_mqtt_port(
...
Send multiple `gov.cbp.borderwait.mqtt.Port` messages in a batch.
...
await producer.send_gov_cbp_borderwait_mqtt_port_batch(```
...
##### `send_gov_cbp_borderwait_mqtt_wait_time`Dispatchers have the following protected methods:



```python### Methods:

async def send_gov_cbp_borderwait_mqtt_wait_time(
...
Send a single `gov.cbp.borderwait.mqtt.WaitTime` message. Current wait times at a US land border port of entry,
flattened from the CBP nested lane structure. Reports delay in minutes and number of open lanes for each combination of
traveler category (passenger vehicle, pedestrian, commercial vehicle) and lane type (standard, SENTRI/NEXUS, Ready Lane,
...
await producer.send_gov_cbp_borderwait_mqtt_wait_time(
...
Send multiple `gov.cbp.borderwait.mqtt.WaitTime` messages in a batch.
