...
"""
Tests for cbp_border_wait_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../cbp_border_wait_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../cbp_border_wait_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../cbp_border_wait_amqp_producer_amqp_producer/src')))

from cbp_border_wait_amqp_producer_amqp_producer import *
from cbp_border_wait_amqp_producer_data import Port
from test_port import Test_Port
from cbp_border_wait_amqp_producer_data import WaitTime
...
class TestGovCbpBorderwaitAmqpProducer:
    """Test cases for GovCbpBorderwaitAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(GovCbpBorderwaitAmqpProducer)
    ...
    def test_send_port(self, artemis_container):
        """Send and receive a Port message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Port.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                assert annotations.get(symbol('x-opt-partition-key')) == str("{port_number}".format(port_number="value"))[:128]
        finally:
            producer.close()

    def test_send_port_single_fresh_connection(self, artemis_container):
        """Send exactly one Port message on a fresh producer connection."""
        payload = Test_Port.create_instance()
    ...
    def test_send_wait_time(self, artemis_container):
        """Send and receive a WaitTime message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_WaitTime.create_instance()
    ...
    def test_send_wait_time_single_fresh_connection(self, artemis_container):
        """Send exactly one WaitTime message on a fresh producer connection."""
        payload = Test_WaitTime.create_instance()
