...
import os
import signal
from cbp_border_wait_mqtt_producer_data import *
from cbp_border_wait_mqtt_producer_mqtt_client.client import GovCbpBorderwaitMqttProducer, GovCbpBorderwaitMqttDispatcher

async def handle_gov_cbp_borderwait_mqtt_port(mqtt_msg,cloud_event, gov_cbp_borderwait_mqtt_port_data):
    """ Handles the gov.cbp.borderwait.mqtt.Port message """
    print(f"Received gov.cbp.borderwait.mqtt.Port on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {gov_cbp_borderwait_mqtt_port_data}")

async def handle_gov_cbp_borderwait_mqtt_wait_time(mqtt_msg,cloud_event, gov_cbp_borderwait_mqtt_wait_time_data):
    """ Handles the gov.cbp.borderwait.mqtt.WaitTime message """
    print(f"Received gov.cbp.borderwait.mqtt.WaitTime on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {gov_cbp_borderwait_mqtt_wait_time_data}")
