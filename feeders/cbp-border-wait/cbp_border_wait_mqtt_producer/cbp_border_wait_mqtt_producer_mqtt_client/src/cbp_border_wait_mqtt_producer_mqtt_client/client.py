...
def get_default_topic_mappings_gov_cbp_borderwait_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the gov.cbp.borderwait.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "gov.cbp.borderwait.mqtt.Port": "traffic/us/cbp/cbp-border-wait/{border_slug}/{port_number}/info",
        "gov.cbp.borderwait.mqtt.WaitTime": "traffic/us/cbp/cbp-border-wait/{border_slug}/{port_number}/wait-time",
    }


def get_subscription_topics_gov_cbp_borderwait_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_gov_cbp_borderwait_mqtt()
    topics = []
...
class GovCbpBorderwaitMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the gov.cbp.borderwait.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_gov_cbp_borderwait_mqtt()
        ...
        self.gov_cbp_borderwait_mqtt_port_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, cbp_border_wait_mqtt_producer_data.Port, Dict[str, str]], Awaitable[None]]] = None
        
        self.gov_cbp_borderwait_mqtt_wait_time_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, cbp_border_wait_mqtt_producer_data.WaitTime, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "gov.cbp.borderwait.Port":
            if self.gov_cbp_borderwait_mqtt_port_async:
                try:
                    ...
                    data = cbp_border_wait_mqtt_producer_data.Port.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "gov.cbp.borderwait.mqtt.Port")
                    await self.gov_cbp_borderwait_mqtt_port_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in gov_cbp_borderwait_mqtt_port handler: {e}")
            return
        
        if event_type == "gov.cbp.borderwait.WaitTime":
            if self.gov_cbp_borderwait_mqtt_wait_time_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = cbp_border_wait_mqtt_producer_data.WaitTime.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "gov.cbp.borderwait.mqtt.WaitTime")
                    await self.gov_cbp_borderwait_mqtt_wait_time_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in gov_cbp_borderwait_mqtt_wait_time handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_gov_cbp_borderwait_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_gov_cbp_borderwait_mqtt_port(self,
        port_number: str,
        border_slug: str,
        data: cbp_border_wait_mqtt_producer_data.Port,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'gov.cbp.borderwait.mqtt.Port' event to an MQTT topic.

        Args:
        
            port_number: URI template variable for 'port_number'
            border_slug: URI template variable for 'border_slug'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/us/cbp/cbp-border-wait/{border_slug}/{port_number}/info'
        ...
        """
        target_topic = topic if topic is not None else "traffic/us/cbp/cbp-border-wait/{border_slug}/{port_number}/info"
        _topic_template_values: Dict[str, str] = {
            "port_number": str(port_number),
            "border_slug": str(border_slug),
        }
        if _topic_template_values:
            target_topic = _apply_topic_template(target_topic, _topic_template_values)

        attributes = {
             "type":"gov.cbp.borderwait.Port",
             "source":"https://bwt.cbp.gov",
             "subject":"{port_number}".format(port_number = port_number)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_gov_cbp_borderwait_mqtt_wait_time(self,
        port_number: str,
        border_slug: str,
        data: cbp_border_wait_mqtt_producer_data.WaitTime,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'gov.cbp.borderwait.mqtt.WaitTime' event to an MQTT topic.
        ...
        Args:
            ...
            border_slug: URI template variable for 'border_slug'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/us/cbp/cbp-border-wait/{border_slug}/{port_number}/wait-time'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "traffic/us/cbp/cbp-border-wait/{border_slug}/{port_number}/wait-time"
        _topic_template_values: Dict[str, str] = {
            "port_number": str(port_number),
            "border_slug": str(border_slug),
        }
        ...
        attributes = {
             "type":"gov.cbp.borderwait.WaitTime",
             "source":"https://bwt.cbp.gov",
             "subject":"{port_number}".format(port_number = port_number)
        }
        attributes["datacontenttype"] = content_type
        ...
        _ensure_message_expiry(publish_kwargs, 7200)
        self.client.publish(target_topic, payload, **publish_kwargs)
