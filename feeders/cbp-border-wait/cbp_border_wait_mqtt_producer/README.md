# Cbp_border_wait_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the cbp_border_wait_mqtt_producer message definitions.

# Cbp_border_wait_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - GovCbpBorderwaitMqttEventDispatcher
...
from cbp_border_wait_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = GovCbpBorderwaitMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### GovCbpBorderwaitMqttEventDispatcher



# Publish message`GovCbpBorderwaitMqttEventDispatcher` handles events for the gov.cbp.borderwait.mqtt message group.
...
from cbp_border_wait_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = GovCbpBorderwaitMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe GovCbpBorderwaitMqttEventDispatcher defines the following event handler hooks.
...
```##### `gov_cbp_borderwait_mqtt_port_async`



## Test```python

gov_cbp_borderwait_mqtt_port_async:  Callable[[PartitionContext, EventData, CloudEvent, Port], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `gov.cbp.borderwait.mqtt.Port`: Reference data for a US Customs and Border Protection
land border port of entry. The CBP Border Wait Time system covers approximately 81 ports along the US-Canada and US-
Mexico borders. Each port record identifies the crossing name, operating hours, border (Canadian or Mexican), and
current operational status. Emitted at bridge startup and periodically refreshed.
...
- `data`: The event data of type `cbp_border_wait_mqtt_producer_data.Port`.

Example:

```python
async def gov_cbp_borderwait_mqtt_port_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Port) -> None:
...
```##### `gov_cbp_borderwait_mqtt_wait_time_async`



## Test```python

gov_cbp_borderwait_mqtt_wait_time_async:  Callable[[PartitionContext, EventData, CloudEvent, WaitTime], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `gov.cbp.borderwait.mqtt.WaitTime`: Current wait times at a US land border port of
entry, flattened from the CBP nested lane structure. Reports delay in minutes and number of open lanes for each
combination of traveler category (passenger vehicle, pedestrian, commercial vehicle) and lane type (standard,
SENTRI/NEXUS, Ready Lane, FAST). Wait times are updated approximately every hour by CBP officers at each port.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `cbp_border_wait_mqtt_producer_data.WaitTime`.

Example:

```python
async def gov_cbp_borderwait_mqtt_wait_time_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: WaitTime) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
