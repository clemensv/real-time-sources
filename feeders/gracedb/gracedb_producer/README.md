...
# Gracedb-producer Kafka Producer# Gracedb-producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - OrgLigoGracedbEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    OrgLigoGracedbMqttEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    OrgLigoGracedbAmqpEventDispatcher
...
- OrgLigoGracedbProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- OrgLigoGracedbMqttProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- OrgLigoGracedbAmqpProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### OrgLigoGracedbEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`OrgLigoGracedbEventDispatcher` handles events for the org.ligo.gracedb message group.
...
from gracedb-producer import OrgLigoGracedbProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = OrgLigoGracedbProducer(
...
await producer.send_org_ligo_gracedb_superevent(```python

    data=Superevent(...),add_consumer(self, consumer: KafkaConsumer)
...
The OrgLigoGracedbEventDispatcher defines the following event handler hooks.
...
producer = OrgLigoGracedbProducer(
    ...
    security_protocol='SASL_SSL',##### `org_ligo_gracedb_superevent_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'org_ligo_gracedb_superevent_async:  Callable[[ConsumerRecord, CloudEvent, Superevent],
Awaitable[None]]
...
Asynchronous handler hook for `org.ligo.gracedb.Superevent`: A gravitational wave candidate superevent from the
LIGO/Virgo/KAGRA collaboration, published via GraceDB. A superevent aggregates one or more pipeline detections into a
single candidate and carries significance, classification, and alert-lifecycle metadata.
...
### OrgLigoGracedbProducer- `data`: The event data of type `gracedb_producer_data.Superevent`.



Producer for `org.ligo.gracedb` message group.Example:



#### Constructor```python

async def org_ligo_gracedb_superevent_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Superevent) -> None:

```python    # Process the event data

OrgLigoGracedbProducer(    await some_processing_function(record, cloud_event, data)
...
org_ligo_gracedb_dispatcher.org_ligo_gracedb_superevent_async = org_ligo_gracedb_superevent_event
...
### OrgLigoGracedbMqttProducer- `data`: The event data of type `gracedb_producer_data.Superevent`.



Producer for `org.ligo.gracedb.mqtt` message group.Example:
...
OrgLigoGracedbMqttProducer(    await some_processing_function(record, cloud_event, data)
...
org_ligo_gracedb_mqtt_dispatcher.org_ligo_gracedb_superevent_async = org_ligo_gracedb_superevent_event
...
### OrgLigoGracedbAmqpProducer- `data`: The event data of type `gracedb_producer_data.Superevent`.



Producer for `org.ligo.gracedb.amqp` message group.Example:
...
OrgLigoGracedbAmqpProducer(    await some_processing_function(record, cloud_event, data)
...
org_ligo_gracedb_amqp_dispatcher.org_ligo_gracedb_superevent_async = org_ligo_gracedb_superevent_event
...
##### `send_org_ligo_gracedb_superevent`Dispatchers have the following protected methods:



```python### Methods:

async def send_org_ligo_gracedb_superevent(
...
Send a single `org.ligo.gracedb.Superevent` message. A gravitational wave candidate superevent from the LIGO/Virgo/KAGRA
collaboration, published via GraceDB. A superevent aggregates one or more pipeline detections into a single candidate
and carries significance, classification, and alert-lifecycle metadata.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `Superevent`
...
await producer.send_org_ligo_gracedb_superevent(

    data=Superevent(...),Args:
...
Send multiple `org.ligo.gracedb.Superevent` messages in a batch.
...
await producer.send_org_ligo_gracedb_superevent_batch(```

    messages=[

        Superevent(...),Initializes the runner with a Kafka consumer.
...
- **Scales horizontally** across multiple brokers and partitions### OrgLigoGracedbMqttEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`OrgLigoGracedbMqttEventDispatcher` handles events for the org.ligo.gracedb.mqtt message group.
...
The OrgLigoGracedbMqttEventDispatcher defines the following event handler hooks.
...
producer = OrgLigoGracedbProducer(
    ...
    security_protocol='SASL_SSL',##### `org_ligo_gracedb_mqtt_superevent_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'org_ligo_gracedb_mqtt_superevent_async:  Callable[[ConsumerRecord, CloudEvent,
Superevent], Awaitable[None]]

)```

```

Asynchronous handler hook for `org.ligo.gracedb.mqtt.Superevent`: A gravitational wave candidate superevent from the
LIGO/Virgo/KAGRA collaboration, published via GraceDB. A superevent aggregates one or more pipeline detections into a
single candidate and carries significance, classification, and alert-lifecycle metadata.
...
async def org_ligo_gracedb_mqtt_superevent_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Superevent) ->
None:
...
org_ligo_gracedb_dispatcher.org_ligo_gracedb_mqtt_superevent_async = org_ligo_gracedb_mqtt_superevent_event
...
org_ligo_gracedb_mqtt_dispatcher.org_ligo_gracedb_mqtt_superevent_async = org_ligo_gracedb_mqtt_superevent_event
...
org_ligo_gracedb_amqp_dispatcher.org_ligo_gracedb_mqtt_superevent_async = org_ligo_gracedb_mqtt_superevent_event
...
##### `send_org_ligo_gracedb_mqtt_superevent`Dispatchers have the following protected methods:



```python### Methods:

async def send_org_ligo_gracedb_mqtt_superevent(
...
Send a single `org.ligo.gracedb.mqtt.Superevent` message. A gravitational wave candidate superevent from the
LIGO/Virgo/KAGRA collaboration, published via GraceDB. A superevent aggregates one or more pipeline detections into a
single candidate and carries significance, classification, and alert-lifecycle metadata.Args:
...
await producer.send_org_ligo_gracedb_mqtt_superevent(
...
Send multiple `org.ligo.gracedb.mqtt.Superevent` messages in a batch.
...
await producer.send_org_ligo_gracedb_mqtt_superevent_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### OrgLigoGracedbAmqpEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`OrgLigoGracedbAmqpEventDispatcher` handles events for the org.ligo.gracedb.amqp message group.
...
The OrgLigoGracedbAmqpEventDispatcher defines the following event handler hooks.
...
producer = OrgLigoGracedbProducer(
    ...
    security_protocol='SASL_SSL',##### `org_ligo_gracedb_amqp_superevent_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'org_ligo_gracedb_amqp_superevent_async:  Callable[[ConsumerRecord, CloudEvent,
Superevent], Awaitable[None]]
...
Asynchronous handler hook for `org.ligo.gracedb.amqp.Superevent`: A gravitational wave candidate superevent from the
LIGO/Virgo/KAGRA collaboration, published via GraceDB. A superevent aggregates one or more pipeline detections into a
single candidate and carries significance, classification, and alert-lifecycle metadata.
...
async def org_ligo_gracedb_amqp_superevent_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Superevent) ->
None:
...
org_ligo_gracedb_dispatcher.org_ligo_gracedb_amqp_superevent_async = org_ligo_gracedb_amqp_superevent_event
...
org_ligo_gracedb_mqtt_dispatcher.org_ligo_gracedb_amqp_superevent_async = org_ligo_gracedb_amqp_superevent_event
...
org_ligo_gracedb_amqp_dispatcher.org_ligo_gracedb_amqp_superevent_async = org_ligo_gracedb_amqp_superevent_event
...
##### `send_org_ligo_gracedb_amqp_superevent`Dispatchers have the following protected methods:



```python### Methods:

async def send_org_ligo_gracedb_amqp_superevent(
...
Send a single `org.ligo.gracedb.amqp.Superevent` message. A gravitational wave candidate superevent from the
LIGO/Virgo/KAGRA collaboration, published via GraceDB. A superevent aggregates one or more pipeline detections into a
single candidate and carries significance, classification, and alert-lifecycle metadata.Args:
...
await producer.send_org_ligo_gracedb_amqp_superevent(
...
Send multiple `org.ligo.gracedb.amqp.Superevent` messages in a batch.
...
await producer.send_org_ligo_gracedb_amqp_superevent_batch(```
