...
"""
Tests for gracedb_amqp_producer_amqp_producer
"""
import base64
import json
import os
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../gracedb_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../gracedb_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../gracedb_amqp_producer_amqp_producer/src')))

from gracedb_amqp_producer_amqp_producer import *
from gracedb_amqp_producer_data import Superevent
from test_gracedb_amqp_producer_data_superevent import Test_Superevent
...
class TestOrgLigoGracedbAmqpProducer:
    """Test cases for OrgLigoGracedbAmqpProducer"""
    ...
    def test_send_superevent(self, artemis_container):
        """Send and receive a Superevent message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Superevent.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "org.ligo.gracedb.amqp.Superevent"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{superevent_id}".format(superevent_id="value")
        finally:
            producer.close()
