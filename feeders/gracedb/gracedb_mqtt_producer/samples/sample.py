...
import os
import signal
from gracedb_mqtt_producer_data import *
from gracedb_mqtt_producer_mqtt_client.client import OrgLigoGracedbMqttProducer, OrgLigoGracedbMqttDispatcher

async def handle_org_ligo_gracedb_mqtt_superevent(mqtt_msg,cloud_event, org_ligo_gracedb_mqtt_superevent_data):
    """ Handles the org.ligo.gracedb.mqtt.Superevent message """
    print(f"Received org.ligo.gracedb.mqtt.Superevent on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_ligo_gracedb_mqtt_superevent_data}")
