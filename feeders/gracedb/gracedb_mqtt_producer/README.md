# Gracedb_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the gracedb_mqtt_producer message definitions.

# Gracedb_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - OrgLigoGracedbMqttEventDispatcher
...
from gracedb_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = OrgLigoGracedbMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgLigoGracedbMqttEventDispatcher



# Publish message`OrgLigoGracedbMqttEventDispatcher` handles events for the org.ligo.gracedb.mqtt message group.
...
from gracedb_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = OrgLigoGracedbMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe OrgLigoGracedbMqttEventDispatcher defines the following event handler hooks.
...
```##### `org_ligo_gracedb_mqtt_superevent_async`



## Test```python

org_ligo_gracedb_mqtt_superevent_async:  Callable[[PartitionContext, EventData, CloudEvent, Superevent],
Awaitable[None]]
...
- `data`: The event data of type `gracedb_mqtt_producer_data.Superevent`.

Example:

```python
async def org_ligo_gracedb_mqtt_superevent_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Superevent) -> None:
