...
def get_default_topic_mappings_org_ligo_gracedb_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.ligo.gracedb.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.ligo.gracedb.mqtt.Superevent": "seismic/intl/ligo/gracedb/{category}/{group}/{superevent_id}/superevent",
    }


def get_subscription_topics_org_ligo_gracedb_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_org_ligo_gracedb_mqtt()
    topics = []
...
class OrgLigoGracedbMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.ligo.gracedb.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_ligo_gracedb_mqtt()
        ...
        self.org_ligo_gracedb_mqtt_superevent_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, gracedb_mqtt_producer_data.Superevent, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.ligo.gracedb.Superevent":
            if self.org_ligo_gracedb_mqtt_superevent_async:
                try:
                    ...
                    data = gracedb_mqtt_producer_data.Superevent.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.ligo.gracedb.mqtt.Superevent")
                    await self.org_ligo_gracedb_mqtt_superevent_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_ligo_gracedb_mqtt_superevent handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_ligo_gracedb_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_org_ligo_gracedb_mqtt_superevent(self,
        source_uri: str,
        superevent_id: str,
        created: str,
        category: str,
        group: str,
        data: gracedb_mqtt_producer_data.Superevent,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.ligo.gracedb.mqtt.Superevent' event to an MQTT topic.

        Args:
        
            source_uri: URI template variable for 'source_uri'
            superevent_id: URI template variable for 'superevent_id'
            created: URI template variable for 'created'
            category: URI template variable for 'category'
            group: URI template variable for 'group'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'seismic/intl/ligo/gracedb/{category}/{group}/{superevent_id}/superevent'
        ...
        """
        target_topic = topic if topic is not None else "seismic/intl/ligo/gracedb/{category}/{group}/{superevent_id}/superevent"
        ...
        attributes = {
             ...
             "subject":"{superevent_id}".format(superevent_id = superevent_id),
             "time":"{created}".format(created = created)
        }
        attributes["datacontenttype"] = content_type
