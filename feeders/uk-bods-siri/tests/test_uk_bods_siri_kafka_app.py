...
def test_kafka_feed_once_emits_reference_and_telemetry(tmp_path):
    from uk_bods_siri_kafka import app as kafka_app
    ...
    with mock.patch.object(kafka_app, "Producer", return_value=fake_raw_producer), mock.patch.object(
        kafka_app, "UkGovDftBodsKafkaEventProducer", return_value=fake_event_producer
    ):
        asyncio.run(
            kafka_app.feed(
                api=api,
                kafka_config={"bootstrap.servers": "localhost:9092"},
                kafka_topic="uk-bods-siri",
                polling_interval=1,
    ...
    assert fake_event_producer.send_uk_gov_dft_bods_kafka_operator.call_count == 1
    assert fake_event_producer.send_uk_gov_dft_bods_kafka_vehicle_position.call_count == 1
