...
# Uk_bods_siri_producer Kafka Producer# Uk_bods_siri_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - UkGovDftBodsKafkaEventDispatcher
...
- UkGovDftBodsKafkaProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### UkGovDftBodsKafkaEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`UkGovDftBodsKafkaEventDispatcher` handles events for the uk.gov.dft.bods.kafka message group.
...
from uk_bods_siri_producer import UkGovDftBodsKafkaProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = UkGovDftBodsKafkaProducer(
...
await producer.send_uk_gov_dft_bods_kafka_vehicle_position(```python
...
The UkGovDftBodsKafkaEventDispatcher defines the following event handler hooks.
...
producer = UkGovDftBodsKafkaProducer(
    ...
    security_protocol='SASL_SSL',##### `uk_gov_dft_bods_kafka_vehicle_position_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_dft_bods_kafka_vehicle_position_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `uk.gov.dft.bods.kafka.VehiclePosition`: One current BODS SIRI-VM vehicle activity from
the bulk AVL archive. Each event represents the latest monitored vehicle journey state for one physical bus at one poll
cycle.
...
### UkGovDftBodsKafkaProducer- `data`: The event data of type `uk_bods_siri_producer_data.VehiclePosition`.



Producer for `uk.gov.dft.bods.kafka` message group.Example:



#### Constructor```python

async def uk_gov_dft_bods_kafka_vehicle_position_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
VehiclePosition) -> None:

```python    # Process the event data

UkGovDftBodsKafkaProducer(    await some_processing_function(record, cloud_event, data)
...
uk_gov_dft_bods_kafka_dispatcher.uk_gov_dft_bods_kafka_vehicle_position_async =
uk_gov_dft_bods_kafka_vehicle_position_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `uk_gov_dft_bods_kafka_operator_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_dft_bods_kafka_operator_async:  Callable[[ConsumerRecord, CloudEvent, Operator],
Awaitable[None]]
...
Asynchronous handler hook for `uk.gov.dft.bods.kafka.Operator`: Reference record for one bus operator code observed in
the BODS SIRI-VM bulk archive. The bridge extracts distinct operators from the same archive that supplies telemetry so
consumers can build operator context without a separate metadata call.
...
### UkGovDftBodsKafkaProducer- `data`: The event data of type `uk_bods_siri_producer_data.Operator`.
...
async def uk_gov_dft_bods_kafka_operator_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Operator) -> None:
...
uk_gov_dft_bods_kafka_dispatcher.uk_gov_dft_bods_kafka_operator_async = uk_gov_dft_bods_kafka_operator_event
...
##### `send_uk_gov_dft_bods_kafka_vehicle_position`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_dft_bods_kafka_vehicle_position(
...
Send a single `uk.gov.dft.bods.kafka.VehiclePosition` message. One current BODS SIRI-VM vehicle activity from the bulk
AVL archive. Each event represents the latest monitored vehicle journey state for one physical bus at one poll
cycle.Args:
...
await producer.send_uk_gov_dft_bods_kafka_vehicle_position(
...
##### `send_uk_gov_dft_bods_kafka_vehicle_position_batch`##### `_dispatch_record`



```python```python

async def send_uk_gov_dft_bods_kafka_vehicle_position_batch(_dispatch_record(self, record)
...
Send multiple `uk.gov.dft.bods.kafka.VehiclePosition` messages in a batch.
...
await producer.send_uk_gov_dft_bods_kafka_vehicle_position_batch(```
...
##### `send_uk_gov_dft_bods_kafka_operator`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_dft_bods_kafka_operator(
...
Send a single `uk.gov.dft.bods.kafka.Operator` message. Reference record for one bus operator code observed in the BODS
SIRI-VM bulk archive. The bridge extracts distinct operators from the same archive that supplies telemetry so consumers
can build operator context without a separate metadata call.Args:
...
await producer.send_uk_gov_dft_bods_kafka_operator(
...
##### `send_uk_gov_dft_bods_kafka_operator_batch`##### `_dispatch_record`



```python```python

async def send_uk_gov_dft_bods_kafka_operator_batch(_dispatch_record(self, record)

    self,```

    messages: List[Operator],
...
Send multiple `uk.gov.dft.bods.kafka.Operator` messages in a batch.
...
await producer.send_uk_gov_dft_bods_kafka_operator_batch(```
