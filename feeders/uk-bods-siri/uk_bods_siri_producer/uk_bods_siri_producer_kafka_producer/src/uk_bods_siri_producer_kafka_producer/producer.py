...
class UkGovDftBodsKafkaEventProducer:
    ...
    def send_uk_gov_dft_bods_kafka_vehicle_position(self,_feedurl : str, _operator_ref : str, _vehicle_ref : str, data: VehiclePosition, content_type: str = "application/json", _time: typing.Optional[typing.Union[str, datetime]] = None, flush_producer=True, key_mapper: typing.Callable[[CloudEvent, VehiclePosition], str]=None) -> None:
        """
        Sends the 'uk.gov.dft.bods.kafka.VehiclePosition' event to the Kafka topic
        ...
        Args:
            ...
            key_mapper(Callable[[CloudEvent, VehiclePosition], str]): A function to map the CloudEvent contents to a Kafka key (default: None).
                The default key is derived from the xRegistry Kafka key declaration '{operator_ref}/{vehicle_ref}'
        """
        kafka_key = "{operator_ref}/{vehicle_ref}".format(operator_ref=_operator_ref, vehicle_ref=_vehicle_ref)
        attributes = {
             "type":"uk.gov.dft.bods.VehiclePosition",
             "source":"{feedurl}".format(feedurl = _feedurl),
             "subject":"{operator_ref}/{vehicle_ref}".format(operator_ref = _operator_ref,vehicle_ref = _vehicle_ref)
        }
    ...
    def send_uk_gov_dft_bods_kafka_operator(self,_feedurl : str, _operator_ref : str, data: Operator, content_type: str = "application/json", _time: typing.Optional[typing.Union[str, datetime]] = None, flush_producer=True, key_mapper: typing.Callable[[CloudEvent, Operator], str]=None) -> None:
        """
        Sends the 'uk.gov.dft.bods.kafka.Operator' event to the Kafka topic
        ...
        Args:
            ...
            key_mapper(Callable[[CloudEvent, Operator], str]): A function to map the CloudEvent contents to a Kafka key (default: None).
                The default key is derived from the xRegistry Kafka key declaration '{operator_ref}'
        """
        kafka_key = "{operator_ref}".format(operator_ref=_operator_ref)
        attributes = {
             "type":"uk.gov.dft.bods.Operator",
             "source":"{feedurl}".format(feedurl = _feedurl),
             "subject":"{operator_ref}".format(operator_ref = _operator_ref)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        event = CloudEvent.create(attributes, data)
    ...
    @classmethod
    def from_connection_string(cls, connection_string: str, topic: typing.Optional[str]=None, content_mode: typing.Literal['structured','binary']='structured') -> 'UkGovDftBodsKafkaEventProducer':
        """
