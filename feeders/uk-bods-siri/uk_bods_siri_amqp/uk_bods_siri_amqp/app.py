...
from uk_bods_siri_amqp_producer_amqp_producer import UkGovDftBodsAmqpProducer
from uk_bods_siri_amqp_producer_data import Operator, VehiclePosition
...
def _build_producer(
    *,
    ...
    if auth_mode == "entra":
        ...
        return UkGovDftBodsAmqpProducer(
            host=host,
            address=address,
            port=port,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="UK BODS SIRI-VM → AMQP bridge.")
    ...
    feed_parser.add_argument("--address", type=str, default=os.getenv("AMQP_ADDRESS", "uk-bods-siri"))
