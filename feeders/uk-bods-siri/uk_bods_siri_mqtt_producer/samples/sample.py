...
import os
import signal
from uk_bods_siri_mqtt_producer_data import *
from uk_bods_siri_mqtt_producer_mqtt_client.client import UkGovDftBodsMqttProducer, UkGovDftBodsMqttDispatcher

async def handle_uk_gov_dft_bods_mqtt_vehicle_position(mqtt_msg,cloud_event, uk_gov_dft_bods_mqtt_vehicle_position_data):
    """ Handles the uk.gov.dft.bods.mqtt.VehiclePosition message """
    print(f"Received uk.gov.dft.bods.mqtt.VehiclePosition on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_gov_dft_bods_mqtt_vehicle_position_data}")

async def handle_uk_gov_dft_bods_mqtt_operator(mqtt_msg,cloud_event, uk_gov_dft_bods_mqtt_operator_data):
    """ Handles the uk.gov.dft.bods.mqtt.Operator message """
    print(f"Received uk.gov.dft.bods.mqtt.Operator on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_gov_dft_bods_mqtt_operator_data}")
