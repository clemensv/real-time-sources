...
def get_default_topic_mappings_uk_gov_dft_bods_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the uk.gov.dft.bods.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "uk.gov.dft.bods.mqtt.VehiclePosition": "transit/uk/dft/bods/{operator_ref}/{vehicle_ref}/position",
        "uk.gov.dft.bods.mqtt.Operator": "transit/uk/dft/bods/{operator_ref}/info",
    }


def get_subscription_topics_uk_gov_dft_bods_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_uk_gov_dft_bods_mqtt()
    topics = []
...
class UkGovDftBodsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the uk.gov.dft.bods.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_uk_gov_dft_bods_mqtt()
        ...
        self.uk_gov_dft_bods_mqtt_vehicle_position_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, uk_bods_siri_mqtt_producer_data.VehiclePosition, Dict[str, str]], Awaitable[None]]] = None
        
        self.uk_gov_dft_bods_mqtt_operator_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, uk_bods_siri_mqtt_producer_data.Operator, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "uk.gov.dft.bods.VehiclePosition":
            if self.uk_gov_dft_bods_mqtt_vehicle_position_async:
                try:
                    ...
                    data = uk_bods_siri_mqtt_producer_data.VehiclePosition.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.gov.dft.bods.mqtt.VehiclePosition")
                    await self.uk_gov_dft_bods_mqtt_vehicle_position_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_gov_dft_bods_mqtt_vehicle_position handler: {e}")
            return
        
        if event_type == "uk.gov.dft.bods.Operator":
            if self.uk_gov_dft_bods_mqtt_operator_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = uk_bods_siri_mqtt_producer_data.Operator.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.gov.dft.bods.mqtt.Operator")
                    await self.uk_gov_dft_bods_mqtt_operator_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_gov_dft_bods_mqtt_operator handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_uk_gov_dft_bods_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_uk_gov_dft_bods_mqtt_vehicle_position(self,
        feedurl: str,
        operator_ref: str,
        vehicle_ref: str,
        data: uk_bods_siri_mqtt_producer_data.VehiclePosition,
        ...
        """
        Publish the 'uk.gov.dft.bods.mqtt.VehiclePosition' event to an MQTT topic.

        Args:
        
            feedurl: URI template variable for 'feedurl'
            operator_ref: URI template variable for 'operator_ref'
            vehicle_ref: URI template variable for 'vehicle_ref'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'transit/uk/dft/bods/{operator_ref}/{vehicle_ref}/position'
        ...
        """
        target_topic = topic if topic is not None else "transit/uk/dft/bods/{operator_ref}/{vehicle_ref}/position"
        ...
        attributes = {
             "type":"uk.gov.dft.bods.VehiclePosition",
             "source":"{feedurl}".format(feedurl = feedurl),
             "subject":"{operator_ref}/{vehicle_ref}".format(operator_ref = operator_ref,vehicle_ref = vehicle_ref)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_uk_gov_dft_bods_mqtt_operator(self,
        feedurl: str,
        operator_ref: str,
        data: uk_bods_siri_mqtt_producer_data.Operator,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'uk.gov.dft.bods.mqtt.Operator' event to an MQTT topic.
        ...
        Args:
            ...
            operator_ref: URI template variable for 'operator_ref'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'transit/uk/dft/bods/{operator_ref}/info'
                with URI template placeholders substituted from the keyword arguments.
            ...
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "transit/uk/dft/bods/{operator_ref}/info"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
            "operator_ref": str(operator_ref),
        }
        ...
        attributes = {
             "type":"uk.gov.dft.bods.Operator",
             "source":"{feedurl}".format(feedurl = feedurl),
             "subject":"{operator_ref}".format(operator_ref = operator_ref)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
