# Uk_bods_siri_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the uk_bods_siri_mqtt_producer message definitions.

# Uk_bods_siri_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - UkGovDftBodsMqttEventDispatcher
...
from uk_bods_siri_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = UkGovDftBodsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### UkGovDftBodsMqttEventDispatcher



# Publish message`UkGovDftBodsMqttEventDispatcher` handles events for the uk.gov.dft.bods.mqtt message group.
...
from uk_bods_siri_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = UkGovDftBodsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe UkGovDftBodsMqttEventDispatcher defines the following event handler hooks.
...
```##### `uk_gov_dft_bods_mqtt_vehicle_position_async`



## Test```python

uk_gov_dft_bods_mqtt_vehicle_position_async:  Callable[[PartitionContext, EventData, CloudEvent, VehiclePosition],
Awaitable[None]]
...
```Asynchronous handler hook for `uk.gov.dft.bods.mqtt.VehiclePosition`: One current BODS SIRI-VM vehicle activity from
...
- `data`: The event data of type `uk_bods_siri_mqtt_producer_data.VehiclePosition`.

Example:

```python
async def uk_gov_dft_bods_mqtt_vehicle_position_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: VehiclePosition) -> None:
...
```python
uk_gov_dft_bods_mqtt_dispatcher.uk_gov_dft_bods_mqtt_vehicle_position_async =
uk_gov_dft_bods_mqtt_vehicle_position_event
```
...
```##### `uk_gov_dft_bods_mqtt_operator_async`



## Test```python

uk_gov_dft_bods_mqtt_operator_async:  Callable[[PartitionContext, EventData, CloudEvent, Operator], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `uk.gov.dft.bods.mqtt.Operator`: Reference record for one bus operator code observed in
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `uk_bods_siri_mqtt_producer_data.Operator`.

Example:

```python
async def uk_gov_dft_bods_mqtt_operator_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```python
uk_gov_dft_bods_mqtt_dispatcher.uk_gov_dft_bods_mqtt_operator_async = uk_gov_dft_bods_mqtt_operator_event
```
