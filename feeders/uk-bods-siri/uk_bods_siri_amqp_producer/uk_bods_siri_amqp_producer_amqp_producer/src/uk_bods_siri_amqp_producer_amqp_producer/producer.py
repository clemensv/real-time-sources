...
class UkGovDftBodsAmqpProducer:
    """
    Producer class to send messages in the `uk.gov.dft.bods.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_vehicle_position(self,
        ...
        """
        Send the `uk.gov.dft.bods.amqp.VehiclePosition` message
        One current BODS SIRI-VM vehicle activity from the bulk AVL archive. Each event represents the latest monitored vehicle journey state for one physical bus at one poll cycle.
        ...
        attributes = {
            "type":
            "uk.gov.dft.bods.VehiclePosition",
            "source":
    ...
    def send_vehicle_position_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `uk.gov.dft.bods.amqp.VehiclePosition` messages
    ...
    def send_operator(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `uk.gov.dft.bods.amqp.Operator` message
        Reference record for one bus operator code observed in the BODS SIRI-VM bulk archive. The bridge extracts distinct operators from the same archive that supplies telemetry so consumers can build operator context without a separate metadata call.
    ...
    def send_operator_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `uk.gov.dft.bods.amqp.Operator` messages
