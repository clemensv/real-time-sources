# Ndw_road_traffic_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the ndw_road_traffic_mqtt_producer message definitions.

# Ndw_road_traffic_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - NLNDWAVGMqttEventDispatcher,

    NLNDWDRIPMqttEventDispatcher,

    NLNDWMSIMqttEventDispatcher,

    NLNDWSituationsMqttEventDispatcher
...
from ndw_road_traffic_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = NLNDWAVGMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NLNDWAVGMqttEventDispatcher



# Publish message`NLNDWAVGMqttEventDispatcher` handles events for the NL.NDW.AVG.mqtt message group.
...
from ndw_road_traffic_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = NLNDWAVGMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
client = NLNDWDRIPMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
Hub.Args:- `consumer_group_name`: The name of the consumer group.- `eventhubs_fully_qualified_namespace`: The fully
qualified namespace of the Event Hub.

client = NLNDWMSIMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
Hub.Args:- `consumer_group_name`: The name of the consumer group.- `eventhubs_fully_qualified_namespace`: The fully
qualified namespace of the Event Hub.

client = NLNDWSituationsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe NLNDWAVGMqttEventDispatcher defines the following event handler hooks.
...
```##### `nl_ndw_avg_point_measurement_site_mqtt_async`



## Test```python

nl_ndw_avg_point_measurement_site_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, PointMeasurementSite],
Awaitable[None]]
...
```Asynchronous handler hook for `NL.NDW.AVG.PointMeasurementSite.mqtt`: Reference record for a point measurement site
from the Dutch NDW DATEX II measurement_current feed. Contains location, sensor technology type, and lane configuration
for a fixed inductive-loop or microwave sensor.
...
- `data`: The event data of type `ndw_road_traffic_mqtt_producer_data.PointMeasurementSite`.

Example:

```python
async def nl_ndw_avg_point_measurement_site_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: PointMeasurementSite) -> None:
...
```##### `nl_ndw_avg_route_measurement_site_mqtt_async`



## Test```python

nl_ndw_avg_route_measurement_site_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RouteMeasurementSite],
Awaitable[None]]
...
```Asynchronous handler hook for `NL.NDW.AVG.RouteMeasurementSite.mqtt`: Reference record for a route (section)
measurement site from the Dutch NDW DATEX II measurement_current feed. Covers a road segment between two coordinates
used for travel time computation.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ndw_road_traffic_mqtt_producer_data.RouteMeasurementSite`.

Example:

```python
async def nl_ndw_avg_route_measurement_site_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RouteMeasurementSite) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `nl_ndw_avg_traffic_observation_mqtt_async`



## Test```python

nl_ndw_avg_traffic_observation_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, TrafficObservation],
Awaitable[None]]
...
```Asynchronous handler hook for `NL.NDW.AVG.TrafficObservation.mqtt`: Aggregated traffic speed and flow observation
from the Dutch NDW DATEX II trafficspeed feed. Each record represents one measurement site with speed averaged and flow
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ndw_road_traffic_mqtt_producer_data.TrafficObservation`.

Example:

```python
async def nl_ndw_avg_traffic_observation_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: TrafficObservation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `nl_ndw_avg_travel_time_observation_mqtt_async`



## Test```python

nl_ndw_avg_travel_time_observation_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```Asynchronous handler hook for `NL.NDW.AVG.TravelTimeObservation.mqtt`: Travel time observation for a road segment
from the Dutch NDW DATEX II traveltime feed. Contains the actual measured travel time and the static free-flow reference
time for a route measurement site.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ndw_road_traffic_mqtt_producer_data.TravelTimeObservation`.

Example:

```python
async def nl_ndw_avg_travel_time_observation_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: TravelTimeObservation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = NLNDWDRIPMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NLNDWDRIPMqttEventDispatcher



# Publish message`NLNDWDRIPMqttEventDispatcher` handles events for the NL.NDW.DRIP.mqtt message group.
...
## BuildThe NLNDWDRIPMqttEventDispatcher defines the following event handler hooks.
...
```##### `nl_ndw_drip_drip_sign_mqtt_async`



## Test```python

nl_ndw_drip_drip_sign_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, DripSign], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `NL.NDW.DRIP.DripSign.mqtt`: Reference record for a Dynamic Route Information Panel
(DRIP) sign from the Dutch NDW DATEX II dynamische_route_informatie_paneel feed. Describes the physical installation,
location, and type of an individual VMS sign unit.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ndw_road_traffic_mqtt_producer_data.DripSign`.

Example:

```python
async def nl_ndw_drip_drip_sign_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: DripSign) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `nl_ndw_drip_drip_display_state_mqtt_async`



## Test```python

nl_ndw_drip_drip_display_state_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, DripDisplayState],
Awaitable[None]]
...
```Asynchronous handler hook for `NL.NDW.DRIP.DripDisplayState.mqtt`: Current display state of a Dynamic Route
Information Panel (DRIP) sign from the Dutch NDW DATEX II dynamische_route_informatie_paneel feed. Captures the active
text, pictogram codes, and operational state of the sign.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ndw_road_traffic_mqtt_producer_data.DripDisplayState`.

Example:

```python
async def nl_ndw_drip_drip_display_state_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: DripDisplayState) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = NLNDWMSIMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NLNDWMSIMqttEventDispatcher



# Publish message`NLNDWMSIMqttEventDispatcher` handles events for the NL.NDW.MSI.mqtt message group.
...
## BuildThe NLNDWMSIMqttEventDispatcher defines the following event handler hooks.
...
```##### `nl_ndw_msi_msi_sign_mqtt_async`



## Test```python

nl_ndw_msi_msi_sign_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, MsiSign], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ndw_road_traffic_mqtt_producer_data.MsiSign`.

Example:

```python
async def nl_ndw_msi_msi_sign_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: MsiSign) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `nl_ndw_msi_msi_display_state_mqtt_async`



## Test```python

nl_ndw_msi_msi_display_state_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, MsiDisplayState],
Awaitable[None]]
...
```Asynchronous handler hook for `NL.NDW.MSI.MsiDisplayState.mqtt`: Current display state of a Matrix Signal
Installation (MSI) sign from the Dutch NDW DATEX II Matrixsignaalinformatie feed. Captures the displayed image code,
operational state, and any speed limit shown.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ndw_road_traffic_mqtt_producer_data.MsiDisplayState`.

Example:

```python
async def nl_ndw_msi_msi_display_state_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: MsiDisplayState) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = NLNDWSituationsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NLNDWSituationsMqttEventDispatcher



# Publish message`NLNDWSituationsMqttEventDispatcher` handles events for the NL.NDW.Situations.mqtt message group.
...
## BuildThe NLNDWSituationsMqttEventDispatcher defines the following event handler hooks.
...
```##### `nl_ndw_situations_roadwork_mqtt_async`



## Test```python

nl_ndw_situations_roadwork_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, Roadwork], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ndw_road_traffic_mqtt_producer_data.Roadwork`.

Example:

```python
async def nl_ndw_situations_roadwork_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Roadwork) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `nl_ndw_situations_bridge_opening_mqtt_async`



## Test```python

nl_ndw_situations_bridge_opening_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, BridgeOpening],
Awaitable[None]]
...
```Asynchronous handler hook for `NL.NDW.Situations.BridgeOpening.mqtt`: Bridge opening event from the Dutch NDW DATEX
II planningsfeed_brugopeningen feed. Represents a scheduled or active bridge opening that causes temporary road closure.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ndw_road_traffic_mqtt_producer_data.BridgeOpening`.

Example:

```python
async def nl_ndw_situations_bridge_opening_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: BridgeOpening) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `nl_ndw_situations_temporary_closure_mqtt_async`



## Test```python

nl_ndw_situations_temporary_closure_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, TemporaryClosure],
Awaitable[None]]
...
```Asynchronous handler hook for `NL.NDW.Situations.TemporaryClosure.mqtt`: Temporary road closure from the Dutch NDW
DATEX II tijdelijke_verkeersmaatregelen_afsluitingen feed. Represents a temporary closure of a road section or lane on
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ndw_road_traffic_mqtt_producer_data.TemporaryClosure`.

Example:

```python
async def nl_ndw_situations_temporary_closure_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: TemporaryClosure) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `nl_ndw_situations_temporary_speed_limit_mqtt_async`



## Test```python

nl_ndw_situations_temporary_speed_limit_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```Asynchronous handler hook for `NL.NDW.Situations.TemporarySpeedLimit.mqtt`: Temporary speed limit measure from the
Dutch NDW DATEX II tijdelijke_verkeersmaatregelen_maximum_snelheden feed. Represents a temporary reduction in maximum
speed on a section of the national road network.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ndw_road_traffic_mqtt_producer_data.TemporarySpeedLimit`.

Example:

```python
async def nl_ndw_situations_temporary_speed_limit_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: TemporarySpeedLimit) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `nl_ndw_situations_safety_related_message_mqtt_async`



## Test```python

nl_ndw_situations_safety_related_message_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```Asynchronous handler hook for `NL.NDW.Situations.SafetyRelatedMessage.mqtt`: Safety-related traffic information
message from the Dutch NDW DATEX II veiligheidsgerelateerde_berichten_srti feed. Contains urgent safety alerts and
hazard notifications on the national road network.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ndw_road_traffic_mqtt_producer_data.SafetyRelatedMessage`.

Example:

```python
async def nl_ndw_situations_safety_related_message_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: SafetyRelatedMessage) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
