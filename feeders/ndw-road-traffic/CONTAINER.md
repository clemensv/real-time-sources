...
|---|---|
| `MAX_RECORDS_PER_FAMILY` | Optional cap on records emitted per event family per poll; `0` disables the cap (default `0`). |
| `NDW_BASE_URL` | NDW open-data base URL. |
| `REFERENCE_REFRESH_INTERVAL` | Seconds between refreshes of reference data (measurement-site metadata). |
| `SITUATION_INTERVAL` | Seconds between polls of the situation (incident/road-works) feed. |
