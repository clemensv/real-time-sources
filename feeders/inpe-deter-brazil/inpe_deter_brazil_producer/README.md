...
# Inpe-deter-brazil-producer Kafka Producer# Inpe-deter-brazil-producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - BRINPEDETEREventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    BRINPEDETERMqttEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    BRINPEDETERAmqpEventDispatcher
...
- BRINPEDETERProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- BRINPEDETERMqttProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- BRINPEDETERAmqpProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### BRINPEDETEREventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`BRINPEDETEREventDispatcher` handles events for the BR.INPE.DETER message group.
...
from inpe-deter-brazil-producer import BRINPEDETERProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = BRINPEDETERProducer(
...
await producer.send_br_inpe_deter_deforestation_alert(```python

    data=DeforestationAlert(...),add_consumer(self, consumer: KafkaConsumer)
...
The BRINPEDETEREventDispatcher defines the following event handler hooks.
...
producer = BRINPEDETERProducer(
    ...
    security_protocol='SASL_SSL',##### `br_inpe_deter_deforestation_alert_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'br_inpe_deter_deforestation_alert_async:  Callable[[ConsumerRecord, CloudEvent,
DeforestationAlert], Awaitable[None]]

)```

```

Asynchronous handler hook for `BR.INPE.DETER.DeforestationAlert`: INPE DETER deforestation alert for Amazon and Cerrado
biomes.
...
### BRINPEDETERProducer- `data`: The event data of type `inpe_deter_brazil_producer_data.DeforestationAlert`.



Producer for `BR.INPE.DETER` message group.Example:



#### Constructor```python

async def br_inpe_deter_deforestation_alert_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
DeforestationAlert) -> None:

```python    # Process the event data

BRINPEDETERProducer(    await some_processing_function(record, cloud_event, data)
...
br_inpe_deter_dispatcher.br_inpe_deter_deforestation_alert_async = br_inpe_deter_deforestation_alert_event
...
### BRINPEDETERMqttProducer- `data`: The event data of type `inpe_deter_brazil_producer_data.DeforestationAlert`.



Producer for `BR.INPE.DETER.mqtt` message group.Example:
...
BRINPEDETERMqttProducer(    await some_processing_function(record, cloud_event, data)
...
br_inpe_deter_mqtt_dispatcher.br_inpe_deter_deforestation_alert_async = br_inpe_deter_deforestation_alert_event
...
### BRINPEDETERAmqpProducer- `data`: The event data of type `inpe_deter_brazil_producer_data.DeforestationAlert`.



Producer for `BR.INPE.DETER.amqp` message group.Example:
...
BRINPEDETERAmqpProducer(    await some_processing_function(record, cloud_event, data)
...
br_inpe_deter_amqp_dispatcher.br_inpe_deter_deforestation_alert_async = br_inpe_deter_deforestation_alert_event
...
##### `send_br_inpe_deter_deforestation_alert`Dispatchers have the following protected methods:



```python### Methods:

async def send_br_inpe_deter_deforestation_alert(
...
Send a single `BR.INPE.DETER.DeforestationAlert` message. INPE DETER deforestation alert for Amazon and Cerrado
biomes.Args:
...
- `data`: Message data of type `DeforestationAlert`
...
await producer.send_br_inpe_deter_deforestation_alert(

    data=DeforestationAlert(...),Args:
...
Send multiple `BR.INPE.DETER.DeforestationAlert` messages in a batch.
...
await producer.send_br_inpe_deter_deforestation_alert_batch(```

    messages=[

        DeforestationAlert(...),Initializes the runner with a Kafka consumer.

        DeforestationAlert(...),

        DeforestationAlert(...)Args:
...
- **Scales horizontally** across multiple brokers and partitions### BRINPEDETERMqttEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`BRINPEDETERMqttEventDispatcher` handles events for the BR.INPE.DETER.mqtt message group.
...
The BRINPEDETERMqttEventDispatcher defines the following event handler hooks.
...
producer = BRINPEDETERProducer(
    ...
    security_protocol='SASL_SSL',##### `br_inpe_deter_mqtt_deforestation_alert_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'br_inpe_deter_mqtt_deforestation_alert_async:  Callable[[ConsumerRecord, CloudEvent,
DeforestationAlert], Awaitable[None]]
...
Asynchronous handler hook for `BR.INPE.DETER.mqtt.DeforestationAlert`: INPE DETER deforestation alert for Amazon and
Cerrado biomes.
...
async def br_inpe_deter_mqtt_deforestation_alert_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
DeforestationAlert) -> None:
...
br_inpe_deter_dispatcher.br_inpe_deter_mqtt_deforestation_alert_async = br_inpe_deter_mqtt_deforestation_alert_event
...
br_inpe_deter_mqtt_dispatcher.br_inpe_deter_mqtt_deforestation_alert_async =
br_inpe_deter_mqtt_deforestation_alert_event
...
br_inpe_deter_amqp_dispatcher.br_inpe_deter_mqtt_deforestation_alert_async =
br_inpe_deter_mqtt_deforestation_alert_event
...
##### `send_br_inpe_deter_mqtt_deforestation_alert`Dispatchers have the following protected methods:



```python### Methods:

async def send_br_inpe_deter_mqtt_deforestation_alert(
...
Send a single `BR.INPE.DETER.mqtt.DeforestationAlert` message. INPE DETER deforestation alert for Amazon and Cerrado
biomes.Args:
...
await producer.send_br_inpe_deter_mqtt_deforestation_alert(
...
Send multiple `BR.INPE.DETER.mqtt.DeforestationAlert` messages in a batch.
...
await producer.send_br_inpe_deter_mqtt_deforestation_alert_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### BRINPEDETERAmqpEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`BRINPEDETERAmqpEventDispatcher` handles events for the BR.INPE.DETER.amqp message group.
...
The BRINPEDETERAmqpEventDispatcher defines the following event handler hooks.
...
producer = BRINPEDETERProducer(
    ...
    security_protocol='SASL_SSL',##### `br_inpe_deter_amqp_deforestation_alert_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'br_inpe_deter_amqp_deforestation_alert_async:  Callable[[ConsumerRecord, CloudEvent,
DeforestationAlert], Awaitable[None]]
...
Asynchronous handler hook for `BR.INPE.DETER.amqp.DeforestationAlert`: INPE DETER deforestation alert for Amazon and
Cerrado biomes.
...
async def br_inpe_deter_amqp_deforestation_alert_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
DeforestationAlert) -> None:
...
br_inpe_deter_dispatcher.br_inpe_deter_amqp_deforestation_alert_async = br_inpe_deter_amqp_deforestation_alert_event
...
br_inpe_deter_mqtt_dispatcher.br_inpe_deter_amqp_deforestation_alert_async =
br_inpe_deter_amqp_deforestation_alert_event
...
br_inpe_deter_amqp_dispatcher.br_inpe_deter_amqp_deforestation_alert_async =
br_inpe_deter_amqp_deforestation_alert_event
...
##### `send_br_inpe_deter_amqp_deforestation_alert`Dispatchers have the following protected methods:



```python### Methods:

async def send_br_inpe_deter_amqp_deforestation_alert(
...
Send a single `BR.INPE.DETER.amqp.DeforestationAlert` message. INPE DETER deforestation alert for Amazon and Cerrado
biomes.Args:
...
await producer.send_br_inpe_deter_amqp_deforestation_alert(
...
Send multiple `BR.INPE.DETER.amqp.DeforestationAlert` messages in a batch.
...
await producer.send_br_inpe_deter_amqp_deforestation_alert_batch(```
