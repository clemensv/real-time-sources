# Inpe_deter_brazil_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the inpe_deter_brazil_mqtt_producer message definitions.

# Inpe_deter_brazil_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - BRINPEDETERMqttEventDispatcher
...
from inpe_deter_brazil_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = BRINPEDETERMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### BRINPEDETERMqttEventDispatcher



# Publish message`BRINPEDETERMqttEventDispatcher` handles events for the BR.INPE.DETER.mqtt message group.
...
from inpe_deter_brazil_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = BRINPEDETERMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe BRINPEDETERMqttEventDispatcher defines the following event handler hooks.
...
```##### `br_inpe_deter_mqtt_deforestation_alert_async`



## Test```python

br_inpe_deter_mqtt_deforestation_alert_async:  Callable[[PartitionContext, EventData, CloudEvent, DeforestationAlert],
Awaitable[None]]
...
- `data`: The event data of type `inpe_deter_brazil_mqtt_producer_data.DeforestationAlert`.

Example:

```python
async def br_inpe_deter_mqtt_deforestation_alert_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: DeforestationAlert) -> None:
