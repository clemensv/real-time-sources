...
"""
Tests for inpe_deter_brazil_amqp_producer_amqp_producer
"""
import base64
import json
import os
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../inpe_deter_brazil_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../inpe_deter_brazil_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../inpe_deter_brazil_amqp_producer_amqp_producer/src')))

from inpe_deter_brazil_amqp_producer_amqp_producer import *
from inpe_deter_brazil_amqp_producer_data import DeforestationAlert
from test_inpe_deter_brazil_amqp_producer_data_deforestationalert import Test_DeforestationAlert
...
class TestBRINPEDETERAmqpProducer:
    """Test cases for BRINPEDETERAmqpProducer"""
    ...
    def test_send_deforestation_alert(self, artemis_container):
        """Send and receive a DeforestationAlert message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_DeforestationAlert.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "BR.INPE.DETER.amqp.DeforestationAlert"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{biome}/{alert_id}".format(biome="value", alert_id="value")
        finally:
            producer.close()
