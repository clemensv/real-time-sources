...
class BRINPEDETERAmqpProducer:
    """
    Producer class to send messages in the `BR.INPE.DETER.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_deforestation_alert(self,
        ...
        """
        Send the `BR.INPE.DETER.amqp.DeforestationAlert` message
        ...
        attributes = {
            ...
            "subject":
            "{biome}/{alert_id}".format(biome=_biome, alert_id=_alert_id),
            "time":
        ...
        amqp_msg.subject = "{biome}/{alert_id}".format(biome=_biome, alert_id=_alert_id)
    ...
    def send_deforestation_alert_batch(self,
        data_array: typing.List[DeforestationAlert],
        _source_uri: str,
        _biome: str,
        _alert_id: str,
        _view_date: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `BR.INPE.DETER.amqp.DeforestationAlert` messages
        
        Args:
            data_array (typing.List[DeforestationAlert]): Array of message data objects
            _source_uri (str): Value for placeholder source_uri in attribute source
            _biome (str): Value for placeholder biome in attribute subject
            _alert_id (str): Value for placeholder alert_id in attribute subject
            _view_date (str): Value for placeholder view_date in attribute time
        ...
        for data in data_array:
            self.send_deforestation_alert(
                data=data,
                _source_uri=_source_uri,
                _biome=_biome,
                _alert_id=_alert_id,
                _view_date=_view_date,
