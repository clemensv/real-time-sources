...
"""
Tests for syke_hydro_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../syke_hydro_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../syke_hydro_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../syke_hydro_amqp_producer_amqp_producer/src')))

from syke_hydro_amqp_producer_amqp_producer import *
...
class TestFISYKEHydrologyAmqpProducer:
    """Test cases for FISYKEHydrologyAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(FISYKEHydrologyAmqpProducer)
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "FI.SYKE.Hydrology.amqp.Station"
    ...
    def test_send_station_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'FI.SYKE.Hydrology.amqp.Station'
        assert received.body is not None
        assert received.subject == "{station_id}".format(station_id="value")
        assert properties.get('basin') == "{basin}".format(basin="value")
        assert annotations.get(symbol('x-opt-partition-key')) == str("{station_id}".format(station_id="value"))[:128]
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "FI.SYKE.Hydrology.amqp.WaterLevelObservation"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_water_level_observation_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'FI.SYKE.Hydrology.amqp.WaterLevelObservation'
        assert received.body is not None
        assert received.subject == "{station_id}".format(station_id="value")
        assert properties.get('basin') == "{basin}".format(basin="value")
        assert annotations.get(symbol('x-opt-partition-key')) == str("{station_id}".format(station_id="value"))[:128]
