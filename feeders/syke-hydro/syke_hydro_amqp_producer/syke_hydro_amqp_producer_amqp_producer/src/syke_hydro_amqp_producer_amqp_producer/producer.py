    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from syke_hydro_amqp_producer_data import Station
from syke_hydro_amqp_producer_data import WaterLevelObservation

class FISYKEHydrologyAmqpProducer:
    """
    Producer class to send messages in the `FI.SYKE.Hydrology.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        ...
        """
        Send the `FI.SYKE.Hydrology.amqp.Station` message
        A reference record for one Finnish hydrological monitoring station published by the Finnish Environment Institute (SYKE). It fires when the bridge publishes or refreshes the station catalog so consumers can interpret measurement events.
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `FI.SYKE.Hydrology.amqp.Station` messages
    ...
    def send_water_level_observation(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `FI.SYKE.Hydrology.amqp.WaterLevelObservation` message
        A current measurement from the Finnish Environment Institute (SYKE) for one monitoring site. It carries water level and discharge observations when the upstream feed reports a new or refreshed value.
    ...
    def send_water_level_observation_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `FI.SYKE.Hydrology.amqp.WaterLevelObservation` messages
