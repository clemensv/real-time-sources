...
import os
import signal
from syke_hydro_mqtt_producer_data import *
from syke_hydro_mqtt_producer_mqtt_client.client import FISYKEHydrologyMqttProducer, FISYKEHydrologyMqttDispatcher

async def handle_fi_syke_hydrology_mqtt_station(mqtt_msg,cloud_event, fi_syke_hydrology_mqtt_station_data):
    """ Handles the FI.SYKE.Hydrology.mqtt.Station message """
    print(f"Received FI.SYKE.Hydrology.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {fi_syke_hydrology_mqtt_station_data}")

async def handle_fi_syke_hydrology_mqtt_water_level_observation(mqtt_msg,cloud_event, fi_syke_hydrology_mqtt_water_level_observation_data):
    """ Handles the FI.SYKE.Hydrology.mqtt.WaterLevelObservation message """
    print(f"Received FI.SYKE.Hydrology.mqtt.WaterLevelObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {fi_syke_hydrology_mqtt_water_level_observation_data}")
