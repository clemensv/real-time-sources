...
def get_default_topic_mappings_mode_s_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the Mode_S.mqtt message group.
...
def get_subscription_topics_mode_s_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_mode_s_mqtt()
    topics = []
...
class ModeSMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the Mode_S.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_mode_s_mqtt()
        ...
        self.mode_s_adsb_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, mode_s_mqtt_producer_data.Record, Dict[str, str]], Awaitable[None]]] = None
        
        self.mode_s_altitude_reply_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, mode_s_mqtt_producer_data.Record, Dict[str, str]], Awaitable[None]]] = None
        
        self.mode_s_identity_reply_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, mode_s_mqtt_producer_data.Record, Dict[str, str]], Awaitable[None]]] = None
        
        self.mode_s_acquisition_reply_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, mode_s_mqtt_producer_data.Record, Dict[str, str]], Awaitable[None]]] = None
        
        self.mode_s_comm_baltitude_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, mode_s_mqtt_producer_data.Record, Dict[str, str]], Awaitable[None]]] = None
        
        self.mode_s_comm_bidentity_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, mode_s_mqtt_producer_data.Record, Dict[str, str]], Awaitable[None]]] = None
        ...
        if event_type == "Mode_S.ADSB":
            if self.mode_s_adsb_mqtt_async:
                try:
                    ...
                    data = mode_s_mqtt_producer_data.Record.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Mode_S.ADSB.mqtt")
                    await self.mode_s_adsb_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
        ...
        if event_type == "Mode_S.AltitudeReply":
            if self.mode_s_altitude_reply_mqtt_async:
                try:
                    ...
                    # CloudEvent.data is now a dict or string, not bytes
                    data = mode_s_mqtt_producer_data.Record.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Mode_S.AltitudeReply.mqtt")
                    await self.mode_s_altitude_reply_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
        ...
        if event_type == "Mode_S.IdentityReply":
            if self.mode_s_identity_reply_mqtt_async:
                try:
                    ...
                    # CloudEvent.data is now a dict or string, not bytes
                    data = mode_s_mqtt_producer_data.Record.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Mode_S.IdentityReply.mqtt")
                    await self.mode_s_identity_reply_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
        ...
        if event_type == "Mode_S.AcquisitionReply":
            if self.mode_s_acquisition_reply_mqtt_async:
                try:
                    ...
                    # CloudEvent.data is now a dict or string, not bytes
                    data = mode_s_mqtt_producer_data.Record.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Mode_S.AcquisitionReply.mqtt")
                    await self.mode_s_acquisition_reply_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
        ...
        if event_type == "Mode_S.CommBAltitude":
            if self.mode_s_comm_baltitude_mqtt_async:
                try:
                    ...
                    # CloudEvent.data is now a dict or string, not bytes
                    data = mode_s_mqtt_producer_data.Record.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Mode_S.CommBAltitude.mqtt")
                    await self.mode_s_comm_baltitude_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
        ...
        if event_type == "Mode_S.CommBIdentity":
            if self.mode_s_comm_bidentity_mqtt_async:
                try:
                    ...
                    # CloudEvent.data is now a dict or string, not bytes
                    data = mode_s_mqtt_producer_data.Record.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Mode_S.CommBIdentity.mqtt")
                    await self.mode_s_comm_bidentity_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in mode_s_comm_bidentity_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_mode_s_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_mode_s_adsb_mqtt(self,
        feedurl: str,
        icao24: str,
        receiver_id: str,
        data: mode_s_mqtt_producer_data.Record,
    ...
    async def publish_mode_s_altitude_reply_mqtt(self,
        feedurl: str,
        icao24: str,
        receiver_id: str,
        data: mode_s_mqtt_producer_data.Record,
    ...
    async def publish_mode_s_identity_reply_mqtt(self,
        feedurl: str,
        icao24: str,
        receiver_id: str,
        data: mode_s_mqtt_producer_data.Record,
    ...
    async def publish_mode_s_acquisition_reply_mqtt(self,
        feedurl: str,
        icao24: str,
        receiver_id: str,
        data: mode_s_mqtt_producer_data.Record,
    ...
    async def publish_mode_s_comm_baltitude_mqtt(self,
        feedurl: str,
        icao24: str,
        receiver_id: str,
        data: mode_s_mqtt_producer_data.Record,
    ...
    async def publish_mode_s_comm_bidentity_mqtt(self,
        feedurl: str,
        icao24: str,
        receiver_id: str,
        data: mode_s_mqtt_producer_data.Record,
