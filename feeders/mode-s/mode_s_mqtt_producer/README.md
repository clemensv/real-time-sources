# Mode_s_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the mode_s_mqtt_producer message definitions.

# Mode_s_mqtt_producer Event Dispatcher for Azure Event Hubs
...
from mode_s_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = ModeSMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### ModeSMqttEventDispatcher



# Publish message`ModeSMqttEventDispatcher` handles events for the Mode_S.mqtt message group.
...
from mode_s_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = ModeSMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe ModeSMqttEventDispatcher defines the following event handler hooks.
...
```##### `mode_s_adsb_mqtt_async`



## Test```python

mode_s_adsb_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, Record], Awaitable[None]]
...
- `data`: The event data of type `mode_s_mqtt_producer_data.Record`.

Example:

```python
async def mode_s_adsb_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent, data:
Record) -> None:
...
```##### `mode_s_altitude_reply_mqtt_async`



## Test```python

mode_s_altitude_reply_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, Record], Awaitable[None]]
...
```python
async def mode_s_altitude_reply_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Record) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `mode_s_identity_reply_mqtt_async`



## Test```python

mode_s_identity_reply_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, Record], Awaitable[None]]
...
```python
async def mode_s_identity_reply_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Record) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `mode_s_acquisition_reply_mqtt_async`



## Test```python

mode_s_acquisition_reply_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, Record], Awaitable[None]]
...
```python
async def mode_s_acquisition_reply_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Record) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `mode_s_comm_baltitude_mqtt_async`



## Test```python

mode_s_comm_baltitude_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, Record], Awaitable[None]]
...
```python
async def mode_s_comm_baltitude_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Record) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `mode_s_comm_bidentity_mqtt_async`



## Test```python

mode_s_comm_bidentity_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, Record], Awaitable[None]]
...
```python
async def mode_s_comm_bidentity_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Record) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
