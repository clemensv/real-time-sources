...
Events from configurable SIRI-compatible transit feeds covering distinct operators and live vehicle positions extracted from VehicleMonitoring payloads.
...
- **`longitude`** (double or null, required, deg (°)): WGS84 longitude from `MonitoredVehicleJourney/VehicleLocation/Longitude` expressed in decimal degrees. Positive values are east of Greenwich. Null when the AVL feed does not provide a usable vehicle location. Constraints: minimum `-180`, maximum `180`.
- **`latitude`** (double or null, required, deg (°)): WGS84 latitude from `MonitoredVehicleJourney/VehicleLocation/Latitude` expressed in decimal degrees. Positive values are north of the equator. Null when the AVL feed does not provide a usable vehicle location. Constraints: minimum `-90`, maximum `90`.
- **`bearing`** (integer or null, required, deg (°)): Vehicle heading from `MonitoredVehicleJourney/Bearing` in whole degrees clockwise from true north. Null when the AVL producer omits heading information. Constraints: minimum `0`, maximum `360`.
...
- xRegistry manifest: [`xreg/siri.xreg.json`](xreg/siri.xreg.json)
