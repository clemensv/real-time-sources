...
"""
Tests for siri_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../siri_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../siri_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../siri_amqp_producer_amqp_producer/src')))

from siri_amqp_producer_amqp_producer import *
from siri_amqp_producer_data import VehiclePosition
from test_siri_amqp_producer_data_vehicleposition import Test_VehiclePosition
from siri_amqp_producer_data import Operator
from test_siri_amqp_producer_data_operator import Test_Operator
...
class TestOrgSiriAmqpProducer:
    """Test cases for OrgSiriAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(OrgSiriAmqpProducer)
    ...
    def test_send_vehicle_position(self, artemis_container):
        """Send and receive a VehiclePosition message via ActiveMQ Artemis."""
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "org.siri.amqp.VehiclePosition"
                ...
                assert properties.get('vehicle_ref') == "{vehicle_ref}".format(vehicle_ref="value")
                assert annotations.get(symbol('x-opt-partition-key')) == str("{operator_ref}/{vehicle_ref}".format(operator_ref="value", vehicle_ref="value"))[:128]
        finally:
            producer.close()

    def test_send_vehicle_position_single_fresh_connection(self, artemis_container):
        """Send exactly one VehiclePosition message on a fresh producer connection."""
        payload = Test_VehiclePosition.create_instance()
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.siri.amqp.VehiclePosition'
        assert received.body is not None
        assert received.subject == "{operator_ref}/{vehicle_ref}".format(operator_ref="value", vehicle_ref="value")
        assert properties.get('operator_ref') == "{operator_ref}".format(operator_ref="value")
        assert properties.get('vehicle_ref') == "{vehicle_ref}".format(vehicle_ref="value")
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "org.siri.amqp.Operator"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                ...
                assert properties.get('operator_ref') == "{operator_ref}".format(operator_ref="value")
                assert annotations.get(symbol('x-opt-partition-key')) == str("{operator_ref}".format(operator_ref="value"))[:128]
        finally:
            producer.close()
    ...
    def test_send_operator_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.siri.amqp.Operator'
        assert received.body is not None
        assert received.subject == "{operator_ref}".format(operator_ref="value")
        assert properties.get('operator_ref') == "{operator_ref}".format(operator_ref="value")
        assert annotations.get(symbol('x-opt-partition-key')) == str("{operator_ref}".format(operator_ref="value"))[:128]
