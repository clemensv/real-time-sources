    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from siri_amqp_producer_data import VehiclePosition
from siri_amqp_producer_data import Operator

class OrgSiriAmqpProducer:
    """
    Producer class to send messages in the `org.siri.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_vehicle_position(self,
        ...
        """
        Send the `org.siri.amqp.VehiclePosition` message
        One current SIRI VehicleActivity observation from a configured provider feed. Each event represents the latest monitored vehicle journey state for one physical vehicle at one poll cycle.
        
        Args:
            _feedurl (str): Value for placeholder feedurl in attribute source
            _operator_ref (str): Value for placeholder operator_ref in attribute subject
            _vehicle_ref (str): Value for placeholder vehicle_ref in attribute subject
        ...
        attributes = {
            ...
            "subject":
            "{operator_ref}/{vehicle_ref}".format(operator_ref=_operator_ref, vehicle_ref=_vehicle_ref),
        }
        ...
        amqp_msg.subject = "{operator_ref}/{vehicle_ref}".format(operator_ref=_operator_ref, vehicle_ref=_vehicle_ref)

        app_properties = {}
        app_properties["operator_ref"] = "{operator_ref}".format(operator_ref=_operator_ref)
        app_properties["vehicle_ref"] = "{vehicle_ref}".format(vehicle_ref=_vehicle_ref)
        if app_properties:
        ...
        annotations = {}
        annotation_value = "{operator_ref}/{vehicle_ref}".format(operator_ref=_operator_ref, vehicle_ref=_vehicle_ref)
    ...
    def send_vehicle_position_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.siri.amqp.VehiclePosition` messages
    ...
    def send_operator(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `org.siri.amqp.Operator` message
        Reference record for one operator code observed in the configured SIRI feed. The bridge extracts distinct operators from the same upstream payloads that supply telemetry so consumers can build operator context without a separate metadata call.
        ...
        attributes = {
            ...
            "{feedurl}".format(feedurl=_feedurl),
            "subject":
            "{operator_ref}".format(operator_ref=_operator_ref),
        }
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        ...
        if amqp_creation_time is not None:
            amqp_msg.creation_time = amqp_creation_time
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "{operator_ref}".format(operator_ref=_operator_ref)
        ...
        annotations = {}
        annotation_value = "{operator_ref}".format(operator_ref=_operator_ref)
        annotation_value = str(annotation_value)[:128]
        annotations[symbol("x-opt-partition-key")] = annotation_value
        if annotations:
            if amqp_msg.annotations is None:
    ...
    def send_operator_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.siri.amqp.Operator` messages
