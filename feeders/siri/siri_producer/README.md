...
# Siri_producer Kafka Producer# Siri_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - OrgSiriKafkaEventDispatcher
...
- OrgSiriKafkaProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### OrgSiriKafkaEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`OrgSiriKafkaEventDispatcher` handles events for the org.siri.kafka message group.
...
from siri_producer import OrgSiriKafkaProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = OrgSiriKafkaProducer(
...
await producer.send_org_siri_kafka_vehicle_position(```python
...
The OrgSiriKafkaEventDispatcher defines the following event handler hooks.
...
producer = OrgSiriKafkaProducer(
    ...
    security_protocol='SASL_SSL',##### `org_siri_kafka_vehicle_position_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'org_siri_kafka_vehicle_position_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `org.siri.kafka.VehiclePosition`: One current SIRI VehicleActivity observation from a
configured provider feed. Each event represents the latest monitored vehicle journey state for one physical vehicle at
one poll cycle.
...
### OrgSiriKafkaProducer- `data`: The event data of type `siri_producer_data.VehiclePosition`.



Producer for `org.siri.kafka` message group.Example:



#### Constructor```python

async def org_siri_kafka_vehicle_position_event(record: ConsumerRecord, cloud_event: CloudEvent, data: VehiclePosition)
-> None:

```python    # Process the event data

OrgSiriKafkaProducer(    await some_processing_function(record, cloud_event, data)
...
org_siri_kafka_dispatcher.org_siri_kafka_vehicle_position_async = org_siri_kafka_vehicle_position_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `org_siri_kafka_operator_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'org_siri_kafka_operator_async:  Callable[[ConsumerRecord, CloudEvent, Operator],
Awaitable[None]]
...
Asynchronous handler hook for `org.siri.kafka.Operator`: Reference record for one operator code observed in the
configured SIRI feed. The bridge extracts distinct operators from the same upstream payloads that supply telemetry so
...
### OrgSiriKafkaProducer- `data`: The event data of type `siri_producer_data.Operator`.
...
async def org_siri_kafka_operator_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Operator) -> None:
...
org_siri_kafka_dispatcher.org_siri_kafka_operator_async = org_siri_kafka_operator_event
...
##### `send_org_siri_kafka_vehicle_position`Dispatchers have the following protected methods:



```python### Methods:

async def send_org_siri_kafka_vehicle_position(
...
Send a single `org.siri.kafka.VehiclePosition` message. One current SIRI VehicleActivity observation from a configured
provider feed. Each event represents the latest monitored vehicle journey state for one physical vehicle at one poll
cycle.Args:
...
await producer.send_org_siri_kafka_vehicle_position(
...
##### `send_org_siri_kafka_vehicle_position_batch`##### `_dispatch_record`



```python```python

async def send_org_siri_kafka_vehicle_position_batch(_dispatch_record(self, record)
...
Send multiple `org.siri.kafka.VehiclePosition` messages in a batch.
...
await producer.send_org_siri_kafka_vehicle_position_batch(```
...
##### `send_org_siri_kafka_operator`Dispatchers have the following protected methods:



```python### Methods:

async def send_org_siri_kafka_operator(
...
Send a single `org.siri.kafka.Operator` message. Reference record for one operator code observed in the configured SIRI
feed. The bridge extracts distinct operators from the same upstream payloads that supply telemetry so consumers can
build operator context without a separate metadata call.Args:
...
await producer.send_org_siri_kafka_operator(
...
##### `send_org_siri_kafka_operator_batch`##### `_dispatch_record`



```python```python

async def send_org_siri_kafka_operator_batch(_dispatch_record(self, record)
...
Send multiple `org.siri.kafka.Operator` messages in a batch.
...
await producer.send_org_siri_kafka_operator_batch(```
