...
import os
import signal
from siri_mqtt_producer_data import *
from siri_mqtt_producer_mqtt_client.client import OrgSiriMqttProducer, OrgSiriMqttDispatcher

async def handle_org_siri_mqtt_vehicle_position(mqtt_msg,cloud_event, org_siri_mqtt_vehicle_position_data):
    """ Handles the org.siri.mqtt.VehiclePosition message """
    print(f"Received org.siri.mqtt.VehiclePosition on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_siri_mqtt_vehicle_position_data}")

async def handle_org_siri_mqtt_operator(mqtt_msg,cloud_event, org_siri_mqtt_operator_data):
    """ Handles the org.siri.mqtt.Operator message """
    print(f"Received org.siri.mqtt.Operator on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_siri_mqtt_operator_data}")
