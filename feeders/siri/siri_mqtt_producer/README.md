# Siri_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the siri_mqtt_producer message definitions.

# Siri_mqtt_producer Event Dispatcher for Azure Event Hubs
...
from siri_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = OrgSiriMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgSiriMqttEventDispatcher



# Publish message`OrgSiriMqttEventDispatcher` handles events for the org.siri.mqtt message group.
...
from siri_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = OrgSiriMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe OrgSiriMqttEventDispatcher defines the following event handler hooks.
...
```##### `org_siri_mqtt_vehicle_position_async`



## Test```python

org_siri_mqtt_vehicle_position_async:  Callable[[PartitionContext, EventData, CloudEvent, VehiclePosition],
Awaitable[None]]
...
```Asynchronous handler hook for `org.siri.mqtt.VehiclePosition`: One current SIRI VehicleActivity observation from a
...
- `data`: The event data of type `siri_mqtt_producer_data.VehiclePosition`.

Example:

```python
async def org_siri_mqtt_vehicle_position_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```python
org_siri_mqtt_dispatcher.org_siri_mqtt_vehicle_position_async = org_siri_mqtt_vehicle_position_event
```
...
```##### `org_siri_mqtt_operator_async`



## Test```python

org_siri_mqtt_operator_async:  Callable[[PartitionContext, EventData, CloudEvent, Operator], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `org.siri.mqtt.Operator`: Reference record for one operator code observed in the
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `siri_mqtt_producer_data.Operator`.

Example:

```python
async def org_siri_mqtt_operator_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: Operator) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
org_siri_mqtt_dispatcher.org_siri_mqtt_operator_async = org_siri_mqtt_operator_event
```
