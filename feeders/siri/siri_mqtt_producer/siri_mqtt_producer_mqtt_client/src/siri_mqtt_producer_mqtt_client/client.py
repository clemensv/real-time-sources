...
def get_default_topic_mappings_org_siri_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.siri.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.siri.mqtt.VehiclePosition": "transit/siri/{operator_ref}/{vehicle_ref}/position",
        "org.siri.mqtt.Operator": "transit/siri/{operator_ref}/info",
    }


def get_subscription_topics_org_siri_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_org_siri_mqtt()
    topics = []
...
class OrgSiriMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.siri.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_siri_mqtt()
        ...
        self.org_siri_mqtt_vehicle_position_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, siri_mqtt_producer_data.VehiclePosition, Dict[str, str]], Awaitable[None]]] = None
        
        self.org_siri_mqtt_operator_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, siri_mqtt_producer_data.Operator, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.siri.VehiclePosition":
            if self.org_siri_mqtt_vehicle_position_async:
                try:
                    ...
                    data = siri_mqtt_producer_data.VehiclePosition.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.siri.mqtt.VehiclePosition")
                    await self.org_siri_mqtt_vehicle_position_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_siri_mqtt_vehicle_position handler: {e}")
            return
        
        if event_type == "org.siri.Operator":
            if self.org_siri_mqtt_operator_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = siri_mqtt_producer_data.Operator.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.siri.mqtt.Operator")
                    await self.org_siri_mqtt_operator_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_siri_mqtt_operator handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_siri_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_org_siri_mqtt_vehicle_position(self,
        feedurl: str,
        operator_ref: str,
        vehicle_ref: str,
        data: siri_mqtt_producer_data.VehiclePosition,
        ...
        """
        Publish the 'org.siri.mqtt.VehiclePosition' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'transit/siri/{operator_ref}/{vehicle_ref}/position'
        ...
        """
        target_topic = topic if topic is not None else "transit/siri/{operator_ref}/{vehicle_ref}/position"
    ...
    async def publish_org_siri_mqtt_operator(self,
        feedurl: str,
        operator_ref: str,
        data: siri_mqtt_producer_data.Operator,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.siri.mqtt.Operator' event to an MQTT topic.
        ...
        Args:
            ...
            operator_ref: URI template variable for 'operator_ref'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'transit/siri/{operator_ref}/info'
                with URI template placeholders substituted from the keyword arguments.
            ...
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "transit/siri/{operator_ref}/info"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
            "operator_ref": str(operator_ref),
        }
