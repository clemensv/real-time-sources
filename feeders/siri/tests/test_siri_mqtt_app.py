...
def test_mqtt_feed_once_emits_reference_and_telemetry(tmp_path):
    from siri_mqtt import app as mqtt_app
    ...
    with mock.patch.object(mqtt_app.mqtt, "Client", return_value=fake_paho), mock.patch.object(
        mqtt_app, "OrgSiriMqttMqttClient", return_value=fake_client
    ):
        asyncio.run(
            mqtt_app.feed(
                api=api,
    ...
    fake_client.publish_org_siri_mqtt_operator.assert_awaited_once()
    fake_client.publish_org_siri_mqtt_vehicle_position.assert_awaited_once()
