    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from chmi_hydro_amqp_producer_data import Station
from chmi_hydro_amqp_producer_data import WaterLevelObservation

class CZGovCHMIHydroAmqpProducer:
    """
    Producer class to send messages in the `CZ.Gov.CHMI.Hydro.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        ...
        """
        Send the `CZ.Gov.CHMI.Hydro.amqp.Station` message
        A reference record for one Czech hydrological monitoring station published by the Czech Hydrometeorological Institute (ČHMÚ). It fires when the bridge publishes or refreshes the station catalog so consumers can interpret measurement events.
        
        Args:
            _station_id (str): Value for placeholder station_id in attribute subject
            _stream_name (str): Value for AMQP protocol option placeholder stream_name
        ...
        app_properties = {}
        app_properties["stream_name"] = "{stream_name}".format(stream_name=_stream_name)
        if app_properties:
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `CZ.Gov.CHMI.Hydro.amqp.Station` messages
    ...
    def send_water_level_observation(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `CZ.Gov.CHMI.Hydro.amqp.WaterLevelObservation` message
        A current measurement from the Czech Hydrometeorological Institute (ČHMÚ) for one monitoring site. It carries water level and discharge observations when the upstream feed reports a new or refreshed value.
    ...
    def send_water_level_observation_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `CZ.Gov.CHMI.Hydro.amqp.WaterLevelObservation` messages
