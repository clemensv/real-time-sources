...
import os
import signal
from chmi_hydro_mqtt_producer_data import *
from chmi_hydro_mqtt_producer_mqtt_client.client import CZGovCHMIHydroMqttProducer, CZGovCHMIHydroMqttDispatcher

async def handle_cz_gov_chmi_hydro_mqtt_station(mqtt_msg,cloud_event, cz_gov_chmi_hydro_mqtt_station_data):
    """ Handles the CZ.Gov.CHMI.Hydro.mqtt.Station message """
    print(f"Received CZ.Gov.CHMI.Hydro.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {cz_gov_chmi_hydro_mqtt_station_data}")

async def handle_cz_gov_chmi_hydro_mqtt_water_level_observation(mqtt_msg,cloud_event, cz_gov_chmi_hydro_mqtt_water_level_observation_data):
    """ Handles the CZ.Gov.CHMI.Hydro.mqtt.WaterLevelObservation message """
    print(f"Received CZ.Gov.CHMI.Hydro.mqtt.WaterLevelObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {cz_gov_chmi_hydro_mqtt_water_level_observation_data}")
