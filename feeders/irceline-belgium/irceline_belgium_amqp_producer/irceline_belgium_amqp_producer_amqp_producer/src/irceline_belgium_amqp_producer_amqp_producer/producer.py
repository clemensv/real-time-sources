    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from irceline_belgium_amqp_producer_data import Station
from irceline_belgium_amqp_producer_data import Timeseries
from irceline_belgium_amqp_producer_data import Observation

class BeIrcelineStationsAmqpProducer:
    """
    Producer class to send messages in the `be.irceline.Stations.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        ...
        """
        Send the `be.irceline.Stations.amqp.Station` message
        ...
        Args:
            ...
            _pollutant (str): Value for AMQP protocol option placeholder pollutant
        ...
        app_properties["pollutant"] = "{pollutant}".format(pollutant=_pollutant)
        if app_properties:
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `be.irceline.Stations.amqp.Station` messages
...
class BeIrcelineTimeseriesAmqpProducer:
    """
    Producer class to send messages in the `be.irceline.Timeseries.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_timeseries(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `be.irceline.Timeseries.amqp.Timeseries` message
        ...
        attributes = {
            ...
            "subject":
            "{timeseries_id}".format(timeseries_id=_timeseries_id),
        }
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        ...
        if amqp_creation_time is not None:
            amqp_msg.creation_time = amqp_creation_time
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "{timeseries_id}".format(timeseries_id=_timeseries_id)
    ...
    def send_timeseries_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `be.irceline.Timeseries.amqp.Timeseries` messages
    ...
    def send_observation(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `be.irceline.Timeseries.amqp.Observation` message
        A current environmental measurement from Belgium's IRCELINE interregional environment agency. It carries pollutant concentration and air-quality measurements when the upstream feed reports a new or refreshed value.
    ...
    def send_observation_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `be.irceline.Timeseries.amqp.Observation` messages
