...
import os
import signal
from irceline_belgium_mqtt_producer_data import *
from irceline_belgium_mqtt_producer_mqtt_client.client import BeIrcelineStationsMqttProducer, BeIrcelineStationsMqttDispatcher

async def handle_be_irceline_stations_mqtt_station(mqtt_msg,cloud_event, be_irceline_stations_mqtt_station_data):
    """ Handles the be.irceline.Stations.mqtt.Station message """
    print(f"Received be.irceline.Stations.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {be_irceline_stations_mqtt_station_data}")
from irceline_belgium_mqtt_producer_mqtt_client.client import BeIrcelineTimeseriesMqttProducer, BeIrcelineTimeseriesMqttDispatcher

async def handle_be_irceline_timeseries_mqtt_timeseries(mqtt_msg,cloud_event, be_irceline_timeseries_mqtt_timeseries_data):
    """ Handles the be.irceline.Timeseries.mqtt.Timeseries message """
    print(f"Received be.irceline.Timeseries.mqtt.Timeseries on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {be_irceline_timeseries_mqtt_timeseries_data}")

async def handle_be_irceline_timeseries_mqtt_observation(mqtt_msg,cloud_event, be_irceline_timeseries_mqtt_observation_data):
    """ Handles the be.irceline.Timeseries.mqtt.Observation message """
    print(f"Received be.irceline.Timeseries.mqtt.Observation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {be_irceline_timeseries_mqtt_observation_data}")
