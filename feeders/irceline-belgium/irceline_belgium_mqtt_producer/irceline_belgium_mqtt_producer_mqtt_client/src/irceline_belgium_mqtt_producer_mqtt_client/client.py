...
def get_default_topic_mappings_be_irceline_stations_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the be.irceline.Stations.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "be.irceline.Stations.mqtt.Station": "air-quality/be/irceline/irceline-belgium/{region}/{station_id}/info",
    }


def get_subscription_topics_be_irceline_stations_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_be_irceline_stations_mqtt()
    topics = []
...
class BeIrcelineStationsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the be.irceline.Stations.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_be_irceline_stations_mqtt()
        ...
        self.be_irceline_stations_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, irceline_belgium_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "be.irceline.Station":
            if self.be_irceline_stations_mqtt_station_async:
                try:
                    ...
                    data = irceline_belgium_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "be.irceline.Stations.mqtt.Station")
                    await self.be_irceline_stations_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in be_irceline_stations_mqtt_station handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_be_irceline_stations_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_be_irceline_stations_mqtt_station(self,
        station_id: str,
        region: str,
        data: irceline_belgium_mqtt_producer_data.Station,
        ...
        """
        Publish the 'be.irceline.Stations.mqtt.Station' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'air-quality/be/irceline/irceline-belgium/{region}/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "air-quality/be/irceline/irceline-belgium/{region}/{station_id}/info"
...
def get_default_topic_mappings_be_irceline_timeseries_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the be.irceline.Timeseries.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "be.irceline.Timeseries.mqtt.Timeseries": "air-quality/be/irceline/irceline-belgium/{region}/{station_id}/{pollutant}/timeseries",
        "be.irceline.Timeseries.mqtt.Observation": "air-quality/be/irceline/irceline-belgium/{region}/{station_id}/{pollutant}/observation",
    }


def get_subscription_topics_be_irceline_timeseries_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_be_irceline_timeseries_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class BeIrcelineTimeseriesMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the be.irceline.Timeseries.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_be_irceline_timeseries_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.be_irceline_timeseries_mqtt_timeseries_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, irceline_belgium_mqtt_producer_data.Timeseries, Dict[str, str]], Awaitable[None]]] = None
        
        self.be_irceline_timeseries_mqtt_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, irceline_belgium_mqtt_producer_data.Observation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "be.irceline.Timeseries":
            if self.be_irceline_timeseries_mqtt_timeseries_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = irceline_belgium_mqtt_producer_data.Timeseries.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "be.irceline.Timeseries.mqtt.Timeseries")
                    await self.be_irceline_timeseries_mqtt_timeseries_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in be_irceline_timeseries_mqtt_timeseries handler: {e}")
            return
        
        if event_type == "be.irceline.Observation":
            if self.be_irceline_timeseries_mqtt_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = irceline_belgium_mqtt_producer_data.Observation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "be.irceline.Timeseries.mqtt.Observation")
                    await self.be_irceline_timeseries_mqtt_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in be_irceline_timeseries_mqtt_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_be_irceline_timeseries_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_be_irceline_timeseries_mqtt_timeseries(self,
        timeseries_id: str,
        region: str,
        station_id: str,
        pollutant: str,
        data: irceline_belgium_mqtt_producer_data.Timeseries,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'be.irceline.Timeseries.mqtt.Timeseries' event to an MQTT topic.
        ...
        Args:
            ...
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/be/irceline/irceline-belgium/{region}/{station_id}/{pollutant}/timeseries'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/be/irceline/irceline-belgium/{region}/{station_id}/{pollutant}/timeseries"
        _topic_template_values: Dict[str, str] = {
    ...
    async def publish_be_irceline_timeseries_mqtt_observation(self,
        timeseries_id: str,
        region: str,
        station_id: str,
        pollutant: str,
        data: irceline_belgium_mqtt_producer_data.Observation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'be.irceline.Timeseries.mqtt.Observation' event to an MQTT topic.
        ...
        Args:
            ...
            pollutant: URI template variable for 'pollutant'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/be/irceline/irceline-belgium/{region}/{station_id}/{pollutant}/observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/be/irceline/irceline-belgium/{region}/{station_id}/{pollutant}/observation"
        _topic_template_values: Dict[str, str] = {
            "timeseries_id": str(timeseries_id),
            "region": str(region),
            "station_id": str(station_id),
