# Ndl_netherlands_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the ndl_netherlands_mqtt_producer message definitions.

# Ndl_netherlands_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - NLNDWTrafficMeasurementsMqttEventDispatcher,

    NLNDWTrafficSituationsMqttEventDispatcher
...
from ndl_netherlands_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = NLNDWTrafficMeasurementsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NLNDWTrafficMeasurementsMqttEventDispatcher



# Publish message`NLNDWTrafficMeasurementsMqttEventDispatcher` handles events for the NL.NDW.Traffic.Measurements.mqtt
message group.
...
from ndl_netherlands_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = NLNDWTrafficMeasurementsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of
...
client = NLNDWTrafficSituationsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe NLNDWTrafficMeasurementsMqttEventDispatcher defines the following event handler hooks.
...
```##### `nl_ndw_traffic_traffic_speed_mqtt_async`



## Test```python

nl_ndw_traffic_traffic_speed_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, TrafficSpeed],
Awaitable[None]]
...
```Asynchronous handler hook for `NL.NDW.Traffic.TrafficSpeed.mqtt`: Aggregated traffic speed and flow measurement per
road segment from the Dutch NDW DATEX II trafficspeed feed. Each record represents one measurement site with speed
averaged and flow summed across all reporting lanes.
...
- `data`: The event data of type `ndl_netherlands_mqtt_producer_data.TrafficSpeed`.

Example:

```python
async def nl_ndw_traffic_traffic_speed_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: TrafficSpeed) -> None:
...
```python
nl_ndw_traffic_measurements_mqtt_dispatcher.nl_ndw_traffic_traffic_speed_mqtt_async =
nl_ndw_traffic_traffic_speed_mqtt_event
```
...
```##### `nl_ndw_traffic_travel_time_mqtt_async`



## Test```python

nl_ndw_traffic_travel_time_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, TravelTime], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `NL.NDW.Traffic.TravelTime.mqtt`: Travel time measurement for a road segment from the
Dutch NDW DATEX II traveltime feed. Each record contains the actual measured travel time and the static free-flow
reference time for a measurement site.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ndl_netherlands_mqtt_producer_data.TravelTime`.

Example:

```python
async def nl_ndw_traffic_travel_time_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: TravelTime) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
nl_ndw_traffic_measurements_mqtt_dispatcher.nl_ndw_traffic_travel_time_mqtt_async =
nl_ndw_traffic_travel_time_mqtt_event
```


## Generated Event Dispatchers

client = NLNDWTrafficSituationsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NLNDWTrafficSituationsMqttEventDispatcher



# Publish message`NLNDWTrafficSituationsMqttEventDispatcher` handles events for the NL.NDW.Traffic.Situations.mqtt
message group.
...
## BuildThe NLNDWTrafficSituationsMqttEventDispatcher defines the following event handler hooks.
...
```##### `nl_ndw_traffic_traffic_situation_mqtt_async`



## Test```python

nl_ndw_traffic_traffic_situation_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, TrafficSituation],
Awaitable[None]]
...
```Asynchronous handler hook for `NL.NDW.Traffic.TrafficSituation.mqtt`: Current traffic situation record from the Dutch
NDW DATEX II actueel_beeld (current situation overview) feed. Includes road works, closures, lane management, and other
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ndl_netherlands_mqtt_producer_data.TrafficSituation`.

Example:

```python
async def nl_ndw_traffic_traffic_situation_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: TrafficSituation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
nl_ndw_traffic_situations_mqtt_dispatcher.nl_ndw_traffic_traffic_situation_mqtt_async =
nl_ndw_traffic_traffic_situation_mqtt_event
```
