...
class GoesXrayFlux:
    ...
    AvroType: typing.ClassVar[avro.schema.Schema] = avro.schema.parse(
        "{\"type\": \"record\", \"name\": \"GoesXrayFlux\", \"doc\": \"One-minute resolution full-disk solar X-ray flux measurement from the GOES primary satellite XRS (X-Ray Sensor) instrument, as reported by the SWPC goes/primary/xrays-7-day endpoint. Two energy bands are reported: 0.05-0.4 nm (short, harder X-rays) and 0.1-0.8 nm (long, softer X-rays). The 0.1-0.8 nm band is used for the official solar flare classification scale (A < B < C < M < X).\", \"fields\": [{\"name\": \"time_tag\", \"type\": \"string\", \"doc\": \"UTC date-time of the X-ray flux measurement from the GOES XRS instrument, in ISO 8601 format (e.g., '2024-01-01T00:00:00Z').\"}, {\"name\": \"satellite\", \"type\": \"integer\", \"doc\": \"GOES satellite number providing the measurement (e.g., 18 for GOES-18, 19 for GOES-19). The primary operational GOES satellite changes when new spacecraft are commissioned.\"}, {\"name\": \"flux\", \"type\": \"double\", \"doc\": \"Solar X-ray irradiance in the specified energy band. Flare classification thresholds for the 0.1-0.8 nm band: A (< 1e-7), B (1e-7 to 1e-6), C (1e-6 to 1e-5), M (1e-5 to 1e-4), X (>= 1e-4). Background is typically A1-B1 level during solar minimum.\"}, {\"name\": \"energy\", \"type\": \"string\", \"doc\": \"X-ray energy band identifier: '0.05-0.4nm' (short wavelength, harder X-rays) or '0.1-0.8nm' (long wavelength, softer X-rays used for flare classification).\"}]}"
    )
    ...
    @classmethod
    def from_serializer_dict(cls, data: dict) -> 'GoesXrayFlux':
        """
        ...
        return cls(**data)
    @classmethod
    def from_avro_dict(cls, data: dict) -> 'GoesXrayFlux':
        """
    ...
    @classmethod
    def from_data(cls, data: typing.Any, content_type_string: typing.Optional[str] = None) -> typing.Optional['GoesXrayFlux']:
        """
        ...
        if base_content_type in ['avro/binary', 'application/vnd.apache.avro+avro']:
            if isinstance(data, bytes):
                ...
                with io.BytesIO(data) as stream:
                    ...
                    return GoesXrayFlux.from_avro_dict(_record)
            else:
        ...
        if base_content_type == 'application/json':
            if isinstance(data, (bytes, str)):
                ...
                return GoesXrayFlux.from_serializer_dict(_record)
            else:
                raise NotImplementedError('Data is not of a supported type for JSON deserialization')
        raise NotImplementedError(f'Unsupported media type {content_type}')

    @classmethod
    def create_instance(cls) -> 'GoesXrayFlux':
        """
        ...
        """
        return cls(
            time_tag='pbwbdyqoezfygrlikvyz',
            satellite=int(10),
            flux=float(19.73880304227451),
            energy='hlrhnhtrsuajqwdxpvsq'
        )