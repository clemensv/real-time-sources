...
class GoesElectronFlux:
    ...
    AvroType: typing.ClassVar[avro.schema.Schema] = avro.schema.parse(
        "{\"type\": \"record\", \"name\": \"GoesElectronFlux\", \"doc\": \"Five-minute resolution integral electron flux measurement from the GOES primary satellite SEISS/SGPS instrument, as reported by the SWPC goes/primary/integral-electrons-3-day endpoint. The >= 2 MeV channel is monitored for deep dielectric charging risk to geosynchronous spacecraft. Elevated fluxes (> 1000 e/cm\u00b2/sr/s) sustained for several days can cause internal charging failures in satellite electronics.\", \"fields\": [{\"name\": \"time_tag\", \"type\": \"string\", \"doc\": \"UTC date-time of the electron flux measurement in ISO 8601 format.\"}, {\"name\": \"satellite\", \"type\": \"integer\", \"doc\": \"GOES satellite number providing the measurement (e.g., 19 for GOES-19).\"}, {\"name\": \"flux\", \"type\": \"double\", \"doc\": \"Integral electron flux above the specified energy threshold, in electrons per cm\u00b2 per steradian per second. Values above 1000 indicate elevated deep dielectric charging risk for geosynchronous satellites.\"}, {\"name\": \"energy\", \"type\": \"string\", \"doc\": \"Electron energy threshold identifier, currently '>=2 MeV'. This channel is monitored for spacecraft internal charging hazard assessment.\"}]}"
    )
    ...
    flux: float=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="flux"))
    energy: str=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="energy"))

    @classmethod
    def from_serializer_dict(cls, data: dict) -> 'GoesElectronFlux':
        """
        ...
        return cls(**data)
    @classmethod
    def from_avro_dict(cls, data: dict) -> 'GoesElectronFlux':
        """
    ...
    @classmethod
    def from_data(cls, data: typing.Any, content_type_string: typing.Optional[str] = None) -> typing.Optional['GoesElectronFlux']:
        """
        ...
        if base_content_type in ['avro/binary', 'application/vnd.apache.avro+avro']:
            if isinstance(data, bytes):
                ...
                with io.BytesIO(data) as stream:
                    ...
                    return GoesElectronFlux.from_avro_dict(_record)
            else:
        ...
        if base_content_type == 'application/json':
            if isinstance(data, (bytes, str)):
                ...
                return GoesElectronFlux.from_serializer_dict(_record)
            else:
                raise NotImplementedError('Data is not of a supported type for JSON deserialization')
        raise NotImplementedError(f'Unsupported media type {content_type}')

    @classmethod
    def create_instance(cls) -> 'GoesElectronFlux':
        """
