...
def get_default_topic_mappings_microsoft_opendata_us_noaa_swpc_goes_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the Microsoft.OpenData.US.NOAA.SWPC.GOES.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "Microsoft.OpenData.US.NOAA.SWPC.GoesXrayFlux.mqtt": "space-weather/us/noaa/noaa-goes/{satellite}/xrs/{event}",
        "Microsoft.OpenData.US.NOAA.SWPC.GoesProtonFlux.mqtt": "space-weather/us/noaa/noaa-goes/{satellite}/sgps/{event}",
        "Microsoft.OpenData.US.NOAA.SWPC.GoesElectronFlux.mqtt": "space-weather/us/noaa/noaa-goes/{satellite}/exis/{event}",
        "Microsoft.OpenData.US.NOAA.SWPC.GoesMagnetometer.mqtt": "space-weather/us/noaa/noaa-goes/{satellite}/magnetometer/{event}",
        "Microsoft.OpenData.US.NOAA.SWPC.SpaceWeatherAlert.mqtt": "space-weather/us/noaa/noaa-goes/alerts/{product_id}/alert",
        "Microsoft.OpenData.US.NOAA.SWPC.XrayFlare.mqtt": "space-weather/us/noaa/noaa-goes/flares/{flare_class}/{begin_time}/flare",
    }


def get_subscription_topics_microsoft_opendata_us_noaa_swpc_goes_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_microsoft_opendata_us_noaa_swpc_goes_mqtt()
    topics = []
...
class MicrosoftOpenDataUSNOAASWPCGOESMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the Microsoft.OpenData.US.NOAA.SWPC.GOES.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_microsoft_opendata_us_noaa_swpc_goes_mqtt()
        ...
        self.microsoft_open_data_us_noaa_swpc_goes_xray_flux_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, noaa_goes_mqtt_producer_data.GoesXrayFlux, Dict[str, str]], Awaitable[None]]] = None
        
        self.microsoft_open_data_us_noaa_swpc_goes_proton_flux_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, noaa_goes_mqtt_producer_data.GoesProtonFlux, Dict[str, str]], Awaitable[None]]] = None
        
        self.microsoft_open_data_us_noaa_swpc_goes_electron_flux_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, noaa_goes_mqtt_producer_data.GoesElectronFlux, Dict[str, str]], Awaitable[None]]] = None
        
        self.microsoft_open_data_us_noaa_swpc_goes_magnetometer_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, noaa_goes_mqtt_producer_data.GoesMagnetometer, Dict[str, str]], Awaitable[None]]] = None
        
        self.microsoft_open_data_us_noaa_swpc_space_weather_alert_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, noaa_goes_mqtt_producer_data.SpaceWeatherAlert, Dict[str, str]], Awaitable[None]]] = None
        
        self.microsoft_open_data_us_noaa_swpc_xray_flare_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, noaa_goes_mqtt_producer_data.XrayFlare, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "Microsoft.OpenData.US.NOAA.SWPC.GoesXrayFlux":
            if self.microsoft_open_data_us_noaa_swpc_goes_xray_flux_mqtt_async:
                try:
                    ...
                    data = noaa_goes_mqtt_producer_data.GoesXrayFlux.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Microsoft.OpenData.US.NOAA.SWPC.GoesXrayFlux.mqtt")
                    await self.microsoft_open_data_us_noaa_swpc_goes_xray_flux_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in microsoft_open_data_us_noaa_swpc_goes_xray_flux_mqtt handler: {e}")
            return
        
        if event_type == "Microsoft.OpenData.US.NOAA.SWPC.GoesProtonFlux":
            if self.microsoft_open_data_us_noaa_swpc_goes_proton_flux_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = noaa_goes_mqtt_producer_data.GoesProtonFlux.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Microsoft.OpenData.US.NOAA.SWPC.GoesProtonFlux.mqtt")
                    await self.microsoft_open_data_us_noaa_swpc_goes_proton_flux_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in microsoft_open_data_us_noaa_swpc_goes_proton_flux_mqtt handler: {e}")
            return
        
        if event_type == "Microsoft.OpenData.US.NOAA.SWPC.GoesElectronFlux":
            if self.microsoft_open_data_us_noaa_swpc_goes_electron_flux_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = noaa_goes_mqtt_producer_data.GoesElectronFlux.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Microsoft.OpenData.US.NOAA.SWPC.GoesElectronFlux.mqtt")
                    await self.microsoft_open_data_us_noaa_swpc_goes_electron_flux_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in microsoft_open_data_us_noaa_swpc_goes_electron_flux_mqtt handler: {e}")
            return
        
        if event_type == "Microsoft.OpenData.US.NOAA.SWPC.GoesMagnetometer":
            if self.microsoft_open_data_us_noaa_swpc_goes_magnetometer_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = noaa_goes_mqtt_producer_data.GoesMagnetometer.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Microsoft.OpenData.US.NOAA.SWPC.GoesMagnetometer.mqtt")
                    await self.microsoft_open_data_us_noaa_swpc_goes_magnetometer_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in microsoft_open_data_us_noaa_swpc_goes_magnetometer_mqtt handler: {e}")
            return
        
        if event_type == "Microsoft.OpenData.US.NOAA.SWPC.SpaceWeatherAlert":
            if self.microsoft_open_data_us_noaa_swpc_space_weather_alert_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = noaa_goes_mqtt_producer_data.SpaceWeatherAlert.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Microsoft.OpenData.US.NOAA.SWPC.SpaceWeatherAlert.mqtt")
                    await self.microsoft_open_data_us_noaa_swpc_space_weather_alert_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in microsoft_open_data_us_noaa_swpc_space_weather_alert_mqtt handler: {e}")
            return
        
        if event_type == "Microsoft.OpenData.US.NOAA.SWPC.XrayFlare":
            if self.microsoft_open_data_us_noaa_swpc_xray_flare_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = noaa_goes_mqtt_producer_data.XrayFlare.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Microsoft.OpenData.US.NOAA.SWPC.XrayFlare.mqtt")
                    await self.microsoft_open_data_us_noaa_swpc_xray_flare_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in microsoft_open_data_us_noaa_swpc_xray_flare_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_microsoft_opendata_us_noaa_swpc_goes_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_microsoft_open_data_us_noaa_swpc_goes_xray_flux_mqtt(self,
        satellite: str,
        energy: str,
        time_tag: str,
        event: str,
        data: noaa_goes_mqtt_producer_data.GoesXrayFlux,
        ...
        """
        Publish the 'Microsoft.OpenData.US.NOAA.SWPC.GoesXrayFlux.mqtt' event to an MQTT topic.

        Args:
        
            satellite: URI template variable for 'satellite'
            energy: URI template variable for 'energy'
            time_tag: URI template variable for 'time_tag'
            event: URI template variable for 'event'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'space-weather/us/noaa/noaa-goes/{satellite}/xrs/{event}'
        ...
        """
        target_topic = topic if topic is not None else "space-weather/us/noaa/noaa-goes/{satellite}/xrs/{event}"
        ...
        attributes = {
             ...
             "subject":"{satellite}/{energy}/{time_tag}".format(satellite = satellite,energy = energy,time_tag = time_tag)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_microsoft_open_data_us_noaa_swpc_goes_proton_flux_mqtt(self,
        satellite: str,
        energy: str,
        time_tag: str,
        event: str,
        data: noaa_goes_mqtt_producer_data.GoesProtonFlux,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Microsoft.OpenData.US.NOAA.SWPC.GoesProtonFlux.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            event: URI template variable for 'event'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'space-weather/us/noaa/noaa-goes/{satellite}/sgps/{event}'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "space-weather/us/noaa/noaa-goes/{satellite}/sgps/{event}"
        _topic_template_values: Dict[str, str] = {
            "satellite": str(satellite),
            "energy": str(energy),
            "time_tag": str(time_tag),
    ...
    async def publish_microsoft_open_data_us_noaa_swpc_goes_electron_flux_mqtt(self,
        satellite: str,
        energy: str,
        time_tag: str,
        event: str,
        data: noaa_goes_mqtt_producer_data.GoesElectronFlux,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Microsoft.OpenData.US.NOAA.SWPC.GoesElectronFlux.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            event: URI template variable for 'event'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'space-weather/us/noaa/noaa-goes/{satellite}/exis/{event}'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "space-weather/us/noaa/noaa-goes/{satellite}/exis/{event}"
        _topic_template_values: Dict[str, str] = {
            "satellite": str(satellite),
            "energy": str(energy),
            "time_tag": str(time_tag),
    ...
    async def publish_microsoft_open_data_us_noaa_swpc_goes_magnetometer_mqtt(self,
        satellite: str,
        time_tag: str,
        event: str,
        data: noaa_goes_mqtt_producer_data.GoesMagnetometer,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Microsoft.OpenData.US.NOAA.SWPC.GoesMagnetometer.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            event: URI template variable for 'event'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'space-weather/us/noaa/noaa-goes/{satellite}/magnetometer/{event}'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "space-weather/us/noaa/noaa-goes/{satellite}/magnetometer/{event}"
        _topic_template_values: Dict[str, str] = {
            "satellite": str(satellite),
            "time_tag": str(time_tag),
            "event": str(event),
        ...
        attributes = {
             ...
             "source":"https://services.swpc.noaa.gov",
             "subject":"{satellite}/{time_tag}".format(satellite = satellite,time_tag = time_tag)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
    ...
    async def publish_microsoft_open_data_us_noaa_swpc_space_weather_alert_mqtt(self,
        product_id: str,
        data: noaa_goes_mqtt_producer_data.SpaceWeatherAlert,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Microsoft.OpenData.US.NOAA.SWPC.SpaceWeatherAlert.mqtt' event to an MQTT topic.

        Args:
        
            product_id: URI template variable for 'product_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'space-weather/us/noaa/noaa-goes/alerts/{product_id}/alert'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "space-weather/us/noaa/noaa-goes/alerts/{product_id}/alert"
        _topic_template_values: Dict[str, str] = {
        ...
        attributes = {
             ...
             "source":"https://services.swpc.noaa.gov",
             "subject":"{product_id}".format(product_id = product_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
    ...
    async def publish_microsoft_open_data_us_noaa_swpc_xray_flare_mqtt(self,
        satellite: str,
        begin_time: str,
        flare_class: str,
        data: noaa_goes_mqtt_producer_data.XrayFlare,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Microsoft.OpenData.US.NOAA.SWPC.XrayFlare.mqtt' event to an MQTT topic.

        Args:
        
            satellite: URI template variable for 'satellite'
            begin_time: URI template variable for 'begin_time'
            flare_class: URI template variable for 'flare_class'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'space-weather/us/noaa/noaa-goes/flares/{flare_class}/{begin_time}/flare'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "space-weather/us/noaa/noaa-goes/flares/{flare_class}/{begin_time}/flare"
        _topic_template_values: Dict[str, str] = {
            "satellite": str(satellite),
        ...
        attributes = {
             ...
             "source":"https://services.swpc.noaa.gov",
             "subject":"{satellite}/{begin_time}".format(satellite = satellite,begin_time = begin_time)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
