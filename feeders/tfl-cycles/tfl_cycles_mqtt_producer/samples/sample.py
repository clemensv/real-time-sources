...
import os
import signal
from tfl_cycles_mqtt_producer_data import *
from tfl_cycles_mqtt_producer_mqtt_client.client import UKGovTfLCyclesMqttStationsProducer, UKGovTfLCyclesMqttStationsDispatcher

async def handle_uk_gov_tf_l_cycles_mqtt_station_information(mqtt_msg,cloud_event, uk_gov_tf_l_cycles_mqtt_station_information_data):
    """ Handles the UK.Gov.TfL.Cycles.mqtt.StationInformation message """
    print(f"Received UK.Gov.TfL.Cycles.mqtt.StationInformation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_gov_tf_l_cycles_mqtt_station_information_data}")

async def handle_uk_gov_tf_l_cycles_mqtt_station_status(mqtt_msg,cloud_event, uk_gov_tf_l_cycles_mqtt_station_status_data):
    """ Handles the UK.Gov.TfL.Cycles.mqtt.StationStatus message """
    print(f"Received UK.Gov.TfL.Cycles.mqtt.StationStatus on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_gov_tf_l_cycles_mqtt_station_status_data}")
