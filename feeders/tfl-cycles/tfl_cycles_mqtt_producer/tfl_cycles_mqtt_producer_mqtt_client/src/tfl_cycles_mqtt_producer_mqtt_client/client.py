...
import tfl_cycles_mqtt_producer_data
from tfl_cycles_mqtt_producer_data import StationInformation
from tfl_cycles_mqtt_producer_data import StationStatus
...
def get_default_topic_mappings_uk_gov_tfl_cycles_mqtt_stations() -> Dict[str, str]:
    """
    Get the default topic mappings for the UK.Gov.TfL.Cycles.mqtt.stations message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "UK.Gov.TfL.Cycles.mqtt.StationInformation": "mobility/tfl-cycles/{station_id}/info",
        "UK.Gov.TfL.Cycles.mqtt.StationStatus": "mobility/tfl-cycles/{station_id}/status",
    }


def get_subscription_topics_uk_gov_tfl_cycles_mqtt_stations(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_uk_gov_tfl_cycles_mqtt_stations()
    topics = []
...
class UKGovTfLCyclesMqttStationsMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the UK.Gov.TfL.Cycles.mqtt.stations message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_uk_gov_tfl_cycles_mqtt_stations()
        ...
        self.uk_gov_tf_l_cycles_mqtt_station_information_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, tfl_cycles_mqtt_producer_data.StationInformation, Dict[str, str]], Awaitable[None]]] = None
        
        self.uk_gov_tf_l_cycles_mqtt_station_status_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, tfl_cycles_mqtt_producer_data.StationStatus, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "UK.Gov.TfL.Cycles.StationInformation":
            if self.uk_gov_tf_l_cycles_mqtt_station_information_async:
                try:
                    ...
                    data = tfl_cycles_mqtt_producer_data.StationInformation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "UK.Gov.TfL.Cycles.mqtt.StationInformation")
                    await self.uk_gov_tf_l_cycles_mqtt_station_information_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_gov_tf_l_cycles_mqtt_station_information handler: {e}")
            return
        
        if event_type == "UK.Gov.TfL.Cycles.StationStatus":
            if self.uk_gov_tf_l_cycles_mqtt_station_status_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = tfl_cycles_mqtt_producer_data.StationStatus.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "UK.Gov.TfL.Cycles.mqtt.StationStatus")
                    await self.uk_gov_tf_l_cycles_mqtt_station_status_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_gov_tf_l_cycles_mqtt_station_status handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_uk_gov_tfl_cycles_mqtt_stations(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_uk_gov_tf_l_cycles_mqtt_station_information(self,
        feedurl: str,
        station_id: str,
        data: tfl_cycles_mqtt_producer_data.StationInformation,
        ...
        """
        Publish the 'UK.Gov.TfL.Cycles.mqtt.StationInformation' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'mobility/tfl-cycles/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "mobility/tfl-cycles/{station_id}/info"
    ...
    async def publish_uk_gov_tf_l_cycles_mqtt_station_status(self,
        feedurl: str,
        station_id: str,
        data: tfl_cycles_mqtt_producer_data.StationStatus,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'UK.Gov.TfL.Cycles.mqtt.StationStatus' event to an MQTT topic.
        ...
        Args:
            ...
            station_id: URI template variable for 'station_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'mobility/tfl-cycles/{station_id}/status'
                with URI template placeholders substituted from the keyword arguments.
            ...
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "mobility/tfl-cycles/{station_id}/status"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
            "station_id": str(station_id),
        }
