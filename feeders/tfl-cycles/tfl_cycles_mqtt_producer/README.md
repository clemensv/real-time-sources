# Tfl_cycles_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the tfl_cycles_mqtt_producer message definitions.

# Tfl_cycles_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - UKGovTfLCyclesMqttStationsEventDispatcher
...
from tfl_cycles_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = UKGovTfLCyclesMqttStationsMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### UKGovTfLCyclesMqttStationsEventDispatcher



# Publish message`UKGovTfLCyclesMqttStationsEventDispatcher` handles events for the UK.Gov.TfL.Cycles.mqtt.stations
message group.
...
from tfl_cycles_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = UKGovTfLCyclesMqttStationsMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe UKGovTfLCyclesMqttStationsEventDispatcher defines the following event handler hooks.
...
```##### `uk_gov_tf_l_cycles_mqtt_station_information_async`



## Test```python

uk_gov_tf_l_cycles_mqtt_station_information_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```Asynchronous handler hook for `UK.Gov.TfL.Cycles.mqtt.StationInformation`: Reference data describing one Transport
for London cycle-hire docking station (BikePoint), sourced from the TfL Unified API `GET /BikePoint` response `Place`
...
- `data`: The event data of type `tfl_cycles_mqtt_producer_data.StationInformation`.

Example:

```python
async def uk_gov_tf_l_cycles_mqtt_station_information_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: StationInformation) -> None:
...
```##### `uk_gov_tf_l_cycles_mqtt_station_status_async`



## Test```python

uk_gov_tf_l_cycles_mqtt_station_status_async:  Callable[[PartitionContext, EventData, CloudEvent, StationStatus],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `tfl_cycles_mqtt_producer_data.StationStatus`.

Example:

```python
async def uk_gov_tf_l_cycles_mqtt_station_status_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: StationStatus) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
