...
def get_default_topic_mappings_jp_tepco_denkiyoho_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the JP.TEPCO.Denkiyoho.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "JP.TEPCO.Denkiyoho.mqtt.SupplyCapacity": "energy/jp/tepco/tepco-denkiyoho/jp-tepco/supply-capacity",
        "JP.TEPCO.Denkiyoho.mqtt.PeakDemandForecast": "energy/jp/tepco/tepco-denkiyoho/jp-tepco/peak-demand-forecast",
        "JP.TEPCO.Denkiyoho.mqtt.DemandActual": "energy/jp/tepco/tepco-denkiyoho/jp-tepco/demand-actual",
        "JP.TEPCO.Denkiyoho.mqtt.DemandForecast": "energy/jp/tepco/tepco-denkiyoho/jp-tepco/demand-forecast",
        "JP.TEPCO.Denkiyoho.mqtt.Info": "energy/jp/tepco/tepco-denkiyoho/jp-tepco/info",
    }


def get_subscription_topics_jp_tepco_denkiyoho_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_jp_tepco_denkiyoho_mqtt()
    topics = []
...
class JPTEPCODenkiyohoMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the JP.TEPCO.Denkiyoho.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_jp_tepco_denkiyoho_mqtt()
        ...
        self.jp_tepco_denkiyoho_mqtt_supply_capacity_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, tepco_denkiyoho_mqtt_producer_data.SupplyCapacity, Dict[str, str]], Awaitable[None]]] = None
        
        self.jp_tepco_denkiyoho_mqtt_peak_demand_forecast_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, tepco_denkiyoho_mqtt_producer_data.PeakDemandForecast, Dict[str, str]], Awaitable[None]]] = None
        
        self.jp_tepco_denkiyoho_mqtt_demand_actual_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, tepco_denkiyoho_mqtt_producer_data.DemandActual, Dict[str, str]], Awaitable[None]]] = None
        
        self.jp_tepco_denkiyoho_mqtt_demand_forecast_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, tepco_denkiyoho_mqtt_producer_data.DemandForecast, Dict[str, str]], Awaitable[None]]] = None
        
        self.jp_tepco_denkiyoho_mqtt_info_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, tepco_denkiyoho_mqtt_producer_data.Info, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "JP.TEPCO.Denkiyoho.SupplyCapacity":
            if self.jp_tepco_denkiyoho_mqtt_supply_capacity_async:
                try:
                    ...
                    data = tepco_denkiyoho_mqtt_producer_data.SupplyCapacity.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "JP.TEPCO.Denkiyoho.mqtt.SupplyCapacity")
                    await self.jp_tepco_denkiyoho_mqtt_supply_capacity_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_tepco_denkiyoho_mqtt_supply_capacity handler: {e}")
            return
        
        if event_type == "JP.TEPCO.Denkiyoho.PeakDemandForecast":
            if self.jp_tepco_denkiyoho_mqtt_peak_demand_forecast_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = tepco_denkiyoho_mqtt_producer_data.PeakDemandForecast.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "JP.TEPCO.Denkiyoho.mqtt.PeakDemandForecast")
                    await self.jp_tepco_denkiyoho_mqtt_peak_demand_forecast_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_tepco_denkiyoho_mqtt_peak_demand_forecast handler: {e}")
            return
        
        if event_type == "JP.TEPCO.Denkiyoho.DemandActual":
            if self.jp_tepco_denkiyoho_mqtt_demand_actual_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = tepco_denkiyoho_mqtt_producer_data.DemandActual.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "JP.TEPCO.Denkiyoho.mqtt.DemandActual")
                    await self.jp_tepco_denkiyoho_mqtt_demand_actual_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_tepco_denkiyoho_mqtt_demand_actual handler: {e}")
            return
        
        if event_type == "JP.TEPCO.Denkiyoho.DemandForecast":
            if self.jp_tepco_denkiyoho_mqtt_demand_forecast_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = tepco_denkiyoho_mqtt_producer_data.DemandForecast.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "JP.TEPCO.Denkiyoho.mqtt.DemandForecast")
                    await self.jp_tepco_denkiyoho_mqtt_demand_forecast_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_tepco_denkiyoho_mqtt_demand_forecast handler: {e}")
            return
        
        if event_type == "JP.TEPCO.Denkiyoho.Info":
            if self.jp_tepco_denkiyoho_mqtt_info_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = tepco_denkiyoho_mqtt_producer_data.Info.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "JP.TEPCO.Denkiyoho.mqtt.Info")
                    await self.jp_tepco_denkiyoho_mqtt_info_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_tepco_denkiyoho_mqtt_info handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_jp_tepco_denkiyoho_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_jp_tepco_denkiyoho_mqtt_supply_capacity(self,
        ...
        """
        Publish the 'JP.TEPCO.Denkiyoho.mqtt.SupplyCapacity' event to an MQTT topic.
    ...
    async def publish_jp_tepco_denkiyoho_mqtt_peak_demand_forecast(self,
        ...
        content_type: str = "application/json") -> None:
        """
        Publish the 'JP.TEPCO.Denkiyoho.mqtt.PeakDemandForecast' event to an MQTT topic.
    ...
    async def publish_jp_tepco_denkiyoho_mqtt_demand_actual(self,
        ...
        content_type: str = "application/json") -> None:
        """
        Publish the 'JP.TEPCO.Denkiyoho.mqtt.DemandActual' event to an MQTT topic.
    ...
    async def publish_jp_tepco_denkiyoho_mqtt_demand_forecast(self,
        ...
        content_type: str = "application/json") -> None:
        """
        Publish the 'JP.TEPCO.Denkiyoho.mqtt.DemandForecast' event to an MQTT topic.
    ...
    async def publish_jp_tepco_denkiyoho_mqtt_info(self,
        area_code: str,
        data: tepco_denkiyoho_mqtt_producer_data.Info,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'JP.TEPCO.Denkiyoho.mqtt.Info' event to an MQTT topic.

        Args:
        
            area_code: URI template variable for 'area_code'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'energy/jp/tepco/tepco-denkiyoho/jp-tepco/info'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "energy/jp/tepco/tepco-denkiyoho/jp-tepco/info"
        _topic_template_values: Dict[str, str] = {
            "area_code": str(area_code),
        }
        if _topic_template_values:
