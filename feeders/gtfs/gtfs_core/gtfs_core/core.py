...
def map_agency(rows: Iterable[Dict[str, Any]]) -> Generator[Agency, None, None]:
    ...
    for row in rows:
        yield Agency(
            agencyId=str(row.get("agency_id")) if row.get("agency_id") is not None else None,
            agencyName=str(row.get("agency_name")) if row.get("agency_name") is not None else None,
            agencyUrl=str(row.get("agency_url")) if row.get("agency_url") is not None else None,
            agencyTimezone=str(row.get("agency_timezone")) if row.get("agency_timezone") is not None else None,
...
def map_areas(rows: Iterable[Dict[str, Any]]) -> Generator[Areas, None, None]:
    ...
    for row in rows:
        yield Areas(
            areaId=str(row.get("area_id")) if row.get("area_id") is not None else None,
            areaName=str(row.get("area_name")) if row.get("area_name") is not None else None,
...
def map_attributions(rows: Iterable[Dict[str, Any]]) -> Generator[Attributions, None, None]:
    ...
    for row in rows:
        yield Attributions(
            ...
            organizationName=str(row.get("organization_name")) if row.get("organization_name") is not None else None,
...
def map_booking_rules(rows: Iterable[Dict[str, Any]]) -> Generator[BookingRules, None, None]:
    ...
    for row in rows:
        yield BookingRules(
            bookingRuleId=str(row.get("booking_rule_id")) if row.get("booking_rule_id") is not None else None,
            bookingRuleName=str(row.get("booking_rule_name")) if row.get("booking_rule_name") is not None else None,
...
def map_fare_attributes(rows: Iterable[Dict[str, Any]]) -> Generator[FareAttributes, None, None]:
    ...
    for row in rows:
        yield FareAttributes(
            fareId=str(row.get("fare_id")) if row.get("fare_id") is not None else None,
            price=float(row.get("price") or 0),
            currencyType=str(row.get("currency_type")) if row.get("currency_type") is not None else None,
...
def map_fare_leg_rules(rows: Iterable[Dict[str, Any]]) -> Generator[FareLegRules, None, None]:
    ...
    for row in rows:
        yield FareLegRules(
            fareLegRuleId=str(row.get("fare_leg_rule_id")) if row.get("fare_leg_rule_id") is not None else None,
            fareProductId=str(row.get("fare_product_id")) if row.get("fare_product_id") is not None else None,
...
def map_fare_media(rows: Iterable[Dict[str, Any]]) -> Generator[FareMedia, None, None]:
    ...
    for row in rows:
        yield FareMedia(
            fareMediaId=str(row.get("fare_media_id")) if row.get("fare_media_id") is not None else None,
            fareMediaName=str(row.get("fare_media_name")) if row.get("fare_media_name") is not None else None,
...
def map_fare_products(rows: Iterable[Dict[str, Any]]) -> Generator[FareProducts, None, None]:
    ...
    for row in rows:
        yield FareProducts(
            fareProductId=str(row.get("fare_product_id")) if row.get("fare_product_id") is not None else None,
            fareProductName=str(row.get("fare_product_name")) if row.get("fare_product_name") is not None else None,
...
def map_fare_transfer_rules(rows: Iterable[Dict[str, Any]]) -> Generator[FareTransferRules, None, None]:
    ...
    for row in rows:
        yield FareTransferRules(
            fareTransferRuleId=str(row.get("fare_transfer_rule_id")) if row.get("fare_transfer_rule_id") is not None else None,
            fareProductId=str(row.get("fare_product_id")) if row.get("fare_product_id") is not None else None,
...
def map_feed_info(rows: Iterable[Dict[str, Any]]) -> Generator[FeedInfo, None, None]:
    ...
    for row in rows:
        yield FeedInfo(
            feedPublisherName=str(row.get("feed_publisher_name")) if row.get("feed_publisher_name") is not None else None,
            feedPublisherUrl=str(row.get("feed_publisher_url")) if row.get("feed_publisher_url") is not None else None,
            feedLang=str(row.get("feed_lang")) if row.get("feed_lang") is not None else None,
...
def map_frequencies(rows: Iterable[Dict[str, Any]]) -> Generator[Frequencies, None, None]:
    ...
    for row in rows:
        yield Frequencies(
            ...
            startTime=str(row.get("start_time")) if row.get("start_time") is not None else None,
            endTime=str(row.get("end_time")) if row.get("end_time") is not None else None,
...
def map_levels(rows: Iterable[Dict[str, Any]]) -> Generator[Levels, None, None]:
    ...
    for row in rows:
        yield Levels(
            levelId=str(row.get("level_id")) if row.get("level_id") is not None else None,
...
def map_location_groups(rows: Iterable[Dict[str, Any]]) -> Generator[LocationGroups, None, None]:
    ...
    for row in rows:
        yield LocationGroups(
            locationGroupId=str(row.get("location_group_id")) if row.get("location_group_id") is not None else None,
            locationGroupName=str(row.get("location_group_name")) if row.get("location_group_name") is not None else None,
...
def map_location_group_stores(rows: Iterable[Dict[str, Any]]) -> Generator[LocationGroupStores, None, None]:
    ...
    for row in rows:
        yield LocationGroupStores(
            locationGroupStoreId=str(row.get("location_group_store_id")) if row.get("location_group_store_id") is not None else None,
            locationGroupId=str(row.get("location_group_id")) if row.get("location_group_id") is not None else None,
            storeId=str(row.get("store_id")) if row.get("store_id") is not None else None
        )
...
def map_networks(rows: Iterable[Dict[str, Any]]) -> Generator[Networks, None, None]:
    ...
    for row in rows:
        yield Networks(
            networkId=str(row.get("network_id")) if row.get("network_id") is not None else None,
            networkName=str(row.get("network_name")) if row.get("network_name") is not None else None,
...
def map_pathways(rows: Iterable[Dict[str, Any]]) -> Generator[Pathways, None, None]:
    ...
    for row in rows:
        yield Pathways(
            pathwayId=str(row.get("pathway_id")) if row.get("pathway_id") is not None else None,
            fromStopId=str(row.get("from_stop_id")) if row.get("from_stop_id") is not None else None,
            toStopId=str(row.get("to_stop_id")) if row.get("to_stop_id") is not None else None,
...
def map_route_networks(rows: Iterable[Dict[str, Any]]) -> Generator[RouteNetworks, None, None]:
    ...
    for row in rows:
        yield RouteNetworks(
            routeNetworkId=str(row.get("route_network_id")) if row.get("route_network_id") is not None else None,
...
def map_shapes(rows: Iterable[Dict[str, Any]]) -> Generator[Shapes, None, None]:
    ...
    for row in rows:
        yield Shapes(
            shapeId=str(row.get("shape_id")) if row.get("shape_id") is not None else None,
...
def map_stop_areas(rows: Iterable[Dict[str, Any]]) -> Generator[StopAreas, None, None]:
    ...
    for row in rows:
        yield StopAreas(
            stopAreaId=str(row.get("stop_area_id")) if row.get("stop_area_id") is not None else None,
            areaId=str(row.get("area_id")) if row.get("area_id") is not None else None,
            stopId=str(row.get("stop_id")) if row.get("stop_id") is not None else None,
        )
...
def map_timeframes(rows: Iterable[Dict[str, Any]], calendar_rows: List[Dict[str, Any]], calendar_dates_rows: List[Dict[str, Any]]) -> Generator[Timeframes, None, None]:
    ...
    for row in rows:
        ...
        yield Timeframes(
            timeframeGroupId=str(row.get("timeframe_group_id")) if row.get("timeframe_group_id") is not None else None,
...
def map_translations(rows: Iterable[Dict[str, Any]]) -> Generator[Translations, None, None]:
    ...
    for row in rows:
        yield Translations(
            tableName=str(row.get("table_name")) if row.get("table_name") is not None else None,
            fieldName=str(row.get("field_name")) if row.get("field_name") is not None else None,
            language=str(row.get("language")) if row.get("language") is not None else None,
            translation=str(row.get("translation")) if row.get("translation") is not None else None
        )
