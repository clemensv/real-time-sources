...
- `hsl-helsinki` — disabled, keyless Helsinki Regional Transport (HSL, Finland) GTFS-Realtime feeds from `realtime.hsl.fi`: vehicle positions (~1 s), trip updates (~15 s), and service alerts (~5 min) for metro, tram, bus, commuter rail, and the Suomenlinna ferry, plus the public HSL GTFS Schedule zip. No API key needed — the `api.digitransit.fi` subscription key applies only to the separate GraphQL routing/geocoding APIs, not these protobuf feeds.
- `bkk-budapest` — disabled, BKK Futar (Budapest, Hungary) GTFS-Realtime feeds from `go.bkk.hu`: vehicle positions, trip updates, and service alerts for metro, tram, trolleybus, bus, HEV suburban rail, and Danube boats. The realtime feeds need a free BKK OpenData key carried as the `?key=${BKK_API_KEY}` query parameter (register at `opendata.bkk.hu`, generate under Key management); the GTFS Schedule zip at `bkk.hu` is keyless.
- `stm-montreal` — disabled, Societe de transport de Montreal (STM; Montreal, Canada) GTFS-Realtime feeds from `api.stm.info`: vehicle positions and trip updates for the STM bus and metro network (no service-alerts feed). The realtime feeds need a free STM API key carried as the `apiKey` request header via `${STM_API_KEY}` (register at `portail.developpeurs.stm.info`, create an application, add the GTFS-Realtime API and accept its terms, then publish the app to activate the key); the GTFS Schedule zip at `stm.info` is keyless. STM open data is licensed CC-BY 4.0.
- `sncf-france` — disabled, keyless Societe nationale des chemins de fer francais (SNCF, France) national-rail GTFS-Realtime feeds from the French National Access Point proxy `proxy.transport.data.gouv.fr`: trip updates and service alerts for TGV / TGV INOUI high-speed, Intercites, and TER regional trains (no vehicle-positions feed). The NAP re-publishes the SNCF feed with no API key required, and the SNCF GTFS Schedule zip on `data.gouv.fr` is likewise keyless. Licensed ODbL.
