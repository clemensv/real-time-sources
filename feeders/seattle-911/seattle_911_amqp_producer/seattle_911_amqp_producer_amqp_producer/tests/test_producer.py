...
"""
Tests for seattle_911_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../seattle_911_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../seattle_911_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../seattle_911_amqp_producer_amqp_producer/src')))

from seattle_911_amqp_producer_amqp_producer import *
from seattle_911_amqp_producer_data import Incident
...
class TestUSWASeattleFire911AmqpProducer:
    """Test cases for USWASeattleFire911AmqpProducer"""
    
    def test_producer_initialization(self, artemis_container):
        """Test that producer initializes correctly"""
        producer = USWASeattleFire911AmqpProducer(
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(USWASeattleFire911AmqpProducer)
    ...
    def test_send_incident(self, artemis_container):
        """Send and receive a Incident message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Incident.create_instance()
        ...
        try:
            ...
            for i in range(5):
                producer.send_incident(
                    data=payload,
                    _incident_number="value",
                    _incident_type_slug="value",
            ...
            for i in range(5):
                ...
                assert annotations.get(symbol('x-opt-partition-key')) == str("{incident_number}".format(incident_number="value"))[:128]
        finally:
            producer.close()

    def test_send_incident_single_fresh_connection(self, artemis_container):
        """Send exactly one Incident message on a fresh producer connection."""
        payload = Test_Incident.create_instance()
