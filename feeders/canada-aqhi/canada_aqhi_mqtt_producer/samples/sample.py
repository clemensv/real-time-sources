...
import os
import signal
from canada_aqhi_mqtt_producer_data import *
from canada_aqhi_mqtt_producer_mqtt_client.client import CaGcWeatherAqhiMqttProducer, CaGcWeatherAqhiMqttDispatcher

async def handle_ca_gc_weather_aqhi_mqtt_community(mqtt_msg,cloud_event, ca_gc_weather_aqhi_mqtt_community_data):
    """ Handles the ca.gc.weather.aqhi.mqtt.Community message """
    print(f"Received ca.gc.weather.aqhi.mqtt.Community on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {ca_gc_weather_aqhi_mqtt_community_data}")

async def handle_ca_gc_weather_aqhi_mqtt_observation(mqtt_msg,cloud_event, ca_gc_weather_aqhi_mqtt_observation_data):
    """ Handles the ca.gc.weather.aqhi.mqtt.Observation message """
    print(f"Received ca.gc.weather.aqhi.mqtt.Observation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {ca_gc_weather_aqhi_mqtt_observation_data}")

async def handle_ca_gc_weather_aqhi_mqtt_forecast(mqtt_msg,cloud_event, ca_gc_weather_aqhi_mqtt_forecast_data):
    """ Handles the ca.gc.weather.aqhi.mqtt.Forecast message """
    print(f"Received ca.gc.weather.aqhi.mqtt.Forecast on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {ca_gc_weather_aqhi_mqtt_forecast_data}")
