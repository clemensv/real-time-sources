# Canada_aqhi_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the canada_aqhi_mqtt_producer message definitions.

# Canada_aqhi_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - CaGcWeatherAqhiMqttEventDispatcher
...
from canada_aqhi_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = CaGcWeatherAqhiMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### CaGcWeatherAqhiMqttEventDispatcher



# Publish message`CaGcWeatherAqhiMqttEventDispatcher` handles events for the ca.gc.weather.aqhi.mqtt message group.
...
from canada_aqhi_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = CaGcWeatherAqhiMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe CaGcWeatherAqhiMqttEventDispatcher defines the following event handler hooks.
...
```##### `ca_gc_weather_aqhi_mqtt_community_async`



## Test```python

ca_gc_weather_aqhi_mqtt_community_async:  Callable[[PartitionContext, EventData, CloudEvent, Community],
Awaitable[None]]
...
- `data`: The event data of type `canada_aqhi_mqtt_producer_data.Community`.

Example:

```python
async def ca_gc_weather_aqhi_mqtt_community_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Community) -> None:
...
```##### `ca_gc_weather_aqhi_mqtt_observation_async`



## Test```python

ca_gc_weather_aqhi_mqtt_observation_async:  Callable[[PartitionContext, EventData, CloudEvent, Observation],
Awaitable[None]]
...
```Asynchronous handler hook for `ca.gc.weather.aqhi.mqtt.Observation`: Latest AQHI observation for a reporting
community. Observations are published hourly and may include decimal AQHI values.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `canada_aqhi_mqtt_producer_data.Observation`.

Example:

```python
async def ca_gc_weather_aqhi_mqtt_observation_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `ca_gc_weather_aqhi_mqtt_forecast_async`



## Test```python

ca_gc_weather_aqhi_mqtt_forecast_async:  Callable[[PartitionContext, EventData, CloudEvent, Forecast], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `canada_aqhi_mqtt_producer_data.Forecast`.

Example:

```python
async def ca_gc_weather_aqhi_mqtt_forecast_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Forecast) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
