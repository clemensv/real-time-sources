...
def get_default_topic_mappings_ca_gc_weather_aqhi_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the ca.gc.weather.aqhi.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "ca.gc.weather.aqhi.mqtt.Community": "air-quality/ca/eccc/canada-aqhi/{province}/{community_name}/info",
        "ca.gc.weather.aqhi.mqtt.Observation": "air-quality/ca/eccc/canada-aqhi/{province}/{community_name}/observation",
        "ca.gc.weather.aqhi.mqtt.Forecast": "air-quality/ca/eccc/canada-aqhi/{province}/{community_name}/forecast",
    }


def get_subscription_topics_ca_gc_weather_aqhi_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_ca_gc_weather_aqhi_mqtt()
    topics = []
...
class CaGcWeatherAqhiMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the ca.gc.weather.aqhi.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_ca_gc_weather_aqhi_mqtt()
        ...
        self.ca_gc_weather_aqhi_mqtt_community_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, canada_aqhi_mqtt_producer_data.Community, Dict[str, str]], Awaitable[None]]] = None
        
        self.ca_gc_weather_aqhi_mqtt_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, canada_aqhi_mqtt_producer_data.Observation, Dict[str, str]], Awaitable[None]]] = None
        
        self.ca_gc_weather_aqhi_mqtt_forecast_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, canada_aqhi_mqtt_producer_data.Forecast, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "ca.gc.weather.aqhi.Community":
            if self.ca_gc_weather_aqhi_mqtt_community_async:
                try:
                    ...
                    data = canada_aqhi_mqtt_producer_data.Community.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "ca.gc.weather.aqhi.mqtt.Community")
                    await self.ca_gc_weather_aqhi_mqtt_community_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in ca_gc_weather_aqhi_mqtt_community handler: {e}")
            return
        
        if event_type == "ca.gc.weather.aqhi.Observation":
            if self.ca_gc_weather_aqhi_mqtt_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = canada_aqhi_mqtt_producer_data.Observation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "ca.gc.weather.aqhi.mqtt.Observation")
                    await self.ca_gc_weather_aqhi_mqtt_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in ca_gc_weather_aqhi_mqtt_observation handler: {e}")
            return
        
        if event_type == "ca.gc.weather.aqhi.Forecast":
            if self.ca_gc_weather_aqhi_mqtt_forecast_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = canada_aqhi_mqtt_producer_data.Forecast.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "ca.gc.weather.aqhi.mqtt.Forecast")
                    await self.ca_gc_weather_aqhi_mqtt_forecast_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in ca_gc_weather_aqhi_mqtt_forecast handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_ca_gc_weather_aqhi_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_ca_gc_weather_aqhi_mqtt_community(self,
        province: str,
        community_name: str,
        data: canada_aqhi_mqtt_producer_data.Community,
        ...
        Args:
            ...
            community_name: URI template variable for 'community_name'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/ca/eccc/canada-aqhi/{province}/{community_name}/info'
    ...
    async def publish_ca_gc_weather_aqhi_mqtt_observation(self,
        province: str,
        community_name: str,
        data: canada_aqhi_mqtt_producer_data.Observation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        ...
        Args:
            ...
            community_name: URI template variable for 'community_name'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/ca/eccc/canada-aqhi/{province}/{community_name}/observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
    ...
    async def publish_ca_gc_weather_aqhi_mqtt_forecast(self,
        province: str,
        community_name: str,
        data: canada_aqhi_mqtt_producer_data.Forecast,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        ...
        Args:
            ...
            community_name: URI template variable for 'community_name'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/ca/eccc/canada-aqhi/{province}/{community_name}/forecast'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
