...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../canada_aqhi_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../canada_aqhi_amqp_producer_amqp_producer/src')))
...
class TestCaGcWeatherAqhiAmqpProducer:
    """Test cases for CaGcWeatherAqhiAmqpProducer"""
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "ca.gc.weather.aqhi.Community"
    ...
    def test_send_community_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'ca.gc.weather.aqhi.Community'
        assert received.body is not None
        assert received.subject == "{province}/{community_name}".format(province="value", community_name="value")
        assert properties.get('province') == "{province}".format(province="value")
        assert properties.get('community_name') == "{community_name}".format(community_name="value")
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "ca.gc.weather.aqhi.Observation"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_observation_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'ca.gc.weather.aqhi.Observation'
        assert received.body is not None
        assert received.subject == "{province}/{community_name}".format(province="value", community_name="value")
        assert properties.get('province') == "{province}".format(province="value")
        assert properties.get('community_name') == "{community_name}".format(community_name="value")
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "ca.gc.weather.aqhi.Forecast"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_forecast_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'ca.gc.weather.aqhi.Forecast'
        assert received.body is not None
        assert received.subject == "{province}/{community_name}".format(province="value", community_name="value")
        assert properties.get('province') == "{province}".format(province="value")
        assert properties.get('community_name') == "{community_name}".format(community_name="value")
