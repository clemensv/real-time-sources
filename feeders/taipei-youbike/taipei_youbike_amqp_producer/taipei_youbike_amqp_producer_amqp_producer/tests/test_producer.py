...
"""
Tests for taipei_youbike_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../taipei_youbike_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../taipei_youbike_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../taipei_youbike_amqp_producer_amqp_producer/src')))

from taipei_youbike_amqp_producer_amqp_producer import *
from taipei_youbike_amqp_producer_data import StationInformation
from test_stationinformation import Test_StationInformation
from taipei_youbike_amqp_producer_data import StationStatus
...
class TestTWYouBikeAmqpStationsProducer:
    """Test cases for TWYouBikeAmqpStationsProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(TWYouBikeAmqpStationsProducer)
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "TW.YouBike.StationInformation"
    ...
    def test_send_station_information_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'TW.YouBike.StationInformation'
        assert received.body is not None
        assert received.subject == "{station_id}".format(station_id="value")
        assert annotations.get(symbol('x-opt-partition-key')) == str("{station_id}".format(station_id="value"))[:128]
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "TW.YouBike.StationStatus"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_station_status_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'TW.YouBike.StationStatus'
        assert received.body is not None
        assert received.subject == "{station_id}".format(station_id="value")
        assert annotations.get(symbol('x-opt-partition-key')) == str("{station_id}".format(station_id="value"))[:128]
