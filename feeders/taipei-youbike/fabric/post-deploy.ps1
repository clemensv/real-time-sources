<#
.SYNOPSIS
    taipei-youbike-specific post-deploy hook: wires the YouBike 2.0 Taiwan Fabric
    Map's Kusto-backed layers against a freshly deployed taipei-youbike Eventhouse
    database.
...
.DESCRIPTION
    ...
    The generic deployer has already applied `kql/taipei-youbike.kql` which lays
    down:
        - bracketed CloudEvents tables
          `['TW.YouBike.StationInformation']` (station identity +
          location + capacity) and `['TW.YouBike.StationStatus']`
    ...
    it reads from the TAIPEI_YOUBIKE_FABRIC_MAP_ID environment variable. If that
    variable is missing the hook auto-creates a blank Map item named
    `taipei-youbike-map` (overridable via TAIPEI_YOUBIKE_FABRIC_MAP_NAME) in the
...
.PARAMETER MapId
    ...
    $env:TAIPEI_YOUBIKE_FABRIC_MAP_ID.

.PARAMETER KqlDatabaseId
    Standalone-mode override. GUID of the KQL database with the taipei-youbike
    tables.
...
.PARAMETER KustoDatabase
    KQL database name (default: taipei_youbike). Must match the name used by the
    generic deployer.
...
.EXAMPLE
    ...
    ./tools/deploy-fabric/deploy-fabric.ps1 -Source taipei-youbike `
        -ResourceGroup rg-youbike -Workspace <ws>
...
param(
    [hashtable] $Context,
    [string]    $WorkspaceId,
    [string]    $MapId          = $env:TAIPEI_YOUBIKE_FABRIC_MAP_ID,
    [string]    $KqlDatabaseId,
    [string]    $KustoUri,
    [string]    $KustoDatabase  = "taipei_youbike"
)
...
if (-not $MapId) {
    ...
    $mapName = if ($env:TAIPEI_YOUBIKE_FABRIC_MAP_NAME) { $env:TAIPEI_YOUBIKE_FABRIC_MAP_NAME } else { "taipei-youbike-map" }
    Write-Host "  [taipei-youbike post-deploy] TAIPEI_YOUBIKE_FABRIC_MAP_ID not set; auto-creating Map item '$mapName' in workspace $WorkspaceId..." -ForegroundColor Yellow
    ...
    if ($existing) {
        $MapId = $existing.id
        Write-Host "  [taipei-youbike post-deploy] Reusing existing Map '$mapName' (id $MapId)" -ForegroundColor Green
    } else {
        ...
        $MapId = $created.id
        Write-Host "  [taipei-youbike post-deploy] Created Map '$mapName' (id $MapId)" -ForegroundColor Green
    }
    $env:TAIPEI_YOUBIKE_FABRIC_MAP_ID = $MapId
}
...
if (-not $env:FABRIC_TOKEN) {
    Write-Host "  [taipei-youbike post-deploy] Acquiring Fabric token via az CLI..."
    $env:FABRIC_TOKEN = (az account get-access-token `
        --resource "https://api.fabric.microsoft.com" `
        --query accessToken -o tsv)
}
if (-not $env:KUSTO_TOKEN) {
    Write-Host "  [taipei-youbike post-deploy] Acquiring Kusto token via az CLI..."
...
$script = Join-Path $PSScriptRoot "wire_taipei_youbike_map.py"
...
if ($LASTEXITCODE -ne 0) { Write-Warning "wire_taipei_youbike_map.py exited with $LASTEXITCODE (map wiring failed but core deployment succeeded)" }
