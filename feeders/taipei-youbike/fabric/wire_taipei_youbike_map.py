...
for a freshly-deployed taipei-youbike (YouBike 2.0 Taiwan, station-based
public bikeshare) source.

Like the tfl-cycles map this mirrors, taipei-youbike is a **fixed
docking-station** feed: ~9,348 stationary YouBike 2.0 docks whose *identity and
...
``taipei-youbike/kql/taipei-youbike.kql`` deployment lays down the following
schema in the target Eventhouse (KQL database ``taipei_youbike``):

  * Bracketed, dotted CloudEvents-shaped base tables:
    ``['TW.YouBike.StationInformation']`` - station identity, bilingual name,
                                            administrative district, WGS84
                                            location and nominal docking
                                            capacity (reference),
    ``['TW.YouBike.StationStatus']``      - live availability counts
                                            (telemetry).
...
``taipeiyoubike_bbox()`` (falling back to the raw station footprint, and finally
to central Taipei) so the script needs no manual tuning.
...
Inputs (env vars or CLI args, mirroring the sibling tfl-cycles map's
``wire_tfl_cycles_map.py``):

  FABRIC_WORKSPACE_ID   - GUID of the Fabric workspace containing the Map
  FABRIC_MAP_ID         - GUID of the Fabric Map item to patch
  FABRIC_KQL_DB_ID      - GUID of the KQL database with the taipei-youbike tables
  KUSTO_CLUSTER_URI     - https://<cluster>.kusto.fabric.microsoft.com
  KUSTO_DB              - KQL database name (default ``taipei_youbike``)
...
#
# We lean on the materialized views (``['TW.YouBike.StationInformationLatest']``
# and ``['TW.YouBike.StationStatusLatest']``) created by taipei-youbike.kql.
...
#
# ``taipeiyoubike_stations()`` is the single enriched JOIN (StationInformationLatest
...
# generic HTTP 400 and no syntax detail. taipei-youbike uses ``lon`` (not ``long``)
...
TAIPEI_YOUBIKE_FUNCTIONS: list[str] = [
    # ------------------------------------------------------------------
    # Every located docking station enriched with its live availability.
    # Location comes from StationInformationLatest, counts from
    # StationStatusLatest; the two are joined on the stable station_id. Null /
    # 0,0 coordinates are dropped up front. Precomputes bike-availability +
    # empty-dock colour ramps, a capacity-scaled bubble ``radius`` and a human
    # ``label``. All located stations are kept (no service_status filter) so the
    # layer count tracks the full ~9,300-station YouBike 2.0 footprint.
    r"""
.create-or-alter function with (folder = "Map", skipvalidation = "true") taipeiyoubike_stations() {
    ...
    | project ['geometry'], station_id, ['name_en'], ['name_tw'], ['district_en'], lat, lon, capacity,
              num_bikes_available, num_bikes_yb1, num_bikes_yb2, num_ebikes_available,
              num_empty_docks, num_forbidden_docks, ['total_docks'],
              availability_level, service_status, updated_at,
...
""",
    ...
    r"""
.create-or-alter function with (folder = "Map", skipvalidation = "true") taipeiyoubike_bbox() {
    taipeiyoubike_stations()
...
def _layers() -> list[Layer]:
    return [
        Layer(
            "taipei-youbike station availability",
            "taipeiyoubike_stations()",
            bubble_options(
                ...
                tooltip_keys=[
                    "name_en", "district_en", "fill_state", "num_bikes_available",
                    "num_empty_docks", "capacity", "num_ebikes_available", "avail_percent",
                ],
            ),
            [_text_filter("fill_state"), _number_filter("avail_percent", 0, 100)],
        ),
        Layer(
            "taipei-youbike station labels",
            "taipeiyoubike_stations()",
            bubble_options(
                visible=True, radius=2, labels=True, label_size=11, min_zoom=14,
                color="#1A1A1A", palette=None,
                tooltip_keys=[
                    "name_en", "num_bikes_available", "num_empty_docks", "capacity",
                ],
            ),
            [],
        ),
        Layer(
            "taipei-youbike empty docks",
            "taipeiyoubike_stations()",
            bubble_options(
                ...
                tooltip_keys=[
                    "name_en", "district_en", "docks_state", "num_empty_docks",
                    "num_bikes_available", "capacity", "docks_percent",
                ],
            ),
...
TAIPEI_CENTER = [121.5, 25.04]  # lon, lat - final basemap fallback
...
def wire(*, workspace_id: str, map_id: str, kql_db_id: str,
    ...
    print(f"Creating {len(TAIPEI_YOUBIKE_FUNCTIONS)} helper functions in {kusto_db}")
    for f in TAIPEI_YOUBIKE_FUNCTIONS:
    ...
    print("Discovering bounding box from taipeiyoubike_bbox()")
    bbox = _bbox(ks, kusto_uri, kusto_db)
    if bbox is None:
        print("  no stations indexed yet; centring on Taipei")
    else:
    ...
    if removed_src_ids:
        print(f"  removing {len(removed_src_ids)} pre-existing taipei-youbike layer source(s)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--kusto-db",     default=os.environ.get("KUSTO_DB", "taipei_youbike"))
