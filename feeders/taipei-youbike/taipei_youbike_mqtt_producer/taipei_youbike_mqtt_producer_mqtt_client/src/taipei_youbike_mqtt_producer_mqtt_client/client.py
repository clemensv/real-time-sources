...
import taipei_youbike_mqtt_producer_data
from taipei_youbike_mqtt_producer_data import StationInformation
from taipei_youbike_mqtt_producer_data import StationStatus
...
def get_default_topic_mappings_tw_youbike_mqtt_stations() -> Dict[str, str]:
    """
    Get the default topic mappings for the TW.YouBike.mqtt.stations message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "TW.YouBike.mqtt.StationInformation": "mobility/taipei-youbike/{station_id}/info",
        "TW.YouBike.mqtt.StationStatus": "mobility/taipei-youbike/{station_id}/status",
    }


def get_subscription_topics_tw_youbike_mqtt_stations(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_tw_youbike_mqtt_stations()
    topics = []
...
class TWYouBikeMqttStationsMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the TW.YouBike.mqtt.stations message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_tw_youbike_mqtt_stations()
        ...
        self.tw_you_bike_mqtt_station_information_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, taipei_youbike_mqtt_producer_data.StationInformation, Dict[str, str]], Awaitable[None]]] = None
        
        self.tw_you_bike_mqtt_station_status_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, taipei_youbike_mqtt_producer_data.StationStatus, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "TW.YouBike.StationInformation":
            if self.tw_you_bike_mqtt_station_information_async:
                try:
                    ...
                    data = taipei_youbike_mqtt_producer_data.StationInformation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "TW.YouBike.mqtt.StationInformation")
                    await self.tw_you_bike_mqtt_station_information_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in tw_you_bike_mqtt_station_information handler: {e}")
            return
        
        if event_type == "TW.YouBike.StationStatus":
            if self.tw_you_bike_mqtt_station_status_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = taipei_youbike_mqtt_producer_data.StationStatus.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "TW.YouBike.mqtt.StationStatus")
                    await self.tw_you_bike_mqtt_station_status_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in tw_you_bike_mqtt_station_status handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_tw_youbike_mqtt_stations(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_tw_you_bike_mqtt_station_information(self,
        feedurl: str,
        station_id: str,
        data: taipei_youbike_mqtt_producer_data.StationInformation,
        ...
        """
        Publish the 'TW.YouBike.mqtt.StationInformation' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'mobility/taipei-youbike/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "mobility/taipei-youbike/{station_id}/info"
    ...
    async def publish_tw_you_bike_mqtt_station_status(self,
        feedurl: str,
        station_id: str,
        data: taipei_youbike_mqtt_producer_data.StationStatus,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'TW.YouBike.mqtt.StationStatus' event to an MQTT topic.
        ...
        Args:
            ...
            station_id: URI template variable for 'station_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'mobility/taipei-youbike/{station_id}/status'
                with URI template placeholders substituted from the keyword arguments.
            ...
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "mobility/taipei-youbike/{station_id}/status"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
            "station_id": str(station_id),
        }
