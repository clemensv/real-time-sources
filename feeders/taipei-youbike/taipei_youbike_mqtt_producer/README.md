# Taipei_youbike_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the taipei_youbike_mqtt_producer message definitions.

# Taipei_youbike_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - TWYouBikeMqttStationsEventDispatcher
...
from taipei_youbike_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = TWYouBikeMqttStationsMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### TWYouBikeMqttStationsEventDispatcher



# Publish message`TWYouBikeMqttStationsEventDispatcher` handles events for the TW.YouBike.mqtt.stations message group.
...
from taipei_youbike_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = TWYouBikeMqttStationsMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe TWYouBikeMqttStationsEventDispatcher defines the following event handler hooks.
...
```##### `tw_you_bike_mqtt_station_information_async`



## Test```python

tw_you_bike_mqtt_station_information_async:  Callable[[PartitionContext, EventData, CloudEvent, StationInformation],
Awaitable[None]]
...
- `data`: The event data of type `taipei_youbike_mqtt_producer_data.StationInformation`.

Example:

```python
async def tw_you_bike_mqtt_station_information_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `tw_you_bike_mqtt_station_status_async`



## Test```python

tw_you_bike_mqtt_station_status_async:  Callable[[PartitionContext, EventData, CloudEvent, StationStatus],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `taipei_youbike_mqtt_producer_data.StationStatus`.

Example:

```python
async def tw_you_bike_mqtt_station_status_event(partition_context: PartitionContext, event: EventData, cloud_event:
