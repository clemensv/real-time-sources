...
import os
import signal
from taipei_youbike_mqtt_producer_data import *
from taipei_youbike_mqtt_producer_mqtt_client.client import TWYouBikeMqttStationsProducer, TWYouBikeMqttStationsDispatcher

async def handle_tw_you_bike_mqtt_station_information(mqtt_msg,cloud_event, tw_you_bike_mqtt_station_information_data):
    """ Handles the TW.YouBike.mqtt.StationInformation message """
    print(f"Received TW.YouBike.mqtt.StationInformation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {tw_you_bike_mqtt_station_information_data}")

async def handle_tw_you_bike_mqtt_station_status(mqtt_msg,cloud_event, tw_you_bike_mqtt_station_status_data):
    """ Handles the TW.YouBike.mqtt.StationStatus message """
    print(f"Received TW.YouBike.mqtt.StationStatus on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {tw_you_bike_mqtt_station_status_data}")
