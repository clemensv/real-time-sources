"""MQTT feeder application for Taiwan YouBike 2.0 -> Unified Namespace.

Wraps the upstream YouBike station poller from :mod:`taipei_youbike_core` and the
generated :class:`TWYouBikeMqttStationsMqttClient`. The MQTT broker is reached
via a plain ``mqtt://host:port`` URL (or ``mqtts://`` for TLS); username/password
are optional. CloudEvent semantics are preserved end-to-end: binary mode maps CE
attributes to MQTT 5 user properties so subscribers can route on ``type``,
``source`` and ``subject`` without cracking the JSON body. Station info is
published under ``mobility/taipei-youbike/{station_id}/info`` and availability
under ``.../status``.
"""
...
from taipei_youbike_mqtt_producer_data import StationInformation, StationStatus
from taipei_youbike_mqtt_producer_mqtt_client.client import TWYouBikeMqttStationsMqttClient
...
async def feed(
    ...
    *,
    ...
    mqtt_client = TWYouBikeMqttStationsMqttClient(
        client=paho_client,
            ...
            try:
                ...
                for p in parsed:
                    ...
                    if reference_due or previous_info.get(p.station_id) != signature:
                        try:
                            await mqtt_client.publish_tw_you_bike_mqtt_station_information(
                                feedurl=f"{FEED_URL}#{p.station_id}",
                                station_id=p.station_id,
                                data=_build_info(p),
                            )
                            info_count += 1
                ...
                for p in parsed:
                    ...
                    if previous_status.get(p.station_id) != signature:
                        try:
                            await mqtt_client.publish_tw_you_bike_mqtt_station_status(
                                feedurl=f"{FEED_URL}#{p.station_id}",
                                station_id=p.station_id,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Taiwan YouBike 2.0 -> MQTT/UNS bridge.")
    subparsers = parser.add_subparsers(dest="command")

    feed_parser = subparsers.add_parser("feed", help="Feed YouBike reference and availability CloudEvents to MQTT")
