"""AMQP 1.0 feeder application for Taiwan YouBike 2.0 stations.

Wraps the upstream YouBike station poller from :mod:`taipei_youbike_core` and the
generated :class:`TWYouBikeAmqpStationsProducer`. Supports three deployment
...
from taipei_youbike_amqp_producer_data import StationInformation, StationStatus
from taipei_youbike_amqp_producer_amqp_producer.producer import TWYouBikeAmqpStationsProducer
...
def _build_producer(
    *,
    ...
    if auth_mode == "entra":
        ...
        return TWYouBikeAmqpStationsProducer(
            host=host,
            address=address,
            port=port,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Taiwan YouBike 2.0 -> AMQP 1.0 bridge.")
    subparsers = parser.add_subparsers(dest="command")

    feed_parser = subparsers.add_parser("feed", help="Feed YouBike reference and availability CloudEvents over AMQP 1.0")
    ...
    feed_parser.add_argument("--address", type=str,
                             default=os.getenv("AMQP_ADDRESS", "taipei-youbike"),
                             help="AMQP address (queue or topic name). Default: taipei-youbike")
