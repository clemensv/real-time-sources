...
import os
import signal
from elexon_bmrs_mqtt_producer_data import *
from elexon_bmrs_mqtt_producer_mqtt_client.client import UKCoElexonBMRSMqttProducer, UKCoElexonBMRSMqttDispatcher

async def handle_uk_co_elexon_bmrs_mqtt_generation_mix(mqtt_msg,cloud_event, uk_co_elexon_bmrs_mqtt_generation_mix_data):
    """ Handles the UK.Co.Elexon.BMRS.mqtt.GenerationMix message """
    print(f"Received UK.Co.Elexon.BMRS.mqtt.GenerationMix on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_co_elexon_bmrs_mqtt_generation_mix_data}")

async def handle_uk_co_elexon_bmrs_mqtt_demand_outturn(mqtt_msg,cloud_event, uk_co_elexon_bmrs_mqtt_demand_outturn_data):
    """ Handles the UK.Co.Elexon.BMRS.mqtt.DemandOutturn message """
    print(f"Received UK.Co.Elexon.BMRS.mqtt.DemandOutturn on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_co_elexon_bmrs_mqtt_demand_outturn_data}")

async def handle_uk_co_elexon_bmrs_mqtt_info(mqtt_msg,cloud_event, uk_co_elexon_bmrs_mqtt_info_data):
    """ Handles the UK.Co.Elexon.BMRS.mqtt.Info message """
    print(f"Received UK.Co.Elexon.BMRS.mqtt.Info on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_co_elexon_bmrs_mqtt_info_data}")
