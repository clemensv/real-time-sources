...
def get_default_topic_mappings_uk_co_elexon_bmrs_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the UK.Co.Elexon.BMRS.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "UK.Co.Elexon.BMRS.mqtt.GenerationMix": "energy/gb/elexon/elexon-bmrs/gb/{settlement_date}/{settlement_period}/generation-mix",
        "UK.Co.Elexon.BMRS.mqtt.DemandOutturn": "energy/gb/elexon/elexon-bmrs/gb/{settlement_date}/{settlement_period}/demand-outturn",
        "UK.Co.Elexon.BMRS.mqtt.Info": "energy/gb/elexon/elexon-bmrs/gb/{settlement_date}/{settlement_period}/info",
    }


def get_subscription_topics_uk_co_elexon_bmrs_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_uk_co_elexon_bmrs_mqtt()
    topics = []
...
class UKCoElexonBMRSMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the UK.Co.Elexon.BMRS.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_uk_co_elexon_bmrs_mqtt()
        ...
        self.uk_co_elexon_bmrs_mqtt_generation_mix_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, elexon_bmrs_mqtt_producer_data.GenerationMix, Dict[str, str]], Awaitable[None]]] = None
        
        self.uk_co_elexon_bmrs_mqtt_demand_outturn_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, elexon_bmrs_mqtt_producer_data.DemandOutturn, Dict[str, str]], Awaitable[None]]] = None
        
        self.uk_co_elexon_bmrs_mqtt_info_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, elexon_bmrs_mqtt_producer_data.Info, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "UK.Co.Elexon.BMRS.GenerationMix":
            if self.uk_co_elexon_bmrs_mqtt_generation_mix_async:
                try:
                    ...
                    data = elexon_bmrs_mqtt_producer_data.GenerationMix.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "UK.Co.Elexon.BMRS.mqtt.GenerationMix")
                    await self.uk_co_elexon_bmrs_mqtt_generation_mix_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_co_elexon_bmrs_mqtt_generation_mix handler: {e}")
            return
        
        if event_type == "UK.Co.Elexon.BMRS.DemandOutturn":
            if self.uk_co_elexon_bmrs_mqtt_demand_outturn_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = elexon_bmrs_mqtt_producer_data.DemandOutturn.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "UK.Co.Elexon.BMRS.mqtt.DemandOutturn")
                    await self.uk_co_elexon_bmrs_mqtt_demand_outturn_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_co_elexon_bmrs_mqtt_demand_outturn handler: {e}")
            return
        
        if event_type == "UK.Co.Elexon.BMRS.Info":
            if self.uk_co_elexon_bmrs_mqtt_info_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = elexon_bmrs_mqtt_producer_data.Info.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "UK.Co.Elexon.BMRS.mqtt.Info")
                    await self.uk_co_elexon_bmrs_mqtt_info_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_co_elexon_bmrs_mqtt_info handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_uk_co_elexon_bmrs_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_uk_co_elexon_bmrs_mqtt_generation_mix(self,
        settlement_period: str,
        settlement_date: str,
        data: elexon_bmrs_mqtt_producer_data.GenerationMix,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'UK.Co.Elexon.BMRS.mqtt.GenerationMix' event to an MQTT topic.

        Args:
        
            settlement_period: URI template variable for 'settlement_period'
            settlement_date: URI template variable for 'settlement_date'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'energy/gb/elexon/elexon-bmrs/gb/{settlement_date}/{settlement_period}/generation-mix'
        ...
        """
        target_topic = topic if topic is not None else "energy/gb/elexon/elexon-bmrs/gb/{settlement_date}/{settlement_period}/generation-mix"
        ...
        attributes = {
             "type":"UK.Co.Elexon.BMRS.GenerationMix",
             "source":"https://data.elexon.co.uk/bmrs",
             "subject":"{settlement_period}".format(settlement_period = settlement_period)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_uk_co_elexon_bmrs_mqtt_demand_outturn(self,
        settlement_period: str,
        settlement_date: str,
        data: elexon_bmrs_mqtt_producer_data.DemandOutturn,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'UK.Co.Elexon.BMRS.mqtt.DemandOutturn' event to an MQTT topic.
        ...
        Args:
            ...
            settlement_date: URI template variable for 'settlement_date'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'energy/gb/elexon/elexon-bmrs/gb/{settlement_date}/{settlement_period}/demand-outturn'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "energy/gb/elexon/elexon-bmrs/gb/{settlement_date}/{settlement_period}/demand-outturn"
        _topic_template_values: Dict[str, str] = {
            "settlement_period": str(settlement_period),
            "settlement_date": str(settlement_date),
        }
        ...
        attributes = {
             "type":"UK.Co.Elexon.BMRS.DemandOutturn",
             "source":"https://data.elexon.co.uk/bmrs",
             "subject":"{settlement_period}".format(settlement_period = settlement_period)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_uk_co_elexon_bmrs_mqtt_info(self,
        settlement_period: str,
        settlement_date: str,
        data: elexon_bmrs_mqtt_producer_data.Info,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'UK.Co.Elexon.BMRS.mqtt.Info' event to an MQTT topic.
        ...
        Args:
            ...
            settlement_date: URI template variable for 'settlement_date'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'energy/gb/elexon/elexon-bmrs/gb/{settlement_date}/{settlement_period}/info'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "energy/gb/elexon/elexon-bmrs/gb/{settlement_date}/{settlement_period}/info"
        _topic_template_values: Dict[str, str] = {
            "settlement_period": str(settlement_period),
            "settlement_date": str(settlement_date),
        }
