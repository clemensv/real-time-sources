...
class DemandOutturn:
    ...
    Attributes:
        ...
        publish_time (typing.Optional[datetime.datetime])
    ...
    publish_time: typing.Optional[datetime.datetime]=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="publish_time", encoder=lambda d: d.isoformat() if isinstance(d, datetime.datetime) else d if d else None, decoder=lambda d: datetime.datetime.fromisoformat(d) if isinstance(d, str) else d if d else None, mm_field=fields.DateTime(format='iso')))
    initial_demand_outturn_mw: typing.Optional[float]=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="initial_demand_outturn_mw"))
    initial_transmission_system_demand_outturn_mw: typing.Optional[float]=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="initial_transmission_system_demand_outturn_mw"))
    ...
    def from_data(cls, data: typing.Any, content_type_string: typing.Optional[str] = None) -> typing.Optional['DemandOutturn']:
        ...
        if base_content_type == 'application/json':
            if isinstance(data, (bytes, str)):
                ...
                return DemandOutturn.from_serializer_dict(_record)
            else:
    ...
    def create_instance(cls) -> 'DemandOutturn':
        ...
        """
        return cls(
            settlement_period=int(99),
            settlement_date='ptufzqioqvsbqlpyaswm',
            start_time=datetime.datetime.now(datetime.timezone.utc),
            publish_time=datetime.datetime.now(datetime.timezone.utc),
            initial_demand_outturn_mw=float(63.75136867166537),
            initial_transmission_system_demand_outturn_mw=float(28.588658480747664)
        )