# Elexon_bmrs_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the elexon_bmrs_mqtt_producer message definitions.

# Elexon_bmrs_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - UKCoElexonBMRSMqttEventDispatcher
...
from elexon_bmrs_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = UKCoElexonBMRSMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### UKCoElexonBMRSMqttEventDispatcher



# Publish message`UKCoElexonBMRSMqttEventDispatcher` handles events for the UK.Co.Elexon.BMRS.mqtt message group.
...
from elexon_bmrs_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = UKCoElexonBMRSMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe UKCoElexonBMRSMqttEventDispatcher defines the following event handler hooks.
...
```##### `uk_co_elexon_bmrs_mqtt_generation_mix_async`



## Test```python

uk_co_elexon_bmrs_mqtt_generation_mix_async:  Callable[[PartitionContext, EventData, CloudEvent, GenerationMix],
Awaitable[None]]
...
- `data`: The event data of type `elexon_bmrs_mqtt_producer_data.GenerationMix`.

Example:

```python
async def uk_co_elexon_bmrs_mqtt_generation_mix_event(partition_context: PartitionContext, event: EventData,
...
```##### `uk_co_elexon_bmrs_mqtt_demand_outturn_async`



## Test```python

uk_co_elexon_bmrs_mqtt_demand_outturn_async:  Callable[[PartitionContext, EventData, CloudEvent, DemandOutturn],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `elexon_bmrs_mqtt_producer_data.DemandOutturn`.

Example:

```python
async def uk_co_elexon_bmrs_mqtt_demand_outturn_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: DemandOutturn) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `uk_co_elexon_bmrs_mqtt_info_async`



## Test```python

uk_co_elexon_bmrs_mqtt_info_async:  Callable[[PartitionContext, EventData, CloudEvent, Info], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `elexon_bmrs_mqtt_producer_data.Info`.

Example:

```python
async def uk_co_elexon_bmrs_mqtt_info_event(partition_context: PartitionContext, event: EventData, cloud_event:
