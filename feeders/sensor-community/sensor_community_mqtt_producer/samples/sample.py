...
import os
import signal
from sensor_community_mqtt_producer_data import *
from sensor_community_mqtt_producer_mqtt_client.client import IoSensorCommunityMqttProducer, IoSensorCommunityMqttDispatcher

async def handle_io_sensor_community_mqtt_sensor_info(mqtt_msg,cloud_event, io_sensor_community_mqtt_sensor_info_data):
    """ Handles the io.sensor.community.mqtt.SensorInfo message """
    print(f"Received io.sensor.community.mqtt.SensorInfo on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {io_sensor_community_mqtt_sensor_info_data}")

async def handle_io_sensor_community_mqtt_sensor_reading(mqtt_msg,cloud_event, io_sensor_community_mqtt_sensor_reading_data):
    """ Handles the io.sensor.community.mqtt.SensorReading message """
    print(f"Received io.sensor.community.mqtt.SensorReading on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {io_sensor_community_mqtt_sensor_reading_data}")
