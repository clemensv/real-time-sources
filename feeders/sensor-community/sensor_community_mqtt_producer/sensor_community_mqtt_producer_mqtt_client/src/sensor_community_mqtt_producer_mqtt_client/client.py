...
def get_default_topic_mappings_io_sensor_community_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the io.sensor.community.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "io.sensor.community.mqtt.SensorInfo": "air-quality/intl/sensor-community/sensor-community/{country}/{geohash5}/{sensor_id}/info",
        "io.sensor.community.mqtt.SensorReading": "air-quality/intl/sensor-community/sensor-community/{country}/{geohash5}/{sensor_id}/reading",
    }


def get_subscription_topics_io_sensor_community_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_io_sensor_community_mqtt()
    topics = []
...
class IoSensorCommunityMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the io.sensor.community.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_io_sensor_community_mqtt()
        ...
        self.io_sensor_community_mqtt_sensor_info_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, sensor_community_mqtt_producer_data.SensorInfo, Dict[str, str]], Awaitable[None]]] = None
        
        self.io_sensor_community_mqtt_sensor_reading_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, sensor_community_mqtt_producer_data.SensorReading, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "io.sensor.community.SensorInfo":
            if self.io_sensor_community_mqtt_sensor_info_async:
                try:
                    ...
                    data = sensor_community_mqtt_producer_data.SensorInfo.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "io.sensor.community.mqtt.SensorInfo")
                    await self.io_sensor_community_mqtt_sensor_info_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in io_sensor_community_mqtt_sensor_info handler: {e}")
            return
        
        if event_type == "io.sensor.community.SensorReading":
            if self.io_sensor_community_mqtt_sensor_reading_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = sensor_community_mqtt_producer_data.SensorReading.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "io.sensor.community.mqtt.SensorReading")
                    await self.io_sensor_community_mqtt_sensor_reading_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in io_sensor_community_mqtt_sensor_reading handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_io_sensor_community_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_io_sensor_community_mqtt_sensor_info(self,
        feedurl: str,
        sensor_id: str,
        country: str,
        geohash5: str,
        data: sensor_community_mqtt_producer_data.SensorInfo,
        ...
        """
        Publish the 'io.sensor.community.mqtt.SensorInfo' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'air-quality/intl/sensor-community/sensor-community/{country}/{geohash5}/{sensor_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "air-quality/intl/sensor-community/sensor-community/{country}/{geohash5}/{sensor_id}/info"
        ...
        attributes = {
             ...
             "subject":"{sensor_id}".format(sensor_id = sensor_id)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_io_sensor_community_mqtt_sensor_reading(self,
        feedurl: str,
        sensor_id: str,
        country: str,
        geohash5: str,
        data: sensor_community_mqtt_producer_data.SensorReading,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'io.sensor.community.mqtt.SensorReading' event to an MQTT topic.
        ...
        Args:
            ...
            geohash5: URI template variable for 'geohash5'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/intl/sensor-community/sensor-community/{country}/{geohash5}/{sensor_id}/reading'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/intl/sensor-community/sensor-community/{country}/{geohash5}/{sensor_id}/reading"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
            "sensor_id": str(sensor_id),
            "country": str(country),
