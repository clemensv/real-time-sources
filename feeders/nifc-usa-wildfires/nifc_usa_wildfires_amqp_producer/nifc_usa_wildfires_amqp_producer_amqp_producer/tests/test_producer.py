...
"""
Tests for nifc_usa_wildfires_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../nifc_usa_wildfires_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../nifc_usa_wildfires_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../nifc_usa_wildfires_amqp_producer_amqp_producer/src')))

from nifc_usa_wildfires_amqp_producer_amqp_producer import *
from nifc_usa_wildfires_amqp_producer_data import WildfireIncident
...
class TestGovNIFCWildfiresAmqpProducer:
    """Test cases for GovNIFCWildfiresAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(GovNIFCWildfiresAmqpProducer)
    ...
    def test_send_wildfire_incident(self, artemis_container):
        """Send and receive a WildfireIncident message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_WildfireIncident.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                assert properties.get('status') == "{status}".format(status="value")
        finally:
            producer.close()

    def test_send_wildfire_incident_single_fresh_connection(self, artemis_container):
        """Send exactly one WildfireIncident message on a fresh producer connection."""
        payload = Test_WildfireIncident.create_instance()
