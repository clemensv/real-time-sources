...
import os
import signal
from nifc_usa_wildfires_mqtt_producer_data import *
from nifc_usa_wildfires_mqtt_producer_mqtt_client.client import GovNIFCWildfiresMqttProducer, GovNIFCWildfiresMqttDispatcher

async def handle_gov_nifc_wildfires_mqtt_wildfire_incident(mqtt_msg,cloud_event, gov_nifc_wildfires_mqtt_wildfire_incident_data):
    """ Handles the Gov.NIFC.Wildfires.mqtt.WildfireIncident message """
    print(f"Received Gov.NIFC.Wildfires.mqtt.WildfireIncident on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {gov_nifc_wildfires_mqtt_wildfire_incident_data}")
