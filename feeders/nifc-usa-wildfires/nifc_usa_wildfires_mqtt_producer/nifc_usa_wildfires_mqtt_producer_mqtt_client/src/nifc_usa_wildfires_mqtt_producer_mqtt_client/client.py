...
def get_default_topic_mappings_gov_nifc_wildfires_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the Gov.NIFC.Wildfires.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "Gov.NIFC.Wildfires.mqtt.WildfireIncident": "wildfire/us/nifc/nifc-usa-wildfires/{state}/{status}/{irwin_id}/incident",
    }


def get_subscription_topics_gov_nifc_wildfires_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_gov_nifc_wildfires_mqtt()
    topics = []
...
class GovNIFCWildfiresMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the Gov.NIFC.Wildfires.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_gov_nifc_wildfires_mqtt()
        ...
        self.gov_nifc_wildfires_mqtt_wildfire_incident_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, nifc_usa_wildfires_mqtt_producer_data.WildfireIncident, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "Gov.NIFC.Wildfires.WildfireIncident":
            if self.gov_nifc_wildfires_mqtt_wildfire_incident_async:
                try:
                    ...
                    data = nifc_usa_wildfires_mqtt_producer_data.WildfireIncident.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Gov.NIFC.Wildfires.mqtt.WildfireIncident")
                    await self.gov_nifc_wildfires_mqtt_wildfire_incident_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in gov_nifc_wildfires_mqtt_wildfire_incident handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_gov_nifc_wildfires_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_gov_nifc_wildfires_mqtt_wildfire_incident(self,
        source_uri: str,
        irwin_id: str,
        modified_on_datetime: str,
        state: str,
        status: str,
        data: nifc_usa_wildfires_mqtt_producer_data.WildfireIncident,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Gov.NIFC.Wildfires.mqtt.WildfireIncident' event to an MQTT topic.

        Args:
        
            source_uri: URI template variable for 'source_uri'
            irwin_id: URI template variable for 'irwin_id'
            modified_on_datetime: URI template variable for 'modified_on_datetime'
            state: URI template variable for 'state'
            status: URI template variable for 'status'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'wildfire/us/nifc/nifc-usa-wildfires/{state}/{status}/{irwin_id}/incident'
        ...
        """
        target_topic = topic if topic is not None else "wildfire/us/nifc/nifc-usa-wildfires/{state}/{status}/{irwin_id}/incident"
        _topic_template_values: Dict[str, str] = {
            "source_uri": str(source_uri),
            "irwin_id": str(irwin_id),
            "modified_on_datetime": str(modified_on_datetime),
            "state": str(state),
            "status": str(status),
        }
        ...
        attributes = {
             ...
             "subject":"{irwin_id}".format(irwin_id = irwin_id),
             "time":"{modified_on_datetime}".format(modified_on_datetime = modified_on_datetime)
        }
        attributes["datacontenttype"] = content_type
