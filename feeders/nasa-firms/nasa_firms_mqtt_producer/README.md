# Nasa_firms_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the nasa_firms_mqtt_producer message definitions.

# Nasa_firms_mqtt_producer Event Dispatcher for Azure Event Hubs
...
from nasa_firms_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = NASAFIRMSMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NASAFIRMSMqttEventDispatcher



# Publish message`NASAFIRMSMqttEventDispatcher` handles events for the NASA.FIRMS.mqtt message group.
...
from nasa_firms_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = NASAFIRMSMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe NASAFIRMSMqttEventDispatcher defines the following event handler hooks.
...
```##### `nasa_firms_mqtt_fire_detection_async`



## Test```python

nasa_firms_mqtt_fire_detection_async:  Callable[[PartitionContext, EventData, CloudEvent, FireDetection],
Awaitable[None]]
...
- `data`: The event data of type `nasa_firms_mqtt_producer_data.FireDetection`.

Example:

```python
async def nasa_firms_mqtt_fire_detection_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: FireDetection) -> None:
...
```##### `nasa_firms_mqtt_data_availability_async`



## Test```python

nasa_firms_mqtt_data_availability_async:  Callable[[PartitionContext, EventData, CloudEvent, DataAvailability],
Awaitable[None]]
...
```Asynchronous handler hook for `NASA.FIRMS.mqtt.DataAvailability`: Reference record describing one FIRMS product
source: its current data-availability window (min/max date), sensor family, platform, and nominal resolution. Emitted at
bridge startup and refreshed periodically so consumers can interpret detection freshness and coverage on the same topic
as the telemetry.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `nasa_firms_mqtt_producer_data.DataAvailability`.

Example:

```python
async def nasa_firms_mqtt_data_availability_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: DataAvailability) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
