...
import os
import signal
from nasa_firms_mqtt_producer_data import *
from nasa_firms_mqtt_producer_mqtt_client.client import NASAFIRMSMqttProducer, NASAFIRMSMqttDispatcher

async def handle_nasa_firms_mqtt_fire_detection(mqtt_msg,cloud_event, nasa_firms_mqtt_fire_detection_data):
    """ Handles the NASA.FIRMS.mqtt.FireDetection message """
    print(f"Received NASA.FIRMS.mqtt.FireDetection on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {nasa_firms_mqtt_fire_detection_data}")

async def handle_nasa_firms_mqtt_data_availability(mqtt_msg,cloud_event, nasa_firms_mqtt_data_availability_data):
    """ Handles the NASA.FIRMS.mqtt.DataAvailability message """
    print(f"Received NASA.FIRMS.mqtt.DataAvailability on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {nasa_firms_mqtt_data_availability_data}")
