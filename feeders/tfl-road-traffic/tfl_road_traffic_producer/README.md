...
# Tfl-road-traffic-producer Kafka Producer# Tfl-road-traffic-producer Event Dispatcher for Apache Kafka
...
4. [Generated Producer Classes](#generated-producer-classes)    UkGovTflRoadAmqpEventDispatcher
...
- UkGovTflRoadCorridorsProducersolution for event-driven applications.
...
- UkGovTflRoadAmqpProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### UkGovTflRoadCorridorsEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`UkGovTflRoadCorridorsEventDispatcher` handles events for the uk.gov.tfl.road.corridors message group.
...
The UkGovTflRoadCorridorsEventDispatcher defines the following event handler hooks.
...
producer = UkGovTflRoadCorridorsProducer(
    ...
    security_protocol='SASL_SSL',##### `uk_gov_tfl_road_road_corridor_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_tfl_road_road_corridor_async:  Callable[[ConsumerRecord, CloudEvent,
RoadCorridor], Awaitable[None]]

)```

```

Asynchronous handler hook for `uk.gov.tfl.road.RoadCorridor`: Reference record for a TfL managed road corridor fetched
from GET /Road.
...
### UkGovTflRoadCorridorsProducer- `data`: The event data of type `tfl_road_traffic_producer_data.RoadCorridor`.
...
uk_gov_tfl_road_corridors_dispatcher.uk_gov_tfl_road_road_corridor_async = uk_gov_tfl_road_road_corridor_event
...
### UkGovTflRoadDisruptionsProducer- `data`: The event data of type `tfl_road_traffic_producer_data.RoadCorridor`.
...
uk_gov_tfl_road_disruptions_dispatcher.uk_gov_tfl_road_road_corridor_async = uk_gov_tfl_road_road_corridor_event
...
### UkGovTflRoadAmqpProducer- `data`: The event data of type `tfl_road_traffic_producer_data.RoadCorridor`.



Producer for `uk.gov.tfl.road.amqp` message group.Example:
...
UkGovTflRoadAmqpProducer(    await some_processing_function(record, cloud_event, data)
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_road_corridor_async = uk_gov_tfl_road_road_corridor_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `uk_gov_tfl_road_road_status_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_tfl_road_road_status_async:  Callable[[ConsumerRecord, CloudEvent, RoadStatus],
Awaitable[None]]
...
Asynchronous handler hook for `uk.gov.tfl.road.RoadStatus`: Real-time status snapshot for a TfL managed road corridor
...
uk_gov_tfl_road_corridors_dispatcher.uk_gov_tfl_road_road_status_async = uk_gov_tfl_road_road_status_event
...
### UkGovTflRoadAmqpProducer- `data`: The event data of type `tfl_road_traffic_producer_data.RoadStatus`.
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_road_status_async = uk_gov_tfl_road_road_status_event
...
##### `send_uk_gov_tfl_road_road_corridor`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_tfl_road_road_corridor(
...
Send a single `uk.gov.tfl.road.RoadCorridor` message. Reference record for a TfL managed road corridor fetched from GET
/Road.Args:
...
- `data`: Message data of type `RoadCorridor`
...
await producer.send_uk_gov_tfl_road_road_corridor(

    data=RoadCorridor(...),Args:
...
Send multiple `uk.gov.tfl.road.RoadCorridor` messages in a batch.
...
await producer.send_uk_gov_tfl_road_road_corridor_batch(```

    messages=[

        RoadCorridor(...),Initializes the runner with a Kafka consumer.
...
##### `send_uk_gov_tfl_road_road_status`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_tfl_road_road_status(
...
Send a single `uk.gov.tfl.road.RoadStatus` message. Real-time status snapshot for a TfL managed road corridor fetched
...
await producer.send_uk_gov_tfl_road_road_status(
...
Send multiple `uk.gov.tfl.road.RoadStatus` messages in a batch.
...
await producer.send_uk_gov_tfl_road_road_status_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### UkGovTflRoadDisruptionsEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`UkGovTflRoadDisruptionsEventDispatcher` handles events for the uk.gov.tfl.road.disruptions message group.
...
The UkGovTflRoadDisruptionsEventDispatcher defines the following event handler hooks.
...
producer = UkGovTflRoadCorridorsProducer(
    ...
    security_protocol='SASL_SSL',##### `uk_gov_tfl_road_road_disruption_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_tfl_road_road_disruption_async:  Callable[[ConsumerRecord, CloudEvent,
RoadDisruption], Awaitable[None]]

)```

```

Asynchronous handler hook for `uk.gov.tfl.road.RoadDisruption`: Real-time road disruption event on the TfL road network
fetched from GET /Road/all/Disruption.
...
uk_gov_tfl_road_corridors_dispatcher.uk_gov_tfl_road_road_disruption_async = uk_gov_tfl_road_road_disruption_event
...
uk_gov_tfl_road_disruptions_dispatcher.uk_gov_tfl_road_road_disruption_async = uk_gov_tfl_road_road_disruption_event
...
### UkGovTflRoadAmqpProducer- `data`: The event data of type `tfl_road_traffic_producer_data.RoadDisruption`.
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_road_disruption_async = uk_gov_tfl_road_road_disruption_event
...
##### `send_uk_gov_tfl_road_road_disruption`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_tfl_road_road_disruption(
...
Send a single `uk.gov.tfl.road.RoadDisruption` message. Real-time road disruption event on the TfL road network fetched
from GET /Road/all/Disruption.Args:
...
await producer.send_uk_gov_tfl_road_road_disruption(
...
Send multiple `uk.gov.tfl.road.RoadDisruption` messages in a batch.
...
producer = UkGovTflRoadCorridorsProducer(
    ...
    security_protocol='SASL_SSL',##### `uk_gov_tfl_road_mqtt_road_corridor_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_tfl_road_mqtt_road_corridor_async:  Callable[[ConsumerRecord, CloudEvent,
RoadCorridor], Awaitable[None]]
...
async def uk_gov_tfl_road_mqtt_road_corridor_event(record: ConsumerRecord, cloud_event: CloudEvent, data: RoadCorridor)
-> None:
...
uk_gov_tfl_road_corridors_dispatcher.uk_gov_tfl_road_mqtt_road_corridor_async = uk_gov_tfl_road_mqtt_road_corridor_event
...
uk_gov_tfl_road_disruptions_dispatcher.uk_gov_tfl_road_mqtt_road_corridor_async =
uk_gov_tfl_road_mqtt_road_corridor_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_mqtt_road_corridor_async = uk_gov_tfl_road_mqtt_road_corridor_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `uk_gov_tfl_road_mqtt_road_status_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_tfl_road_mqtt_road_status_async:  Callable[[ConsumerRecord, CloudEvent,
RoadStatus], Awaitable[None]]
...
async def uk_gov_tfl_road_mqtt_road_status_event(record: ConsumerRecord, cloud_event: CloudEvent, data: RoadStatus) ->
None:
...
uk_gov_tfl_road_corridors_dispatcher.uk_gov_tfl_road_mqtt_road_status_async = uk_gov_tfl_road_mqtt_road_status_event
...
uk_gov_tfl_road_disruptions_dispatcher.uk_gov_tfl_road_mqtt_road_status_async = uk_gov_tfl_road_mqtt_road_status_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_mqtt_road_status_async = uk_gov_tfl_road_mqtt_road_status_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_mqtt_road_disruption_serious_async =
uk_gov_tfl_road_mqtt_road_disruption_serious_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_mqtt_road_disruption_severe_async =
uk_gov_tfl_road_mqtt_road_disruption_severe_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_mqtt_road_disruption_moderate_async =
uk_gov_tfl_road_mqtt_road_disruption_moderate_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_mqtt_road_disruption_minor_async =
uk_gov_tfl_road_mqtt_road_disruption_minor_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_mqtt_road_disruption_information_async =
uk_gov_tfl_road_mqtt_road_disruption_information_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_mqtt_road_disruption_closure_async =
uk_gov_tfl_road_mqtt_road_disruption_closure_event
...
##### `send_uk_gov_tfl_road_mqtt_road_corridor`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_tfl_road_mqtt_road_corridor(
...
Send a single `uk.gov.tfl.road.mqtt.RoadCorridor` message. Reference record for a TfL managed road corridor fetched from
GET /Road.Args:
...
await producer.send_uk_gov_tfl_road_mqtt_road_corridor(
...
Send multiple `uk.gov.tfl.road.mqtt.RoadCorridor` messages in a batch.
...
await producer.send_uk_gov_tfl_road_mqtt_road_corridor_batch(```
...
##### `send_uk_gov_tfl_road_mqtt_road_status`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_tfl_road_mqtt_road_status(
...
Send a single `uk.gov.tfl.road.mqtt.RoadStatus` message. Real-time status snapshot for a TfL managed road corridor
fetched from GET /Road/all/Status.Args:
...
await producer.send_uk_gov_tfl_road_mqtt_road_status(
...
Send multiple `uk.gov.tfl.road.mqtt.RoadStatus` messages in a batch.
...
await producer.send_uk_gov_tfl_road_mqtt_road_status_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### UkGovTflRoadAmqpEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`UkGovTflRoadAmqpEventDispatcher` handles events for the uk.gov.tfl.road.amqp message group.
...
The UkGovTflRoadAmqpEventDispatcher defines the following event handler hooks.
...
producer = UkGovTflRoadCorridorsProducer(
    ...
    security_protocol='SASL_SSL',##### `uk_gov_tfl_road_amqp_road_corridor_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_tfl_road_amqp_road_corridor_async:  Callable[[ConsumerRecord, CloudEvent,
RoadCorridor], Awaitable[None]]
...
Asynchronous handler hook for `uk.gov.tfl.road.amqp.RoadCorridor`: Reference record for a TfL managed road corridor
fetched from GET /Road.
...
async def uk_gov_tfl_road_amqp_road_corridor_event(record: ConsumerRecord, cloud_event: CloudEvent, data: RoadCorridor)
-> None:
...
uk_gov_tfl_road_corridors_dispatcher.uk_gov_tfl_road_amqp_road_corridor_async = uk_gov_tfl_road_amqp_road_corridor_event
...
uk_gov_tfl_road_disruptions_dispatcher.uk_gov_tfl_road_amqp_road_corridor_async =
uk_gov_tfl_road_amqp_road_corridor_event
...
uk_gov_tfl_road_mqtt_dispatcher.uk_gov_tfl_road_amqp_road_corridor_async = uk_gov_tfl_road_amqp_road_corridor_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_amqp_road_corridor_async = uk_gov_tfl_road_amqp_road_corridor_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `uk_gov_tfl_road_amqp_road_status_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_tfl_road_amqp_road_status_async:  Callable[[ConsumerRecord, CloudEvent,
RoadStatus], Awaitable[None]]
...
Asynchronous handler hook for `uk.gov.tfl.road.amqp.RoadStatus`: Real-time status snapshot for a TfL managed road
corridor fetched from GET /Road/all/Status.
...
async def uk_gov_tfl_road_amqp_road_status_event(record: ConsumerRecord, cloud_event: CloudEvent, data: RoadStatus) ->
None:
...
uk_gov_tfl_road_corridors_dispatcher.uk_gov_tfl_road_amqp_road_status_async = uk_gov_tfl_road_amqp_road_status_event
...
uk_gov_tfl_road_disruptions_dispatcher.uk_gov_tfl_road_amqp_road_status_async = uk_gov_tfl_road_amqp_road_status_event
...
uk_gov_tfl_road_mqtt_dispatcher.uk_gov_tfl_road_amqp_road_status_async = uk_gov_tfl_road_amqp_road_status_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_amqp_road_status_async = uk_gov_tfl_road_amqp_road_status_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `uk_gov_tfl_road_amqp_road_disruption_serious_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_tfl_road_amqp_road_disruption_serious_async:  Callable[[ConsumerRecord,
CloudEvent, RoadDisruption], Awaitable[None]]
...
Asynchronous handler hook for `uk.gov.tfl.road.amqp.RoadDisruptionSerious`: Real-time road disruption event on the TfL
road network fetched from GET /Road/all/Disruption.
...
async def uk_gov_tfl_road_amqp_road_disruption_serious_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
RoadDisruption) -> None:
...
uk_gov_tfl_road_corridors_dispatcher.uk_gov_tfl_road_amqp_road_disruption_serious_async =
uk_gov_tfl_road_amqp_road_disruption_serious_event
...
uk_gov_tfl_road_disruptions_dispatcher.uk_gov_tfl_road_amqp_road_disruption_serious_async =
uk_gov_tfl_road_amqp_road_disruption_serious_event
...
uk_gov_tfl_road_mqtt_dispatcher.uk_gov_tfl_road_amqp_road_disruption_serious_async =
uk_gov_tfl_road_amqp_road_disruption_serious_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_amqp_road_disruption_serious_async =
uk_gov_tfl_road_amqp_road_disruption_serious_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `uk_gov_tfl_road_amqp_road_disruption_severe_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_tfl_road_amqp_road_disruption_severe_async:  Callable[[ConsumerRecord,
CloudEvent, RoadDisruption], Awaitable[None]]
...
Asynchronous handler hook for `uk.gov.tfl.road.amqp.RoadDisruptionSevere`: Real-time road disruption event on the TfL
road network fetched from GET /Road/all/Disruption.
...
async def uk_gov_tfl_road_amqp_road_disruption_severe_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
RoadDisruption) -> None:
...
uk_gov_tfl_road_corridors_dispatcher.uk_gov_tfl_road_amqp_road_disruption_severe_async =
uk_gov_tfl_road_amqp_road_disruption_severe_event
...
uk_gov_tfl_road_disruptions_dispatcher.uk_gov_tfl_road_amqp_road_disruption_severe_async =
uk_gov_tfl_road_amqp_road_disruption_severe_event
...
uk_gov_tfl_road_mqtt_dispatcher.uk_gov_tfl_road_amqp_road_disruption_severe_async =
uk_gov_tfl_road_amqp_road_disruption_severe_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_amqp_road_disruption_severe_async =
uk_gov_tfl_road_amqp_road_disruption_severe_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `uk_gov_tfl_road_amqp_road_disruption_moderate_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_tfl_road_amqp_road_disruption_moderate_async:  Callable[[ConsumerRecord,
CloudEvent, RoadDisruption], Awaitable[None]]
...
Asynchronous handler hook for `uk.gov.tfl.road.amqp.RoadDisruptionModerate`: Real-time road disruption event on the TfL
road network fetched from GET /Road/all/Disruption.
...
async def uk_gov_tfl_road_amqp_road_disruption_moderate_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
RoadDisruption) -> None:
...
uk_gov_tfl_road_corridors_dispatcher.uk_gov_tfl_road_amqp_road_disruption_moderate_async =
uk_gov_tfl_road_amqp_road_disruption_moderate_event
...
uk_gov_tfl_road_disruptions_dispatcher.uk_gov_tfl_road_amqp_road_disruption_moderate_async =
uk_gov_tfl_road_amqp_road_disruption_moderate_event
...
uk_gov_tfl_road_mqtt_dispatcher.uk_gov_tfl_road_amqp_road_disruption_moderate_async =
uk_gov_tfl_road_amqp_road_disruption_moderate_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_amqp_road_disruption_moderate_async =
uk_gov_tfl_road_amqp_road_disruption_moderate_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `uk_gov_tfl_road_amqp_road_disruption_minor_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_tfl_road_amqp_road_disruption_minor_async:  Callable[[ConsumerRecord,
CloudEvent, RoadDisruption], Awaitable[None]]
...
Asynchronous handler hook for `uk.gov.tfl.road.amqp.RoadDisruptionMinor`: Real-time road disruption event on the TfL
road network fetched from GET /Road/all/Disruption.
...
async def uk_gov_tfl_road_amqp_road_disruption_minor_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
RoadDisruption) -> None:
...
uk_gov_tfl_road_corridors_dispatcher.uk_gov_tfl_road_amqp_road_disruption_minor_async =
uk_gov_tfl_road_amqp_road_disruption_minor_event
...
uk_gov_tfl_road_disruptions_dispatcher.uk_gov_tfl_road_amqp_road_disruption_minor_async =
uk_gov_tfl_road_amqp_road_disruption_minor_event
...
uk_gov_tfl_road_mqtt_dispatcher.uk_gov_tfl_road_amqp_road_disruption_minor_async =
uk_gov_tfl_road_amqp_road_disruption_minor_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_amqp_road_disruption_minor_async =
uk_gov_tfl_road_amqp_road_disruption_minor_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `uk_gov_tfl_road_amqp_road_disruption_information_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_tfl_road_amqp_road_disruption_information_async:  Callable[[ConsumerRecord,
CloudEvent, RoadDisruption], Awaitable[None]]
...
Asynchronous handler hook for `uk.gov.tfl.road.amqp.RoadDisruptionInformation`: Real-time road disruption event on the
TfL road network fetched from GET /Road/all/Disruption.
...
async def uk_gov_tfl_road_amqp_road_disruption_information_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
RoadDisruption) -> None:
...
uk_gov_tfl_road_corridors_dispatcher.uk_gov_tfl_road_amqp_road_disruption_information_async =
uk_gov_tfl_road_amqp_road_disruption_information_event
...
uk_gov_tfl_road_disruptions_dispatcher.uk_gov_tfl_road_amqp_road_disruption_information_async =
uk_gov_tfl_road_amqp_road_disruption_information_event
...
uk_gov_tfl_road_mqtt_dispatcher.uk_gov_tfl_road_amqp_road_disruption_information_async =
uk_gov_tfl_road_amqp_road_disruption_information_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_amqp_road_disruption_information_async =
uk_gov_tfl_road_amqp_road_disruption_information_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `uk_gov_tfl_road_amqp_road_disruption_closure_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_tfl_road_amqp_road_disruption_closure_async:  Callable[[ConsumerRecord,
CloudEvent, RoadDisruption], Awaitable[None]]
...
Asynchronous handler hook for `uk.gov.tfl.road.amqp.RoadDisruptionClosure`: Real-time road disruption event on the TfL
road network fetched from GET /Road/all/Disruption.
...
async def uk_gov_tfl_road_amqp_road_disruption_closure_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
RoadDisruption) -> None:
...
uk_gov_tfl_road_corridors_dispatcher.uk_gov_tfl_road_amqp_road_disruption_closure_async =
uk_gov_tfl_road_amqp_road_disruption_closure_event
...
uk_gov_tfl_road_disruptions_dispatcher.uk_gov_tfl_road_amqp_road_disruption_closure_async =
uk_gov_tfl_road_amqp_road_disruption_closure_event
...
uk_gov_tfl_road_mqtt_dispatcher.uk_gov_tfl_road_amqp_road_disruption_closure_async =
uk_gov_tfl_road_amqp_road_disruption_closure_event
...
uk_gov_tfl_road_amqp_dispatcher.uk_gov_tfl_road_amqp_road_disruption_closure_async =
uk_gov_tfl_road_amqp_road_disruption_closure_event
...
##### `send_uk_gov_tfl_road_amqp_road_corridor`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_tfl_road_amqp_road_corridor(
...
Send a single `uk.gov.tfl.road.amqp.RoadCorridor` message. Reference record for a TfL managed road corridor fetched from
GET /Road.Args:
...
await producer.send_uk_gov_tfl_road_amqp_road_corridor(
...
Send multiple `uk.gov.tfl.road.amqp.RoadCorridor` messages in a batch.
...
await producer.send_uk_gov_tfl_road_amqp_road_corridor_batch(```
...
##### `send_uk_gov_tfl_road_amqp_road_status`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_tfl_road_amqp_road_status(
...
Send a single `uk.gov.tfl.road.amqp.RoadStatus` message. Real-time status snapshot for a TfL managed road corridor
fetched from GET /Road/all/Status.Args:
...
await producer.send_uk_gov_tfl_road_amqp_road_status(
...
Send multiple `uk.gov.tfl.road.amqp.RoadStatus` messages in a batch.
...
await producer.send_uk_gov_tfl_road_amqp_road_status_batch(```
...
##### `send_uk_gov_tfl_road_amqp_road_disruption_serious`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_tfl_road_amqp_road_disruption_serious(
...
Send a single `uk.gov.tfl.road.amqp.RoadDisruptionSerious` message. Real-time road disruption event on the TfL road
network fetched from GET /Road/all/Disruption.Args:
...
await producer.send_uk_gov_tfl_road_amqp_road_disruption_serious(
...
Send multiple `uk.gov.tfl.road.amqp.RoadDisruptionSerious` messages in a batch.
...
await producer.send_uk_gov_tfl_road_amqp_road_disruption_serious_batch(```
...
##### `send_uk_gov_tfl_road_amqp_road_disruption_severe`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_tfl_road_amqp_road_disruption_severe(
...
Send a single `uk.gov.tfl.road.amqp.RoadDisruptionSevere` message. Real-time road disruption event on the TfL road
network fetched from GET /Road/all/Disruption.Args:
...
await producer.send_uk_gov_tfl_road_amqp_road_disruption_severe(
...
Send multiple `uk.gov.tfl.road.amqp.RoadDisruptionSevere` messages in a batch.
...
await producer.send_uk_gov_tfl_road_amqp_road_disruption_severe_batch(```
...
##### `send_uk_gov_tfl_road_amqp_road_disruption_moderate`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_tfl_road_amqp_road_disruption_moderate(
...
Send a single `uk.gov.tfl.road.amqp.RoadDisruptionModerate` message. Real-time road disruption event on the TfL road
network fetched from GET /Road/all/Disruption.Args:
...
await producer.send_uk_gov_tfl_road_amqp_road_disruption_moderate(
...
Send multiple `uk.gov.tfl.road.amqp.RoadDisruptionModerate` messages in a batch.
...
await producer.send_uk_gov_tfl_road_amqp_road_disruption_moderate_batch(```
...
##### `send_uk_gov_tfl_road_amqp_road_disruption_minor`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_tfl_road_amqp_road_disruption_minor(
...
Send a single `uk.gov.tfl.road.amqp.RoadDisruptionMinor` message. Real-time road disruption event on the TfL road
network fetched from GET /Road/all/Disruption.Args:
...
await producer.send_uk_gov_tfl_road_amqp_road_disruption_minor(
...
Send multiple `uk.gov.tfl.road.amqp.RoadDisruptionMinor` messages in a batch.
...
await producer.send_uk_gov_tfl_road_amqp_road_disruption_minor_batch(```
...
##### `send_uk_gov_tfl_road_amqp_road_disruption_information`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_tfl_road_amqp_road_disruption_information(
...
Send a single `uk.gov.tfl.road.amqp.RoadDisruptionInformation` message. Real-time road disruption event on the TfL road
network fetched from GET /Road/all/Disruption.Args:
...
await producer.send_uk_gov_tfl_road_amqp_road_disruption_information(
...
Send multiple `uk.gov.tfl.road.amqp.RoadDisruptionInformation` messages in a batch.
...
await producer.send_uk_gov_tfl_road_amqp_road_disruption_information_batch(```
...
##### `send_uk_gov_tfl_road_amqp_road_disruption_closure`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_tfl_road_amqp_road_disruption_closure(
...
Send a single `uk.gov.tfl.road.amqp.RoadDisruptionClosure` message. Real-time road disruption event on the TfL road
network fetched from GET /Road/all/Disruption.Args:
...
await producer.send_uk_gov_tfl_road_amqp_road_disruption_closure(
...
Send multiple `uk.gov.tfl.road.amqp.RoadDisruptionClosure` messages in a batch.
...
await producer.send_uk_gov_tfl_road_amqp_road_disruption_closure_batch(```
