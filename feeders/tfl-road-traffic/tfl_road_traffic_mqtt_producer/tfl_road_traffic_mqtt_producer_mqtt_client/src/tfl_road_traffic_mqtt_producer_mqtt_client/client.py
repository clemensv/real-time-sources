...
def get_subscription_topics_uk_gov_tfl_road_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_uk_gov_tfl_road_mqtt()
    topics = []
...
class UkGovTflRoadMqttMqttClient(_ClientBase):
        ...
        if event_type == "uk.gov.tfl.road.RoadDisruption":
            if self.uk_gov_tfl_road_mqtt_road_disruption_serious_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = tfl_road_traffic_mqtt_producer_data.RoadDisruption.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.gov.tfl.road.mqtt.RoadDisruptionSerious")
                    await self.uk_gov_tfl_road_mqtt_road_disruption_serious_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
        ...
        if event_type == "uk.gov.tfl.road.RoadDisruption":
            if self.uk_gov_tfl_road_mqtt_road_disruption_severe_async:
                try:
                    ...
                    # CloudEvent.data is now a dict or string, not bytes
                    data = tfl_road_traffic_mqtt_producer_data.RoadDisruption.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.gov.tfl.road.mqtt.RoadDisruptionSevere")
                    await self.uk_gov_tfl_road_mqtt_road_disruption_severe_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
        ...
        if event_type == "uk.gov.tfl.road.RoadDisruption":
            if self.uk_gov_tfl_road_mqtt_road_disruption_moderate_async:
                try:
                    ...
                    # CloudEvent.data is now a dict or string, not bytes
                    data = tfl_road_traffic_mqtt_producer_data.RoadDisruption.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.gov.tfl.road.mqtt.RoadDisruptionModerate")
                    await self.uk_gov_tfl_road_mqtt_road_disruption_moderate_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
        ...
        if event_type == "uk.gov.tfl.road.RoadDisruption":
            if self.uk_gov_tfl_road_mqtt_road_disruption_minor_async:
                try:
                    ...
                    # CloudEvent.data is now a dict or string, not bytes
                    data = tfl_road_traffic_mqtt_producer_data.RoadDisruption.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.gov.tfl.road.mqtt.RoadDisruptionMinor")
                    await self.uk_gov_tfl_road_mqtt_road_disruption_minor_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
        ...
        if event_type == "uk.gov.tfl.road.RoadDisruption":
            if self.uk_gov_tfl_road_mqtt_road_disruption_information_async:
                try:
                    ...
                    # CloudEvent.data is now a dict or string, not bytes
                    data = tfl_road_traffic_mqtt_producer_data.RoadDisruption.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.gov.tfl.road.mqtt.RoadDisruptionInformation")
                    await self.uk_gov_tfl_road_mqtt_road_disruption_information_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
        ...
        if event_type == "uk.gov.tfl.road.RoadDisruption":
            if self.uk_gov_tfl_road_mqtt_road_disruption_closure_async:
                try:
                    ...
                    # CloudEvent.data is now a dict or string, not bytes
                    data = tfl_road_traffic_mqtt_producer_data.RoadDisruption.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.gov.tfl.road.mqtt.RoadDisruptionClosure")
                    await self.uk_gov_tfl_road_mqtt_road_disruption_closure_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
    ...
    async def publish_uk_gov_tfl_road_mqtt_road_disruption_serious(self,
        ...
        severity: str,
        disruption_id: str,
        data: tfl_road_traffic_mqtt_producer_data.RoadDisruption,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
