...
"""
Tests for uk_ea_flood_monitoring_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../uk_ea_flood_monitoring_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../uk_ea_flood_monitoring_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../uk_ea_flood_monitoring_amqp_producer_amqp_producer/src')))

from uk_ea_flood_monitoring_amqp_producer_amqp_producer import *
...
class TestUKGovEnvironmentEAFloodMonitoringAmqpProducer:
    """Test cases for UKGovEnvironmentEAFloodMonitoringAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(UKGovEnvironmentEAFloodMonitoringAmqpProducer)
    ...
    def test_send_station(self, artemis_container):
        ...
        try:
            ...
            for i in range(5):
                ...
                assert annotations.get(symbol('x-opt-partition-key')) == str("{station_reference}".format(station_reference="value"))[:128]
        finally:
            producer.close()
    ...
    def test_send_reading(self, artemis_container):
        """Send and receive a Reading message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Reading.create_instance()
    ...
    def test_send_reading_single_fresh_connection(self, artemis_container):
        """Send exactly one Reading message on a fresh producer connection."""
        payload = Test_Reading.create_instance()
