    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from uk_ea_flood_monitoring_amqp_producer_data import Station
from uk_ea_flood_monitoring_amqp_producer_data import Reading

class UKGovEnvironmentEAFloodMonitoringAmqpProducer:
    """
    Producer class to send messages in the `UK.Gov.Environment.EA.FloodMonitoring.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        data: Station,
        _station_reference: str,
        _river: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send the `UK.Gov.Environment.EA.FloodMonitoring.amqp.Station` message
        A reference record for one monitoring stations across England published by the Environment Agency flood-monitoring API. It fires when the bridge publishes or refreshes the station catalog so consumers can interpret measurement events.
        
        Args:
            _station_reference (str): Value for placeholder station_reference in attribute subject
            _river (str): Value for AMQP protocol option placeholder river
        ...
        attributes = {
            ...
            "subject":
            "{station_reference}".format(station_reference=_station_reference),
        }
        ...
        amqp_msg.subject = "{station_reference}".format(station_reference=_station_reference)

        app_properties = {}
        app_properties["river"] = "{river}".format(river=_river)
        if app_properties:
        ...
        annotations = {}
        annotation_value = "{station_reference}".format(station_reference=_station_reference)
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `UK.Gov.Environment.EA.FloodMonitoring.amqp.Station` messages
        ...
        for data in data_array:
            self.send_station(
                data=data,
                _station_reference=_station_reference,
                _time=_time,
                _river=_river,
    ...
    def send_reading(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `UK.Gov.Environment.EA.FloodMonitoring.amqp.Reading` message
        A reading event from the Environment Agency flood-monitoring API. It represents source data for monitoring stations across England as published by the upstream feed.
        
        Args:
            _station_reference (str): Value for placeholder station_reference in attribute subject
            _river (str): Value for AMQP protocol option placeholder river
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            data (Reading): The message data object
            content_type (str): The content type of the message data (default: 'application/json')
        """
        # Build CloudEvent attributes
        attributes = {
    ...
    def send_reading_batch(self,
        data_array: typing.List[Reading],
        _station_reference: str,
        _river: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `UK.Gov.Environment.EA.FloodMonitoring.amqp.Reading` messages
        
        Args:
            data_array (typing.List[Reading]): Array of message data objects
            _station_reference (str): Value for placeholder station_reference in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            _river (str): Value for AMQP protocol option placeholder river
            content_type (str): The content type of the message data
