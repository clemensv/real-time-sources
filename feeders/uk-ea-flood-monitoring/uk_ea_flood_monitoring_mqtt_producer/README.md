# Uk_ea_flood_monitoring_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the uk_ea_flood_monitoring_mqtt_producer message definitions.

# Uk_ea_flood_monitoring_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - UKGovEnvironmentEAFloodMonitoringMqttEventDispatcher
...
from uk_ea_flood_monitoring_mqtt_producer_mqtt_client import *This module defines an event processing framework for
Azure Event Hubs,
...
client = UKGovEnvironmentEAFloodMonitoringMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### UKGovEnvironmentEAFloodMonitoringMqttEventDispatcher



# Publish message`UKGovEnvironmentEAFloodMonitoringMqttEventDispatcher` handles events for the
UK.Gov.Environment.EA.FloodMonitoring.mqtt message group.
...
from uk_ea_flood_monitoring_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = UKGovEnvironmentEAFloodMonitoringMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The
...
## BuildThe UKGovEnvironmentEAFloodMonitoringMqttEventDispatcher defines the following event handler hooks.
...
```##### `uk_gov_environment_ea_flood_monitoring_mqtt_station_async`



## Test```python

uk_gov_environment_ea_flood_monitoring_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station],
Awaitable[None]]
...
- `data`: The event data of type `uk_ea_flood_monitoring_mqtt_producer_data.Station`.

Example:

```python
async def uk_gov_environment_ea_flood_monitoring_mqtt_station_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: Station) -> None:
...
```##### `uk_gov_environment_ea_flood_monitoring_mqtt_reading_async`



## Test```python

uk_gov_environment_ea_flood_monitoring_mqtt_reading_async:  Callable[[PartitionContext, EventData, CloudEvent, Reading],
Awaitable[None]]
...
```Asynchronous handler hook for `UK.Gov.Environment.EA.FloodMonitoring.mqtt.Reading`: A reading event from the
Environment Agency flood-monitoring API. It represents source data for monitoring stations across England as published
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `uk_ea_flood_monitoring_mqtt_producer_data.Reading`.

Example:

```python
async def uk_gov_environment_ea_flood_monitoring_mqtt_reading_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: Reading) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
