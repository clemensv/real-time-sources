...
import os
import signal
from uk_ea_flood_monitoring_mqtt_producer_data import *
from uk_ea_flood_monitoring_mqtt_producer_mqtt_client.client import UKGovEnvironmentEAFloodMonitoringMqttProducer, UKGovEnvironmentEAFloodMonitoringMqttDispatcher

async def handle_uk_gov_environment_ea_flood_monitoring_mqtt_station(mqtt_msg,cloud_event, uk_gov_environment_ea_flood_monitoring_mqtt_station_data):
    """ Handles the UK.Gov.Environment.EA.FloodMonitoring.mqtt.Station message """
    print(f"Received UK.Gov.Environment.EA.FloodMonitoring.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_gov_environment_ea_flood_monitoring_mqtt_station_data}")

async def handle_uk_gov_environment_ea_flood_monitoring_mqtt_reading(mqtt_msg,cloud_event, uk_gov_environment_ea_flood_monitoring_mqtt_reading_data):
    """ Handles the UK.Gov.Environment.EA.FloodMonitoring.mqtt.Reading message """
    print(f"Received UK.Gov.Environment.EA.FloodMonitoring.mqtt.Reading on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_gov_environment_ea_flood_monitoring_mqtt_reading_data}")
