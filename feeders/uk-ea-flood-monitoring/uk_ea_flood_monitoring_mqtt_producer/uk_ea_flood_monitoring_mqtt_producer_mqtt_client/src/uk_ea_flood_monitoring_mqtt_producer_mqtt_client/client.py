...
def get_default_topic_mappings_uk_gov_environment_ea_floodmonitoring_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the UK.Gov.Environment.EA.FloodMonitoring.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "UK.Gov.Environment.EA.FloodMonitoring.mqtt.Station": "hydro/gb/ea/uk-ea-flood-monitoring/{river}/{station_reference}/info",
        "UK.Gov.Environment.EA.FloodMonitoring.mqtt.Reading": "hydro/gb/ea/uk-ea-flood-monitoring/{river}/{station_reference}/reading",
    }


def get_subscription_topics_uk_gov_environment_ea_floodmonitoring_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_uk_gov_environment_ea_floodmonitoring_mqtt()
    topics = []
...
class UKGovEnvironmentEAFloodMonitoringMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the UK.Gov.Environment.EA.FloodMonitoring.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_uk_gov_environment_ea_floodmonitoring_mqtt()
        ...
        self.uk_gov_environment_ea_flood_monitoring_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, uk_ea_flood_monitoring_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.uk_gov_environment_ea_flood_monitoring_mqtt_reading_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, uk_ea_flood_monitoring_mqtt_producer_data.Reading, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "UK.Gov.Environment.EA.FloodMonitoring.Station":
            if self.uk_gov_environment_ea_flood_monitoring_mqtt_station_async:
                try:
                    ...
                    data = uk_ea_flood_monitoring_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "UK.Gov.Environment.EA.FloodMonitoring.mqtt.Station")
                    await self.uk_gov_environment_ea_flood_monitoring_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_gov_environment_ea_flood_monitoring_mqtt_station handler: {e}")
            return
        
        if event_type == "UK.Gov.Environment.EA.FloodMonitoring.Reading":
            if self.uk_gov_environment_ea_flood_monitoring_mqtt_reading_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = uk_ea_flood_monitoring_mqtt_producer_data.Reading.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "UK.Gov.Environment.EA.FloodMonitoring.mqtt.Reading")
                    await self.uk_gov_environment_ea_flood_monitoring_mqtt_reading_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_gov_environment_ea_flood_monitoring_mqtt_reading handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_uk_gov_environment_ea_floodmonitoring_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_uk_gov_environment_ea_flood_monitoring_mqtt_station(self,
        station_reference: str,
        river: str,
        data: uk_ea_flood_monitoring_mqtt_producer_data.Station,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'UK.Gov.Environment.EA.FloodMonitoring.mqtt.Station' event to an MQTT topic.

        Args:
        
            station_reference: URI template variable for 'station_reference'
            river: URI template variable for 'river'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/gb/ea/uk-ea-flood-monitoring/{river}/{station_reference}/info'
        ...
        """
        target_topic = topic if topic is not None else "hydro/gb/ea/uk-ea-flood-monitoring/{river}/{station_reference}/info"
        _topic_template_values: Dict[str, str] = {
            "station_reference": str(station_reference),
            "river": str(river),
        }
        ...
        attributes = {
             "type":"UK.Gov.Environment.EA.FloodMonitoring.Station",
             "source":"https://environment.data.gov.uk/flood-monitoring",
             "subject":"{station_reference}".format(station_reference = station_reference)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_uk_gov_environment_ea_flood_monitoring_mqtt_reading(self,
        station_reference: str,
        river: str,
        data: uk_ea_flood_monitoring_mqtt_producer_data.Reading,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'UK.Gov.Environment.EA.FloodMonitoring.mqtt.Reading' event to an MQTT topic.
        ...
        Args:
            ...
            river: URI template variable for 'river'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/gb/ea/uk-ea-flood-monitoring/{river}/{station_reference}/reading'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "hydro/gb/ea/uk-ea-flood-monitoring/{river}/{station_reference}/reading"
        _topic_template_values: Dict[str, str] = {
            "station_reference": str(station_reference),
            "river": str(river),
        }
        ...
        attributes = {
             "type":"UK.Gov.Environment.EA.FloodMonitoring.Reading",
             "source":"https://environment.data.gov.uk/flood-monitoring",
             "subject":"{station_reference}".format(station_reference = station_reference)
        }
        attributes["datacontenttype"] = content_type
