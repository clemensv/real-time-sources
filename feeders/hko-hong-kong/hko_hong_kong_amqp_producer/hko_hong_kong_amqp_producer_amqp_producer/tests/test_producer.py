...
"""
Tests for hko_hong_kong_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../hko_hong_kong_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../hko_hong_kong_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../hko_hong_kong_amqp_producer_amqp_producer/src')))

from hko_hong_kong_amqp_producer_amqp_producer import *
from hko_hong_kong_amqp_producer_data import Station
from test_station import Test_Station
from hko_hong_kong_amqp_producer_data import WeatherObservation
...
class TestHKGovHKOWeatherAmqpProducer:
    """Test cases for HKGovHKOWeatherAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(HKGovHKOWeatherAmqpProducer)
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "HK.Gov.HKO.Weather.amqp.Station"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{place_id}".format(place_id="value")
    ...
    def test_send_station_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'HK.Gov.HKO.Weather.amqp.Station'
        assert received.body is not None
        assert received.subject == "{place_id}".format(place_id="value")
        assert properties.get('district') == "{district}".format(district="value")
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "HK.Gov.HKO.Weather.amqp.WeatherObservation"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_weather_observation_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'HK.Gov.HKO.Weather.amqp.WeatherObservation'
        assert received.body is not None
        assert received.subject == "{place_id}".format(place_id="value")
        assert properties.get('district') == "{district}".format(district="value")
