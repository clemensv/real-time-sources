...
# Epa-uv-producer Kafka Producer# Epa-uv-producer Event Dispatcher for Apache Kafka
...
4. [Generated Producer Classes](#generated-producer-classes)    USEPAUVIndexAmqpEventDispatcher
...
- USEPAUVIndexAmqpProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### USEPAUVIndexEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`USEPAUVIndexEventDispatcher` handles events for the US.EPA.UVIndex message group.
...
The USEPAUVIndexEventDispatcher defines the following event handler hooks.
...
producer = USEPAUVIndexProducer(
    ...
    security_protocol='SASL_SSL',##### `us_epa_uvindex_hourly_forecast_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'us_epa_uvindex_hourly_forecast_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `US.EPA.UVIndex.HourlyForecast`: An hourly UV Index forecast from the US EPA Envirofacts
UV service for one configured city/state location. It carries the forecast valid hour, normalized location identity, and
UV index value for operational exposure planning.
...
us_epa_uvindex_dispatcher.us_epa_uvindex_hourly_forecast_async = us_epa_uvindex_hourly_forecast_event
...
### USEPAUVIndexAmqpProducer- `data`: The event data of type `epa_uv_producer_data.HourlyForecast`.



Producer for `US.EPA.UVIndex.amqp` message group.Example:
...
USEPAUVIndexAmqpProducer(    await some_processing_function(record, cloud_event, data)
...
us_epa_uvindex_amqp_dispatcher.us_epa_uvindex_hourly_forecast_async = us_epa_uvindex_hourly_forecast_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `us_epa_uvindex_daily_forecast_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'us_epa_uvindex_daily_forecast_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `US.EPA.UVIndex.DailyForecast`: A daily UV Index forecast from the US EPA Envirofacts UV
service for one configured city/state location. It carries the forecast date, normalized location identity, UV index
value, and EPA UV alert indicator for public-health planning.
...
### USEPAUVIndexAmqpProducer- `data`: The event data of type `epa_uv_producer_data.DailyForecast`.
...
us_epa_uvindex_amqp_dispatcher.us_epa_uvindex_daily_forecast_async = us_epa_uvindex_daily_forecast_event
...
##### `send_us_epa_uvindex_hourly_forecast`Dispatchers have the following protected methods:



```python### Methods:

async def send_us_epa_uvindex_hourly_forecast(
...
Send a single `US.EPA.UVIndex.HourlyForecast` message. An hourly UV Index forecast from the US EPA Envirofacts UV
service for one configured city/state location. It carries the forecast valid hour, normalized location identity, and UV
index value for operational exposure planning.Args:
...
await producer.send_us_epa_uvindex_hourly_forecast(
...
Send multiple `US.EPA.UVIndex.HourlyForecast` messages in a batch.
...
await producer.send_us_epa_uvindex_hourly_forecast_batch(```
...
##### `send_us_epa_uvindex_daily_forecast`Dispatchers have the following protected methods:



```python### Methods:

async def send_us_epa_uvindex_daily_forecast(
...
Send a single `US.EPA.UVIndex.DailyForecast` message. A daily UV Index forecast from the US EPA Envirofacts UV service
for one configured city/state location. It carries the forecast date, normalized location identity, UV index value, and
EPA UV alert indicator for public-health planning.Args:
...
await producer.send_us_epa_uvindex_daily_forecast(
...
Send multiple `US.EPA.UVIndex.DailyForecast` messages in a batch.
...
Asynchronous handler hook for `US.EPA.UVIndex.mqtt.HourlyForecast`: An hourly UV Index forecast from the US EPA
Envirofacts UV service for one configured city/state location. It carries the forecast valid hour, normalized location
identity, and UV index value for operational exposure planning.
...
us_epa_uvindex_amqp_dispatcher.us_epa_uvindex_mqtt_hourly_forecast_async = us_epa_uvindex_mqtt_hourly_forecast_event
...
Asynchronous handler hook for `US.EPA.UVIndex.mqtt.DailyForecast`: A daily UV Index forecast from the US EPA Envirofacts
UV service for one configured city/state location. It carries the forecast date, normalized location identity, UV index
value, and EPA UV alert indicator for public-health planning.
...
us_epa_uvindex_amqp_dispatcher.us_epa_uvindex_mqtt_daily_forecast_async = us_epa_uvindex_mqtt_daily_forecast_event
...
Send a single `US.EPA.UVIndex.mqtt.HourlyForecast` message. An hourly UV Index forecast from the US EPA Envirofacts UV
service for one configured city/state location. It carries the forecast valid hour, normalized location identity, and UV
index value for operational exposure planning.Args:
...
Send a single `US.EPA.UVIndex.mqtt.DailyForecast` message. A daily UV Index forecast from the US EPA Envirofacts UV
service for one configured city/state location. It carries the forecast date, normalized location identity, UV index
value, and EPA UV alert indicator for public-health planning.Args:
...
- **Scales horizontally** across multiple brokers and partitions### USEPAUVIndexAmqpEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`USEPAUVIndexAmqpEventDispatcher` handles events for the US.EPA.UVIndex.amqp message group.
...
The USEPAUVIndexAmqpEventDispatcher defines the following event handler hooks.
...
producer = USEPAUVIndexProducer(
    ...
    security_protocol='SASL_SSL',##### `us_epa_uvindex_amqp_hourly_forecast_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'us_epa_uvindex_amqp_hourly_forecast_async:  Callable[[ConsumerRecord, CloudEvent,
HourlyForecast], Awaitable[None]]
...
Asynchronous handler hook for `US.EPA.UVIndex.amqp.HourlyForecast`: An hourly UV Index forecast from the US EPA
Envirofacts UV service for one configured city/state location. It carries the forecast valid hour, normalized location
identity, and UV index value for operational exposure planning.
...
async def us_epa_uvindex_amqp_hourly_forecast_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
HourlyForecast) -> None:
...
us_epa_uvindex_dispatcher.us_epa_uvindex_amqp_hourly_forecast_async = us_epa_uvindex_amqp_hourly_forecast_event
...
us_epa_uvindex_mqtt_dispatcher.us_epa_uvindex_amqp_hourly_forecast_async = us_epa_uvindex_amqp_hourly_forecast_event
...
us_epa_uvindex_amqp_dispatcher.us_epa_uvindex_amqp_hourly_forecast_async = us_epa_uvindex_amqp_hourly_forecast_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `us_epa_uvindex_amqp_daily_forecast_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'us_epa_uvindex_amqp_daily_forecast_async:  Callable[[ConsumerRecord, CloudEvent,
DailyForecast], Awaitable[None]]
...
Asynchronous handler hook for `US.EPA.UVIndex.amqp.DailyForecast`: A daily UV Index forecast from the US EPA Envirofacts
UV service for one configured city/state location. It carries the forecast date, normalized location identity, UV index
value, and EPA UV alert indicator for public-health planning.
...
async def us_epa_uvindex_amqp_daily_forecast_event(record: ConsumerRecord, cloud_event: CloudEvent, data: DailyForecast)
-> None:
...
us_epa_uvindex_dispatcher.us_epa_uvindex_amqp_daily_forecast_async = us_epa_uvindex_amqp_daily_forecast_event
...
us_epa_uvindex_mqtt_dispatcher.us_epa_uvindex_amqp_daily_forecast_async = us_epa_uvindex_amqp_daily_forecast_event
...
us_epa_uvindex_amqp_dispatcher.us_epa_uvindex_amqp_daily_forecast_async = us_epa_uvindex_amqp_daily_forecast_event
...
##### `send_us_epa_uvindex_amqp_hourly_forecast`Dispatchers have the following protected methods:



```python### Methods:

async def send_us_epa_uvindex_amqp_hourly_forecast(
...
Send a single `US.EPA.UVIndex.amqp.HourlyForecast` message. An hourly UV Index forecast from the US EPA Envirofacts UV
service for one configured city/state location. It carries the forecast valid hour, normalized location identity, and UV
index value for operational exposure planning.Args:
...
await producer.send_us_epa_uvindex_amqp_hourly_forecast(
...
Send multiple `US.EPA.UVIndex.amqp.HourlyForecast` messages in a batch.
...
await producer.send_us_epa_uvindex_amqp_hourly_forecast_batch(```
...
##### `send_us_epa_uvindex_amqp_daily_forecast`Dispatchers have the following protected methods:



```python### Methods:

async def send_us_epa_uvindex_amqp_daily_forecast(
...
Send a single `US.EPA.UVIndex.amqp.DailyForecast` message. A daily UV Index forecast from the US EPA Envirofacts UV
service for one configured city/state location. It carries the forecast date, normalized location identity, UV index
value, and EPA UV alert indicator for public-health planning.Args:
...
await producer.send_us_epa_uvindex_amqp_daily_forecast(
...
Send multiple `US.EPA.UVIndex.amqp.DailyForecast` messages in a batch.
...
await producer.send_us_epa_uvindex_amqp_daily_forecast_batch(```
