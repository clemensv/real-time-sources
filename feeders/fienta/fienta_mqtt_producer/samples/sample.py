...
import os
import signal
from fienta_mqtt_producer_data import *
from fienta_mqtt_producer_mqtt_client.client import ComFientaMqttProducer, ComFientaMqttDispatcher

async def handle_com_fienta_mqtt_event(mqtt_msg,cloud_event, com_fienta_mqtt_event_data):
    """ Handles the Com.Fienta.mqtt.Event message """
    print(f"Received Com.Fienta.mqtt.Event on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {com_fienta_mqtt_event_data}")

async def handle_com_fienta_mqtt_event_sale_status(mqtt_msg,cloud_event, com_fienta_mqtt_event_sale_status_data):
    """ Handles the Com.Fienta.mqtt.EventSaleStatus message """
    print(f"Received Com.Fienta.mqtt.EventSaleStatus on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {com_fienta_mqtt_event_sale_status_data}")
