# Fienta_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the fienta_mqtt_producer message definitions.

# Fienta_mqtt_producer Event Dispatcher for Azure Event Hubs
...
from fienta_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = ComFientaMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### ComFientaMqttEventDispatcher



# Publish message`ComFientaMqttEventDispatcher` handles events for the Com.Fienta.mqtt message group.
...
from fienta_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = ComFientaMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe ComFientaMqttEventDispatcher defines the following event handler hooks.
...
```##### `com_fienta_mqtt_event_async`



## Test```python

com_fienta_mqtt_event_async:  Callable[[PartitionContext, EventData, CloudEvent, Event], Awaitable[None]]
...
- `data`: The event data of type `fienta_mqtt_producer_data.Event`.

Example:

```python
async def com_fienta_mqtt_event_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: Event) -> None:
...
```##### `com_fienta_mqtt_event_sale_status_async`



## Test```python

com_fienta_mqtt_event_sale_status_async:  Callable[[PartitionContext, EventData, CloudEvent, EventSaleStatus],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `fienta_mqtt_producer_data.EventSaleStatus`.

Example:

```python
async def com_fienta_mqtt_event_sale_status_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: EventSaleStatus) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
