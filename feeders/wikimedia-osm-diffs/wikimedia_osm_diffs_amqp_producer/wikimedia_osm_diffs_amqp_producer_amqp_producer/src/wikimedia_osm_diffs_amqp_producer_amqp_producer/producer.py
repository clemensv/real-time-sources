    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from wikimedia_osm_diffs_amqp_producer_data import MapChange
from wikimedia_osm_diffs_amqp_producer_data import ReplicationState

class OrgOpenStreetMapDiffsAmqpProducer:
    """
    Producer class to send messages in the `Org.OpenStreetMap.Diffs.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_map_change(self,
        ...
        """
        Send the `Org.OpenStreetMap.Diffs.amqp.MapChange` message
        ...
        attributes = {
            ...
            "subject":
            "{element_type}/{element_id}".format(element_type=_element_type, element_id=_element_id),
        }
        ...
        amqp_msg.subject = "{element_type}/{element_id}".format(element_type=_element_type, element_id=_element_id)
    ...
    def send_map_change_batch(self,
        data_array: typing.List[MapChange],
        _element_type: str,
        _element_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `Org.OpenStreetMap.Diffs.amqp.MapChange` messages
        
        Args:
            data_array (typing.List[MapChange]): Array of message data objects
            _element_type (str): Value for placeholder element_type in attribute subject
            _element_id (str): Value for placeholder element_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    def send_replication_state(self,
        data: ReplicationState,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send the `Org.OpenStreetMap.Diffs.amqp.ReplicationState` message
        ...
        if amqp_creation_time is not None:
            amqp_msg.creation_time = amqp_creation_time
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "replication_state"
    ...
    def send_replication_state_batch(self,
        data_array: typing.List[ReplicationState],
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `Org.OpenStreetMap.Diffs.amqp.ReplicationState` messages
        
        Args:
            data_array (typing.List[ReplicationState]): Array of message data objects
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type (str): The content type of the message data
        """
        for data in data_array:
            self.send_replication_state(
                data=data,
                _time=_time,
                content_type=content_type)
