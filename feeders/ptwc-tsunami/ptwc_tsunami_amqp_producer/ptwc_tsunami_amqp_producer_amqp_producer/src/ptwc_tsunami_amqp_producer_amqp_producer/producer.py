...
class PTWCBulletinsAmqpProducer:
    """
    Producer class to send messages in the `PTWC.Bulletins.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_tsunami_bulletin(self,
        ...
        """
        Send the `PTWC.Bulletins.amqp.TsunamiBulletin` message
        ...
        Args:
            ...
            _ptwc_level (str): Value for AMQP protocol option placeholder ptwc_level
        ...
        attributes = {
            ...
            "subject":
            "{bulletin_id}".format(bulletin_id=_bulletin_id),
        }
        ...
        amqp_msg.subject = "{bulletin_id}".format(bulletin_id=_bulletin_id)

        app_properties = {}
        app_properties["basin"] = "{basin}".format(basin=_basin)
        app_properties["ptwc_level"] = "{ptwc_level}".format(ptwc_level=_ptwc_level)
        if app_properties:
        ...
        annotations = {}
        annotation_value = "{bulletin_id}".format(bulletin_id=_bulletin_id)
    ...
    def send_tsunami_bulletin_batch(self,
        data_array: typing.List[TsunamiBulletin],
        _bulletin_id: str,
        _basin: str,
        _ptwc_level: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `PTWC.Bulletins.amqp.TsunamiBulletin` messages
        
        Args:
            data_array (typing.List[TsunamiBulletin]): Array of message data objects
            _bulletin_id (str): Value for placeholder bulletin_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            _basin (str): Value for AMQP protocol option placeholder basin
            _ptwc_level (str): Value for AMQP protocol option placeholder ptwc_level
        ...
        for data in data_array:
            self.send_tsunami_bulletin(
                data=data,
                _bulletin_id=_bulletin_id,
                _time=_time,
                _basin=_basin,
                _ptwc_level=_ptwc_level,
