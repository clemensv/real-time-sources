...
"""
Tests for ptwc_tsunami_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../ptwc_tsunami_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../ptwc_tsunami_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../ptwc_tsunami_amqp_producer_amqp_producer/src')))

from ptwc_tsunami_amqp_producer_amqp_producer import *
from ptwc_tsunami_amqp_producer_data import TsunamiBulletin
...
class TestPTWCBulletinsAmqpProducer:
    """Test cases for PTWCBulletinsAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(PTWCBulletinsAmqpProducer)
    ...
    def test_send_tsunami_bulletin(self, artemis_container):
        """Send and receive a TsunamiBulletin message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_TsunamiBulletin.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                assert properties.get('ptwc_level') == "{ptwc_level}".format(ptwc_level="value")
                assert annotations.get(symbol('x-opt-partition-key')) == str("{bulletin_id}".format(bulletin_id="value"))[:128]
        finally:
            producer.close()

    def test_send_tsunami_bulletin_single_fresh_connection(self, artemis_container):
        """Send exactly one TsunamiBulletin message on a fresh producer connection."""
        payload = Test_TsunamiBulletin.create_instance()
