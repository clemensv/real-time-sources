...
import os
import signal
from ptwc_tsunami_mqtt_producer_data import *
from ptwc_tsunami_mqtt_producer_mqtt_client.client import PTWCBulletinsMqttProducer, PTWCBulletinsMqttDispatcher

async def handle_ptwc_bulletins_mqtt_tsunami_bulletin(mqtt_msg,cloud_event, ptwc_bulletins_mqtt_tsunami_bulletin_data):
    """ Handles the PTWC.Bulletins.mqtt.TsunamiBulletin message """
    print(f"Received PTWC.Bulletins.mqtt.TsunamiBulletin on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {ptwc_bulletins_mqtt_tsunami_bulletin_data}")
