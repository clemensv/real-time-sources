...
def test_microsoft_opendata_us_noaa_mqtt_microsoftopendatausnoaamqttwaterlevel(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.mqtt.WaterLevel":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_mqtt_water_level(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_mqtt_microsoftopendatausnoaamqttpredictions(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.mqtt.Predictions":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_mqtt_predictions(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_mqtt_microsoftopendatausnoaamqttairpressure(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.mqtt.AirPressure":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_mqtt_air_pressure(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_mqtt_microsoftopendatausnoaamqttairtemperature(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.mqtt.AirTemperature":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_mqtt_air_temperature(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_mqtt_microsoftopendatausnoaamqttwatertemperature(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.mqtt.WaterTemperature":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_mqtt_water_temperature(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_mqtt_microsoftopendatausnoaamqttwind(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.mqtt.Wind":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_mqtt_wind(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_mqtt_microsoftopendatausnoaamqtthumidity(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.mqtt.Humidity":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_mqtt_humidity(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_mqtt_microsoftopendatausnoaamqttconductivity(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.mqtt.Conductivity":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_mqtt_conductivity(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_mqtt_microsoftopendatausnoaamqttsalinity(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.mqtt.Salinity":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_mqtt_salinity(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_mqtt_microsoftopendatausnoaamqttstation(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.mqtt.Station":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_mqtt_station(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_mqtt_microsoftopendatausnoaamqttvisibility(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.mqtt.Visibility":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_mqtt_visibility(_datacontenttype = f'test_{i}',_time = datetime.datetime.now().isoformat(),_dataschema = f'test_{i}',_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_mqtt_microsoftopendatausnoaamqttcurrents(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.mqtt.Currents":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_mqtt_currents(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_mqtt_microsoftopendatausnoaamqttcurrentpredictions(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.mqtt.CurrentPredictions":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_mqtt_current_predictions(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_amqp_microsoftopendatausnoaaamqpwaterlevel(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.amqp.WaterLevel":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_amqp_water_level(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_amqp_microsoftopendatausnoaaamqppredictions(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.amqp.Predictions":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_amqp_predictions(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_amqp_microsoftopendatausnoaaamqpairpressure(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.amqp.AirPressure":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_amqp_air_pressure(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_amqp_microsoftopendatausnoaaamqpairtemperature(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.amqp.AirTemperature":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_amqp_air_temperature(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_amqp_microsoftopendatausnoaaamqpwatertemperature(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.amqp.WaterTemperature":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_amqp_water_temperature(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_amqp_microsoftopendatausnoaaamqpwind(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.amqp.Wind":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_amqp_wind(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_amqp_microsoftopendatausnoaaamqphumidity(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.amqp.Humidity":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_amqp_humidity(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_amqp_microsoftopendatausnoaaamqpconductivity(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.amqp.Conductivity":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_amqp_conductivity(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_amqp_microsoftopendatausnoaaamqpsalinity(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.amqp.Salinity":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_amqp_salinity(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_amqp_microsoftopendatausnoaaamqpstation(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.amqp.Station":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_amqp_station(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_amqp_microsoftopendatausnoaaamqpvisibility(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.amqp.Visibility":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_amqp_visibility(_datacontenttype = f'test_{i}',_time = datetime.datetime.now().isoformat(),_dataschema = f'test_{i}',_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_amqp_microsoftopendatausnoaaamqpcurrents(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.amqp.Currents":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_amqp_currents(_station_id = f'test_{i}', data = event_data)
...
def test_microsoft_opendata_us_noaa_amqp_microsoftopendatausnoaaamqpcurrentpredictions(kafka_emulator):
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "Microsoft.OpenData.US.NOAA.amqp.CurrentPredictions":
                return msg.key().decode('utf-8') if msg.key() else None
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_microsoft_open_data_us_noaa_amqp_current_predictions(_station_id = f'test_{i}', data = event_data)
