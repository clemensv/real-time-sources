# Noaa_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the noaa_mqtt_producer message definitions.

# Noaa_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - MicrosoftOpenDataUSNOAAMqttEventDispatcher
...
from noaa_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = MicrosoftOpenDataUSNOAAMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### MicrosoftOpenDataUSNOAAMqttEventDispatcher



# Publish message`MicrosoftOpenDataUSNOAAMqttEventDispatcher` handles events for the Microsoft.OpenData.US.NOAA.mqtt
message group.
...
from noaa_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = MicrosoftOpenDataUSNOAAMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe MicrosoftOpenDataUSNOAAMqttEventDispatcher defines the following event handler hooks.
...
```##### `microsoft_open_data_us_noaa_mqtt_water_level_async`



## Test```python

microsoft_open_data_us_noaa_mqtt_water_level_async:  Callable[[PartitionContext, EventData, CloudEvent, WaterLevel],
Awaitable[None]]
...
- `data`: The event data of type `noaa_mqtt_producer_data.WaterLevel`.

Example:

```python
async def microsoft_open_data_us_noaa_mqtt_water_level_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: WaterLevel) -> None:
...
```##### `microsoft_open_data_us_noaa_mqtt_predictions_async`



## Test```python

microsoft_open_data_us_noaa_mqtt_predictions_async:  Callable[[PartitionContext, EventData, CloudEvent, Predictions],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `noaa_mqtt_producer_data.Predictions`.

Example:

```python
async def microsoft_open_data_us_noaa_mqtt_predictions_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: Predictions) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `microsoft_open_data_us_noaa_mqtt_air_pressure_async`



## Test```python

microsoft_open_data_us_noaa_mqtt_air_pressure_async:  Callable[[PartitionContext, EventData, CloudEvent, AirPressure],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `noaa_mqtt_producer_data.AirPressure`.

Example:

```python
async def microsoft_open_data_us_noaa_mqtt_air_pressure_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: AirPressure) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `microsoft_open_data_us_noaa_mqtt_air_temperature_async`



## Test```python

microsoft_open_data_us_noaa_mqtt_air_temperature_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `noaa_mqtt_producer_data.AirTemperature`.

Example:

```python
async def microsoft_open_data_us_noaa_mqtt_air_temperature_event(partition_context: PartitionContext, event: EventData,
...
```##### `microsoft_open_data_us_noaa_mqtt_water_temperature_async`



## Test```python

microsoft_open_data_us_noaa_mqtt_water_temperature_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `noaa_mqtt_producer_data.WaterTemperature`.

Example:

```python
async def microsoft_open_data_us_noaa_mqtt_water_temperature_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: WaterTemperature) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `microsoft_open_data_us_noaa_mqtt_wind_async`



## Test```python

microsoft_open_data_us_noaa_mqtt_wind_async:  Callable[[PartitionContext, EventData, CloudEvent, Wind], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `noaa_mqtt_producer_data.Wind`.

Example:

```python
async def microsoft_open_data_us_noaa_mqtt_wind_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: Wind) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `microsoft_open_data_us_noaa_mqtt_humidity_async`



## Test```python

microsoft_open_data_us_noaa_mqtt_humidity_async:  Callable[[PartitionContext, EventData, CloudEvent, Humidity],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `noaa_mqtt_producer_data.Humidity`.

Example:

```python
async def microsoft_open_data_us_noaa_mqtt_humidity_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: Humidity) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `microsoft_open_data_us_noaa_mqtt_conductivity_async`



## Test```python

microsoft_open_data_us_noaa_mqtt_conductivity_async:  Callable[[PartitionContext, EventData, CloudEvent, Conductivity],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `noaa_mqtt_producer_data.Conductivity`.

Example:

```python
async def microsoft_open_data_us_noaa_mqtt_conductivity_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: Conductivity) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `microsoft_open_data_us_noaa_mqtt_salinity_async`



## Test```python

microsoft_open_data_us_noaa_mqtt_salinity_async:  Callable[[PartitionContext, EventData, CloudEvent, Salinity],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `noaa_mqtt_producer_data.Salinity`.

Example:

```python
async def microsoft_open_data_us_noaa_mqtt_salinity_event(partition_context: PartitionContext, event: EventData,
...
```##### `microsoft_open_data_us_noaa_mqtt_station_async`



## Test```python

microsoft_open_data_us_noaa_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `noaa_mqtt_producer_data.Station`.

Example:

```python
async def microsoft_open_data_us_noaa_mqtt_station_event(partition_context: PartitionContext, event: EventData,
...
```##### `microsoft_open_data_us_noaa_mqtt_visibility_async`



## Test```python

microsoft_open_data_us_noaa_mqtt_visibility_async:  Callable[[PartitionContext, EventData, CloudEvent, Visibility],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `noaa_mqtt_producer_data.Visibility`.

Example:

```python
async def microsoft_open_data_us_noaa_mqtt_visibility_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: Visibility) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `microsoft_open_data_us_noaa_mqtt_currents_async`



## Test```python

microsoft_open_data_us_noaa_mqtt_currents_async:  Callable[[PartitionContext, EventData, CloudEvent, Currents],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `noaa_mqtt_producer_data.Currents`.

Example:

```python
async def microsoft_open_data_us_noaa_mqtt_currents_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: Currents) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `microsoft_open_data_us_noaa_mqtt_current_predictions_async`



## Test```python

microsoft_open_data_us_noaa_mqtt_current_predictions_async:  Callable[[PartitionContext, EventData, CloudEvent,
CurrentPredictions], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `noaa_mqtt_producer_data.CurrentPredictions`.

Example:

```python
async def microsoft_open_data_us_noaa_mqtt_current_predictions_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: CurrentPredictions) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
