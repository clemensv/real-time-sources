...
class GovCaWaterCdecAmqpProducer:
    """
    Producer class to send messages in the `gov.ca.water.cdec.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_reservoir_reading(self,
        ...
        """
        Send the `gov.ca.water.cdec.amqp.ReservoirReading` message
        A current reservoir record from the California Data Exchange Center (CDEC). It reports the latest storage, elevation, capacity, or related reservoir status available for one reservoir.
        ...
        attributes = {
            ...
            "subject":
            "{station_id}/{sensor_num}".format(station_id=_station_id, sensor_num=_sensor_num),
        }
        ...
        amqp_msg.subject = "{station_id}/{sensor_num}".format(station_id=_station_id, sensor_num=_sensor_num)

        app_properties = {}
        app_properties["basin"] = "{basin}".format(basin=_basin)
        app_properties["sensor_num"] = "{sensor_num}".format(sensor_num=_sensor_num)
        if app_properties:
        ...
        annotations = {}
        annotation_value = "{station_id}/{sensor_num}".format(station_id=_station_id, sensor_num=_sensor_num)
    ...
    def send_reservoir_reading_batch(self,
        data_array: typing.List[ReservoirReading],
        _feedurl: str,
        _station_id: str,
        _sensor_num: str,
        _basin: str,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `gov.ca.water.cdec.amqp.ReservoirReading` messages
        
        Args:
            data_array (typing.List[ReservoirReading]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _station_id (str): Value for placeholder station_id in attribute subject
            _sensor_num (str): Value for placeholder sensor_num in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        for data in data_array:
            self.send_reservoir_reading(
                data=data,
                _feedurl=_feedurl,
                _station_id=_station_id,
                _sensor_num=_sensor_num,
                _time=_time,
                _basin=_basin,
