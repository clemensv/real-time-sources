...
"""
Tests for cdec_reservoirs_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../cdec_reservoirs_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../cdec_reservoirs_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../cdec_reservoirs_amqp_producer_amqp_producer/src')))

from cdec_reservoirs_amqp_producer_amqp_producer import *
...
class TestGovCaWaterCdecAmqpProducer:
    """Test cases for GovCaWaterCdecAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(GovCaWaterCdecAmqpProducer)
    ...
    def test_send_reservoir_reading(self, artemis_container):
        """Send and receive a ReservoirReading message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_ReservoirReading.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                assert properties.get('sensor_num') == "{sensor_num}".format(sensor_num="value")
                assert annotations.get(symbol('x-opt-partition-key')) == str("{station_id}/{sensor_num}".format(station_id="value", sensor_num="value"))[:128]
        finally:
            producer.close()

    def test_send_reservoir_reading_single_fresh_connection(self, artemis_container):
        """Send exactly one ReservoirReading message on a fresh producer connection."""
        payload = Test_ReservoirReading.create_instance()
