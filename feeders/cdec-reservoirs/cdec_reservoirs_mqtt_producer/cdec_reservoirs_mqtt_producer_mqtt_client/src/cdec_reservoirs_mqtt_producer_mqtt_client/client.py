...
def get_default_topic_mappings_gov_ca_water_cdec_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the gov.ca.water.cdec.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "gov.ca.water.cdec.mqtt.ReservoirReading": "hydro/us/cdec/cdec-reservoirs/{basin}/{station_id}/{sensor_num}/reservoir-reading",
    }


def get_subscription_topics_gov_ca_water_cdec_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_gov_ca_water_cdec_mqtt()
    topics = []
...
class GovCaWaterCdecMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the gov.ca.water.cdec.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_gov_ca_water_cdec_mqtt()
        ...
        self.gov_ca_water_cdec_mqtt_reservoir_reading_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, cdec_reservoirs_mqtt_producer_data.ReservoirReading, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "gov.ca.water.cdec.ReservoirReading":
            if self.gov_ca_water_cdec_mqtt_reservoir_reading_async:
                try:
                    ...
                    data = cdec_reservoirs_mqtt_producer_data.ReservoirReading.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "gov.ca.water.cdec.mqtt.ReservoirReading")
                    await self.gov_ca_water_cdec_mqtt_reservoir_reading_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in gov_ca_water_cdec_mqtt_reservoir_reading handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_gov_ca_water_cdec_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_gov_ca_water_cdec_mqtt_reservoir_reading(self,
        feedurl: str,
        station_id: str,
        sensor_num: str,
        basin: str,
        data: cdec_reservoirs_mqtt_producer_data.ReservoirReading,
        ...
        """
        Publish the 'gov.ca.water.cdec.mqtt.ReservoirReading' event to an MQTT topic.

        Args:
        
            feedurl: URI template variable for 'feedurl'
            station_id: URI template variable for 'station_id'
            sensor_num: URI template variable for 'sensor_num'
            basin: URI template variable for 'basin'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/us/cdec/cdec-reservoirs/{basin}/{station_id}/{sensor_num}/reservoir-reading'
        ...
        """
        target_topic = topic if topic is not None else "hydro/us/cdec/cdec-reservoirs/{basin}/{station_id}/{sensor_num}/reservoir-reading"
        ...
        attributes = {
             ...
             "subject":"{station_id}/{sensor_num}".format(station_id = station_id,sensor_num = sensor_num)
        }
        attributes["datacontenttype"] = content_type
