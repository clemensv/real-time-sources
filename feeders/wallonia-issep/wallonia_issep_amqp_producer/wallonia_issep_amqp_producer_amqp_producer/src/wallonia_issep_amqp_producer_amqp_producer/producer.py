...
class BeIssepAirqualitySensorsAmqpProducer:
    """
    Producer class to send messages in the `be.issep.airquality.Sensors.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_sensor_configuration(self,
        ...
        """
        Send the `be.issep.airquality.Sensors.amqp.SensorConfiguration` message
        ...
        attributes = {
            ...
            "source":
            "https://www.odwb.be/api/explore/v2.1/catalog/datasets/last-data-capteurs-qualite-de-l-air-issep/records",
            "subject":
            "{configuration_id}".format(configuration_id=_configuration_id),
        }
        ...
        amqp_msg.subject = "{configuration_id}".format(configuration_id=_configuration_id)
    ...
    def send_sensor_configuration_batch(self,
        data_array: typing.List[SensorConfiguration],
        _configuration_id: str,
        _province: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `be.issep.airquality.Sensors.amqp.SensorConfiguration` messages
        
        Args:
            data_array (typing.List[SensorConfiguration]): Array of message data objects
            _configuration_id (str): Value for placeholder configuration_id in attribute subject
            _province (str): Value for AMQP protocol option placeholder province
            content_type (str): The content type of the message data
        """
        for data in data_array:
            self.send_sensor_configuration(
                data=data,
                _configuration_id=_configuration_id,
                _province=_province,
    ...
    def send_observation(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `be.issep.airquality.Sensors.amqp.Observation` message
        A current environmental measurement from Wallonia's Institut Scientifique de Service Public (ISSeP). It carries pollutant concentration measurements when the upstream feed reports a new or refreshed value.
    ...
    def send_observation_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `be.issep.airquality.Sensors.amqp.Observation` messages
