    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from xceed_amqp_producer_data import Event
from xceed_amqp_producer_data import EventAdmission

class XceedAmqpProducer:
    """
    Producer class to send messages in the `xceed.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_event(self,
        ...
        """
        Send the `xceed.amqp.Event` message
    ...
    def send_event_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `xceed.amqp.Event` messages
...
class XceedAdmissionsAmqpProducer:
    """
    Producer class to send messages in the `xceed.admissions.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_event_admission(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `xceed.admissions.amqp.EventAdmission` message
        ...
        attributes = {
            ...
            "{feedurl}".format(feedurl=_feedurl),
            "subject":
            "{event_id}/{admission_id}".format(event_id=_event_id, admission_id=_admission_id),
        }
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        ...
        if amqp_creation_time is not None:
            amqp_msg.creation_time = amqp_creation_time
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "{event_id}/{admission_id}".format(event_id=_event_id, admission_id=_admission_id)
        ...
        annotations = {}
        annotation_value = "{event_id}/{admission_id}".format(event_id=_event_id, admission_id=_admission_id)
        annotation_value = str(annotation_value)[:128]
        annotations[symbol("x-opt-partition-key")] = annotation_value
        if annotations:
            if amqp_msg.annotations is None:
    ...
    def send_event_admission_batch(self,
        data_array: typing.List[EventAdmission],
        _feedurl: str,
        _event_id: str,
        _admission_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `xceed.admissions.amqp.EventAdmission` messages
        
        Args:
            data_array (typing.List[EventAdmission]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _event_id (str): Value for placeholder event_id in attribute subject
            _admission_id (str): Value for placeholder admission_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        """
        for data in data_array:
            self.send_event_admission(
                data=data,
                _feedurl=_feedurl,
                _event_id=_event_id,
                _admission_id=_admission_id,
                _time=_time,
                content_type=content_type)
