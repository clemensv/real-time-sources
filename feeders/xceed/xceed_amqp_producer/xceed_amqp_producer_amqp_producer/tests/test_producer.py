...
"""
Tests for xceed_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../xceed_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../xceed_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../xceed_amqp_producer_amqp_producer/src')))

from xceed_amqp_producer_amqp_producer import *
...
class TestXceedAmqpProducer:
    """Test cases for XceedAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(XceedAmqpProducer)
...
class TestXceedAdmissionsAmqpProducer:
    """Test cases for XceedAdmissionsAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        fake_connection = FakeConnection()
        producer = object.__new__(XceedAdmissionsAmqpProducer)
        producer._sender = fake_sender
        producer._connection = fake_connection
        producer._blocking_sender_is_presettled = True
    ...
    def test_send_event_admission(self, artemis_container):
        """Send and receive a EventAdmission message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_EventAdmission.create_instance()
    ...
    def test_send_event_admission_single_fresh_connection(self, artemis_container):
        """Send exactly one EventAdmission message on a fresh producer connection."""
        payload = Test_EventAdmission.create_instance()
