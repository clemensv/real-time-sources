...
def get_default_topic_mappings_xceed_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the xceed.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "xceed.mqtt.Event": "civic-events/intl/xceed/xceed/{city}/{event_id}/event",
    }


def get_subscription_topics_xceed_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_xceed_mqtt()
    topics = []
...
class XceedMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the xceed.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_xceed_mqtt()
        ...
        self.xceed_mqtt_event_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, xceed_mqtt_producer_data.Event, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "xceed.Event":
            if self.xceed_mqtt_event_async:
                try:
                    ...
                    data = xceed_mqtt_producer_data.Event.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "xceed.mqtt.Event")
                    await self.xceed_mqtt_event_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in xceed_mqtt_event handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_xceed_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_xceed_mqtt_event(self,
        feedurl: str,
        event_id: str,
        city: str,
        data: xceed_mqtt_producer_data.Event,
        ...
        """
        Publish the 'xceed.mqtt.Event' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'civic-events/intl/xceed/xceed/{city}/{event_id}/event'
        ...
        """
        target_topic = topic if topic is not None else "civic-events/intl/xceed/xceed/{city}/{event_id}/event"
...
def get_default_topic_mappings_xceed_admissions_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the xceed.admissions.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "xceed.admissions.mqtt.EventAdmission": "civic-events/intl/xceed/xceed/admissions/{event_id}/{admission_id}/admission",
    }


def get_subscription_topics_xceed_admissions_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_xceed_admissions_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class XceedAdmissionsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the xceed.admissions.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_xceed_admissions_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        
        # Message handler callbacks (Dispatcher pattern)
        
        self.xceed_admissions_mqtt_event_admission_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, xceed_mqtt_producer_data.EventAdmission, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "xceed.EventAdmission":
            if self.xceed_admissions_mqtt_event_admission_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = xceed_mqtt_producer_data.EventAdmission.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "xceed.admissions.mqtt.EventAdmission")
                    await self.xceed_admissions_mqtt_event_admission_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in xceed_admissions_mqtt_event_admission handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_xceed_admissions_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_xceed_admissions_mqtt_event_admission(self,
        feedurl: str,
        event_id: str,
        admission_id: str,
        data: xceed_mqtt_producer_data.EventAdmission,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'xceed.admissions.mqtt.EventAdmission' event to an MQTT topic.

        Args:
        
            feedurl: URI template variable for 'feedurl'
            event_id: URI template variable for 'event_id'
            admission_id: URI template variable for 'admission_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'civic-events/intl/xceed/xceed/admissions/{event_id}/{admission_id}/admission'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "civic-events/intl/xceed/xceed/admissions/{event_id}/{admission_id}/admission"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
            "event_id": str(event_id),
        ...
        attributes = {
             ...
             "source":"{feedurl}".format(feedurl = feedurl),
             "subject":"{event_id}/{admission_id}".format(event_id = event_id,admission_id = admission_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
