...
import os
import signal
from xceed_mqtt_producer_data import *
from xceed_mqtt_producer_mqtt_client.client import XceedMqttProducer, XceedMqttDispatcher

async def handle_xceed_mqtt_event(mqtt_msg,cloud_event, xceed_mqtt_event_data):
    """ Handles the xceed.mqtt.Event message """
    print(f"Received xceed.mqtt.Event on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {xceed_mqtt_event_data}")
from xceed_mqtt_producer_mqtt_client.client import XceedAdmissionsMqttProducer, XceedAdmissionsMqttDispatcher

async def handle_xceed_admissions_mqtt_event_admission(mqtt_msg,cloud_event, xceed_admissions_mqtt_event_admission_data):
    """ Handles the xceed.admissions.mqtt.EventAdmission message """
    print(f"Received xceed.admissions.mqtt.EventAdmission on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {xceed_admissions_mqtt_event_admission_data}")
