...
def get_default_topic_mappings_ticketmaster_events_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the Ticketmaster.Events.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "Ticketmaster.Events.mqtt.Event": "civic-events/intl/ticketmaster/ticketmaster/{country}/{city}/{venue_id}/{event_id}/event",
    }


def get_subscription_topics_ticketmaster_events_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_ticketmaster_events_mqtt()
    topics = []
...
class TicketmasterEventsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the Ticketmaster.Events.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_ticketmaster_events_mqtt()
        ...
        self.ticketmaster_events_mqtt_event_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, ticketmaster_mqtt_producer_data.Event, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "Ticketmaster.Events.Event":
            if self.ticketmaster_events_mqtt_event_async:
                try:
                    ...
                    data = ticketmaster_mqtt_producer_data.Event.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Ticketmaster.Events.mqtt.Event")
                    await self.ticketmaster_events_mqtt_event_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in ticketmaster_events_mqtt_event handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_ticketmaster_events_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_ticketmaster_events_mqtt_event(self,
        ...
        city: str,
        venue_id: str,
        data: ticketmaster_mqtt_producer_data.Event,
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'civic-events/intl/ticketmaster/ticketmaster/{country}/{city}/{venue_id}/{event_id}/event'
...
def get_default_topic_mappings_ticketmaster_reference_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the Ticketmaster.Reference.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "Ticketmaster.Reference.mqtt.Venue": "civic-events/intl/ticketmaster/ticketmaster/{country}/{city}/{venue_id}/venue",
        "Ticketmaster.Reference.mqtt.Attraction": "civic-events/intl/ticketmaster/ticketmaster/{country}/{city}/{segment}/{entity_id}/attraction",
        "Ticketmaster.Reference.mqtt.Classification": "civic-events/intl/ticketmaster/ticketmaster/{country}/{city}/{segment}/{entity_id}/classification",
        "Ticketmaster.Reference.mqtt.Info": "civic-events/intl/ticketmaster/ticketmaster/{country}/{city}/{segment}/{entity_id}/info",
    }


def get_subscription_topics_ticketmaster_reference_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_ticketmaster_reference_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class TicketmasterReferenceMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the Ticketmaster.Reference.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_ticketmaster_reference_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.ticketmaster_reference_mqtt_venue_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, ticketmaster_mqtt_producer_data.Venue, Dict[str, str]], Awaitable[None]]] = None
        
        self.ticketmaster_reference_mqtt_attraction_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, ticketmaster_mqtt_producer_data.Attraction, Dict[str, str]], Awaitable[None]]] = None
        
        self.ticketmaster_reference_mqtt_classification_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, ticketmaster_mqtt_producer_data.Classification, Dict[str, str]], Awaitable[None]]] = None
        
        self.ticketmaster_reference_mqtt_info_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, ticketmaster_mqtt_producer_data.Info, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "Ticketmaster.Reference.Venue":
            if self.ticketmaster_reference_mqtt_venue_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = ticketmaster_mqtt_producer_data.Venue.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Ticketmaster.Reference.mqtt.Venue")
                    await self.ticketmaster_reference_mqtt_venue_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in ticketmaster_reference_mqtt_venue handler: {e}")
            return
        
        if event_type == "Ticketmaster.Reference.Attraction":
            if self.ticketmaster_reference_mqtt_attraction_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = ticketmaster_mqtt_producer_data.Attraction.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Ticketmaster.Reference.mqtt.Attraction")
                    await self.ticketmaster_reference_mqtt_attraction_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in ticketmaster_reference_mqtt_attraction handler: {e}")
            return
        
        if event_type == "Ticketmaster.Reference.Classification":
            if self.ticketmaster_reference_mqtt_classification_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = ticketmaster_mqtt_producer_data.Classification.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Ticketmaster.Reference.mqtt.Classification")
                    await self.ticketmaster_reference_mqtt_classification_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in ticketmaster_reference_mqtt_classification handler: {e}")
            return
        
        if event_type == "Ticketmaster.Reference.Info":
            if self.ticketmaster_reference_mqtt_info_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = ticketmaster_mqtt_producer_data.Info.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Ticketmaster.Reference.mqtt.Info")
                    await self.ticketmaster_reference_mqtt_info_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in ticketmaster_reference_mqtt_info handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_ticketmaster_reference_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_ticketmaster_reference_mqtt_venue(self,
        entity_id: str,
        country: str,
        city: str,
        venue_id: str,
        data: ticketmaster_mqtt_producer_data.Venue,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            entity_id: URI template variable for 'entity_id'
            country: URI template variable for 'country'
            city: URI template variable for 'city'
            venue_id: URI template variable for 'venue_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'civic-events/intl/ticketmaster/ticketmaster/{country}/{city}/{venue_id}/venue'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        attributes = {
             "type":"Ticketmaster.Reference.Venue",
             "source":"https://app.ticketmaster.com/discovery/v2/venues",
    ...
    async def publish_ticketmaster_reference_mqtt_attraction(self,
        entity_id: str,
        country: str,
        city: str,
        segment: str,
        data: ticketmaster_mqtt_producer_data.Attraction,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            country: URI template variable for 'country'
            city: URI template variable for 'city'
            segment: URI template variable for 'segment'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'civic-events/intl/ticketmaster/ticketmaster/{country}/{city}/{segment}/{entity_id}/attraction'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        attributes = {
             "type":"Ticketmaster.Reference.Attraction",
             "source":"https://app.ticketmaster.com/discovery/v2/attractions",
             "subject":"{entity_id}".format(entity_id = entity_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
    ...
    async def publish_ticketmaster_reference_mqtt_classification(self,
        entity_id: str,
        country: str,
        city: str,
        segment: str,
        data: ticketmaster_mqtt_producer_data.Classification,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            segment: URI template variable for 'segment'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'civic-events/intl/ticketmaster/ticketmaster/{country}/{city}/{segment}/{entity_id}/classification'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        attributes = {
             "type":"Ticketmaster.Reference.Classification",
             "source":"https://app.ticketmaster.com/discovery/v2/classifications",
             "subject":"{entity_id}".format(entity_id = entity_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
    ...
    async def publish_ticketmaster_reference_mqtt_info(self,
        entity_id: str,
        country: str,
        city: str,
        segment: str,
        data: ticketmaster_mqtt_producer_data.Info,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            segment: URI template variable for 'segment'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'civic-events/intl/ticketmaster/ticketmaster/{country}/{city}/{segment}/{entity_id}/info'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        attributes = {
             "type":"Ticketmaster.Reference.Info",
