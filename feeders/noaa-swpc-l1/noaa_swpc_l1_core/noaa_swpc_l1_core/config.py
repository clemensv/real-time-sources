"""Shared configuration models and Kafka connection-string parsers.

All three transport feeders accept the same upstream-side knobs:
polling interval, dedup state file, single-shot ``--once`` mode,
spacecraft override. Those live on :class:`FeedConfig`.
...
the Docker E2E harness. Parsing those is shared because MQTT and AMQP have
their own URL conventions.
"""
    ...
    def from_env(
        ...
        *,
        ...
        if spacecraft is None:
            spacecraft = os.getenv("SPACECRAFT", "dscovr").lower()
        if backfill_minutes is None:
            backfill_minutes = int(os.getenv("BACKFILL_MINUTES", "5"))
        return cls(
