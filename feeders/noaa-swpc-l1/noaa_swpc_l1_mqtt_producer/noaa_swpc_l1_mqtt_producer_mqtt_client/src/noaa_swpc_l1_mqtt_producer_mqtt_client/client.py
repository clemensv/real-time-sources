...
def get_default_topic_mappings_gov_noaa_swpc_l1_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the gov.noaa.swpc.l1.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "gov.noaa.swpc.l1.mqtt.PropagatedSolarWind": "space-weather/us/noaa-swpc/l1/{spacecraft}/propagated-solar-wind",
    }


def get_subscription_topics_gov_noaa_swpc_l1_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_gov_noaa_swpc_l1_mqtt()
    topics = []
...
class GovNoaaSwpcL1MqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the gov.noaa.swpc.l1.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_gov_noaa_swpc_l1_mqtt()
        ...
        self.gov_noaa_swpc_l1_mqtt_propagated_solar_wind_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, noaa_swpc_l1_mqtt_producer_data.PropagatedSolarWind, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "gov.noaa.swpc.l1.PropagatedSolarWind":
            if self.gov_noaa_swpc_l1_mqtt_propagated_solar_wind_async:
                try:
                    ...
                    data = noaa_swpc_l1_mqtt_producer_data.PropagatedSolarWind.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "gov.noaa.swpc.l1.mqtt.PropagatedSolarWind")
                    await self.gov_noaa_swpc_l1_mqtt_propagated_solar_wind_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in gov_noaa_swpc_l1_mqtt_propagated_solar_wind handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_gov_noaa_swpc_l1_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_gov_noaa_swpc_l1_mqtt_propagated_solar_wind(self,
        feedurl: str,
        spacecraft: str,
        time_tag: str,
        data: noaa_swpc_l1_mqtt_producer_data.PropagatedSolarWind,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'gov.noaa.swpc.l1.mqtt.PropagatedSolarWind' event to an MQTT topic.

        Args:
        
            feedurl: URI template variable for 'feedurl'
            spacecraft: URI template variable for 'spacecraft'
            time_tag: URI template variable for 'time_tag'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'space-weather/us/noaa-swpc/l1/{spacecraft}/propagated-solar-wind'
        ...
        """
        target_topic = topic if topic is not None else "space-weather/us/noaa-swpc/l1/{spacecraft}/propagated-solar-wind"
        ...
        attributes = {
             ...
             "time":"{time_tag}".format(time_tag = time_tag)
        }
        attributes["datacontenttype"] = content_type
