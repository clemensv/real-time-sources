# Noaa_swpc_l1_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the noaa_swpc_l1_mqtt_producer message definitions.

# Noaa_swpc_l1_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - GovNoaaSwpcL1MqttEventDispatcher
...
from noaa_swpc_l1_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = GovNoaaSwpcL1MqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### GovNoaaSwpcL1MqttEventDispatcher



# Publish message`GovNoaaSwpcL1MqttEventDispatcher` handles events for the gov.noaa.swpc.l1.mqtt message group.
...
from noaa_swpc_l1_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = GovNoaaSwpcL1MqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe GovNoaaSwpcL1MqttEventDispatcher defines the following event handler hooks.
...
```##### `gov_noaa_swpc_l1_mqtt_propagated_solar_wind_async`



## Test```python

gov_noaa_swpc_l1_mqtt_propagated_solar_wind_async:  Callable[[PartitionContext, EventData, CloudEvent,
PropagatedSolarWind], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `gov.noaa.swpc.l1.mqtt.PropagatedSolarWind`: One minute-resolution solar-wind
observation at L1 with forward-propagated Earth-arrival timestamp. Fuses plasma (speed/density/temperature), magnetic
field (Bx/By/Bz/Bt in GSM), and bulk velocity vector (Vx/Vy/Vz in GSM) into a single row per `time_tag`. The
...
- `data`: The event data of type `noaa_swpc_l1_mqtt_producer_data.PropagatedSolarWind`.

Example:

```python
async def gov_noaa_swpc_l1_mqtt_propagated_solar_wind_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: PropagatedSolarWind) -> None:
...
```python
gov_noaa_swpc_l1_mqtt_dispatcher.gov_noaa_swpc_l1_mqtt_propagated_solar_wind_async =
gov_noaa_swpc_l1_mqtt_propagated_solar_wind_event
```
