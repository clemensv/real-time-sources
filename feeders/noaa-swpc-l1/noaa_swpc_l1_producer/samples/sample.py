...
async def main(connection_string: Optional[str], producer_config: Optional[str], topic: Optional[str]):
    ...
    if connection_string:
        ...
        gov_noaa_swpc_l1_kafka_event_producer = GovNoaaSwpcL1KafkaEventProducer.from_connection_string(connection_string, topic, 'binary')
    else:
        # use a Kafka producer configuration provided as JSON text
        kafka_producer = KafkaProducer(json.loads(producer_config))
        gov_noaa_swpc_l1_kafka_event_producer = GovNoaaSwpcL1KafkaEventProducer(kafka_producer, topic, 'binary')

    # ---- gov.noaa.swpc.l1.kafka.PropagatedSolarWind ----
    # TODO: Supply event data for the gov.noaa.swpc.l1.kafka.PropagatedSolarWind event
