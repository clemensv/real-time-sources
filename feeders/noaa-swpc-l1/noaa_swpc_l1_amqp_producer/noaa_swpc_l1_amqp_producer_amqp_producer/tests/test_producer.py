...
"""
Tests for noaa_swpc_l1_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../noaa_swpc_l1_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../noaa_swpc_l1_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../noaa_swpc_l1_amqp_producer_amqp_producer/src')))

from noaa_swpc_l1_amqp_producer_amqp_producer import *
...
class TestGovNoaaSwpcL1AmqpProducer:
    """Test cases for GovNoaaSwpcL1AmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(GovNoaaSwpcL1AmqpProducer)
    ...
    def test_send_propagated_solar_wind(self, artemis_container):
        """Send and receive a PropagatedSolarWind message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_PropagatedSolarWind.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "gov.noaa.swpc.l1.amqp.PropagatedSolarWind"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{spacecraft}".format(spacecraft="value")
                assert annotations.get(symbol('x-opt-partition-key')) == str("{spacecraft}".format(spacecraft="value"))[:128]
        finally:
            producer.close()

    def test_send_propagated_solar_wind_single_fresh_connection(self, artemis_container):
        """Send exactly one PropagatedSolarWind message on a fresh producer connection."""
        payload = Test_PropagatedSolarWind.create_instance()
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'gov.noaa.swpc.l1.amqp.PropagatedSolarWind'
        assert received.body is not None
        assert received.subject == "{spacecraft}".format(spacecraft="value")
        assert annotations.get(symbol('x-opt-partition-key')) == str("{spacecraft}".format(spacecraft="value"))[:128]
