    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from noaa_swpc_l1_amqp_producer_data import PropagatedSolarWind

class GovNoaaSwpcL1AmqpProducer:
    """
    Producer class to send messages in the `gov.noaa.swpc.l1.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_propagated_solar_wind(self,
        ...
        """
        Send the `gov.noaa.swpc.l1.amqp.PropagatedSolarWind` message
        One minute-resolution solar-wind observation at L1 with forward-propagated Earth-arrival timestamp. Fuses plasma (speed/density/temperature), magnetic field (Bx/By/Bz/Bt in GSM), and bulk velocity vector (Vx/Vy/Vz in GSM) into a single row per `time_tag`. The `propagated_time_tag` field is the upstream's predicted Earth arrival time for the solar-wind parcel observed at L1 at `time_tag`. The CloudEvent `time` attribute mirrors `time_tag` (the L1 observation time) — consumers that need the Earth-arrival time must read `data.propagated_time_tag` instead. Many sub-fields are nullable — instrument gaps on DSCOVR/ACE are routine, especially for the velocity components which DSCOVR does not measure reliably. Sourced from `GET https://services.swpc.noaa.gov/products/geospace/propagated-solar-wind.json` — the document is a 7-day rolling window (~10080 rows) of array-of-arrays form with the column header at index 0.
        ...
        attributes = {
            "type":
            "gov.noaa.swpc.l1.PropagatedSolarWind",
            "source":
        ...
        amqp_msg.subject = "{spacecraft}".format(spacecraft=_spacecraft)
        ...
        annotations = {}
        annotation_value = "{spacecraft}".format(spacecraft=_spacecraft)
    ...
    def send_propagated_solar_wind_batch(self,
        data_array: typing.List[PropagatedSolarWind],
        _feedurl: str,
        _spacecraft: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `gov.noaa.swpc.l1.amqp.PropagatedSolarWind` messages
        
        Args:
            data_array (typing.List[PropagatedSolarWind]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _spacecraft (str): Value for placeholder spacecraft in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        for data in data_array:
            self.send_propagated_solar_wind(
                data=data,
                _feedurl=_feedurl,
                _spacecraft=_spacecraft,
                _time=_time,
