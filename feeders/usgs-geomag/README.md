...
<sub>14 observatories, 1-min geomagnetic field · Kafka · MQTT · AMQP · <a href="https://www.usgs.gov/programs/geomagnetism">upstream</a> · <a href="https://geomag.usgs.gov/ws/">API docs</a></sub>
...
[🚀 **Deploy to Azure**](https://clemensv.github.io/real-time-sources#usgs-geomag) &nbsp;·&nbsp;
...
This feeder turns USGS Geomagnetism data into a real-time CloudEvents stream over Apache Kafka, MQTT 5.0 (Unified Namespace), and AMQP 1.0.
...
**USGS Geomag** ships three transport variants from one source bridge:
...
usgs-geomag/
  ...
  usgs_geomag_mqtt/
  usgs_geomag_amqp/
  usgs_geomag_producer/
  usgs_geomag_mqtt_producer/
  usgs_geomag_amqp_producer/
  Dockerfile
  Dockerfile.mqtt
  Dockerfile.amqp
  notebook/
...
docker run --rm \
  -v "$PWD/state:/state" \
  -e GEOMAG_LAST_POLLED_FILE=/state/usgs-geomag.json \
...
docker run --rm \
  -v "$PWD/state:/state" \
  -e GEOMAG_LAST_POLLED_FILE=/state/usgs-geomag.json \
  -e AMQP_BROKER_URL="amqp://<user>:<password>@<broker-host>:5672/usgs-geomag" \
  ghcr.io/clemensv/real-time-sources-usgs-geomag-amqp:latest
```
...
USGS Geomagnetism observatory data APIs.
...
USGS Geomagnetism targets Microsoft Fabric end-to-end: events land in a Fabric **Event Stream** (custom endpoint), an attached **Eventhouse / KQL database** materializes the contract from [`kql/`](kql/).

Two hosting models are supported. Use the deploy buttons on the [project portal](https://clemensv.github.io/real-time-sources#usgs-geomag) to launch either — both walk you through the same Fabric workspace selection and follow-up steps.

#### Fabric Notebook feeder &nbsp;<sub><i>(recommended for low-volume polling)</i></sub>

A scheduled Fabric Notebook in [`notebook/`](notebook/) runs the poller inside the Fabric workspace itself, against a per-source Fabric **Environment** that bundles the `usgs_geomag` package and the generated producer sub-packages. The Event Stream custom-endpoint connection string is looked up at runtime via the public Fabric Topology API using the workspace identity — no secrets in the notebook, no separate container host to manage. Dedupe state lives in OneLake under `/lakehouse/default/Files/feeder-state/usgs-geomag/`.
...
[![Deploy Fabric ACI](https://img.shields.io/badge/Fabric-Container%20Feeder-117865?logo=microsoftfabric&logoColor=white)](https://clemensv.github.io/real-time-sources#usgs-geomag/fabric-aci)


### Deploying into Azure Container Instances

6 one-click deployment templates across Kafka, MQTT, and AMQP transports. These templates host the container directly in Azure (without a Fabric workspace). All templates create a storage account and file share for persistent dedupe state.
...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fusgs-geomag%2Fazure-template.json)
...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fusgs-geomag%2Fazure-template-with-eventhub.json)
...
Deploy the AMQP container together with a new Azure Service Bus namespace (Standard SKU) and queue. A user-assigned managed identity is granted the Azure Service Bus Data Sender role; the feeder authenticates via Entra ID (no SAS keys).
...
Deploy the MQTT container together with a new Azure Event Grid namespace (MQTT broker enabled, Standard SKU). A user-assigned managed identity is granted the EventGrid TopicSpace Publisher role; the feeder authenticates via Entra JWT (no certificates or passwords).
