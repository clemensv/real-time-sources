...
class OrgGbfsAmqpSystemProducer:
    """
    Producer class to send messages in the `org.gbfs.amqp.system` message group via AMQP 1.0 protocol.
    """
    ...
    def send_system_information(self,
        ...
        """
        Send the `org.gbfs.amqp.SystemInformation` message
        Reference data describing one GBFS system as published in `system_information.json`, including the public system name, operator, customer-facing URL, timezone, and support metadata. Emitted at bridge startup and on reference refresh so consumers can join station and vehicle telemetry against stable system metadata.
        ...
        attributes = {
            ...
            "source":
            "{feed_url}".format(feed_url=_feed_url),
            "subject":
        ...
        amqp_msg.subject = "{system_id}".format(system_id=_system_id)
        ...
        annotations = {}
        annotation_value = "{system_id}".format(system_id=_system_id)
    ...
    def send_system_information_batch(self,
        data_array: typing.List[SystemInformation],
        _feed_url: str,
        _system_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.gbfs.amqp.SystemInformation` messages
        
        Args:
            data_array (typing.List[SystemInformation]): Array of message data objects
            _feed_url (str): Value for placeholder feed_url in attribute source
            _system_id (str): Value for placeholder system_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
...
class OrgGbfsAmqpStationsProducer:
    """
    Producer class to send messages in the `org.gbfs.amqp.stations` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station_information(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `org.gbfs.amqp.StationInformation` message
        Reference data for one dock-based GBFS station from `station_information.json`. The event carries the stable station identifier, public name, WGS 84 coordinates, docking capacity, and optional address / regional context so consumers can interpret real-time availability telemetry.
        ...
        if amqp_creation_time is not None:
            amqp_msg.creation_time = amqp_creation_time
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "{system_id}/{station_id}".format(system_id=_system_id, station_id=_station_id)
        ...
        annotations = {}
        annotation_value = "{system_id}/{station_id}".format(system_id=_system_id, station_id=_station_id)
        annotation_value = str(annotation_value)[:128]
        annotations[symbol("x-opt-partition-key")] = annotation_value
        if annotations:
            if amqp_msg.annotations is None:
    ...
    def send_station_information_batch(self,
        data_array: typing.List[StationInformation],
        _feed_url: str,
        _system_id: str,
        _station_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.gbfs.amqp.StationInformation` messages
        
        Args:
            data_array (typing.List[StationInformation]): Array of message data objects
            _feed_url (str): Value for placeholder feed_url in attribute source
            _system_id (str): Value for placeholder system_id in attribute subject
            _station_id (str): Value for placeholder station_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    def send_station_status(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `org.gbfs.amqp.StationStatus` message
        Real-time station availability telemetry from `station_status.json`. Each event describes the current rentable-bike count, dock availability, operating flags, and the upstream `last_reported` timestamp for one GBFS station.
    ...
    def send_station_status_batch(self,
        data_array: typing.List[StationStatus],
        _feed_url: str,
        _system_id: str,
        _station_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.gbfs.amqp.StationStatus` messages
        
        Args:
            data_array (typing.List[StationStatus]): Array of message data objects
            _feed_url (str): Value for placeholder feed_url in attribute source
            _system_id (str): Value for placeholder system_id in attribute subject
            _station_id (str): Value for placeholder station_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
...
class OrgGbfsAmqpFreeBikesProducer:
    """
    Producer class to send messages in the `org.gbfs.amqp.free_bikes` message group via AMQP 1.0 protocol.
    """
    ...
    def send_free_bike_status(self,
        data: FreeBikeStatus,
        _feed_url: str,
        _system_id: str,
        _bike_id: str,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `org.gbfs.amqp.FreeBikeStatus` message
        Real-time dockless vehicle telemetry from `free_bike_status.json` (renamed `vehicle_status.json` in GBFS v3). Each event carries the last known location, reservation and disablement state, optional vehicle type reference, and optional remaining range for a single rentable vehicle that is not currently in an active trip.
        ...
        attributes = {
            ...
            "{feed_url}".format(feed_url=_feed_url),
            "subject":
            "{system_id}/{bike_id}".format(system_id=_system_id, bike_id=_bike_id),
        }
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        ...
        if amqp_creation_time is not None:
            amqp_msg.creation_time = amqp_creation_time
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "{system_id}/{bike_id}".format(system_id=_system_id, bike_id=_bike_id)
        ...
        annotations = {}
        annotation_value = "{system_id}/{bike_id}".format(system_id=_system_id, bike_id=_bike_id)
        annotation_value = str(annotation_value)[:128]
        annotations[symbol("x-opt-partition-key")] = annotation_value
        if annotations:
            if amqp_msg.annotations is None:
    ...
    def send_free_bike_status_batch(self,
        data_array: typing.List[FreeBikeStatus],
        _feed_url: str,
        _system_id: str,
        _bike_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.gbfs.amqp.FreeBikeStatus` messages
        
        Args:
            data_array (typing.List[FreeBikeStatus]): Array of message data objects
            _feed_url (str): Value for placeholder feed_url in attribute source
            _system_id (str): Value for placeholder system_id in attribute subject
            _bike_id (str): Value for placeholder bike_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
