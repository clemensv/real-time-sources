...
def get_default_topic_mappings_org_gbfs_mqtt_system() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.gbfs.mqtt.system message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.gbfs.mqtt.SystemInformation": "mobility/gbfs/{system_id}/system",
    }


def get_subscription_topics_org_gbfs_mqtt_system(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_org_gbfs_mqtt_system()
    topics = []
...
class OrgGbfsMqttSystemMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.gbfs.mqtt.system message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_gbfs_mqtt_system()
        ...
        self.org_gbfs_mqtt_system_information_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, gbfs_bikeshare_mqtt_producer_data.SystemInformation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.gbfs.SystemInformation":
            if self.org_gbfs_mqtt_system_information_async:
                try:
                    ...
                    data = gbfs_bikeshare_mqtt_producer_data.SystemInformation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.gbfs.mqtt.SystemInformation")
                    await self.org_gbfs_mqtt_system_information_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_gbfs_mqtt_system_information handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_gbfs_mqtt_system(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_org_gbfs_mqtt_system_information(self,
        feed_url: str,
        system_id: str,
        data: gbfs_bikeshare_mqtt_producer_data.SystemInformation,
        ...
        """
        Publish the 'org.gbfs.mqtt.SystemInformation' event to an MQTT topic.

        Args:
        
            feed_url: URI template variable for 'feed_url'
            system_id: URI template variable for 'system_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'mobility/gbfs/{system_id}/system'
        ...
        """
        target_topic = topic if topic is not None else "mobility/gbfs/{system_id}/system"
        ...
        attributes = {
             ...
             "source":"{feed_url}".format(feed_url = feed_url),
             "subject":"{system_id}".format(system_id = system_id)
        }
        attributes["datacontenttype"] = content_type
...
def get_default_topic_mappings_org_gbfs_mqtt_stations() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.gbfs.mqtt.stations message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.gbfs.mqtt.StationInformation": "mobility/gbfs/{system_id}/{station_id}/info",
        "org.gbfs.mqtt.StationStatus": "mobility/gbfs/{system_id}/{station_id}/status",
    }


def get_subscription_topics_org_gbfs_mqtt_stations(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_org_gbfs_mqtt_stations()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class OrgGbfsMqttStationsMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.gbfs.mqtt.stations message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_gbfs_mqtt_stations()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.org_gbfs_mqtt_station_information_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, gbfs_bikeshare_mqtt_producer_data.StationInformation, Dict[str, str]], Awaitable[None]]] = None
        
        self.org_gbfs_mqtt_station_status_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, gbfs_bikeshare_mqtt_producer_data.StationStatus, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.gbfs.StationInformation":
            if self.org_gbfs_mqtt_station_information_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = gbfs_bikeshare_mqtt_producer_data.StationInformation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.gbfs.mqtt.StationInformation")
                    await self.org_gbfs_mqtt_station_information_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_gbfs_mqtt_station_information handler: {e}")
            return
        
        if event_type == "org.gbfs.StationStatus":
            if self.org_gbfs_mqtt_station_status_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = gbfs_bikeshare_mqtt_producer_data.StationStatus.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.gbfs.mqtt.StationStatus")
                    await self.org_gbfs_mqtt_station_status_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_gbfs_mqtt_station_status handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_gbfs_mqtt_stations(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_org_gbfs_mqtt_station_information(self,
        feed_url: str,
        system_id: str,
        station_id: str,
        data: gbfs_bikeshare_mqtt_producer_data.StationInformation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.gbfs.mqtt.StationInformation' event to an MQTT topic.
        ...
        Args:
            ...
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'mobility/gbfs/{system_id}/{station_id}/info'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "mobility/gbfs/{system_id}/{station_id}/info"
        _topic_template_values: Dict[str, str] = {
            "feed_url": str(feed_url),
            "system_id": str(system_id),
        ...
        attributes = {
             ...
             "source":"{feed_url}".format(feed_url = feed_url),
             "subject":"{system_id}/{station_id}".format(system_id = system_id,station_id = station_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
    ...
    async def publish_org_gbfs_mqtt_station_status(self,
        feed_url: str,
        system_id: str,
        station_id: str,
        data: gbfs_bikeshare_mqtt_producer_data.StationStatus,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.gbfs.mqtt.StationStatus' event to an MQTT topic.
        ...
        Args:
            ...
            station_id: URI template variable for 'station_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'mobility/gbfs/{system_id}/{station_id}/status'
                with URI template placeholders substituted from the keyword arguments.
            ...
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "mobility/gbfs/{system_id}/{station_id}/status"
        _topic_template_values: Dict[str, str] = {
            "feed_url": str(feed_url),
            "system_id": str(system_id),
            "station_id": str(station_id),
...
def get_default_topic_mappings_org_gbfs_mqtt_free_bikes() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.gbfs.mqtt.free_bikes message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.gbfs.mqtt.FreeBikeStatus": "mobility/gbfs/{system_id}/{bike_id}/position",
    }


def get_subscription_topics_org_gbfs_mqtt_free_bikes(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_org_gbfs_mqtt_free_bikes()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class OrgGbfsMqttFreeBikesMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.gbfs.mqtt.free_bikes message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_gbfs_mqtt_free_bikes()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.org_gbfs_mqtt_free_bike_status_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, gbfs_bikeshare_mqtt_producer_data.FreeBikeStatus, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.gbfs.FreeBikeStatus":
            if self.org_gbfs_mqtt_free_bike_status_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = gbfs_bikeshare_mqtt_producer_data.FreeBikeStatus.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.gbfs.mqtt.FreeBikeStatus")
                    await self.org_gbfs_mqtt_free_bike_status_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_gbfs_mqtt_free_bike_status handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_gbfs_mqtt_free_bikes(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_org_gbfs_mqtt_free_bike_status(self,
        feed_url: str,
        system_id: str,
        bike_id: str,
        data: gbfs_bikeshare_mqtt_producer_data.FreeBikeStatus,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.gbfs.mqtt.FreeBikeStatus' event to an MQTT topic.

        Args:
        
            feed_url: URI template variable for 'feed_url'
            system_id: URI template variable for 'system_id'
            bike_id: URI template variable for 'bike_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'mobility/gbfs/{system_id}/{bike_id}/position'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (0).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "mobility/gbfs/{system_id}/{bike_id}/position"
        _topic_template_values: Dict[str, str] = {
            "feed_url": str(feed_url),
            "system_id": str(system_id),
            "bike_id": str(bike_id),
        ...
        attributes = {
             ...
             "source":"{feed_url}".format(feed_url = feed_url),
             "subject":"{system_id}/{bike_id}".format(system_id = system_id,bike_id = bike_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
