...
class StationStatus:
    """
    Near-real-time station availability data from the GBFS `station_status.json` feed. Each record reports the current inventory of rentable vehicles and available docks together with boolean operating flags and the upstream `last_reported` timestamp for the station.
    ...
    last_reported: int=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="last_reported"))
    ...
    def create_instance(cls) -> 'StationStatus':
        ...
        """
        return cls(
            system_id='wnekmzvilrqjkhvkznxr',
            station_id='caefborxgviasoglextx',
            num_bikes_available=int(46),
            num_docks_available=int(91),
            num_ebikes_available=int(20),
            is_installed=False,
            is_renting=False,
            is_returning=True,
            last_reported=int(100)
        )