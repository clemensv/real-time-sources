# Gbfs_bikeshare_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the gbfs_bikeshare_mqtt_producer message definitions.

# Gbfs_bikeshare_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - OrgGbfsMqttSystemEventDispatcher,

    OrgGbfsMqttStationsEventDispatcher,

    OrgGbfsMqttFreeBikesEventDispatcher
...
from gbfs_bikeshare_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = OrgGbfsMqttSystemMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgGbfsMqttSystemEventDispatcher



# Publish message`OrgGbfsMqttSystemEventDispatcher` handles events for the org.gbfs.mqtt.system message group.
...
from gbfs_bikeshare_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = OrgGbfsMqttSystemMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
client = OrgGbfsMqttStationsMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
Hub.Args:- `consumer_group_name`: The name of the consumer group.- `eventhubs_fully_qualified_namespace`: The fully
qualified namespace of the Event Hub.

client = OrgGbfsMqttFreeBikesMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe OrgGbfsMqttSystemEventDispatcher defines the following event handler hooks.
...
```##### `org_gbfs_mqtt_system_information_async`



## Test```python

org_gbfs_mqtt_system_information_async:  Callable[[PartitionContext, EventData, CloudEvent, SystemInformation],
Awaitable[None]]
...
```Asynchronous handler hook for `org.gbfs.mqtt.SystemInformation`: Reference data describing one GBFS system as
published in `system_information.json`, including the public system name, operator, customer-facing URL, timezone, and
support metadata. Emitted at bridge startup and on reference refresh so consumers can join station and vehicle telemetry
against stable system metadata.
...
- `data`: The event data of type `gbfs_bikeshare_mqtt_producer_data.SystemInformation`.

Example:

```python
async def org_gbfs_mqtt_system_information_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: SystemInformation) -> None:
...
client = OrgGbfsMqttStationsMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgGbfsMqttStationsEventDispatcher



# Publish message`OrgGbfsMqttStationsEventDispatcher` handles events for the org.gbfs.mqtt.stations message group.
...
## BuildThe OrgGbfsMqttStationsEventDispatcher defines the following event handler hooks.
...
```##### `org_gbfs_mqtt_station_information_async`



## Test```python

org_gbfs_mqtt_station_information_async:  Callable[[PartitionContext, EventData, CloudEvent, StationInformation],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `gbfs_bikeshare_mqtt_producer_data.StationInformation`.

Example:

```python
async def org_gbfs_mqtt_station_information_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: StationInformation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `org_gbfs_mqtt_station_status_async`



## Test```python

org_gbfs_mqtt_station_status_async:  Callable[[PartitionContext, EventData, CloudEvent, StationStatus], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `gbfs_bikeshare_mqtt_producer_data.StationStatus`.

Example:

```python
async def org_gbfs_mqtt_station_status_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: StationStatus) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = OrgGbfsMqttFreeBikesMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgGbfsMqttFreeBikesEventDispatcher



# Publish message`OrgGbfsMqttFreeBikesEventDispatcher` handles events for the org.gbfs.mqtt.free_bikes message group.
...
## BuildThe OrgGbfsMqttFreeBikesEventDispatcher defines the following event handler hooks.
...
```##### `org_gbfs_mqtt_free_bike_status_async`



## Test```python

org_gbfs_mqtt_free_bike_status_async:  Callable[[PartitionContext, EventData, CloudEvent, FreeBikeStatus],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `gbfs_bikeshare_mqtt_producer_data.FreeBikeStatus`.

Example:

```python
async def org_gbfs_mqtt_free_bike_status_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: FreeBikeStatus) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
