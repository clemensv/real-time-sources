...
import os
import signal
from gbfs_bikeshare_mqtt_producer_data import *
from gbfs_bikeshare_mqtt_producer_mqtt_client.client import OrgGbfsMqttSystemProducer, OrgGbfsMqttSystemDispatcher

async def handle_org_gbfs_mqtt_system_information(mqtt_msg,cloud_event, org_gbfs_mqtt_system_information_data):
    """ Handles the org.gbfs.mqtt.SystemInformation message """
    print(f"Received org.gbfs.mqtt.SystemInformation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_gbfs_mqtt_system_information_data}")
from gbfs_bikeshare_mqtt_producer_mqtt_client.client import OrgGbfsMqttStationsProducer, OrgGbfsMqttStationsDispatcher

async def handle_org_gbfs_mqtt_station_information(mqtt_msg,cloud_event, org_gbfs_mqtt_station_information_data):
    """ Handles the org.gbfs.mqtt.StationInformation message """
    print(f"Received org.gbfs.mqtt.StationInformation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_gbfs_mqtt_station_information_data}")

async def handle_org_gbfs_mqtt_station_status(mqtt_msg,cloud_event, org_gbfs_mqtt_station_status_data):
    """ Handles the org.gbfs.mqtt.StationStatus message """
    print(f"Received org.gbfs.mqtt.StationStatus on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_gbfs_mqtt_station_status_data}")
from gbfs_bikeshare_mqtt_producer_mqtt_client.client import OrgGbfsMqttFreeBikesProducer, OrgGbfsMqttFreeBikesDispatcher

async def handle_org_gbfs_mqtt_free_bike_status(mqtt_msg,cloud_event, org_gbfs_mqtt_free_bike_status_data):
    """ Handles the org.gbfs.mqtt.FreeBikeStatus message """
    print(f"Received org.gbfs.mqtt.FreeBikeStatus on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_gbfs_mqtt_free_bike_status_data}")
