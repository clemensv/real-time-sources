...
Subscribe to `gbfs-bikeshare`. The record key is `{system_id}`, `{system_id}/{station_id}`, `{system_id}/{bike_id}`. Each key template is explained in the event catalog below. Kafka uses the key for partition routing: events with the same key go to the same partition and keep per-key order, but consumers still receive an interleaved stream.
...
class H(MessagingHandler):
    def on_start(self,e): e.container.create_receiver('amqps://user:pass@localhost:5671/gbfs-bikeshare')
...
| `AMQP/1.0` | source address `amqps://localhost:5671/gbfs-bikeshare`, message subject `{system_id}` |
...
| `AMQP/1.0` | source address `amqps://localhost:5671/gbfs-bikeshare`, message subject `{system_id}/{station_id}` |
...
| `AMQP/1.0` | source address `amqps://localhost:5671/gbfs-bikeshare`, message subject `{system_id}/{bike_id}` |
...
- xRegistry manifest: [`xreg/gbfs-bikeshare.xreg.json`](xreg/gbfs-bikeshare.xreg.json)
