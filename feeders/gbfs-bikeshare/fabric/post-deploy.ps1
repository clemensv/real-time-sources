...
function Resolve-PythonCommand {
    ...
    throw "Python is required to wire the gbfs-bikeshare Fabric Map but was not found on PATH."
}
...
$mapName = if ($env:GBFS_BIKESHARE_FABRIC_MAP_NAME) { $env:GBFS_BIKESHARE_FABRIC_MAP_NAME } else { "gbfs-bikeshare-map" }
$existingMapId = if ($env:GBFS_BIKESHARE_FABRIC_MAP_ID) { $env:GBFS_BIKESHARE_FABRIC_MAP_ID } else { $null }

if (-not $existingMapId) {
    Write-Host "=== gbfs-bikeshare post-deploy: ensuring Fabric Map '$mapName' ==="
...
if (-not $existingMapId) {
    ...
    $bodyFile = Join-Path ([System.IO.Path]::GetTempPath()) ("gbfs-bikeshare-map-create-{0}.json" -f ([Guid]::NewGuid().ToString("N")))
...
$wireScript = Join-Path $PSScriptRoot "wire_gbfs_bikeshare_map.py"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "wire_gbfs_bikeshare_map.py failed with exit code $LASTEXITCODE"
    }
} finally {
