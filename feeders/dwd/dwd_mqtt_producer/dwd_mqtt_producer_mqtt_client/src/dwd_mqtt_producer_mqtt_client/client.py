...
def get_default_topic_mappings_de_dwd_cdc_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the DE.DWD.CDC.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "DE.DWD.CDC.mqtt.StationMetadata": "weather/de/dwd/dwd/{state}/{station_id}/info",
        "DE.DWD.CDC.mqtt.AirTemperature10Min": "weather/de/dwd/dwd/{state}/{station_id}/air-temperature-10min",
        "DE.DWD.CDC.mqtt.Precipitation10Min": "weather/de/dwd/dwd/{state}/{station_id}/precipitation-10min",
        "DE.DWD.CDC.mqtt.Wind10Min": "weather/de/dwd/dwd/{state}/{station_id}/wind-10min",
        "DE.DWD.CDC.mqtt.Solar10Min": "weather/de/dwd/dwd/{state}/{station_id}/solar-10min",
        "DE.DWD.CDC.mqtt.HourlyObservation": "weather/de/dwd/dwd/{state}/{station_id}/hourly-observation",
        "DE.DWD.CDC.mqtt.ExtremeWind10Min": "weather/de/dwd/dwd/{state}/{station_id}/extreme-wind-10min",
        "DE.DWD.CDC.mqtt.ExtremeTemperature10Min": "weather/de/dwd/dwd/{state}/{station_id}/extreme-temperature-10min",
    }


def get_subscription_topics_de_dwd_cdc_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_de_dwd_cdc_mqtt()
    topics = []
...
class DEDWDCDCMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the DE.DWD.CDC.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_de_dwd_cdc_mqtt()
        ...
        self.de_dwd_cdc_mqtt_station_metadata_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_mqtt_producer_data.StationMetadata, Dict[str, str]], Awaitable[None]]] = None
        
        self.de_dwd_cdc_mqtt_air_temperature10_min_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_mqtt_producer_data.AirTemperature10Min, Dict[str, str]], Awaitable[None]]] = None
        
        self.de_dwd_cdc_mqtt_precipitation10_min_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_mqtt_producer_data.Precipitation10Min, Dict[str, str]], Awaitable[None]]] = None
        
        self.de_dwd_cdc_mqtt_wind10_min_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_mqtt_producer_data.Wind10Min, Dict[str, str]], Awaitable[None]]] = None
        
        self.de_dwd_cdc_mqtt_solar10_min_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_mqtt_producer_data.Solar10Min, Dict[str, str]], Awaitable[None]]] = None
        
        self.de_dwd_cdc_mqtt_hourly_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_mqtt_producer_data.HourlyObservation, Dict[str, str]], Awaitable[None]]] = None
        
        self.de_dwd_cdc_mqtt_extreme_wind10_min_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_mqtt_producer_data.ExtremeWind10Min, Dict[str, str]], Awaitable[None]]] = None
        
        self.de_dwd_cdc_mqtt_extreme_temperature10_min_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_mqtt_producer_data.ExtremeTemperature10Min, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "DE.DWD.CDC.StationMetadata":
            if self.de_dwd_cdc_mqtt_station_metadata_async:
                try:
                    ...
                    data = dwd_mqtt_producer_data.StationMetadata.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.CDC.mqtt.StationMetadata")
                    await self.de_dwd_cdc_mqtt_station_metadata_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_cdc_mqtt_station_metadata handler: {e}")
            return
        
        if event_type == "DE.DWD.CDC.AirTemperature10Min":
            if self.de_dwd_cdc_mqtt_air_temperature10_min_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dwd_mqtt_producer_data.AirTemperature10Min.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.CDC.mqtt.AirTemperature10Min")
                    await self.de_dwd_cdc_mqtt_air_temperature10_min_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_cdc_mqtt_air_temperature10_min handler: {e}")
            return
        
        if event_type == "DE.DWD.CDC.Precipitation10Min":
            if self.de_dwd_cdc_mqtt_precipitation10_min_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dwd_mqtt_producer_data.Precipitation10Min.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.CDC.mqtt.Precipitation10Min")
                    await self.de_dwd_cdc_mqtt_precipitation10_min_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_cdc_mqtt_precipitation10_min handler: {e}")
            return
        
        if event_type == "DE.DWD.CDC.Wind10Min":
            if self.de_dwd_cdc_mqtt_wind10_min_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dwd_mqtt_producer_data.Wind10Min.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.CDC.mqtt.Wind10Min")
                    await self.de_dwd_cdc_mqtt_wind10_min_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_cdc_mqtt_wind10_min handler: {e}")
            return
        
        if event_type == "DE.DWD.CDC.Solar10Min":
            if self.de_dwd_cdc_mqtt_solar10_min_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dwd_mqtt_producer_data.Solar10Min.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.CDC.mqtt.Solar10Min")
                    await self.de_dwd_cdc_mqtt_solar10_min_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_cdc_mqtt_solar10_min handler: {e}")
            return
        
        if event_type == "DE.DWD.CDC.HourlyObservation":
            if self.de_dwd_cdc_mqtt_hourly_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dwd_mqtt_producer_data.HourlyObservation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.CDC.mqtt.HourlyObservation")
                    await self.de_dwd_cdc_mqtt_hourly_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_cdc_mqtt_hourly_observation handler: {e}")
            return
        
        if event_type == "DE.DWD.CDC.ExtremeWind10Min":
            if self.de_dwd_cdc_mqtt_extreme_wind10_min_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dwd_mqtt_producer_data.ExtremeWind10Min.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.CDC.mqtt.ExtremeWind10Min")
                    await self.de_dwd_cdc_mqtt_extreme_wind10_min_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_cdc_mqtt_extreme_wind10_min handler: {e}")
            return
        
        if event_type == "DE.DWD.CDC.ExtremeTemperature10Min":
            if self.de_dwd_cdc_mqtt_extreme_temperature10_min_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dwd_mqtt_producer_data.ExtremeTemperature10Min.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.CDC.mqtt.ExtremeTemperature10Min")
                    await self.de_dwd_cdc_mqtt_extreme_temperature10_min_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_cdc_mqtt_extreme_temperature10_min handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_de_dwd_cdc_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_de_dwd_cdc_mqtt_station_metadata(self,
        station_id: str,
        state: str,
        data: dwd_mqtt_producer_data.StationMetadata,
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'weather/de/dwd/dwd/{state}/{station_id}/info'
    ...
    async def publish_de_dwd_cdc_mqtt_air_temperature10_min(self,
        station_id: str,
        state: str,
        data: dwd_mqtt_producer_data.AirTemperature10Min,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            state: URI template variable for 'state'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/de/dwd/dwd/{state}/{station_id}/air-temperature-10min'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    async def publish_de_dwd_cdc_mqtt_precipitation10_min(self,
        station_id: str,
        state: str,
        data: dwd_mqtt_producer_data.Precipitation10Min,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            state: URI template variable for 'state'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/de/dwd/dwd/{state}/{station_id}/precipitation-10min'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    async def publish_de_dwd_cdc_mqtt_wind10_min(self,
        station_id: str,
        state: str,
        data: dwd_mqtt_producer_data.Wind10Min,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            state: URI template variable for 'state'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/de/dwd/dwd/{state}/{station_id}/wind-10min'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    async def publish_de_dwd_cdc_mqtt_solar10_min(self,
        station_id: str,
        state: str,
        data: dwd_mqtt_producer_data.Solar10Min,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            state: URI template variable for 'state'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/de/dwd/dwd/{state}/{station_id}/solar-10min'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    async def publish_de_dwd_cdc_mqtt_hourly_observation(self,
        station_id: str,
        state: str,
        data: dwd_mqtt_producer_data.HourlyObservation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            state: URI template variable for 'state'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/de/dwd/dwd/{state}/{station_id}/hourly-observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    async def publish_de_dwd_cdc_mqtt_extreme_wind10_min(self,
        station_id: str,
        state: str,
        data: dwd_mqtt_producer_data.ExtremeWind10Min,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            state: URI template variable for 'state'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/de/dwd/dwd/{state}/{station_id}/extreme-wind-10min'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        attributes = {
             "type":"DE.DWD.CDC.ExtremeWind10Min",
             "source":"https://opendata.dwd.de",
             "subject":"{station_id}".format(station_id = station_id)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_de_dwd_cdc_mqtt_extreme_temperature10_min(self,
        station_id: str,
        state: str,
        data: dwd_mqtt_producer_data.ExtremeTemperature10Min,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            state: URI template variable for 'state'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/de/dwd/dwd/{state}/{station_id}/extreme-temperature-10min'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        attributes = {
             "type":"DE.DWD.CDC.ExtremeTemperature10Min",
             "source":"https://opendata.dwd.de",
             "subject":"{station_id}".format(station_id = station_id)
        }
        attributes["datacontenttype"] = content_type
...
def get_default_topic_mappings_de_dwd_weather_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the DE.DWD.Weather.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "DE.DWD.Weather.mqtt.Alert": "alerts/de/dwd/dwd/{state}/{severity}/{identifier}/alert",
    }


def get_subscription_topics_de_dwd_weather_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_de_dwd_weather_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class DEDWDWeatherMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the DE.DWD.Weather.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_de_dwd_weather_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.de_dwd_weather_mqtt_alert_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_mqtt_producer_data.Alert, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "DE.DWD.Weather.Alert":
            if self.de_dwd_weather_mqtt_alert_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dwd_mqtt_producer_data.Alert.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.Weather.mqtt.Alert")
                    await self.de_dwd_weather_mqtt_alert_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_weather_mqtt_alert handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_de_dwd_weather_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_de_dwd_weather_mqtt_alert(self,
        state: str,
        severity: str,
        identifier: str,
        data: dwd_mqtt_producer_data.Alert,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'alerts/de/dwd/dwd/{state}/{severity}/{identifier}/alert'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
...
def get_default_topic_mappings_de_dwd_radar_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the DE.DWD.Radar.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "DE.DWD.Radar.mqtt.RadarProductCatalog": "weather/de/dwd/dwd/catalogs/{kind}/catalog",
        "DE.DWD.Radar.mqtt.RadarFileProduct": "weather/de/dwd/dwd/products/radar/{product_type}/{file_id}/file",
    }


def get_subscription_topics_de_dwd_radar_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_de_dwd_radar_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class DEDWDRadarMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the DE.DWD.Radar.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_de_dwd_radar_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.de_dwd_radar_mqtt_radar_product_catalog_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_mqtt_producer_data.RadarProductCatalog, Dict[str, str]], Awaitable[None]]] = None
        
        self.de_dwd_radar_mqtt_radar_file_product_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_mqtt_producer_data.RadarFileProduct, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "DE.DWD.Radar.RadarProductCatalog":
            if self.de_dwd_radar_mqtt_radar_product_catalog_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dwd_mqtt_producer_data.RadarProductCatalog.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.Radar.mqtt.RadarProductCatalog")
                    await self.de_dwd_radar_mqtt_radar_product_catalog_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_radar_mqtt_radar_product_catalog handler: {e}")
            return
        
        if event_type == "DE.DWD.Radar.RadarFileProduct":
            if self.de_dwd_radar_mqtt_radar_file_product_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dwd_mqtt_producer_data.RadarFileProduct.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.Radar.mqtt.RadarFileProduct")
                    await self.de_dwd_radar_mqtt_radar_file_product_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_radar_mqtt_radar_file_product handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_de_dwd_radar_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_de_dwd_radar_mqtt_radar_product_catalog(self,
        kind: str,
        data: dwd_mqtt_producer_data.RadarProductCatalog,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            kind: URI template variable for 'kind'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/de/dwd/dwd/catalogs/{kind}/catalog'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    async def publish_de_dwd_radar_mqtt_radar_file_product(self,
        product_type: str,
        file_id: str,
        data: dwd_mqtt_producer_data.RadarFileProduct,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            product_type: URI template variable for 'product_type'
            file_id: URI template variable for 'file_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/de/dwd/dwd/products/radar/{product_type}/{file_id}/file'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
...
def get_default_topic_mappings_de_dwd_forecast_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the DE.DWD.Forecast.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "DE.DWD.Forecast.mqtt.ForecastModelCatalog": "weather/de/dwd/dwd/catalogs/{kind}/catalog",
        "DE.DWD.Forecast.mqtt.IconD2ForecastFile": "weather/de/dwd/dwd/products/icon-d2/{variable}/{file_id}/file",
    }


def get_subscription_topics_de_dwd_forecast_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_de_dwd_forecast_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class DEDWDForecastMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the DE.DWD.Forecast.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_de_dwd_forecast_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.de_dwd_forecast_mqtt_forecast_model_catalog_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_mqtt_producer_data.ForecastModelCatalog, Dict[str, str]], Awaitable[None]]] = None
        
        self.de_dwd_forecast_mqtt_icon_d2_forecast_file_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_mqtt_producer_data.IconD2ForecastFile, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "DE.DWD.Forecast.ForecastModelCatalog":
            if self.de_dwd_forecast_mqtt_forecast_model_catalog_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dwd_mqtt_producer_data.ForecastModelCatalog.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.Forecast.mqtt.ForecastModelCatalog")
                    await self.de_dwd_forecast_mqtt_forecast_model_catalog_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_forecast_mqtt_forecast_model_catalog handler: {e}")
            return
        
        if event_type == "DE.DWD.Forecast.IconD2ForecastFile":
            if self.de_dwd_forecast_mqtt_icon_d2_forecast_file_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dwd_mqtt_producer_data.IconD2ForecastFile.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.Forecast.mqtt.IconD2ForecastFile")
                    await self.de_dwd_forecast_mqtt_icon_d2_forecast_file_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_forecast_mqtt_icon_d2_forecast_file handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_de_dwd_forecast_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_de_dwd_forecast_mqtt_forecast_model_catalog(self,
        kind: str,
        data: dwd_mqtt_producer_data.ForecastModelCatalog,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
    ...
    async def publish_de_dwd_forecast_mqtt_icon_d2_forecast_file(self,
        variable: str,
        file_id: str,
        data: dwd_mqtt_producer_data.IconD2ForecastFile,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        ...
        Args:
            ...
            variable: URI template variable for 'variable'
            file_id: URI template variable for 'file_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/de/dwd/dwd/products/icon-d2/{variable}/{file_id}/file'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
