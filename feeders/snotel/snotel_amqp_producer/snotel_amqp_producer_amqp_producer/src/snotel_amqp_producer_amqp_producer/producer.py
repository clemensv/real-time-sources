...
class GovUsdaNrcsSnotelAmqpProducer:
    """
    Producer class to send messages in the `gov.usda.nrcs.snotel.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        data: Station,
        _station_triplet: str,
        _state: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send the `gov.usda.nrcs.snotel.amqp.Station` message
        ...
        attributes = {
            ...
            "subject":
            "{station_triplet}".format(station_triplet=_station_triplet),
        }
        ...
        amqp_msg.subject = "{station_triplet}".format(station_triplet=_station_triplet)
        ...
        annotations = {}
        annotation_value = "{station_triplet}".format(station_triplet=_station_triplet)
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `gov.usda.nrcs.snotel.amqp.Station` messages
        ...
        for data in data_array:
            self.send_station(
                data=data,
                _station_triplet=_station_triplet,
                _time=_time,
                _state=_state,
                content_type=content_type)
    
    
    def send_snow_observation(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `gov.usda.nrcs.snotel.amqp.SnowObservation` message
        
        Args:
            _station_triplet (str): Value for placeholder station_triplet in attribute subject
            _state (str): Value for AMQP protocol option placeholder state
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            data (SnowObservation): The message data object
            content_type (str): The content type of the message data (default: 'application/json')
        """
        # Build CloudEvent attributes
        attributes = {
    ...
    def send_snow_observation_batch(self,
        data_array: typing.List[SnowObservation],
        _station_triplet: str,
        _state: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `gov.usda.nrcs.snotel.amqp.SnowObservation` messages
        
        Args:
            data_array (typing.List[SnowObservation]): Array of message data objects
            _station_triplet (str): Value for placeholder station_triplet in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            _state (str): Value for AMQP protocol option placeholder state
            content_type (str): The content type of the message data
