...
"""
Tests for snotel_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../snotel_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../snotel_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../snotel_amqp_producer_amqp_producer/src')))

from snotel_amqp_producer_amqp_producer import *
from snotel_amqp_producer_data import Station
from test_station import Test_Station
from snotel_amqp_producer_data import SnowObservation
...
class TestGovUsdaNrcsSnotelAmqpProducer:
    """Test cases for GovUsdaNrcsSnotelAmqpProducer"""
    
    def test_producer_initialization(self, artemis_container):
        """Test that producer initializes correctly"""
        producer = GovUsdaNrcsSnotelAmqpProducer(
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(GovUsdaNrcsSnotelAmqpProducer)
    ...
    def test_send_station(self, artemis_container):
        ...
        try:
            ...
            for i in range(5):
                ...
                assert annotations.get(symbol('x-opt-partition-key')) == str("{station_triplet}".format(station_triplet="value"))[:128]
        finally:
            producer.close()
    ...
    def test_send_snow_observation(self, artemis_container):
        """Send and receive a SnowObservation message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_SnowObservation.create_instance()
        ...
        try:
            ...
            # Send 5 messages to test proper message settlement and ordering
            for i in range(5):
                producer.send_snow_observation(
                    data=payload,
                    _station_triplet="value",
                    _state="value",
                    _time=datetime.datetime.now(datetime.timezone.utc).isoformat(),
    ...
    def test_send_snow_observation_single_fresh_connection(self, artemis_container):
        """Send exactly one SnowObservation message on a fresh producer connection."""
        payload = Test_SnowObservation.create_instance()
