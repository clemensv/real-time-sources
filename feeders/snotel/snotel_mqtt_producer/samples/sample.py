...
import os
import signal
from snotel_mqtt_producer_data import *
from snotel_mqtt_producer_mqtt_client.client import GovUsdaNrcsSnotelMqttProducer, GovUsdaNrcsSnotelMqttDispatcher

async def handle_gov_usda_nrcs_snotel_mqtt_station(mqtt_msg,cloud_event, gov_usda_nrcs_snotel_mqtt_station_data):
    """ Handles the gov.usda.nrcs.snotel.mqtt.Station message """
    print(f"Received gov.usda.nrcs.snotel.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {gov_usda_nrcs_snotel_mqtt_station_data}")

async def handle_gov_usda_nrcs_snotel_mqtt_snow_observation(mqtt_msg,cloud_event, gov_usda_nrcs_snotel_mqtt_snow_observation_data):
    """ Handles the gov.usda.nrcs.snotel.mqtt.SnowObservation message """
    print(f"Received gov.usda.nrcs.snotel.mqtt.SnowObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {gov_usda_nrcs_snotel_mqtt_snow_observation_data}")
