...
"""
Tests for australia_wildfires_amqp_producer_amqp_producer
"""
import base64
import json
import os
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../australia_wildfires_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../australia_wildfires_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../australia_wildfires_amqp_producer_amqp_producer/src')))

from australia_wildfires_amqp_producer_amqp_producer import *
from australia_wildfires_amqp_producer_data import FireIncident
from test_australia_wildfires_amqp_producer_data_fireincident import Test_FireIncident
...
class TestAUGovEmergencyWildfiresAmqpProducer:
    """Test cases for AUGovEmergencyWildfiresAmqpProducer"""
    ...
    def test_send_fire_incident(self, artemis_container):
        """Send and receive a FireIncident message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_FireIncident.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "AU.Gov.Emergency.Wildfires.amqp.FireIncident"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{state}/{incident_id}".format(state="value", incident_id="value")
        finally:
            producer.close()
