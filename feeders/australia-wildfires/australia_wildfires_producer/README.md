...
# Australia_wildfires_producer Kafka Producer# Australia_wildfires_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - AUGovEmergencyWildfiresEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    AUGovEmergencyWildfiresMqttEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    AUGovEmergencyWildfiresAmqpEventDispatcher
...
- AUGovEmergencyWildfiresProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- AUGovEmergencyWildfiresMqttProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- AUGovEmergencyWildfiresAmqpProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### AUGovEmergencyWildfiresEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`AUGovEmergencyWildfiresEventDispatcher` handles events for the AU.Gov.Emergency.Wildfires message group.
...
from australia_wildfires_producer import AUGovEmergencyWildfiresProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = AUGovEmergencyWildfiresProducer(
...
await producer.send_au_gov_emergency_wildfires_fire_incident(```python

    data=FireIncident(...),add_consumer(self, consumer: KafkaConsumer)
...
The AUGovEmergencyWildfiresEventDispatcher defines the following event handler hooks.
...
producer = AUGovEmergencyWildfiresProducer(
    ...
    security_protocol='SASL_SSL',##### `au_gov_emergency_wildfires_fire_incident_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'au_gov_emergency_wildfires_fire_incident_async:  Callable[[ConsumerRecord, CloudEvent,
FireIncident], Awaitable[None]]

)```

```

Asynchronous handler hook for `AU.Gov.Emergency.Wildfires.FireIncident`: An incident update from Australian emergency-
services incident feeds. It reports the current status, location, and classification for a wildfire or emergency
incident.
...
### AUGovEmergencyWildfiresProducer- `data`: The event data of type `australia_wildfires_producer_data.FireIncident`.



Producer for `AU.Gov.Emergency.Wildfires` message group.Example:



#### Constructor```python

async def au_gov_emergency_wildfires_fire_incident_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
FireIncident) -> None:

```python    # Process the event data

AUGovEmergencyWildfiresProducer(    await some_processing_function(record, cloud_event, data)
...
au_gov_emergency_wildfires_dispatcher.au_gov_emergency_wildfires_fire_incident_async =
au_gov_emergency_wildfires_fire_incident_event
...
### AUGovEmergencyWildfiresMqttProducer- `data`: The event data of type
`australia_wildfires_producer_data.FireIncident`.



Producer for `AU.Gov.Emergency.Wildfires.mqtt` message group.Example:
...
AUGovEmergencyWildfiresMqttProducer(    await some_processing_function(record, cloud_event, data)
...
au_gov_emergency_wildfires_mqtt_dispatcher.au_gov_emergency_wildfires_fire_incident_async =
au_gov_emergency_wildfires_fire_incident_event
...
### AUGovEmergencyWildfiresAmqpProducer- `data`: The event data of type
`australia_wildfires_producer_data.FireIncident`.



Producer for `AU.Gov.Emergency.Wildfires.amqp` message group.Example:
...
AUGovEmergencyWildfiresAmqpProducer(    await some_processing_function(record, cloud_event, data)
...
au_gov_emergency_wildfires_amqp_dispatcher.au_gov_emergency_wildfires_fire_incident_async =
au_gov_emergency_wildfires_fire_incident_event
...
##### `send_au_gov_emergency_wildfires_fire_incident`Dispatchers have the following protected methods:



```python### Methods:

async def send_au_gov_emergency_wildfires_fire_incident(
...
Send a single `AU.Gov.Emergency.Wildfires.FireIncident` message. An incident update from Australian emergency-services
incident feeds. It reports the current status, location, and classification for a wildfire or emergency incident.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `FireIncident`
...
await producer.send_au_gov_emergency_wildfires_fire_incident(

    data=FireIncident(...),Args:
...
Send multiple `AU.Gov.Emergency.Wildfires.FireIncident` messages in a batch.
...
await producer.send_au_gov_emergency_wildfires_fire_incident_batch(```

    messages=[

        FireIncident(...),Initializes the runner with a Kafka consumer.
...
- **Scales horizontally** across multiple brokers and partitions### AUGovEmergencyWildfiresMqttEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`AUGovEmergencyWildfiresMqttEventDispatcher` handles events for the AU.Gov.Emergency.Wildfires.mqtt message group.
...
The AUGovEmergencyWildfiresMqttEventDispatcher defines the following event handler hooks.
...
producer = AUGovEmergencyWildfiresProducer(
    ...
    security_protocol='SASL_SSL',##### `au_gov_emergency_wildfires_mqtt_fire_incident_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'au_gov_emergency_wildfires_mqtt_fire_incident_async:  Callable[[ConsumerRecord,
CloudEvent, FireIncident], Awaitable[None]]

)```

```

Asynchronous handler hook for `AU.Gov.Emergency.Wildfires.mqtt.FireIncident`: An incident update from Australian
emergency-services incident feeds. It reports the current status, location, and classification for a wildfire or
emergency incident.
...
async def au_gov_emergency_wildfires_mqtt_fire_incident_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
FireIncident) -> None:
...
au_gov_emergency_wildfires_dispatcher.au_gov_emergency_wildfires_mqtt_fire_incident_async =
...
au_gov_emergency_wildfires_amqp_dispatcher.au_gov_emergency_wildfires_mqtt_fire_incident_async =
au_gov_emergency_wildfires_mqtt_fire_incident_event
...
##### `send_au_gov_emergency_wildfires_mqtt_fire_incident`Dispatchers have the following protected methods:



```python### Methods:

async def send_au_gov_emergency_wildfires_mqtt_fire_incident(
...
Send a single `AU.Gov.Emergency.Wildfires.mqtt.FireIncident` message. An incident update from Australian emergency-
services incident feeds. It reports the current status, location, and classification for a wildfire or emergency
incident.Args:
...
await producer.send_au_gov_emergency_wildfires_mqtt_fire_incident(
...
Send multiple `AU.Gov.Emergency.Wildfires.mqtt.FireIncident` messages in a batch.
...
await producer.send_au_gov_emergency_wildfires_mqtt_fire_incident_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### AUGovEmergencyWildfiresAmqpEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`AUGovEmergencyWildfiresAmqpEventDispatcher` handles events for the AU.Gov.Emergency.Wildfires.amqp message group.
...
The AUGovEmergencyWildfiresAmqpEventDispatcher defines the following event handler hooks.
...
producer = AUGovEmergencyWildfiresProducer(
    ...
    security_protocol='SASL_SSL',##### `au_gov_emergency_wildfires_amqp_fire_incident_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'au_gov_emergency_wildfires_amqp_fire_incident_async:  Callable[[ConsumerRecord,
CloudEvent, FireIncident], Awaitable[None]]
...
Asynchronous handler hook for `AU.Gov.Emergency.Wildfires.amqp.FireIncident`: An incident update from Australian
emergency-services incident feeds. It reports the current status, location, and classification for a wildfire or
emergency incident.
...
async def au_gov_emergency_wildfires_amqp_fire_incident_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
FireIncident) -> None:
...
au_gov_emergency_wildfires_dispatcher.au_gov_emergency_wildfires_amqp_fire_incident_async =
au_gov_emergency_wildfires_amqp_fire_incident_event
...
au_gov_emergency_wildfires_mqtt_dispatcher.au_gov_emergency_wildfires_amqp_fire_incident_async =
au_gov_emergency_wildfires_amqp_fire_incident_event
...
au_gov_emergency_wildfires_amqp_dispatcher.au_gov_emergency_wildfires_amqp_fire_incident_async =
au_gov_emergency_wildfires_amqp_fire_incident_event
...
##### `send_au_gov_emergency_wildfires_amqp_fire_incident`Dispatchers have the following protected methods:



```python### Methods:

async def send_au_gov_emergency_wildfires_amqp_fire_incident(
...
Send a single `AU.Gov.Emergency.Wildfires.amqp.FireIncident` message. An incident update from Australian emergency-
services incident feeds. It reports the current status, location, and classification for a wildfire or emergency
incident.Args:
...
await producer.send_au_gov_emergency_wildfires_amqp_fire_incident(
...
Send multiple `AU.Gov.Emergency.Wildfires.amqp.FireIncident` messages in a batch.
...
await producer.send_au_gov_emergency_wildfires_amqp_fire_incident_batch(```
