# Geosphere_austria_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the geosphere_austria_mqtt_producer message definitions.

# Geosphere_austria_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - AtGeosphereTawesMqttEventDispatcher
...
from geosphere_austria_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = AtGeosphereTawesMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### AtGeosphereTawesMqttEventDispatcher



# Publish message`AtGeosphereTawesMqttEventDispatcher` handles events for the at.geosphere.tawes.mqtt message group.
...
from geosphere_austria_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = AtGeosphereTawesMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe AtGeosphereTawesMqttEventDispatcher defines the following event handler hooks.
...
```##### `at_geosphere_tawes_mqtt_weather_station_async`



## Test```python

at_geosphere_tawes_mqtt_weather_station_async:  Callable[[PartitionContext, EventData, CloudEvent, WeatherStation],
Awaitable[None]]
...
- `data`: The event data of type `geosphere_austria_mqtt_producer_data.WeatherStation`.

Example:

```python
async def at_geosphere_tawes_mqtt_weather_station_event(partition_context: PartitionContext, event: EventData,
...
```##### `at_geosphere_tawes_mqtt_weather_observation_async`



## Test```python

at_geosphere_tawes_mqtt_weather_observation_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `geosphere_austria_mqtt_producer_data.WeatherObservation`.

Example:

```python
async def at_geosphere_tawes_mqtt_weather_observation_event(partition_context: PartitionContext, event: EventData,
