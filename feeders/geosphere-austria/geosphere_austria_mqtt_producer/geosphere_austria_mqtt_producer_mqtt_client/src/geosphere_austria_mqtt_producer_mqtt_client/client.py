...
def get_default_topic_mappings_at_geosphere_tawes_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the at.geosphere.tawes.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "at.geosphere.tawes.mqtt.WeatherStation": "weather/at/geosphere/geosphere-austria/{bundesland}/{station_id}/info",
        "at.geosphere.tawes.mqtt.WeatherObservation": "weather/at/geosphere/geosphere-austria/{bundesland}/{station_id}/observation",
    }


def get_subscription_topics_at_geosphere_tawes_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_at_geosphere_tawes_mqtt()
    topics = []
...
class AtGeosphereTawesMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the at.geosphere.tawes.mqtt message group."""
    ...
    def __init__(
        ...
        self.at_geosphere_tawes_mqtt_weather_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, geosphere_austria_mqtt_producer_data.WeatherStation, Dict[str, str]], Awaitable[None]]] = None
        
        self.at_geosphere_tawes_mqtt_weather_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, geosphere_austria_mqtt_producer_data.WeatherObservation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "at.geosphere.tawes.WeatherStation":
            if self.at_geosphere_tawes_mqtt_weather_station_async:
                try:
                    ...
                    data = geosphere_austria_mqtt_producer_data.WeatherStation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "at.geosphere.tawes.mqtt.WeatherStation")
                    await self.at_geosphere_tawes_mqtt_weather_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in at_geosphere_tawes_mqtt_weather_station handler: {e}")
            return
        
        if event_type == "at.geosphere.tawes.WeatherObservation":
            if self.at_geosphere_tawes_mqtt_weather_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = geosphere_austria_mqtt_producer_data.WeatherObservation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "at.geosphere.tawes.mqtt.WeatherObservation")
                    await self.at_geosphere_tawes_mqtt_weather_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in at_geosphere_tawes_mqtt_weather_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_at_geosphere_tawes_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_at_geosphere_tawes_mqtt_weather_station(self,
        station_id: str,
        bundesland: str,
        data: geosphere_austria_mqtt_producer_data.WeatherStation,
        ...
        """
        Publish the 'at.geosphere.tawes.mqtt.WeatherStation' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'weather/at/geosphere/geosphere-austria/{bundesland}/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "weather/at/geosphere/geosphere-austria/{bundesland}/{station_id}/info"
        ...
        attributes = {
             "type":"at.geosphere.tawes.WeatherStation",
             "source":"https://dataset.api.hub.geosphere.at/v1/station/current/tawes-v1-10min/metadata",
    ...
    async def publish_at_geosphere_tawes_mqtt_weather_observation(self,
        station_id: str,
        bundesland: str,
        data: geosphere_austria_mqtt_producer_data.WeatherObservation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'at.geosphere.tawes.mqtt.WeatherObservation' event to an MQTT topic.
        ...
        Args:
            ...
            bundesland: URI template variable for 'bundesland'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/at/geosphere/geosphere-austria/{bundesland}/{station_id}/observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "weather/at/geosphere/geosphere-austria/{bundesland}/{station_id}/observation"
        _topic_template_values: Dict[str, str] = {
            "station_id": str(station_id),
            "bundesland": str(bundesland),
        }
        ...
        attributes = {
             "type":"at.geosphere.tawes.WeatherObservation",
             "source":"https://dataset.api.hub.geosphere.at/v1/station/current/tawes-v1-10min",
             "subject":"{station_id}".format(station_id = station_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
