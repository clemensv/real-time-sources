...
import os
import signal
from geosphere_austria_mqtt_producer_data import *
from geosphere_austria_mqtt_producer_mqtt_client.client import AtGeosphereTawesMqttProducer, AtGeosphereTawesMqttDispatcher

async def handle_at_geosphere_tawes_mqtt_weather_station(mqtt_msg,cloud_event, at_geosphere_tawes_mqtt_weather_station_data):
    """ Handles the at.geosphere.tawes.mqtt.WeatherStation message """
    print(f"Received at.geosphere.tawes.mqtt.WeatherStation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {at_geosphere_tawes_mqtt_weather_station_data}")

async def handle_at_geosphere_tawes_mqtt_weather_observation(mqtt_msg,cloud_event, at_geosphere_tawes_mqtt_weather_observation_data):
    """ Handles the at.geosphere.tawes.mqtt.WeatherObservation message """
    print(f"Received at.geosphere.tawes.mqtt.WeatherObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {at_geosphere_tawes_mqtt_weather_observation_data}")
