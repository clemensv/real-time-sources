# Laqn_london_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the laqn_london_mqtt_producer message definitions.

# Laqn_london_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - UkKclLaqnMqttEventDispatcher,

    UkKclLaqnSpeciesMqttEventDispatcher
...
from laqn_london_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = UkKclLaqnMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### UkKclLaqnMqttEventDispatcher



# Publish message`UkKclLaqnMqttEventDispatcher` handles events for the uk.kcl.laqn.mqtt message group.
...
from laqn_london_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = UkKclLaqnMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
client = UkKclLaqnSpeciesMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe UkKclLaqnMqttEventDispatcher defines the following event handler hooks.
...
```##### `uk_kcl_laqn_mqtt_site_async`



## Test```python

uk_kcl_laqn_mqtt_site_async:  Callable[[PartitionContext, EventData, CloudEvent, Site], Awaitable[None]]
...
- `data`: The event data of type `laqn_london_mqtt_producer_data.Site`.

Example:

```python
async def uk_kcl_laqn_mqtt_site_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: Site) -> None:
...
```##### `uk_kcl_laqn_mqtt_measurement_async`



## Test```python

uk_kcl_laqn_mqtt_measurement_async:  Callable[[PartitionContext, EventData, CloudEvent, Measurement], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `uk.kcl.laqn.mqtt.Measurement`: LAQN hourly pollutant measurement for a site and
species at a GMT timestamp.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `laqn_london_mqtt_producer_data.Measurement`.

Example:

```python
async def uk_kcl_laqn_mqtt_measurement_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `uk_kcl_laqn_mqtt_daily_index_async`



## Test```python

uk_kcl_laqn_mqtt_daily_index_async:  Callable[[PartitionContext, EventData, CloudEvent, DailyIndex], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `laqn_london_mqtt_producer_data.DailyIndex`.

Example:

```python
async def uk_kcl_laqn_mqtt_daily_index_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: DailyIndex) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = UkKclLaqnSpeciesMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### UkKclLaqnSpeciesMqttEventDispatcher



# Publish message`UkKclLaqnSpeciesMqttEventDispatcher` handles events for the uk.kcl.laqn.species.mqtt message group.
...
## BuildThe UkKclLaqnSpeciesMqttEventDispatcher defines the following event handler hooks.
...
```##### `uk_kcl_laqn_species_mqtt_species_async`



## Test```python

uk_kcl_laqn_species_mqtt_species_async:  Callable[[PartitionContext, EventData, CloudEvent, Species], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `uk.kcl.laqn.species.mqtt.Species`: LAQN pollutant reference data, including
descriptive text and health guidance for a pollutant code.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `laqn_london_mqtt_producer_data.Species`.

Example:

```python
async def uk_kcl_laqn_species_mqtt_species_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Species) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
