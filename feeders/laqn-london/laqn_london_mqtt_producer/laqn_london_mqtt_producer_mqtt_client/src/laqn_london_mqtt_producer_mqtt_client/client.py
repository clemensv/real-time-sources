...
def get_default_topic_mappings_uk_kcl_laqn_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the uk.kcl.laqn.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "uk.kcl.laqn.mqtt.Site": "air-quality/gb/london/laqn-london/{borough}/{site_code}/{species_code}/info",
        "uk.kcl.laqn.mqtt.Measurement": "air-quality/gb/london/laqn-london/{borough}/{site_code}/{species_code}/measurement",
        "uk.kcl.laqn.mqtt.DailyIndex": "air-quality/gb/london/laqn-london/{borough}/{site_code}/{species_code}/daily-index",
    }


def get_subscription_topics_uk_kcl_laqn_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_uk_kcl_laqn_mqtt()
    topics = []
...
class UkKclLaqnMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the uk.kcl.laqn.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_uk_kcl_laqn_mqtt()
        ...
        self.uk_kcl_laqn_mqtt_site_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, laqn_london_mqtt_producer_data.Site, Dict[str, str]], Awaitable[None]]] = None
        
        self.uk_kcl_laqn_mqtt_measurement_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, laqn_london_mqtt_producer_data.Measurement, Dict[str, str]], Awaitable[None]]] = None
        
        self.uk_kcl_laqn_mqtt_daily_index_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, laqn_london_mqtt_producer_data.DailyIndex, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "uk.kcl.laqn.Site":
            if self.uk_kcl_laqn_mqtt_site_async:
                try:
                    ...
                    data = laqn_london_mqtt_producer_data.Site.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.kcl.laqn.mqtt.Site")
                    await self.uk_kcl_laqn_mqtt_site_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_kcl_laqn_mqtt_site handler: {e}")
            return
        
        if event_type == "uk.kcl.laqn.Measurement":
            if self.uk_kcl_laqn_mqtt_measurement_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = laqn_london_mqtt_producer_data.Measurement.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.kcl.laqn.mqtt.Measurement")
                    await self.uk_kcl_laqn_mqtt_measurement_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_kcl_laqn_mqtt_measurement handler: {e}")
            return
        
        if event_type == "uk.kcl.laqn.DailyIndex":
            if self.uk_kcl_laqn_mqtt_daily_index_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = laqn_london_mqtt_producer_data.DailyIndex.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.kcl.laqn.mqtt.DailyIndex")
                    await self.uk_kcl_laqn_mqtt_daily_index_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_kcl_laqn_mqtt_daily_index handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_uk_kcl_laqn_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_uk_kcl_laqn_mqtt_site(self,
        site_code: str,
        borough: str,
        species_code: str,
        data: laqn_london_mqtt_producer_data.Site,
        ...
        """
        Publish the 'uk.kcl.laqn.mqtt.Site' event to an MQTT topic.

        Args:
        
            site_code: URI template variable for 'site_code'
            borough: URI template variable for 'borough'
            species_code: URI template variable for 'species_code'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/gb/london/laqn-london/{borough}/{site_code}/{species_code}/info'
        ...
        """
        target_topic = topic if topic is not None else "air-quality/gb/london/laqn-london/{borough}/{site_code}/{species_code}/info"
        ...
        attributes = {
             ...
             "subject":"{site_code}".format(site_code = site_code)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_uk_kcl_laqn_mqtt_measurement(self,
        site_code: str,
        borough: str,
        species_code: str,
        data: laqn_london_mqtt_producer_data.Measurement,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'uk.kcl.laqn.mqtt.Measurement' event to an MQTT topic.
        ...
        Args:
            ...
            species_code: URI template variable for 'species_code'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/gb/london/laqn-london/{borough}/{site_code}/{species_code}/measurement'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/gb/london/laqn-london/{borough}/{site_code}/{species_code}/measurement"
        _topic_template_values: Dict[str, str] = {
            "site_code": str(site_code),
            "borough": str(borough),
            "species_code": str(species_code),
    ...
    async def publish_uk_kcl_laqn_mqtt_daily_index(self,
        site_code: str,
        borough: str,
        species_code: str,
        data: laqn_london_mqtt_producer_data.DailyIndex,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'uk.kcl.laqn.mqtt.DailyIndex' event to an MQTT topic.
        ...
        Args:
            ...
            species_code: URI template variable for 'species_code'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/gb/london/laqn-london/{borough}/{site_code}/{species_code}/daily-index'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/gb/london/laqn-london/{borough}/{site_code}/{species_code}/daily-index"
        _topic_template_values: Dict[str, str] = {
            "site_code": str(site_code),
            "borough": str(borough),
            "species_code": str(species_code),
...
def get_default_topic_mappings_uk_kcl_laqn_species_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the uk.kcl.laqn.species.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "uk.kcl.laqn.species.mqtt.Species": "air-quality/gb/london/laqn-london/{borough}/{site_code}/{species_code}/species",
    }


def get_subscription_topics_uk_kcl_laqn_species_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_uk_kcl_laqn_species_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class UkKclLaqnSpeciesMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the uk.kcl.laqn.species.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_uk_kcl_laqn_species_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        
        # Message handler callbacks (Dispatcher pattern)
        
        self.uk_kcl_laqn_species_mqtt_species_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, laqn_london_mqtt_producer_data.Species, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "uk.kcl.laqn.Species":
            if self.uk_kcl_laqn_species_mqtt_species_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = laqn_london_mqtt_producer_data.Species.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.kcl.laqn.species.mqtt.Species")
                    await self.uk_kcl_laqn_species_mqtt_species_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_kcl_laqn_species_mqtt_species handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_uk_kcl_laqn_species_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_uk_kcl_laqn_species_mqtt_species(self,
        species_code: str,
        borough: str,
        site_code: str,
        data: laqn_london_mqtt_producer_data.Species,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'uk.kcl.laqn.species.mqtt.Species' event to an MQTT topic.
        ...
        Args:
            ...
            site_code: URI template variable for 'site_code'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/gb/london/laqn-london/{borough}/{site_code}/{species_code}/species'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/gb/london/laqn-london/{borough}/{site_code}/{species_code}/species"
        _topic_template_values: Dict[str, str] = {
            "species_code": str(species_code),
            "borough": str(borough),
            "site_code": str(site_code),
        ...
        attributes = {
             ...
             "subject":"{species_code}".format(species_code = species_code)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
