...
def test_uk_kcl_laqn_mqtt_ukkcllaqnmqttsite(kafka_emulator):
    """Test the UkKclLaqnMqttSite event from the Uk.Kcl.Laqn.Mqtt message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_uk_kcl_laqn_mqtt_ukkcllaqnmqttsite',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = UkKclLaqnMqttEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    event_data = Test_Site.create_instance()
...
def test_uk_kcl_laqn_mqtt_ukkcllaqnmqttmeasurement(kafka_emulator):
    """Test the UkKclLaqnMqttMeasurement event from the Uk.Kcl.Laqn.Mqtt message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_uk_kcl_laqn_mqtt_ukkcllaqnmqttmeasurement',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
...
def test_uk_kcl_laqn_mqtt_ukkcllaqnmqttdailyindex(kafka_emulator):
    """Test the UkKclLaqnMqttDailyIndex event from the Uk.Kcl.Laqn.Mqtt message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_uk_kcl_laqn_mqtt_ukkcllaqnmqttdailyindex',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
...
def test_uk_kcl_laqn_amqp_ukkcllaqnamqpsite(kafka_emulator):
    """Test the UkKclLaqnAmqpSite event from the Uk.Kcl.Laqn.Amqp message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_uk_kcl_laqn_amqp_ukkcllaqnamqpsite',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = UkKclLaqnAmqpEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    event_data = Test_Site.create_instance()
...
def test_uk_kcl_laqn_amqp_ukkcllaqnamqpmeasurement(kafka_emulator):
    """Test the UkKclLaqnAmqpMeasurement event from the Uk.Kcl.Laqn.Amqp message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_uk_kcl_laqn_amqp_ukkcllaqnamqpmeasurement',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
...
def test_uk_kcl_laqn_amqp_ukkcllaqnamqpdailyindex(kafka_emulator):
    """Test the UkKclLaqnAmqpDailyIndex event from the Uk.Kcl.Laqn.Amqp message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_uk_kcl_laqn_amqp_ukkcllaqnamqpdailyindex',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
...
def test_uk_kcl_laqn_species_mqtt_ukkcllaqnspeciesmqttspecies(kafka_emulator):
    """Test the UkKclLaqnSpeciesMqttSpecies event from the Uk.Kcl.Laqn.Species.Mqtt message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_uk_kcl_laqn_species_mqtt_ukkcllaqnspeciesmqttspecies',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = UkKclLaqnSpeciesMqttEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    event_data = Test_Species.create_instance()
...
def test_uk_kcl_laqn_species_amqp_ukkcllaqnspeciesamqpspecies(kafka_emulator):
    """Test the UkKclLaqnSpeciesAmqpSpecies event from the Uk.Kcl.Laqn.Species.Amqp message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_uk_kcl_laqn_species_amqp_ukkcllaqnspeciesamqpspecies',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = UkKclLaqnSpeciesAmqpEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    event_data = Test_Species.create_instance()
