...
import hubeau_hydrometrie_amqp_producer_data
from hubeau_hydrometrie_amqp_producer_amqp_producer.producer import *
...
def feed(host, port, address='hubeau-hydrometrie', username=None, password=None, tls=False, content_mode='binary', auth_mode='password', entra_audience='https://servicebus.azure.net/.default', entra_client_id=None, sas_key_name=None, sas_key=None, once=False):
    ...
    try:
        for method in [getattr(producer,n) for n in dir(producer) if n.startswith('send_') and not n.endswith('_batch')]:
            method(**_required_call_kwargs(method, hubeau_hydrometrie_amqp_producer_data))
    finally:
...
def main(argv=None):
    ...
    p=argparse.ArgumentParser(description='hubeau-hydrometrie AMQP 1.0 bridge')
    ...
    f.add_argument('--address', default=os.getenv('AMQP_ADDRESS','hubeau-hydrometrie'))
