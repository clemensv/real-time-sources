...
"""
Tests for hubeau_hydrometrie_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../hubeau_hydrometrie_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../hubeau_hydrometrie_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../hubeau_hydrometrie_amqp_producer_amqp_producer/src')))

from hubeau_hydrometrie_amqp_producer_amqp_producer import *
...
class TestFRGovEaufranceHubEauHydrometrieAmqpProducer:
    """Test cases for FRGovEaufranceHubEauHydrometrieAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(FRGovEaufranceHubEauHydrometrieAmqpProducer)
    ...
    def test_send_station(self, artemis_container):
        ...
        try:
            ...
            for i in range(5):
                ...
                assert annotations.get(symbol('x-opt-partition-key')) == str("{code_station}".format(code_station="value"))[:128]
        finally:
            producer.close()
