    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from hubeau_hydrometrie_amqp_producer_data import Station
from hubeau_hydrometrie_amqp_producer_data import Observation

class FRGovEaufranceHubEauHydrometrieAmqpProducer:
    """
    Producer class to send messages in the `FR.Gov.Eaufrance.HubEau.Hydrometrie.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        ...
        """
        Send the `FR.Gov.Eaufrance.HubEau.Hydrometrie.amqp.Station` message
        A reference record for one French hydrometric monitoring station published by the French Eaufrance Hub'Eau hydrometry API. It fires when the bridge publishes or refreshes the station catalog so consumers can interpret measurement events.
        ...
        attributes = {
            ...
            "subject":
            "{code_station}".format(code_station=_code_station),
        }
        ...
        amqp_msg.subject = "{code_station}".format(code_station=_code_station)
        ...
        annotations = {}
        annotation_value = "{code_station}".format(code_station=_code_station)
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `FR.Gov.Eaufrance.HubEau.Hydrometrie.amqp.Station` messages
    ...
    def send_observation(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `FR.Gov.Eaufrance.HubEau.Hydrometrie.amqp.Observation` message
        A current measurement from the French Eaufrance Hub'Eau hydrometry API for one monitoring site. It carries water height and discharge observations when the upstream feed reports a new or refreshed value.
    ...
    def send_observation_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `FR.Gov.Eaufrance.HubEau.Hydrometrie.amqp.Observation` messages
