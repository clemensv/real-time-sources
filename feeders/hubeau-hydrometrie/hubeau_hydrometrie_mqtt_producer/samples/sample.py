...
import os
import signal
from hubeau_hydrometrie_mqtt_producer_data import *
from hubeau_hydrometrie_mqtt_producer_mqtt_client.client import FRGovEaufranceHubEauHydrometrieMqttProducer, FRGovEaufranceHubEauHydrometrieMqttDispatcher

async def handle_fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_station(mqtt_msg,cloud_event, fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_station_data):
    """ Handles the FR.Gov.Eaufrance.HubEau.Hydrometrie.mqtt.Station message """
    print(f"Received FR.Gov.Eaufrance.HubEau.Hydrometrie.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_station_data}")

async def handle_fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_observation(mqtt_msg,cloud_event, fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_observation_data):
    """ Handles the FR.Gov.Eaufrance.HubEau.Hydrometrie.mqtt.Observation message """
    print(f"Received FR.Gov.Eaufrance.HubEau.Hydrometrie.mqtt.Observation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_observation_data}")
