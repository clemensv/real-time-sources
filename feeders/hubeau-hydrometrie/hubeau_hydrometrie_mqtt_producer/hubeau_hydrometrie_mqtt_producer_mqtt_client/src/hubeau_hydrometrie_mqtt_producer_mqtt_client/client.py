...
def get_default_topic_mappings_fr_gov_eaufrance_hubeau_hydrometrie_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the FR.Gov.Eaufrance.HubEau.Hydrometrie.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "FR.Gov.Eaufrance.HubEau.Hydrometrie.mqtt.Station": "hydro/fr/hubeau/hubeau-hydrometrie/{basin}/{code_station}/info",
        "FR.Gov.Eaufrance.HubEau.Hydrometrie.mqtt.Observation": "hydro/fr/hubeau/hubeau-hydrometrie/{basin}/{code_station}/observation",
    }


def get_subscription_topics_fr_gov_eaufrance_hubeau_hydrometrie_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_fr_gov_eaufrance_hubeau_hydrometrie_mqtt()
    topics = []
...
class FRGovEaufranceHubEauHydrometrieMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the FR.Gov.Eaufrance.HubEau.Hydrometrie.mqtt message group."""
    ...
    def __init__(
        ...
        self.fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, hubeau_hydrometrie_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, hubeau_hydrometrie_mqtt_producer_data.Observation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "FR.Gov.Eaufrance.HubEau.Hydrometrie.Station":
            if self.fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_station_async:
                try:
                    ...
                    data = hubeau_hydrometrie_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "FR.Gov.Eaufrance.HubEau.Hydrometrie.mqtt.Station")
                    await self.fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_station handler: {e}")
            return
        
        if event_type == "FR.Gov.Eaufrance.HubEau.Hydrometrie.Observation":
            if self.fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = hubeau_hydrometrie_mqtt_producer_data.Observation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "FR.Gov.Eaufrance.HubEau.Hydrometrie.mqtt.Observation")
                    await self.fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_fr_gov_eaufrance_hubeau_hydrometrie_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_station(self,
        code_station: str,
        basin: str,
        data: hubeau_hydrometrie_mqtt_producer_data.Station,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'FR.Gov.Eaufrance.HubEau.Hydrometrie.mqtt.Station' event to an MQTT topic.

        Args:
        
            code_station: URI template variable for 'code_station'
            basin: URI template variable for 'basin'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/fr/hubeau/hubeau-hydrometrie/{basin}/{code_station}/info'
        ...
        """
        target_topic = topic if topic is not None else "hydro/fr/hubeau/hubeau-hydrometrie/{basin}/{code_station}/info"
        ...
        attributes = {
             ...
             "subject":"{code_station}".format(code_station = code_station)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_observation(self,
        code_station: str,
        basin: str,
        data: hubeau_hydrometrie_mqtt_producer_data.Observation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'FR.Gov.Eaufrance.HubEau.Hydrometrie.mqtt.Observation' event to an MQTT topic.
        ...
        Args:
            ...
            basin: URI template variable for 'basin'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/fr/hubeau/hubeau-hydrometrie/{basin}/{code_station}/observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "hydro/fr/hubeau/hubeau-hydrometrie/{basin}/{code_station}/observation"
        _topic_template_values: Dict[str, str] = {
            "code_station": str(code_station),
            "basin": str(basin),
        }
