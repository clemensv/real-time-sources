# Hubeau_hydrometrie_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the hubeau_hydrometrie_mqtt_producer message definitions.

# Hubeau_hydrometrie_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - FRGovEaufranceHubEauHydrometrieMqttEventDispatcher
...
from hubeau_hydrometrie_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = FRGovEaufranceHubEauHydrometrieMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### FRGovEaufranceHubEauHydrometrieMqttEventDispatcher



# Publish message`FRGovEaufranceHubEauHydrometrieMqttEventDispatcher` handles events for the
FR.Gov.Eaufrance.HubEau.Hydrometrie.mqtt message group.
...
from hubeau_hydrometrie_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = FRGovEaufranceHubEauHydrometrieMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The
...
## BuildThe FRGovEaufranceHubEauHydrometrieMqttEventDispatcher defines the following event handler hooks.
...
```##### `fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_station_async`



## Test```python

fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station],
Awaitable[None]]
...
- `data`: The event data of type `hubeau_hydrometrie_mqtt_producer_data.Station`.

Example:

```python
async def fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_station_event(partition_context: PartitionContext, event: EventData,
...
```##### `fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_observation_async`



## Test```python

fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_observation_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `hubeau_hydrometrie_mqtt_producer_data.Observation`.

Example:

```python
async def fr_gov_eaufrance_hub_eau_hydrometrie_mqtt_observation_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: Observation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
