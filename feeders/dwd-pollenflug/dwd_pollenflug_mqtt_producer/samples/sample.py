...
import os
import signal
from dwd_pollenflug_mqtt_producer_data import *
from dwd_pollenflug_mqtt_producer_mqtt_client.client import DEDWDPollenflugMqttProducer, DEDWDPollenflugMqttDispatcher

async def handle_de_dwd_pollenflug_mqtt_region(mqtt_msg,cloud_event, de_dwd_pollenflug_mqtt_region_data):
    """ Handles the DE.DWD.Pollenflug.mqtt.Region message """
    print(f"Received DE.DWD.Pollenflug.mqtt.Region on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {de_dwd_pollenflug_mqtt_region_data}")

async def handle_de_dwd_pollenflug_mqtt_pollen_forecast(mqtt_msg,cloud_event, de_dwd_pollenflug_mqtt_pollen_forecast_data):
    """ Handles the DE.DWD.Pollenflug.mqtt.PollenForecast message """
    print(f"Received DE.DWD.Pollenflug.mqtt.PollenForecast on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {de_dwd_pollenflug_mqtt_pollen_forecast_data}")
