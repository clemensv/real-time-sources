# Dwd_pollenflug_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the dwd_pollenflug_mqtt_producer message definitions.

# Dwd_pollenflug_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - DEDWDPollenflugMqttEventDispatcher
...
from dwd_pollenflug_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = DEDWDPollenflugMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### DEDWDPollenflugMqttEventDispatcher



# Publish message`DEDWDPollenflugMqttEventDispatcher` handles events for the DE.DWD.Pollenflug.mqtt message group.
...
from dwd_pollenflug_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = DEDWDPollenflugMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe DEDWDPollenflugMqttEventDispatcher defines the following event handler hooks.
...
```##### `de_dwd_pollenflug_mqtt_region_async`



## Test```python

de_dwd_pollenflug_mqtt_region_async:  Callable[[PartitionContext, EventData, CloudEvent, Region], Awaitable[None]]
...
- `data`: The event data of type `dwd_pollenflug_mqtt_producer_data.Region`.

Example:

```python
async def de_dwd_pollenflug_mqtt_region_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `de_dwd_pollenflug_mqtt_pollen_forecast_async`



## Test```python

de_dwd_pollenflug_mqtt_pollen_forecast_async:  Callable[[PartitionContext, EventData, CloudEvent, PollenForecast],
Awaitable[None]]
...
```Asynchronous handler hook for `DE.DWD.Pollenflug.mqtt.PollenForecast`: A forecast from Germany's Deutscher
Wetterdienst (DWD) for one area or station. It carries pollen exposure forecasts by region and plant type for the period
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dwd_pollenflug_mqtt_producer_data.PollenForecast`.

Example:

```python
async def de_dwd_pollenflug_mqtt_pollen_forecast_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: PollenForecast) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
