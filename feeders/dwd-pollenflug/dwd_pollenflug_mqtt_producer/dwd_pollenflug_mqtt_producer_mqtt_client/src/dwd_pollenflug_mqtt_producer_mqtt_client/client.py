...
def get_default_topic_mappings_de_dwd_pollenflug_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the DE.DWD.Pollenflug.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "DE.DWD.Pollenflug.mqtt.Region": "air-quality/de/dwd/dwd-pollenflug/{region_id}/info",
        "DE.DWD.Pollenflug.mqtt.PollenForecast": "air-quality/de/dwd/dwd-pollenflug/{region_id}/{pollen_type}/pollen-forecast",
    }


def get_subscription_topics_de_dwd_pollenflug_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_de_dwd_pollenflug_mqtt()
    topics = []
...
class DEDWDPollenflugMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the DE.DWD.Pollenflug.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_de_dwd_pollenflug_mqtt()
        ...
        self.de_dwd_pollenflug_mqtt_region_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_pollenflug_mqtt_producer_data.Region, Dict[str, str]], Awaitable[None]]] = None
        
        self.de_dwd_pollenflug_mqtt_pollen_forecast_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, dwd_pollenflug_mqtt_producer_data.PollenForecast, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "DE.DWD.Pollenflug.Region":
            if self.de_dwd_pollenflug_mqtt_region_async:
                try:
                    ...
                    data = dwd_pollenflug_mqtt_producer_data.Region.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.Pollenflug.mqtt.Region")
                    await self.de_dwd_pollenflug_mqtt_region_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_pollenflug_mqtt_region handler: {e}")
            return
        
        if event_type == "DE.DWD.Pollenflug.PollenForecast":
            if self.de_dwd_pollenflug_mqtt_pollen_forecast_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dwd_pollenflug_mqtt_producer_data.PollenForecast.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "DE.DWD.Pollenflug.mqtt.PollenForecast")
                    await self.de_dwd_pollenflug_mqtt_pollen_forecast_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_dwd_pollenflug_mqtt_pollen_forecast handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_de_dwd_pollenflug_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_de_dwd_pollenflug_mqtt_region(self,
        region_id: str,
        data: dwd_pollenflug_mqtt_producer_data.Region,
        ...
        """
        Publish the 'DE.DWD.Pollenflug.mqtt.Region' event to an MQTT topic.

        Args:
        
            region_id: URI template variable for 'region_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/de/dwd/dwd-pollenflug/{region_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "air-quality/de/dwd/dwd-pollenflug/{region_id}/info"
    ...
    async def publish_de_dwd_pollenflug_mqtt_pollen_forecast(self,
        region_id: str,
        pollen_type: str,
        data: dwd_pollenflug_mqtt_producer_data.PollenForecast,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'DE.DWD.Pollenflug.mqtt.PollenForecast' event to an MQTT topic.

        Args:
        
            region_id: URI template variable for 'region_id'
            pollen_type: URI template variable for 'pollen_type'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/de/dwd/dwd-pollenflug/{region_id}/{pollen_type}/pollen-forecast'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/de/dwd/dwd-pollenflug/{region_id}/{pollen_type}/pollen-forecast"
        _topic_template_values: Dict[str, str] = {
            "region_id": str(region_id),
            "pollen_type": str(pollen_type),
        }
        if _topic_template_values:
            target_topic = _apply_topic_template(target_topic, _topic_template_values)
        ...
        attributes = {
             ...
             "source":"https://opendata.dwd.de/climate_environment/health/alerts/",
             "subject":"{region_id}/{pollen_type}".format(region_id = region_id,pollen_type = pollen_type)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
