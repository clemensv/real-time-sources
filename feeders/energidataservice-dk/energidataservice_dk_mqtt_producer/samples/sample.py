...
import os
import signal
from energidataservice_dk_mqtt_producer_data import *
from energidataservice_dk_mqtt_producer_mqtt_client.client import DkEnerginetEnergidataserviceMqttProducer, DkEnerginetEnergidataserviceMqttDispatcher

async def handle_dk_energinet_energidataservice_mqtt_power_system_snapshot(mqtt_msg,cloud_event, dk_energinet_energidataservice_mqtt_power_system_snapshot_data):
    """ Handles the dk.energinet.energidataservice.mqtt.PowerSystemSnapshot message """
    print(f"Received dk.energinet.energidataservice.mqtt.PowerSystemSnapshot on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {dk_energinet_energidataservice_mqtt_power_system_snapshot_data}")

async def handle_dk_energinet_energidataservice_mqtt_spot_price(mqtt_msg,cloud_event, dk_energinet_energidataservice_mqtt_spot_price_data):
    """ Handles the dk.energinet.energidataservice.mqtt.SpotPrice message """
    print(f"Received dk.energinet.energidataservice.mqtt.SpotPrice on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {dk_energinet_energidataservice_mqtt_spot_price_data}")

async def handle_dk_energinet_energidataservice_mqtt_info(mqtt_msg,cloud_event, dk_energinet_energidataservice_mqtt_info_data):
    """ Handles the dk.energinet.energidataservice.mqtt.Info message """
    print(f"Received dk.energinet.energidataservice.mqtt.Info on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {dk_energinet_energidataservice_mqtt_info_data}")
