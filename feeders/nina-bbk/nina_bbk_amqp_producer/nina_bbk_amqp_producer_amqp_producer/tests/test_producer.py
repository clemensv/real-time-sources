...
"""
Tests for nina_bbk_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../nina_bbk_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../nina_bbk_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../nina_bbk_amqp_producer_amqp_producer/src')))

from nina_bbk_amqp_producer_amqp_producer import *
from nina_bbk_amqp_producer_data import CivilWarning
from test_civilwarning import Test_CivilWarning
...
class TestNINAWarningsAmqpProducer:
    """Test cases for NINAWarningsAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(NINAWarningsAmqpProducer)
    ...
    def test_send_civil_warning(self, artemis_container):
        """Send and receive a CivilWarning message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_CivilWarning.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                assert annotations.get(symbol('x-opt-partition-key')) == str("{warning_id}".format(warning_id="value"))[:128]
        finally:
            producer.close()

    def test_send_civil_warning_single_fresh_connection(self, artemis_container):
        """Send exactly one CivilWarning message on a fresh producer connection."""
        payload = Test_CivilWarning.create_instance()
