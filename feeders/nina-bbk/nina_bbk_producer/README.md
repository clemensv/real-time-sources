...
# Nina-bbk-producer Kafka Producer# Nina-bbk-producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - NINAWarningsEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    NINAWarningsMqttEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    NINAWarningsAmqpEventDispatcher
...
- NINAWarningsProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- NINAWarningsMqttProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- NINAWarningsAmqpProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### NINAWarningsEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`NINAWarningsEventDispatcher` handles events for the NINA.Warnings message group.
...
from nina-bbk-producer import NINAWarningsProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = NINAWarningsProducer(
...
await producer.send_nina_civil_warning(```python

    data=CivilWarning(...),add_consumer(self, consumer: KafkaConsumer)
...
The NINAWarningsEventDispatcher defines the following event handler hooks.
...
producer = NINAWarningsProducer(
    ...
    security_protocol='SASL_SSL',##### `nina_civil_warning_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'nina_civil_warning_async:  Callable[[ConsumerRecord, CloudEvent, CivilWarning],
Awaitable[None]]
...
Asynchronous handler hook for `NINA.CivilWarning`: A civil protection warning from Germany's NINA/BBK warning system.
Aggregates warnings from MOWAS (federal), KATWARN (municipal), BIWAPP (municipal), DWD (weather service), LHP (flood
centers), and police services. Each warning follows the CAP (Common Alerting Protocol) structure with multi-language
support.
...
### NINAWarningsProducer- `data`: The event data of type `nina_bbk_producer_data.CivilWarning`.



Producer for `NINA.Warnings` message group.Example:



#### Constructor```python

async def nina_civil_warning_event(record: ConsumerRecord, cloud_event: CloudEvent, data: CivilWarning) -> None:

```python    # Process the event data

NINAWarningsProducer(    await some_processing_function(record, cloud_event, data)
...
nina_warnings_dispatcher.nina_civil_warning_async = nina_civil_warning_event
...
### NINAWarningsMqttProducer- `data`: The event data of type `nina_bbk_producer_data.CivilWarning`.



Producer for `NINA.Warnings.mqtt` message group.Example:
...
NINAWarningsMqttProducer(    await some_processing_function(record, cloud_event, data)
...
nina_warnings_mqtt_dispatcher.nina_civil_warning_async = nina_civil_warning_event
...
### NINAWarningsAmqpProducer- `data`: The event data of type `nina_bbk_producer_data.CivilWarning`.



Producer for `NINA.Warnings.amqp` message group.Example:
...
NINAWarningsAmqpProducer(    await some_processing_function(record, cloud_event, data)
...
nina_warnings_amqp_dispatcher.nina_civil_warning_async = nina_civil_warning_event
...
##### `send_nina_civil_warning`Dispatchers have the following protected methods:



```python### Methods:

async def send_nina_civil_warning(
...
Send a single `NINA.CivilWarning` message. A civil protection warning from Germany's NINA/BBK warning system. Aggregates
warnings from MOWAS (federal), KATWARN (municipal), BIWAPP (municipal), DWD (weather service), LHP (flood centers), and
police services. Each warning follows the CAP (Common Alerting Protocol) structure with multi-language support.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `CivilWarning`
...
await producer.send_nina_civil_warning(

    data=CivilWarning(...),Args:
...
Send multiple `NINA.CivilWarning` messages in a batch.
...
await producer.send_nina_civil_warning_batch(```

    messages=[

        CivilWarning(...),Initializes the runner with a Kafka consumer.
...
- **Scales horizontally** across multiple brokers and partitions### NINAWarningsMqttEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`NINAWarningsMqttEventDispatcher` handles events for the NINA.Warnings.mqtt message group.
...
The NINAWarningsMqttEventDispatcher defines the following event handler hooks.
...
producer = NINAWarningsProducer(
    ...
    security_protocol='SASL_SSL',##### `nina_warnings_mqtt_civil_warning_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'nina_warnings_mqtt_civil_warning_async:  Callable[[ConsumerRecord, CloudEvent,
CivilWarning], Awaitable[None]]

)```

```

Asynchronous handler hook for `NINA.Warnings.mqtt.CivilWarning`: A civil protection warning from Germany's NINA/BBK
warning system. Aggregates warnings from MOWAS (federal), KATWARN (municipal), BIWAPP (municipal), DWD (weather
service), LHP (flood centers), and police services. Each warning follows the CAP (Common Alerting Protocol) structure
...
async def nina_warnings_mqtt_civil_warning_event(record: ConsumerRecord, cloud_event: CloudEvent, data: CivilWarning) ->
None:
...
nina_warnings_dispatcher.nina_warnings_mqtt_civil_warning_async = nina_warnings_mqtt_civil_warning_event
...
nina_warnings_mqtt_dispatcher.nina_warnings_mqtt_civil_warning_async = nina_warnings_mqtt_civil_warning_event
...
nina_warnings_amqp_dispatcher.nina_warnings_mqtt_civil_warning_async = nina_warnings_mqtt_civil_warning_event
...
##### `send_nina_warnings_mqtt_civil_warning`Dispatchers have the following protected methods:



```python### Methods:

async def send_nina_warnings_mqtt_civil_warning(
...
Send a single `NINA.Warnings.mqtt.CivilWarning` message. A civil protection warning from Germany's NINA/BBK warning
system. Aggregates warnings from MOWAS (federal), KATWARN (municipal), BIWAPP (municipal), DWD (weather service), LHP
(flood centers), and police services. Each warning follows the CAP (Common Alerting Protocol) structure with multi-
language support.Args:
...
await producer.send_nina_warnings_mqtt_civil_warning(
...
Send multiple `NINA.Warnings.mqtt.CivilWarning` messages in a batch.
...
await producer.send_nina_warnings_mqtt_civil_warning_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### NINAWarningsAmqpEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`NINAWarningsAmqpEventDispatcher` handles events for the NINA.Warnings.amqp message group.
...
The NINAWarningsAmqpEventDispatcher defines the following event handler hooks.
...
producer = NINAWarningsProducer(
    ...
    security_protocol='SASL_SSL',##### `nina_warnings_amqp_civil_warning_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'nina_warnings_amqp_civil_warning_async:  Callable[[ConsumerRecord, CloudEvent,
CivilWarning], Awaitable[None]]
...
Asynchronous handler hook for `NINA.Warnings.amqp.CivilWarning`: A civil protection warning from Germany's NINA/BBK
warning system. Aggregates warnings from MOWAS (federal), KATWARN (municipal), BIWAPP (municipal), DWD (weather
service), LHP (flood centers), and police services. Each warning follows the CAP (Common Alerting Protocol) structure
with multi-language support.
...
async def nina_warnings_amqp_civil_warning_event(record: ConsumerRecord, cloud_event: CloudEvent, data: CivilWarning) ->
None:
...
nina_warnings_dispatcher.nina_warnings_amqp_civil_warning_async = nina_warnings_amqp_civil_warning_event
...
nina_warnings_mqtt_dispatcher.nina_warnings_amqp_civil_warning_async = nina_warnings_amqp_civil_warning_event
...
nina_warnings_amqp_dispatcher.nina_warnings_amqp_civil_warning_async = nina_warnings_amqp_civil_warning_event
...
##### `send_nina_warnings_amqp_civil_warning`Dispatchers have the following protected methods:



```python### Methods:

async def send_nina_warnings_amqp_civil_warning(
...
Send a single `NINA.Warnings.amqp.CivilWarning` message. A civil protection warning from Germany's NINA/BBK warning
system. Aggregates warnings from MOWAS (federal), KATWARN (municipal), BIWAPP (municipal), DWD (weather service), LHP
(flood centers), and police services. Each warning follows the CAP (Common Alerting Protocol) structure with multi-
language support.Args:
...
await producer.send_nina_warnings_amqp_civil_warning(
...
Send multiple `NINA.Warnings.amqp.CivilWarning` messages in a batch.
...
await producer.send_nina_warnings_amqp_civil_warning_batch(```
