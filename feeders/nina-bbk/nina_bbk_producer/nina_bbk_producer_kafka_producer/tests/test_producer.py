...
def test_nina_warnings_ninacivilwarning(kafka_emulator):
    ...
    for i in range(5):
        producer_instance.send_nina_civil_warning(_warning_id = f'test_{i}', _time = datetime.datetime.now(datetime.timezone.utc).isoformat(),
            data = event_data)
...
def test_nina_warnings_mqtt_ninawarningsmqttcivilwarning(kafka_emulator):
    """Test the NINAWarningsMqttCivilWarning event from the NINA.Warnings.Mqtt message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_nina_warnings_mqtt_ninawarningsmqttcivilwarning',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = NINAWarningsMqttEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    event_data = Test_CivilWarning.create_instance()
    
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_nina_warnings_mqtt_civil_warning(_warning_id = f'test_{i}', _time = datetime.datetime.now(datetime.timezone.utc).isoformat(),
            data = event_data)
...
def test_nina_warnings_amqp_ninawarningsamqpcivilwarning(kafka_emulator):
    """Test the NINAWarningsAmqpCivilWarning event from the NINA.Warnings.Amqp message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_nina_warnings_amqp_ninawarningsamqpcivilwarning',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = NINAWarningsAmqpEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    event_data = Test_CivilWarning.create_instance()
    
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_nina_warnings_amqp_civil_warning(_warning_id = f'test_{i}', _time = datetime.datetime.now(datetime.timezone.utc).isoformat(),
            data = event_data)
