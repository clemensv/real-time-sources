# Nina_bbk_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the nina_bbk_mqtt_producer message definitions.

# Nina_bbk_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - NINAWarningsMqttEventDispatcher
...
from nina_bbk_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = NINAWarningsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NINAWarningsMqttEventDispatcher



# Publish message`NINAWarningsMqttEventDispatcher` handles events for the NINA.Warnings.mqtt message group.
...
from nina_bbk_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = NINAWarningsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe NINAWarningsMqttEventDispatcher defines the following event handler hooks.
...
```##### `nina_warnings_mqtt_civil_warning_async`



## Test```python

nina_warnings_mqtt_civil_warning_async:  Callable[[PartitionContext, EventData, CloudEvent, CivilWarning],
Awaitable[None]]
...
- `data`: The event data of type `nina_bbk_mqtt_producer_data.CivilWarning`.

Example:

```python
async def nina_warnings_mqtt_civil_warning_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: CivilWarning) -> None:
