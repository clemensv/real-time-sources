...
import os
import signal
from nina_bbk_mqtt_producer_data import *
from nina_bbk_mqtt_producer_mqtt_client.client import NINAWarningsMqttProducer, NINAWarningsMqttDispatcher

async def handle_nina_warnings_mqtt_civil_warning(mqtt_msg,cloud_event, nina_warnings_mqtt_civil_warning_data):
    """ Handles the NINA.Warnings.mqtt.CivilWarning message """
    print(f"Received NINA.Warnings.mqtt.CivilWarning on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {nina_warnings_mqtt_civil_warning_data}")
