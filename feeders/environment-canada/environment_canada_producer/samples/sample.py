...
async def main(connection_string: Optional[str], producer_config: Optional[str], topic: Optional[str]):
    ...
    if connection_string:
        ...
        cagov_ecccweather_event_producer = CAGovECCCWeatherEventProducer.from_connection_string(connection_string, topic, 'binary')
    else:
        # use a Kafka producer configuration provided as JSON text
        kafka_producer = KafkaProducer(json.loads(producer_config))
        cagov_ecccweather_event_producer = CAGovECCCWeatherEventProducer(kafka_producer, topic, 'binary')

    # ---- CA.Gov.ECCC.Weather.Station ----
    # TODO: Supply event data for the CA.Gov.ECCC.Weather.Station event
    _station = Station()

    # sends the 'CA.Gov.ECCC.Weather.Station' event to Kafka topic.
    await cagov_ecccweather_event_producer.send_ca_gov_eccc_weather_station(_msc_id = 'TODO: replace me', data = _station)
    print(f"Sent 'CA.Gov.ECCC.Weather.Station' event: {_station.to_json()}")

    # ---- CA.Gov.ECCC.Weather.WeatherObservation ----
    # TODO: Supply event data for the CA.Gov.ECCC.Weather.WeatherObservation event
