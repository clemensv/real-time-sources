# Environment_canada_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the environment_canada_mqtt_producer message definitions.

# Environment_canada_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - CAGovECCCWeatherEventDispatcher,

    CAGovECCCWeatherMqttEventDispatcher,

    CAGovECCCWeatherAmqpEventDispatcher
...
from environment_canada_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = CAGovECCCWeatherMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### CAGovECCCWeatherEventDispatcher



# Publish message`CAGovECCCWeatherEventDispatcher` handles events for the CA.Gov.ECCC.Weather message group.
...
from environment_canada_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = CAGovECCCWeatherMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
client = CAGovECCCWeatherMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
Hub.Args:- `consumer_group_name`: The name of the consumer group.- `eventhubs_fully_qualified_namespace`: The fully
qualified namespace of the Event Hub.

client = CAGovECCCWeatherAmqpMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe CAGovECCCWeatherEventDispatcher defines the following event handler hooks.
...
```##### `ca_gov_eccc_weather_station_async`



## Test```python

ca_gov_eccc_weather_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `environment_canada_mqtt_producer_data.Station`.

Example:

```python
async def ca_gov_eccc_weather_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `ca_gov_eccc_weather_weather_observation_async`



## Test```python

ca_gov_eccc_weather_weather_observation_async:  Callable[[PartitionContext, EventData, CloudEvent, WeatherObservation],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `environment_canada_mqtt_producer_data.WeatherObservation`.

Example:

```python
async def ca_gov_eccc_weather_weather_observation_event(partition_context: PartitionContext, event: EventData,
...
client = CAGovECCCWeatherMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### CAGovECCCWeatherMqttEventDispatcher



# Publish message`CAGovECCCWeatherMqttEventDispatcher` handles events for the CA.Gov.ECCC.Weather.mqtt message group.
...
## BuildThe CAGovECCCWeatherMqttEventDispatcher defines the following event handler hooks.
...
```##### `ca_gov_eccc_weather_mqtt_station_async`



## Test```python

ca_gov_eccc_weather_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
```python
async def ca_gov_eccc_weather_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Station) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `ca_gov_eccc_weather_mqtt_weather_observation_async`



## Test```python

ca_gov_eccc_weather_mqtt_weather_observation_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```python
async def ca_gov_eccc_weather_mqtt_weather_observation_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: WeatherObservation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = CAGovECCCWeatherAmqpMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### CAGovECCCWeatherAmqpEventDispatcher



# Publish message`CAGovECCCWeatherAmqpEventDispatcher` handles events for the CA.Gov.ECCC.Weather.amqp message group.
...
## BuildThe CAGovECCCWeatherAmqpEventDispatcher defines the following event handler hooks.
...
```##### `ca_gov_eccc_weather_amqp_station_async`



## Test```python

ca_gov_eccc_weather_amqp_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
```python
async def ca_gov_eccc_weather_amqp_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Station) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `ca_gov_eccc_weather_amqp_weather_observation_async`



## Test```python

ca_gov_eccc_weather_amqp_weather_observation_async:  Callable[[PartitionContext, EventData, CloudEvent,
WeatherObservation], Awaitable[None]]
...
```python
async def ca_gov_eccc_weather_amqp_weather_observation_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: WeatherObservation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
