...
def get_default_topic_mappings_fi_fmi_opendata_airquality_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the fi.fmi.opendata.airquality.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "fi.fmi.opendata.airquality.mqtt.Station": "weather/fi/fmi/fmi-finland/{region}/{fmisid}/info",
        "fi.fmi.opendata.airquality.mqtt.Observation": "weather/fi/fmi/fmi-finland/{region}/{fmisid}/observation",
    }


def get_subscription_topics_fi_fmi_opendata_airquality_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_fi_fmi_opendata_airquality_mqtt()
    topics = []
...
class FiFmiOpendataAirqualityMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the fi.fmi.opendata.airquality.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_fi_fmi_opendata_airquality_mqtt()
        ...
        self.fi_fmi_opendata_airquality_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, fmi_finland_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.fi_fmi_opendata_airquality_mqtt_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, fmi_finland_mqtt_producer_data.Observation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "fi.fmi.opendata.airquality.Station":
            if self.fi_fmi_opendata_airquality_mqtt_station_async:
                try:
                    ...
                    data = fmi_finland_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.fmi.opendata.airquality.mqtt.Station")
                    await self.fi_fmi_opendata_airquality_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_fmi_opendata_airquality_mqtt_station handler: {e}")
            return
        
        if event_type == "fi.fmi.opendata.airquality.Observation":
            if self.fi_fmi_opendata_airquality_mqtt_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = fmi_finland_mqtt_producer_data.Observation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "fi.fmi.opendata.airquality.mqtt.Observation")
                    await self.fi_fmi_opendata_airquality_mqtt_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_fmi_opendata_airquality_mqtt_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_fi_fmi_opendata_airquality_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_fi_fmi_opendata_airquality_mqtt_station(self,
        fmisid: str,
        region: str,
        data: fmi_finland_mqtt_producer_data.Station,
        ...
        """
        Publish the 'fi.fmi.opendata.airquality.mqtt.Station' event to an MQTT topic.

        Args:
        
            fmisid: URI template variable for 'fmisid'
            region: URI template variable for 'region'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/fi/fmi/fmi-finland/{region}/{fmisid}/info'
        ...
        """
        target_topic = topic if topic is not None else "weather/fi/fmi/fmi-finland/{region}/{fmisid}/info"
        ...
        attributes = {
             ...
             "subject":"{fmisid}".format(fmisid = fmisid)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_fi_fmi_opendata_airquality_mqtt_observation(self,
        fmisid: str,
        region: str,
        data: fmi_finland_mqtt_producer_data.Observation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'fi.fmi.opendata.airquality.mqtt.Observation' event to an MQTT topic.
        ...
        Args:
            ...
            region: URI template variable for 'region'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/fi/fmi/fmi-finland/{region}/{fmisid}/observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "weather/fi/fmi/fmi-finland/{region}/{fmisid}/observation"
        _topic_template_values: Dict[str, str] = {
            "fmisid": str(fmisid),
            "region": str(region),
        }
