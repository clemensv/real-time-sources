...
def _ensure_message_expiry(publish_kwargs: Dict[str, typing.Any], seconds: int) -> None:
    if not _MQTT5_AVAILABLE or _MqttProperties is None or _MqttPacketTypes is None:
        return
...
def get_default_topic_mappings_be_irail_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the be.irail.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "be.irail.mqtt.Station": "transit/be/irail/irail/{station_id}/info",
        "be.irail.mqtt.StationBoard": "transit/be/irail/irail/{station_id}/station-board",
        "be.irail.mqtt.ArrivalBoard": "transit/be/irail/irail/{station_id}/arrival-board",
    }


def get_subscription_topics_be_irail_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_be_irail_mqtt()
    topics = []
...
class BeIrailMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the be.irail.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_be_irail_mqtt()
        ...
        self.be_irail_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, irail_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.be_irail_mqtt_station_board_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, irail_mqtt_producer_data.StationBoard, Dict[str, str]], Awaitable[None]]] = None
        
        self.be_irail_mqtt_arrival_board_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, irail_mqtt_producer_data.ArrivalBoard, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "be.irail.Station":
            if self.be_irail_mqtt_station_async:
                try:
                    ...
                    data = irail_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "be.irail.mqtt.Station")
                    await self.be_irail_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in be_irail_mqtt_station handler: {e}")
            return
        
        if event_type == "be.irail.StationBoard":
            if self.be_irail_mqtt_station_board_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = irail_mqtt_producer_data.StationBoard.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "be.irail.mqtt.StationBoard")
                    await self.be_irail_mqtt_station_board_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in be_irail_mqtt_station_board handler: {e}")
            return
        
        if event_type == "be.irail.ArrivalBoard":
            if self.be_irail_mqtt_arrival_board_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = irail_mqtt_producer_data.ArrivalBoard.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "be.irail.mqtt.ArrivalBoard")
                    await self.be_irail_mqtt_arrival_board_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in be_irail_mqtt_arrival_board handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_be_irail_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_be_irail_mqtt_station(self,
        station_id: str,
        data: irail_mqtt_producer_data.Station,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'be.irail.mqtt.Station' event to an MQTT topic.

        Args:
        
            station_id: URI template variable for 'station_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'transit/be/irail/irail/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "transit/be/irail/irail/{station_id}/info"
    ...
    async def publish_be_irail_mqtt_station_board(self,
        station_id: str,
        data: irail_mqtt_producer_data.StationBoard,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'be.irail.mqtt.StationBoard' event to an MQTT topic.

        Args:
        
            station_id: URI template variable for 'station_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'transit/be/irail/irail/{station_id}/station-board'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "transit/be/irail/irail/{station_id}/station-board"
        _topic_template_values: Dict[str, str] = {
            "station_id": str(station_id),
        }
        if _topic_template_values:
        ...
        _ensure_message_expiry(publish_kwargs, 900)
        self.client.publish(target_topic, payload, **publish_kwargs)

    
    async def publish_be_irail_mqtt_arrival_board(self,
        station_id: str,
        data: irail_mqtt_producer_data.ArrivalBoard,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'be.irail.mqtt.ArrivalBoard' event to an MQTT topic.

        Args:
        
            station_id: URI template variable for 'station_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'transit/be/irail/irail/{station_id}/arrival-board'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "transit/be/irail/irail/{station_id}/arrival-board"
        _topic_template_values: Dict[str, str] = {
            "station_id": str(station_id),
        }
        if _topic_template_values:
