...
import os
import signal
from irail_mqtt_producer_data import *
from irail_mqtt_producer_mqtt_client.client import BeIrailMqttProducer, BeIrailMqttDispatcher

async def handle_be_irail_mqtt_station(mqtt_msg,cloud_event, be_irail_mqtt_station_data):
    """ Handles the be.irail.mqtt.Station message """
    print(f"Received be.irail.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {be_irail_mqtt_station_data}")

async def handle_be_irail_mqtt_station_board(mqtt_msg,cloud_event, be_irail_mqtt_station_board_data):
    """ Handles the be.irail.mqtt.StationBoard message """
    print(f"Received be.irail.mqtt.StationBoard on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {be_irail_mqtt_station_board_data}")

async def handle_be_irail_mqtt_arrival_board(mqtt_msg,cloud_event, be_irail_mqtt_arrival_board_data):
    """ Handles the be.irail.mqtt.ArrivalBoard message """
    print(f"Received be.irail.mqtt.ArrivalBoard on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {be_irail_mqtt_arrival_board_data}")
