# Irail_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the irail_mqtt_producer message definitions.

# Irail_mqtt_producer Event Dispatcher for Azure Event Hubs
...
from irail_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = BeIrailMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### BeIrailMqttEventDispatcher



# Publish message`BeIrailMqttEventDispatcher` handles events for the be.irail.mqtt message group.
...
from irail_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = BeIrailMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe BeIrailMqttEventDispatcher defines the following event handler hooks.
...
```##### `be_irail_mqtt_station_async`



## Test```python

be_irail_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `be.irail.mqtt.Station`: Metadata for a Belgian railway station in the NMBS/SNCB
network as provided by the iRail API. The Belgian rail network comprises approximately 600 passenger stations. Each
station has a unique UIC-derived numeric identifier assigned by the NMBS, geographic coordinates, and multilingual
names.
...
- `data`: The event data of type `irail_mqtt_producer_data.Station`.

Example:

```python
async def be_irail_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: Station) -> None:
...
```##### `be_irail_mqtt_station_board_async`



## Test```python

be_irail_mqtt_station_board_async:  Callable[[PartitionContext, EventData, CloudEvent, StationBoard], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `be.irail.mqtt.StationBoard`: A real-time departure board snapshot for a Belgian
railway station from the iRail liveboard API. Contains all currently scheduled departures from the station with real-
time delay information, platform assignments, cancellation status, and crowd-sourced occupancy levels. The board is
polled periodically and each event replaces the previous board state for the same station.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `irail_mqtt_producer_data.StationBoard`.

Example:

```python
async def be_irail_mqtt_station_board_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: StationBoard) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `be_irail_mqtt_arrival_board_async`



## Test```python

be_irail_mqtt_arrival_board_async:  Callable[[PartitionContext, EventData, CloudEvent, ArrivalBoard], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `irail_mqtt_producer_data.ArrivalBoard`.

Example:

```python
async def be_irail_mqtt_arrival_board_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: ArrivalBoard) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
