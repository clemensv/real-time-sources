...
def test_be_irail_beirailstation(kafka_emulator):
    ...
    for i in range(5):
        producer_instance.send_be_irail_station(_station_id = f'test_{i}', _time = datetime.datetime.now(datetime.timezone.utc).isoformat(),
            data = event_data)
...
def test_be_irail_beirailstationboard(kafka_emulator):
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_be_irail_station_board(_station_id = f'test_{i}', _time = datetime.datetime.now(datetime.timezone.utc).isoformat(),
            data = event_data)
...
def test_be_irail_beirailarrivalboard(kafka_emulator):
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_be_irail_arrival_board(_station_id = f'test_{i}', _time = datetime.datetime.now(datetime.timezone.utc).isoformat(),
            data = event_data)
...
def test_be_irail_mqtt_beirailmqttstation(kafka_emulator):
    """Test the BeIrailMqttStation event from the Be.Irail.Mqtt message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_be_irail_mqtt_beirailmqttstation',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = BeIrailMqttEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    event_data = Test_Station.create_instance()
    
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_be_irail_mqtt_station(_station_id = f'test_{i}', _time = datetime.datetime.now(datetime.timezone.utc).isoformat(),
            data = event_data)
...
def test_be_irail_mqtt_beirailmqttstationboard(kafka_emulator):
    """Test the BeIrailMqttStationBoard event from the Be.Irail.Mqtt message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_be_irail_mqtt_beirailmqttstationboard',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_be_irail_mqtt_station_board(_station_id = f'test_{i}', _time = datetime.datetime.now(datetime.timezone.utc).isoformat(),
            data = event_data)
...
def test_be_irail_mqtt_beirailmqttarrivalboard(kafka_emulator):
    """Test the BeIrailMqttArrivalBoard event from the Be.Irail.Mqtt message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_be_irail_mqtt_beirailmqttarrivalboard',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_be_irail_mqtt_arrival_board(_station_id = f'test_{i}', _time = datetime.datetime.now(datetime.timezone.utc).isoformat(),
            data = event_data)
...
def test_be_irail_amqp_beirailamqpstation(kafka_emulator):
    """Test the BeIrailAmqpStation event from the Be.Irail.Amqp message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_be_irail_amqp_beirailamqpstation',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = BeIrailAmqpEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    event_data = Test_Station.create_instance()
    
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_be_irail_amqp_station(_station_id = f'test_{i}', _time = datetime.datetime.now(datetime.timezone.utc).isoformat(),
            data = event_data)
...
def test_be_irail_amqp_beirailamqpstationboard(kafka_emulator):
    """Test the BeIrailAmqpStationBoard event from the Be.Irail.Amqp message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_be_irail_amqp_beirailamqpstationboard',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_be_irail_amqp_station_board(_station_id = f'test_{i}', _time = datetime.datetime.now(datetime.timezone.utc).isoformat(),
            data = event_data)
...
def test_be_irail_amqp_beirailamqparrivalboard(kafka_emulator):
    """Test the BeIrailAmqpArrivalBoard event from the Be.Irail.Amqp message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_be_irail_amqp_beirailamqparrivalboard',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_be_irail_amqp_arrival_board(_station_id = f'test_{i}', _time = datetime.datetime.now(datetime.timezone.utc).isoformat(),
            data = event_data)
