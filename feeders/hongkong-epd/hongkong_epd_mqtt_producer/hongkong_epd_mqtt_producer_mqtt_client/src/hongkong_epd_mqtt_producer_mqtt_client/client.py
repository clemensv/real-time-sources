...
import hongkong_epd_mqtt_producer_data
from hongkong_epd_mqtt_producer_data import Station
from hongkong_epd_mqtt_producer_data import AQHIReading
...
def get_default_topic_mappings_hk_gov_epd_aqhi_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the HK.Gov.EPD.AQHI.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "HK.Gov.EPD.AQHI.mqtt.Station": "aq/hk/epd/hongkong-epd/{district}/{station_id}/info",
        "HK.Gov.EPD.AQHI.mqtt.AQHIReading": "aq/hk/epd/hongkong-epd/{district}/{station_id}/aqhi",
    }


def get_subscription_topics_hk_gov_epd_aqhi_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_hk_gov_epd_aqhi_mqtt()
    topics = []
...
class HKGovEPDAQHIMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the HK.Gov.EPD.AQHI.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_hk_gov_epd_aqhi_mqtt()
        ...
        self.hk_gov_epd_aqhi_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, hongkong_epd_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.hk_gov_epd_aqhi_mqtt_aqhireading_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, hongkong_epd_mqtt_producer_data.AQHIReading, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "HK.Gov.EPD.AQHI.Station":
            if self.hk_gov_epd_aqhi_mqtt_station_async:
                try:
                    ...
                    data = hongkong_epd_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "HK.Gov.EPD.AQHI.mqtt.Station")
                    await self.hk_gov_epd_aqhi_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in hk_gov_epd_aqhi_mqtt_station handler: {e}")
            return
        
        if event_type == "HK.Gov.EPD.AQHI.AQHIReading":
            if self.hk_gov_epd_aqhi_mqtt_aqhireading_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = hongkong_epd_mqtt_producer_data.AQHIReading.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "HK.Gov.EPD.AQHI.mqtt.AQHIReading")
                    await self.hk_gov_epd_aqhi_mqtt_aqhireading_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in hk_gov_epd_aqhi_mqtt_aqhireading handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_hk_gov_epd_aqhi_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_hk_gov_epd_aqhi_mqtt_station(self,
        station_id: str,
        district: str,
        data: hongkong_epd_mqtt_producer_data.Station,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'HK.Gov.EPD.AQHI.mqtt.Station' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'aq/hk/epd/hongkong-epd/{district}/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "aq/hk/epd/hongkong-epd/{district}/{station_id}/info"
    ...
    async def publish_hk_gov_epd_aqhi_mqtt_aqhireading(self,
        station_id: str,
        district: str,
        data: hongkong_epd_mqtt_producer_data.AQHIReading,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'HK.Gov.EPD.AQHI.mqtt.AQHIReading' event to an MQTT topic.
        ...
        Args:
            ...
            district: URI template variable for 'district'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'aq/hk/epd/hongkong-epd/{district}/{station_id}/aqhi'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "aq/hk/epd/hongkong-epd/{district}/{station_id}/aqhi"
        _topic_template_values: Dict[str, str] = {
            "station_id": str(station_id),
            "district": str(district),
        }
