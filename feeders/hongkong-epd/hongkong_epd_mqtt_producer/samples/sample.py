...
import os
import signal
from hongkong_epd_mqtt_producer_data import *
from hongkong_epd_mqtt_producer_mqtt_client.client import HKGovEPDAQHIMqttProducer, HKGovEPDAQHIMqttDispatcher

async def handle_hk_gov_epd_aqhi_mqtt_station(mqtt_msg,cloud_event, hk_gov_epd_aqhi_mqtt_station_data):
    """ Handles the HK.Gov.EPD.AQHI.mqtt.Station message """
    print(f"Received HK.Gov.EPD.AQHI.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {hk_gov_epd_aqhi_mqtt_station_data}")

async def handle_hk_gov_epd_aqhi_mqtt_aqhireading(mqtt_msg,cloud_event, hk_gov_epd_aqhi_mqtt_aqhireading_data):
    """ Handles the HK.Gov.EPD.AQHI.mqtt.AQHIReading message """
    print(f"Received HK.Gov.EPD.AQHI.mqtt.AQHIReading on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {hk_gov_epd_aqhi_mqtt_aqhireading_data}")
