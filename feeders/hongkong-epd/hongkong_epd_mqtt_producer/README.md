# Hongkong_epd_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the hongkong_epd_mqtt_producer message definitions.

# Hongkong_epd_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - HKGovEPDAQHIMqttEventDispatcher
...
from hongkong_epd_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = HKGovEPDAQHIMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### HKGovEPDAQHIMqttEventDispatcher



# Publish message`HKGovEPDAQHIMqttEventDispatcher` handles events for the HK.Gov.EPD.AQHI.mqtt message group.
...
from hongkong_epd_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = HKGovEPDAQHIMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe HKGovEPDAQHIMqttEventDispatcher defines the following event handler hooks.
...
```##### `hk_gov_epd_aqhi_mqtt_station_async`



## Test```python

hk_gov_epd_aqhi_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `HK.Gov.EPD.AQHI.mqtt.Station`:
...
- `data`: The event data of type `hongkong_epd_mqtt_producer_data.Station`.

Example:

```python
async def hk_gov_epd_aqhi_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `hk_gov_epd_aqhi_mqtt_aqhireading_async`



## Test```python

hk_gov_epd_aqhi_mqtt_aqhireading_async:  Callable[[PartitionContext, EventData, CloudEvent, AQHIReading],
Awaitable[None]]
...
```Asynchronous handler hook for `HK.Gov.EPD.AQHI.mqtt.AQHIReading`:
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `hongkong_epd_mqtt_producer_data.AQHIReading`.

Example:

```python
async def hk_gov_epd_aqhi_mqtt_aqhireading_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: AQHIReading) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
