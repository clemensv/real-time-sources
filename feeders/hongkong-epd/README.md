...
[🚀 **Deploy to Azure**](https://clemensv.github.io/real-time-sources#hongkong-epd) &nbsp;·&nbsp;
[📓 **Fabric Notebook**](https://clemensv.github.io/real-time-sources#hongkong-epd/fabric-notebook) &nbsp;·&nbsp;
[🐳 **docker pull**](CONTAINER.md) &nbsp;·&nbsp;
[📑 **Event schemas**](EVENTS.md) &nbsp;·&nbsp;
[🗄️ **KQL schema**](kql/hongkong_epd.kql) &nbsp;·&nbsp;
[↗ **Upstream**](https://www.aqhi.gov.hk/)

</td></tr></table>
<!-- source-hero:end -->

This feeder turns the upstream Hong Kong EPD AQHI air-quality feed into a real-time CloudEvents stream over Apache Kafka, MQTT 5.0 (Unified Namespace), or AMQP 1.0.
...
|---|---|---|---|
| **Kafka** | `ghcr.io/clemensv/real-time-sources-hongkong-epd` | Apache Kafka 2.x compatible (including Azure Event Hubs and Microsoft Fabric Event Streams) | One topic with CloudEvents JSON and xRegistry-defined keying |
| **MQTT** | `ghcr.io/clemensv/real-time-sources-hongkong-epd-mqtt` | MQTT 5.0 broker (including Azure Event Grid MQTT namespace) | Unified-Namespace-style topic publishing with CloudEvents metadata as MQTT user properties |
| **AMQP** | `ghcr.io/clemensv/real-time-sources-hongkong-epd-amqp` | AMQP 1.0 brokers (generic + Azure Service Bus / Event Hubs) | Single AMQP node/address with binary CloudEvents |

All variants share:

- The same upstream poller semantics and dedupe model.
- The same xRegistry contract in `xreg/hongkong_epd.xreg.json`.
...
```text
hongkong-epd/
  xreg/hongkong_epd.xreg.json                # shared xRegistry contract
  hongkong_epd/                        # Kafka feeder application
  hongkong_epd_mqtt/                        # MQTT/UNS feeder application
  hongkong_epd_amqp/                        # AMQP 1.0 feeder application
  hongkong_epd_producer/               # xRegistry-generated Kafka producer
  hongkong_epd_mqtt_producer/               # xRegistry-generated MQTT producer
  hongkong_epd_amqp_producer/               # xRegistry-generated AMQP producer
...
- Outbound network access to the upstream Hong Kong EPD AQHI endpoints.
...
```bash
docker run --rm   -v "$PWD/state:/state"   -e STATE_FILE=/state/hongkong-epd.json   -e CONNECTION_STRING="<event-hubs-or-fabric-connection-string>"   ghcr.io/clemensv/real-time-sources-hongkong-epd:latest
```

### MQTT (Unified Namespace)

```bash
docker run --rm   -v "$PWD/state:/state"   -e STATE_FILE=/state/hongkong-epd.json   -e MQTT_BROKER_URL="mqtts://<broker-host>:8883"   -e MQTT_USERNAME="<username>"   -e MQTT_PASSWORD="<password>"   ghcr.io/clemensv/real-time-sources-hongkong-epd-mqtt:latest
```

### AMQP 1.0

```bash
docker run --rm   -v "$PWD/state:/state"   -e STATE_FILE=/state/hongkong-epd.json   -e AMQP_BROKER_URL="amqp://<user>:<password>@<host>:5672/hongkong-epd"   ghcr.io/clemensv/real-time-sources-hongkong-epd-amqp:latest
```
...
Hong Kong EPD AQHI targets Microsoft Fabric end-to-end: events land in a Fabric **Event Stream** (custom endpoint), an attached **Eventhouse / KQL database** materializes the contract from [`kql/`](kql/).

Two hosting models are supported. Use the deploy buttons on the [project portal](https://clemensv.github.io/real-time-sources#hongkong-epd) to launch either — both walk you through the same Fabric workspace selection and follow-up steps.

#### Fabric Notebook feeder &nbsp;<sub><i>(recommended for low-volume polling)</i></sub>

A scheduled Fabric Notebook in [`notebook/`](notebook/) runs the poller inside the Fabric workspace itself, against a per-source Fabric **Environment** that bundles the `hongkong_epd` package and the generated producer sub-packages. The Event Stream custom-endpoint connection string is looked up at runtime via the public Fabric Topology API using the workspace identity — no secrets in the notebook, no separate container host to manage. Dedupe state lives in OneLake under `/lakehouse/default/Files/feeder-state/hongkong-epd/`.
...
[![Deploy Fabric ACI](https://img.shields.io/badge/Fabric-Container%20Feeder-117865?logo=microsoftfabric&logoColor=white)](https://clemensv.github.io/real-time-sources#hongkong-epd/fabric-aci)
...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fhongkong-epd%2Fazure-template.json)
...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fhongkong-epd%2Fazure-template-with-eventhub.json)
...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fhongkong-epd%2Fazure-template-with-servicebus.json)

#### MQTT — bring your own broker

Deploy the MQTT container against an existing MQTT 5 broker (`mqtt://` or `mqtts://`). You provide the broker URL and optional username/password credentials at deploy time.

[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fhongkong-epd%2Fazure-template-mqtt.json)

#### MQTT — provision a new Event Grid namespace MQTT broker

Deploy the MQTT container together with a new [Azure Event Grid namespace](https://learn.microsoft.com/azure/event-grid/mqtt-overview) with MQTT broker enabled, a topic space rooted at `air-quality/hk/epd/hongkong-epd/#`, a user-assigned managed identity, and the **EventGrid TopicSpaces Publisher** role assignment. The feeder authenticates with MQTT v5 enhanced authentication (`OAUTH2-JWT`) — no shared keys to rotate.

[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fhongkong-epd%2Fazure-template-with-eventgrid-mqtt.json)
