...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../hongkong_epd_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../hongkong_epd_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../hongkong_epd_producer_kafka_producer/src')))
...
def test_hk_gov_epd_aqhi_hkgovepdaqhistation(kafka_emulator):
    """Test the HKGovEPDAQHIStation event from the HK.Gov.EPD.AQHI message group"""
    ...
    consumer = Consumer({
        ...
        'group.id': 'test_hk_gov_epd_aqhi_hkgovepdaqhistation',  # Unique group per test
    ...
    producer_instance = HKGovEPDAQHIEventProducer(kafka_producer, topic, 'binary')
...
def test_hk_gov_epd_aqhi_hkgovepdaqhiaqhireading(kafka_emulator):
    """Test the HKGovEPDAQHIAQHIReading event from the HK.Gov.EPD.AQHI message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_hk_gov_epd_aqhi_hkgovepdaqhiaqhireading',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
...
def test_hk_gov_epd_aqhi_cross_event_type_kafka_key(kafka_emulator):
    """Test that different event types in HK.Gov.EPD.AQHI produce the same Kafka key for the same placeholder values"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_hk_gov_epd_aqhi_cross_key',
    ...
    data2 = Test_AQHIReading.create_instance()

    producer_instance.send_hk_gov_epd_aqhi_station(_station_id = shared_key_value, data = data1)
    producer_instance.send_hk_gov_epd_aqhi_aqhireading(_station_id = shared_key_value, data = data2)
    kafka_producer.flush(timeout=5.0)
    ...
    while len(collected_keys) < 2 and time.time() < timeout:
        ...
        if msg is None or msg.error():
            continue
        cloudevent = parse_cloudevent(msg)
        if cloudevent['type'] in ["HK.Gov.EPD.AQHI.Station", "HK.Gov.EPD.AQHI.AQHIReading"]:
