...
def get_default_topic_mappings_blueskyfirehose_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the BlueskyFirehose.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "Bluesky.Feed.Post.mqtt": "social/intl/bluesky/bluesky/{collection}/{lang}/{did}/post",
        "Bluesky.Feed.Like.mqtt": "social/intl/bluesky/bluesky/{collection}/{lang}/{did}/like",
        "Bluesky.Feed.Repost.mqtt": "social/intl/bluesky/bluesky/{collection}/{lang}/{did}/repost",
        "Bluesky.Graph.Follow.mqtt": "social/intl/bluesky/bluesky/{collection}/{lang}/{did}/follow",
        "Bluesky.Graph.Block.mqtt": "social/intl/bluesky/bluesky/{collection}/{lang}/{did}/block",
        "Bluesky.Actor.Profile.mqtt": "social/intl/bluesky/bluesky/{collection}/{lang}/{did}/profile",
    }


def get_subscription_topics_blueskyfirehose_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_blueskyfirehose_mqtt()
    topics = []
...
class BlueskyFirehoseMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the BlueskyFirehose.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_blueskyfirehose_mqtt()
        ...
        self.bluesky_feed_post_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, bluesky_mqtt_producer_data.Post, Dict[str, str]], Awaitable[None]]] = None
        
        self.bluesky_feed_like_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, bluesky_mqtt_producer_data.Like, Dict[str, str]], Awaitable[None]]] = None
        
        self.bluesky_feed_repost_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, bluesky_mqtt_producer_data.Repost, Dict[str, str]], Awaitable[None]]] = None
        
        self.bluesky_graph_follow_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, bluesky_mqtt_producer_data.Follow, Dict[str, str]], Awaitable[None]]] = None
        
        self.bluesky_graph_block_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, bluesky_mqtt_producer_data.Block, Dict[str, str]], Awaitable[None]]] = None
        
        self.bluesky_actor_profile_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, bluesky_mqtt_producer_data.Profile, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "Bluesky.Feed.Post":
            if self.bluesky_feed_post_mqtt_async:
                try:
                    ...
                    data = bluesky_mqtt_producer_data.Post.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Bluesky.Feed.Post.mqtt")
                    await self.bluesky_feed_post_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in bluesky_feed_post_mqtt handler: {e}")
            return
        
        if event_type == "Bluesky.Feed.Like":
            if self.bluesky_feed_like_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = bluesky_mqtt_producer_data.Like.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Bluesky.Feed.Like.mqtt")
                    await self.bluesky_feed_like_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in bluesky_feed_like_mqtt handler: {e}")
            return
        
        if event_type == "Bluesky.Feed.Repost":
            if self.bluesky_feed_repost_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = bluesky_mqtt_producer_data.Repost.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Bluesky.Feed.Repost.mqtt")
                    await self.bluesky_feed_repost_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in bluesky_feed_repost_mqtt handler: {e}")
            return
        
        if event_type == "Bluesky.Graph.Follow":
            if self.bluesky_graph_follow_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = bluesky_mqtt_producer_data.Follow.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Bluesky.Graph.Follow.mqtt")
                    await self.bluesky_graph_follow_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in bluesky_graph_follow_mqtt handler: {e}")
            return
        
        if event_type == "Bluesky.Graph.Block":
            if self.bluesky_graph_block_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = bluesky_mqtt_producer_data.Block.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Bluesky.Graph.Block.mqtt")
                    await self.bluesky_graph_block_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in bluesky_graph_block_mqtt handler: {e}")
            return
        
        if event_type == "Bluesky.Actor.Profile":
            if self.bluesky_actor_profile_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = bluesky_mqtt_producer_data.Profile.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Bluesky.Actor.Profile.mqtt")
                    await self.bluesky_actor_profile_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in bluesky_actor_profile_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_blueskyfirehose_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_bluesky_feed_post_mqtt(self,
        firehoseurl: str,
        did: str,
        collection: str,
        lang: str,
        data: bluesky_mqtt_producer_data.Post,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Bluesky.Feed.Post.mqtt' event to an MQTT topic.

        Args:
        
            firehoseurl: URI template variable for 'firehoseurl'
            did: URI template variable for 'did'
            collection: URI template variable for 'collection'
            lang: URI template variable for 'lang'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'social/intl/bluesky/bluesky/{collection}/{lang}/{did}/post'
        ...
        """
        target_topic = topic if topic is not None else "social/intl/bluesky/bluesky/{collection}/{lang}/{did}/post"
        _topic_template_values: Dict[str, str] = {
            "firehoseurl": str(firehoseurl),
            "did": str(did),
        ...
        attributes = {
             ...
             "source":"{firehoseurl}".format(firehoseurl = firehoseurl),
             "subject":"{did}".format(did = did)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_bluesky_feed_like_mqtt(self,
        firehoseurl: str,
        did: str,
        collection: str,
        lang: str,
        data: bluesky_mqtt_producer_data.Like,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Bluesky.Feed.Like.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            lang: URI template variable for 'lang'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'social/intl/bluesky/bluesky/{collection}/{lang}/{did}/like'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (0).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "social/intl/bluesky/bluesky/{collection}/{lang}/{did}/like"
        _topic_template_values: Dict[str, str] = {
            "firehoseurl": str(firehoseurl),
            "did": str(did),
            "collection": str(collection),
    ...
    async def publish_bluesky_feed_repost_mqtt(self,
        firehoseurl: str,
        did: str,
        collection: str,
        lang: str,
        data: bluesky_mqtt_producer_data.Repost,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Bluesky.Feed.Repost.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            lang: URI template variable for 'lang'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'social/intl/bluesky/bluesky/{collection}/{lang}/{did}/repost'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (0).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "social/intl/bluesky/bluesky/{collection}/{lang}/{did}/repost"
        _topic_template_values: Dict[str, str] = {
            "firehoseurl": str(firehoseurl),
            "did": str(did),
            "collection": str(collection),
    ...
    async def publish_bluesky_graph_follow_mqtt(self,
        firehoseurl: str,
        did: str,
        collection: str,
        lang: str,
        data: bluesky_mqtt_producer_data.Follow,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Bluesky.Graph.Follow.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            lang: URI template variable for 'lang'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'social/intl/bluesky/bluesky/{collection}/{lang}/{did}/follow'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (0).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "social/intl/bluesky/bluesky/{collection}/{lang}/{did}/follow"
        _topic_template_values: Dict[str, str] = {
            "firehoseurl": str(firehoseurl),
            "did": str(did),
            "collection": str(collection),
    ...
    async def publish_bluesky_graph_block_mqtt(self,
        firehoseurl: str,
        did: str,
        collection: str,
        lang: str,
        data: bluesky_mqtt_producer_data.Block,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Bluesky.Graph.Block.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            lang: URI template variable for 'lang'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'social/intl/bluesky/bluesky/{collection}/{lang}/{did}/block'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (0).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "social/intl/bluesky/bluesky/{collection}/{lang}/{did}/block"
        _topic_template_values: Dict[str, str] = {
            "firehoseurl": str(firehoseurl),
            "did": str(did),
            "collection": str(collection),
    ...
    async def publish_bluesky_actor_profile_mqtt(self,
        firehoseurl: str,
        did: str,
        collection: str,
        lang: str,
        data: bluesky_mqtt_producer_data.Profile,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Bluesky.Actor.Profile.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            lang: URI template variable for 'lang'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'social/intl/bluesky/bluesky/{collection}/{lang}/{did}/profile'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (0).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "social/intl/bluesky/bluesky/{collection}/{lang}/{did}/profile"
        _topic_template_values: Dict[str, str] = {
            "firehoseurl": str(firehoseurl),
            "did": str(did),
            "collection": str(collection),
