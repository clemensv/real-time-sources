...
"""
Tests for bluesky_amqp_producer_amqp_producer
"""
import base64
import json
import os
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../bluesky_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../bluesky_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../bluesky_amqp_producer_amqp_producer/src')))

from bluesky_amqp_producer_amqp_producer import *
from bluesky_amqp_producer_data import Post
from test_bluesky_amqp_producer_data_post import Test_Post
from bluesky_amqp_producer_data import Like
from test_bluesky_amqp_producer_data_like import Test_Like
from bluesky_amqp_producer_data import Repost
from test_bluesky_amqp_producer_data_repost import Test_Repost
from bluesky_amqp_producer_data import Follow
from test_bluesky_amqp_producer_data_follow import Test_Follow
from bluesky_amqp_producer_data import Block
from test_bluesky_amqp_producer_data_block import Test_Block
from bluesky_amqp_producer_data import Profile
from test_bluesky_amqp_producer_data_profile import Test_Profile
...
class TestBlueskyFirehoseAmqpProducer:
    """Test cases for BlueskyFirehoseAmqpProducer"""
    ...
    def test_send_post(self, artemis_container):
        """Send and receive a Post message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Post.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "BlueskyFirehose.amqp.Post"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{did}".format(did="value")
        finally:
            producer.close()
    
    def test_send_like(self, artemis_container):
        """Send and receive a Like message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Like.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "BlueskyFirehose.amqp.Like"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_repost(self, artemis_container):
        """Send and receive a Repost message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Repost.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "BlueskyFirehose.amqp.Repost"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_follow(self, artemis_container):
        """Send and receive a Follow message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Follow.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "BlueskyFirehose.amqp.Follow"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_block(self, artemis_container):
        """Send and receive a Block message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Block.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "BlueskyFirehose.amqp.Block"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_profile(self, artemis_container):
        """Send and receive a Profile message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Profile.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "BlueskyFirehose.amqp.Profile"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
