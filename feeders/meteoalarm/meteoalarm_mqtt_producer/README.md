# Meteoalarm_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the meteoalarm_mqtt_producer message definitions.

# Meteoalarm_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - MeteoalarmWarningsMqttEventDispatcher
...
from meteoalarm_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = MeteoalarmWarningsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### MeteoalarmWarningsMqttEventDispatcher



# Publish message`MeteoalarmWarningsMqttEventDispatcher` handles events for the Meteoalarm.Warnings.mqtt message group.
...
from meteoalarm_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = MeteoalarmWarningsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe MeteoalarmWarningsMqttEventDispatcher defines the following event handler hooks.
...
```##### `meteoalarm_warnings_mqtt_weather_warning_async`



## Test```python

meteoalarm_warnings_mqtt_weather_warning_async:  Callable[[PartitionContext, EventData, CloudEvent, WeatherWarning],
Awaitable[None]]
...
- `data`: The event data of type `meteoalarm_mqtt_producer_data.WeatherWarning`.

Example:

```python
async def meteoalarm_warnings_mqtt_weather_warning_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: WeatherWarning) -> None:
