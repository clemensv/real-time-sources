...
import os
import signal
from rws_waterwebservices_mqtt_producer_data import *
from rws_waterwebservices_mqtt_producer_mqtt_client.client import NLRWSWaterwebservicesMqttProducer, NLRWSWaterwebservicesMqttDispatcher

async def handle_nl_rws_waterwebservices_mqtt_station(mqtt_msg,cloud_event, nl_rws_waterwebservices_mqtt_station_data):
    """ Handles the NL.RWS.Waterwebservices.mqtt.Station message """
    print(f"Received NL.RWS.Waterwebservices.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {nl_rws_waterwebservices_mqtt_station_data}")

async def handle_nl_rws_waterwebservices_mqtt_water_level_observation(mqtt_msg,cloud_event, nl_rws_waterwebservices_mqtt_water_level_observation_data):
    """ Handles the NL.RWS.Waterwebservices.mqtt.WaterLevelObservation message """
    print(f"Received NL.RWS.Waterwebservices.mqtt.WaterLevelObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {nl_rws_waterwebservices_mqtt_water_level_observation_data}")
