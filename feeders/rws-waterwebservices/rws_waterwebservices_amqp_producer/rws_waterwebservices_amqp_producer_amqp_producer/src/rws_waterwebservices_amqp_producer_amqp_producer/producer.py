...
class NLRWSWaterwebservicesAmqpProducer:
    """
    Producer class to send messages in the `NL.RWS.Waterwebservices.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        ...
        """
        Send the `NL.RWS.Waterwebservices.amqp.Station` message
        A reference record for one Dutch water monitoring location published by Rijkswaterstaat. It fires when the bridge publishes or refreshes the station catalog so consumers can interpret measurement events.
        ...
        attributes = {
            ...
            "subject":
            "{station_code}".format(station_code=_station_code),
        }
        ...
        amqp_msg.subject = "{station_code}".format(station_code=_station_code)
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `NL.RWS.Waterwebservices.amqp.Station` messages
    ...
    def send_water_level_observation(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `NL.RWS.Waterwebservices.amqp.WaterLevelObservation` message
        A current measurement from Rijkswaterstaat for one monitoring site. It carries water level observations when the upstream feed reports a new or refreshed value.
    ...
    def send_water_level_observation_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `NL.RWS.Waterwebservices.amqp.WaterLevelObservation` messages
