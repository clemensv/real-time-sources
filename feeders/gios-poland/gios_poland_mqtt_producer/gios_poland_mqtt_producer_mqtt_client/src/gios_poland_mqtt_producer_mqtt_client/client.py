...
def get_default_topic_mappings_pl_gov_gios_airquality_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the pl.gov.gios.airquality.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "pl.gov.gios.airquality.mqtt.Station": "air-quality/pl/gios/gios-poland/{voivodeship}/{station_id}/{pollutant}/info",
        "pl.gov.gios.airquality.mqtt.Sensor": "air-quality/pl/gios/gios-poland/{voivodeship}/{station_id}/{pollutant}/sensor",
        "pl.gov.gios.airquality.mqtt.Measurement": "air-quality/pl/gios/gios-poland/{voivodeship}/{station_id}/{pollutant}/measurement",
        "pl.gov.gios.airquality.mqtt.AirQualityIndex": "air-quality/pl/gios/gios-poland/{voivodeship}/{station_id}/{pollutant}/aqi",
    }


def get_subscription_topics_pl_gov_gios_airquality_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_pl_gov_gios_airquality_mqtt()
    topics = []
...
class PlGovGiosAirqualityMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the pl.gov.gios.airquality.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_pl_gov_gios_airquality_mqtt()
        ...
        self.pl_gov_gios_airquality_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, gios_poland_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.pl_gov_gios_airquality_mqtt_sensor_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, gios_poland_mqtt_producer_data.Sensor, Dict[str, str]], Awaitable[None]]] = None
        
        self.pl_gov_gios_airquality_mqtt_measurement_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, gios_poland_mqtt_producer_data.Measurement, Dict[str, str]], Awaitable[None]]] = None
        
        self.pl_gov_gios_airquality_mqtt_air_quality_index_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, gios_poland_mqtt_producer_data.AirQualityIndex, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "pl.gov.gios.airquality.Station":
            if self.pl_gov_gios_airquality_mqtt_station_async:
                try:
                    ...
                    data = gios_poland_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "pl.gov.gios.airquality.mqtt.Station")
                    await self.pl_gov_gios_airquality_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in pl_gov_gios_airquality_mqtt_station handler: {e}")
            return
        
        if event_type == "pl.gov.gios.airquality.Sensor":
            if self.pl_gov_gios_airquality_mqtt_sensor_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = gios_poland_mqtt_producer_data.Sensor.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "pl.gov.gios.airquality.mqtt.Sensor")
                    await self.pl_gov_gios_airquality_mqtt_sensor_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in pl_gov_gios_airquality_mqtt_sensor handler: {e}")
            return
        
        if event_type == "pl.gov.gios.airquality.Measurement":
            if self.pl_gov_gios_airquality_mqtt_measurement_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = gios_poland_mqtt_producer_data.Measurement.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "pl.gov.gios.airquality.mqtt.Measurement")
                    await self.pl_gov_gios_airquality_mqtt_measurement_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in pl_gov_gios_airquality_mqtt_measurement handler: {e}")
            return
        
        if event_type == "pl.gov.gios.airquality.AirQualityIndex":
            if self.pl_gov_gios_airquality_mqtt_air_quality_index_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = gios_poland_mqtt_producer_data.AirQualityIndex.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "pl.gov.gios.airquality.mqtt.AirQualityIndex")
                    await self.pl_gov_gios_airquality_mqtt_air_quality_index_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in pl_gov_gios_airquality_mqtt_air_quality_index handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_pl_gov_gios_airquality_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_pl_gov_gios_airquality_mqtt_station(self,
        station_id: str,
        voivodeship: str,
        pollutant: str,
        data: gios_poland_mqtt_producer_data.Station,
        ...
        """
        Publish the 'pl.gov.gios.airquality.mqtt.Station' event to an MQTT topic.

        Args:
        
            station_id: URI template variable for 'station_id'
            voivodeship: URI template variable for 'voivodeship'
            pollutant: URI template variable for 'pollutant'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/pl/gios/gios-poland/{voivodeship}/{station_id}/{pollutant}/info'
        ...
        """
        target_topic = topic if topic is not None else "air-quality/pl/gios/gios-poland/{voivodeship}/{station_id}/{pollutant}/info"
    ...
    async def publish_pl_gov_gios_airquality_mqtt_sensor(self,
        station_id: str,
        sensor_id: str,
        voivodeship: str,
        pollutant: str,
        data: gios_poland_mqtt_producer_data.Sensor,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'pl.gov.gios.airquality.mqtt.Sensor' event to an MQTT topic.
        ...
        Args:
            ...
            pollutant: URI template variable for 'pollutant'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/pl/gios/gios-poland/{voivodeship}/{station_id}/{pollutant}/sensor'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/pl/gios/gios-poland/{voivodeship}/{station_id}/{pollutant}/sensor"
        _topic_template_values: Dict[str, str] = {
            "station_id": str(station_id),
    ...
    async def publish_pl_gov_gios_airquality_mqtt_measurement(self,
        station_id: str,
        sensor_id: str,
        voivodeship: str,
        pollutant: str,
        data: gios_poland_mqtt_producer_data.Measurement,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'pl.gov.gios.airquality.mqtt.Measurement' event to an MQTT topic.
        ...
        Args:
            ...
            pollutant: URI template variable for 'pollutant'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/pl/gios/gios-poland/{voivodeship}/{station_id}/{pollutant}/measurement'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/pl/gios/gios-poland/{voivodeship}/{station_id}/{pollutant}/measurement"
        _topic_template_values: Dict[str, str] = {
            "station_id": str(station_id),
            "sensor_id": str(sensor_id),
            "voivodeship": str(voivodeship),
    ...
    async def publish_pl_gov_gios_airquality_mqtt_air_quality_index(self,
        station_id: str,
        voivodeship: str,
        pollutant: str,
        data: gios_poland_mqtt_producer_data.AirQualityIndex,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'pl.gov.gios.airquality.mqtt.AirQualityIndex' event to an MQTT topic.
        ...
        Args:
            ...
            pollutant: URI template variable for 'pollutant'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/pl/gios/gios-poland/{voivodeship}/{station_id}/{pollutant}/aqi'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/pl/gios/gios-poland/{voivodeship}/{station_id}/{pollutant}/aqi"
        _topic_template_values: Dict[str, str] = {
            "station_id": str(station_id),
            "voivodeship": str(voivodeship),
            "pollutant": str(pollutant),
