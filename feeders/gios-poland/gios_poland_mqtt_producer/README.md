# Gios_poland_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the gios_poland_mqtt_producer message definitions.

# Gios_poland_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - PlGovGiosAirqualityMqttEventDispatcher
...
from gios_poland_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = PlGovGiosAirqualityMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### PlGovGiosAirqualityMqttEventDispatcher



# Publish message`PlGovGiosAirqualityMqttEventDispatcher` handles events for the pl.gov.gios.airquality.mqtt message
group.
...
from gios_poland_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = PlGovGiosAirqualityMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe PlGovGiosAirqualityMqttEventDispatcher defines the following event handler hooks.
...
```##### `pl_gov_gios_airquality_mqtt_station_async`



## Test```python

pl_gov_gios_airquality_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station],
Awaitable[None]]
...
- `data`: The event data of type `gios_poland_mqtt_producer_data.Station`.

Example:

```python
async def pl_gov_gios_airquality_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `pl_gov_gios_airquality_mqtt_sensor_async`



## Test```python

pl_gov_gios_airquality_mqtt_sensor_async:  Callable[[PartitionContext, EventData, CloudEvent, Sensor], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `gios_poland_mqtt_producer_data.Sensor`.

Example:

```python
async def pl_gov_gios_airquality_mqtt_sensor_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Sensor) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `pl_gov_gios_airquality_mqtt_measurement_async`



## Test```python

pl_gov_gios_airquality_mqtt_measurement_async:  Callable[[PartitionContext, EventData, CloudEvent, Measurement],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `gios_poland_mqtt_producer_data.Measurement`.

Example:

```python
async def pl_gov_gios_airquality_mqtt_measurement_event(partition_context: PartitionContext, event: EventData,
...
```##### `pl_gov_gios_airquality_mqtt_air_quality_index_async`



## Test```python

pl_gov_gios_airquality_mqtt_air_quality_index_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `gios_poland_mqtt_producer_data.AirQualityIndex`.

Example:

```python
async def pl_gov_gios_airquality_mqtt_air_quality_index_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: AirQualityIndex) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
