...
import os
import signal
from gios_poland_mqtt_producer_data import *
from gios_poland_mqtt_producer_mqtt_client.client import PlGovGiosAirqualityMqttProducer, PlGovGiosAirqualityMqttDispatcher

async def handle_pl_gov_gios_airquality_mqtt_station(mqtt_msg,cloud_event, pl_gov_gios_airquality_mqtt_station_data):
    """ Handles the pl.gov.gios.airquality.mqtt.Station message """
    print(f"Received pl.gov.gios.airquality.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {pl_gov_gios_airquality_mqtt_station_data}")

async def handle_pl_gov_gios_airquality_mqtt_sensor(mqtt_msg,cloud_event, pl_gov_gios_airquality_mqtt_sensor_data):
    """ Handles the pl.gov.gios.airquality.mqtt.Sensor message """
    print(f"Received pl.gov.gios.airquality.mqtt.Sensor on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {pl_gov_gios_airquality_mqtt_sensor_data}")

async def handle_pl_gov_gios_airquality_mqtt_measurement(mqtt_msg,cloud_event, pl_gov_gios_airquality_mqtt_measurement_data):
    """ Handles the pl.gov.gios.airquality.mqtt.Measurement message """
    print(f"Received pl.gov.gios.airquality.mqtt.Measurement on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {pl_gov_gios_airquality_mqtt_measurement_data}")

async def handle_pl_gov_gios_airquality_mqtt_air_quality_index(mqtt_msg,cloud_event, pl_gov_gios_airquality_mqtt_air_quality_index_data):
    """ Handles the pl.gov.gios.airquality.mqtt.AirQualityIndex message """
    print(f"Received pl.gov.gios.airquality.mqtt.AirQualityIndex on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {pl_gov_gios_airquality_mqtt_air_quality_index_data}")
