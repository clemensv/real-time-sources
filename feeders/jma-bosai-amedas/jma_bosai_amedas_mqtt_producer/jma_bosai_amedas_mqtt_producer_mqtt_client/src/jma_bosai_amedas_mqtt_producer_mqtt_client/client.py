...
def get_default_topic_mappings_jp_jma_amedas_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the JP.JMA.Amedas.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "JP.JMA.Amedas.mqtt.Station": "weather/jp/jma/jma-bosai-amedas/{prefecture}/{station_code}/{event}",
        "JP.JMA.Amedas.mqtt.Observation": "weather/jp/jma/jma-bosai-amedas/{prefecture}/{station_code}/{event}",
    }


def get_subscription_topics_jp_jma_amedas_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_jp_jma_amedas_mqtt()
    topics = []
...
class JPJMAAmedasMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the JP.JMA.Amedas.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_jp_jma_amedas_mqtt()
        ...
        self.jp_jma_amedas_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, jma_bosai_amedas_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.jp_jma_amedas_mqtt_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, jma_bosai_amedas_mqtt_producer_data.Observation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "JP.JMA.Amedas.Station":
            if self.jp_jma_amedas_mqtt_station_async:
                try:
                    ...
                    data = jma_bosai_amedas_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "JP.JMA.Amedas.mqtt.Station")
                    await self.jp_jma_amedas_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_jma_amedas_mqtt_station handler: {e}")
            return
        
        if event_type == "JP.JMA.Amedas.Observation":
            if self.jp_jma_amedas_mqtt_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = jma_bosai_amedas_mqtt_producer_data.Observation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "JP.JMA.Amedas.mqtt.Observation")
                    await self.jp_jma_amedas_mqtt_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_jma_amedas_mqtt_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_jp_jma_amedas_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_jp_jma_amedas_mqtt_station(self,
        feedurl: str,
        station_code: str,
        prefecture: str,
        event: str,
        data: jma_bosai_amedas_mqtt_producer_data.Station,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'JP.JMA.Amedas.mqtt.Station' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'weather/jp/jma/jma-bosai-amedas/{prefecture}/{station_code}/{event}'
        ...
        """
        target_topic = topic if topic is not None else "weather/jp/jma/jma-bosai-amedas/{prefecture}/{station_code}/{event}"
        ...
        attributes = {
             "type":"JP.JMA.Amedas.Station",
             "source":"{feedurl}".format(feedurl = feedurl),
             "subject":"jp.jma.amedas/{station_code}".format(station_code = station_code)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_jp_jma_amedas_mqtt_observation(self,
        feedurl: str,
        station_code: str,
        prefecture: str,
        event: str,
        data: jma_bosai_amedas_mqtt_producer_data.Observation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'JP.JMA.Amedas.mqtt.Observation' event to an MQTT topic.
        ...
        attributes = {
             "type":"JP.JMA.Amedas.Observation",
             "source":"{feedurl}".format(feedurl = feedurl),
             "subject":"jp.jma.amedas/{station_code}".format(station_code = station_code)
        }
        attributes["datacontenttype"] = content_type
