...
import os
import signal
from jma_bosai_amedas_mqtt_producer_data import *
from jma_bosai_amedas_mqtt_producer_mqtt_client.client import JPJMAAmedasMqttProducer, JPJMAAmedasMqttDispatcher

async def handle_jp_jma_amedas_mqtt_station(mqtt_msg,cloud_event, jp_jma_amedas_mqtt_station_data):
    """ Handles the JP.JMA.Amedas.mqtt.Station message """
    print(f"Received JP.JMA.Amedas.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_jma_amedas_mqtt_station_data}")

async def handle_jp_jma_amedas_mqtt_observation(mqtt_msg,cloud_event, jp_jma_amedas_mqtt_observation_data):
    """ Handles the JP.JMA.Amedas.mqtt.Observation message """
    print(f"Received JP.JMA.Amedas.mqtt.Observation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_jma_amedas_mqtt_observation_data}")
