# Jma_bosai_amedas_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the jma_bosai_amedas_mqtt_producer message definitions.

# Jma_bosai_amedas_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - JPJMAAmedasMqttEventDispatcher
...
from jma_bosai_amedas_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = JPJMAAmedasMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### JPJMAAmedasMqttEventDispatcher



# Publish message`JPJMAAmedasMqttEventDispatcher` handles events for the JP.JMA.Amedas.mqtt message group.
...
from jma_bosai_amedas_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = JPJMAAmedasMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe JPJMAAmedasMqttEventDispatcher defines the following event handler hooks.
...
```##### `jp_jma_amedas_mqtt_station_async`



## Test```python

jp_jma_amedas_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `JP.JMA.Amedas.mqtt.Station`: Reference event for one JMA AMeDAS station from the Bosai
...
- `data`: The event data of type `jma_bosai_amedas_mqtt_producer_data.Station`.

Example:

```python
async def jp_jma_amedas_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```python
jp_jma_amedas_mqtt_dispatcher.jp_jma_amedas_mqtt_station_async = jp_jma_amedas_mqtt_station_event
```
...
```##### `jp_jma_amedas_mqtt_observation_async`



## Test```python

jp_jma_amedas_mqtt_observation_async:  Callable[[PartitionContext, EventData, CloudEvent, Observation], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `JP.JMA.Amedas.mqtt.Observation`: Telemetry event for one JMA AMeDAS station
observation in a ten-minute Bosai map snapshot. Each measurement value is nullable because JMA publishes different
fields by station capability and by observation availability.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `jma_bosai_amedas_mqtt_producer_data.Observation`.

Example:

```python
async def jp_jma_amedas_mqtt_observation_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```python
jp_jma_amedas_mqtt_dispatcher.jp_jma_amedas_mqtt_observation_async = jp_jma_amedas_mqtt_observation_event
```
