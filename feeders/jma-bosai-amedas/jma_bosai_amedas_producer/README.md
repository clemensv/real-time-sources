...
# Jma_bosai_amedas_producer Kafka Producer# Jma_bosai_amedas_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - JPJMAAmedasEventDispatcher
...
- JPJMAAmedasProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### JPJMAAmedasEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`JPJMAAmedasEventDispatcher` handles events for the JP.JMA.Amedas message group.
...
from jma_bosai_amedas_producer import JPJMAAmedasProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = JPJMAAmedasProducer(
...
await producer.send_jp_jma_amedas_station(```python
...
The JPJMAAmedasEventDispatcher defines the following event handler hooks.
...
producer = JPJMAAmedasProducer(
    ...
    security_protocol='SASL_SSL',##### `jp_jma_amedas_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'jp_jma_amedas_station_async:  Callable[[ConsumerRecord, CloudEvent, Station],
Awaitable[None]]
...
Asynchronous handler hook for `JP.JMA.Amedas.Station`: Reference event for one JMA AMeDAS station from the Bosai
amedastable.json station table, including names, geodetic position, elevation, station capability tier, and measurement
capability bitmask.
...
### JPJMAAmedasProducer- `data`: The event data of type `jma_bosai_amedas_producer_data.Station`.



Producer for `JP.JMA.Amedas` message group.Example:



#### Constructor```python

async def jp_jma_amedas_station_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Station) -> None:

```python    # Process the event data

JPJMAAmedasProducer(    await some_processing_function(record, cloud_event, data)
...
jp_jma_amedas_dispatcher.jp_jma_amedas_station_async = jp_jma_amedas_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `jp_jma_amedas_observation_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'jp_jma_amedas_observation_async:  Callable[[ConsumerRecord, CloudEvent, Observation],
Awaitable[None]]
...
Asynchronous handler hook for `JP.JMA.Amedas.Observation`: Telemetry event for one JMA AMeDAS station observation in a
ten-minute Bosai map snapshot. Each measurement value is nullable because JMA publishes different fields by station
capability and by observation availability.
...
### JPJMAAmedasProducer- `data`: The event data of type `jma_bosai_amedas_producer_data.Observation`.
...
async def jp_jma_amedas_observation_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Observation) -> None:
...
jp_jma_amedas_dispatcher.jp_jma_amedas_observation_async = jp_jma_amedas_observation_event
...
##### `send_jp_jma_amedas_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_jp_jma_amedas_station(
...
Send a single `JP.JMA.Amedas.Station` message. Reference event for one JMA AMeDAS station from the Bosai
amedastable.json station table, including names, geodetic position, elevation, station capability tier, and measurement
capability bitmask.Args:
...
await producer.send_jp_jma_amedas_station(
...
Send multiple `JP.JMA.Amedas.Station` messages in a batch.
...
await producer.send_jp_jma_amedas_station_batch(```
...
##### `send_jp_jma_amedas_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_jp_jma_amedas_observation(
...
Send a single `JP.JMA.Amedas.Observation` message. Telemetry event for one JMA AMeDAS station observation in a ten-
minute Bosai map snapshot. Each measurement value is nullable because JMA publishes different fields by station
capability and by observation availability.Args:
...
await producer.send_jp_jma_amedas_observation(
...
Send multiple `JP.JMA.Amedas.Observation` messages in a batch.
...
await producer.send_jp_jma_amedas_observation_batch(```
