...
class LightningStrike:
    ...
    sensors: typing.Optional[str]=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="sensors"))
    latitude: float=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="latitude"))
    longitude: float=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="longitude"))

    @classmethod
    def from_serializer_dict(cls, data: dict) -> 'LightningStrike':
        """
    ...
    @classmethod
    def from_data(cls, data: typing.Any, content_type_string: typing.Optional[str] = None) -> typing.Optional['LightningStrike']:
        """
        ...
        if base_content_type == 'application/json':
            if isinstance(data, (bytes, str)):
                ...
                return LightningStrike.from_serializer_dict(_record)
            else:
                raise NotImplementedError('Data is not of a supported type for JSON deserialization')
        raise NotImplementedError(f'Unsupported media type {content_type}')

    @classmethod
    def create_instance(cls) -> 'LightningStrike':
        """
        ...
        """
        return cls(
            strike_id='bfzpfygnufmlqkptlyri',
            observed=datetime.datetime.now(datetime.timezone.utc),
            created=datetime.datetime.now(datetime.timezone.utc),
            type=int(90),
            amp=float(20.06760411121101),
            strokes=int(89),
            sensors='pqcmmvxishgnnogcccal',
            latitude=float(43.525249728382285),
            longitude=float(23.871550671232544)
        )