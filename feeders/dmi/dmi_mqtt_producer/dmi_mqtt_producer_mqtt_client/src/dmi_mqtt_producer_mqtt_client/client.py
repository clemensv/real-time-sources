...
def get_default_topic_mappings_dk_dmi_metobs_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the dk.dmi.metObs.mqtt message group.
...
def get_subscription_topics_dk_dmi_metobs_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_dk_dmi_metobs_mqtt()
    topics = []
...
class DkDmiMetObsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the dk.dmi.metObs.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_dk_dmi_metobs_mqtt()
        ...
        if event_type == "dk.dmi.metObs.MetObsObservation":
            if self.dk_dmi_met_obs_mqtt_observation_async:
                try:
                    ...
                    print(f"Error in dk_dmi_met_obs_mqtt_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_dk_dmi_metobs_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_dk_dmi_met_obs_mqtt_observation(self,
        feedurl: str,
        station_id: str,
        parameter_id: str,
...
def get_default_topic_mappings_dk_dmi_oceanobs_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the dk.dmi.oceanObs.mqtt message group.
...
def get_subscription_topics_dk_dmi_oceanobs_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_dk_dmi_oceanobs_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class DkDmiOceanObsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the dk.dmi.oceanObs.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_dk_dmi_oceanobs_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        ...
        if event_type == "dk.dmi.oceanObs.OceanStation":
            if self.dk_dmi_ocean_obs_mqtt_station_async:
                try:
                    ...
                    print(f"Error in dk_dmi_ocean_obs_mqtt_station handler: {e}")
            return
        
        if event_type == "dk.dmi.oceanObs.TidewaterStation":
            if self.dk_dmi_ocean_obs_mqtt_tidewater_station_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dmi_mqtt_producer_data.TidewaterStation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "dk.dmi.oceanObs.mqtt.TidewaterStation")
                    await self.dk_dmi_ocean_obs_mqtt_tidewater_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
        ...
        if event_type == "dk.dmi.oceanObs.OceanObservation":
            if self.dk_dmi_ocean_obs_mqtt_observation_async:
                try:
                    ...
                    print(f"Error in dk_dmi_ocean_obs_mqtt_observation handler: {e}")
            return
        
        if event_type == "dk.dmi.oceanObs.TidewaterPrediction":
            if self.dk_dmi_ocean_obs_mqtt_tidewater_prediction_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = dmi_mqtt_producer_data.TidewaterPrediction.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "dk.dmi.oceanObs.mqtt.TidewaterPrediction")
                    await self.dk_dmi_ocean_obs_mqtt_tidewater_prediction_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in dk_dmi_ocean_obs_mqtt_tidewater_prediction handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_dk_dmi_oceanobs_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_dk_dmi_ocean_obs_mqtt_tidewater_station(self,
        feedurl: str,
        station_id: str,
        data: dmi_mqtt_producer_data.TidewaterStation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
    ...
    async def publish_dk_dmi_ocean_obs_mqtt_observation(self,
        feedurl: str,
        station_id: str,
        parameter_id: str,
    ...
    async def publish_dk_dmi_ocean_obs_mqtt_tidewater_prediction(self,
        feedurl: str,
        station_id: str,
        data: dmi_mqtt_producer_data.TidewaterPrediction,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
