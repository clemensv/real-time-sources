# Dmi_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the dmi_mqtt_producer message definitions.

# Dmi_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - DkDmiMetObsMqttEventDispatcher,

    DkDmiOceanObsMqttEventDispatcher
...
from dmi_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = DkDmiMetObsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### DkDmiMetObsMqttEventDispatcher



# Publish message`DkDmiMetObsMqttEventDispatcher` handles events for the dk.dmi.metObs.mqtt message group.
...
from dmi_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = DkDmiMetObsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
client = DkDmiOceanObsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe DkDmiMetObsMqttEventDispatcher defines the following event handler hooks.
...
```##### `dk_dmi_met_obs_mqtt_station_async`
...
```Asynchronous handler hook for `dk.dmi.metObs.mqtt.Station`: Reference data for a DMI meteorological observation
...
```python
dk_dmi_met_obs_mqtt_dispatcher.dk_dmi_met_obs_mqtt_station_async = dk_dmi_met_obs_mqtt_station_event
```
...
```##### `dk_dmi_met_obs_mqtt_observation_async`
...
```Asynchronous handler hook for `dk.dmi.metObs.mqtt.Observation`: A single meteorological observation value reported by
a station for one parameter at a specific observed time. Cadence depends on parameter (10-minute or hourly).
...
```python
dk_dmi_met_obs_mqtt_dispatcher.dk_dmi_met_obs_mqtt_observation_async = dk_dmi_met_obs_mqtt_observation_event
```


## Generated Event Dispatchers

client = DkDmiOceanObsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### DkDmiOceanObsMqttEventDispatcher



# Publish message`DkDmiOceanObsMqttEventDispatcher` handles events for the dk.dmi.oceanObs.mqtt message group.
...
## BuildThe DkDmiOceanObsMqttEventDispatcher defines the following event handler hooks.
...
```##### `dk_dmi_ocean_obs_mqtt_station_async`
...
```Asynchronous handler hook for `dk.dmi.oceanObs.mqtt.Station`: Reference data for an oceanographic observation station
...
```python
dk_dmi_ocean_obs_mqtt_dispatcher.dk_dmi_ocean_obs_mqtt_station_async = dk_dmi_ocean_obs_mqtt_station_event
```
...
```##### `dk_dmi_ocean_obs_mqtt_tidewater_station_async`



## Test```python

dk_dmi_ocean_obs_mqtt_tidewater_station_async:  Callable[[PartitionContext, EventData, CloudEvent, TidewaterStation],
Awaitable[None]]
...
```Asynchronous handler hook for `dk.dmi.oceanObs.mqtt.TidewaterStation`: Reference data for a station/grid-point at
which DMI publishes tidewater (sea-level) predictions. The set partially overlaps physical tide gauges but also includes
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dmi_mqtt_producer_data.TidewaterStation`.

Example:

```python
async def dk_dmi_ocean_obs_mqtt_tidewater_station_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: TidewaterStation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
dk_dmi_ocean_obs_mqtt_dispatcher.dk_dmi_ocean_obs_mqtt_tidewater_station_async =
dk_dmi_ocean_obs_mqtt_tidewater_station_event
```
...
```##### `dk_dmi_ocean_obs_mqtt_observation_async`
...
```Asynchronous handler hook for `dk.dmi.oceanObs.mqtt.Observation`: A single oceanographic observation. Parameters are
...
```python
dk_dmi_ocean_obs_mqtt_dispatcher.dk_dmi_ocean_obs_mqtt_observation_async = dk_dmi_ocean_obs_mqtt_observation_event
```
...
```##### `dk_dmi_ocean_obs_mqtt_tidewater_prediction_async`



## Test```python

dk_dmi_ocean_obs_mqtt_tidewater_prediction_async:  Callable[[PartitionContext, EventData, CloudEvent,
TidewaterPrediction], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `dk.dmi.oceanObs.mqtt.TidewaterPrediction`: A deterministic tidewater (sea-level)
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dmi_mqtt_producer_data.TidewaterPrediction`.

Example:

```python
async def dk_dmi_ocean_obs_mqtt_tidewater_prediction_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: TidewaterPrediction) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
dk_dmi_ocean_obs_mqtt_dispatcher.dk_dmi_ocean_obs_mqtt_tidewater_prediction_async =
dk_dmi_ocean_obs_mqtt_tidewater_prediction_event
```
