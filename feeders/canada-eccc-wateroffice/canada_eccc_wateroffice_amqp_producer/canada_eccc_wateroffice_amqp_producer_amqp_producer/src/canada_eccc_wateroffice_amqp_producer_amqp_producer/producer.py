    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from canada_eccc_wateroffice_amqp_producer_data import Station
from canada_eccc_wateroffice_amqp_producer_data import Observation

class CAGovECCCHydroAmqpProducer:
    """
    Producer class to send messages in the `CA.Gov.ECCC.Hydro.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        ...
        """
        Send the `CA.Gov.ECCC.Hydro.amqp.Station` message
        A reference record for one Canadian hydrometric station published by Environment and Climate Change Canada Water Survey of Canada. It fires when the bridge publishes or refreshes the station catalog so consumers can interpret measurement events.
        ...
        attributes = {
            ...
            "subject":
            "stations/{station_number}".format(station_number=_station_number),
        }
        ...
        amqp_msg.subject = "stations/{station_number}".format(station_number=_station_number)
        ...
        annotations = {}
        annotation_value = "stations/{station_number}".format(station_number=_station_number)
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `CA.Gov.ECCC.Hydro.amqp.Station` messages
    ...
    def send_observation(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `CA.Gov.ECCC.Hydro.amqp.Observation` message
        A current measurement from Environment and Climate Change Canada Water Survey of Canada for one monitoring site. It carries real-time water level and flow observations when the upstream feed reports a new or refreshed value.
    ...
    def send_observation_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `CA.Gov.ECCC.Hydro.amqp.Observation` messages
