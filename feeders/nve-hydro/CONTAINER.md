...
This document covers the published OCI container images for the NVE Hydro feeder, their environment-variable contract, authentication modes, and one-click Azure deployments. For the project overview see [README.md](README.md); for the CloudEvents contract see [EVENTS.md](EVENTS.md).
...
The NVE Hydro source is exposed as a polling API upstream. These container images package polling cadence control, stateful dedupe, CloudEvents production, and transport/auth wiring so operators can deploy a ready-to-run feeder instead of building source-specific integration code.

## What ships in the box

| Image | Transport | Default behavior |
|---|---|---|
| `ghcr.io/clemensv/real-time-sources-nve-hydro` | Apache Kafka 2.x | JSON CloudEvents on one topic; key template `{station_id}` |
| `ghcr.io/clemensv/real-time-sources-nve-hydro-mqtt` | MQTT 5.0 | Unified-Namespace topic template `(see EVENTS.md)` |
| `ghcr.io/clemensv/real-time-sources-nve-hydro-amqp` | AMQP 1.0 | Binary CloudEvents to AMQP node `nve-hydro` |
...
| Default entry point | Kafka `["python", "-m", "nve_hydro", "feed"]`; MQTT `["python", "-m", "nve_hydro_mqtt", "feed"]`; AMQP `["python", "-m", "nve_hydro_amqp", "feed"]` |
...
docker pull ghcr.io/clemensv/real-time-sources-nve-hydro-mqtt:latest
docker pull ghcr.io/clemensv/real-time-sources-nve-hydro-amqp:latest
```
...
| Variable | Description |
|---|---|
| `NVE_API_KEY` | **Required.** API key for the NVE Hydrological API (`https://hydapi.nve.no`), also accepted via `--api-key`. The feeder logs an error and exits at startup if neither is set. |
| `MOCK_MODE` | Set to `true` to emit deterministic synthetic observations instead of calling the live NVE API (offline testing). |
...
- [`xreg/nve_hydro.xreg.json`](xreg/nve_hydro.xreg.json) — source contract used to generate producer bindings.
