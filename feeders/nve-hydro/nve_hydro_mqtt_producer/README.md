# Nve_hydro_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the nve_hydro_mqtt_producer message definitions.

# Nve_hydro_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - NONVEHydrologyMqttEventDispatcher
...
from nve_hydro_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = NONVEHydrologyMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NONVEHydrologyMqttEventDispatcher



# Publish message`NONVEHydrologyMqttEventDispatcher` handles events for the NO.NVE.Hydrology.mqtt message group.
...
from nve_hydro_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = NONVEHydrologyMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe NONVEHydrologyMqttEventDispatcher defines the following event handler hooks.
...
```##### `no_nve_hydrology_mqtt_station_async`



## Test```python

no_nve_hydrology_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `nve_hydro_mqtt_producer_data.Station`.

Example:

```python
async def no_nve_hydrology_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `no_nve_hydrology_mqtt_water_level_observation_async`



## Test```python

no_nve_hydrology_mqtt_water_level_observation_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `nve_hydro_mqtt_producer_data.WaterLevelObservation`.

Example:

```python
async def no_nve_hydrology_mqtt_water_level_observation_event(partition_context: PartitionContext, event: EventData,
