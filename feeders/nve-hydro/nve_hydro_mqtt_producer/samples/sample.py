...
import os
import signal
from nve_hydro_mqtt_producer_data import *
from nve_hydro_mqtt_producer_mqtt_client.client import NONVEHydrologyMqttProducer, NONVEHydrologyMqttDispatcher

async def handle_no_nve_hydrology_mqtt_station(mqtt_msg,cloud_event, no_nve_hydrology_mqtt_station_data):
    """ Handles the NO.NVE.Hydrology.mqtt.Station message """
    print(f"Received NO.NVE.Hydrology.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {no_nve_hydrology_mqtt_station_data}")

async def handle_no_nve_hydrology_mqtt_water_level_observation(mqtt_msg,cloud_event, no_nve_hydrology_mqtt_water_level_observation_data):
    """ Handles the NO.NVE.Hydrology.mqtt.WaterLevelObservation message """
    print(f"Received NO.NVE.Hydrology.mqtt.WaterLevelObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {no_nve_hydrology_mqtt_water_level_observation_data}")
