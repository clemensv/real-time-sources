...
import celestrak_mqtt_producer_data
from celestrak_mqtt_producer_data import SatelliteCatalogEntry
from celestrak_mqtt_producer_data import OrbitMeanElements
from celestrak_mqtt_producer_data import SupplementalOrbitMeanElements
...
def get_default_topic_mappings_org_celestrak_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.celestrak.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.celestrak.mqtt.SatelliteCatalogEntry": "space/celestrak/satcat/{NORAD_CAT_ID}",
        "org.celestrak.mqtt.OrbitMeanElements": "space/celestrak/gp/{NORAD_CAT_ID}",
        "org.celestrak.mqtt.SupplementalOrbitMeanElements": "space/celestrak/supgp/{NORAD_CAT_ID}",
    }


def get_subscription_topics_org_celestrak_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_org_celestrak_mqtt()
    topics = []
...
class OrgCelestrakMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.celestrak.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_celestrak_mqtt()
        ...
        self.org_celestrak_mqtt_satellite_catalog_entry_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, celestrak_mqtt_producer_data.SatelliteCatalogEntry, Dict[str, str]], Awaitable[None]]] = None
        
        self.org_celestrak_mqtt_orbit_mean_elements_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, celestrak_mqtt_producer_data.OrbitMeanElements, Dict[str, str]], Awaitable[None]]] = None
        
        self.org_celestrak_mqtt_supplemental_orbit_mean_elements_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, celestrak_mqtt_producer_data.SupplementalOrbitMeanElements, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.celestrak.SatelliteCatalogEntry":
            if self.org_celestrak_mqtt_satellite_catalog_entry_async:
                try:
                    ...
                    data = celestrak_mqtt_producer_data.SatelliteCatalogEntry.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.celestrak.mqtt.SatelliteCatalogEntry")
                    await self.org_celestrak_mqtt_satellite_catalog_entry_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_celestrak_mqtt_satellite_catalog_entry handler: {e}")
            return
        
        if event_type == "org.celestrak.OrbitMeanElements":
            if self.org_celestrak_mqtt_orbit_mean_elements_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = celestrak_mqtt_producer_data.OrbitMeanElements.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.celestrak.mqtt.OrbitMeanElements")
                    await self.org_celestrak_mqtt_orbit_mean_elements_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_celestrak_mqtt_orbit_mean_elements handler: {e}")
            return
        
        if event_type == "org.celestrak.SupplementalOrbitMeanElements":
            if self.org_celestrak_mqtt_supplemental_orbit_mean_elements_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = celestrak_mqtt_producer_data.SupplementalOrbitMeanElements.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.celestrak.mqtt.SupplementalOrbitMeanElements")
                    await self.org_celestrak_mqtt_supplemental_orbit_mean_elements_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_celestrak_mqtt_supplemental_orbit_mean_elements handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_celestrak_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_org_celestrak_mqtt_satellite_catalog_entry(self,
        feedurl: str,
        norad_cat_id: str,
        data: celestrak_mqtt_producer_data.SatelliteCatalogEntry,
        ...
        """
        Publish the 'org.celestrak.mqtt.SatelliteCatalogEntry' event to an MQTT topic.

        Args:
        
            feedurl: URI template variable for 'feedurl'
            norad_cat_id: URI template variable for 'NORAD_CAT_ID'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'space/celestrak/satcat/{NORAD_CAT_ID}'
        ...
        """
        target_topic = topic if topic is not None else "space/celestrak/satcat/{NORAD_CAT_ID}"
        ...
        attributes = {
             ...
             "subject":"{NORAD_CAT_ID}".format(NORAD_CAT_ID = norad_cat_id)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_org_celestrak_mqtt_orbit_mean_elements(self,
        feedurl: str,
        norad_cat_id: str,
        data: celestrak_mqtt_producer_data.OrbitMeanElements,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.celestrak.mqtt.OrbitMeanElements' event to an MQTT topic.
        ...
        Args:
            ...
            norad_cat_id: URI template variable for 'NORAD_CAT_ID'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'space/celestrak/gp/{NORAD_CAT_ID}'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "space/celestrak/gp/{NORAD_CAT_ID}"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
            "NORAD_CAT_ID": str(norad_cat_id),
        }
    ...
    async def publish_org_celestrak_mqtt_supplemental_orbit_mean_elements(self,
        feedurl: str,
        norad_cat_id: str,
        data: celestrak_mqtt_producer_data.SupplementalOrbitMeanElements,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.celestrak.mqtt.SupplementalOrbitMeanElements' event to an MQTT topic.
        ...
        Args:
            ...
            norad_cat_id: URI template variable for 'NORAD_CAT_ID'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'space/celestrak/supgp/{NORAD_CAT_ID}'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "space/celestrak/supgp/{NORAD_CAT_ID}"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
            "NORAD_CAT_ID": str(norad_cat_id),
        }
