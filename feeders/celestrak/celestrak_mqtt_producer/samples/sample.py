...
import os
import signal
from celestrak_mqtt_producer_data import *
from celestrak_mqtt_producer_mqtt_client.client import OrgCelestrakMqttProducer, OrgCelestrakMqttDispatcher

async def handle_org_celestrak_mqtt_satellite_catalog_entry(mqtt_msg,cloud_event, org_celestrak_mqtt_satellite_catalog_entry_data):
    """ Handles the org.celestrak.mqtt.SatelliteCatalogEntry message """
    print(f"Received org.celestrak.mqtt.SatelliteCatalogEntry on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_celestrak_mqtt_satellite_catalog_entry_data}")

async def handle_org_celestrak_mqtt_orbit_mean_elements(mqtt_msg,cloud_event, org_celestrak_mqtt_orbit_mean_elements_data):
    """ Handles the org.celestrak.mqtt.OrbitMeanElements message """
    print(f"Received org.celestrak.mqtt.OrbitMeanElements on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_celestrak_mqtt_orbit_mean_elements_data}")

async def handle_org_celestrak_mqtt_supplemental_orbit_mean_elements(mqtt_msg,cloud_event, org_celestrak_mqtt_supplemental_orbit_mean_elements_data):
    """ Handles the org.celestrak.mqtt.SupplementalOrbitMeanElements message """
    print(f"Received org.celestrak.mqtt.SupplementalOrbitMeanElements on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_celestrak_mqtt_supplemental_orbit_mean_elements_data}")
