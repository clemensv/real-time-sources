# Celestrak_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the celestrak_mqtt_producer message definitions.

# Celestrak_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - OrgCelestrakMqttEventDispatcher
...
from celestrak_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = OrgCelestrakMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgCelestrakMqttEventDispatcher



# Publish message`OrgCelestrakMqttEventDispatcher` handles events for the org.celestrak.mqtt message group.
...
from celestrak_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = OrgCelestrakMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe OrgCelestrakMqttEventDispatcher defines the following event handler hooks.
...
```##### `org_celestrak_mqtt_satellite_catalog_entry_async`



## Test```python

org_celestrak_mqtt_satellite_catalog_entry_async:  Callable[[PartitionContext, EventData, CloudEvent,
SatelliteCatalogEntry], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `org.celestrak.mqtt.SatelliteCatalogEntry`: A CelesTrak SATCAT reference record for one
...
- `data`: The event data of type `celestrak_mqtt_producer_data.SatelliteCatalogEntry`.

Example:

```python
async def org_celestrak_mqtt_satellite_catalog_entry_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: SatelliteCatalogEntry) -> None:
...
```python
org_celestrak_mqtt_dispatcher.org_celestrak_mqtt_satellite_catalog_entry_async =
org_celestrak_mqtt_satellite_catalog_entry_event
```
...
```##### `org_celestrak_mqtt_orbit_mean_elements_async`



## Test```python

org_celestrak_mqtt_orbit_mean_elements_async:  Callable[[PartitionContext, EventData, CloudEvent, OrbitMeanElements],
Awaitable[None]]
...
```Asynchronous handler hook for `org.celestrak.mqtt.OrbitMeanElements`: A CelesTrak GP (General Perturbations) OMM
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `celestrak_mqtt_producer_data.OrbitMeanElements`.

Example:

```python
async def org_celestrak_mqtt_orbit_mean_elements_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: OrbitMeanElements) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
org_celestrak_mqtt_dispatcher.org_celestrak_mqtt_orbit_mean_elements_async =
org_celestrak_mqtt_orbit_mean_elements_event
```
...
```##### `org_celestrak_mqtt_supplemental_orbit_mean_elements_async`



## Test```python

org_celestrak_mqtt_supplemental_orbit_mean_elements_async:  Callable[[PartitionContext, EventData, CloudEvent,
SupplementalOrbitMeanElements], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `org.celestrak.mqtt.SupplementalOrbitMeanElements`: A CelesTrak SupGP (Supplemental GP)
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `celestrak_mqtt_producer_data.SupplementalOrbitMeanElements`.

Example:

```python
async def org_celestrak_mqtt_supplemental_orbit_mean_elements_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: SupplementalOrbitMeanElements) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
org_celestrak_mqtt_dispatcher.org_celestrak_mqtt_supplemental_orbit_mean_elements_async =
org_celestrak_mqtt_supplemental_orbit_mean_elements_event
```
