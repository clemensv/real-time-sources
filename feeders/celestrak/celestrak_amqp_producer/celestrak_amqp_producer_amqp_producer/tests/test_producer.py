...
"""
Tests for celestrak_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../celestrak_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../celestrak_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../celestrak_amqp_producer_amqp_producer/src')))

from celestrak_amqp_producer_amqp_producer import *
from celestrak_amqp_producer_data import SatelliteCatalogEntry
from test_satellitecatalogentry import Test_SatelliteCatalogEntry
from celestrak_amqp_producer_data import OrbitMeanElements
from test_orbitmeanelements import Test_OrbitMeanElements
from celestrak_amqp_producer_data import SupplementalOrbitMeanElements
...
class TestOrgCelestrakAmqpProducer:
    """Test cases for OrgCelestrakAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(OrgCelestrakAmqpProducer)
    ...
    def test_send_satellite_catalog_entry(self, artemis_container):
        """Send and receive a SatelliteCatalogEntry message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_SatelliteCatalogEntry.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "org.celestrak.SatelliteCatalogEntry"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{NORAD_CAT_ID}".format(NORAD_CAT_ID="value")
                assert properties.get('norad_cat_id') == "{NORAD_CAT_ID}".format(NORAD_CAT_ID="value")
                assert properties.get('data_kind') == "satcat"
        finally:
            producer.close()

    def test_send_satellite_catalog_entry_single_fresh_connection(self, artemis_container):
        """Send exactly one SatelliteCatalogEntry message on a fresh producer connection."""
        payload = Test_SatelliteCatalogEntry.create_instance()
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.celestrak.SatelliteCatalogEntry'
        assert received.body is not None
        assert received.subject == "{NORAD_CAT_ID}".format(NORAD_CAT_ID="value")
        assert properties.get('norad_cat_id') == "{NORAD_CAT_ID}".format(NORAD_CAT_ID="value")
        assert properties.get('data_kind') == "satcat"
    
    def test_send_orbit_mean_elements(self, artemis_container):
        """Send and receive a OrbitMeanElements message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_OrbitMeanElements.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "org.celestrak.OrbitMeanElements"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                ...
                assert received.subject == "{NORAD_CAT_ID}".format(NORAD_CAT_ID="value")
                assert properties.get('norad_cat_id') == "{NORAD_CAT_ID}".format(NORAD_CAT_ID="value")
                assert properties.get('data_kind') == "gp"
        finally:
            producer.close()

    def test_send_orbit_mean_elements_single_fresh_connection(self, artemis_container):
        """Send exactly one OrbitMeanElements message on a fresh producer connection."""
        payload = Test_OrbitMeanElements.create_instance()
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.celestrak.OrbitMeanElements'
        assert received.body is not None
        assert received.subject == "{NORAD_CAT_ID}".format(NORAD_CAT_ID="value")
        assert properties.get('norad_cat_id') == "{NORAD_CAT_ID}".format(NORAD_CAT_ID="value")
        assert properties.get('data_kind') == "gp"
    
    def test_send_supplemental_orbit_mean_elements(self, artemis_container):
        """Send and receive a SupplementalOrbitMeanElements message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_SupplementalOrbitMeanElements.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "org.celestrak.SupplementalOrbitMeanElements"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                ...
                assert received.subject == "{NORAD_CAT_ID}".format(NORAD_CAT_ID="value")
                assert properties.get('norad_cat_id') == "{NORAD_CAT_ID}".format(NORAD_CAT_ID="value")
                assert properties.get('data_kind') == "supgp"
        finally:
            producer.close()

    def test_send_supplemental_orbit_mean_elements_single_fresh_connection(self, artemis_container):
        """Send exactly one SupplementalOrbitMeanElements message on a fresh producer connection."""
        payload = Test_SupplementalOrbitMeanElements.create_instance()
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.celestrak.SupplementalOrbitMeanElements'
        assert received.body is not None
        assert received.subject == "{NORAD_CAT_ID}".format(NORAD_CAT_ID="value")
        assert properties.get('norad_cat_id') == "{NORAD_CAT_ID}".format(NORAD_CAT_ID="value")
        assert properties.get('data_kind') == "supgp"
