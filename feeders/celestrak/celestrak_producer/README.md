...
# Celestrak_producer Kafka Producer# Celestrak_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - OrgCelestrakKafkaEventDispatcher
...
- OrgCelestrakKafkaProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### OrgCelestrakKafkaEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`OrgCelestrakKafkaEventDispatcher` handles events for the org.celestrak.kafka message group.
...
from celestrak_producer import OrgCelestrakKafkaProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = OrgCelestrakKafkaProducer(
...
await producer.send_org_celestrak_kafka_satellite_catalog_entry(```python

    data=SatelliteCatalogEntry(...),add_consumer(self, consumer: KafkaConsumer)
...
The OrgCelestrakKafkaEventDispatcher defines the following event handler hooks.
...
producer = OrgCelestrakKafkaProducer(
    ...
    security_protocol='SASL_SSL',##### `org_celestrak_kafka_satellite_catalog_entry_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'org_celestrak_kafka_satellite_catalog_entry_async:  Callable[[ConsumerRecord,
CloudEvent, SatelliteCatalogEntry], Awaitable[None]]

)```

```

Asynchronous handler hook for `org.celestrak.kafka.SatelliteCatalogEntry`: A CelesTrak SATCAT reference record for one
catalogued space object. Emitted at start-up and on periodic refresh so consumers can resolve every `NORAD_CAT_ID` to a
named, described object.
...
### OrgCelestrakKafkaProducer- `data`: The event data of type `celestrak_producer_data.SatelliteCatalogEntry`.



Producer for `org.celestrak.kafka` message group.Example:



#### Constructor```python

async def org_celestrak_kafka_satellite_catalog_entry_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
SatelliteCatalogEntry) -> None:

```python    # Process the event data

OrgCelestrakKafkaProducer(    await some_processing_function(record, cloud_event, data)
...
org_celestrak_kafka_dispatcher.org_celestrak_kafka_satellite_catalog_entry_async =
org_celestrak_kafka_satellite_catalog_entry_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `org_celestrak_kafka_orbit_mean_elements_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'org_celestrak_kafka_orbit_mean_elements_async:  Callable[[ConsumerRecord, CloudEvent,
OrbitMeanElements], Awaitable[None]]

)```

```

Asynchronous handler hook for `org.celestrak.kafka.OrbitMeanElements`: A CelesTrak GP (General Perturbations) OMM
orbital element set for one object. Emitted when a new or refreshed element set is observed.
...
### OrgCelestrakKafkaProducer- `data`: The event data of type `celestrak_producer_data.OrbitMeanElements`.
...
async def org_celestrak_kafka_orbit_mean_elements_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
OrbitMeanElements) -> None:
...
org_celestrak_kafka_dispatcher.org_celestrak_kafka_orbit_mean_elements_async =
org_celestrak_kafka_orbit_mean_elements_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `org_celestrak_kafka_supplemental_orbit_mean_elements_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'org_celestrak_kafka_supplemental_orbit_mean_elements_async:  Callable[[ConsumerRecord,
CloudEvent, SupplementalOrbitMeanElements], Awaitable[None]]

)```

```

Asynchronous handler hook for `org.celestrak.kafka.SupplementalOrbitMeanElements`: A CelesTrak SupGP (Supplemental GP)
OMM element set for one object, from an operator or analysis-centre source. Emitted when supplemental sources are
configured.
...
### OrgCelestrakKafkaProducer- `data`: The event data of type `celestrak_producer_data.SupplementalOrbitMeanElements`.
...
async def org_celestrak_kafka_supplemental_orbit_mean_elements_event(record: ConsumerRecord, cloud_event: CloudEvent,
data: SupplementalOrbitMeanElements) -> None:
...
org_celestrak_kafka_dispatcher.org_celestrak_kafka_supplemental_orbit_mean_elements_async =
org_celestrak_kafka_supplemental_orbit_mean_elements_event
...
##### `send_org_celestrak_kafka_satellite_catalog_entry`Dispatchers have the following protected methods:



```python### Methods:

async def send_org_celestrak_kafka_satellite_catalog_entry(
...
Send a single `org.celestrak.kafka.SatelliteCatalogEntry` message. A CelesTrak SATCAT reference record for one
catalogued space object. Emitted at start-up and on periodic refresh so consumers can resolve every `NORAD_CAT_ID` to a
named, described object.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `SatelliteCatalogEntry`
...
await producer.send_org_celestrak_kafka_satellite_catalog_entry(

    data=SatelliteCatalogEntry(...),Args:
...
##### `send_org_celestrak_kafka_satellite_catalog_entry_batch`##### `_dispatch_record`



```python```python

async def send_org_celestrak_kafka_satellite_catalog_entry_batch(_dispatch_record(self, record)

    self,```

    messages: List[SatelliteCatalogEntry],
...
Send multiple `org.celestrak.kafka.SatelliteCatalogEntry` messages in a batch.
...
await producer.send_org_celestrak_kafka_satellite_catalog_entry_batch(```

    messages=[

        SatelliteCatalogEntry(...),Initializes the runner with a Kafka consumer.

        SatelliteCatalogEntry(...),

        SatelliteCatalogEntry(...)Args:
...
##### `send_org_celestrak_kafka_orbit_mean_elements`Dispatchers have the following protected methods:



```python### Methods:

async def send_org_celestrak_kafka_orbit_mean_elements(
...
Send a single `org.celestrak.kafka.OrbitMeanElements` message. A CelesTrak GP (General Perturbations) OMM orbital
element set for one object. Emitted when a new or refreshed element set is observed.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `OrbitMeanElements`
...
await producer.send_org_celestrak_kafka_orbit_mean_elements(

    data=OrbitMeanElements(...),Args:
...
##### `send_org_celestrak_kafka_orbit_mean_elements_batch`##### `_dispatch_record`



```python```python

async def send_org_celestrak_kafka_orbit_mean_elements_batch(_dispatch_record(self, record)

    self,```

    messages: List[OrbitMeanElements],
...
Send multiple `org.celestrak.kafka.OrbitMeanElements` messages in a batch.
...
await producer.send_org_celestrak_kafka_orbit_mean_elements_batch(```

    messages=[

        OrbitMeanElements(...),Initializes the runner with a Kafka consumer.

        OrbitMeanElements(...),

        OrbitMeanElements(...)Args:
...
##### `send_org_celestrak_kafka_supplemental_orbit_mean_elements`Dispatchers have the following protected methods:



```python### Methods:

async def send_org_celestrak_kafka_supplemental_orbit_mean_elements(
...
Send a single `org.celestrak.kafka.SupplementalOrbitMeanElements` message. A CelesTrak SupGP (Supplemental GP) OMM
element set for one object, from an operator or analysis-centre source. Emitted when supplemental sources are
configured.Args:
...
- `data`: Message data of type `SupplementalOrbitMeanElements`
...
await producer.send_org_celestrak_kafka_supplemental_orbit_mean_elements(

    data=SupplementalOrbitMeanElements(...),Args:
...
##### `send_org_celestrak_kafka_supplemental_orbit_mean_elements_batch`##### `_dispatch_record`



```python```python

async def send_org_celestrak_kafka_supplemental_orbit_mean_elements_batch(_dispatch_record(self, record)

    self,```

    messages: List[SupplementalOrbitMeanElements],
...
Send multiple `org.celestrak.kafka.SupplementalOrbitMeanElements` messages in a batch.
...
await producer.send_org_celestrak_kafka_supplemental_orbit_mean_elements_batch(```

    messages=[

        SupplementalOrbitMeanElements(...),Initializes the runner with a Kafka consumer.

        SupplementalOrbitMeanElements(...),

        SupplementalOrbitMeanElements(...)Args:
