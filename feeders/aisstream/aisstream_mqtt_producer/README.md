# Aisstream_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the aisstream_mqtt_producer message definitions.

# Aisstream_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - IOAISstreamMqttEventDispatcher
...
from aisstream_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = IOAISstreamMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### IOAISstreamMqttEventDispatcher



# Publish message`IOAISstreamMqttEventDispatcher` handles events for the IO.AISstream.mqtt message group.
...
from aisstream_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = IOAISstreamMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe IOAISstreamMqttEventDispatcher defines the following event handler hooks.
...
```##### `io_aisstream_mqtt_position_report_async`



## Test```python

io_aisstream_mqtt_position_report_async:  Callable[[PartitionContext, EventData, CloudEvent, PositionReport],
Awaitable[None]]
...
- `data`: The event data of type `aisstream_mqtt_producer_data.PositionReport`.

Example:

```python
async def io_aisstream_mqtt_position_report_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: PositionReport) -> None:
