...
from aisstream_mqtt_producer_data import ShipStatic
from aisstream_mqtt_producer_data import AidToNavigation
...
def get_default_topic_mappings_io_aisstream_mqtt() -> Dict[str, str]:
    ...
    return {
        ...
        "IO.AISstream.mqtt.ShipStatic": "maritime/intl/aisstream/aisstream/{flag}/{ship_type}/{geohash5}/{mmsi}/static",
        "IO.AISstream.mqtt.AidToNavigation": "maritime/intl/aisstream/aisstream/{flag}/{ship_type}/{geohash5}/{mmsi}/aid-to-navigation",
    }
...
class IOAISstreamMqttMqttClient(_ClientBase):
    ...
    def __init__(
        ...
        self.io_aisstream_mqtt_ship_static_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, aisstream_mqtt_producer_data.ShipStatic, Dict[str, str]], Awaitable[None]]] = None
        
        self.io_aisstream_mqtt_aid_to_navigation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, aisstream_mqtt_producer_data.AidToNavigation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "IO.AISstream.mqtt.PositionReport":
        ...
        if event_type == "IO.AISstream.mqtt.ShipStatic":
            if self.io_aisstream_mqtt_ship_static_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = aisstream_mqtt_producer_data.ShipStatic.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "IO.AISstream.mqtt.ShipStatic")
                    await self.io_aisstream_mqtt_ship_static_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in io_aisstream_mqtt_ship_static handler: {e}")
            return
        
        if event_type == "IO.AISstream.mqtt.AidToNavigation":
            if self.io_aisstream_mqtt_aid_to_navigation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = aisstream_mqtt_producer_data.AidToNavigation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "IO.AISstream.mqtt.AidToNavigation")
                    await self.io_aisstream_mqtt_aid_to_navigation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in io_aisstream_mqtt_aid_to_navigation handler: {e}")
            return
    ...
    async def publish_io_aisstream_mqtt_ship_static(self,
        mmsi: str,
        flag: str,
        ship_type: str,
        geohash5: str,
        data: aisstream_mqtt_producer_data.ShipStatic,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'IO.AISstream.mqtt.ShipStatic' event to an MQTT topic.
        ...
        Args:
            ...
            geohash5: URI template variable for 'geohash5'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'maritime/intl/aisstream/aisstream/{flag}/{ship_type}/{geohash5}/{mmsi}/static'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (0).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "maritime/intl/aisstream/aisstream/{flag}/{ship_type}/{geohash5}/{mmsi}/static"
        _topic_template_values: Dict[str, str] = {
            "mmsi": str(mmsi),
            "flag": str(flag),
            "ship_type": str(ship_type),
    ...
    async def publish_io_aisstream_mqtt_aid_to_navigation(self,
        mmsi: str,
        flag: str,
        ship_type: str,
        geohash5: str,
        data: aisstream_mqtt_producer_data.AidToNavigation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'IO.AISstream.mqtt.AidToNavigation' event to an MQTT topic.
        ...
        Args:
            ...
            geohash5: URI template variable for 'geohash5'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'maritime/intl/aisstream/aisstream/{flag}/{ship_type}/{geohash5}/{mmsi}/aid-to-navigation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (0).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "maritime/intl/aisstream/aisstream/{flag}/{ship_type}/{geohash5}/{mmsi}/aid-to-navigation"
        _topic_template_values: Dict[str, str] = {
            "mmsi": str(mmsi),
            "flag": str(flag),
            "ship_type": str(ship_type),
