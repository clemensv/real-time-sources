...
# Blitzortung-producer Kafka Producer# Blitzortung-producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - BlitzortungLightningEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    BlitzortungLightningMqttEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    BlitzortungLightningAmqpEventDispatcher
...
- BlitzortungLightningProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- BlitzortungLightningMqttProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- BlitzortungLightningAmqpProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### BlitzortungLightningEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`BlitzortungLightningEventDispatcher` handles events for the Blitzortung.Lightning message group.
...
from blitzortung-producer import BlitzortungLightningProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = BlitzortungLightningProducer(
...
await producer.send_blitzortung_lightning_lightning_stroke(```python

    data=LightningStroke(...),add_consumer(self, consumer: KafkaConsumer)
...
The BlitzortungLightningEventDispatcher defines the following event handler hooks.
...
producer = BlitzortungLightningProducer(
    ...
    security_protocol='SASL_SSL',##### `blitzortung_lightning_lightning_stroke_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'blitzortung_lightning_lightning_stroke_async:  Callable[[ConsumerRecord, CloudEvent,
LightningStroke], Awaitable[None]]

)```

```

Asynchronous handler hook for `Blitzortung.Lightning.LightningStroke`: Live lightning-stroke event from the public
LightningMaps / Blitzortung websocket feed. Each event represents one source-scoped stroke identifier with its
...
### BlitzortungLightningProducer- `data`: The event data of type `blitzortung_producer_data.LightningStroke`.



Producer for `Blitzortung.Lightning` message group.Example:



#### Constructor```python

async def blitzortung_lightning_lightning_stroke_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
LightningStroke) -> None:

```python    # Process the event data

BlitzortungLightningProducer(    await some_processing_function(record, cloud_event, data)
...
blitzortung_lightning_dispatcher.blitzortung_lightning_lightning_stroke_async =
blitzortung_lightning_lightning_stroke_event
...
### BlitzortungLightningMqttProducer- `data`: The event data of type `blitzortung_producer_data.LightningStroke`.



Producer for `Blitzortung.Lightning.mqtt` message group.Example:
...
BlitzortungLightningMqttProducer(    await some_processing_function(record, cloud_event, data)
...
blitzortung_lightning_mqtt_dispatcher.blitzortung_lightning_lightning_stroke_async =
blitzortung_lightning_lightning_stroke_event
...
### BlitzortungLightningAmqpProducer- `data`: The event data of type `blitzortung_producer_data.LightningStroke`.



Producer for `Blitzortung.Lightning.amqp` message group.Example:
...
BlitzortungLightningAmqpProducer(    await some_processing_function(record, cloud_event, data)
...
blitzortung_lightning_amqp_dispatcher.blitzortung_lightning_lightning_stroke_async =
blitzortung_lightning_lightning_stroke_event
...
##### `send_blitzortung_lightning_lightning_stroke`Dispatchers have the following protected methods:



```python### Methods:

async def send_blitzortung_lightning_lightning_stroke(
...
Send a single `Blitzortung.Lightning.LightningStroke` message. Live lightning-stroke event from the public LightningMaps
/ Blitzortung websocket feed. Each event represents one source-scoped stroke identifier with its observation time,
coordinates, upstream delay and accuracy values, and optionally the detector participation flags carried in the public
sta object.Args:
...
- `data`: Message data of type `LightningStroke`
...
await producer.send_blitzortung_lightning_lightning_stroke(

    data=LightningStroke(...),Args:
...
Send multiple `Blitzortung.Lightning.LightningStroke` messages in a batch.
...
await producer.send_blitzortung_lightning_lightning_stroke_batch(```

    messages=[

        LightningStroke(...),Initializes the runner with a Kafka consumer.

        LightningStroke(...),

        LightningStroke(...)Args:
...
- **Scales horizontally** across multiple brokers and partitions### BlitzortungLightningMqttEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`BlitzortungLightningMqttEventDispatcher` handles events for the Blitzortung.Lightning.mqtt message group.
...
The BlitzortungLightningMqttEventDispatcher defines the following event handler hooks.
...
producer = BlitzortungLightningProducer(
    ...
    security_protocol='SASL_SSL',##### `blitzortung_lightning_mqtt_lightning_stroke_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'blitzortung_lightning_mqtt_lightning_stroke_async:  Callable[[ConsumerRecord,
CloudEvent, LightningStroke], Awaitable[None]]

)```

```

Asynchronous handler hook for `Blitzortung.Lightning.mqtt.LightningStroke`: Live lightning-stroke event from the public
LightningMaps / Blitzortung websocket feed. Each event represents one source-scoped stroke identifier with its
observation time, coordinates, upstream delay and accuracy values, and optionally the detector participation flags
carried in the public sta object.
...
async def blitzortung_lightning_mqtt_lightning_stroke_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
LightningStroke) -> None:
...
blitzortung_lightning_dispatcher.blitzortung_lightning_mqtt_lightning_stroke_async =
blitzortung_lightning_mqtt_lightning_stroke_event
...
blitzortung_lightning_mqtt_dispatcher.blitzortung_lightning_mqtt_lightning_stroke_async =
blitzortung_lightning_mqtt_lightning_stroke_event
...
blitzortung_lightning_amqp_dispatcher.blitzortung_lightning_mqtt_lightning_stroke_async =
blitzortung_lightning_mqtt_lightning_stroke_event
...
##### `send_blitzortung_lightning_mqtt_lightning_stroke`Dispatchers have the following protected methods:



```python### Methods:

async def send_blitzortung_lightning_mqtt_lightning_stroke(
...
Send a single `Blitzortung.Lightning.mqtt.LightningStroke` message. Live lightning-stroke event from the public
LightningMaps / Blitzortung websocket feed. Each event represents one source-scoped stroke identifier with its
observation time, coordinates, upstream delay and accuracy values, and optionally the detector participation flags
carried in the public sta object.Args:
...
await producer.send_blitzortung_lightning_mqtt_lightning_stroke(
...
Send multiple `Blitzortung.Lightning.mqtt.LightningStroke` messages in a batch.
...
await producer.send_blitzortung_lightning_mqtt_lightning_stroke_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### BlitzortungLightningAmqpEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`BlitzortungLightningAmqpEventDispatcher` handles events for the Blitzortung.Lightning.amqp message group.
...
The BlitzortungLightningAmqpEventDispatcher defines the following event handler hooks.
...
producer = BlitzortungLightningProducer(
    ...
    security_protocol='SASL_SSL',##### `blitzortung_lightning_amqp_lightning_stroke_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'blitzortung_lightning_amqp_lightning_stroke_async:  Callable[[ConsumerRecord,
CloudEvent, LightningStroke], Awaitable[None]]
...
Asynchronous handler hook for `Blitzortung.Lightning.amqp.LightningStroke`: Live lightning-stroke event from the public
LightningMaps / Blitzortung websocket feed. Each event represents one source-scoped stroke identifier with its
observation time, coordinates, upstream delay and accuracy values, and optionally the detector participation flags
carried in the public sta object.
...
async def blitzortung_lightning_amqp_lightning_stroke_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
LightningStroke) -> None:
...
blitzortung_lightning_dispatcher.blitzortung_lightning_amqp_lightning_stroke_async =
blitzortung_lightning_amqp_lightning_stroke_event
...
blitzortung_lightning_mqtt_dispatcher.blitzortung_lightning_amqp_lightning_stroke_async =
blitzortung_lightning_amqp_lightning_stroke_event
...
blitzortung_lightning_amqp_dispatcher.blitzortung_lightning_amqp_lightning_stroke_async =
blitzortung_lightning_amqp_lightning_stroke_event
...
##### `send_blitzortung_lightning_amqp_lightning_stroke`Dispatchers have the following protected methods:



```python### Methods:

async def send_blitzortung_lightning_amqp_lightning_stroke(
...
Send a single `Blitzortung.Lightning.amqp.LightningStroke` message. Live lightning-stroke event from the public
LightningMaps / Blitzortung websocket feed. Each event represents one source-scoped stroke identifier with its
observation time, coordinates, upstream delay and accuracy values, and optionally the detector participation flags
carried in the public sta object.Args:
...
await producer.send_blitzortung_lightning_amqp_lightning_stroke(
...
Send multiple `Blitzortung.Lightning.amqp.LightningStroke` messages in a batch.
...
await producer.send_blitzortung_lightning_amqp_lightning_stroke_batch(```
