""" DetectorParticipation dataclass. """
...
@dataclass
class DetectorParticipation:
    """
    One detector entry expanded from the upstream sta object on a live stroke. The public feed exposes integer status flags per station, but the public documentation does not currently publish a bit-level meaning for those status codes, so this contract preserves the upstream integer verbatim.
    ...
    def from_serializer_dict(cls, data: dict) -> 'DetectorParticipation':
        ...
        return cls(**data)
    @classmethod
    def from_avro_dict(cls, data: dict) -> 'DetectorParticipation':
        """
    ...
    @classmethod
    def from_data(cls, data: typing.Any, content_type_string: typing.Optional[str] = None) -> typing.Optional['DetectorParticipation']:
        """
        ...
        if base_content_type in ['avro/binary', 'application/vnd.apache.avro+avro']:
            if isinstance(data, bytes):
                ...
                with io.BytesIO(data) as stream:
                    ...
                    return DetectorParticipation.from_avro_dict(_record)
            else:
        ...
        if base_content_type == 'application/json':
            if isinstance(data, (bytes, str)):
                ...
                return DetectorParticipation.from_serializer_dict(_record)
            else:
                raise NotImplementedError('Data is not of a supported type for JSON deserialization')
        raise NotImplementedError(f'Unsupported media type {content_type}')

    @classmethod
    def create_instance(cls) -> 'DetectorParticipation':
        """
