...
"""
Tests for blitzortung_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../blitzortung_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../blitzortung_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../blitzortung_amqp_producer_amqp_producer/src')))
...
class TestBlitzortungLightningAmqpProducer:
    """Test cases for BlitzortungLightningAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(BlitzortungLightningAmqpProducer)
    ...
    def test_send_lightning_stroke(self, artemis_container):
        """Send and receive a LightningStroke message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_LightningStroke.create_instance()
    ...
    def test_send_lightning_stroke_single_fresh_connection(self, artemis_container):
        """Send exactly one LightningStroke message on a fresh producer connection."""
        payload = Test_LightningStroke.create_instance()
