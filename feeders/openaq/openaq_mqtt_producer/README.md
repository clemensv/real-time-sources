# Openaq_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the openaq_mqtt_producer message definitions.

# Openaq_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - OrgOpenaqLocationsMqttEventDispatcher,

    OrgOpenaqSensorsMqttEventDispatcher
...
from openaq_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = OrgOpenaqLocationsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgOpenaqLocationsMqttEventDispatcher



# Publish message`OrgOpenaqLocationsMqttEventDispatcher` handles events for the org.openaq.locations.mqtt message group.
...
from openaq_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = OrgOpenaqLocationsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
client = OrgOpenaqSensorsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe OrgOpenaqLocationsMqttEventDispatcher defines the following event handler hooks.
...
```##### `org_openaq_mqtt_location_async`



## Test```python

org_openaq_mqtt_location_async:  Callable[[PartitionContext, EventData, CloudEvent, Location], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `org.openaq.mqtt.Location`: Reference catalog metadata for one OpenAQ monitoring
location from API v3 `GET /v3/locations`. It identifies a physical or mobile air-quality monitoring site, its WGS 84
coordinates, country, ownership/provider context, monitor/mobile flags, license attribution, and first/last observation
dates. Emitted before measurement events and refreshed periodically so consumers can interpret measurements without
separately calling OpenAQ.
...
- `data`: The event data of type `openaq_mqtt_producer_data.Location`.

Example:

```python
async def org_openaq_mqtt_location_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: Location) -> None:
...
client = OrgOpenaqSensorsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgOpenaqSensorsMqttEventDispatcher



# Publish message`OrgOpenaqSensorsMqttEventDispatcher` handles events for the org.openaq.sensors.mqtt message group.
...
## BuildThe OrgOpenaqSensorsMqttEventDispatcher defines the following event handler hooks.
...
```##### `org_openaq_mqtt_sensor_async`



## Test```python

org_openaq_mqtt_sensor_async:  Callable[[PartitionContext, EventData, CloudEvent, Sensor], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `openaq_mqtt_producer_data.Sensor`.

Example:

```python
async def org_openaq_mqtt_sensor_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: Sensor) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `org_openaq_mqtt_measurement_async`



## Test```python

org_openaq_mqtt_measurement_async:  Callable[[PartitionContext, EventData, CloudEvent, Measurement], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `openaq_mqtt_producer_data.Measurement`.

Example:

```python
async def org_openaq_mqtt_measurement_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Measurement) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
