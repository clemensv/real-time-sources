...
def get_default_topic_mappings_org_openaq_locations_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.openaq.locations.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.openaq.mqtt.Location": "air-quality/global/openaq/{country_iso}/{location_id}/info",
    }


def get_subscription_topics_org_openaq_locations_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_org_openaq_locations_mqtt()
    topics = []
...
class OrgOpenaqLocationsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.openaq.locations.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_openaq_locations_mqtt()
        ...
        self.org_openaq_mqtt_location_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, openaq_mqtt_producer_data.Location, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.openaq.Location":
            if self.org_openaq_mqtt_location_async:
                try:
                    ...
                    data = openaq_mqtt_producer_data.Location.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.openaq.mqtt.Location")
                    await self.org_openaq_mqtt_location_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_openaq_mqtt_location handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_openaq_locations_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_org_openaq_mqtt_location(self,
        location_id: str,
        country_iso: str,
        data: openaq_mqtt_producer_data.Location,
        ...
        """
        Publish the 'org.openaq.mqtt.Location' event to an MQTT topic.

        Args:
        
            location_id: URI template variable for 'location_id'
            country_iso: URI template variable for 'country_iso'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/global/openaq/{country_iso}/{location_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "air-quality/global/openaq/{country_iso}/{location_id}/info"
...
def get_default_topic_mappings_org_openaq_sensors_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.openaq.sensors.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.openaq.mqtt.Sensor": "air-quality/global/openaq/{country_iso}/{location_id}/{parameter_name}/{sensor_id}/sensor",
        "org.openaq.mqtt.Measurement": "air-quality/global/openaq/{country_iso}/{location_id}/{parameter_name}/{sensor_id}/measurement",
    }


def get_subscription_topics_org_openaq_sensors_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_org_openaq_sensors_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class OrgOpenaqSensorsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.openaq.sensors.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_openaq_sensors_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.org_openaq_mqtt_sensor_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, openaq_mqtt_producer_data.Sensor, Dict[str, str]], Awaitable[None]]] = None
        
        self.org_openaq_mqtt_measurement_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, openaq_mqtt_producer_data.Measurement, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.openaq.Sensor":
            if self.org_openaq_mqtt_sensor_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = openaq_mqtt_producer_data.Sensor.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.openaq.mqtt.Sensor")
                    await self.org_openaq_mqtt_sensor_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_openaq_mqtt_sensor handler: {e}")
            return
        
        if event_type == "org.openaq.Measurement":
            if self.org_openaq_mqtt_measurement_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = openaq_mqtt_producer_data.Measurement.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.openaq.mqtt.Measurement")
                    await self.org_openaq_mqtt_measurement_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_openaq_mqtt_measurement handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_openaq_sensors_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_org_openaq_mqtt_sensor(self,
        location_id: str,
        sensor_id: str,
        country_iso: str,
        parameter_name: str,
        data: openaq_mqtt_producer_data.Sensor,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.openaq.mqtt.Sensor' event to an MQTT topic.
        ...
        Args:
            ...
            country_iso: URI template variable for 'country_iso'
            parameter_name: URI template variable for 'parameter_name'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/global/openaq/{country_iso}/{location_id}/{parameter_name}/{sensor_id}/sensor'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/global/openaq/{country_iso}/{location_id}/{parameter_name}/{sensor_id}/sensor"
        _topic_template_values: Dict[str, str] = {
            "location_id": str(location_id),
        ...
        attributes = {
             ...
             "source":"https://api.openaq.org/v3",
             "subject":"{location_id}/{sensor_id}".format(location_id = location_id,sensor_id = sensor_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
    ...
    async def publish_org_openaq_mqtt_measurement(self,
        location_id: str,
        sensor_id: str,
        country_iso: str,
        parameter_name: str,
        data: openaq_mqtt_producer_data.Measurement,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.openaq.mqtt.Measurement' event to an MQTT topic.
        ...
        Args:
            ...
            parameter_name: URI template variable for 'parameter_name'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/global/openaq/{country_iso}/{location_id}/{parameter_name}/{sensor_id}/measurement'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/global/openaq/{country_iso}/{location_id}/{parameter_name}/{sensor_id}/measurement"
        _topic_template_values: Dict[str, str] = {
            "location_id": str(location_id),
            "sensor_id": str(sensor_id),
            "country_iso": str(country_iso),
