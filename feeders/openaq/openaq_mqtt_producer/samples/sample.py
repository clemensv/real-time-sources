...
import os
import signal
from openaq_mqtt_producer_data import *
from openaq_mqtt_producer_mqtt_client.client import OrgOpenaqLocationsMqttProducer, OrgOpenaqLocationsMqttDispatcher

async def handle_org_openaq_mqtt_location(mqtt_msg,cloud_event, org_openaq_mqtt_location_data):
    """ Handles the org.openaq.mqtt.Location message """
    print(f"Received org.openaq.mqtt.Location on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_openaq_mqtt_location_data}")
from openaq_mqtt_producer_mqtt_client.client import OrgOpenaqSensorsMqttProducer, OrgOpenaqSensorsMqttDispatcher

async def handle_org_openaq_mqtt_sensor(mqtt_msg,cloud_event, org_openaq_mqtt_sensor_data):
    """ Handles the org.openaq.mqtt.Sensor message """
    print(f"Received org.openaq.mqtt.Sensor on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_openaq_mqtt_sensor_data}")

async def handle_org_openaq_mqtt_measurement(mqtt_msg,cloud_event, org_openaq_mqtt_measurement_data):
    """ Handles the org.openaq.mqtt.Measurement message """
    print(f"Received org.openaq.mqtt.Measurement on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_openaq_mqtt_measurement_data}")
