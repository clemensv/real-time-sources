...
class OrgOpenaqLocationsAmqpProducer:
    """
    Producer class to send messages in the `org.openaq.locations.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_location(self,
        ...
        Args:
            ...
            _country_iso (str): Value for AMQP protocol option placeholder country_iso
        ...
        app_properties = {}
        app_properties["country_iso"] = "{country_iso}".format(country_iso=_country_iso)
        if app_properties:
        ...
        annotations = {}
        annotation_value = "{location_id}".format(location_id=_location_id)
    ...
    def send_location_batch(self,
        data_array: typing.List[Location],
        _location_id: str,
        _country_iso: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.openaq.amqp.Location` messages
        
        Args:
            data_array (typing.List[Location]): Array of message data objects
            _location_id (str): Value for placeholder location_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            _country_iso (str): Value for AMQP protocol option placeholder country_iso
...
class OrgOpenaqSensorsAmqpProducer:
    """
    Producer class to send messages in the `org.openaq.sensors.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_sensor(self,
        ...
        Args:
            ...
            _country_iso (str): Value for AMQP protocol option placeholder country_iso
            _parameter_name (str): Value for AMQP protocol option placeholder parameter_name
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        attributes = {
            ...
            "https://api.openaq.org/v3",
            "subject":
            "{location_id}/{sensor_id}".format(location_id=_location_id, sensor_id=_sensor_id),
        }
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        ...
        if amqp_creation_time is not None:
            amqp_msg.creation_time = amqp_creation_time
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "{location_id}/{sensor_id}".format(location_id=_location_id, sensor_id=_sensor_id)

        app_properties = {}
        app_properties["country_iso"] = "{country_iso}".format(country_iso=_country_iso)
        app_properties["parameter_name"] = "{parameter_name}".format(parameter_name=_parameter_name)
        if app_properties:
            if amqp_msg.properties is None:
                amqp_msg.properties = {}
            amqp_msg.properties.update(app_properties)
        ...
        annotations = {}
        annotation_value = "{location_id}/{sensor_id}".format(location_id=_location_id, sensor_id=_sensor_id)
        annotation_value = str(annotation_value)[:128]
        annotations[symbol("x-opt-partition-key")] = annotation_value
        if annotations:
            if amqp_msg.annotations is None:
    ...
    def send_sensor_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.openaq.amqp.Sensor` messages
        
        Args:
            data_array (typing.List[Sensor]): Array of message data objects
            _location_id (str): Value for placeholder location_id in attribute subject
            _sensor_id (str): Value for placeholder sensor_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            _country_iso (str): Value for AMQP protocol option placeholder country_iso
    ...
    def send_measurement(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `org.openaq.amqp.Measurement` message
    ...
    def send_measurement_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.openaq.amqp.Measurement` messages
