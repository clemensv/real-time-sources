...
"""
Tests for openaq_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../openaq_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../openaq_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../openaq_amqp_producer_amqp_producer/src')))

from openaq_amqp_producer_amqp_producer import *
from openaq_amqp_producer_data import Location
from test_location import Test_Location
from openaq_amqp_producer_data import Sensor
from test_sensor import Test_Sensor
from openaq_amqp_producer_data import Measurement
...
class TestOrgOpenaqLocationsAmqpProducer:
    """Test cases for OrgOpenaqLocationsAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(OrgOpenaqLocationsAmqpProducer)
    ...
    def test_send_location(self, artemis_container):
        ...
        payload = Test_Location.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "org.openaq.Location"
                ...
                assert properties.get('country_iso') == "{country_iso}".format(country_iso="value")
                assert annotations.get(symbol('x-opt-partition-key')) == str("{location_id}".format(location_id="value"))[:128]
        finally:
            producer.close()
    ...
    def test_send_location_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.openaq.Location'
        assert received.body is not None
        assert received.subject == "{location_id}".format(location_id="value")
        assert properties.get('country_iso') == "{country_iso}".format(country_iso="value")
        assert annotations.get(symbol('x-opt-partition-key')) == str("{location_id}".format(location_id="value"))[:128]
...
class TestOrgOpenaqSensorsAmqpProducer:
    """Test cases for OrgOpenaqSensorsAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        fake_connection = FakeConnection()
        producer = object.__new__(OrgOpenaqSensorsAmqpProducer)
        producer._sender = fake_sender
        producer._connection = fake_connection
        producer._blocking_sender_is_presettled = True
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "org.openaq.Sensor"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{location_id}/{sensor_id}".format(location_id="value", sensor_id="value")
                assert properties.get('country_iso') == "{country_iso}".format(country_iso="value")
                assert properties.get('parameter_name') == "{parameter_name}".format(parameter_name="value")
                assert annotations.get(symbol('x-opt-partition-key')) == str("{location_id}/{sensor_id}".format(location_id="value", sensor_id="value"))[:128]
        finally:
            producer.close()
    ...
    def test_send_sensor_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.openaq.Sensor'
        assert received.body is not None
        assert received.subject == "{location_id}/{sensor_id}".format(location_id="value", sensor_id="value")
        assert properties.get('country_iso') == "{country_iso}".format(country_iso="value")
        assert properties.get('parameter_name') == "{parameter_name}".format(parameter_name="value")
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "org.openaq.Measurement"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_measurement_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.openaq.Measurement'
        assert received.body is not None
        assert received.subject == "{location_id}/{sensor_id}".format(location_id="value", sensor_id="value")
        assert properties.get('country_iso') == "{country_iso}".format(country_iso="value")
        assert properties.get('parameter_name') == "{parameter_name}".format(parameter_name="value")
