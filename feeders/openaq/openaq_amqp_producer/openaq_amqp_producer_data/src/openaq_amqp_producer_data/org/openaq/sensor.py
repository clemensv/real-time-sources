...
class Sensor:
    """
    Reference metadata for one OpenAQ sensor from API v3 `GET /v3/locations/{locations_id}/sensors`. A sensor represents one parameter-specific measurement stream at a location, carrying the stable sensor id, sensor name, parameter id/name/units, date coverage, and latest value metadata when OpenAQ exposes it.
    ...
    Attributes:
        ...
        parameter_units (str)
        parameter_display_name (typing.Optional[str])
        datetime_first (typing.Optional[datetime.datetime])
        datetime_last (typing.Optional[datetime.datetime])
        latest_value (typing.Optional[float])
        latest_datetime (typing.Optional[datetime.datetime])
    ...
    sensor_name: str=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="sensor_name"))
    ...
    parameter_display_name: typing.Optional[str]=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="parameter_display_name"))
    ...
    latest_value: typing.Optional[float]=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="latest_value"))
    latest_datetime: typing.Optional[datetime.datetime]=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="latest_datetime", encoder=lambda d: d.isoformat() if isinstance(d, datetime.datetime) else d if d else None, decoder=lambda d: datetime.datetime.fromisoformat(d) if isinstance(d, str) else d if d else None, mm_field=fields.DateTime(format='iso')))
    ...
    @classmethod
    def from_serializer_dict(cls, data: dict) -> 'Sensor':
        """
    ...
    @classmethod
    def from_data(cls, data: typing.Any, content_type_string: typing.Optional[str] = None) -> typing.Optional['Sensor']:
        """
        ...
        if base_content_type == 'application/json':
            if isinstance(data, (bytes, str)):
                ...
                return Sensor.from_serializer_dict(_record)
            else:
                raise NotImplementedError('Data is not of a supported type for JSON deserialization')
        raise NotImplementedError(f'Unsupported media type {content_type}')

    @classmethod
    def create_instance(cls) -> 'Sensor':
        """
