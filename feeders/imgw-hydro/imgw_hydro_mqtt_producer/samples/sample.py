...
import os
import signal
from imgw_hydro_mqtt_producer_data import *
from imgw_hydro_mqtt_producer_mqtt_client.client import PLGovIMGWHydroMqttProducer, PLGovIMGWHydroMqttDispatcher

async def handle_pl_gov_imgw_hydro_mqtt_station(mqtt_msg,cloud_event, pl_gov_imgw_hydro_mqtt_station_data):
    """ Handles the PL.Gov.IMGW.Hydro.mqtt.Station message """
    print(f"Received PL.Gov.IMGW.Hydro.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {pl_gov_imgw_hydro_mqtt_station_data}")

async def handle_pl_gov_imgw_hydro_mqtt_water_level_observation(mqtt_msg,cloud_event, pl_gov_imgw_hydro_mqtt_water_level_observation_data):
    """ Handles the PL.Gov.IMGW.Hydro.mqtt.WaterLevelObservation message """
    print(f"Received PL.Gov.IMGW.Hydro.mqtt.WaterLevelObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {pl_gov_imgw_hydro_mqtt_water_level_observation_data}")
