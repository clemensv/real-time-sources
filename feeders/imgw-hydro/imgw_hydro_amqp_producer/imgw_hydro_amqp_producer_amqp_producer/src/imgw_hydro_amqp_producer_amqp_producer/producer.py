    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from imgw_hydro_amqp_producer_data import Station
from imgw_hydro_amqp_producer_data import WaterLevelObservation

class PLGovIMGWHydroAmqpProducer:
    """
    Producer class to send messages in the `PL.Gov.IMGW.Hydro.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        ...
        """
        Send the `PL.Gov.IMGW.Hydro.amqp.Station` message
        A reference record for one Polish hydrological monitoring station published by the Polish Institute of Meteorology and Water Management (IMGW-PIB). It fires when the bridge publishes or refreshes the station catalog so consumers can interpret measurement events.
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `PL.Gov.IMGW.Hydro.amqp.Station` messages
    ...
    def send_water_level_observation(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `PL.Gov.IMGW.Hydro.amqp.WaterLevelObservation` message
        A current measurement from the Polish Institute of Meteorology and Water Management (IMGW-PIB) for one monitoring site. It carries water level and discharge observations when the upstream feed reports a new or refreshed value.
    ...
    def send_water_level_observation_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `PL.Gov.IMGW.Hydro.amqp.WaterLevelObservation` messages
