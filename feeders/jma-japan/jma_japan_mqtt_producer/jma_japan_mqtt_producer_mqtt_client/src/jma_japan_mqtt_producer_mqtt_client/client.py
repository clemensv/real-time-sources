...
def get_default_topic_mappings_jp_go_jma_weatherbulletins_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the jp.go.jma.WeatherBulletins.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "jp.go.jma.WeatherBulletins.mqtt.WeatherBulletin": "weather/jp/jma/jma-japan/{office}/{bulletin_id}/bulletin",
    }


def get_subscription_topics_jp_go_jma_weatherbulletins_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_jp_go_jma_weatherbulletins_mqtt()
    topics = []
...
class JpGoJmaWeatherBulletinsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the jp.go.jma.WeatherBulletins.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_jp_go_jma_weatherbulletins_mqtt()
        ...
        self.jp_go_jma_weather_bulletins_mqtt_weather_bulletin_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, jma_japan_mqtt_producer_data.WeatherBulletin, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "jp.go.jma.WeatherBulletin":
            if self.jp_go_jma_weather_bulletins_mqtt_weather_bulletin_async:
                try:
                    ...
                    data = jma_japan_mqtt_producer_data.WeatherBulletin.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "jp.go.jma.WeatherBulletins.mqtt.WeatherBulletin")
                    await self.jp_go_jma_weather_bulletins_mqtt_weather_bulletin_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_go_jma_weather_bulletins_mqtt_weather_bulletin handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_jp_go_jma_weatherbulletins_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_jp_go_jma_weather_bulletins_mqtt_weather_bulletin(self,
        office: str,
        bulletin_id: str,
        data: jma_japan_mqtt_producer_data.WeatherBulletin,
        ...
        """
        Publish the 'jp.go.jma.WeatherBulletins.mqtt.WeatherBulletin' event to an MQTT topic.

        Args:
        
            office: URI template variable for 'office'
            bulletin_id: URI template variable for 'bulletin_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/jp/jma/jma-japan/{office}/{bulletin_id}/bulletin'
        ...
        """
        target_topic = topic if topic is not None else "weather/jp/jma/jma-japan/{office}/{bulletin_id}/bulletin"
        ...
        attributes = {
             ...
             "subject":"{office}/{bulletin_id}".format(office = office,bulletin_id = bulletin_id)
        }
        attributes["datacontenttype"] = content_type
