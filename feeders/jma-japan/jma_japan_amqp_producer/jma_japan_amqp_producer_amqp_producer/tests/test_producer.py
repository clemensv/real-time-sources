...
"""
Tests for jma_japan_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../jma_japan_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../jma_japan_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../jma_japan_amqp_producer_amqp_producer/src')))

from jma_japan_amqp_producer_amqp_producer import *
from jma_japan_amqp_producer_data import WeatherBulletin
...
class TestJpGoJmaWeatherBulletinsAmqpProducer:
    """Test cases for JpGoJmaWeatherBulletinsAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(JpGoJmaWeatherBulletinsAmqpProducer)
    ...
    def test_send_weather_bulletin(self, artemis_container):
        """Send and receive a WeatherBulletin message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_WeatherBulletin.create_instance()
    ...
    def test_send_weather_bulletin_single_fresh_connection(self, artemis_container):
        """Send exactly one WeatherBulletin message on a fresh producer connection."""
        payload = Test_WeatherBulletin.create_instance()
