# Luchtmeetnet_nl_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the luchtmeetnet_nl_mqtt_producer message definitions.

# Luchtmeetnet_nl_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - NlRivmLuchtmeetnetMqttEventDispatcher,

    NlRivmLuchtmeetnetComponentsMqttEventDispatcher
...
from luchtmeetnet_nl_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = NlRivmLuchtmeetnetMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NlRivmLuchtmeetnetMqttEventDispatcher



# Publish message`NlRivmLuchtmeetnetMqttEventDispatcher` handles events for the nl.rivm.luchtmeetnet.mqtt message group.
...
from luchtmeetnet_nl_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = NlRivmLuchtmeetnetMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
client = NlRivmLuchtmeetnetComponentsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name
...
## BuildThe NlRivmLuchtmeetnetMqttEventDispatcher defines the following event handler hooks.
...
```##### `nl_rivm_luchtmeetnet_mqtt_station_async`



## Test```python

nl_rivm_luchtmeetnet_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `luchtmeetnet_nl_mqtt_producer_data.Station`.

Example:

```python
async def nl_rivm_luchtmeetnet_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `nl_rivm_luchtmeetnet_mqtt_measurement_async`



## Test```python

nl_rivm_luchtmeetnet_mqtt_measurement_async:  Callable[[PartitionContext, EventData, CloudEvent, Measurement],
Awaitable[None]]
...
```Asynchronous handler hook for `nl.rivm.luchtmeetnet.mqtt.Measurement`: Hourly Luchtmeetnet measurement for a station
and component formula.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `luchtmeetnet_nl_mqtt_producer_data.Measurement`.

Example:

```python
async def nl_rivm_luchtmeetnet_mqtt_measurement_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: Measurement) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `nl_rivm_luchtmeetnet_mqtt_lki_async`



## Test```python

nl_rivm_luchtmeetnet_mqtt_lki_async:  Callable[[PartitionContext, EventData, CloudEvent, LKI], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `luchtmeetnet_nl_mqtt_producer_data.LKI`.

Example:

```python
async def nl_rivm_luchtmeetnet_mqtt_lki_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: LKI) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = NlRivmLuchtmeetnetComponentsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NlRivmLuchtmeetnetComponentsMqttEventDispatcher



# Publish message`NlRivmLuchtmeetnetComponentsMqttEventDispatcher` handles events for the
nl.rivm.luchtmeetnet.components.mqtt message group.
...
## BuildThe NlRivmLuchtmeetnetComponentsMqttEventDispatcher defines the following event handler hooks.
...
```##### `nl_rivm_luchtmeetnet_components_mqtt_component_async`



## Test```python

nl_rivm_luchtmeetnet_components_mqtt_component_async:  Callable[[PartitionContext, EventData, CloudEvent, Component],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `luchtmeetnet_nl_mqtt_producer_data.Component`.

Example:

```python
async def nl_rivm_luchtmeetnet_components_mqtt_component_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: Component) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
