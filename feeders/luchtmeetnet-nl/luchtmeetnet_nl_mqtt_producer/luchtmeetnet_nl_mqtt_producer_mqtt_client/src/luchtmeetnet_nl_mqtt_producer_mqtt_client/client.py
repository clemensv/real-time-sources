...
def get_default_topic_mappings_nl_rivm_luchtmeetnet_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the nl.rivm.luchtmeetnet.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "nl.rivm.luchtmeetnet.mqtt.Station": "air-quality/nl/rijkswaterstaat/luchtmeetnet-nl/{region}/{station_number}/{formula}/info",
        "nl.rivm.luchtmeetnet.mqtt.Measurement": "air-quality/nl/rijkswaterstaat/luchtmeetnet-nl/{region}/{station_number}/{formula}/measurement",
        "nl.rivm.luchtmeetnet.mqtt.LKI": "air-quality/nl/rijkswaterstaat/luchtmeetnet-nl/{region}/{station_number}/{formula}/lki",
    }


def get_subscription_topics_nl_rivm_luchtmeetnet_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_nl_rivm_luchtmeetnet_mqtt()
    topics = []
...
class NlRivmLuchtmeetnetMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the nl.rivm.luchtmeetnet.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_nl_rivm_luchtmeetnet_mqtt()
        ...
        self.nl_rivm_luchtmeetnet_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, luchtmeetnet_nl_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.nl_rivm_luchtmeetnet_mqtt_measurement_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, luchtmeetnet_nl_mqtt_producer_data.Measurement, Dict[str, str]], Awaitable[None]]] = None
        
        self.nl_rivm_luchtmeetnet_mqtt_lki_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, luchtmeetnet_nl_mqtt_producer_data.LKI, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "nl.rivm.luchtmeetnet.Station":
            if self.nl_rivm_luchtmeetnet_mqtt_station_async:
                try:
                    ...
                    data = luchtmeetnet_nl_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "nl.rivm.luchtmeetnet.mqtt.Station")
                    await self.nl_rivm_luchtmeetnet_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in nl_rivm_luchtmeetnet_mqtt_station handler: {e}")
            return
        
        if event_type == "nl.rivm.luchtmeetnet.Measurement":
            if self.nl_rivm_luchtmeetnet_mqtt_measurement_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = luchtmeetnet_nl_mqtt_producer_data.Measurement.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "nl.rivm.luchtmeetnet.mqtt.Measurement")
                    await self.nl_rivm_luchtmeetnet_mqtt_measurement_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in nl_rivm_luchtmeetnet_mqtt_measurement handler: {e}")
            return
        
        if event_type == "nl.rivm.luchtmeetnet.LKI":
            if self.nl_rivm_luchtmeetnet_mqtt_lki_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = luchtmeetnet_nl_mqtt_producer_data.LKI.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "nl.rivm.luchtmeetnet.mqtt.LKI")
                    await self.nl_rivm_luchtmeetnet_mqtt_lki_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in nl_rivm_luchtmeetnet_mqtt_lki handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_nl_rivm_luchtmeetnet_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_nl_rivm_luchtmeetnet_mqtt_station(self,
        station_number: str,
        region: str,
        formula: str,
        data: luchtmeetnet_nl_mqtt_producer_data.Station,
        ...
        """
        Publish the 'nl.rivm.luchtmeetnet.mqtt.Station' event to an MQTT topic.

        Args:
        
            station_number: URI template variable for 'station_number'
            region: URI template variable for 'region'
            formula: URI template variable for 'formula'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/nl/rijkswaterstaat/luchtmeetnet-nl/{region}/{station_number}/{formula}/info'
        ...
        """
        target_topic = topic if topic is not None else "air-quality/nl/rijkswaterstaat/luchtmeetnet-nl/{region}/{station_number}/{formula}/info"
        ...
        attributes = {
             ...
             "subject":"{station_number}".format(station_number = station_number)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_nl_rivm_luchtmeetnet_mqtt_measurement(self,
        station_number: str,
        region: str,
        formula: str,
        data: luchtmeetnet_nl_mqtt_producer_data.Measurement,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'nl.rivm.luchtmeetnet.mqtt.Measurement' event to an MQTT topic.
        ...
        Args:
            ...
            formula: URI template variable for 'formula'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/nl/rijkswaterstaat/luchtmeetnet-nl/{region}/{station_number}/{formula}/measurement'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/nl/rijkswaterstaat/luchtmeetnet-nl/{region}/{station_number}/{formula}/measurement"
        _topic_template_values: Dict[str, str] = {
            "station_number": str(station_number),
            "region": str(region),
            "formula": str(formula),
    ...
    async def publish_nl_rivm_luchtmeetnet_mqtt_lki(self,
        station_number: str,
        region: str,
        formula: str,
        data: luchtmeetnet_nl_mqtt_producer_data.LKI,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'nl.rivm.luchtmeetnet.mqtt.LKI' event to an MQTT topic.
        ...
        Args:
            ...
            formula: URI template variable for 'formula'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/nl/rijkswaterstaat/luchtmeetnet-nl/{region}/{station_number}/{formula}/lki'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/nl/rijkswaterstaat/luchtmeetnet-nl/{region}/{station_number}/{formula}/lki"
        _topic_template_values: Dict[str, str] = {
            "station_number": str(station_number),
            "region": str(region),
            "formula": str(formula),
...
def get_default_topic_mappings_nl_rivm_luchtmeetnet_components_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the nl.rivm.luchtmeetnet.components.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "nl.rivm.luchtmeetnet.components.mqtt.Component": "air-quality/nl/rijkswaterstaat/luchtmeetnet-nl/{region}/{station_number}/{formula}/component",
    }


def get_subscription_topics_nl_rivm_luchtmeetnet_components_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_nl_rivm_luchtmeetnet_components_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class NlRivmLuchtmeetnetComponentsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the nl.rivm.luchtmeetnet.components.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_nl_rivm_luchtmeetnet_components_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        
        # Message handler callbacks (Dispatcher pattern)
        
        self.nl_rivm_luchtmeetnet_components_mqtt_component_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, luchtmeetnet_nl_mqtt_producer_data.Component, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "nl.rivm.luchtmeetnet.components.Component":
            if self.nl_rivm_luchtmeetnet_components_mqtt_component_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = luchtmeetnet_nl_mqtt_producer_data.Component.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "nl.rivm.luchtmeetnet.components.mqtt.Component")
                    await self.nl_rivm_luchtmeetnet_components_mqtt_component_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in nl_rivm_luchtmeetnet_components_mqtt_component handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_nl_rivm_luchtmeetnet_components_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_nl_rivm_luchtmeetnet_components_mqtt_component(self,
        formula: str,
        region: str,
        station_number: str,
        data: luchtmeetnet_nl_mqtt_producer_data.Component,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'nl.rivm.luchtmeetnet.components.mqtt.Component' event to an MQTT topic.
        ...
        Args:
            ...
            station_number: URI template variable for 'station_number'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/nl/rijkswaterstaat/luchtmeetnet-nl/{region}/{station_number}/{formula}/component'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/nl/rijkswaterstaat/luchtmeetnet-nl/{region}/{station_number}/{formula}/component"
        _topic_template_values: Dict[str, str] = {
            "formula": str(formula),
            "region": str(region),
            "station_number": str(station_number),
        ...
        attributes = {
             ...
             "subject":"{formula}".format(formula = formula)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
