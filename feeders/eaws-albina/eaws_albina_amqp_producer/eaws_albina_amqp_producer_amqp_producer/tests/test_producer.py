...
"""
Tests for eaws_albina_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../eaws_albina_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../eaws_albina_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../eaws_albina_amqp_producer_amqp_producer/src')))

from eaws_albina_amqp_producer_amqp_producer import *
...
class TestOrgEAWSALBINAAmqpProducer:
    """Test cases for OrgEAWSALBINAAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(OrgEAWSALBINAAmqpProducer)
    ...
    def test_send_avalanche_bulletin(self, artemis_container):
        """Send and receive a AvalancheBulletin message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_AvalancheBulletin.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                assert properties.get('danger_level') == "{danger_level}".format(danger_level="value")
                assert annotations.get(symbol('x-opt-partition-key')) == str("{region_id}".format(region_id="value"))[:128]
        finally:
            producer.close()

    def test_send_avalanche_bulletin_single_fresh_connection(self, artemis_container):
        """Send exactly one AvalancheBulletin message on a fresh producer connection."""
        payload = Test_AvalancheBulletin.create_instance()
