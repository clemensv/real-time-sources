...
import os
import signal
from eaws_albina_mqtt_producer_data import *
from eaws_albina_mqtt_producer_mqtt_client.client import OrgEAWSALBINAMqttProducer, OrgEAWSALBINAMqttDispatcher

async def handle_org_eaws_albina_mqtt_avalanche_bulletin(mqtt_msg,cloud_event, org_eaws_albina_mqtt_avalanche_bulletin_data):
    """ Handles the org.EAWS.ALBINA.mqtt.AvalancheBulletin message """
    print(f"Received org.EAWS.ALBINA.mqtt.AvalancheBulletin on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_eaws_albina_mqtt_avalanche_bulletin_data}")
