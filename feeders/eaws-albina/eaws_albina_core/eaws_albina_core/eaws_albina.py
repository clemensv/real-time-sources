...
class AlbinaPoller:
    ...
    def parse_bulletins(data: dict, lang: str) -> List[AvalancheBulletin]:
        ...
        for b in data.get('bulletins', []):
            ...
            pub_time = datetime.datetime.fromisoformat(pub_time_str.replace("Z", "+00:00")); vt_start = datetime.datetime.fromisoformat(vt_start_str.replace("Z", "+00:00")); vt_end = datetime.datetime.fromisoformat(vt_end_str.replace("Z", "+00:00")); danger_ratings = b.get('dangerRatings', []); max_name, max_val = AlbinaPoller.compute_max_danger(danger_ratings); max_enum = DANGER_RATING_ENUM_MAP.get(max_name) if max_name else None; avalanche_problems = b.get('avalancheProblems', []); tendency_list = b.get('tendency', []); tendency_type = tendency_list[0].get('tendencyType') if tendency_list else None; custom = b.get('customData', {}); lwd = custom.get('LWD_Tyrol', {}); patterns = lwd.get('dangerPatterns'); patterns_json = json.dumps(patterns) if patterns else None; activity = b.get('avalancheActivity', {}); highlights = activity.get('highlights'); snowpack = b.get('snowpackStructure', {}); snowpack_comment = snowpack.get('comment')
