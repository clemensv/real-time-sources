# Kystverket_ais_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the kystverket_ais_mqtt_producer message definitions.

# Kystverket_ais_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - NOKystverketAISMqttEventDispatcher
...
from kystverket_ais_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = NOKystverketAISMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NOKystverketAISMqttEventDispatcher



# Publish message`NOKystverketAISMqttEventDispatcher` handles events for the NO.Kystverket.AIS.mqtt message group.
...
from kystverket_ais_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = NOKystverketAISMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe NOKystverketAISMqttEventDispatcher defines the following event handler hooks.
...
```##### `no_kystverket_ais_mqtt_position_report_async`



## Test```python

no_kystverket_ais_mqtt_position_report_async:  Callable[[PartitionContext, EventData, CloudEvent, PositionReport],
Awaitable[None]]
...
- `data`: The event data of type `kystverket_ais_mqtt_producer_data.PositionReport`.

Example:

```python
async def no_kystverket_ais_mqtt_position_report_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: PositionReport) -> None:
...
```##### `no_kystverket_ais_mqtt_ship_static_async`



## Test```python

no_kystverket_ais_mqtt_ship_static_async:  Callable[[PartitionContext, EventData, CloudEvent, ShipStatic],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `kystverket_ais_mqtt_producer_data.ShipStatic`.

Example:

```python
async def no_kystverket_ais_mqtt_ship_static_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `no_kystverket_ais_mqtt_aid_to_navigation_async`



## Test```python

no_kystverket_ais_mqtt_aid_to_navigation_async:  Callable[[PartitionContext, EventData, CloudEvent, AidToNavigation],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `kystverket_ais_mqtt_producer_data.AidToNavigation`.

Example:

```python
async def no_kystverket_ais_mqtt_aid_to_navigation_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: AidToNavigation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
