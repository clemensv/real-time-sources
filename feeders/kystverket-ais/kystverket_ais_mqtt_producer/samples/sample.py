...
from kystverket_ais_mqtt_producer_mqtt_client.client import NOKystverketAISMqttProducer, NOKystverketAISMqttDispatcher

async def handle_no_kystverket_ais_mqtt_position_report(mqtt_msg,cloud_event, no_kystverket_ais_mqtt_position_report_data):
    """ Handles the NO.Kystverket.AIS.mqtt.PositionReport message """
    print(f"Received NO.Kystverket.AIS.mqtt.PositionReport on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {no_kystverket_ais_mqtt_position_report_data}")

async def handle_no_kystverket_ais_mqtt_ship_static(mqtt_msg,cloud_event, no_kystverket_ais_mqtt_ship_static_data):
    """ Handles the NO.Kystverket.AIS.mqtt.ShipStatic message """
    print(f"Received NO.Kystverket.AIS.mqtt.ShipStatic on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {no_kystverket_ais_mqtt_ship_static_data}")

async def handle_no_kystverket_ais_mqtt_aid_to_navigation(mqtt_msg,cloud_event, no_kystverket_ais_mqtt_aid_to_navigation_data):
    """ Handles the NO.Kystverket.AIS.mqtt.AidToNavigation message """
    print(f"Received NO.Kystverket.AIS.mqtt.AidToNavigation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {no_kystverket_ais_mqtt_aid_to_navigation_data}")
