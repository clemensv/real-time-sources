...
# Entur_norway_producer Kafka Producer# Entur_norway_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - NoEnturJourneysEventDispatcher,
...
- NoEnturJourneysProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- NoEnturSituationsProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### NoEnturJourneysEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`NoEnturJourneysEventDispatcher` handles events for the no.entur.journeys message group.
...
The NoEnturJourneysEventDispatcher defines the following event handler hooks.
...
producer = NoEnturJourneysProducer(
    ...
    security_protocol='SASL_SSL',##### `no_entur_dated_service_journey_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'no_entur_dated_service_journey_async:  Callable[[ConsumerRecord, CloudEvent,
DatedServiceJourney], Awaitable[None]]

)```

```

Asynchronous handler hook for `no.entur.DatedServiceJourney`: Reference data for a dated service journey in the
Norwegian public transport network. A DatedServiceJourney is a specific vehicle journey operating on a particular
operating day, identified by its NeTEx ServiceJourney reference and operating day. This reference event is emitted at
bridge startup from the SIRI-ET feed and refreshed periodically to provide downstream consumers with the timetable
...
### NoEnturJourneysProducer- `data`: The event data of type `entur_norway_producer_data.DatedServiceJourney`.
...
no_entur_journeys_dispatcher.no_entur_dated_service_journey_async = no_entur_dated_service_journey_event
...
### NoEnturSituationsProducer- `data`: The event data of type `entur_norway_producer_data.DatedServiceJourney`.
...
no_entur_situations_dispatcher.no_entur_dated_service_journey_async = no_entur_dated_service_journey_event
...
### NoEnturMqttProducer- `data`: The event data of type `entur_norway_producer_data.DatedServiceJourney`.
...
no_entur_mqtt_dispatcher.no_entur_dated_service_journey_async = no_entur_dated_service_journey_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `no_entur_estimated_vehicle_journey_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'no_entur_estimated_vehicle_journey_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `no.entur.EstimatedVehicleJourney`: Real-time estimated timetable update for a vehicle
...
only changed journeys since the last poll.
...
no_entur_journeys_dispatcher.no_entur_estimated_vehicle_journey_async = no_entur_estimated_vehicle_journey_event
...
no_entur_situations_dispatcher.no_entur_estimated_vehicle_journey_async = no_entur_estimated_vehicle_journey_event
...
no_entur_mqtt_dispatcher.no_entur_estimated_vehicle_journey_async = no_entur_estimated_vehicle_journey_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `no_entur_monitored_vehicle_journey_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'no_entur_monitored_vehicle_journey_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `no.entur.MonitoredVehicleJourney`: Real-time vehicle monitoring update from the Entur
SIRI-VM feed (GET /realtime/v1/rest/vm). Contains the current geographic position of a vehicle, its bearing, delay,
occupancy status, and the next monitored call. Uses incremental requestorId polling for efficient change delivery.
...
no_entur_journeys_dispatcher.no_entur_monitored_vehicle_journey_async = no_entur_monitored_vehicle_journey_event
...
no_entur_situations_dispatcher.no_entur_monitored_vehicle_journey_async = no_entur_monitored_vehicle_journey_event
...
##### `send_no_entur_dated_service_journey`Dispatchers have the following protected methods:



```python### Methods:

async def send_no_entur_dated_service_journey(

    self,##### `_process_event`

    data: DatedServiceJourney,
...
Send a single `no.entur.DatedServiceJourney` message. Reference data for a dated service journey in the Norwegian public
transport network. A DatedServiceJourney is a specific vehicle journey operating on a particular operating day,
identified by its NeTEx ServiceJourney reference and operating day. This reference event is emitted at bridge startup
from the SIRI-ET feed and refreshed periodically to provide downstream consumers with the timetable context for
subsequent telemetry events.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `DatedServiceJourney`
...
await producer.send_no_entur_dated_service_journey(

    data=DatedServiceJourney(...),Args:
...
Send multiple `no.entur.DatedServiceJourney` messages in a batch.
...
await producer.send_no_entur_dated_service_journey_batch(```

    messages=[

        DatedServiceJourney(...),Initializes the runner with a Kafka consumer.

        DatedServiceJourney(...),

        DatedServiceJourney(...)Args:
...
##### `send_no_entur_estimated_vehicle_journey`Dispatchers have the following protected methods:



```python### Methods:

async def send_no_entur_estimated_vehicle_journey(
...
Send a single `no.entur.EstimatedVehicleJourney` message. Real-time estimated timetable update for a vehicle journey
from the Entur SIRI-ET feed (GET /realtime/v1/rest/et). Contains updated arrival and departure times for each stop along
the journey, cancellation flags, and extra journey markers. Uses incremental requestorId polling to receive only changed
journeys since the last poll.Args:
...
await producer.send_no_entur_estimated_vehicle_journey(
...
Send multiple `no.entur.EstimatedVehicleJourney` messages in a batch.
...
await producer.send_no_entur_estimated_vehicle_journey_batch(```
...
##### `send_no_entur_monitored_vehicle_journey`Dispatchers have the following protected methods:



```python### Methods:

async def send_no_entur_monitored_vehicle_journey(
...
Send a single `no.entur.MonitoredVehicleJourney` message. Real-time vehicle monitoring update from the Entur SIRI-VM
feed (GET /realtime/v1/rest/vm). Contains the current geographic position of a vehicle, its bearing, delay, occupancy
...
await producer.send_no_entur_monitored_vehicle_journey(
...
Send multiple `no.entur.MonitoredVehicleJourney` messages in a batch.
...
await producer.send_no_entur_monitored_vehicle_journey_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### NoEnturSituationsEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`NoEnturSituationsEventDispatcher` handles events for the no.entur.situations message group.
...
The NoEnturSituationsEventDispatcher defines the following event handler hooks.
...
producer = NoEnturJourneysProducer(
    ...
    security_protocol='SASL_SSL',##### `no_entur_pt_situation_element_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'no_entur_pt_situation_element_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `no.entur.PtSituationElement`: Real-time transit disruption or service alert from the
...
no_entur_journeys_dispatcher.no_entur_pt_situation_element_async = no_entur_pt_situation_element_event
...
no_entur_situations_dispatcher.no_entur_pt_situation_element_async = no_entur_pt_situation_element_event
...
no_entur_mqtt_dispatcher.no_entur_pt_situation_element_async = no_entur_pt_situation_element_event
...
##### `send_no_entur_pt_situation_element`Dispatchers have the following protected methods:



```python### Methods:

async def send_no_entur_pt_situation_element(
...
Send a single `no.entur.PtSituationElement` message. Real-time transit disruption or service alert from the Entur SIRI-
SX feed (GET /realtime/v1/rest/sx). Represents a published situation element describing a service disruption, delay
...
await producer.send_no_entur_pt_situation_element(
...
Send multiple `no.entur.PtSituationElement` messages in a batch.
...
await producer.send_no_entur_pt_situation_element_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### NoEnturMqttEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`NoEnturMqttEventDispatcher` handles events for the no.entur.mqtt message group.
...
The NoEnturMqttEventDispatcher defines the following event handler hooks.
...
producer = NoEnturJourneysProducer(
    ...
    security_protocol='SASL_SSL',##### `no_entur_mqtt_estimated_vehicle_journey_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'no_entur_mqtt_estimated_vehicle_journey_async:  Callable[[ConsumerRecord, CloudEvent,
EstimatedVehicleJourney], Awaitable[None]]
...
Asynchronous handler hook for `no.entur.mqtt.EstimatedVehicleJourney`: Real-time estimated timetable update for a
...
no_entur_journeys_dispatcher.no_entur_mqtt_estimated_vehicle_journey_async =
no_entur_mqtt_estimated_vehicle_journey_event
...
no_entur_situations_dispatcher.no_entur_mqtt_estimated_vehicle_journey_async =
no_entur_mqtt_estimated_vehicle_journey_event
...
no_entur_mqtt_dispatcher.no_entur_mqtt_estimated_vehicle_journey_async = no_entur_mqtt_estimated_vehicle_journey_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `no_entur_mqtt_monitored_vehicle_journey_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'no_entur_mqtt_monitored_vehicle_journey_async:  Callable[[ConsumerRecord, CloudEvent,
MonitoredVehicleJourney], Awaitable[None]]
...
Asynchronous handler hook for `no.entur.mqtt.MonitoredVehicleJourney`: Real-time vehicle monitoring update from the
...
no_entur_journeys_dispatcher.no_entur_mqtt_monitored_vehicle_journey_async =
no_entur_mqtt_monitored_vehicle_journey_event
...
no_entur_situations_dispatcher.no_entur_mqtt_monitored_vehicle_journey_async =
no_entur_mqtt_monitored_vehicle_journey_event
...
no_entur_mqtt_dispatcher.no_entur_mqtt_monitored_vehicle_journey_async = no_entur_mqtt_monitored_vehicle_journey_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `no_entur_mqtt_pt_situation_element_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'no_entur_mqtt_pt_situation_element_async:  Callable[[ConsumerRecord, CloudEvent,
PtSituationElement], Awaitable[None]]
...
Asynchronous handler hook for `no.entur.mqtt.PtSituationElement`: Real-time transit disruption or service alert from the
Entur SIRI-SX feed (GET /realtime/v1/rest/sx). Represents a published situation element describing a service disruption,
delay cause, or passenger information notice. Includes affected lines, stops, severity level, and validity periods.
...
no_entur_journeys_dispatcher.no_entur_mqtt_pt_situation_element_async = no_entur_mqtt_pt_situation_element_event
...
no_entur_situations_dispatcher.no_entur_mqtt_pt_situation_element_async = no_entur_mqtt_pt_situation_element_event
...
##### `send_no_entur_mqtt_estimated_vehicle_journey`Dispatchers have the following protected methods:



```python### Methods:

async def send_no_entur_mqtt_estimated_vehicle_journey(
...
Send a single `no.entur.mqtt.EstimatedVehicleJourney` message. Real-time estimated timetable update for a vehicle
journey from the Entur SIRI-ET feed (GET /realtime/v1/rest/et). Contains updated arrival and departure times for each
stop along the journey, cancellation flags, and extra journey markers. Uses incremental requestorId polling to receive
...
await producer.send_no_entur_mqtt_estimated_vehicle_journey(
...
Send multiple `no.entur.mqtt.EstimatedVehicleJourney` messages in a batch.
...
await producer.send_no_entur_mqtt_estimated_vehicle_journey_batch(```
...
##### `send_no_entur_mqtt_monitored_vehicle_journey`Dispatchers have the following protected methods:



```python### Methods:

async def send_no_entur_mqtt_monitored_vehicle_journey(
...
Send a single `no.entur.mqtt.MonitoredVehicleJourney` message. Real-time vehicle monitoring update from the Entur SIRI-
...
await producer.send_no_entur_mqtt_monitored_vehicle_journey(
...
Send multiple `no.entur.mqtt.MonitoredVehicleJourney` messages in a batch.
...
await producer.send_no_entur_mqtt_monitored_vehicle_journey_batch(```
...
##### `send_no_entur_mqtt_pt_situation_element`Dispatchers have the following protected methods:



```python### Methods:

async def send_no_entur_mqtt_pt_situation_element(
...
Send a single `no.entur.mqtt.PtSituationElement` message. Real-time transit disruption or service alert from the Entur
...
await producer.send_no_entur_mqtt_pt_situation_element(
...
Send multiple `no.entur.mqtt.PtSituationElement` messages in a batch.
