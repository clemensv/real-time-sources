...
def get_default_topic_mappings_no_entur_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the no.entur.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "no.entur.mqtt.EstimatedVehicleJourney": "transit/no/entur/entur-norway/et/{operator_ref}/{line_ref}/{service_journey_id}/estimated-vehicle-journey",
        "no.entur.mqtt.MonitoredVehicleJourney": "transit/no/entur/entur-norway/vm/{operator_ref}/{line_ref}/{service_journey_id}/monitored-vehicle-journey",
        "no.entur.mqtt.PtSituationElement": "transit/no/entur/entur-norway/sx/{severity}/{situation_number}/situation",
    }


def get_subscription_topics_no_entur_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_no_entur_mqtt()
    topics = []
...
class NoEnturMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the no.entur.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_no_entur_mqtt()
        ...
        self.no_entur_mqtt_estimated_vehicle_journey_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, entur_norway_mqtt_producer_data.EstimatedVehicleJourney, Dict[str, str]], Awaitable[None]]] = None
        
        self.no_entur_mqtt_monitored_vehicle_journey_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, entur_norway_mqtt_producer_data.MonitoredVehicleJourney, Dict[str, str]], Awaitable[None]]] = None
        
        self.no_entur_mqtt_pt_situation_element_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, entur_norway_mqtt_producer_data.PtSituationElement, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "no.entur.EstimatedVehicleJourney":
            if self.no_entur_mqtt_estimated_vehicle_journey_async:
                try:
                    ...
                    data = entur_norway_mqtt_producer_data.EstimatedVehicleJourney.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "no.entur.mqtt.EstimatedVehicleJourney")
                    await self.no_entur_mqtt_estimated_vehicle_journey_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in no_entur_mqtt_estimated_vehicle_journey handler: {e}")
            return
        
        if event_type == "no.entur.MonitoredVehicleJourney":
            if self.no_entur_mqtt_monitored_vehicle_journey_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = entur_norway_mqtt_producer_data.MonitoredVehicleJourney.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "no.entur.mqtt.MonitoredVehicleJourney")
                    await self.no_entur_mqtt_monitored_vehicle_journey_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in no_entur_mqtt_monitored_vehicle_journey handler: {e}")
            return
        
        if event_type == "no.entur.PtSituationElement":
            if self.no_entur_mqtt_pt_situation_element_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = entur_norway_mqtt_producer_data.PtSituationElement.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "no.entur.mqtt.PtSituationElement")
                    await self.no_entur_mqtt_pt_situation_element_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in no_entur_mqtt_pt_situation_element handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_no_entur_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_no_entur_mqtt_estimated_vehicle_journey(self,
        operating_day: str,
        service_journey_id: str,
        operator_ref: str,
        line_ref: str,
        data: entur_norway_mqtt_producer_data.EstimatedVehicleJourney,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'no.entur.mqtt.EstimatedVehicleJourney' event to an MQTT topic.

        Args:
        
            operating_day: URI template variable for 'operating_day'
            service_journey_id: URI template variable for 'service_journey_id'
            operator_ref: URI template variable for 'operator_ref'
            line_ref: URI template variable for 'line_ref'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'transit/no/entur/entur-norway/et/{operator_ref}/{line_ref}/{service_journey_id}/estimated-vehicle-journey'
        ...
        """
        target_topic = topic if topic is not None else "transit/no/entur/entur-norway/et/{operator_ref}/{line_ref}/{service_journey_id}/estimated-vehicle-journey"
        _topic_template_values: Dict[str, str] = {
            "operating_day": str(operating_day),
            "service_journey_id": str(service_journey_id),
        ...
        attributes = {
             ...
             "subject":"journeys/{operating_day}/{service_journey_id}".format(operating_day = operating_day,service_journey_id = service_journey_id)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_no_entur_mqtt_monitored_vehicle_journey(self,
        operating_day: str,
        service_journey_id: str,
        operator_ref: str,
        line_ref: str,
        data: entur_norway_mqtt_producer_data.MonitoredVehicleJourney,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'no.entur.mqtt.MonitoredVehicleJourney' event to an MQTT topic.
        ...
        Args:
            ...
            line_ref: URI template variable for 'line_ref'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'transit/no/entur/entur-norway/vm/{operator_ref}/{line_ref}/{service_journey_id}/monitored-vehicle-journey'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "transit/no/entur/entur-norway/vm/{operator_ref}/{line_ref}/{service_journey_id}/monitored-vehicle-journey"
        _topic_template_values: Dict[str, str] = {
            "operating_day": str(operating_day),
            "service_journey_id": str(service_journey_id),
            "operator_ref": str(operator_ref),
    ...
    async def publish_no_entur_mqtt_pt_situation_element(self,
        situation_number: str,
        severity: str,
        data: entur_norway_mqtt_producer_data.PtSituationElement,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'no.entur.mqtt.PtSituationElement' event to an MQTT topic.

        Args:
        
            situation_number: URI template variable for 'situation_number'
            severity: URI template variable for 'severity'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'transit/no/entur/entur-norway/sx/{severity}/{situation_number}/situation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "transit/no/entur/entur-norway/sx/{severity}/{situation_number}/situation"
        _topic_template_values: Dict[str, str] = {
            "situation_number": str(situation_number),
        ...
        attributes = {
             ...
             "subject":"situations/{situation_number}".format(situation_number = situation_number)
        }
        attributes["datacontenttype"] = content_type
        byte_data = data.to_byte_array(content_type) if data is not None else b''
        # to_byte_array returns str for text content types (e.g. JSON);
