# Entur_norway_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the entur_norway_mqtt_producer message definitions.

# Entur_norway_mqtt_producer Event Dispatcher for Azure Event Hubs
...
from entur_norway_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = NoEnturMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NoEnturMqttEventDispatcher



# Publish message`NoEnturMqttEventDispatcher` handles events for the no.entur.mqtt message group.
...
from entur_norway_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = NoEnturMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe NoEnturMqttEventDispatcher defines the following event handler hooks.
...
```##### `no_entur_mqtt_estimated_vehicle_journey_async`



## Test```python

no_entur_mqtt_estimated_vehicle_journey_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `data`: The event data of type `entur_norway_mqtt_producer_data.EstimatedVehicleJourney`.

Example:

```python
async def no_entur_mqtt_estimated_vehicle_journey_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: EstimatedVehicleJourney) -> None:
...
```##### `no_entur_mqtt_monitored_vehicle_journey_async`



## Test```python

no_entur_mqtt_monitored_vehicle_journey_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `entur_norway_mqtt_producer_data.MonitoredVehicleJourney`.

Example:

```python
async def no_entur_mqtt_monitored_vehicle_journey_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: MonitoredVehicleJourney) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `no_entur_mqtt_pt_situation_element_async`



## Test```python

no_entur_mqtt_pt_situation_element_async:  Callable[[PartitionContext, EventData, CloudEvent, PtSituationElement],
Awaitable[None]]
...
```Asynchronous handler hook for `no.entur.mqtt.PtSituationElement`: Real-time transit disruption or service alert from
the Entur SIRI-SX feed (GET /realtime/v1/rest/sx). Represents a published situation element describing a service
disruption, delay cause, or passenger information notice. Includes affected lines, stops, severity level, and validity
periods.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `entur_norway_mqtt_producer_data.PtSituationElement`.

Example:

```python
async def no_entur_mqtt_pt_situation_element_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: PtSituationElement) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
