...
import os
import signal
from entur_norway_mqtt_producer_data import *
from entur_norway_mqtt_producer_mqtt_client.client import NoEnturMqttProducer, NoEnturMqttDispatcher

async def handle_no_entur_mqtt_estimated_vehicle_journey(mqtt_msg,cloud_event, no_entur_mqtt_estimated_vehicle_journey_data):
    """ Handles the no.entur.mqtt.EstimatedVehicleJourney message """
    print(f"Received no.entur.mqtt.EstimatedVehicleJourney on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {no_entur_mqtt_estimated_vehicle_journey_data}")

async def handle_no_entur_mqtt_monitored_vehicle_journey(mqtt_msg,cloud_event, no_entur_mqtt_monitored_vehicle_journey_data):
    """ Handles the no.entur.mqtt.MonitoredVehicleJourney message """
    print(f"Received no.entur.mqtt.MonitoredVehicleJourney on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {no_entur_mqtt_monitored_vehicle_journey_data}")

async def handle_no_entur_mqtt_pt_situation_element(mqtt_msg,cloud_event, no_entur_mqtt_pt_situation_element_data):
    """ Handles the no.entur.mqtt.PtSituationElement message """
    print(f"Received no.entur.mqtt.PtSituationElement on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {no_entur_mqtt_pt_situation_element_data}")
