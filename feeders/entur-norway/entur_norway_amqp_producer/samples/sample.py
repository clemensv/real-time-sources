
"""
Sample usage of entur_norway_amqp_producer_amqp_producer

This example demonstrates how to send messages using the AMQP 1.0 producer.
"""
import sys
from entur_norway_amqp_producer_amqp_producer import *

def main():
    """Main function"""
    
    
    # Create producer
    print("Creating AMQP producer...")
    producer = NoEnturAmqpProducer(
        host="localhost",  # Change to your AMQP broker hostname
        address="my-queue",  # Change to your queue/topic name
        port=5672,  # Use 5671 for TLS
        username="guest",  # Change to your username
        password="guest",  # Change to your password
        content_mode='structured',  # or 'binary' for CloudEvents
        format_type='application/json'
    )
    
    try:
        
        
        # Send EstimatedVehicleJourney message
        print("Sending EstimatedVehicleJourney message...")
        # TODO: Create a EstimatedVehicleJourney instance with actual data
        # data = EstimatedVehicleJourney(...)
        # producer.send_estimated_vehicle_journey(
        #     data=data,
        #     content_type="application/json"
        # )
        # print("EstimatedVehicleJourney message sent successfully!")
        
        
        
        # Send MonitoredVehicleJourney message
        print("Sending MonitoredVehicleJourney message...")
        # TODO: Create a MonitoredVehicleJourney instance with actual data
        # data = MonitoredVehicleJourney(...)
        # producer.send_monitored_vehicle_journey(
        #     data=data,
        #     content_type="application/json"
        # )
        # print("MonitoredVehicleJourney message sent successfully!")
        
        
        
        # Send PtSituationElement message
        print("Sending PtSituationElement message...")
        # TODO: Create a PtSituationElement instance with actual data
        # data = PtSituationElement(...)
        # producer.send_pt_situation_element(
        #     data=data,
        #     content_type="application/json"
        # )
        # print("PtSituationElement message sent successfully!")
        
        
        print("\nAll messages sent successfully!")
        
    except Exception as e:
        print(f"Error sending messages: {e}", file=sys.stderr)
        return 1
    finally:
        producer.close()
        print("Producer closed")
    
    return 0
    

if __name__ == "__main__":
    sys.exit(main())