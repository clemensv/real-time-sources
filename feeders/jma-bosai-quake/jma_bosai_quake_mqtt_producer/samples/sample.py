...
import os
import signal
from jma_bosai_quake_mqtt_producer_data import *
from jma_bosai_quake_mqtt_producer_mqtt_client.client import JPJMAQuakeMqttProducer, JPJMAQuakeMqttDispatcher

async def handle_jp_jma_quake_mqtt_earthquake_report(mqtt_msg,cloud_event, jp_jma_quake_mqtt_earthquake_report_data):
    """ Handles the JP.JMA.Quake.mqtt.EarthquakeReport message """
    print(f"Received JP.JMA.Quake.mqtt.EarthquakeReport on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_jma_quake_mqtt_earthquake_report_data}")
