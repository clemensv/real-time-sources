...
def get_default_topic_mappings_uk_gov_defra_aurn_stations_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the uk.gov.defra.aurn.Stations.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "uk.gov.defra.aurn.Stations.mqtt.Station": "air-quality/gb/defra/defra-aurn/{region}/{station_id}/info",
    }


def get_subscription_topics_uk_gov_defra_aurn_stations_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_uk_gov_defra_aurn_stations_mqtt()
    topics = []
...
class UkGovDefraAurnStationsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the uk.gov.defra.aurn.Stations.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_uk_gov_defra_aurn_stations_mqtt()
        ...
        self.uk_gov_defra_aurn_stations_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, defra_aurn_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "uk.gov.defra.aurn.Station":
            if self.uk_gov_defra_aurn_stations_mqtt_station_async:
                try:
                    ...
                    data = defra_aurn_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.gov.defra.aurn.Stations.mqtt.Station")
                    await self.uk_gov_defra_aurn_stations_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_gov_defra_aurn_stations_mqtt_station handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_uk_gov_defra_aurn_stations_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_uk_gov_defra_aurn_stations_mqtt_station(self,
        station_id: str,
        region: str,
        data: defra_aurn_mqtt_producer_data.Station,
        ...
        """
        Publish the 'uk.gov.defra.aurn.Stations.mqtt.Station' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'air-quality/gb/defra/defra-aurn/{region}/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "air-quality/gb/defra/defra-aurn/{region}/{station_id}/info"
...
def get_default_topic_mappings_uk_gov_defra_aurn_timeseries_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the uk.gov.defra.aurn.Timeseries.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "uk.gov.defra.aurn.Timeseries.mqtt.Timeseries": "air-quality/gb/defra/defra-aurn/{region}/{station_id}/{pollutant}/timeseries",
        "uk.gov.defra.aurn.Timeseries.mqtt.Observation": "air-quality/gb/defra/defra-aurn/{region}/{station_id}/{pollutant}/observation",
    }


def get_subscription_topics_uk_gov_defra_aurn_timeseries_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_uk_gov_defra_aurn_timeseries_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class UkGovDefraAurnTimeseriesMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the uk.gov.defra.aurn.Timeseries.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_uk_gov_defra_aurn_timeseries_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        
        # Message handler callbacks (Dispatcher pattern)
        
        self.uk_gov_defra_aurn_timeseries_mqtt_timeseries_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, defra_aurn_mqtt_producer_data.Timeseries, Dict[str, str]], Awaitable[None]]] = None
        
        self.uk_gov_defra_aurn_timeseries_mqtt_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, defra_aurn_mqtt_producer_data.Observation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "uk.gov.defra.aurn.Timeseries":
            if self.uk_gov_defra_aurn_timeseries_mqtt_timeseries_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = defra_aurn_mqtt_producer_data.Timeseries.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.gov.defra.aurn.Timeseries.mqtt.Timeseries")
                    await self.uk_gov_defra_aurn_timeseries_mqtt_timeseries_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_gov_defra_aurn_timeseries_mqtt_timeseries handler: {e}")
            return
        
        if event_type == "uk.gov.defra.aurn.Observation":
            if self.uk_gov_defra_aurn_timeseries_mqtt_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = defra_aurn_mqtt_producer_data.Observation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.gov.defra.aurn.Timeseries.mqtt.Observation")
                    await self.uk_gov_defra_aurn_timeseries_mqtt_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_gov_defra_aurn_timeseries_mqtt_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_uk_gov_defra_aurn_timeseries_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_uk_gov_defra_aurn_timeseries_mqtt_timeseries(self,
        timeseries_id: str,
        region: str,
        station_id: str,
        pollutant: str,
        data: defra_aurn_mqtt_producer_data.Timeseries,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'uk.gov.defra.aurn.Timeseries.mqtt.Timeseries' event to an MQTT topic.

        Args:
        
            timeseries_id: URI template variable for 'timeseries_id'
            region: URI template variable for 'region'
            station_id: URI template variable for 'station_id'
            pollutant: URI template variable for 'pollutant'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/gb/defra/defra-aurn/{region}/{station_id}/{pollutant}/timeseries'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/gb/defra/defra-aurn/{region}/{station_id}/{pollutant}/timeseries"
        _topic_template_values: Dict[str, str] = {
        ...
        attributes = {
             ...
             "subject":"{timeseries_id}".format(timeseries_id = timeseries_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
    ...
    async def publish_uk_gov_defra_aurn_timeseries_mqtt_observation(self,
        timeseries_id: str,
        region: str,
        station_id: str,
        pollutant: str,
        data: defra_aurn_mqtt_producer_data.Observation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'uk.gov.defra.aurn.Timeseries.mqtt.Observation' event to an MQTT topic.
        ...
        Args:
            ...
            pollutant: URI template variable for 'pollutant'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/gb/defra/defra-aurn/{region}/{station_id}/{pollutant}/observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/gb/defra/defra-aurn/{region}/{station_id}/{pollutant}/observation"
        _topic_template_values: Dict[str, str] = {
            "timeseries_id": str(timeseries_id),
            "region": str(region),
            "station_id": str(station_id),
