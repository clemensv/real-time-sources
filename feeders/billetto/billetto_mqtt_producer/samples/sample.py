...
import os
import signal
from billetto_mqtt_producer_data import *
from billetto_mqtt_producer_mqtt_client.client import BillettoEventsMqttProducer, BillettoEventsMqttDispatcher

async def handle_billetto_events_mqtt_event(mqtt_msg,cloud_event, billetto_events_mqtt_event_data):
    """ Handles the Billetto.Events.mqtt.Event message """
    print(f"Received Billetto.Events.mqtt.Event on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {billetto_events_mqtt_event_data}")
