...
def get_default_topic_mappings_billetto_events_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the Billetto.Events.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "Billetto.Events.mqtt.Event": "civic-events/intl/billetto/billetto/{country}/{city}/{category}/{event_id}/event",
    }


def get_subscription_topics_billetto_events_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_billetto_events_mqtt()
    topics = []
...
class BillettoEventsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the Billetto.Events.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_billetto_events_mqtt()
        ...
        self.billetto_events_mqtt_event_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, billetto_mqtt_producer_data.Event, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "Billetto.Events.Event":
            if self.billetto_events_mqtt_event_async:
                try:
                    ...
                    data = billetto_mqtt_producer_data.Event.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Billetto.Events.mqtt.Event")
                    await self.billetto_events_mqtt_event_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in billetto_events_mqtt_event handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_billetto_events_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_billetto_events_mqtt_event(self,
        ...
        city: str,
        category: str,
        data: billetto_mqtt_producer_data.Event,
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'civic-events/intl/billetto/billetto/{country}/{city}/{category}/{event_id}/event'
