# Billetto_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the billetto_mqtt_producer message definitions.

# Billetto_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - BillettoEventsMqttEventDispatcher
...
from billetto_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = BillettoEventsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### BillettoEventsMqttEventDispatcher



# Publish message`BillettoEventsMqttEventDispatcher` handles events for the Billetto.Events.mqtt message group.
...
from billetto_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = BillettoEventsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe BillettoEventsMqttEventDispatcher defines the following event handler hooks.
...
```##### `billetto_events_mqtt_event_async`



## Test```python

billetto_events_mqtt_event_async:  Callable[[PartitionContext, EventData, CloudEvent, Event], Awaitable[None]]
...
- `data`: The event data of type `billetto_mqtt_producer_data.Event`.

Example:

```python
async def billetto_events_mqtt_event_event(partition_context: PartitionContext, event: EventData, cloud_event:
