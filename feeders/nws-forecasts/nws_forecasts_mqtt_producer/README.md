# Nws_forecasts_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the nws_forecasts_mqtt_producer message definitions.

# Nws_forecasts_mqtt_producer Event Dispatcher for Azure Event Hubs
...
from nws_forecasts_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = MicrosoftOpenDataUSNOAANWSForecastsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### MicrosoftOpenDataUSNOAANWSForecastsMqttEventDispatcher



# Publish message`MicrosoftOpenDataUSNOAANWSForecastsMqttEventDispatcher` handles events for the
...
from nws_forecasts_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = MicrosoftOpenDataUSNOAANWSForecastsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The
...
## BuildThe MicrosoftOpenDataUSNOAANWSForecastsMqttEventDispatcher defines the following event handler hooks.
...
```##### `microsoft_open_data_us_noaa_nws_forecasts_forecast_zone_mqtt_async`



## Test```python

microsoft_open_data_us_noaa_nws_forecasts_forecast_zone_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
ForecastZone], Awaitable[None]]
...
- `data`: The event data of type `nws_forecasts_mqtt_producer_data.ForecastZone`.

Example:

```python
async def microsoft_open_data_us_noaa_nws_forecasts_forecast_zone_mqtt_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: ForecastZone) -> None:
...
```##### `microsoft_open_data_us_noaa_nws_forecasts_land_zone_forecast_mqtt_async`



## Test```python

microsoft_open_data_us_noaa_nws_forecasts_land_zone_forecast_mqtt_async:  Callable[[PartitionContext, EventData,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `nws_forecasts_mqtt_producer_data.LandZoneForecast`.

Example:

```python
async def microsoft_open_data_us_noaa_nws_forecasts_land_zone_forecast_mqtt_event(partition_context: PartitionContext,
event: EventData, cloud_event: CloudEvent, data: LandZoneForecast) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `microsoft_open_data_us_noaa_nws_forecasts_marine_zone_forecast_mqtt_async`



## Test```python

microsoft_open_data_us_noaa_nws_forecasts_marine_zone_forecast_mqtt_async:  Callable[[PartitionContext, EventData,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `nws_forecasts_mqtt_producer_data.MarineZoneForecast`.

Example:

```python
async def microsoft_open_data_us_noaa_nws_forecasts_marine_zone_forecast_mqtt_event(partition_context: PartitionContext,
event: EventData, cloud_event: CloudEvent, data: MarineZoneForecast) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
