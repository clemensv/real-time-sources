...
def get_default_topic_mappings_microsoft_opendata_us_noaa_nws_forecasts_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the Microsoft.OpenData.US.NOAA.NWS.Forecasts.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "Microsoft.OpenData.US.NOAA.NWS.Forecasts.ForecastZone.mqtt": "weather/us/noaa/nws-forecasts/{state}/{zone_type}/{zone_id}/{event}",
        "Microsoft.OpenData.US.NOAA.NWS.Forecasts.LandZoneForecast.mqtt": "weather/us/noaa/nws-forecasts/{state}/{zone_type}/{zone_id}/{event}",
        "Microsoft.OpenData.US.NOAA.NWS.Forecasts.MarineZoneForecast.mqtt": "weather/us/noaa/nws-forecasts/{state}/{zone_type}/{zone_id}/{event}",
    }


def get_subscription_topics_microsoft_opendata_us_noaa_nws_forecasts_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_microsoft_opendata_us_noaa_nws_forecasts_mqtt()
    topics = []
...
class MicrosoftOpenDataUSNOAANWSForecastsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the Microsoft.OpenData.US.NOAA.NWS.Forecasts.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_microsoft_opendata_us_noaa_nws_forecasts_mqtt()
        ...
        self.microsoft_open_data_us_noaa_nws_forecasts_forecast_zone_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, nws_forecasts_mqtt_producer_data.ForecastZone, Dict[str, str]], Awaitable[None]]] = None
        
        self.microsoft_open_data_us_noaa_nws_forecasts_land_zone_forecast_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, nws_forecasts_mqtt_producer_data.LandZoneForecast, Dict[str, str]], Awaitable[None]]] = None
        
        self.microsoft_open_data_us_noaa_nws_forecasts_marine_zone_forecast_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, nws_forecasts_mqtt_producer_data.MarineZoneForecast, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "Microsoft.OpenData.US.NOAA.NWS.ForecastZone":
            if self.microsoft_open_data_us_noaa_nws_forecasts_forecast_zone_mqtt_async:
                try:
                    ...
                    data = nws_forecasts_mqtt_producer_data.ForecastZone.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Microsoft.OpenData.US.NOAA.NWS.Forecasts.ForecastZone.mqtt")
                    await self.microsoft_open_data_us_noaa_nws_forecasts_forecast_zone_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in microsoft_open_data_us_noaa_nws_forecasts_forecast_zone_mqtt handler: {e}")
            return
        
        if event_type == "Microsoft.OpenData.US.NOAA.NWS.LandZoneForecast":
            if self.microsoft_open_data_us_noaa_nws_forecasts_land_zone_forecast_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = nws_forecasts_mqtt_producer_data.LandZoneForecast.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Microsoft.OpenData.US.NOAA.NWS.Forecasts.LandZoneForecast.mqtt")
                    await self.microsoft_open_data_us_noaa_nws_forecasts_land_zone_forecast_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in microsoft_open_data_us_noaa_nws_forecasts_land_zone_forecast_mqtt handler: {e}")
            return
        
        if event_type == "Microsoft.OpenData.US.NOAA.NWS.MarineZoneForecast":
            if self.microsoft_open_data_us_noaa_nws_forecasts_marine_zone_forecast_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = nws_forecasts_mqtt_producer_data.MarineZoneForecast.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Microsoft.OpenData.US.NOAA.NWS.Forecasts.MarineZoneForecast.mqtt")
                    await self.microsoft_open_data_us_noaa_nws_forecasts_marine_zone_forecast_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in microsoft_open_data_us_noaa_nws_forecasts_marine_zone_forecast_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_microsoft_opendata_us_noaa_nws_forecasts_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_microsoft_open_data_us_noaa_nws_forecasts_forecast_zone_mqtt(self,
        zone_id: str,
        state: str,
        zone_type: str,
        event: str,
        data: nws_forecasts_mqtt_producer_data.ForecastZone,
        ...
        """
        Publish the 'Microsoft.OpenData.US.NOAA.NWS.Forecasts.ForecastZone.mqtt' event to an MQTT topic.

        Args:
        
            zone_id: URI template variable for 'zone_id'
            state: URI template variable for 'state'
            zone_type: URI template variable for 'zone_type'
            event: URI template variable for 'event'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/us/noaa/nws-forecasts/{state}/{zone_type}/{zone_id}/{event}'
        ...
        """
        target_topic = topic if topic is not None else "weather/us/noaa/nws-forecasts/{state}/{zone_type}/{zone_id}/{event}"
        ...
        attributes = {
             ...
             "subject":"{zone_id}".format(zone_id = zone_id)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_microsoft_open_data_us_noaa_nws_forecasts_land_zone_forecast_mqtt(self,
        zone_id: str,
        state: str,
        zone_type: str,
        event: str,
        data: nws_forecasts_mqtt_producer_data.LandZoneForecast,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Microsoft.OpenData.US.NOAA.NWS.Forecasts.LandZoneForecast.mqtt' event to an MQTT topic.
    ...
    async def publish_microsoft_open_data_us_noaa_nws_forecasts_marine_zone_forecast_mqtt(self,
        zone_id: str,
        state: str,
        zone_type: str,
        event: str,
        data: nws_forecasts_mqtt_producer_data.MarineZoneForecast,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Microsoft.OpenData.US.NOAA.NWS.Forecasts.MarineZoneForecast.mqtt' event to an MQTT topic.
