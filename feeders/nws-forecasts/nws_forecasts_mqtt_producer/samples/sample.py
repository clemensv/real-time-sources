...
import os
import signal
from nws_forecasts_mqtt_producer_data import *
from nws_forecasts_mqtt_producer_mqtt_client.client import MicrosoftOpenDataUSNOAANWSForecastsMqttProducer, MicrosoftOpenDataUSNOAANWSForecastsMqttDispatcher

async def handle_microsoft_open_data_us_noaa_nws_forecasts_forecast_zone_mqtt(mqtt_msg,cloud_event, microsoft_open_data_us_noaa_nws_forecasts_forecast_zone_mqtt_data):
    """ Handles the Microsoft.OpenData.US.NOAA.NWS.Forecasts.ForecastZone.mqtt message """
    print(f"Received Microsoft.OpenData.US.NOAA.NWS.Forecasts.ForecastZone.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {microsoft_open_data_us_noaa_nws_forecasts_forecast_zone_mqtt_data}")

async def handle_microsoft_open_data_us_noaa_nws_forecasts_land_zone_forecast_mqtt(mqtt_msg,cloud_event, microsoft_open_data_us_noaa_nws_forecasts_land_zone_forecast_mqtt_data):
    """ Handles the Microsoft.OpenData.US.NOAA.NWS.Forecasts.LandZoneForecast.mqtt message """
    print(f"Received Microsoft.OpenData.US.NOAA.NWS.Forecasts.LandZoneForecast.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {microsoft_open_data_us_noaa_nws_forecasts_land_zone_forecast_mqtt_data}")

async def handle_microsoft_open_data_us_noaa_nws_forecasts_marine_zone_forecast_mqtt(mqtt_msg,cloud_event, microsoft_open_data_us_noaa_nws_forecasts_marine_zone_forecast_mqtt_data):
    """ Handles the Microsoft.OpenData.US.NOAA.NWS.Forecasts.MarineZoneForecast.mqtt message """
    print(f"Received Microsoft.OpenData.US.NOAA.NWS.Forecasts.MarineZoneForecast.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {microsoft_open_data_us_noaa_nws_forecasts_marine_zone_forecast_mqtt_data}")
