...
"""
Tests for nws_forecasts_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../nws_forecasts_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../nws_forecasts_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../nws_forecasts_amqp_producer_amqp_producer/src')))

from nws_forecasts_amqp_producer_amqp_producer import *
from nws_forecasts_amqp_producer_data import ForecastZone
from test_forecastzone import Test_ForecastZone
from nws_forecasts_amqp_producer_data import LandZoneForecast
from test_landzoneforecast import Test_LandZoneForecast
from nws_forecasts_amqp_producer_data import MarineZoneForecast
...
class TestMicrosoftOpenDataUSNOAANWSForecastsAmqpProducer:
    """Test cases for MicrosoftOpenDataUSNOAANWSForecastsAmqpProducer"""
    
    def test_producer_initialization(self, artemis_container):
        """Test that producer initializes correctly"""
        producer = MicrosoftOpenDataUSNOAANWSForecastsAmqpProducer(
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(MicrosoftOpenDataUSNOAANWSForecastsAmqpProducer)
    ...
    def test_send_amqp(self, artemis_container):
        ...
        payload = Test_ForecastZone.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "Microsoft.OpenData.US.NOAA.NWS.Forecasts.ForecastZone.amqp"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{zone_id}".format(zone_id="value")
    ...
    def test_send_amqp_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'Microsoft.OpenData.US.NOAA.NWS.Forecasts.ForecastZone.amqp'
        assert received.body is not None
        assert received.subject == "{zone_id}".format(zone_id="value")
        assert properties.get('state') == "{state}".format(state="value")
        assert properties.get('zone_type') == "{zone_type}".format(zone_type="value")
    ...
    def test_send_amqp(self, artemis_container):
        """Send and receive a Amqp message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_LandZoneForecast.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "Microsoft.OpenData.US.NOAA.NWS.Forecasts.LandZoneForecast.amqp"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_amqp_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'Microsoft.OpenData.US.NOAA.NWS.Forecasts.LandZoneForecast.amqp'
        assert received.body is not None
        assert received.subject == "{zone_id}".format(zone_id="value")
        assert properties.get('state') == "{state}".format(state="value")
        assert properties.get('zone_type') == "{zone_type}".format(zone_type="value")
    ...
    def test_send_amqp(self, artemis_container):
        """Send and receive a Amqp message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_MarineZoneForecast.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "Microsoft.OpenData.US.NOAA.NWS.Forecasts.MarineZoneForecast.amqp"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_amqp_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'Microsoft.OpenData.US.NOAA.NWS.Forecasts.MarineZoneForecast.amqp'
        assert received.body is not None
        assert received.subject == "{zone_id}".format(zone_id="value")
        assert properties.get('state') == "{state}".format(state="value")
        assert properties.get('zone_type') == "{zone_type}".format(zone_type="value")
