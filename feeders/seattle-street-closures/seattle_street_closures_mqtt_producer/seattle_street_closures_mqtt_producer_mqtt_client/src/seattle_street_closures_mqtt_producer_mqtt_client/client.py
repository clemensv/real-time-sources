...
def get_default_topic_mappings_us_wa_seattle_streetclosures_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the US.WA.Seattle.StreetClosures.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "US.WA.Seattle.StreetClosures.StreetClosure.mqtt": "traffic/us/seattle/seattle-street-closures/{neighborhood}/{closure_id}/closure",
    }


def get_subscription_topics_us_wa_seattle_streetclosures_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_us_wa_seattle_streetclosures_mqtt()
    topics = []
...
class USWASeattleStreetClosuresMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the US.WA.Seattle.StreetClosures.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_us_wa_seattle_streetclosures_mqtt()
        ...
        self.us_wa_seattle_street_closures_street_closure_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, seattle_street_closures_mqtt_producer_data.StreetClosure, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "US.WA.Seattle.StreetClosures.StreetClosure":
            if self.us_wa_seattle_street_closures_street_closure_mqtt_async:
                try:
                    ...
                    data = seattle_street_closures_mqtt_producer_data.StreetClosure.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "US.WA.Seattle.StreetClosures.StreetClosure.mqtt")
                    await self.us_wa_seattle_street_closures_street_closure_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in us_wa_seattle_street_closures_street_closure_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_us_wa_seattle_streetclosures_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_us_wa_seattle_street_closures_street_closure_mqtt(self,
        closure_id: str,
        neighborhood: str,
        data: seattle_street_closures_mqtt_producer_data.StreetClosure,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'US.WA.Seattle.StreetClosures.StreetClosure.mqtt' event to an MQTT topic.

        Args:
        
            closure_id: URI template variable for 'closure_id'
            neighborhood: URI template variable for 'neighborhood'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/us/seattle/seattle-street-closures/{neighborhood}/{closure_id}/closure'
        ...
        """
        target_topic = topic if topic is not None else "traffic/us/seattle/seattle-street-closures/{neighborhood}/{closure_id}/closure"
        ...
        attributes = {
             "type":"US.WA.Seattle.StreetClosures.StreetClosure",
             "source":"https://data.seattle.gov/Built-Environment/Street-Closures/ium9-iqtc",
             "subject":"{closure_id}".format(closure_id = closure_id)
        }
        attributes["datacontenttype"] = content_type
