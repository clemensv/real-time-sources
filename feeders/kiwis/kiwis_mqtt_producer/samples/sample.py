...
import os
import signal
from kiwis_mqtt_producer_data import *
from kiwis_mqtt_producer_mqtt_client.client import OrgKiwisStationMqttProducer, OrgKiwisStationMqttDispatcher

async def handle_org_kiwis_station_mqtt_station(mqtt_msg,cloud_event, org_kiwis_station_mqtt_station_data):
    """ Handles the org.kiwis.station.mqtt.Station message """
    print(f"Received org.kiwis.station.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_kiwis_station_mqtt_station_data}")
from kiwis_mqtt_producer_mqtt_client.client import OrgKiwisTimeseriesMqttProducer, OrgKiwisTimeseriesMqttDispatcher

async def handle_org_kiwis_timeseries_mqtt_timeseries(mqtt_msg,cloud_event, org_kiwis_timeseries_mqtt_timeseries_data):
    """ Handles the org.kiwis.timeseries.mqtt.Timeseries message """
    print(f"Received org.kiwis.timeseries.mqtt.Timeseries on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_kiwis_timeseries_mqtt_timeseries_data}")

async def handle_org_kiwis_timeseries_mqtt_timeseries_value(mqtt_msg,cloud_event, org_kiwis_timeseries_mqtt_timeseries_value_data):
    """ Handles the org.kiwis.timeseries.mqtt.TimeseriesValue message """
    print(f"Received org.kiwis.timeseries.mqtt.TimeseriesValue on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_kiwis_timeseries_mqtt_timeseries_value_data}")
