...
def get_default_topic_mappings_org_kiwis_station_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.kiwis.station.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.kiwis.station.mqtt.Station": "hydro/kiwis/{kiwis_id}/stations/{station_id}/info",
    }


def get_subscription_topics_org_kiwis_station_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_org_kiwis_station_mqtt()
    topics = []
...
class OrgKiwisStationMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.kiwis.station.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_kiwis_station_mqtt()
        ...
        self.org_kiwis_station_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, kiwis_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.kiwis.Station":
            if self.org_kiwis_station_mqtt_station_async:
                try:
                    ...
                    data = kiwis_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.kiwis.station.mqtt.Station")
                    await self.org_kiwis_station_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_kiwis_station_mqtt_station handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_kiwis_station_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_org_kiwis_station_mqtt_station(self,
        base_url: str,
        kiwis_id: str,
        station_id: str,
        data: kiwis_mqtt_producer_data.Station,
        ...
        """
        Publish the 'org.kiwis.station.mqtt.Station' event to an MQTT topic.

        Args:
        
            base_url: URI template variable for 'base_url'
            kiwis_id: URI template variable for 'kiwis_id'
            station_id: URI template variable for 'station_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/kiwis/{kiwis_id}/stations/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "hydro/kiwis/{kiwis_id}/stations/{station_id}/info"
        _topic_template_values: Dict[str, str] = {
            "base_url": str(base_url),
            "kiwis_id": str(kiwis_id),
        ...
        attributes = {
             ...
             "subject":"{kiwis_id}/{station_id}".format(kiwis_id = kiwis_id,station_id = station_id)
        }
        attributes["datacontenttype"] = content_type
...
def get_default_topic_mappings_org_kiwis_timeseries_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.kiwis.timeseries.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.kiwis.timeseries.mqtt.Timeseries": "hydro/kiwis/{kiwis_id}/timeseries/{ts_id}/metadata",
        "org.kiwis.timeseries.mqtt.TimeseriesValue": "hydro/kiwis/{kiwis_id}/timeseries/{ts_id}/value",
    }


def get_subscription_topics_org_kiwis_timeseries_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_org_kiwis_timeseries_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class OrgKiwisTimeseriesMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.kiwis.timeseries.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_kiwis_timeseries_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.org_kiwis_timeseries_mqtt_timeseries_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, kiwis_mqtt_producer_data.Timeseries, Dict[str, str]], Awaitable[None]]] = None
        
        self.org_kiwis_timeseries_mqtt_timeseries_value_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, kiwis_mqtt_producer_data.TimeseriesValue, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.kiwis.Timeseries":
            if self.org_kiwis_timeseries_mqtt_timeseries_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = kiwis_mqtt_producer_data.Timeseries.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.kiwis.timeseries.mqtt.Timeseries")
                    await self.org_kiwis_timeseries_mqtt_timeseries_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_kiwis_timeseries_mqtt_timeseries handler: {e}")
            return
        
        if event_type == "org.kiwis.TimeseriesValue":
            if self.org_kiwis_timeseries_mqtt_timeseries_value_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = kiwis_mqtt_producer_data.TimeseriesValue.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.kiwis.timeseries.mqtt.TimeseriesValue")
                    await self.org_kiwis_timeseries_mqtt_timeseries_value_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_kiwis_timeseries_mqtt_timeseries_value handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_kiwis_timeseries_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_org_kiwis_timeseries_mqtt_timeseries(self,
        base_url: str,
        kiwis_id: str,
        ts_id: str,
        data: kiwis_mqtt_producer_data.Timeseries,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.kiwis.timeseries.mqtt.Timeseries' event to an MQTT topic.

        Args:
        
            base_url: URI template variable for 'base_url'
            kiwis_id: URI template variable for 'kiwis_id'
            ts_id: URI template variable for 'ts_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/kiwis/{kiwis_id}/timeseries/{ts_id}/metadata'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "hydro/kiwis/{kiwis_id}/timeseries/{ts_id}/metadata"
        _topic_template_values: Dict[str, str] = {
            "base_url": str(base_url),
            "kiwis_id": str(kiwis_id),
            "ts_id": str(ts_id),
        ...
        attributes = {
             ...
             "source":"{base_url}".format(base_url = base_url),
             "subject":"{kiwis_id}/{ts_id}".format(kiwis_id = kiwis_id,ts_id = ts_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
    ...
    async def publish_org_kiwis_timeseries_mqtt_timeseries_value(self,
        base_url: str,
        kiwis_id: str,
        ts_id: str,
        data: kiwis_mqtt_producer_data.TimeseriesValue,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.kiwis.timeseries.mqtt.TimeseriesValue' event to an MQTT topic.
        ...
        Args:
            ...
            ts_id: URI template variable for 'ts_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/kiwis/{kiwis_id}/timeseries/{ts_id}/value'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "hydro/kiwis/{kiwis_id}/timeseries/{ts_id}/value"
        _topic_template_values: Dict[str, str] = {
            "base_url": str(base_url),
            "kiwis_id": str(kiwis_id),
            "ts_id": str(ts_id),
