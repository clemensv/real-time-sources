# Kiwis_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the kiwis_mqtt_producer message definitions.

# Kiwis_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - OrgKiwisStationMqttEventDispatcher,

    OrgKiwisTimeseriesMqttEventDispatcher
...
from kiwis_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = OrgKiwisStationMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgKiwisStationMqttEventDispatcher



# Publish message`OrgKiwisStationMqttEventDispatcher` handles events for the org.kiwis.station.mqtt message group.
...
from kiwis_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = OrgKiwisStationMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
client = OrgKiwisTimeseriesMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe OrgKiwisStationMqttEventDispatcher defines the following event handler hooks.
...
```##### `org_kiwis_station_mqtt_station_async`



## Test```python

org_kiwis_station_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `org.kiwis.station.mqtt.Station`: Reference metadata for one observation station
returned by KiWIS getStationList, including stable identifiers, public names, WGS 84 coordinates, river, and catchment
context.
...
- `data`: The event data of type `kiwis_mqtt_producer_data.Station`.

Example:

```python
async def org_kiwis_station_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
client = OrgKiwisTimeseriesMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgKiwisTimeseriesMqttEventDispatcher



# Publish message`OrgKiwisTimeseriesMqttEventDispatcher` handles events for the org.kiwis.timeseries.mqtt message group.
...
## BuildThe OrgKiwisTimeseriesMqttEventDispatcher defines the following event handler hooks.
...
```##### `org_kiwis_timeseries_mqtt_timeseries_async`



## Test```python

org_kiwis_timeseries_mqtt_timeseries_async:  Callable[[PartitionContext, EventData, CloudEvent, Timeseries],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `kiwis_mqtt_producer_data.Timeseries`.

Example:

```python
async def org_kiwis_timeseries_mqtt_timeseries_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Timeseries) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `org_kiwis_timeseries_mqtt_timeseries_value_async`



## Test```python

org_kiwis_timeseries_mqtt_timeseries_value_async:  Callable[[PartitionContext, EventData, CloudEvent, TimeseriesValue],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `kiwis_mqtt_producer_data.TimeseriesValue`.

Example:

```python
async def org_kiwis_timeseries_mqtt_timeseries_value_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: TimeseriesValue) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
