...
"""
Tests for kiwis_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../kiwis_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../kiwis_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../kiwis_amqp_producer_amqp_producer/src')))
...
class TestOrgKiwisStationAmqpProducer:
    """Test cases for OrgKiwisStationAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(OrgKiwisStationAmqpProducer)
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "org.kiwis.Station"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{kiwis_id}/{station_id}".format(kiwis_id="value", station_id="value")
                assert properties.get('kiwis_id') == "{kiwis_id}".format(kiwis_id="value")
        finally:
            producer.close()
    ...
    def test_send_station_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.kiwis.Station'
        assert received.body is not None
        assert received.subject == "{kiwis_id}/{station_id}".format(kiwis_id="value", station_id="value")
        assert properties.get('kiwis_id') == "{kiwis_id}".format(kiwis_id="value")
...
class TestOrgKiwisTimeseriesAmqpProducer:
    """Test cases for OrgKiwisTimeseriesAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        fake_connection = FakeConnection()
        producer = object.__new__(OrgKiwisTimeseriesAmqpProducer)
        producer._sender = fake_sender
        producer._connection = fake_connection
        producer._blocking_sender_is_presettled = True
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "org.kiwis.Timeseries"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{kiwis_id}/{ts_id}".format(kiwis_id="value", ts_id="value")
                assert properties.get('kiwis_id') == "{kiwis_id}".format(kiwis_id="value")
        finally:
            producer.close()
    ...
    def test_send_timeseries_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.kiwis.Timeseries'
        assert received.body is not None
        assert received.subject == "{kiwis_id}/{ts_id}".format(kiwis_id="value", ts_id="value")
        assert properties.get('kiwis_id') == "{kiwis_id}".format(kiwis_id="value")
    
    def test_send_timeseries_value(self, artemis_container):
        """Send and receive a TimeseriesValue message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_TimeseriesValue.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "org.kiwis.TimeseriesValue"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_timeseries_value_single_fresh_connection(self, artemis_container):
        """Send exactly one TimeseriesValue message on a fresh producer connection."""
        payload = Test_TimeseriesValue.create_instance()
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.kiwis.TimeseriesValue'
        assert received.body is not None
        assert received.subject == "{kiwis_id}/{ts_id}".format(kiwis_id="value", ts_id="value")
        assert properties.get('kiwis_id') == "{kiwis_id}".format(kiwis_id="value")
