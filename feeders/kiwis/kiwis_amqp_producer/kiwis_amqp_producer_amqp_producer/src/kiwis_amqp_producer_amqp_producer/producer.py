    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from kiwis_amqp_producer_data import Station
from kiwis_amqp_producer_data import Timeseries
from kiwis_amqp_producer_data import TimeseriesValue

class OrgKiwisStationAmqpProducer:
    """
    Producer class to send messages in the `org.kiwis.station.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        ...
        """
        Send the `org.kiwis.station.amqp.Station` message
        Reference metadata for one observation station returned by KiWIS getStationList, including stable identifiers, public names, WGS 84 coordinates, river, and catchment context.
        ...
        attributes = {
            ...
            "org.kiwis.Station",
            "source":
            "{base_url}".format(base_url=_base_url),
            "subject":
            "{kiwis_id}/{station_id}".format(kiwis_id=_kiwis_id, station_id=_station_id),
        }
        ...
        amqp_msg.subject = "{kiwis_id}/{station_id}".format(kiwis_id=_kiwis_id, station_id=_station_id)

        app_properties = {}
        app_properties["kiwis_id"] = "{kiwis_id}".format(kiwis_id=_kiwis_id)
        if app_properties:
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.kiwis.station.amqp.Station` messages
...
class OrgKiwisTimeseriesAmqpProducer:
    """
    Producer class to send messages in the `org.kiwis.timeseries.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_timeseries(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `org.kiwis.timeseries.amqp.Timeseries` message
        Reference metadata for one KiWIS timeseries returned by getTimeseriesList, including station linkage, parameter labels, units, and coverage range.
        ...
        attributes = {
            ...
            "{base_url}".format(base_url=_base_url),
            "subject":
            "{kiwis_id}/{ts_id}".format(kiwis_id=_kiwis_id, ts_id=_ts_id),
        }
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        ...
        if amqp_creation_time is not None:
            amqp_msg.creation_time = amqp_creation_time
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "{kiwis_id}/{ts_id}".format(kiwis_id=_kiwis_id, ts_id=_ts_id)
    ...
    def send_timeseries_batch(self,
        data_array: typing.List[Timeseries],
        _base_url: str,
        _kiwis_id: str,
        _ts_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.kiwis.timeseries.amqp.Timeseries` messages
        
        Args:
            data_array (typing.List[Timeseries]): Array of message data objects
            _base_url (str): Value for placeholder base_url in attribute source
            _kiwis_id (str): Value for placeholder kiwis_id in attribute subject
            _ts_id (str): Value for placeholder ts_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    def send_timeseries_value(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `org.kiwis.timeseries.amqp.TimeseriesValue` message
        One observation row returned by KiWIS getTimeseriesValues for a configured timeseries, carrying timestamp, numeric value, optional quality code, and the per-timeseries unit metadata needed by consumers.
    ...
    def send_timeseries_value_batch(self,
        data_array: typing.List[TimeseriesValue],
        _base_url: str,
        _kiwis_id: str,
        _ts_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.kiwis.timeseries.amqp.TimeseriesValue` messages
        
        Args:
            data_array (typing.List[TimeseriesValue]): Array of message data objects
            _base_url (str): Value for placeholder base_url in attribute source
            _kiwis_id (str): Value for placeholder kiwis_id in attribute subject
            _ts_id (str): Value for placeholder ts_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
