...
This document covers the published OCI container images for the USGS Instantaneous Values feeder, their environment-variable contract, authentication modes, and one-click Azure deployments. For the project overview see [README.md](README.md); for the CloudEvents contract see [EVENTS.md](EVENTS.md).
...
The USGS Instantaneous Values source is exposed as a polling API upstream. These container images package polling cadence control, stateful dedupe, CloudEvents production, and transport/auth wiring so operators can deploy a ready-to-run feeder instead of building source-specific integration code.

## What ships in the box

| Image | Transport | Default behavior |
|---|---|---|
| `ghcr.io/clemensv/real-time-sources-usgs-iv` | Apache Kafka 2.x | JSON CloudEvents on one topic; key template `{agency_cd}/{site_no}/{parameter_cd}/{timeseries_cd}` |
| `ghcr.io/clemensv/real-time-sources-usgs-iv-mqtt` | MQTT 5.0 | Unified-Namespace topic template `(see EVENTS.md)` |
| `ghcr.io/clemensv/real-time-sources-usgs-iv-amqp` | AMQP 1.0 | Binary CloudEvents to AMQP node `usgs-iv` |
...
| Default entry point | Kafka `["python", "-m", "usgs_iv", "feed"]`; MQTT `["python", "-m", "usgs_iv_mqtt", "feed"]`; AMQP `["python", "-m", "usgs_iv_amqp", "feed"]` |
| Exposed ports | none — outbound publisher only |
| Signals | process exits cleanly on `SIGTERM` |
| Persistent state | `USGS_LAST_POLLED_FILE`; mount a host volume for restart-safe dedupe/checkpoint behavior |
...
docker pull ghcr.io/clemensv/real-time-sources-usgs-iv-mqtt:latest
...
| Variable | Description |
|---|---|
| `USGS_FORCE_DATA_REFRESH` | Set to a truthy value to force a full re-fetch of instantaneous values, ignoring dedupe state. |
| `USGS_FORCE_SITE_REFRESH` | Set to a truthy value to force an immediate refresh of the site reference catalog at startup. |
| `USGS_STATE` | Two-letter US state code to scope the sites polled (for example `NY`); empty uses the configured site list. |
| `USGS_IV_MOCK` | Set to a truthy value to emit deterministic offline sample data instead of calling the live USGS Instantaneous Values service (offline testing / Docker E2E). |

### Kafka image

| Variable | Description |
|---|---|
| `USGS_LAST_POLLED_FILE` | Path to dedupe/checkpoint state file. |
...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fusgs-iv%2Fazure-template-amqp.json)

### Kafka — provision a new Event Hub

[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fusgs-iv%2Fazure-template-with-eventhub.json)

### Kafka — bring your own Event Hub / Kafka

[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fclemensv%2Freal-time-sources%2Fmain%2Ffeeders%2Fusgs-iv%2Fazure-template.json)
