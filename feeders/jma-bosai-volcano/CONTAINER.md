...
|---|---|---|
| `ghcr.io/clemensv/real-time-sources-jma-bosai-volcano` | Kafka | JSON CloudEvents to one Kafka topic |
| `ghcr.io/clemensv/real-time-sources-jma-bosai-volcano-mqtt` | MQTT 5.0 | Binary CloudEvents into UNS topic tree from xRegistry |
| `ghcr.io/clemensv/real-time-sources-jma-bosai-volcano-amqp` | AMQP 1.0 | Binary CloudEvents to one AMQP address |
...
| Default entry point | Kafka: `python -m jma_bosai_volcano`; MQTT: `python -m jma_bosai_volcano_mqtt feed`; AMQP: `python -m jma_bosai_volcano_amqp feed` |
...
docker pull ghcr.io/clemensv/real-time-sources-jma-bosai-volcano-mqtt:latest
docker pull ghcr.io/clemensv/real-time-sources-jma-bosai-volcano-amqp:latest
```
...
docker run --rm \
  ...
  -e AMQP_ADDRESS="jma-bosai-volcano" \
...
|---|---|
| `VOLCANO_METADATA_REFRESH_HOURS` | Hours between refreshes of the volcano metadata catalog (reference data) (default `720`). |
...
| `JMA_BOSAI_VOLCANO_MOCK` | Mock/source test mode switch used for local validation. |
...
Deploys the Kafka image against an existing Azure Event Hubs or Fabric Event Streams custom-endpoint connection string.
