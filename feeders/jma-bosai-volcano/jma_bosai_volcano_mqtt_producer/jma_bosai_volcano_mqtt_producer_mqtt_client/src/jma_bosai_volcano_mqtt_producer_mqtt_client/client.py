...
def get_default_topic_mappings_jp_jma_volcano_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the JP.JMA.Volcano.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "JP.JMA.Volcano.mqtt.Volcano": "weather/jp/jma/jma-bosai-volcano/{prefecture}/{volcano_code}/{event}",
        "JP.JMA.Volcano.mqtt.VolcanicWarning": "weather/jp/jma/jma-bosai-volcano/{prefecture}/{volcano_code}/{event}",
        "JP.JMA.Volcano.mqtt.VolcanicEruption": "weather/jp/jma/jma-bosai-volcano/{prefecture}/{volcano_code}/{event}",
    }


def get_subscription_topics_jp_jma_volcano_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_jp_jma_volcano_mqtt()
    topics = []
...
class JPJMAVolcanoMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the JP.JMA.Volcano.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_jp_jma_volcano_mqtt()
        ...
        self.jp_jma_volcano_mqtt_volcano_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, jma_bosai_volcano_mqtt_producer_data.Volcano, Dict[str, str]], Awaitable[None]]] = None
        
        self.jp_jma_volcano_mqtt_volcanic_warning_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, jma_bosai_volcano_mqtt_producer_data.VolcanicWarning, Dict[str, str]], Awaitable[None]]] = None
        
        self.jp_jma_volcano_mqtt_volcanic_eruption_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, jma_bosai_volcano_mqtt_producer_data.VolcanicEruption, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "JP.JMA.Volcano.Volcano":
            if self.jp_jma_volcano_mqtt_volcano_async:
                try:
                    ...
                    data = jma_bosai_volcano_mqtt_producer_data.Volcano.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "JP.JMA.Volcano.mqtt.Volcano")
                    await self.jp_jma_volcano_mqtt_volcano_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_jma_volcano_mqtt_volcano handler: {e}")
            return
        
        if event_type == "JP.JMA.Volcano.VolcanicWarning":
            if self.jp_jma_volcano_mqtt_volcanic_warning_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = jma_bosai_volcano_mqtt_producer_data.VolcanicWarning.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "JP.JMA.Volcano.mqtt.VolcanicWarning")
                    await self.jp_jma_volcano_mqtt_volcanic_warning_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_jma_volcano_mqtt_volcanic_warning handler: {e}")
            return
        
        if event_type == "JP.JMA.Volcano.VolcanicEruption":
            if self.jp_jma_volcano_mqtt_volcanic_eruption_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = jma_bosai_volcano_mqtt_producer_data.VolcanicEruption.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "JP.JMA.Volcano.mqtt.VolcanicEruption")
                    await self.jp_jma_volcano_mqtt_volcanic_eruption_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_jma_volcano_mqtt_volcanic_eruption handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_jp_jma_volcano_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_jp_jma_volcano_mqtt_volcano(self,
        feedurl: str,
        volcano_code: str,
        prefecture: str,
        event: str,
        data: jma_bosai_volcano_mqtt_producer_data.Volcano,
        ...
        """
        Publish the 'JP.JMA.Volcano.mqtt.Volcano' event to an MQTT topic.

        Args:
        
            feedurl: URI template variable for 'feedurl'
            volcano_code: URI template variable for 'volcano_code'
            prefecture: URI template variable for 'prefecture'
            event: URI template variable for 'event'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/jp/jma/jma-bosai-volcano/{prefecture}/{volcano_code}/{event}'
        ...
        """
        target_topic = topic if topic is not None else "weather/jp/jma/jma-bosai-volcano/{prefecture}/{volcano_code}/{event}"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
            "volcano_code": str(volcano_code),
        ...
        attributes = {
             "type":"JP.JMA.Volcano.Volcano",
             "source":"{feedurl}".format(feedurl = feedurl),
             "subject":"jp.jma.volcano/{volcano_code}".format(volcano_code = volcano_code)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_jp_jma_volcano_mqtt_volcanic_warning(self,
        feedurl: str,
        volcano_code: str,
        prefecture: str,
        event: str,
        data: jma_bosai_volcano_mqtt_producer_data.VolcanicWarning,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'JP.JMA.Volcano.mqtt.VolcanicWarning' event to an MQTT topic.
        ...
        attributes = {
             "type":"JP.JMA.Volcano.VolcanicWarning",
             "source":"{feedurl}".format(feedurl = feedurl),
             "subject":"jp.jma.volcano/{volcano_code}".format(volcano_code = volcano_code)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_jp_jma_volcano_mqtt_volcanic_eruption(self,
        feedurl: str,
        volcano_code: str,
        prefecture: str,
        event: str,
        data: jma_bosai_volcano_mqtt_producer_data.VolcanicEruption,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'JP.JMA.Volcano.mqtt.VolcanicEruption' event to an MQTT topic.
        ...
        attributes = {
             "type":"JP.JMA.Volcano.VolcanicEruption",
             "source":"{feedurl}".format(feedurl = feedurl),
             "subject":"jp.jma.volcano/{volcano_code}".format(volcano_code = volcano_code)
        }
        attributes["datacontenttype"] = content_type
