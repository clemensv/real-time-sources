...
# Jma_bosai_volcano_producer Kafka Producer# Jma_bosai_volcano_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - JPJMAVolcanoEventDispatcher
...
- JPJMAVolcanoProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### JPJMAVolcanoEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`JPJMAVolcanoEventDispatcher` handles events for the JP.JMA.Volcano message group.
...
from jma_bosai_volcano_producer import JPJMAVolcanoProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = JPJMAVolcanoProducer(
...
await producer.send_jp_jma_volcano_volcano(```python

    data=Volcano(...),add_consumer(self, consumer: KafkaConsumer)
...
The JPJMAVolcanoEventDispatcher defines the following event handler hooks.
...
producer = JPJMAVolcanoProducer(
    ...
    security_protocol='SASL_SSL',##### `jp_jma_volcano_volcano_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'jp_jma_volcano_volcano_async:  Callable[[ConsumerRecord, CloudEvent, Volcano],
Awaitable[None]]
...
Asynchronous handler hook for `JP.JMA.Volcano.Volcano`: JMA Bosai volcano catalog reference data emitted at startup and
on monthly refresh.
...
### JPJMAVolcanoProducer- `data`: The event data of type `jma_bosai_volcano_producer_data.Volcano`.



Producer for `JP.JMA.Volcano` message group.Example:



#### Constructor```python

async def jp_jma_volcano_volcano_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Volcano) -> None:

```python    # Process the event data

JPJMAVolcanoProducer(    await some_processing_function(record, cloud_event, data)
...
jp_jma_volcano_dispatcher.jp_jma_volcano_volcano_async = jp_jma_volcano_volcano_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `jp_jma_volcano_volcanic_warning_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'jp_jma_volcano_volcanic_warning_async:  Callable[[ConsumerRecord, CloudEvent,
VolcanicWarning], Awaitable[None]]

)```

```

Asynchronous handler hook for `JP.JMA.Volcano.VolcanicWarning`: JMA Bosai active volcanic warning or forecast for a
target volcano.
...
### JPJMAVolcanoProducer- `data`: The event data of type `jma_bosai_volcano_producer_data.VolcanicWarning`.
...
async def jp_jma_volcano_volcanic_warning_event(record: ConsumerRecord, cloud_event: CloudEvent, data: VolcanicWarning)
-> None:
...
jp_jma_volcano_dispatcher.jp_jma_volcano_volcanic_warning_async = jp_jma_volcano_volcanic_warning_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `jp_jma_volcano_volcanic_eruption_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'jp_jma_volcano_volcanic_eruption_async:  Callable[[ConsumerRecord, CloudEvent,
VolcanicEruption], Awaitable[None]]

)```

```

Asynchronous handler hook for `JP.JMA.Volcano.VolcanicEruption`: JMA Bosai volcanic eruption observation report for a
target volcano.
...
### JPJMAVolcanoProducer- `data`: The event data of type `jma_bosai_volcano_producer_data.VolcanicEruption`.
...
async def jp_jma_volcano_volcanic_eruption_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
VolcanicEruption) -> None:
...
jp_jma_volcano_dispatcher.jp_jma_volcano_volcanic_eruption_async = jp_jma_volcano_volcanic_eruption_event
...
##### `send_jp_jma_volcano_volcano`Dispatchers have the following protected methods:



```python### Methods:

async def send_jp_jma_volcano_volcano(
...
Send a single `JP.JMA.Volcano.Volcano` message. JMA Bosai volcano catalog reference data emitted at startup and on
monthly refresh.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `Volcano`
...
await producer.send_jp_jma_volcano_volcano(
...
Send multiple `JP.JMA.Volcano.Volcano` messages in a batch.
...
await producer.send_jp_jma_volcano_volcano_batch(```

    messages=[

        Volcano(...),Initializes the runner with a Kafka consumer.
...
##### `send_jp_jma_volcano_volcanic_warning`Dispatchers have the following protected methods:



```python### Methods:

async def send_jp_jma_volcano_volcanic_warning(
...
Send a single `JP.JMA.Volcano.VolcanicWarning` message. JMA Bosai active volcanic warning or forecast for a target
volcano.Args:
...
- `data`: Message data of type `VolcanicWarning`
...
await producer.send_jp_jma_volcano_volcanic_warning(

    data=VolcanicWarning(...),Args:
...
Send multiple `JP.JMA.Volcano.VolcanicWarning` messages in a batch.
...
await producer.send_jp_jma_volcano_volcanic_warning_batch(```

    messages=[

        VolcanicWarning(...),Initializes the runner with a Kafka consumer.

        VolcanicWarning(...),

        VolcanicWarning(...)Args:
...
##### `send_jp_jma_volcano_volcanic_eruption`Dispatchers have the following protected methods:



```python### Methods:

async def send_jp_jma_volcano_volcanic_eruption(
...
Send a single `JP.JMA.Volcano.VolcanicEruption` message. JMA Bosai volcanic eruption observation report for a target
volcano.Args:
...
- `data`: Message data of type `VolcanicEruption`
...
await producer.send_jp_jma_volcano_volcanic_eruption(

    data=VolcanicEruption(...),Args:
...
Send multiple `JP.JMA.Volcano.VolcanicEruption` messages in a batch.
...
await producer.send_jp_jma_volcano_volcanic_eruption_batch(```

    messages=[

        VolcanicEruption(...),Initializes the runner with a Kafka consumer.

        VolcanicEruption(...),

        VolcanicEruption(...)Args:
