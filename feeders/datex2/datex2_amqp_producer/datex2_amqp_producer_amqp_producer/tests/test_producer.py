...
"""
Tests for datex2_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../datex2_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../datex2_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../datex2_amqp_producer_amqp_producer/src')))
...
class TestOrgDatex2SituationAmqpProducer:
    """Test cases for OrgDatex2SituationAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(OrgDatex2SituationAmqpProducer)
    ...
    def test_send_amqp(self, artemis_container):
        ...
        payload = Test_SituationRecord.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "org.datex2.situation.SituationRecord"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{supplier_id}/{situation_record_id}".format(supplier_id="value", situation_record_id="value")
                assert annotations.get(symbol('x-opt-partition-key')) == str("{supplier_id}/{situation_record_id}".format(supplier_id="value", situation_record_id="value"))[:128]
        finally:
            producer.close()
    ...
    def test_send_amqp_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.datex2.situation.SituationRecord'
        assert received.body is not None
        assert received.subject == "{supplier_id}/{situation_record_id}".format(supplier_id="value", situation_record_id="value")
        assert annotations.get(symbol('x-opt-partition-key')) == str("{supplier_id}/{situation_record_id}".format(supplier_id="value", situation_record_id="value"))[:128]
...
class TestOrgDatex2MeasuredSiteAmqpProducer:
    """Test cases for OrgDatex2MeasuredSiteAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        fake_connection = FakeConnection()
        producer = object.__new__(OrgDatex2MeasuredSiteAmqpProducer)
        producer._sender = fake_sender
        producer._connection = fake_connection
        producer._blocking_sender_is_presettled = True
    ...
    def test_send_amqp(self, artemis_container):
        """Send and receive a Amqp message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_MeasurementSite.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "org.datex2.measured.MeasurementSite"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{supplier_id}/{measurement_site_id}".format(supplier_id="value", measurement_site_id="value")
                assert annotations.get(symbol('x-opt-partition-key')) == str("{supplier_id}/{measurement_site_id}".format(supplier_id="value", measurement_site_id="value"))[:128]
        finally:
            producer.close()
    ...
    def test_send_amqp_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.datex2.measured.MeasurementSite'
        assert received.body is not None
        assert received.subject == "{supplier_id}/{measurement_site_id}".format(supplier_id="value", measurement_site_id="value")
        assert annotations.get(symbol('x-opt-partition-key')) == str("{supplier_id}/{measurement_site_id}".format(supplier_id="value", measurement_site_id="value"))[:128]
...
class TestOrgDatex2TrafficMeasurementAmqpProducer:
    """Test cases for OrgDatex2TrafficMeasurementAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        fake_connection = FakeConnection()
        producer = object.__new__(OrgDatex2TrafficMeasurementAmqpProducer)
        producer._sender = fake_sender
        producer._connection = fake_connection
        producer._blocking_sender_is_presettled = True
    ...
    def test_send_amqp(self, artemis_container):
        """Send and receive a Amqp message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_TrafficMeasurement.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "org.datex2.measured.TrafficMeasurement"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_amqp_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.datex2.measured.TrafficMeasurement'
        assert received.body is not None
        assert received.subject == "{supplier_id}/{measurement_site_id}".format(supplier_id="value", measurement_site_id="value")
        assert annotations.get(symbol('x-opt-partition-key')) == str("{supplier_id}/{measurement_site_id}".format(supplier_id="value", measurement_site_id="value"))[:128]
