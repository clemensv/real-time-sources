...
import os
import signal
from datex2_mqtt_producer_data import *
from datex2_mqtt_producer_mqtt_client.client import OrgDatex2MeasuredMqttProducer, OrgDatex2MeasuredMqttDispatcher

async def handle_org_datex2_measured_measurement_site_mqtt(mqtt_msg,cloud_event, org_datex2_measured_measurement_site_mqtt_data):
    """ Handles the org.datex2.measured.MeasurementSite.mqtt message """
    print(f"Received org.datex2.measured.MeasurementSite.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_datex2_measured_measurement_site_mqtt_data}")

async def handle_org_datex2_measured_traffic_measurement_mqtt(mqtt_msg,cloud_event, org_datex2_measured_traffic_measurement_mqtt_data):
    """ Handles the org.datex2.measured.TrafficMeasurement.mqtt message """
    print(f"Received org.datex2.measured.TrafficMeasurement.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_datex2_measured_traffic_measurement_mqtt_data}")
from datex2_mqtt_producer_mqtt_client.client import OrgDatex2SituationMqttProducer, OrgDatex2SituationMqttDispatcher

async def handle_org_datex2_situation_situation_record_mqtt(mqtt_msg,cloud_event, org_datex2_situation_situation_record_mqtt_data):
    """ Handles the org.datex2.situation.SituationRecord.mqtt message """
    print(f"Received org.datex2.situation.SituationRecord.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_datex2_situation_situation_record_mqtt_data}")
