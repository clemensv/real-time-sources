# Datex2_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the datex2_mqtt_producer message definitions.

# Datex2_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - OrgDatex2MeasuredMqttEventDispatcher,

    OrgDatex2SituationMqttEventDispatcher
...
from datex2_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = OrgDatex2MeasuredMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgDatex2MeasuredMqttEventDispatcher



# Publish message`OrgDatex2MeasuredMqttEventDispatcher` handles events for the org.datex2.measured.mqtt message group.
...
from datex2_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = OrgDatex2MeasuredMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
client = OrgDatex2SituationMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe OrgDatex2MeasuredMqttEventDispatcher defines the following event handler hooks.
...
```##### `org_datex2_measured_measurement_site_mqtt_async`



## Test```python

org_datex2_measured_measurement_site_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, MeasurementSite],
Awaitable[None]]
...
```Asynchronous handler hook for `org.datex2.measured.MeasurementSite.mqtt`: Reference measurement-site record from a
DATEX II MeasurementSiteTablePublication.
...
- `data`: The event data of type `datex2_mqtt_producer_data.MeasurementSite`.

Example:

```python
async def org_datex2_measured_measurement_site_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: MeasurementSite) -> None:
...
```##### `org_datex2_measured_traffic_measurement_mqtt_async`



## Test```python

org_datex2_measured_traffic_measurement_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```Asynchronous handler hook for `org.datex2.measured.TrafficMeasurement.mqtt`: Traffic speed, flow, occupancy, or
travel-time observation from a DATEX II MeasuredDataPublication.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `datex2_mqtt_producer_data.TrafficMeasurement`.

Example:

```python
async def org_datex2_measured_traffic_measurement_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: TrafficMeasurement) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = OrgDatex2SituationMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgDatex2SituationMqttEventDispatcher



# Publish message`OrgDatex2SituationMqttEventDispatcher` handles events for the org.datex2.situation.mqtt message group.
...
## BuildThe OrgDatex2SituationMqttEventDispatcher defines the following event handler hooks.
...
```##### `org_datex2_situation_situation_record_mqtt_async`



## Test```python

org_datex2_situation_situation_record_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, SituationRecord],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `datex2_mqtt_producer_data.SituationRecord`.

Example:

```python
async def org_datex2_situation_situation_record_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: SituationRecord) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
