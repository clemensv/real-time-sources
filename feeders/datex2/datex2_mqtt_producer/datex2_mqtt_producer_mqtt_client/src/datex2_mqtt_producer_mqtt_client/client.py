...
def get_default_topic_mappings_org_datex2_measured_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.datex2.measured.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.datex2.measured.MeasurementSite.mqtt": "traffic/{country_code}/{operator_id}/datex2/measured/{supplier_id}/{measurement_site_id}/site",
        "org.datex2.measured.TrafficMeasurement.mqtt": "traffic/{country_code}/{operator_id}/datex2/measured/{supplier_id}/{measurement_site_id}/measurement",
    }


def get_subscription_topics_org_datex2_measured_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_org_datex2_measured_mqtt()
    topics = []
...
class OrgDatex2MeasuredMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.datex2.measured.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_datex2_measured_mqtt()
        ...
        self.org_datex2_measured_measurement_site_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, datex2_mqtt_producer_data.MeasurementSite, Dict[str, str]], Awaitable[None]]] = None
        
        self.org_datex2_measured_traffic_measurement_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, datex2_mqtt_producer_data.TrafficMeasurement, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.datex2.measured.MeasurementSite":
            if self.org_datex2_measured_measurement_site_mqtt_async:
                try:
                    ...
                    data = datex2_mqtt_producer_data.MeasurementSite.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.datex2.measured.MeasurementSite.mqtt")
                    await self.org_datex2_measured_measurement_site_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_datex2_measured_measurement_site_mqtt handler: {e}")
            return
        
        if event_type == "org.datex2.measured.TrafficMeasurement":
            if self.org_datex2_measured_traffic_measurement_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = datex2_mqtt_producer_data.TrafficMeasurement.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.datex2.measured.TrafficMeasurement.mqtt")
                    await self.org_datex2_measured_traffic_measurement_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_datex2_measured_traffic_measurement_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_datex2_measured_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_org_datex2_measured_measurement_site_mqtt(self,
        feed_url: str,
        supplier_id: str,
        measurement_site_id: str,
        country_code: str,
        operator_id: str,
        data: datex2_mqtt_producer_data.MeasurementSite,
        ...
        """
        Publish the 'org.datex2.measured.MeasurementSite.mqtt' event to an MQTT topic.

        Args:
        
            feed_url: URI template variable for 'feed_url'
            supplier_id: URI template variable for 'supplier_id'
            measurement_site_id: URI template variable for 'measurement_site_id'
            country_code: URI template variable for 'country_code'
            operator_id: URI template variable for 'operator_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/{country_code}/{operator_id}/datex2/measured/{supplier_id}/{measurement_site_id}/site'
        ...
        """
        target_topic = topic if topic is not None else "traffic/{country_code}/{operator_id}/datex2/measured/{supplier_id}/{measurement_site_id}/site"
        ...
        attributes = {
             ...
             "subject":"{supplier_id}/{measurement_site_id}".format(supplier_id = supplier_id,measurement_site_id = measurement_site_id)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_org_datex2_measured_traffic_measurement_mqtt(self,
        feed_url: str,
        supplier_id: str,
        measurement_site_id: str,
        country_code: str,
        operator_id: str,
        data: datex2_mqtt_producer_data.TrafficMeasurement,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.datex2.measured.TrafficMeasurement.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            operator_id: URI template variable for 'operator_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/{country_code}/{operator_id}/datex2/measured/{supplier_id}/{measurement_site_id}/measurement'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "traffic/{country_code}/{operator_id}/datex2/measured/{supplier_id}/{measurement_site_id}/measurement"
        _topic_template_values: Dict[str, str] = {
            "feed_url": str(feed_url),
            "supplier_id": str(supplier_id),
            "measurement_site_id": str(measurement_site_id),
...
def get_default_topic_mappings_org_datex2_situation_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.datex2.situation.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.datex2.situation.SituationRecord.mqtt": "traffic/{country_code}/{operator_id}/datex2/situations/{supplier_id}/{situation_record_id}/record",
    }


def get_subscription_topics_org_datex2_situation_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_org_datex2_situation_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class OrgDatex2SituationMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.datex2.situation.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_datex2_situation_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.org_datex2_situation_situation_record_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, datex2_mqtt_producer_data.SituationRecord, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.datex2.situation.SituationRecord":
            if self.org_datex2_situation_situation_record_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = datex2_mqtt_producer_data.SituationRecord.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.datex2.situation.SituationRecord.mqtt")
                    await self.org_datex2_situation_situation_record_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_datex2_situation_situation_record_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_datex2_situation_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_org_datex2_situation_situation_record_mqtt(self,
        feed_url: str,
        supplier_id: str,
        situation_record_id: str,
        country_code: str,
        operator_id: str,
        data: datex2_mqtt_producer_data.SituationRecord,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.datex2.situation.SituationRecord.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            operator_id: URI template variable for 'operator_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/{country_code}/{operator_id}/datex2/situations/{supplier_id}/{situation_record_id}/record'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "traffic/{country_code}/{operator_id}/datex2/situations/{supplier_id}/{situation_record_id}/record"
        _topic_template_values: Dict[str, str] = {
            "feed_url": str(feed_url),
            "supplier_id": str(supplier_id),
        ...
        attributes = {
             ...
             "source":"{feed_url}".format(feed_url = feed_url),
             "subject":"{supplier_id}/{situation_record_id}".format(supplier_id = supplier_id,situation_record_id = situation_record_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
