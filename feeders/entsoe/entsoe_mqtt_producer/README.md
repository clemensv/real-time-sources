# Entsoe_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the entsoe_mqtt_producer message definitions.

# Entsoe_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - EuEntsoeTransparencyByDomainMqttEventDispatcher,

    EuEntsoeTransparencyByDomainPsrTypeMqttEventDispatcher,

    EuEntsoeTransparencyCrossBorderMqttEventDispatcher
...
from entsoe_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = EuEntsoeTransparencyByDomainMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### EuEntsoeTransparencyByDomainMqttEventDispatcher



# Publish message`EuEntsoeTransparencyByDomainMqttEventDispatcher` handles events for the
eu.entsoe.transparency.ByDomain.mqtt message group.
...
from entsoe_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = EuEntsoeTransparencyByDomainMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name
of the Event Hub.Args:- `consumer_group_name`: The name of the consumer group.- `eventhubs_fully_qualified_namespace`:
The fully qualified namespace of the Event Hub.

client = EuEntsoeTransparencyByDomainPsrTypeMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The
name of the Event Hub.Args:- `consumer_group_name`: The name of the consumer group.-
`eventhubs_fully_qualified_namespace`: The fully qualified namespace of the Event Hub.

client = EuEntsoeTransparencyCrossBorderMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The
name of the Event Hub.- `blob_account_url`: The URL of the Azure Storage account.
...
## BuildThe EuEntsoeTransparencyByDomainMqttEventDispatcher defines the following event handler hooks.
...
```##### `eu_entsoe_transparency_by_domain_mqtt_day_ahead_prices_async`



## Test```python

eu_entsoe_transparency_by_domain_mqtt_day_ahead_prices_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```Asynchronous handler hook for `eu.entsoe.transparency.ByDomain.mqtt.DayAheadPrices`: MQTT/UNS binary CloudEvents
variant for DayAheadPrices. ENTSO-E poll cycles emit individual market time-series points; retain is false so a retained
slot never misrepresents an old forecast or historical MTU point as current state.
...
- `data`: The event data of type `entsoe_mqtt_producer_data.DayAheadPrices`.

Example:

```python
async def eu_entsoe_transparency_by_domain_mqtt_day_ahead_prices_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: DayAheadPrices) -> None:
...
```##### `eu_entsoe_transparency_by_domain_mqtt_actual_total_load_async`



## Test```python

eu_entsoe_transparency_by_domain_mqtt_actual_total_load_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `entsoe_mqtt_producer_data.ActualTotalLoad`.

Example:

```python
async def eu_entsoe_transparency_by_domain_mqtt_actual_total_load_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: ActualTotalLoad) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `eu_entsoe_transparency_by_domain_mqtt_load_forecast_margin_async`



## Test```python

eu_entsoe_transparency_by_domain_mqtt_load_forecast_margin_async:  Callable[[PartitionContext, EventData, CloudEvent,
LoadForecastMargin], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `entsoe_mqtt_producer_data.LoadForecastMargin`.

Example:

```python
async def eu_entsoe_transparency_by_domain_mqtt_load_forecast_margin_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: LoadForecastMargin) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `eu_entsoe_transparency_by_domain_mqtt_generation_forecast_async`



## Test```python

eu_entsoe_transparency_by_domain_mqtt_generation_forecast_async:  Callable[[PartitionContext, EventData, CloudEvent,
GenerationForecast], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `entsoe_mqtt_producer_data.GenerationForecast`.

Example:

```python
async def eu_entsoe_transparency_by_domain_mqtt_generation_forecast_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: GenerationForecast) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `eu_entsoe_transparency_by_domain_mqtt_reservoir_filling_information_async`



## Test```python

eu_entsoe_transparency_by_domain_mqtt_reservoir_filling_information_async:  Callable[[PartitionContext, EventData,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `entsoe_mqtt_producer_data.ReservoirFillingInformation`.

Example:

```python
async def eu_entsoe_transparency_by_domain_mqtt_reservoir_filling_information_event(partition_context: PartitionContext,
event: EventData, cloud_event: CloudEvent, data: ReservoirFillingInformation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `eu_entsoe_transparency_by_domain_mqtt_actual_generation_async`



## Test```python

eu_entsoe_transparency_by_domain_mqtt_actual_generation_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `entsoe_mqtt_producer_data.ActualGeneration`.

Example:

```python
async def eu_entsoe_transparency_by_domain_mqtt_actual_generation_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: ActualGeneration) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = EuEntsoeTransparencyByDomainPsrTypeMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### EuEntsoeTransparencyByDomainPsrTypeMqttEventDispatcher



# Publish message`EuEntsoeTransparencyByDomainPsrTypeMqttEventDispatcher` handles events for the
...
## BuildThe EuEntsoeTransparencyByDomainPsrTypeMqttEventDispatcher defines the following event handler hooks.
...
```##### `eu_entsoe_transparency_by_domain_psr_type_mqtt_actual_generation_per_type_async`



## Test```python

eu_entsoe_transparency_by_domain_psr_type_mqtt_actual_generation_per_type_async:  Callable[[PartitionContext, EventData,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `entsoe_mqtt_producer_data.ActualGenerationPerType`.

Example:

```python
async def eu_entsoe_transparency_by_domain_psr_type_mqtt_actual_generation_per_type_event(partition_context:
PartitionContext, event: EventData, cloud_event: CloudEvent, data: ActualGenerationPerType) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `eu_entsoe_transparency_by_domain_psr_type_mqtt_wind_solar_forecast_async`



## Test```python

eu_entsoe_transparency_by_domain_psr_type_mqtt_wind_solar_forecast_async:  Callable[[PartitionContext, EventData,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `entsoe_mqtt_producer_data.WindSolarForecast`.

Example:

```python
async def eu_entsoe_transparency_by_domain_psr_type_mqtt_wind_solar_forecast_event(partition_context: PartitionContext,
event: EventData, cloud_event: CloudEvent, data: WindSolarForecast) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `eu_entsoe_transparency_by_domain_psr_type_mqtt_wind_solar_generation_async`



## Test```python

eu_entsoe_transparency_by_domain_psr_type_mqtt_wind_solar_generation_async:  Callable[[PartitionContext, EventData,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `entsoe_mqtt_producer_data.WindSolarGeneration`.

Example:

```python
async def eu_entsoe_transparency_by_domain_psr_type_mqtt_wind_solar_generation_event(partition_context:
PartitionContext, event: EventData, cloud_event: CloudEvent, data: WindSolarGeneration) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
eu_entsoe_transparency_by_domain_psr_type_mqtt_installed_generation_capacity_per_type_async:
Callable[[PartitionContext, EventData, CloudEvent, InstalledGenerationCapacityPerType], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `eu.entsoe.transparency.ByDomainPsrType.mqtt.InstalledGenerationCapacityPerType`:
MQTT/UNS binary CloudEvents variant for InstalledGenerationCapacityPerType. ENTSO-E poll cycles emit individual market
time-series points; retain is false so a retained slot never misrepresents an old forecast or historical MTU point as
current state.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `entsoe_mqtt_producer_data.InstalledGenerationCapacityPerType`.

Example:

```python
async def eu_entsoe_transparency_by_domain_psr_type_mqtt_installed_generation_capacity_per_type_event(partition_context:
PartitionContext, event: EventData, cloud_event: CloudEvent, data: InstalledGenerationCapacityPerType) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = EuEntsoeTransparencyCrossBorderMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### EuEntsoeTransparencyCrossBorderMqttEventDispatcher



# Publish message`EuEntsoeTransparencyCrossBorderMqttEventDispatcher` handles events for the
eu.entsoe.transparency.CrossBorder.mqtt message group.
...
## BuildThe EuEntsoeTransparencyCrossBorderMqttEventDispatcher defines the following event handler hooks.
...
```##### `eu_entsoe_transparency_cross_border_mqtt_cross_border_physical_flows_async`



## Test```python

eu_entsoe_transparency_cross_border_mqtt_cross_border_physical_flows_async:  Callable[[PartitionContext, EventData,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `entsoe_mqtt_producer_data.CrossBorderPhysicalFlows`.

Example:

```python
async def eu_entsoe_transparency_cross_border_mqtt_cross_border_physical_flows_event(partition_context:
PartitionContext, event: EventData, cloud_event: CloudEvent, data: CrossBorderPhysicalFlows) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
