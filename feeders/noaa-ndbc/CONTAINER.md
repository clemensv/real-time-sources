...
The NOAA NDBC source is exposed as a polling API upstream. These container images package polling cadence control, stateful dedupe, CloudEvents production, and transport/auth wiring so operators can deploy a ready-to-run feeder instead of building source-specific integration code.

## What ships in the box

| Image | Transport | Default behavior |
|---|---|---|
| `ghcr.io/clemensv/real-time-sources-noaa-ndbc` | Apache Kafka 2.x | JSON CloudEvents on one topic; key template `{station_id}` |
| `ghcr.io/clemensv/real-time-sources-noaa-ndbc-mqtt` | MQTT 5.0 | Unified-Namespace topic template `(see EVENTS.md)` |
| `ghcr.io/clemensv/real-time-sources-noaa-ndbc-amqp` | AMQP 1.0 | Binary CloudEvents to AMQP node `noaa-ndbc` |
...
| Default entry point | Kafka `["python", "-m", "noaa_ndbc"]`; MQTT `["python", "-m", "noaa_ndbc_mqtt", "feed"]`; AMQP `["python", "-m", "noaa_ndbc_amqp", "feed"]` |
| Exposed ports | none — outbound publisher only |
| Signals | process exits cleanly on `SIGTERM` |
| Persistent state | `NDBC_LAST_POLLED_FILE`; mount a host volume for restart-safe dedupe/checkpoint behavior |
...
docker pull ghcr.io/clemensv/real-time-sources-noaa-ndbc-mqtt:latest
docker pull ghcr.io/clemensv/real-time-sources-noaa-ndbc-amqp:latest
```
...
|---|---|
| `NDBC_LAST_POLLED_FILE` | Path to dedupe/checkpoint state file. |
