""" BuoyHourlyRainMeasurement dataclass. """
...
@dataclass
class BuoyHourlyRainMeasurement:
    """
    ...
    Attributes:
        ...
        accumulation (typing.Optional[float])
        region (typing.Optional[str])
    """
    
    AvroType: typing.ClassVar[avro.schema.Schema] = avro.schema.parse(
        "{\"type\": \"record\", \"name\": \"BuoyHourlyRainMeasurement\", \"doc\": \"Hourly precipitation total from the NDBC .rain realtime2 product. Each record reports the one-hour rain accumulation for a station timestamp.\", \"fields\": [{\"name\": \"station_id\", \"type\": \"string\", \"doc\": \"NDBC station identifier for the station publishing the .rain realtime2 file.\"}, {\"name\": \"timestamp\", \"type\": {\"type\": \"string\", \"logicalType\": \"timestamp-millis\"}, \"doc\": \"Observation timestamp in UTC, constructed from the YYYY MM DD hh mm columns in the NDBC .rain realtime2 file.\"}, {\"name\": \"accumulation\", \"type\": [\"double\", \"null\"], \"doc\": \"Hourly rain accumulation from the ACCUM column. This is the total precipitation accumulated during the 60-minute period ending at the reported timestamp. Unit: millimeters.\", \"default\": null}, {\"name\": \"region\", \"type\": [\"null\", \"string\"], \"doc\": \"Stable routing axis used by MQTT and AMQP transport templates for noaa-ndbc.\", \"default\": null}]}"
    )
    ...
    accumulation: typing.Optional[float]=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="accumulation"))
    region: typing.Optional[str]=dataclasses.field(kw_only=True, metadata=dataclasses_json.config(field_name="region"))

    @classmethod
    def from_serializer_dict(cls, data: dict) -> 'BuoyHourlyRainMeasurement':
        """
        ...
        return cls(**data)
    @classmethod
    def from_avro_dict(cls, data: dict) -> 'BuoyHourlyRainMeasurement':
        """
        ...
        if 'accumulation' in converted and converted['accumulation'] is not None:
            value = converted['accumulation']
    ...
    @classmethod
    def from_data(cls, data: typing.Any, content_type_string: typing.Optional[str] = None) -> typing.Optional['BuoyHourlyRainMeasurement']:
        """
        ...
        if base_content_type in ['avro/binary', 'application/vnd.apache.avro+avro']:
            if isinstance(data, bytes):
                ...
                with io.BytesIO(data) as stream:
                    ...
                    return BuoyHourlyRainMeasurement.from_avro_dict(_record)
            else:
        ...
        if base_content_type == 'application/json':
            if isinstance(data, (bytes, str)):
                ...
                return BuoyHourlyRainMeasurement.from_serializer_dict(_record)
            else:
                raise NotImplementedError('Data is not of a supported type for JSON deserialization')
        raise NotImplementedError(f'Unsupported media type {content_type}')

    @classmethod
    def create_instance(cls) -> 'BuoyHourlyRainMeasurement':
        """
