...
# Fdsn_seismology_producer Kafka Producer# Fdsn_seismology_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - OrgFdsnEventKafkaEventDispatcher
...
- OrgFdsnEventKafkaProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### OrgFdsnEventKafkaEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`OrgFdsnEventKafkaEventDispatcher` handles events for the org.fdsn.event.kafka message group.
...
from fdsn_seismology_producer import OrgFdsnEventKafkaProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = OrgFdsnEventKafkaProducer(
...
await producer.send_org_fdsn_event_kafka_earthquake(```python

    data=Earthquake(...),add_consumer(self, consumer: KafkaConsumer)
...
The OrgFdsnEventKafkaEventDispatcher defines the following event handler hooks.
...
producer = OrgFdsnEventKafkaProducer(
    ...
    security_protocol='SASL_SSL',##### `org_fdsn_event_kafka_earthquake_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'org_fdsn_event_kafka_earthquake_async:  Callable[[ConsumerRecord, CloudEvent,
Earthquake], Awaitable[None]]

)```

```

Asynchronous handler hook for `org.fdsn.event.kafka.Earthquake`: Telemetry event representing one earthquake detection
record returned by an FDSN Event web-service node in the standard pipe-delimited text format.
...
### OrgFdsnEventKafkaProducer- `data`: The event data of type `fdsn_seismology_producer_data.Earthquake`.



Producer for `org.fdsn.event.kafka` message group.Example:



#### Constructor```python

async def org_fdsn_event_kafka_earthquake_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Earthquake) ->
None:

```python    # Process the event data

OrgFdsnEventKafkaProducer(    await some_processing_function(record, cloud_event, data)
...
org_fdsn_event_kafka_dispatcher.org_fdsn_event_kafka_earthquake_async = org_fdsn_event_kafka_earthquake_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `org_fdsn_event_kafka_node_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'org_fdsn_event_kafka_node_async:  Callable[[ConsumerRecord, CloudEvent, Node],
Awaitable[None]]
...
Asynchronous handler hook for `org.fdsn.event.kafka.Node`: Reference-data event for one pre-configured FDSN event-
service node that this feeder can poll.
...
### OrgFdsnEventKafkaProducer- `data`: The event data of type `fdsn_seismology_producer_data.Node`.
...
async def org_fdsn_event_kafka_node_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Node) -> None:
...
org_fdsn_event_kafka_dispatcher.org_fdsn_event_kafka_node_async = org_fdsn_event_kafka_node_event
...
##### `send_org_fdsn_event_kafka_earthquake`Dispatchers have the following protected methods:



```python### Methods:

async def send_org_fdsn_event_kafka_earthquake(
...
Send a single `org.fdsn.event.kafka.Earthquake` message. Telemetry event representing one earthquake detection record
returned by an FDSN Event web-service node in the standard pipe-delimited text format.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `Earthquake`
...
await producer.send_org_fdsn_event_kafka_earthquake(

    data=Earthquake(...),Args:
...
##### `send_org_fdsn_event_kafka_earthquake_batch`##### `_dispatch_record`



```python```python

async def send_org_fdsn_event_kafka_earthquake_batch(_dispatch_record(self, record)

    self,```

    messages: List[Earthquake],
...
Send multiple `org.fdsn.event.kafka.Earthquake` messages in a batch.
...
await producer.send_org_fdsn_event_kafka_earthquake_batch(```

    messages=[

        Earthquake(...),Initializes the runner with a Kafka consumer.
...
##### `send_org_fdsn_event_kafka_node`Dispatchers have the following protected methods:



```python### Methods:

async def send_org_fdsn_event_kafka_node(
...
Send a single `org.fdsn.event.kafka.Node` message. Reference-data event for one pre-configured FDSN event-service node
that this feeder can poll.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `Node`
...
await producer.send_org_fdsn_event_kafka_node(
...
##### `send_org_fdsn_event_kafka_node_batch`##### `_dispatch_record`



```python```python

async def send_org_fdsn_event_kafka_node_batch(_dispatch_record(self, record)
...
Send multiple `org.fdsn.event.kafka.Node` messages in a batch.
...
await producer.send_org_fdsn_event_kafka_node_batch(```

    messages=[

        Node(...),Initializes the runner with a Kafka consumer.
