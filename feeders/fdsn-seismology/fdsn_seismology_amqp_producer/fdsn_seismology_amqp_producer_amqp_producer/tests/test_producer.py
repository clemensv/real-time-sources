...
"""
Tests for fdsn_seismology_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../fdsn_seismology_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../fdsn_seismology_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../fdsn_seismology_amqp_producer_amqp_producer/src')))
...
class TestOrgFdsnEventAmqpProducer:
    """Test cases for OrgFdsnEventAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(OrgFdsnEventAmqpProducer)
    ...
    def test_send_earthquake(self, artemis_container):
        """Send and receive a Earthquake message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Earthquake.create_instance()
    ...
    def test_send_earthquake_single_fresh_connection(self, artemis_container):
        """Send exactly one Earthquake message on a fresh producer connection."""
        payload = Test_Earthquake.create_instance()
    ...
    def test_send_node(self, artemis_container):
        """Send and receive a Node message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Node.create_instance()
    ...
    def test_send_node_single_fresh_connection(self, artemis_container):
        """Send exactly one Node message on a fresh producer connection."""
        payload = Test_Node.create_instance()
