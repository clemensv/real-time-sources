...
class OrgFdsnEventAmqpProducer:
    """
    Producer class to send messages in the `org.fdsn.event.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_earthquake(self,
        ...
        """
        Send the `org.fdsn.event.amqp.Earthquake` message
        Telemetry event representing one earthquake detection record returned by an FDSN Event web-service node in the standard pipe-delimited text format.
        ...
        attributes = {
            ...
            "source":
            "{node_url}".format(node_url=_node_url),
            "subject":
        ...
        amqp_msg.subject = "{contributor}/{event_id}".format(contributor=_contributor, event_id=_event_id)

        app_properties = {}
        app_properties["contributor"] = "{contributor}".format(contributor=_contributor)
        if app_properties:
    ...
    def send_earthquake_batch(self,
        data_array: typing.List[Earthquake],
        _node_url: str,
        _contributor: str,
        _event_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.fdsn.event.amqp.Earthquake` messages
        
        Args:
            data_array (typing.List[Earthquake]): Array of message data objects
            _node_url (str): Value for placeholder node_url in attribute source
            _contributor (str): Value for placeholder contributor in attribute subject
            _event_id (str): Value for placeholder event_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
    ...
    def send_node(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `org.fdsn.event.amqp.Node` message
        ...
        attributes = {
            ...
            "subject":
            "{node_id}".format(node_id=_node_id),
        }
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        ...
        if amqp_creation_time is not None:
            amqp_msg.creation_time = amqp_creation_time
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "{node_id}".format(node_id=_node_id)

        app_properties = {}
        app_properties["node_id"] = "{node_id}".format(node_id=_node_id)
        if app_properties:
            if amqp_msg.properties is None:
                amqp_msg.properties = {}
            amqp_msg.properties.update(app_properties)
    ...
    def send_node_batch(self,
        data_array: typing.List[Node],
        _base_url: str,
        _node_id: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.fdsn.event.amqp.Node` messages
        
        Args:
            data_array (typing.List[Node]): Array of message data objects
            _base_url (str): Value for placeholder base_url in attribute source
            _node_id (str): Value for placeholder node_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type (str): The content type of the message data
