...
function Resolve-PythonCommand {
    ...
    throw "Python is required to wire the fdsn-seismology Fabric Map but was not found on PATH."
}
...
$mapName = if ($env:FDSN_SEISMOLOGY_FABRIC_MAP_NAME) { $env:FDSN_SEISMOLOGY_FABRIC_MAP_NAME } else { "fdsn-seismology-map" }
$existingMapId = if ($env:FDSN_SEISMOLOGY_FABRIC_MAP_ID) { $env:FDSN_SEISMOLOGY_FABRIC_MAP_ID } else { $null }

if (-not $existingMapId) {
    Write-Host "=== fdsn-seismology post-deploy: ensuring Fabric Map '$mapName' ==="
...
if (-not $existingMapId) {
    ...
    $bodyFile = Join-Path ([System.IO.Path]::GetTempPath()) ("fdsn-seismology-map-create-{0}.json" -f ([Guid]::NewGuid().ToString("N")))
...
$wireScript = Join-Path $PSScriptRoot "wire_fdsn_seismology_map.py"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "wire_fdsn_seismology_map.py failed with exit code $LASTEXITCODE"
    }
} finally {
