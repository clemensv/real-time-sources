...
def get_default_topic_mappings_org_fdsn_event_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.fdsn.event.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.fdsn.event.mqtt.Earthquake": "seismology/fdsn/{contributor}/{event_id}",
        "org.fdsn.event.mqtt.Node": "seismology/fdsn/{node_id}/info",
    }


def get_subscription_topics_org_fdsn_event_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_org_fdsn_event_mqtt()
    topics = []
...
class OrgFdsnEventMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.fdsn.event.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_fdsn_event_mqtt()
        ...
        self.org_fdsn_event_mqtt_earthquake_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, fdsn_seismology_mqtt_producer_data.Earthquake, Dict[str, str]], Awaitable[None]]] = None
        
        self.org_fdsn_event_mqtt_node_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, fdsn_seismology_mqtt_producer_data.Node, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.fdsn.event.Earthquake":
            if self.org_fdsn_event_mqtt_earthquake_async:
                try:
                    ...
                    data = fdsn_seismology_mqtt_producer_data.Earthquake.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.fdsn.event.mqtt.Earthquake")
                    await self.org_fdsn_event_mqtt_earthquake_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_fdsn_event_mqtt_earthquake handler: {e}")
            return
        
        if event_type == "org.fdsn.event.Node":
            if self.org_fdsn_event_mqtt_node_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = fdsn_seismology_mqtt_producer_data.Node.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.fdsn.event.mqtt.Node")
                    await self.org_fdsn_event_mqtt_node_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_fdsn_event_mqtt_node handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_fdsn_event_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_org_fdsn_event_mqtt_earthquake(self,
        node_url: str,
        contributor: str,
        event_id: str,
        data: fdsn_seismology_mqtt_producer_data.Earthquake,
        ...
        """
        Publish the 'org.fdsn.event.mqtt.Earthquake' event to an MQTT topic.

        Args:
        
            node_url: URI template variable for 'node_url'
            contributor: URI template variable for 'contributor'
            event_id: URI template variable for 'event_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'seismology/fdsn/{contributor}/{event_id}'
        ...
        """
        target_topic = topic if topic is not None else "seismology/fdsn/{contributor}/{event_id}"
    ...
    async def publish_org_fdsn_event_mqtt_node(self,
        base_url: str,
        node_id: str,
        data: fdsn_seismology_mqtt_producer_data.Node,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.fdsn.event.mqtt.Node' event to an MQTT topic.

        Args:
        
            base_url: URI template variable for 'base_url'
            node_id: URI template variable for 'node_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'seismology/fdsn/{node_id}/info'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            ...
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "seismology/fdsn/{node_id}/info"
        _topic_template_values: Dict[str, str] = {
        ...
        attributes = {
             "type":"org.fdsn.event.Node",
             "source":"{base_url}".format(base_url = base_url),
             "subject":"{node_id}".format(node_id = node_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
