# Fdsn_seismology_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the fdsn_seismology_mqtt_producer message definitions.

# Fdsn_seismology_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - OrgFdsnEventMqttEventDispatcher
...
from fdsn_seismology_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = OrgFdsnEventMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgFdsnEventMqttEventDispatcher



# Publish message`OrgFdsnEventMqttEventDispatcher` handles events for the org.fdsn.event.mqtt message group.
...
from fdsn_seismology_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = OrgFdsnEventMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe OrgFdsnEventMqttEventDispatcher defines the following event handler hooks.
...
```##### `org_fdsn_event_mqtt_earthquake_async`



## Test```python

org_fdsn_event_mqtt_earthquake_async:  Callable[[PartitionContext, EventData, CloudEvent, Earthquake], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `org.fdsn.event.mqtt.Earthquake`: Telemetry event representing one earthquake detection
...
- `data`: The event data of type `fdsn_seismology_mqtt_producer_data.Earthquake`.

Example:

```python
async def org_fdsn_event_mqtt_earthquake_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Earthquake) -> None:
...
```python
org_fdsn_event_mqtt_dispatcher.org_fdsn_event_mqtt_earthquake_async = org_fdsn_event_mqtt_earthquake_event
```
...
```##### `org_fdsn_event_mqtt_node_async`



## Test```python

org_fdsn_event_mqtt_node_async:  Callable[[PartitionContext, EventData, CloudEvent, Node], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `org.fdsn.event.mqtt.Node`: Reference-data event for one pre-configured FDSN event-
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `fdsn_seismology_mqtt_producer_data.Node`.

Example:

```python
async def org_fdsn_event_mqtt_node_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: Node) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
org_fdsn_event_mqtt_dispatcher.org_fdsn_event_mqtt_node_async = org_fdsn_event_mqtt_node_event
```
