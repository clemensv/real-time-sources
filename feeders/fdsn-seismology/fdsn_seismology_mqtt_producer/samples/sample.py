...
import os
import signal
from fdsn_seismology_mqtt_producer_data import *
from fdsn_seismology_mqtt_producer_mqtt_client.client import OrgFdsnEventMqttProducer, OrgFdsnEventMqttDispatcher

async def handle_org_fdsn_event_mqtt_earthquake(mqtt_msg,cloud_event, org_fdsn_event_mqtt_earthquake_data):
    """ Handles the org.fdsn.event.mqtt.Earthquake message """
    print(f"Received org.fdsn.event.mqtt.Earthquake on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_fdsn_event_mqtt_earthquake_data}")

async def handle_org_fdsn_event_mqtt_node(mqtt_msg,cloud_event, org_fdsn_event_mqtt_node_data):
    """ Handles the org.fdsn.event.mqtt.Node message """
    print(f"Received org.fdsn.event.mqtt.Node on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_fdsn_event_mqtt_node_data}")
