...
def get_default_topic_mappings_org_oasis_cap_alerts_alerts_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.oasis.cap.alerts.alerts.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.oasis.cap.alerts.mqtt.CapAlert": "alerts/cap/{cap_source_id}/{identifier_segment}/alert",
    }


def get_subscription_topics_org_oasis_cap_alerts_alerts_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_org_oasis_cap_alerts_alerts_mqtt()
    topics = []
...
class OrgOasisCapAlertsAlertsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.oasis.cap.alerts.alerts.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_oasis_cap_alerts_alerts_mqtt()
        ...
        self.org_oasis_cap_alerts_mqtt_cap_alert_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, cap_alerts_mqtt_producer_data.CapAlert, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.oasis.cap.alerts.CapAlert":
            if self.org_oasis_cap_alerts_mqtt_cap_alert_async:
                try:
                    ...
                    data = cap_alerts_mqtt_producer_data.CapAlert.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.oasis.cap.alerts.mqtt.CapAlert")
                    await self.org_oasis_cap_alerts_mqtt_cap_alert_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_oasis_cap_alerts_mqtt_cap_alert handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_oasis_cap_alerts_alerts_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_org_oasis_cap_alerts_mqtt_cap_alert(self,
        provider_url: str,
        cap_source_id: str,
        identifier: str,
        identifier_segment: str,
        data: cap_alerts_mqtt_producer_data.CapAlert,
        ...
        """
        Publish the 'org.oasis.cap.alerts.mqtt.CapAlert' event to an MQTT topic.

        Args:
        
            provider_url: URI template variable for 'provider_url'
            cap_source_id: URI template variable for 'cap_source_id'
            identifier: URI template variable for 'identifier'
            identifier_segment: URI template variable for 'identifier_segment'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'alerts/cap/{cap_source_id}/{identifier_segment}/alert'
        ...
        """
        target_topic = topic if topic is not None else "alerts/cap/{cap_source_id}/{identifier_segment}/alert"
        _topic_template_values: Dict[str, str] = {
            "provider_url": str(provider_url),
            "cap_source_id": str(cap_source_id),
            "identifier": str(identifier),
            "identifier_segment": str(identifier_segment),
        }
        ...
        attributes = {
             ...
             "source":"{provider_url}".format(provider_url = provider_url),
             "subject":"{cap_source_id}/{identifier}".format(cap_source_id = cap_source_id,identifier = identifier)
        }
        attributes["datacontenttype"] = content_type
...
def get_default_topic_mappings_org_oasis_cap_alerts_zones_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the org.oasis.cap.alerts.zones.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "org.oasis.cap.alerts.mqtt.CapZone": "alerts/cap/{cap_source_id}/zones/{zone_id_segment}/info",
    }


def get_subscription_topics_org_oasis_cap_alerts_zones_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_org_oasis_cap_alerts_zones_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class OrgOasisCapAlertsZonesMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the org.oasis.cap.alerts.zones.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_org_oasis_cap_alerts_zones_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.org_oasis_cap_alerts_mqtt_cap_zone_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, cap_alerts_mqtt_producer_data.CapZone, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "org.oasis.cap.alerts.CapZone":
            if self.org_oasis_cap_alerts_mqtt_cap_zone_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = cap_alerts_mqtt_producer_data.CapZone.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "org.oasis.cap.alerts.mqtt.CapZone")
                    await self.org_oasis_cap_alerts_mqtt_cap_zone_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in org_oasis_cap_alerts_mqtt_cap_zone handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_org_oasis_cap_alerts_zones_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_org_oasis_cap_alerts_mqtt_cap_zone(self,
        provider_url: str,
        cap_source_id: str,
        zone_id: str,
        zone_id_segment: str,
        data: cap_alerts_mqtt_producer_data.CapZone,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'org.oasis.cap.alerts.mqtt.CapZone' event to an MQTT topic.
        ...
        Args:
            ...
            zone_id_segment: URI template variable for 'zone_id_segment'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'alerts/cap/{cap_source_id}/zones/{zone_id_segment}/info'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            ...
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "alerts/cap/{cap_source_id}/zones/{zone_id_segment}/info"
        _topic_template_values: Dict[str, str] = {
            "provider_url": str(provider_url),
            "cap_source_id": str(cap_source_id),
            "zone_id": str(zone_id),
            "zone_id_segment": str(zone_id_segment),
        }
        if _topic_template_values:
            target_topic = _apply_topic_template(target_topic, _topic_template_values)
        ...
        attributes = {
             ...
             "source":"{provider_url}".format(provider_url = provider_url),
             "subject":"{cap_source_id}/{zone_id}".format(cap_source_id = cap_source_id,zone_id = zone_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
