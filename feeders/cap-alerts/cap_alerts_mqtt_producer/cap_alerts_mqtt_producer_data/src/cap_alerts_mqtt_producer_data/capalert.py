...
import json
from cap_alerts_mqtt_producer_data.scopeenum import ScopeEnum
from cap_alerts_mqtt_producer_data.msgtypeenum import MsgTypeenum
from cap_alerts_mqtt_producer_data.valuepair import ValuePair
from cap_alerts_mqtt_producer_data.capinfo import CapInfo
from cap_alerts_mqtt_producer_data.statusenum import StatusEnum
import datetime
...
class CapAlert:
    ...
    def create_instance(cls) -> 'CapAlert':
        ...
        """
        return cls(
            cap_source_id='umpvubvwadonbjahvtsr',
            identifier='hciadtrkyliwffvsjavl',
            sender='ovhbzqnubhksmyqdihcf',
            sent=datetime.datetime.now(datetime.timezone.utc),
            status=StatusEnum.Actual,
            msg_type=MsgTypeenum.Alert,
            source='iqamzievphelmmnawykk',
            scope=ScopeEnum.Public,
            restriction='cipqgvcckdzlhlktgtxa',
            addresses=['vmjcsckpdsxmwxobqqjo'],
            code=[None, None],
            note='isstppdokpaqjdwkqphu',
            references=['fntzecglvvtbowhmfgsx', 'whwvobadzyjqagovvtxm', 'nfrqqjimtxcqduofzynm'],
            incidents=['rnjumoujhtjtkjzqwmbq', 'fazmvooaofildiddynvs', 'ohlsfwwcfukvyihlmvef', 'sgacljvrqoaxrxfqkrsc'],
            info=[None, None, None, None],
            provider_url='sdbdotpmyqkojbojzohf',
            raw_cap_xml='gldivqzuxlcrqpksqfky',
            area_desc='dufzephteywuiifxvjgx',
            same_codes=['znjxdpjtmedvhnuuznpw', 'rwcppsazcpytzpurbmuk', 'pdiswpecrdphofrmjuqq'],
            ugc_codes=['svzmplkztiyikkofqdfl'],
            vtec=['ounmevvpkyqhyobinwku', 'jlzttongkktasubhxyvv'],
            awareness_level='nedzqthanllpkdqnrrri',
            awareness_type='mxkobawgzcohbjmmnpor',
            event_type='uxalelfvsnplbpiqzunw',
            state='ltfddkevzmcqltaorwgz'
        )