# Cap_alerts_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the cap_alerts_mqtt_producer message definitions.

# Cap_alerts_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - OrgOasisCapAlertsAlertsMqttEventDispatcher,

    OrgOasisCapAlertsZonesMqttEventDispatcher
...
from cap_alerts_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = OrgOasisCapAlertsAlertsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgOasisCapAlertsAlertsMqttEventDispatcher



# Publish message`OrgOasisCapAlertsAlertsMqttEventDispatcher` handles events for the org.oasis.cap.alerts.alerts.mqtt
message group.
...
from cap_alerts_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = OrgOasisCapAlertsAlertsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
client = OrgOasisCapAlertsZonesMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe OrgOasisCapAlertsAlertsMqttEventDispatcher defines the following event handler hooks.
...
```##### `org_oasis_cap_alerts_mqtt_cap_alert_async`



## Test```python

org_oasis_cap_alerts_mqtt_cap_alert_async:  Callable[[PartitionContext, EventData, CloudEvent, CapAlert],
Awaitable[None]]
...
```Asynchronous handler hook for `org.oasis.cap.alerts.mqtt.CapAlert`: Generalized Common Alerting Protocol (CAP) 1.2
...
- `data`: The event data of type `cap_alerts_mqtt_producer_data.CapAlert`.

Example:

```python
async def org_oasis_cap_alerts_mqtt_cap_alert_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: CapAlert) -> None:
...
client = OrgOasisCapAlertsZonesMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgOasisCapAlertsZonesMqttEventDispatcher



# Publish message`OrgOasisCapAlertsZonesMqttEventDispatcher` handles events for the org.oasis.cap.alerts.zones.mqtt
message group.
...
## BuildThe OrgOasisCapAlertsZonesMqttEventDispatcher defines the following event handler hooks.
...
```##### `org_oasis_cap_alerts_mqtt_cap_zone_async`



## Test```python

org_oasis_cap_alerts_mqtt_cap_zone_async:  Callable[[PartitionContext, EventData, CloudEvent, CapZone], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `org.oasis.cap.alerts.mqtt.CapZone`: Reference event for an alerting zone, forecast
zone, MeteoAlarm region, or other area catalog entry exposed by a configured CAP source. The feeder emits zone reference
data before alerts when the source exposes metadata such as api.weather.gov /zones, enabling consumers to interpret
alert geocodes temporally.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `cap_alerts_mqtt_producer_data.CapZone`.

Example:

```python
async def org_oasis_cap_alerts_mqtt_cap_zone_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: CapZone) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
