...
import os
import signal
from cap_alerts_mqtt_producer_data import *
from cap_alerts_mqtt_producer_mqtt_client.client import OrgOasisCapAlertsAlertsMqttProducer, OrgOasisCapAlertsAlertsMqttDispatcher

async def handle_org_oasis_cap_alerts_mqtt_cap_alert(mqtt_msg,cloud_event, org_oasis_cap_alerts_mqtt_cap_alert_data):
    """ Handles the org.oasis.cap.alerts.mqtt.CapAlert message """
    print(f"Received org.oasis.cap.alerts.mqtt.CapAlert on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_oasis_cap_alerts_mqtt_cap_alert_data}")
from cap_alerts_mqtt_producer_mqtt_client.client import OrgOasisCapAlertsZonesMqttProducer, OrgOasisCapAlertsZonesMqttDispatcher

async def handle_org_oasis_cap_alerts_mqtt_cap_zone(mqtt_msg,cloud_event, org_oasis_cap_alerts_mqtt_cap_zone_data):
    """ Handles the org.oasis.cap.alerts.mqtt.CapZone message """
    print(f"Received org.oasis.cap.alerts.mqtt.CapZone on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {org_oasis_cap_alerts_mqtt_cap_zone_data}")
