...
import json
from cap_alerts_amqp_producer_data.scopeenum import ScopeEnum
from cap_alerts_amqp_producer_data.msgtypeenum import MsgTypeenum
from cap_alerts_amqp_producer_data.capinfo import CapInfo
from cap_alerts_amqp_producer_data.statusenum import StatusEnum
...
class CapAlert:
    ...
    def create_instance(cls) -> 'CapAlert':
        ...
        """
        return cls(
            cap_source_id='tqeyilutlssgbiwebwnr',
            identifier='fjhimlrnnwytxeywyxcl',
            sender='ftsyqbsyuvdpjuaugkda',
            sent=datetime.datetime.now(datetime.timezone.utc),
            status=StatusEnum.Actual,
            msg_type=MsgTypeenum.Alert,
            source='mjfctgnejxkcwwqadhyb',
            scope=ScopeEnum.Public,
            restriction='hvlfvlepdlxkspdmxxcp',
            addresses=['mckiqfaznjcfnipmhnte', 'fmjgjgetgtzckhhcldpj', 'pevfyzkapedhumgomqsc', 'dfeivwpjxbozjpcyudsd'],
            code=[None],
            note='ppvhywsdunxkzragrcnf',
            references=['ggemelwkgcjcmnrejocn', 'qerhifjbbmxxcqkllumq', 'oijmsiazkdprqxxjkypw', 'mougddrzjeidmkpftgxl'],
            incidents=['qiackprgibgcvqwrrlvn', 'okbshngrjojkjkhcgkvd', 'vfaembmqkhaxiijlmrcf', 'jkezucqypvcstzdjgnld'],
            info=[None, None],
            provider_url='vawlhobaakwzvwortmnw',
            raw_cap_xml='tnlgeqekglbutbvegapr',
            area_desc='ykonuhesjwucqlqxmvkv',
            same_codes=['lgdupwkbhnjgabzsegvh', 'eigkgjbboirosvebyxid', 'zqmrzzsgkymseuvfniaw'],
            ugc_codes=['nhooosmtoujdymoizfbm', 'bqpyerauqsabtmfxjzfu'],
            vtec=['ngujnapqicdexiahdsdh', 'njykalvylguodewnjwcf', 'ytqvldfwexlnucdzzlip', 'rxgxqgqljzimxscbuyhu', 'eqhtxhzyacxcvdstuhjn'],
            awareness_level='chgpbxuvhiawdglpzoui',
            awareness_type='geupvzxchxjbhzndrbku',
            event_type='iuauclrbyogmgvbvjiro',
            state='bfclrcnqdwoobiduciik'
        )