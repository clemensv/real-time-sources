...
class OrgOasisCapAlertsAlertsAmqpProducer:
    """
    Producer class to send messages in the `org.oasis.cap.alerts.alerts.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_cap_alert(self,
        ...
        """
        Send the `org.oasis.cap.alerts.amqp.CapAlert` message
        ...
        attributes = {
            ...
            "source":
            "{provider_url}".format(provider_url=_provider_url),
            "subject":
            "{cap_source_id}/{identifier}".format(cap_source_id=_cap_source_id, identifier=_identifier),
        }
        ...
        amqp_msg.subject = "{cap_source_id}/{identifier}".format(cap_source_id=_cap_source_id, identifier=_identifier)

        app_properties = {}
        app_properties["cap_source_id"] = "{cap_source_id}".format(cap_source_id=_cap_source_id)
    ...
    def send_cap_alert_batch(self,
        data_array: typing.List[CapAlert],
        _provider_url: str,
        _cap_source_id: str,
        _identifier: str,
        _event_type: str,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.oasis.cap.alerts.amqp.CapAlert` messages
        
        Args:
            data_array (typing.List[CapAlert]): Array of message data objects
            _provider_url (str): Value for placeholder provider_url in attribute source
            _cap_source_id (str): Value for placeholder cap_source_id in attribute subject
            _identifier (str): Value for placeholder identifier in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        for data in data_array:
            self.send_cap_alert(
                data=data,
                _provider_url=_provider_url,
                _cap_source_id=_cap_source_id,
...
class OrgOasisCapAlertsZonesAmqpProducer:
    """
    Producer class to send messages in the `org.oasis.cap.alerts.zones.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_cap_zone(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `org.oasis.cap.alerts.amqp.CapZone` message
        ...
        Args:
            ...
            _zone_type (str): Value for AMQP protocol option placeholder zone_type
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        attributes = {
            ...
            "{provider_url}".format(provider_url=_provider_url),
            "subject":
            "{cap_source_id}/{zone_id}".format(cap_source_id=_cap_source_id, zone_id=_zone_id),
        }
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        ...
        if amqp_creation_time is not None:
            amqp_msg.creation_time = amqp_creation_time
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "{cap_source_id}/{zone_id}".format(cap_source_id=_cap_source_id, zone_id=_zone_id)

        app_properties = {}
        app_properties["cap_source_id"] = "{cap_source_id}".format(cap_source_id=_cap_source_id)
        app_properties["zone_type"] = "{zone_type}".format(zone_type=_zone_type)
        if app_properties:
            if amqp_msg.properties is None:
                amqp_msg.properties = {}
            amqp_msg.properties.update(app_properties)
    ...
    def send_cap_zone_batch(self,
        data_array: typing.List[CapZone],
        _provider_url: str,
        _cap_source_id: str,
        _zone_id: str,
        _zone_type: str,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.oasis.cap.alerts.amqp.CapZone` messages
        
        Args:
            data_array (typing.List[CapZone]): Array of message data objects
            _provider_url (str): Value for placeholder provider_url in attribute source
            _cap_source_id (str): Value for placeholder cap_source_id in attribute subject
            _zone_id (str): Value for placeholder zone_id in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
