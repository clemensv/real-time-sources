...
"""
Tests for cap_alerts_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../cap_alerts_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../cap_alerts_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../cap_alerts_amqp_producer_amqp_producer/src')))

from cap_alerts_amqp_producer_amqp_producer import *
from cap_alerts_amqp_producer_data import CapAlert
from test_capalert import Test_CapAlert
from cap_alerts_amqp_producer_data import CapZone
...
class TestOrgOasisCapAlertsAlertsAmqpProducer:
    """Test cases for OrgOasisCapAlertsAlertsAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(OrgOasisCapAlertsAlertsAmqpProducer)
    ...
    def test_send_cap_alert(self, artemis_container):
        """Send and receive a CapAlert message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_CapAlert.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "org.oasis.cap.alerts.CapAlert"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{cap_source_id}/{identifier}".format(cap_source_id="value", identifier="value")
                assert properties.get('cap_source_id') == "{cap_source_id}".format(cap_source_id="value")
                assert properties.get('event_type') == "{event_type}".format(event_type="value")
        finally:
            producer.close()

    def test_send_cap_alert_single_fresh_connection(self, artemis_container):
        """Send exactly one CapAlert message on a fresh producer connection."""
        payload = Test_CapAlert.create_instance()
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.oasis.cap.alerts.CapAlert'
        assert received.body is not None
        assert received.subject == "{cap_source_id}/{identifier}".format(cap_source_id="value", identifier="value")
        assert properties.get('cap_source_id') == "{cap_source_id}".format(cap_source_id="value")
        assert properties.get('event_type') == "{event_type}".format(event_type="value")
...
class TestOrgOasisCapAlertsZonesAmqpProducer:
    """Test cases for OrgOasisCapAlertsZonesAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        fake_connection = FakeConnection()
        producer = object.__new__(OrgOasisCapAlertsZonesAmqpProducer)
        producer._sender = fake_sender
        producer._connection = fake_connection
        producer._blocking_sender_is_presettled = True
    ...
    def test_send_cap_zone(self, artemis_container):
        """Send and receive a CapZone message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_CapZone.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "org.oasis.cap.alerts.CapZone"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{cap_source_id}/{zone_id}".format(cap_source_id="value", zone_id="value")
                assert properties.get('cap_source_id') == "{cap_source_id}".format(cap_source_id="value")
                assert properties.get('zone_type') == "{zone_type}".format(zone_type="value")
        finally:
            producer.close()

    def test_send_cap_zone_single_fresh_connection(self, artemis_container):
        """Send exactly one CapZone message on a fresh producer connection."""
        payload = Test_CapZone.create_instance()
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'org.oasis.cap.alerts.CapZone'
        assert received.body is not None
        assert received.subject == "{cap_source_id}/{zone_id}".format(cap_source_id="value", zone_id="value")
        assert properties.get('cap_source_id') == "{cap_source_id}".format(cap_source_id="value")
        assert properties.get('zone_type') == "{zone_type}".format(zone_type="value")
