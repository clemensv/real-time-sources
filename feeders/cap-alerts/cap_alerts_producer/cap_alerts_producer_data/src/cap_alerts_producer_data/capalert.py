...
import json
from cap_alerts_producer_data.msgtypeenum import MsgTypeenum
from cap_alerts_producer_data.statusenum import StatusEnum
from cap_alerts_producer_data.scopeenum import ScopeEnum
from cap_alerts_producer_data.valuepair import ValuePair
from cap_alerts_producer_data.capinfo import CapInfo
import datetime
...
class CapAlert:
    ...
    def create_instance(cls) -> 'CapAlert':
        ...
        """
        return cls(
            cap_source_id='rwirmhuaoawbhccqrmge',
            identifier='eukojsuojftpirzljsmn',
            sender='mkkbnpzamixeawymsueb',
            sent=datetime.datetime.now(datetime.timezone.utc),
            status=StatusEnum.Actual,
            msg_type=MsgTypeenum.Alert,
            source='clfgeiscuaklkizcilao',
            scope=ScopeEnum.Public,
            restriction='mtjuheoqkjpmjckrvydt',
            addresses=['xkplsjfglfpkhjekpjeo', 'xtkatzhebsnzbjfaouwi', 'jbpefioerrwykddcdrrn'],
            code=[None],
            note='thfwzntfcxafjtkntxgr',
            references=['wpnpzvkfhpdoxpqiaege', 'tywzttnpbykjnljiurct', 'qrpifwynktxoarwtwanj', 'atvfuforwjdheszrcjep', 'vkqtliumqyceuovctkzg'],
            incidents=['roaugbcqgxyrlhmmbglx', 'nnmiuefhukpdwecmhgpq', 'xeeckadphwtipfhbhwfk', 'uyysgbcejbfffdjswltv'],
            info=[None],
            provider_url='zwiaonvghkdeialkajcf',
            raw_cap_xml='pblmtabhkqsetisuaetq',
            area_desc='vfntavrpqfwhceuvcidz',
            same_codes=['yntfxlueawqmuyoyzufi'],
            ugc_codes=['ttwtimnlgbnnrpiblurz'],
            vtec=['jecwoudzadpojoihngfi'],
            awareness_level='fiozllxbqlpzdsymguoc',
            awareness_type='ipntcgbwtbazsphfciip',
            event_type='opccsrhuxsmjrspsbqko',
            state='owayhtaalqwtzslvgamy'
        )