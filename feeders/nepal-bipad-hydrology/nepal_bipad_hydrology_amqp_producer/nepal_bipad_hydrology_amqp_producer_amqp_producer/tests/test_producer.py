...
"""
Tests for nepal_bipad_hydrology_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../nepal_bipad_hydrology_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../nepal_bipad_hydrology_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../nepal_bipad_hydrology_amqp_producer_amqp_producer/src')))
...
class TestNpGovBipadHydrologyAmqpProducer:
    """Test cases for NpGovBipadHydrologyAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(NpGovBipadHydrologyAmqpProducer)
    ...
    def test_send_river_station(self, artemis_container):
        """Send and receive a RiverStation message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_RiverStation.create_instance()
    ...
    def test_send_river_station_single_fresh_connection(self, artemis_container):
        """Send exactly one RiverStation message on a fresh producer connection."""
        payload = Test_RiverStation.create_instance()
