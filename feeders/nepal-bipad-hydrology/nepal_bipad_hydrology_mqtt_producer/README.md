# Nepal_bipad_hydrology_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the nepal_bipad_hydrology_mqtt_producer message definitions.

# Nepal_bipad_hydrology_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - NpGovBipadHydrologyMqttEventDispatcher
...
from nepal_bipad_hydrology_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = NpGovBipadHydrologyMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NpGovBipadHydrologyMqttEventDispatcher



# Publish message`NpGovBipadHydrologyMqttEventDispatcher` handles events for the np.gov.bipad.hydrology.mqtt message
group.
...
from nepal_bipad_hydrology_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = NpGovBipadHydrologyMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe NpGovBipadHydrologyMqttEventDispatcher defines the following event handler hooks.
...
```##### `np_gov_bipad_hydrology_mqtt_river_station_async`



## Test```python

np_gov_bipad_hydrology_mqtt_river_station_async:  Callable[[PartitionContext, EventData, CloudEvent, RiverStation],
Awaitable[None]]
...
- `data`: The event data of type `nepal_bipad_hydrology_mqtt_producer_data.RiverStation`.

Example:

```python
async def np_gov_bipad_hydrology_mqtt_river_station_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RiverStation) -> None:
...
```##### `np_gov_bipad_hydrology_mqtt_water_level_reading_async`



## Test```python

np_gov_bipad_hydrology_mqtt_water_level_reading_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```Asynchronous handler hook for `np.gov.bipad.hydrology.mqtt.WaterLevelReading`: A current measurement from Nepal's
BIPAD Portal for one monitoring site. It carries river water level observations when the upstream feed reports a new or
refreshed value.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `nepal_bipad_hydrology_mqtt_producer_data.WaterLevelReading`.

Example:

```python
async def np_gov_bipad_hydrology_mqtt_water_level_reading_event(partition_context: PartitionContext, event: EventData,
