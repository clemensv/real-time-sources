...
import os
import signal
from nepal_bipad_hydrology_mqtt_producer_data import *
from nepal_bipad_hydrology_mqtt_producer_mqtt_client.client import NpGovBipadHydrologyMqttProducer, NpGovBipadHydrologyMqttDispatcher

async def handle_np_gov_bipad_hydrology_mqtt_river_station(mqtt_msg,cloud_event, np_gov_bipad_hydrology_mqtt_river_station_data):
    """ Handles the np.gov.bipad.hydrology.mqtt.RiverStation message """
    print(f"Received np.gov.bipad.hydrology.mqtt.RiverStation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {np_gov_bipad_hydrology_mqtt_river_station_data}")

async def handle_np_gov_bipad_hydrology_mqtt_water_level_reading(mqtt_msg,cloud_event, np_gov_bipad_hydrology_mqtt_water_level_reading_data):
    """ Handles the np.gov.bipad.hydrology.mqtt.WaterLevelReading message """
    print(f"Received np.gov.bipad.hydrology.mqtt.WaterLevelReading on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {np_gov_bipad_hydrology_mqtt_water_level_reading_data}")
