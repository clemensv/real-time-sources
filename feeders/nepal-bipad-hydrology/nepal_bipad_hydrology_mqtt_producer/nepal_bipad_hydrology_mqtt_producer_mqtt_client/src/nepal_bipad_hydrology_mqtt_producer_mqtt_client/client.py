...
def get_default_topic_mappings_np_gov_bipad_hydrology_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the np.gov.bipad.hydrology.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "np.gov.bipad.hydrology.mqtt.RiverStation": "hydro/np/bipad/nepal-bipad-hydrology/{basin}/{station_id}/info",
        "np.gov.bipad.hydrology.mqtt.WaterLevelReading": "hydro/np/bipad/nepal-bipad-hydrology/{basin}/{station_id}/water-level",
    }


def get_subscription_topics_np_gov_bipad_hydrology_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_np_gov_bipad_hydrology_mqtt()
    topics = []
...
class NpGovBipadHydrologyMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the np.gov.bipad.hydrology.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_np_gov_bipad_hydrology_mqtt()
        ...
        self.np_gov_bipad_hydrology_mqtt_river_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, nepal_bipad_hydrology_mqtt_producer_data.RiverStation, Dict[str, str]], Awaitable[None]]] = None
        
        self.np_gov_bipad_hydrology_mqtt_water_level_reading_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, nepal_bipad_hydrology_mqtt_producer_data.WaterLevelReading, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "np.gov.bipad.hydrology.RiverStation":
            if self.np_gov_bipad_hydrology_mqtt_river_station_async:
                try:
                    ...
                    data = nepal_bipad_hydrology_mqtt_producer_data.RiverStation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "np.gov.bipad.hydrology.mqtt.RiverStation")
                    await self.np_gov_bipad_hydrology_mqtt_river_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in np_gov_bipad_hydrology_mqtt_river_station handler: {e}")
            return
        
        if event_type == "np.gov.bipad.hydrology.WaterLevelReading":
            if self.np_gov_bipad_hydrology_mqtt_water_level_reading_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = nepal_bipad_hydrology_mqtt_producer_data.WaterLevelReading.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "np.gov.bipad.hydrology.mqtt.WaterLevelReading")
                    await self.np_gov_bipad_hydrology_mqtt_water_level_reading_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in np_gov_bipad_hydrology_mqtt_water_level_reading handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_np_gov_bipad_hydrology_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_np_gov_bipad_hydrology_mqtt_river_station(self,
        feedurl: str,
        station_id: str,
        basin: str,
        data: nepal_bipad_hydrology_mqtt_producer_data.RiverStation,
        ...
        """
        Publish the 'np.gov.bipad.hydrology.mqtt.RiverStation' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'hydro/np/bipad/nepal-bipad-hydrology/{basin}/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "hydro/np/bipad/nepal-bipad-hydrology/{basin}/{station_id}/info"
        ...
        attributes = {
             "type":"np.gov.bipad.hydrology.RiverStation",
    ...
    async def publish_np_gov_bipad_hydrology_mqtt_water_level_reading(self,
        feedurl: str,
        station_id: str,
        basin: str,
        data: nepal_bipad_hydrology_mqtt_producer_data.WaterLevelReading,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'np.gov.bipad.hydrology.mqtt.WaterLevelReading' event to an MQTT topic.
        ...
        Args:
            ...
            basin: URI template variable for 'basin'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/np/bipad/nepal-bipad-hydrology/{basin}/{station_id}/water-level'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "hydro/np/bipad/nepal-bipad-hydrology/{basin}/{station_id}/water-level"
        _topic_template_values: Dict[str, str] = {
            "feedurl": str(feedurl),
            "station_id": str(station_id),
            "basin": str(basin),
        ...
        attributes = {
             "type":"np.gov.bipad.hydrology.WaterLevelReading",
             "source":"{feedurl}".format(feedurl = feedurl),
             "subject":"{station_id}".format(station_id = station_id)
        }
        attributes["datacontenttype"] = content_type
