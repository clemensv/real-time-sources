...
import os
import signal
from nextbus_mqtt_producer_data import *
from nextbus_mqtt_producer_mqtt_client.client import NextbusMqttProducer, NextbusMqttDispatcher

async def handle_nextbus_vehicle_position_mqtt(mqtt_msg,cloud_event, nextbus_vehicle_position_mqtt_data):
    """ Handles the nextbus.VehiclePosition.mqtt message """
    print(f"Received nextbus.VehiclePosition.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {nextbus_vehicle_position_mqtt_data}")

async def handle_nextbus_route_config_mqtt(mqtt_msg,cloud_event, nextbus_route_config_mqtt_data):
    """ Handles the nextbus.RouteConfig.mqtt message """
    print(f"Received nextbus.RouteConfig.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {nextbus_route_config_mqtt_data}")

async def handle_nextbus_schedule_mqtt(mqtt_msg,cloud_event, nextbus_schedule_mqtt_data):
    """ Handles the nextbus.Schedule.mqtt message """
    print(f"Received nextbus.Schedule.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {nextbus_schedule_mqtt_data}")

async def handle_nextbus_message_mqtt(mqtt_msg,cloud_event, nextbus_message_mqtt_data):
    """ Handles the nextbus.Message.mqtt message """
    print(f"Received nextbus.Message.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {nextbus_message_mqtt_data}")
