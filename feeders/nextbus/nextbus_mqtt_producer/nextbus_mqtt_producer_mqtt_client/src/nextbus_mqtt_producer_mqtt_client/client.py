...
def get_default_topic_mappings_nextbus_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the nextbus.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "nextbus.VehiclePosition.mqtt": "transit/intl/nextbus/nextbus/{agency_id}/{route_tag}/{event_type}/{stop_or_vehicle_id}",
        "nextbus.RouteConfig.mqtt": "transit/intl/nextbus/nextbus/{agency_id}/{route_tag}/{event_type}/{stop_or_vehicle_id}",
        "nextbus.Schedule.mqtt": "transit/intl/nextbus/nextbus/{agency_id}/{route_tag}/{event_type}/{stop_or_vehicle_id}",
        "nextbus.Message.mqtt": "transit/intl/nextbus/nextbus/{agency_id}/{route_tag}/{event_type}/{stop_or_vehicle_id}",
    }


def get_subscription_topics_nextbus_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_nextbus_mqtt()
    topics = []
...
class NextbusMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the nextbus.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_nextbus_mqtt()
        ...
        self.nextbus_vehicle_position_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, nextbus_mqtt_producer_data.VehiclePosition, Dict[str, str]], Awaitable[None]]] = None
        
        self.nextbus_route_config_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, nextbus_mqtt_producer_data.RouteConfig, Dict[str, str]], Awaitable[None]]] = None
        
        self.nextbus_schedule_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, nextbus_mqtt_producer_data.Schedule, Dict[str, str]], Awaitable[None]]] = None
        
        self.nextbus_message_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, nextbus_mqtt_producer_data.Message, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "nextbus.VehiclePosition":
            if self.nextbus_vehicle_position_mqtt_async:
                try:
                    ...
                    data = nextbus_mqtt_producer_data.VehiclePosition.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "nextbus.VehiclePosition.mqtt")
                    await self.nextbus_vehicle_position_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in nextbus_vehicle_position_mqtt handler: {e}")
            return
        
        if event_type == "nextbus.RouteConfig":
            if self.nextbus_route_config_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = nextbus_mqtt_producer_data.RouteConfig.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "nextbus.RouteConfig.mqtt")
                    await self.nextbus_route_config_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in nextbus_route_config_mqtt handler: {e}")
            return
        
        if event_type == "nextbus.Schedule":
            if self.nextbus_schedule_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = nextbus_mqtt_producer_data.Schedule.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "nextbus.Schedule.mqtt")
                    await self.nextbus_schedule_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in nextbus_schedule_mqtt handler: {e}")
            return
        
        if event_type == "nextbus.Message":
            if self.nextbus_message_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = nextbus_mqtt_producer_data.Message.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "nextbus.Message.mqtt")
                    await self.nextbus_message_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in nextbus_message_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_nextbus_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_nextbus_vehicle_position_mqtt(self,
        agency_id: str,
        route_tag: str,
        vehicle_id: str,
        event_type: str,
        ...
        """
        Publish the 'nextbus.VehiclePosition.mqtt' event to an MQTT topic.
        ...
        """
        target_topic = topic if topic is not None else "transit/intl/nextbus/nextbus/{agency_id}/{route_tag}/{event_type}/{stop_or_vehicle_id}"
    ...
    async def publish_nextbus_route_config_mqtt(self,
        agency_id: str,
        route_tag: str,
        stop_or_vehicle_id: str,
        event_type: str,
        ...
        content_type: str = "application/json") -> None:
        """
        Publish the 'nextbus.RouteConfig.mqtt' event to an MQTT topic.
    ...
    async def publish_nextbus_schedule_mqtt(self,
        agency_id: str,
        route_tag: str,
        stop_or_vehicle_id: str,
        event_type: str,
        ...
        content_type: str = "application/json") -> None:
        """
        Publish the 'nextbus.Schedule.mqtt' event to an MQTT topic.
    ...
    async def publish_nextbus_message_mqtt(self,
        agency_id: str,
        route_tag: str,
        stop_or_vehicle_id: str,
        event_type: str,
        ...
        content_type: str = "application/json") -> None:
        """
        Publish the 'nextbus.Message.mqtt' event to an MQTT topic.
