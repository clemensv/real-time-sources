...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../carbon_intensity_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../carbon_intensity_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../carbon_intensity_producer_kafka_producer/src')))
...
def test_uk_org_carbonintensity_ukorgcarbonintensityintensity(kafka_emulator):
    """Test the UkOrgCarbonintensityIntensity event from the Uk.Org.Carbonintensity message group"""
    ...
    consumer = Consumer({
        ...
        'group.id': 'test_uk_org_carbonintensity_ukorgcarbonintensityintensity',  # Unique group per test
    ...
    producer_instance = UkOrgCarbonintensityEventProducer(kafka_producer, topic, 'binary')
    ...
    for i in range(5):
        ...
        expected_key = "{period_from}".format(period_from=f'test_{i}')
        assert received_key == expected_key, f"Expected Kafka key '{expected_key}' but got '{received_key}'"
    consumer.close()


def test_uk_org_carbonintensity_ukorgcarbonintensitygenerationmix(kafka_emulator):
    """Test the UkOrgCarbonintensityGenerationMix event from the Uk.Org.Carbonintensity message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_uk_org_carbonintensity_ukorgcarbonintensitygenerationmix',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
...
def test_uk_org_carbonintensity_regional_ukorgcarbonintensityregionalintensity(kafka_emulator):
    """Test the UkOrgCarbonintensityRegionalIntensity event from the Uk.Org.Carbonintensity.Regional message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_uk_org_carbonintensity_regional_ukorgcarbonintensityregionalintensity',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = UkOrgCarbonintensityRegionalEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    ...
    for i in range(5):
        received_key = on_event()
        assert received_key is not None, f"Failed to receive message {i+1} of 5"
        expected_key = "{region_id}".format(region_id=f'test_{i}')
        assert received_key == expected_key, f"Expected Kafka key '{expected_key}' but got '{received_key}'"
    consumer.close()


def test_uk_org_carbonintensity_mqtt_ukorgcarbonintensitymqttintensity(kafka_emulator):
    """Test the UkOrgCarbonintensityMqttIntensity event from the Uk.Org.Carbonintensity.Mqtt message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_uk_org_carbonintensity_mqtt_ukorgcarbonintensitymqttintensity',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = UkOrgCarbonintensityMqttEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    event_data = Test_Intensity.create_instance()
...
def test_uk_org_carbonintensity_mqtt_ukorgcarbonintensitymqttgenerationmix(kafka_emulator):
    """Test the UkOrgCarbonintensityMqttGenerationMix event from the Uk.Org.Carbonintensity.Mqtt message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_uk_org_carbonintensity_mqtt_ukorgcarbonintensitymqttgenerationmix',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
...
def test_uk_org_carbonintensity_mqtt_ukorgcarbonintensitymqttregionalintensity(kafka_emulator):
    """Test the UkOrgCarbonintensityMqttRegionalIntensity event from the Uk.Org.Carbonintensity.Mqtt message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_uk_org_carbonintensity_mqtt_ukorgcarbonintensitymqttregionalintensity',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
...
def test_uk_org_carbonintensity_cross_event_type_kafka_key(kafka_emulator):
    """Test that different event types in Uk.Org.Carbonintensity produce the same Kafka key for the same placeholder values"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_uk_org_carbonintensity_cross_key',
    ...
    data1 = Test_Intensity.create_instance()
    data2 = Test_GenerationMix.create_instance()

    producer_instance.send_uk_org_carbonintensity_intensity(_period_from = shared_key_value, _ce_id = shared_key_value, data = data1)
    producer_instance.send_uk_org_carbonintensity_generation_mix(_period_from = shared_key_value, _ce_id = shared_key_value, data = data2)
    kafka_producer.flush(timeout=5.0)
    ...
    while len(collected_keys) < 2 and time.time() < timeout:
        ...
        if msg is None or msg.error():
            continue
        cloudevent = parse_cloudevent(msg)
        if cloudevent['type'] in ["uk.org.carbonintensity.Intensity", "uk.org.carbonintensity.GenerationMix"]:
    ...
    expected_key = "{period_from}".format(period_from=shared_key_value)
