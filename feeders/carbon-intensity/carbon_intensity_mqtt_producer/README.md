# Carbon_intensity_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the carbon_intensity_mqtt_producer message definitions.

# Carbon_intensity_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - UkOrgCarbonintensityMqttEventDispatcher
...
from carbon_intensity_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = UkOrgCarbonintensityMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### UkOrgCarbonintensityMqttEventDispatcher



# Publish message`UkOrgCarbonintensityMqttEventDispatcher` handles events for the uk.org.carbonintensity.mqtt message
group.
...
from carbon_intensity_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = UkOrgCarbonintensityMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe UkOrgCarbonintensityMqttEventDispatcher defines the following event handler hooks.
...
```##### `uk_org_carbonintensity_mqtt_intensity_async`



## Test```python

uk_org_carbonintensity_mqtt_intensity_async:  Callable[[PartitionContext, EventData, CloudEvent, Intensity],
Awaitable[None]]
...
- `data`: The event data of type `carbon_intensity_mqtt_producer_data.Intensity`.

Example:

```python
async def uk_org_carbonintensity_mqtt_intensity_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: Intensity) -> None:
...
```##### `uk_org_carbonintensity_mqtt_generation_mix_async`



## Test```python

uk_org_carbonintensity_mqtt_generation_mix_async:  Callable[[PartitionContext, EventData, CloudEvent, GenerationMix],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `carbon_intensity_mqtt_producer_data.GenerationMix`.

Example:

```python
async def uk_org_carbonintensity_mqtt_generation_mix_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: GenerationMix) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `uk_org_carbonintensity_mqtt_regional_intensity_async`



## Test```python

uk_org_carbonintensity_mqtt_regional_intensity_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `carbon_intensity_mqtt_producer_data.RegionalIntensity`.

Example:

```python
async def uk_org_carbonintensity_mqtt_regional_intensity_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RegionalIntensity) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
