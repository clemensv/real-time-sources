...
import os
import signal
from carbon_intensity_mqtt_producer_data import *
from carbon_intensity_mqtt_producer_mqtt_client.client import UkOrgCarbonintensityMqttProducer, UkOrgCarbonintensityMqttDispatcher

async def handle_uk_org_carbonintensity_mqtt_intensity(mqtt_msg,cloud_event, uk_org_carbonintensity_mqtt_intensity_data):
    """ Handles the uk.org.carbonintensity.mqtt.Intensity message """
    print(f"Received uk.org.carbonintensity.mqtt.Intensity on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_org_carbonintensity_mqtt_intensity_data}")

async def handle_uk_org_carbonintensity_mqtt_generation_mix(mqtt_msg,cloud_event, uk_org_carbonintensity_mqtt_generation_mix_data):
    """ Handles the uk.org.carbonintensity.mqtt.GenerationMix message """
    print(f"Received uk.org.carbonintensity.mqtt.GenerationMix on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_org_carbonintensity_mqtt_generation_mix_data}")

async def handle_uk_org_carbonintensity_mqtt_regional_intensity(mqtt_msg,cloud_event, uk_org_carbonintensity_mqtt_regional_intensity_data):
    """ Handles the uk.org.carbonintensity.mqtt.RegionalIntensity message """
    print(f"Received uk.org.carbonintensity.mqtt.RegionalIntensity on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_org_carbonintensity_mqtt_regional_intensity_data}")
