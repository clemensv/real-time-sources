...
import carbon_intensity_mqtt_producer_data
from carbon_intensity_mqtt_producer_data import Intensity
from carbon_intensity_mqtt_producer_data import GenerationMix
from carbon_intensity_mqtt_producer_data import RegionalIntensity
...
def get_default_topic_mappings_uk_org_carbonintensity_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the uk.org.carbonintensity.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "uk.org.carbonintensity.mqtt.Intensity": "energy/gb/national-grid/carbon-intensity/{region}/intensity",
        "uk.org.carbonintensity.mqtt.GenerationMix": "energy/gb/national-grid/carbon-intensity/{region}/generation-mix",
        "uk.org.carbonintensity.mqtt.RegionalIntensity": "energy/gb/national-grid/carbon-intensity/{region}/regional-intensity",
    }


def get_subscription_topics_uk_org_carbonintensity_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_uk_org_carbonintensity_mqtt()
    topics = []
...
class UkOrgCarbonintensityMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the uk.org.carbonintensity.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_uk_org_carbonintensity_mqtt()
        ...
        self.uk_org_carbonintensity_mqtt_intensity_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, carbon_intensity_mqtt_producer_data.Intensity, Dict[str, str]], Awaitable[None]]] = None
        
        self.uk_org_carbonintensity_mqtt_generation_mix_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, carbon_intensity_mqtt_producer_data.GenerationMix, Dict[str, str]], Awaitable[None]]] = None
        
        self.uk_org_carbonintensity_mqtt_regional_intensity_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, carbon_intensity_mqtt_producer_data.RegionalIntensity, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "uk.org.carbonintensity.Intensity":
            if self.uk_org_carbonintensity_mqtt_intensity_async:
                try:
                    ...
                    data = carbon_intensity_mqtt_producer_data.Intensity.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.org.carbonintensity.mqtt.Intensity")
                    await self.uk_org_carbonintensity_mqtt_intensity_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_org_carbonintensity_mqtt_intensity handler: {e}")
            return
        
        if event_type == "uk.org.carbonintensity.GenerationMix":
            if self.uk_org_carbonintensity_mqtt_generation_mix_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = carbon_intensity_mqtt_producer_data.GenerationMix.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.org.carbonintensity.mqtt.GenerationMix")
                    await self.uk_org_carbonintensity_mqtt_generation_mix_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_org_carbonintensity_mqtt_generation_mix handler: {e}")
            return
        
        if event_type == "uk.org.carbonintensity.RegionalIntensity":
            if self.uk_org_carbonintensity_mqtt_regional_intensity_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = carbon_intensity_mqtt_producer_data.RegionalIntensity.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "uk.org.carbonintensity.mqtt.RegionalIntensity")
                    await self.uk_org_carbonintensity_mqtt_regional_intensity_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in uk_org_carbonintensity_mqtt_regional_intensity handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_uk_org_carbonintensity_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_uk_org_carbonintensity_mqtt_intensity(self,
        period_from: str,
        ce_id: str,
        region: str,
        data: carbon_intensity_mqtt_producer_data.Intensity,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'uk.org.carbonintensity.mqtt.Intensity' event to an MQTT topic.

        Args:
        
            period_from: URI template variable for 'period_from'
            ce_id: URI template variable for 'ce_id'
            region: URI template variable for 'region'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'energy/gb/national-grid/carbon-intensity/{region}/intensity'
        ...
        """
        target_topic = topic if topic is not None else "energy/gb/national-grid/carbon-intensity/{region}/intensity"
        ...
        attributes = {
             "type":"uk.org.carbonintensity.Intensity",
             "source":"https://api.carbonintensity.org.uk",
             "subject":"{period_from}".format(period_from = period_from),
    ...
    async def publish_uk_org_carbonintensity_mqtt_generation_mix(self,
        period_from: str,
        ce_id: str,
        region: str,
        data: carbon_intensity_mqtt_producer_data.GenerationMix,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'uk.org.carbonintensity.mqtt.GenerationMix' event to an MQTT topic.
        ...
        Args:
            ...
            region: URI template variable for 'region'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'energy/gb/national-grid/carbon-intensity/{region}/generation-mix'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "energy/gb/national-grid/carbon-intensity/{region}/generation-mix"
        _topic_template_values: Dict[str, str] = {
            "period_from": str(period_from),
            "ce_id": str(ce_id),
            "region": str(region),
        ...
        attributes = {
             "type":"uk.org.carbonintensity.GenerationMix",
             "source":"https://api.carbonintensity.org.uk",
             "subject":"{period_from}".format(period_from = period_from),
             "id":"{ce_id}".format(ce_id = ce_id)
        }
    ...
    async def publish_uk_org_carbonintensity_mqtt_regional_intensity(self,
        region_id: str,
        ce_id: str,
        region: str,
        data: carbon_intensity_mqtt_producer_data.RegionalIntensity,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'uk.org.carbonintensity.mqtt.RegionalIntensity' event to an MQTT topic.

        Args:
        
            region_id: URI template variable for 'region_id'
            ce_id: URI template variable for 'ce_id'
            region: URI template variable for 'region'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'energy/gb/national-grid/carbon-intensity/{region}/regional-intensity'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "energy/gb/national-grid/carbon-intensity/{region}/regional-intensity"
        _topic_template_values: Dict[str, str] = {
        ...
        attributes = {
             "type":"uk.org.carbonintensity.RegionalIntensity",
             "source":"https://api.carbonintensity.org.uk",
             "subject":"{region_id}".format(region_id = region_id),
             "id":"{ce_id}".format(ce_id = ce_id)
        }
        attributes["datacontenttype"] = content_type
        byte_data = data.to_byte_array(content_type) if data is not None else b''
