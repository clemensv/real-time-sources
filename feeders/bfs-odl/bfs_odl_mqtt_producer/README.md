# Bfs_odl_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the bfs_odl_mqtt_producer message definitions.

# Bfs_odl_mqtt_producer Event Dispatcher for Azure Event Hubs
...
from bfs_odl_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = DeBfsOdlMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### DeBfsOdlMqttEventDispatcher



# Publish message`DeBfsOdlMqttEventDispatcher` handles events for the de.bfs.odl.mqtt message group.
...
from bfs_odl_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = DeBfsOdlMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe DeBfsOdlMqttEventDispatcher defines the following event handler hooks.
...
```##### `de_bfs_odl_mqtt_station_async`



## Test```python

de_bfs_odl_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `de.bfs.odl.mqtt.Station`: Metadata for an ODL (Ortsdosisleistung) measuring station in
the BfS gamma dose rate monitoring network. The BfS operates approximately 1,700 stationary probes across Germany that
...
- `data`: The event data of type `bfs_odl_mqtt_producer_data.Station`.

Example:

```python
async def de_bfs_odl_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: Station) -> None:
...
```python
de_bfs_odl_mqtt_dispatcher.de_bfs_odl_mqtt_station_async = de_bfs_odl_mqtt_station_event
```
...
```##### `de_bfs_odl_mqtt_dose_rate_measurement_async`



## Test```python

de_bfs_odl_mqtt_dose_rate_measurement_async:  Callable[[PartitionContext, EventData, CloudEvent, DoseRateMeasurement],
Awaitable[None]]
...
```Asynchronous handler hook for `de.bfs.odl.mqtt.DoseRateMeasurement`: A one-hour averaged ambient gamma dose rate
measurement from the BfS ODL monitoring network. Each measurement reports the gross gamma dose rate in microsieverts per
hour (µSv/h), optionally split into cosmic and terrestrial components. The cosmic component originates from secondary
cosmic radiation and depends primarily on altitude; the terrestrial component originates from naturally occurring
radionuclides in the ground and varies with local geology.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `bfs_odl_mqtt_producer_data.DoseRateMeasurement`.

Example:

```python
async def de_bfs_odl_mqtt_dose_rate_measurement_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: DoseRateMeasurement) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
de_bfs_odl_mqtt_dispatcher.de_bfs_odl_mqtt_dose_rate_measurement_async = de_bfs_odl_mqtt_dose_rate_measurement_event
```
