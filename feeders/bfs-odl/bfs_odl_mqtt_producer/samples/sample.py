...
import os
import signal
from bfs_odl_mqtt_producer_data import *
from bfs_odl_mqtt_producer_mqtt_client.client import DeBfsOdlMqttProducer, DeBfsOdlMqttDispatcher

async def handle_de_bfs_odl_mqtt_station(mqtt_msg,cloud_event, de_bfs_odl_mqtt_station_data):
    """ Handles the de.bfs.odl.mqtt.Station message """
    print(f"Received de.bfs.odl.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {de_bfs_odl_mqtt_station_data}")

async def handle_de_bfs_odl_mqtt_dose_rate_measurement(mqtt_msg,cloud_event, de_bfs_odl_mqtt_dose_rate_measurement_data):
    """ Handles the de.bfs.odl.mqtt.DoseRateMeasurement message """
    print(f"Received de.bfs.odl.mqtt.DoseRateMeasurement on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {de_bfs_odl_mqtt_dose_rate_measurement_data}")
