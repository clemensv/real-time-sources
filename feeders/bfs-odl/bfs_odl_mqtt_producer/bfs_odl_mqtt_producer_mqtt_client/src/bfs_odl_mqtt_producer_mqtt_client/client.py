...
def get_default_topic_mappings_de_bfs_odl_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the de.bfs.odl.mqtt message group.
...
def get_subscription_topics_de_bfs_odl_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_de_bfs_odl_mqtt()
    topics = []
...
class DeBfsOdlMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the de.bfs.odl.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_de_bfs_odl_mqtt()
        ...
        self.de_bfs_odl_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, bfs_odl_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.de_bfs_odl_mqtt_dose_rate_measurement_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, bfs_odl_mqtt_producer_data.DoseRateMeasurement, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "de.bfs.odl.Station":
            if self.de_bfs_odl_mqtt_station_async:
                try:
                    ...
                    data = bfs_odl_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "de.bfs.odl.mqtt.Station")
                    await self.de_bfs_odl_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_bfs_odl_mqtt_station handler: {e}")
            return
        
        if event_type == "de.bfs.odl.DoseRateMeasurement":
            if self.de_bfs_odl_mqtt_dose_rate_measurement_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = bfs_odl_mqtt_producer_data.DoseRateMeasurement.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "de.bfs.odl.mqtt.DoseRateMeasurement")
                    await self.de_bfs_odl_mqtt_dose_rate_measurement_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in de_bfs_odl_mqtt_dose_rate_measurement handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_de_bfs_odl_mqtt(self._topic_mappings)
        for topic in topics:
