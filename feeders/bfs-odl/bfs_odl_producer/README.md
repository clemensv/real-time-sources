...
# Bfs_odl_producer Kafka Producer# Bfs_odl_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - DeBfsOdlEventDispatcher
...
- DeBfsOdlProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### DeBfsOdlEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`DeBfsOdlEventDispatcher` handles events for the de.bfs.odl message group.
...
from bfs_odl_producer import DeBfsOdlProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = DeBfsOdlProducer(
...
await producer.send_de_bfs_odl_station(```python
...
The DeBfsOdlEventDispatcher defines the following event handler hooks.
...
producer = DeBfsOdlProducer(
    ...
    security_protocol='SASL_SSL',##### `de_bfs_odl_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'de_bfs_odl_station_async:  Callable[[ConsumerRecord, CloudEvent, Station],
Awaitable[None]]
...
Asynchronous handler hook for `de.bfs.odl.Station`: Metadata for an ODL (Ortsdosisleistung) measuring station in the BfS
gamma dose rate monitoring network. The BfS operates approximately 1,700 stationary probes across Germany that
continuously measure ambient gamma dose rate. Station metadata includes the geographic position, elevation above sea
level, postal code, and operational status of each probe.
...
### DeBfsOdlProducer- `data`: The event data of type `bfs_odl_producer_data.Station`.



Producer for `de.bfs.odl` message group.Example:



#### Constructor```python

async def de_bfs_odl_station_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Station) -> None:

```python    # Process the event data

DeBfsOdlProducer(    await some_processing_function(record, cloud_event, data)
...
de_bfs_odl_dispatcher.de_bfs_odl_station_async = de_bfs_odl_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `de_bfs_odl_dose_rate_measurement_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'de_bfs_odl_dose_rate_measurement_async:  Callable[[ConsumerRecord, CloudEvent,
DoseRateMeasurement], Awaitable[None]]

)```

```

Asynchronous handler hook for `de.bfs.odl.DoseRateMeasurement`: A one-hour averaged ambient gamma dose rate measurement
from the BfS ODL monitoring network. Each measurement reports the gross gamma dose rate in microsieverts per hour
(µSv/h), optionally split into cosmic and terrestrial components. The cosmic component originates from secondary cosmic
radiation and depends primarily on altitude; the terrestrial component originates from naturally occurring radionuclides
in the ground and varies with local geology.
...
### DeBfsOdlProducer- `data`: The event data of type `bfs_odl_producer_data.DoseRateMeasurement`.
...
async def de_bfs_odl_dose_rate_measurement_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
DoseRateMeasurement) -> None:
...
de_bfs_odl_dispatcher.de_bfs_odl_dose_rate_measurement_async = de_bfs_odl_dose_rate_measurement_event
...
##### `send_de_bfs_odl_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_de_bfs_odl_station(
...
Send a single `de.bfs.odl.Station` message. Metadata for an ODL (Ortsdosisleistung) measuring station in the BfS gamma
dose rate monitoring network. The BfS operates approximately 1,700 stationary probes across Germany that continuously
measure ambient gamma dose rate. Station metadata includes the geographic position, elevation above sea level, postal
code, and operational status of each probe.Args:
...
await producer.send_de_bfs_odl_station(
...
Send multiple `de.bfs.odl.Station` messages in a batch.
...
await producer.send_de_bfs_odl_station_batch(```
...
##### `send_de_bfs_odl_dose_rate_measurement`Dispatchers have the following protected methods:



```python### Methods:

async def send_de_bfs_odl_dose_rate_measurement(
...
Send a single `de.bfs.odl.DoseRateMeasurement` message. A one-hour averaged ambient gamma dose rate measurement from the
BfS ODL monitoring network. Each measurement reports the gross gamma dose rate in microsieverts per hour (µSv/h),
optionally split into cosmic and terrestrial components. The cosmic component originates from secondary cosmic radiation
and depends primarily on altitude; the terrestrial component originates from naturally occurring radionuclides in the
ground and varies with local geology.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `DoseRateMeasurement`
...
await producer.send_de_bfs_odl_dose_rate_measurement(

    data=DoseRateMeasurement(...),Args:
...
Send multiple `de.bfs.odl.DoseRateMeasurement` messages in a batch.
...
await producer.send_de_bfs_odl_dose_rate_measurement_batch(```

    messages=[

        DoseRateMeasurement(...),Initializes the runner with a Kafka consumer.

        DoseRateMeasurement(...),

        DoseRateMeasurement(...)Args:
