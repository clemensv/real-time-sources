...
def get_default_topic_mappings_sg_gov_nea_weather_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the SG.Gov.NEA.Weather.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "SG.Gov.NEA.Weather.Station.mqtt": "weather/sg/nea/singapore-nea/{region}/{station_id}/{event}",
        "SG.Gov.NEA.Weather.WeatherObservation.mqtt": "weather/sg/nea/singapore-nea/{region}/{station_id}/{event}",
    }


def get_subscription_topics_sg_gov_nea_weather_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_sg_gov_nea_weather_mqtt()
    topics = []
...
class SGGovNEAWeatherMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the SG.Gov.NEA.Weather.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_sg_gov_nea_weather_mqtt()
        ...
        self.sg_gov_nea_weather_station_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, singapore_nea_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.sg_gov_nea_weather_weather_observation_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, singapore_nea_mqtt_producer_data.WeatherObservation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "SG.Gov.NEA.Weather.Station":
            if self.sg_gov_nea_weather_station_mqtt_async:
                try:
                    ...
                    data = singapore_nea_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "SG.Gov.NEA.Weather.Station.mqtt")
                    await self.sg_gov_nea_weather_station_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in sg_gov_nea_weather_station_mqtt handler: {e}")
            return
        
        if event_type == "SG.Gov.NEA.Weather.WeatherObservation":
            if self.sg_gov_nea_weather_weather_observation_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = singapore_nea_mqtt_producer_data.WeatherObservation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "SG.Gov.NEA.Weather.WeatherObservation.mqtt")
                    await self.sg_gov_nea_weather_weather_observation_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in sg_gov_nea_weather_weather_observation_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_sg_gov_nea_weather_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_sg_gov_nea_weather_station_mqtt(self,
        station_id: str,
        region: str,
        event: str,
        data: singapore_nea_mqtt_producer_data.Station,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'SG.Gov.NEA.Weather.Station.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'weather/sg/nea/singapore-nea/{region}/{station_id}/{event}'
        ...
        """
        target_topic = topic if topic is not None else "weather/sg/nea/singapore-nea/{region}/{station_id}/{event}"
    ...
    async def publish_sg_gov_nea_weather_weather_observation_mqtt(self,
        station_id: str,
        region: str,
        event: str,
        data: singapore_nea_mqtt_producer_data.WeatherObservation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'SG.Gov.NEA.Weather.WeatherObservation.mqtt' event to an MQTT topic.
...
def get_default_topic_mappings_sg_gov_nea_airquality_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the SG.Gov.NEA.AirQuality.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "SG.Gov.NEA.AirQuality.Region.mqtt": "air-quality/sg/nea/singapore-nea/{region}/{station_id}/{event}",
        "SG.Gov.NEA.AirQuality.PSIReading.mqtt": "air-quality/sg/nea/singapore-nea/{region}/{station_id}/{event}",
        "SG.Gov.NEA.AirQuality.PM25Reading.mqtt": "air-quality/sg/nea/singapore-nea/{region}/{station_id}/{event}",
    }


def get_subscription_topics_sg_gov_nea_airquality_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_sg_gov_nea_airquality_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class SGGovNEAAirQualityMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the SG.Gov.NEA.AirQuality.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_sg_gov_nea_airquality_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        
        # Message handler callbacks (Dispatcher pattern)
        
        self.sg_gov_nea_air_quality_region_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, singapore_nea_mqtt_producer_data.Region, Dict[str, str]], Awaitable[None]]] = None
        
        self.sg_gov_nea_air_quality_psireading_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, singapore_nea_mqtt_producer_data.PSIReading, Dict[str, str]], Awaitable[None]]] = None
        
        self.sg_gov_nea_air_quality_pm25_reading_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, singapore_nea_mqtt_producer_data.PM25Reading, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "SG.Gov.NEA.AirQuality.Region":
            if self.sg_gov_nea_air_quality_region_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = singapore_nea_mqtt_producer_data.Region.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "SG.Gov.NEA.AirQuality.Region.mqtt")
                    await self.sg_gov_nea_air_quality_region_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in sg_gov_nea_air_quality_region_mqtt handler: {e}")
            return
        
        if event_type == "SG.Gov.NEA.AirQuality.PSIReading":
            if self.sg_gov_nea_air_quality_psireading_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = singapore_nea_mqtt_producer_data.PSIReading.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "SG.Gov.NEA.AirQuality.PSIReading.mqtt")
                    await self.sg_gov_nea_air_quality_psireading_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in sg_gov_nea_air_quality_psireading_mqtt handler: {e}")
            return
        
        if event_type == "SG.Gov.NEA.AirQuality.PM25Reading":
            if self.sg_gov_nea_air_quality_pm25_reading_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = singapore_nea_mqtt_producer_data.PM25Reading.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "SG.Gov.NEA.AirQuality.PM25Reading.mqtt")
                    await self.sg_gov_nea_air_quality_pm25_reading_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in sg_gov_nea_air_quality_pm25_reading_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_sg_gov_nea_airquality_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_sg_gov_nea_air_quality_region_mqtt(self,
        region: str,
        station_id: str,
        event: str,
        data: singapore_nea_mqtt_producer_data.Region,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'SG.Gov.NEA.AirQuality.Region.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            event: URI template variable for 'event'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'air-quality/sg/nea/singapore-nea/{region}/{station_id}/{event}'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "air-quality/sg/nea/singapore-nea/{region}/{station_id}/{event}"
        _topic_template_values: Dict[str, str] = {
            "region": str(region),
            "station_id": str(station_id),
            "event": str(event),
        ...
        attributes = {
             ...
             "source":"https://api.data.gov.sg",
             "subject":"{region}".format(region = region)
        }
        attributes["datacontenttype"] = content_type
        byte_data = data.to_byte_array(content_type) if data is not None else b''
        # to_byte_array returns str for text content types (e.g. JSON);
    ...
    async def publish_sg_gov_nea_air_quality_psireading_mqtt(self,
        region: str,
        station_id: str,
        event: str,
        data: singapore_nea_mqtt_producer_data.PSIReading,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'SG.Gov.NEA.AirQuality.PSIReading.mqtt' event to an MQTT topic.
    ...
    async def publish_sg_gov_nea_air_quality_pm25_reading_mqtt(self,
        region: str,
        station_id: str,
        event: str,
        data: singapore_nea_mqtt_producer_data.PM25Reading,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'SG.Gov.NEA.AirQuality.PM25Reading.mqtt' event to an MQTT topic.
