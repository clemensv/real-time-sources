...
|---|---|
| `AIRQUALITY_POLLING_INTERVAL` | Seconds between air-quality poll cycles (default `3600`). |
| `AIRQUALITY_TOPIC` | Kafka topic for air-quality events (default `singapore-nea-airquality`). |
| `MAX_SIZE` | Maximum number of records to fetch and emit per poll cycle (default `1000`). |
| `SINGAPORE_NEA_MOCK` | Emit one round of sample data and exit. |
| `STATION_FILTER` | Comma-separated station identifiers to include; empty includes all. |
