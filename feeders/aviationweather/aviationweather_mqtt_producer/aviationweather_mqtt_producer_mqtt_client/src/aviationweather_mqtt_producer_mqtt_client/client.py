...
def get_default_topic_mappings_gov_noaa_aviationweather_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the gov.noaa.aviationweather.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "gov.noaa.aviationweather.mqtt.Station": "aviation/intl/noaa/aviationweather/{icao_id}/info",
        "gov.noaa.aviationweather.mqtt.Metar": "aviation/intl/noaa/aviationweather/{icao_id}/metar",
        "gov.noaa.aviationweather.mqtt.Sigmet": "aviation/intl/noaa/aviationweather/sigmets/{region}/{sigmet_id}/sigmet",
    }


def get_subscription_topics_gov_noaa_aviationweather_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_gov_noaa_aviationweather_mqtt()
    topics = []
...
class GovNoaaAviationweatherMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the gov.noaa.aviationweather.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_gov_noaa_aviationweather_mqtt()
        ...
        self.gov_noaa_aviationweather_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, aviationweather_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.gov_noaa_aviationweather_mqtt_metar_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, aviationweather_mqtt_producer_data.Metar, Dict[str, str]], Awaitable[None]]] = None
        
        self.gov_noaa_aviationweather_mqtt_sigmet_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, aviationweather_mqtt_producer_data.Sigmet, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "gov.noaa.aviationweather.Station":
            if self.gov_noaa_aviationweather_mqtt_station_async:
                try:
                    ...
                    data = aviationweather_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "gov.noaa.aviationweather.mqtt.Station")
                    await self.gov_noaa_aviationweather_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in gov_noaa_aviationweather_mqtt_station handler: {e}")
            return
        
        if event_type == "gov.noaa.aviationweather.Metar":
            if self.gov_noaa_aviationweather_mqtt_metar_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = aviationweather_mqtt_producer_data.Metar.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "gov.noaa.aviationweather.mqtt.Metar")
                    await self.gov_noaa_aviationweather_mqtt_metar_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in gov_noaa_aviationweather_mqtt_metar handler: {e}")
            return
        
        if event_type == "gov.noaa.aviationweather.Sigmet":
            if self.gov_noaa_aviationweather_mqtt_sigmet_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = aviationweather_mqtt_producer_data.Sigmet.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "gov.noaa.aviationweather.mqtt.Sigmet")
                    await self.gov_noaa_aviationweather_mqtt_sigmet_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in gov_noaa_aviationweather_mqtt_sigmet handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_gov_noaa_aviationweather_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_gov_noaa_aviationweather_mqtt_station(self,
        icao_id: str,
        data: aviationweather_mqtt_producer_data.Station,
        ...
        """
        Publish the 'gov.noaa.aviationweather.mqtt.Station' event to an MQTT topic.

        Args:
        
            icao_id: URI template variable for 'icao_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'aviation/intl/noaa/aviationweather/{icao_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "aviation/intl/noaa/aviationweather/{icao_id}/info"
        ...
        attributes = {
             ...
             "subject":"{icao_id}".format(icao_id = icao_id)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_gov_noaa_aviationweather_mqtt_metar(self,
        icao_id: str,
        data: aviationweather_mqtt_producer_data.Metar,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'gov.noaa.aviationweather.mqtt.Metar' event to an MQTT topic.

        Args:
        
            icao_id: URI template variable for 'icao_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'aviation/intl/noaa/aviationweather/{icao_id}/metar'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "aviation/intl/noaa/aviationweather/{icao_id}/metar"
        _topic_template_values: Dict[str, str] = {
            "icao_id": str(icao_id),
        }
        if _topic_template_values:
    ...
    async def publish_gov_noaa_aviationweather_mqtt_sigmet(self,
        region: str,
        sigmet_id: str,
        data: aviationweather_mqtt_producer_data.Sigmet,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'gov.noaa.aviationweather.mqtt.Sigmet' event to an MQTT topic.

        Args:
        
            region: URI template variable for 'region'
            sigmet_id: URI template variable for 'sigmet_id'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'aviation/intl/noaa/aviationweather/sigmets/{region}/{sigmet_id}/sigmet'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            ...
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "aviation/intl/noaa/aviationweather/sigmets/{region}/{sigmet_id}/sigmet"
        _topic_template_values: Dict[str, str] = {
            "region": str(region),
            "sigmet_id": str(sigmet_id),
        }
        if _topic_template_values:
            target_topic = _apply_topic_template(target_topic, _topic_template_values)
        ...
        attributes = {
             ...
             "source":"https://aviationweather.gov",
             "subject":"{region}/{sigmet_id}".format(region = region,sigmet_id = sigmet_id)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
