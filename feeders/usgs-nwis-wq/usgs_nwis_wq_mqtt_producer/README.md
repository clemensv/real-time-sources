# Usgs_nwis_wq_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the usgs_nwis_wq_mqtt_producer message definitions.

# Usgs_nwis_wq_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - USGSWaterQualitySitesMqttEventDispatcher,

    USGSWaterQualityReadingsMqttEventDispatcher
...
from usgs_nwis_wq_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = USGSWaterQualitySitesMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### USGSWaterQualitySitesMqttEventDispatcher



# Publish message`USGSWaterQualitySitesMqttEventDispatcher` handles events for the USGS.WaterQuality.Sites.mqtt message
group.
...
from usgs_nwis_wq_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = USGSWaterQualitySitesMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
client = USGSWaterQualityReadingsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of
...
## BuildThe USGSWaterQualitySitesMqttEventDispatcher defines the following event handler hooks.
...
```##### `usgs_water_quality_sites_mqtt_monitoring_site_async`



## Test```python

usgs_water_quality_sites_mqtt_monitoring_site_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `data`: The event data of type `usgs_nwis_wq_mqtt_producer_data.MonitoringSite`.

Example:

```python
async def usgs_water_quality_sites_mqtt_monitoring_site_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: MonitoringSite) -> None:
...
client = USGSWaterQualityReadingsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### USGSWaterQualityReadingsMqttEventDispatcher



# Publish message`USGSWaterQualityReadingsMqttEventDispatcher` handles events for the USGS.WaterQuality.Readings.mqtt
message group.
...
## BuildThe USGSWaterQualityReadingsMqttEventDispatcher defines the following event handler hooks.
...
```##### `usgs_water_quality_readings_mqtt_water_quality_reading_async`



## Test```python

usgs_water_quality_readings_mqtt_water_quality_reading_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```Asynchronous handler hook for `USGS.WaterQuality.Readings.mqtt.WaterQualityReading`: A current measurement from the
U.S. Geological Survey (USGS) Water Services API for one monitoring site. It carries continuous water-quality sensor
readings when the upstream feed reports a new or refreshed value.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `usgs_nwis_wq_mqtt_producer_data.WaterQualityReading`.

Example:

```python
async def usgs_water_quality_readings_mqtt_water_quality_reading_event(partition_context: PartitionContext, event:
