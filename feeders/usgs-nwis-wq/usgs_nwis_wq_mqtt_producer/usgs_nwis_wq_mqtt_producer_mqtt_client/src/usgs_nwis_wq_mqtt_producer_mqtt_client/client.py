...
def get_default_topic_mappings_usgs_waterquality_sites_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the USGS.WaterQuality.Sites.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "USGS.WaterQuality.Sites.mqtt.MonitoringSite": "hydro/us/usgs/usgs-nwis-wq/{state}/{site_number}/info",
    }


def get_subscription_topics_usgs_waterquality_sites_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_usgs_waterquality_sites_mqtt()
    topics = []
...
class USGSWaterQualitySitesMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the USGS.WaterQuality.Sites.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_usgs_waterquality_sites_mqtt()
        ...
        self.usgs_water_quality_sites_mqtt_monitoring_site_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, usgs_nwis_wq_mqtt_producer_data.MonitoringSite, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "USGS.WaterQuality.Sites.MonitoringSite":
            if self.usgs_water_quality_sites_mqtt_monitoring_site_async:
                try:
                    ...
                    data = usgs_nwis_wq_mqtt_producer_data.MonitoringSite.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "USGS.WaterQuality.Sites.mqtt.MonitoringSite")
                    await self.usgs_water_quality_sites_mqtt_monitoring_site_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in usgs_water_quality_sites_mqtt_monitoring_site handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_usgs_waterquality_sites_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_usgs_water_quality_sites_mqtt_monitoring_site(self,
        source_uri: str,
        site_number: str,
        state: str,
        data: usgs_nwis_wq_mqtt_producer_data.MonitoringSite,
        ...
        """
        Publish the 'USGS.WaterQuality.Sites.mqtt.MonitoringSite' event to an MQTT topic.

        Args:
        
            source_uri: URI template variable for 'source_uri'
            site_number: URI template variable for 'site_number'
            state: URI template variable for 'state'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/us/usgs/usgs-nwis-wq/{state}/{site_number}/info'
        ...
        """
        target_topic = topic if topic is not None else "hydro/us/usgs/usgs-nwis-wq/{state}/{site_number}/info"
        ...
        attributes = {
             "type":"USGS.WaterQuality.Sites.MonitoringSite",
             "source":"{source_uri}".format(source_uri = source_uri),
             "subject":"{site_number}".format(site_number = site_number)
        }
        attributes["datacontenttype"] = content_type
...
def get_default_topic_mappings_usgs_waterquality_readings_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the USGS.WaterQuality.Readings.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "USGS.WaterQuality.Readings.mqtt.WaterQualityReading": "hydro/us/usgs/usgs-nwis-wq/{state}/{site_number}/{parameter_code}/water-quality",
    }


def get_subscription_topics_usgs_waterquality_readings_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_usgs_waterquality_readings_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class USGSWaterQualityReadingsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the USGS.WaterQuality.Readings.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_usgs_waterquality_readings_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        self._connect_waiter: Optional[asyncio.Future[None]] = None
        self._connected = False
        ...
        self.usgs_water_quality_readings_mqtt_water_quality_reading_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, usgs_nwis_wq_mqtt_producer_data.WaterQualityReading, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "USGS.WaterQuality.Readings.WaterQualityReading":
            if self.usgs_water_quality_readings_mqtt_water_quality_reading_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = usgs_nwis_wq_mqtt_producer_data.WaterQualityReading.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "USGS.WaterQuality.Readings.mqtt.WaterQualityReading")
                    await self.usgs_water_quality_readings_mqtt_water_quality_reading_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in usgs_water_quality_readings_mqtt_water_quality_reading handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_usgs_waterquality_readings_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_usgs_water_quality_readings_mqtt_water_quality_reading(self,
        source_uri: str,
        site_number: str,
        parameter_code: str,
        state: str,
        data: usgs_nwis_wq_mqtt_producer_data.WaterQualityReading,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'USGS.WaterQuality.Readings.mqtt.WaterQualityReading' event to an MQTT topic.

        Args:
        
            source_uri: URI template variable for 'source_uri'
            site_number: URI template variable for 'site_number'
            parameter_code: URI template variable for 'parameter_code'
            state: URI template variable for 'state'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/us/usgs/usgs-nwis-wq/{state}/{site_number}/{parameter_code}/water-quality'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "hydro/us/usgs/usgs-nwis-wq/{state}/{site_number}/{parameter_code}/water-quality"
        _topic_template_values: Dict[str, str] = {
            "source_uri": str(source_uri),
            "site_number": str(site_number),
        ...
        attributes = {
             "type":"USGS.WaterQuality.Readings.WaterQualityReading",
             "source":"{source_uri}".format(source_uri = source_uri),
             "subject":"{site_number}/{parameter_code}".format(site_number = site_number,parameter_code = parameter_code)
        }
        attributes["datacontenttype"] = content_type
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        byte_data = data.to_byte_array(content_type) if data is not None else b''
