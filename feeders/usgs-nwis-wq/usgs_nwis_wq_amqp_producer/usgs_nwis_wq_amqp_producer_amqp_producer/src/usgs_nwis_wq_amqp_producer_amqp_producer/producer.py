...
class USGSWaterQualitySitesAmqpProducer:
    """
    Producer class to send messages in the `USGS.WaterQuality.Sites.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_monitoring_site(self,
        ...
        """
        Send the `USGS.WaterQuality.Sites.amqp.MonitoringSite` message
        ...
        attributes = {
            ...
            "subject":
            "{site_number}".format(site_number=_site_number),
        }
        ...
        amqp_msg.subject = "{site_number}".format(site_number=_site_number)
        ...
        annotations = {}
        annotation_value = "{site_number}".format(site_number=_site_number)
    ...
    def send_monitoring_site_batch(self,
        data_array: typing.List[MonitoringSite],
        _source_uri: str,
        _site_number: str,
        _state: str,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `USGS.WaterQuality.Sites.amqp.MonitoringSite` messages
        
        Args:
            data_array (typing.List[MonitoringSite]): Array of message data objects
            _source_uri (str): Value for placeholder source_uri in attribute source
            _site_number (str): Value for placeholder site_number in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
            _state (str): Value for AMQP protocol option placeholder state
        ...
        for data in data_array:
            self.send_monitoring_site(
                data=data,
                _source_uri=_source_uri,
                _site_number=_site_number,
                _time=_time,
                _state=_state,
...
class USGSWaterQualityReadingsAmqpProducer:
    """
    Producer class to send messages in the `USGS.WaterQuality.Readings.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_water_quality_reading(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `USGS.WaterQuality.Readings.amqp.WaterQualityReading` message
        ...
        attributes = {
            ...
            "{source_uri}".format(source_uri=_source_uri),
            "subject":
            "{site_number}/{parameter_code}".format(site_number=_site_number, parameter_code=_parameter_code),
        }
        attributes["time"] = _resolve_cloudevents_time(_time, attributes.get("time"))
        ...
        if amqp_creation_time is not None:
            amqp_msg.creation_time = amqp_creation_time
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "{site_number}/{parameter_code}".format(site_number=_site_number, parameter_code=_parameter_code)

        app_properties = {}
        app_properties["state"] = "{state}".format(state=_state)
        app_properties["parameter_code"] = "{parameter_code}".format(parameter_code=_parameter_code)
        if app_properties:
            if amqp_msg.properties is None:
                amqp_msg.properties = {}
            amqp_msg.properties.update(app_properties)
        ...
        annotations = {}
        annotation_value = "{site_number}/{parameter_code}".format(site_number=_site_number, parameter_code=_parameter_code)
        annotation_value = str(annotation_value)[:128]
        annotations[symbol("x-opt-partition-key")] = annotation_value
        if annotations:
            if amqp_msg.annotations is None:
    ...
    def send_water_quality_reading_batch(self,
        data_array: typing.List[WaterQualityReading],
        _source_uri: str,
        _site_number: str,
        _parameter_code: str,
        _state: str,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `USGS.WaterQuality.Readings.amqp.WaterQualityReading` messages
        
        Args:
            data_array (typing.List[WaterQualityReading]): Array of message data objects
            _source_uri (str): Value for placeholder source_uri in attribute source
            _site_number (str): Value for placeholder site_number in attribute subject
            _parameter_code (str): Value for placeholder parameter_code in attribute subject
            _time (typing.Optional[typing.Union[str, datetime]]): CloudEvents time override. Defaults to current UTC when no catalog time is used.
        ...
        """
        for data in data_array:
            self.send_water_quality_reading(
                data=data,
                _source_uri=_source_uri,
                _site_number=_site_number,
                _parameter_code=_parameter_code,
                _time=_time,
                _state=_state,
                content_type=content_type)
