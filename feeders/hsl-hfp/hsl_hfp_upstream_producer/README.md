# Hsl_hfp_upstream MQTT Client

This library provides MQTT producer and consumer functionality for the hsl_hfp_upstream message definitions.

# Hsl_hfp_upstream Event Dispatcher for Azure Event Hubs
...
```    - FiHslHfpUpstreamEventDispatcher
...
from hsl_hfp_upstream_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = FiHslHfpUpstreamMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### FiHslHfpUpstreamEventDispatcher



# Publish message`FiHslHfpUpstreamEventDispatcher` handles events for the fi.hsl.hfp.upstream message group.
...
from hsl_hfp_upstream_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = FiHslHfpUpstreamMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe FiHslHfpUpstreamEventDispatcher defines the following event handler hooks.
...
```##### `fi_hsl_hfp_upstream_vp_async`



## Test```python

fi_hsl_hfp_upstream_vp_async:  Callable[[PartitionContext, EventData, CloudEvent, VehicleEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.vp`: Upstream HFP `vp` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `VP` key. Vehicle position
— the ~1 Hz GPS heartbeat of a vehicle on an ongoing public journey.
...
- `data`: The event data of type `hsl_hfp_upstream_data.VehicleEvent`.

Example:

```python
async def fi_hsl_hfp_upstream_vp_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: VehicleEvent) -> None:
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_vp_async = fi_hsl_hfp_upstream_vp_event
```
...
```##### `fi_hsl_hfp_upstream_due_async`



## Test```python

fi_hsl_hfp_upstream_due_async:  Callable[[PartitionContext, EventData, CloudEvent, VehicleEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.due`: Upstream HFP `due` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `DUE` key. The vehicle will
soon arrive at a stop.
...
```python
async def fi_hsl_hfp_upstream_due_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: VehicleEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_due_async = fi_hsl_hfp_upstream_due_event
```
...
```##### `fi_hsl_hfp_upstream_arr_async`



## Test```python

fi_hsl_hfp_upstream_arr_async:  Callable[[PartitionContext, EventData, CloudEvent, VehicleEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.arr`: Upstream HFP `arr` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `ARR` key. The vehicle has
arrived inside a stop's radius.
...
```python
async def fi_hsl_hfp_upstream_arr_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: VehicleEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_arr_async = fi_hsl_hfp_upstream_arr_event
```
...
```##### `fi_hsl_hfp_upstream_dep_async`



## Test```python

fi_hsl_hfp_upstream_dep_async:  Callable[[PartitionContext, EventData, CloudEvent, VehicleEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.dep`: Upstream HFP `dep` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `DEP` key. The vehicle has
departed and left a stop's radius.
...
```python
async def fi_hsl_hfp_upstream_dep_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: VehicleEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_dep_async = fi_hsl_hfp_upstream_dep_event
```
...
```##### `fi_hsl_hfp_upstream_ars_async`



## Test```python

fi_hsl_hfp_upstream_ars_async:  Callable[[PartitionContext, EventData, CloudEvent, VehicleEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.ars`: Upstream HFP `ars` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `ARS` key. The vehicle has
arrived at a stop (stop-position event).
...
```python
async def fi_hsl_hfp_upstream_ars_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: VehicleEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_ars_async = fi_hsl_hfp_upstream_ars_event
```
...
```##### `fi_hsl_hfp_upstream_pde_async`



## Test```python

fi_hsl_hfp_upstream_pde_async:  Callable[[PartitionContext, EventData, CloudEvent, VehicleEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.pde`: Upstream HFP `pde` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `PDE` key. The vehicle is
ready to depart from a stop.
...
```python
async def fi_hsl_hfp_upstream_pde_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: VehicleEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_pde_async = fi_hsl_hfp_upstream_pde_event
```
...
```##### `fi_hsl_hfp_upstream_pas_async`



## Test```python

fi_hsl_hfp_upstream_pas_async:  Callable[[PartitionContext, EventData, CloudEvent, VehicleEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.pas`: Upstream HFP `pas` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `PAS` key. The vehicle
passes through a stop without stopping.
...
```python
async def fi_hsl_hfp_upstream_pas_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: VehicleEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_pas_async = fi_hsl_hfp_upstream_pas_event
```
...
```##### `fi_hsl_hfp_upstream_wait_async`



## Test```python

fi_hsl_hfp_upstream_wait_async:  Callable[[PartitionContext, EventData, CloudEvent, VehicleEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.wait`: Upstream HFP `wait` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `WAIT` key. The vehicle is
waiting at a stop.
...
```python
async def fi_hsl_hfp_upstream_wait_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: VehicleEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_wait_async = fi_hsl_hfp_upstream_wait_event
```
...
```##### `fi_hsl_hfp_upstream_doo_async`



## Test```python

fi_hsl_hfp_upstream_doo_async:  Callable[[PartitionContext, EventData, CloudEvent, VehicleEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.doo`: Upstream HFP `doo` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `DOO` key. The doors of the
vehicle have been opened.
...
```python
async def fi_hsl_hfp_upstream_doo_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: VehicleEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_doo_async = fi_hsl_hfp_upstream_doo_event
```
...
```##### `fi_hsl_hfp_upstream_doc_async`



## Test```python

fi_hsl_hfp_upstream_doc_async:  Callable[[PartitionContext, EventData, CloudEvent, VehicleEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.doc`: Upstream HFP `doc` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `DOC` key. The doors of the
vehicle have been closed.
...
```python
async def fi_hsl_hfp_upstream_doc_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: VehicleEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_doc_async = fi_hsl_hfp_upstream_doc_event
```
...
```##### `fi_hsl_hfp_upstream_vja_async`



## Test```python

fi_hsl_hfp_upstream_vja_async:  Callable[[PartitionContext, EventData, CloudEvent, VehicleEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.vja`: Upstream HFP `vja` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `VJA` key. The vehicle
signs in to a service journey (trip).
...
```python
async def fi_hsl_hfp_upstream_vja_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: VehicleEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_vja_async = fi_hsl_hfp_upstream_vja_event
```
...
```##### `fi_hsl_hfp_upstream_vjout_async`



## Test```python

fi_hsl_hfp_upstream_vjout_async:  Callable[[PartitionContext, EventData, CloudEvent, VehicleEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.vjout`: Upstream HFP `vjout` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `VJOUT` key. The vehicle
signs off from a service journey after the final stop.
...
```python
async def fi_hsl_hfp_upstream_vjout_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: VehicleEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_vjout_async = fi_hsl_hfp_upstream_vjout_event
```
...
```##### `fi_hsl_hfp_upstream_tlr_async`



## Test```python

fi_hsl_hfp_upstream_tlr_async:  Callable[[PartitionContext, EventData, CloudEvent, TrafficLightEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.tlr`: Upstream HFP `tlr` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `TLR` key. The vehicle
requests traffic-light priority at a junction.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `hsl_hfp_upstream_data.TrafficLightEvent`.

Example:

```python
async def fi_hsl_hfp_upstream_tlr_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: TrafficLightEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_tlr_async = fi_hsl_hfp_upstream_tlr_event
```
...
```##### `fi_hsl_hfp_upstream_tla_async`



## Test```python

fi_hsl_hfp_upstream_tla_async:  Callable[[PartitionContext, EventData, CloudEvent, TrafficLightEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.tla`: Upstream HFP `tla` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `TLA` key. The vehicle
receives a response to a traffic-light priority request.
...
```python
async def fi_hsl_hfp_upstream_tla_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: TrafficLightEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_tla_async = fi_hsl_hfp_upstream_tla_event
```
...
```##### `fi_hsl_hfp_upstream_da_async`



## Test```python

fi_hsl_hfp_upstream_da_async:  Callable[[PartitionContext, EventData, CloudEvent, DriverBlockEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.da`: Upstream HFP `da` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `DA` key. A driver signs in
to the vehicle.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `hsl_hfp_upstream_data.DriverBlockEvent`.

Example:

```python
async def fi_hsl_hfp_upstream_da_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: DriverBlockEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_da_async = fi_hsl_hfp_upstream_da_event
```
...
```##### `fi_hsl_hfp_upstream_dout_async`



## Test```python

fi_hsl_hfp_upstream_dout_async:  Callable[[PartitionContext, EventData, CloudEvent, DriverBlockEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.dout`: Upstream HFP `dout` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `DOUT` key. A driver signs
out of the vehicle.
...
```python
async def fi_hsl_hfp_upstream_dout_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: DriverBlockEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_dout_async = fi_hsl_hfp_upstream_dout_event
```
...
```##### `fi_hsl_hfp_upstream_ba_async`



## Test```python

fi_hsl_hfp_upstream_ba_async:  Callable[[PartitionContext, EventData, CloudEvent, DriverBlockEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.ba`: Upstream HFP `ba` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `BA` key. A driver selects
the block the vehicle will run.
...
```python
async def fi_hsl_hfp_upstream_ba_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: DriverBlockEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_ba_async = fi_hsl_hfp_upstream_ba_event
```
...
```##### `fi_hsl_hfp_upstream_bout_async`



## Test```python

fi_hsl_hfp_upstream_bout_async:  Callable[[PartitionContext, EventData, CloudEvent, DriverBlockEvent], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `fi.hsl.hfp.upstream.bout`: Upstream HFP `bout` message as published by HSL on
`mqtt.hsl.fi` — plain JSON (no CloudEvents envelope), with the event object under the single `BOUT` key. A driver signs
out from the selected block (usually at a depot).
...
```python
async def fi_hsl_hfp_upstream_bout_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: DriverBlockEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
fi_hsl_hfp_upstream_dispatcher.fi_hsl_hfp_upstream_bout_async = fi_hsl_hfp_upstream_bout_event
```
