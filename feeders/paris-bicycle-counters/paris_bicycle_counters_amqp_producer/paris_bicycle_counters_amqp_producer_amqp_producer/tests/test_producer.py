...
"""
Tests for paris_bicycle_counters_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../paris_bicycle_counters_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../paris_bicycle_counters_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../paris_bicycle_counters_amqp_producer_amqp_producer/src')))

from paris_bicycle_counters_amqp_producer_amqp_producer import *
from paris_bicycle_counters_amqp_producer_data import Counter
from test_counter import Test_Counter
from paris_bicycle_counters_amqp_producer_data import BicycleCount
...
class TestFRParisOpenDataVeloAmqpProducer:
    """Test cases for FRParisOpenDataVeloAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(FRParisOpenDataVeloAmqpProducer)
    ...
    def test_send_counter(self, artemis_container):
        """Send and receive a Counter message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Counter.create_instance()
    ...
    def test_send_counter_single_fresh_connection(self, artemis_container):
        """Send exactly one Counter message on a fresh producer connection."""
        payload = Test_Counter.create_instance()
    ...
    def test_send_bicycle_count(self, artemis_container):
        """Send and receive a BicycleCount message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_BicycleCount.create_instance()
    ...
    def test_send_bicycle_count_single_fresh_connection(self, artemis_container):
        """Send exactly one BicycleCount message on a fresh producer connection."""
        payload = Test_BicycleCount.create_instance()
