...
async def main(connection_string: Optional[str], producer_config: Optional[str], topic: Optional[str]):
    ...
    await frparis_open_data_velo_event_producer.send_fr_paris_open_data_velo_bicycle_count(_counter_id = 'TODO: replace me', _ce_id = 'TODO: replace me', data = _bicycle_count)
    ...
    await frparis_open_data_velo_mqtt_event_producer.send_fr_paris_open_data_velo_mqtt_bicycle_count(_counter_id = 'TODO: replace me', _ce_id = 'TODO: replace me', data = _bicycle_count)
    print(f"Sent 'FR.Paris.OpenData.Velo.mqtt.BicycleCount' event: {_bicycle_count.to_json()}")
    if connection_string:
        # use a connection string obtained for an Event Stream from the Microsoft Fabric portal
        # or an Azure Event Hubs connection string
        frparis_open_data_velo_amqp_event_producer = FRParisOpenDataVeloAmqpEventProducer.from_connection_string(connection_string, topic, 'binary')
    else:
        # use a Kafka producer configuration provided as JSON text
        kafka_producer = KafkaProducer(json.loads(producer_config))
        frparis_open_data_velo_amqp_event_producer = FRParisOpenDataVeloAmqpEventProducer(kafka_producer, topic, 'binary')

    # ---- FR.Paris.OpenData.Velo.amqp.Counter ----
    # TODO: Supply event data for the FR.Paris.OpenData.Velo.amqp.Counter event
    _counter = Counter()

    # sends the 'FR.Paris.OpenData.Velo.amqp.Counter' event to Kafka topic.
    await frparis_open_data_velo_amqp_event_producer.send_fr_paris_open_data_velo_amqp_counter(_counter_id = 'TODO: replace me', _ce_id = 'TODO: replace me', data = _counter)
    print(f"Sent 'FR.Paris.OpenData.Velo.amqp.Counter' event: {_counter.to_json()}")

    # ---- FR.Paris.OpenData.Velo.amqp.BicycleCount ----
    # TODO: Supply event data for the FR.Paris.OpenData.Velo.amqp.BicycleCount event
    _bicycle_count = BicycleCount()

    # sends the 'FR.Paris.OpenData.Velo.amqp.BicycleCount' event to Kafka topic.
    await frparis_open_data_velo_amqp_event_producer.send_fr_paris_open_data_velo_amqp_bicycle_count(_counter_id = 'TODO: replace me', _ce_id = 'TODO: replace me', data = _bicycle_count)
    print(f"Sent 'FR.Paris.OpenData.Velo.amqp.BicycleCount' event: {_bicycle_count.to_json()}")
