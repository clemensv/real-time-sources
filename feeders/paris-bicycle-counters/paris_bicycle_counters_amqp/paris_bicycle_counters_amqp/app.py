"""AMQP 1.0 companion feeder for paris-bicycle-counters."""
...
from paris_bicycle_counters_amqp_producer_amqp_producer.producer import FRParisOpenDataVeloAmqpProducer
from paris_bicycle_counters_core import ParisBicycleCounterPoller, ce_datetime

DEFAULT_ENTRA_AUDIENCE_SERVICEBUS = "https://servicebus.azure.net/.default"
SOURCE_ID = "paris-bicycle-counters"
PY_MODULE = "paris_bicycle_counters"
...
def _build_amqp_producer(args):
    ...
    return _apply_partition_key_workaround(FRParisOpenDataVeloAmqpProducer(**kwargs))


async def _run_live(args: argparse.Namespace, producer: FRParisOpenDataVeloAmqpProducer) -> None:
    """Acquire live Paris bicycle counter data and publish via AMQP."""
    poller = ParisBicycleCounterPoller(last_polled_file=args.state_file)
    while True:
        ...
        logger.info("Published %d counter info events", len(counters))

        state = poller.load_state()
        seen_keys = set(state.get("seen_keys", []))
        since = datetime.now(timezone.utc) - timedelta(days=7)
        counts = poller.fetch_bicycle_counts(since=since)
        ...
        logger.info("Published %d new count events via AMQP", len(new_counts))
...
def _add_common_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--mock-mode", action="store_true", default=_env_bool("PARIS_BICYCLE_COUNTERS_MOCK", False))
    return parser
...
async def _async_main(args: argparse.Namespace) -> None:
    ...
    try:
        if args.mock_mode:
            logger.warning("Mock mode is not supported for %s; acquiring live data instead", SOURCE_ID)
