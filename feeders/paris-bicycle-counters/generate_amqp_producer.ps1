...
$outputDir = Join-Path $scriptDir "paris_bicycle_counters_amqp_producer"
if (Test-Path $outputDir) { Remove-Item -Path $outputDir -Recurse -Force }
xrcg generate `
    --style amqpproducer `
    --language py `
    --definitions $xregFile `
    --endpoint FR.Paris.OpenData.Velo.Amqp `
    --projectname paris_bicycle_counters_amqp_producer `
