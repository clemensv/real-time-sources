# Kmi_belgium_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the kmi_belgium_mqtt_producer message definitions.

# Kmi_belgium_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - BEGovKMIWeatherMqttEventDispatcher
...
from kmi_belgium_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = BEGovKMIWeatherMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### BEGovKMIWeatherMqttEventDispatcher



# Publish message`BEGovKMIWeatherMqttEventDispatcher` handles events for the BE.Gov.KMI.Weather.mqtt message group.
...
from kmi_belgium_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = BEGovKMIWeatherMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe BEGovKMIWeatherMqttEventDispatcher defines the following event handler hooks.
...
```##### `be_gov_kmi_weather_mqtt_station_async`



## Test```python

be_gov_kmi_weather_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `kmi_belgium_mqtt_producer_data.Station`.

Example:

```python
async def be_gov_kmi_weather_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `be_gov_kmi_weather_mqtt_weather_observation_async`



## Test```python

be_gov_kmi_weather_mqtt_weather_observation_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```Asynchronous handler hook for `BE.Gov.KMI.Weather.mqtt.WeatherObservation`: A current environmental measurement from
the Royal Meteorological Institute of Belgium (KMI/IRM). It carries weather observations when the upstream feed reports
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `kmi_belgium_mqtt_producer_data.WeatherObservation`.

Example:

```python
async def be_gov_kmi_weather_mqtt_weather_observation_event(partition_context: PartitionContext, event: EventData,
