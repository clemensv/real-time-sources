...
def get_default_topic_mappings_be_gov_kmi_weather_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the BE.Gov.KMI.Weather.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "BE.Gov.KMI.Weather.mqtt.Station": "weather/be/kmi/kmi-belgium/{region}/{station_code}/info",
        "BE.Gov.KMI.Weather.mqtt.WeatherObservation": "weather/be/kmi/kmi-belgium/{region}/{station_code}/observation",
    }


def get_subscription_topics_be_gov_kmi_weather_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_be_gov_kmi_weather_mqtt()
    topics = []
...
class BEGovKMIWeatherMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the BE.Gov.KMI.Weather.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_be_gov_kmi_weather_mqtt()
        ...
        self.be_gov_kmi_weather_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, kmi_belgium_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.be_gov_kmi_weather_mqtt_weather_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, kmi_belgium_mqtt_producer_data.WeatherObservation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "BE.Gov.KMI.Weather.Station":
            if self.be_gov_kmi_weather_mqtt_station_async:
                try:
                    ...
                    data = kmi_belgium_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "BE.Gov.KMI.Weather.mqtt.Station")
                    await self.be_gov_kmi_weather_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in be_gov_kmi_weather_mqtt_station handler: {e}")
            return
        
        if event_type == "BE.Gov.KMI.Weather.WeatherObservation":
            if self.be_gov_kmi_weather_mqtt_weather_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = kmi_belgium_mqtt_producer_data.WeatherObservation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "BE.Gov.KMI.Weather.mqtt.WeatherObservation")
                    await self.be_gov_kmi_weather_mqtt_weather_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in be_gov_kmi_weather_mqtt_weather_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_be_gov_kmi_weather_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_be_gov_kmi_weather_mqtt_station(self,
        station_code: str,
        region: str,
        data: kmi_belgium_mqtt_producer_data.Station,
        ...
        """
        Publish the 'BE.Gov.KMI.Weather.mqtt.Station' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'weather/be/kmi/kmi-belgium/{region}/{station_code}/info'
        ...
        """
        target_topic = topic if topic is not None else "weather/be/kmi/kmi-belgium/{region}/{station_code}/info"
        ...
        attributes = {
             "type":"BE.Gov.KMI.Weather.Station",
             "source":"https://opendata.meteo.be",
             "subject":"{station_code}".format(station_code = station_code)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_be_gov_kmi_weather_mqtt_weather_observation(self,
        station_code: str,
        region: str,
        data: kmi_belgium_mqtt_producer_data.WeatherObservation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        _time: typing.Optional[typing.Union[str, datetime]] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'BE.Gov.KMI.Weather.mqtt.WeatherObservation' event to an MQTT topic.
        ...
        Args:
            ...
            region: URI template variable for 'region'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/be/kmi/kmi-belgium/{region}/{station_code}/observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            _time: Optional CloudEvents time override. Defaults to current UTC when no catalog time is used.
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "weather/be/kmi/kmi-belgium/{region}/{station_code}/observation"
        _topic_template_values: Dict[str, str] = {
            "station_code": str(station_code),
            "region": str(region),
        }
        ...
        attributes = {
             "type":"BE.Gov.KMI.Weather.WeatherObservation",
             "source":"https://opendata.meteo.be",
             "subject":"{station_code}".format(station_code = station_code)
        }
        attributes["datacontenttype"] = content_type
