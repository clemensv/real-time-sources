...
"""
Tests for kmi_belgium_amqp_producer_amqp_producer
"""
import base64
import datetime
import json
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../kmi_belgium_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../kmi_belgium_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../kmi_belgium_amqp_producer_amqp_producer/src')))

from kmi_belgium_amqp_producer_amqp_producer import *
from kmi_belgium_amqp_producer_data import Station
from test_station import Test_Station
from kmi_belgium_amqp_producer_data import WeatherObservation
...
class TestBEGovKMIWeatherAmqpProducer:
    """Test cases for BEGovKMIWeatherAmqpProducer"""
    ...
    def test_presettled_send_waits_for_queued_delivery_to_drain(self):
        ...
        producer = object.__new__(BEGovKMIWeatherAmqpProducer)
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "BE.Gov.KMI.Weather.amqp.Station"
    ...
    def test_send_station_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'BE.Gov.KMI.Weather.amqp.Station'
        assert received.body is not None
        assert received.subject == "{station_code}".format(station_code="value")
        assert properties.get('region') == "{region}".format(region="value")
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "BE.Gov.KMI.Weather.amqp.WeatherObservation"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_weather_observation_single_fresh_connection(self, artemis_container):
        ...
        properties = received.properties or {}
        annotations = received.annotations or {}
        assert properties.get('cloudEvents:type') == 'BE.Gov.KMI.Weather.amqp.WeatherObservation'
        assert received.body is not None
        assert received.subject == "{station_code}".format(station_code="value")
        assert properties.get('region') == "{region}".format(region="value")
