...
import os
import signal
from tepco_denkiyoho_mqtt_producer_data import *
from tepco_denkiyoho_mqtt_producer_mqtt_client.client import JPTEPCODenkiyohoMqttProducer, JPTEPCODenkiyohoMqttDispatcher

async def handle_jp_tepco_denkiyoho_mqtt_supply_capacity(mqtt_msg,cloud_event, jp_tepco_denkiyoho_mqtt_supply_capacity_data):
    """ Handles the JP.TEPCO.Denkiyoho.mqtt.SupplyCapacity message """
    print(f"Received JP.TEPCO.Denkiyoho.mqtt.SupplyCapacity on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_tepco_denkiyoho_mqtt_supply_capacity_data}")

async def handle_jp_tepco_denkiyoho_mqtt_peak_demand_forecast(mqtt_msg,cloud_event, jp_tepco_denkiyoho_mqtt_peak_demand_forecast_data):
    """ Handles the JP.TEPCO.Denkiyoho.mqtt.PeakDemandForecast message """
    print(f"Received JP.TEPCO.Denkiyoho.mqtt.PeakDemandForecast on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_tepco_denkiyoho_mqtt_peak_demand_forecast_data}")

async def handle_jp_tepco_denkiyoho_mqtt_demand_actual(mqtt_msg,cloud_event, jp_tepco_denkiyoho_mqtt_demand_actual_data):
    """ Handles the JP.TEPCO.Denkiyoho.mqtt.DemandActual message """
    print(f"Received JP.TEPCO.Denkiyoho.mqtt.DemandActual on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_tepco_denkiyoho_mqtt_demand_actual_data}")

async def handle_jp_tepco_denkiyoho_mqtt_demand_forecast(mqtt_msg,cloud_event, jp_tepco_denkiyoho_mqtt_demand_forecast_data):
    """ Handles the JP.TEPCO.Denkiyoho.mqtt.DemandForecast message """
    print(f"Received JP.TEPCO.Denkiyoho.mqtt.DemandForecast on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_tepco_denkiyoho_mqtt_demand_forecast_data}")

async def handle_jp_tepco_denkiyoho_mqtt_info(mqtt_msg,cloud_event, jp_tepco_denkiyoho_mqtt_info_data):
    """ Handles the JP.TEPCO.Denkiyoho.mqtt.Info message """
    print(f"Received JP.TEPCO.Denkiyoho.mqtt.Info on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_tepco_denkiyoho_mqtt_info_data}")
