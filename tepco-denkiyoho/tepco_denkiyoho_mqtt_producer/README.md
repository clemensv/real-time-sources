# Tepco_denkiyoho_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the tepco_denkiyoho_mqtt_producer message definitions.

# Tepco_denkiyoho_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - JPTEPCODenkiyohoMqttEventDispatcher
...
from tepco_denkiyoho_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = JPTEPCODenkiyohoMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### JPTEPCODenkiyohoMqttEventDispatcher



# Publish message`JPTEPCODenkiyohoMqttEventDispatcher` handles events for the JP.TEPCO.Denkiyoho.mqtt message group.
...
from tepco_denkiyoho_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = JPTEPCODenkiyohoMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe JPTEPCODenkiyohoMqttEventDispatcher defines the following event handler hooks.
...
```##### `jp_tepco_denkiyoho_mqtt_supply_capacity_async`



## Test```python

jp_tepco_denkiyoho_mqtt_supply_capacity_async:  Callable[[PartitionContext, EventData, CloudEvent, SupplyCapacity],
Awaitable[None]]
...
- `data`: The event data of type `tepco_denkiyoho_mqtt_producer_data.SupplyCapacity`.

Example:

```python
async def jp_tepco_denkiyoho_mqtt_supply_capacity_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: SupplyCapacity) -> None:
...
```##### `jp_tepco_denkiyoho_mqtt_peak_demand_forecast_async`



## Test```python

jp_tepco_denkiyoho_mqtt_peak_demand_forecast_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `tepco_denkiyoho_mqtt_producer_data.PeakDemandForecast`.

Example:

```python
async def jp_tepco_denkiyoho_mqtt_peak_demand_forecast_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: PeakDemandForecast) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `jp_tepco_denkiyoho_mqtt_demand_actual_async`



## Test```python

jp_tepco_denkiyoho_mqtt_demand_actual_async:  Callable[[PartitionContext, EventData, CloudEvent, DemandActual],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `tepco_denkiyoho_mqtt_producer_data.DemandActual`.

Example:

```python
async def jp_tepco_denkiyoho_mqtt_demand_actual_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: DemandActual) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `jp_tepco_denkiyoho_mqtt_demand_forecast_async`



## Test```python

jp_tepco_denkiyoho_mqtt_demand_forecast_async:  Callable[[PartitionContext, EventData, CloudEvent, DemandForecast],
Awaitable[None]]
...
```Asynchronous handler hook for `JP.TEPCO.Denkiyoho.mqtt.DemandForecast`: Hourly electricity demand forecast record
from TEPCO Electricity Forecast for the Tokyo service area.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `tepco_denkiyoho_mqtt_producer_data.DemandForecast`.

Example:

```python
async def jp_tepco_denkiyoho_mqtt_demand_forecast_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: DemandForecast) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `jp_tepco_denkiyoho_mqtt_info_async`



## Test```python

jp_tepco_denkiyoho_mqtt_info_async:  Callable[[PartitionContext, EventData, CloudEvent, Info], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `tepco_denkiyoho_mqtt_producer_data.Info`.

Example:

```python
async def jp_tepco_denkiyoho_mqtt_info_event(partition_context: PartitionContext, event: EventData, cloud_event:
