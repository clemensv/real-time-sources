...
- **Scales horizontally** across multiple brokers and partitions### BillettoEventsEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`BillettoEventsEventDispatcher` handles events for the Billetto.Events message group.
...
The BillettoEventsEventDispatcher defines the following event handler hooks.
...
producer = BillettoEventsProducer(
    ...
    security_protocol='SASL_SSL',##### `billetto_events_event_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'billetto_events_event_async:  Callable[[ConsumerRecord, CloudEvent, Event],
Awaitable[None]]
...
Asynchronous handler hook for `Billetto.Events.Event`: A public ticketed event from the Billetto platform, including
title, schedule, venue, organizer, pricing, and ticket availability status.
...
billetto_events_dispatcher.billetto_events_event_async = billetto_events_event_event
...
##### `send_billetto_events_event`Dispatchers have the following protected methods:



```python### Methods:

async def send_billetto_events_event(
...
Send a single `Billetto.Events.Event` message. A public ticketed event from the Billetto platform, including title,
...
await producer.send_billetto_events_event(
...
Send multiple `Billetto.Events.Event` messages in a batch.
