...
class JpGoJmaWeatherBulletinsAmqpProducer:
    """
    Producer class to send messages in the `jp.go.jma.WeatherBulletins.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_weather_bulletin(self,
        data: WeatherBulletin,
        _office: str,
        _bulletin_id: str,
        content_type: str = 'application/json') -> None:
        """
        Send the `jp.go.jma.WeatherBulletins.amqp.WeatherBulletin` message
        ...
        attributes = {
            "type":
            "jp.go.jma.WeatherBulletin",
            "source":
            "https://www.data.jma.go.jp",
            "subject":
            "{office}/{bulletin_id}".format(office=_office, bulletin_id=_bulletin_id),
        }
        ...
        amqp_msg.subject = "{office}/{bulletin_id}".format(office=_office, bulletin_id=_bulletin_id)
    ...
    def send_weather_bulletin_batch(self,
        data_array: typing.List[WeatherBulletin],
        _office: str,
        _bulletin_id: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `jp.go.jma.WeatherBulletins.amqp.WeatherBulletin` messages
        
        Args:
            data_array (typing.List[WeatherBulletin]): Array of message data objects
            _office (str): Value for placeholder office in attribute subject
            _bulletin_id (str): Value for placeholder bulletin_id in attribute subject
            content_type (str): The content type of the message data
        """
        for data in data_array:
            self.send_weather_bulletin(
                data=data,
                _office=_office,
