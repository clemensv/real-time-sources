# Jma_japan_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the jma_japan_mqtt_producer message definitions.

# Jma_japan_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - JpGoJmaWeatherBulletinsMqttEventDispatcher
...
from jma_japan_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = JpGoJmaWeatherBulletinsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### JpGoJmaWeatherBulletinsMqttEventDispatcher



# Publish message`JpGoJmaWeatherBulletinsMqttEventDispatcher` handles events for the jp.go.jma.WeatherBulletins.mqtt
message group.
...
from jma_japan_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = JpGoJmaWeatherBulletinsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe JpGoJmaWeatherBulletinsMqttEventDispatcher defines the following event handler hooks.
...
```##### `jp_go_jma_weather_bulletins_mqtt_weather_bulletin_async`



## Test```python

jp_go_jma_weather_bulletins_mqtt_weather_bulletin_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```Asynchronous handler hook for `jp.go.jma.WeatherBulletins.mqtt.WeatherBulletin`: An official alert or bulletin from
the Japan Meteorological Agency. It carries official bulletin messages for the affected area and validity period
...
- `data`: The event data of type `jma_japan_mqtt_producer_data.WeatherBulletin`.

Example:

```python
async def jp_go_jma_weather_bulletins_mqtt_weather_bulletin_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: WeatherBulletin) -> None:
