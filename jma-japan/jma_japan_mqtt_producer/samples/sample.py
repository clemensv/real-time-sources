...
import os
import signal
from jma_japan_mqtt_producer_data import *
from jma_japan_mqtt_producer_mqtt_client.client import JpGoJmaWeatherBulletinsMqttProducer, JpGoJmaWeatherBulletinsMqttDispatcher

async def handle_jp_go_jma_weather_bulletins_mqtt_weather_bulletin(mqtt_msg,cloud_event, jp_go_jma_weather_bulletins_mqtt_weather_bulletin_data):
    """ Handles the jp.go.jma.WeatherBulletins.mqtt.WeatherBulletin message """
    print(f"Received jp.go.jma.WeatherBulletins.mqtt.WeatherBulletin on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_go_jma_weather_bulletins_mqtt_weather_bulletin_data}")
