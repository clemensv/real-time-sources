...
def get_default_topic_mappings_dk_energinet_energidataservice_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the dk.energinet.energidataservice.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "dk.energinet.energidataservice.mqtt.PowerSystemSnapshot": "energy/dk/energidataservice/energidataservice-dk/{price_area}/power-system-snapshot",
        "dk.energinet.energidataservice.mqtt.SpotPrice": "energy/dk/energidataservice/energidataservice-dk/{price_area}/spot-price",
        "dk.energinet.energidataservice.mqtt.Info": "energy/dk/energidataservice/energidataservice-dk/{price_area}/info",
    }


def get_subscription_topics_dk_energinet_energidataservice_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_dk_energinet_energidataservice_mqtt()
    topics = []
...
class DkEnerginetEnergidataserviceMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the dk.energinet.energidataservice.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_dk_energinet_energidataservice_mqtt()
        ...
        self.dk_energinet_energidataservice_mqtt_power_system_snapshot_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, energidataservice_dk_mqtt_producer_data.PowerSystemSnapshot, Dict[str, str]], Awaitable[None]]] = None
        
        self.dk_energinet_energidataservice_mqtt_spot_price_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, energidataservice_dk_mqtt_producer_data.SpotPrice, Dict[str, str]], Awaitable[None]]] = None
        
        self.dk_energinet_energidataservice_mqtt_info_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, energidataservice_dk_mqtt_producer_data.Info, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "dk.energinet.energidataservice.PowerSystemSnapshot":
            if self.dk_energinet_energidataservice_mqtt_power_system_snapshot_async:
                try:
                    ...
                    data = energidataservice_dk_mqtt_producer_data.PowerSystemSnapshot.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "dk.energinet.energidataservice.mqtt.PowerSystemSnapshot")
                    await self.dk_energinet_energidataservice_mqtt_power_system_snapshot_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in dk_energinet_energidataservice_mqtt_power_system_snapshot handler: {e}")
            return
        
        if event_type == "dk.energinet.energidataservice.SpotPrice":
            if self.dk_energinet_energidataservice_mqtt_spot_price_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = energidataservice_dk_mqtt_producer_data.SpotPrice.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "dk.energinet.energidataservice.mqtt.SpotPrice")
                    await self.dk_energinet_energidataservice_mqtt_spot_price_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in dk_energinet_energidataservice_mqtt_spot_price handler: {e}")
            return
        
        if event_type == "dk.energinet.energidataservice.Info":
            if self.dk_energinet_energidataservice_mqtt_info_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = energidataservice_dk_mqtt_producer_data.Info.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "dk.energinet.energidataservice.mqtt.Info")
                    await self.dk_energinet_energidataservice_mqtt_info_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in dk_energinet_energidataservice_mqtt_info handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_dk_energinet_energidataservice_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_dk_energinet_energidataservice_mqtt_power_system_snapshot(self,
        price_area: str,
        data: energidataservice_dk_mqtt_producer_data.PowerSystemSnapshot,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'dk.energinet.energidataservice.mqtt.PowerSystemSnapshot' event to an MQTT topic.

        Args:
        
            price_area: URI template variable for 'price_area'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'energy/dk/energidataservice/energidataservice-dk/{price_area}/power-system-snapshot'
        ...
        """
        target_topic = topic if topic is not None else "energy/dk/energidataservice/energidataservice-dk/{price_area}/power-system-snapshot"
        ...
        attributes = {
             "type":"dk.energinet.energidataservice.PowerSystemSnapshot",
             "source":"https://api.energidataservice.dk",
             "subject":"{price_area}".format(price_area = price_area)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_dk_energinet_energidataservice_mqtt_spot_price(self,
        price_area: str,
        data: energidataservice_dk_mqtt_producer_data.SpotPrice,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'dk.energinet.energidataservice.mqtt.SpotPrice' event to an MQTT topic.

        Args:
        
            price_area: URI template variable for 'price_area'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'energy/dk/energidataservice/energidataservice-dk/{price_area}/spot-price'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "energy/dk/energidataservice/energidataservice-dk/{price_area}/spot-price"
        _topic_template_values: Dict[str, str] = {
            "price_area": str(price_area),
        }
        if _topic_template_values:
        ...
        attributes = {
             "type":"dk.energinet.energidataservice.SpotPrice",
             "source":"https://api.energidataservice.dk",
             "subject":"{price_area}".format(price_area = price_area)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_dk_energinet_energidataservice_mqtt_info(self,
        price_area: str,
        data: energidataservice_dk_mqtt_producer_data.Info,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'dk.energinet.energidataservice.mqtt.Info' event to an MQTT topic.

        Args:
        
            price_area: URI template variable for 'price_area'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'energy/dk/energidataservice/energidataservice-dk/{price_area}/info'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "energy/dk/energidataservice/energidataservice-dk/{price_area}/info"
        _topic_template_values: Dict[str, str] = {
            "price_area": str(price_area),
        }
        if _topic_template_values:
        ...
        attributes = {
             "type":"dk.energinet.energidataservice.Info",
