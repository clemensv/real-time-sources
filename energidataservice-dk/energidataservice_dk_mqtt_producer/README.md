# Energidataservice_dk_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the energidataservice_dk_mqtt_producer message definitions.

# Energidataservice_dk_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - DkEnerginetEnergidataserviceMqttEventDispatcher
...
from energidataservice_dk_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = DkEnerginetEnergidataserviceMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### DkEnerginetEnergidataserviceMqttEventDispatcher



# Publish message`DkEnerginetEnergidataserviceMqttEventDispatcher` handles events for the
dk.energinet.energidataservice.mqtt message group.
...
from energidataservice_dk_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = DkEnerginetEnergidataserviceMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name
of the Event Hub.- `blob_account_url`: The URL of the Azure Storage account.
...
## BuildThe DkEnerginetEnergidataserviceMqttEventDispatcher defines the following event handler hooks.
...
```##### `dk_energinet_energidataservice_mqtt_power_system_snapshot_async`



## Test```python

dk_energinet_energidataservice_mqtt_power_system_snapshot_async:  Callable[[PartitionContext, EventData, CloudEvent,
PowerSystemSnapshot], Awaitable[None]]
...
- `data`: The event data of type `energidataservice_dk_mqtt_producer_data.PowerSystemSnapshot`.

Example:

```python
async def dk_energinet_energidataservice_mqtt_power_system_snapshot_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: PowerSystemSnapshot) -> None:
...
```##### `dk_energinet_energidataservice_mqtt_spot_price_async`



## Test```python

dk_energinet_energidataservice_mqtt_spot_price_async:  Callable[[PartitionContext, EventData, CloudEvent, SpotPrice],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `energidataservice_dk_mqtt_producer_data.SpotPrice`.

Example:

```python
async def dk_energinet_energidataservice_mqtt_spot_price_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: SpotPrice) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `dk_energinet_energidataservice_mqtt_info_async`



## Test```python

dk_energinet_energidataservice_mqtt_info_async:  Callable[[PartitionContext, EventData, CloudEvent, Info],
Awaitable[None]]
...
```Asynchronous handler hook for `dk.energinet.energidataservice.mqtt.Info`: Retained reference information for
MQTT/AMQP topic discovery.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `energidataservice_dk_mqtt_producer_data.Info`.

Example:

```python
async def dk_energinet_energidataservice_mqtt_info_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: Info) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
