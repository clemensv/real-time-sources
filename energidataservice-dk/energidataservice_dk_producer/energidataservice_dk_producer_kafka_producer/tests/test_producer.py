...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../energidataservice_dk_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../energidataservice_dk_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../energidataservice_dk_producer_kafka_producer/src')))
...
def test_dk_energinet_energidataservice_dkenerginetenergidataservicepowersystemsnapshot(kafka_emulator):
    """Test the DkEnerginetEnergidataservicePowerSystemSnapshot event from the Dk.Energinet.Energidataservice message group"""
    ...
    consumer = Consumer({
        ...
        'group.id': 'test_dk_energinet_energidataservice_dkenerginetenergidataservicepowersystemsnapshot',  # Unique group per test
...
def test_dk_energinet_energidataservice_dkenerginetenergidataservicespotprice(kafka_emulator):
    """Test the DkEnerginetEnergidataserviceSpotPrice event from the Dk.Energinet.Energidataservice message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_dk_energinet_energidataservice_dkenerginetenergidataservicespotprice',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
...
def test_dk_energinet_energidataservice_cross_event_type_kafka_key(kafka_emulator):
    """Test that different event types in Dk.Energinet.Energidataservice produce the same Kafka key for the same placeholder values"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_dk_energinet_energidataservice_cross_key',
    ...
    data1 = Test_PowerSystemSnapshot.create_instance()
    data2 = Test_SpotPrice.create_instance()

    producer_instance.send_dk_energinet_energidataservice_power_system_snapshot(_price_area = shared_key_value, data = data1)
    producer_instance.send_dk_energinet_energidataservice_spot_price(_price_area = shared_key_value, data = data2)
    kafka_producer.flush(timeout=5.0)
    ...
    while len(collected_keys) < 2 and time.time() < timeout:
        ...
        if msg is None or msg.error():
            continue
        cloudevent = parse_cloudevent(msg)
        if cloudevent['type'] in ["dk.energinet.energidataservice.PowerSystemSnapshot", "dk.energinet.energidataservice.SpotPrice"]:
    ...
    expected_key = "{price_area}".format(price_area=shared_key_value)
