# Jma_bosai_quake_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the jma_bosai_quake_mqtt_producer message definitions.

# Jma_bosai_quake_mqtt_producer Event Dispatcher for Azure Event Hubs
...
from jma_bosai_quake_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = JPJMAQuakeMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### JPJMAQuakeMqttEventDispatcher



# Publish message`JPJMAQuakeMqttEventDispatcher` handles events for the JP.JMA.Quake.mqtt message group.
...
from jma_bosai_quake_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = JPJMAQuakeMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe JPJMAQuakeMqttEventDispatcher defines the following event handler hooks.
...
```##### `jp_jma_quake_mqtt_earthquake_report_async`



## Test```python

jp_jma_quake_mqtt_earthquake_report_async:  Callable[[PartitionContext, EventData, CloudEvent, EarthquakeReport],
Awaitable[None]]
...
- `data`: The event data of type `jma_bosai_quake_mqtt_producer_data.EarthquakeReport`.

Example:

```python
async def jp_jma_quake_mqtt_earthquake_report_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: EarthquakeReport) -> None:
