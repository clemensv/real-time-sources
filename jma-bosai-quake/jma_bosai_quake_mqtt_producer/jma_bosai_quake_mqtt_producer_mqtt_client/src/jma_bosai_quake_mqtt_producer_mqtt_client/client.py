...
def get_default_topic_mappings_jp_jma_quake_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the JP.JMA.Quake.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "JP.JMA.Quake.mqtt.EarthquakeReport": "seismic/jp/jma/jma-bosai-quake/{prefecture}/{magnitude_bucket}/{event_id}/{serial}/report",
    }


def get_subscription_topics_jp_jma_quake_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_jp_jma_quake_mqtt()
    topics = []
...
class JPJMAQuakeMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the JP.JMA.Quake.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_jp_jma_quake_mqtt()
        ...
        self.jp_jma_quake_mqtt_earthquake_report_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, jma_bosai_quake_mqtt_producer_data.EarthquakeReport, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "JP.JMA.Quake.EarthquakeReport":
            if self.jp_jma_quake_mqtt_earthquake_report_async:
                try:
                    ...
                    data = jma_bosai_quake_mqtt_producer_data.EarthquakeReport.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "JP.JMA.Quake.mqtt.EarthquakeReport")
                    await self.jp_jma_quake_mqtt_earthquake_report_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in jp_jma_quake_mqtt_earthquake_report handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_jp_jma_quake_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_jp_jma_quake_mqtt_earthquake_report(self,
        feedurl: str,
        event_id: str,
        serial: str,
        prefecture: str,
        magnitude_bucket: str,
        data: jma_bosai_quake_mqtt_producer_data.EarthquakeReport,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'JP.JMA.Quake.mqtt.EarthquakeReport' event to an MQTT topic.
        ...
        Args:
            ...
            magnitude_bucket: URI template variable for 'magnitude_bucket'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'seismic/jp/jma/jma-bosai-quake/{prefecture}/{magnitude_bucket}/{event_id}/{serial}/report'
        ...
        """
        target_topic = topic if topic is not None else "seismic/jp/jma/jma-bosai-quake/{prefecture}/{magnitude_bucket}/{event_id}/{serial}/report"
        ...
        attributes = {
             "type":"JP.JMA.Quake.EarthquakeReport",
             "source":"{feedurl}".format(feedurl = feedurl),
             "subject":"jp.jma.quake/{event_id}/{serial}".format(event_id = event_id,serial = serial)
        }
        attributes["datacontenttype"] = content_type
