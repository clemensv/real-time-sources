# JMA Bosai Quake container images
...
|---|---|---|
| `ghcr.io/clemensv/real-time-sources-jma-bosai-quake` | Kafka | JSON CloudEvents to one Kafka topic |
| `ghcr.io/clemensv/real-time-sources-jma-bosai-quake-mqtt` | MQTT 5.0 | Binary CloudEvents into UNS topic tree from xRegistry |
| `ghcr.io/clemensv/real-time-sources-jma-bosai-quake-amqp` | AMQP 1.0 | Binary CloudEvents to one AMQP address |
...
| Default entry point | Kafka: `python -m jma_bosai_quake`; MQTT: `python -m jma_bosai_quake_mqtt feed`; AMQP: `python -m jma_bosai_quake_amqp feed` |
| Exposed ports | none — outbound publisher only |
| Persistent state vars | Kafka: `STATE_FILE`; MQTT: `JMA_BOSAI_QUAKE_MQTT_STATE_FILE`; AMQP: `JMA_BOSAI_QUAKE_AMQP_STATE_FILE` |
...
docker pull ghcr.io/clemensv/real-time-sources-jma-bosai-quake-mqtt:latest
...
docker run --rm \
  ...
  -e AMQP_ADDRESS="jma-bosai-quake" \
...
Use `tools/deploy-fabric/deploy-feeder-notebook.ps1 -Source jma-bosai-quake -ResourceGroup <rg> -Location <azure-region> -Workspace <fabric-workspace>` to deploy the notebook in `notebook/`, bind Event Stream/Lakehouse/KQL assets, and schedule poll runs.
...
Use `tools/deploy-fabric/deploy-fabric-aci.ps1 -Source jma-bosai-quake -ResourceGroup <rg> -Location <azure-region> -Workspace <fabric-workspace>` for always-on container hosting that publishes to Fabric Event Streams.
