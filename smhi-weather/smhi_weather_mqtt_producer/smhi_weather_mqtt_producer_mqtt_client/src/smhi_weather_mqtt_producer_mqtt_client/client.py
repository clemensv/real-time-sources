...
import smhi_weather_mqtt_producer_data
from smhi_weather_mqtt_producer_data import Station
from smhi_weather_mqtt_producer_data import WeatherObservation
...
def get_default_topic_mappings_se_gov_smhi_weather_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the SE.Gov.SMHI.Weather.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "SE.Gov.SMHI.Weather.mqtt.Station": "weather/se/smhi/smhi-weather/{lan}/{station_id}/info",
        "SE.Gov.SMHI.Weather.mqtt.WeatherObservation": "weather/se/smhi/smhi-weather/{lan}/{station_id}/observation",
    }


def get_subscription_topics_se_gov_smhi_weather_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_se_gov_smhi_weather_mqtt()
    topics = []
...
class SEGovSMHIWeatherMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the SE.Gov.SMHI.Weather.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_se_gov_smhi_weather_mqtt()
        ...
        self.se_gov_smhi_weather_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, smhi_weather_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.se_gov_smhi_weather_mqtt_weather_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, smhi_weather_mqtt_producer_data.WeatherObservation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "SE.Gov.SMHI.Weather.Station":
            if self.se_gov_smhi_weather_mqtt_station_async:
                try:
                    ...
                    data = smhi_weather_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "SE.Gov.SMHI.Weather.mqtt.Station")
                    await self.se_gov_smhi_weather_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in se_gov_smhi_weather_mqtt_station handler: {e}")
            return
        
        if event_type == "SE.Gov.SMHI.Weather.WeatherObservation":
            if self.se_gov_smhi_weather_mqtt_weather_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = smhi_weather_mqtt_producer_data.WeatherObservation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "SE.Gov.SMHI.Weather.mqtt.WeatherObservation")
                    await self.se_gov_smhi_weather_mqtt_weather_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in se_gov_smhi_weather_mqtt_weather_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_se_gov_smhi_weather_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_se_gov_smhi_weather_mqtt_station(self,
        station_id: str,
        lan: str,
        data: smhi_weather_mqtt_producer_data.Station,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'SE.Gov.SMHI.Weather.mqtt.Station' event to an MQTT topic.

        Args:
        
            station_id: URI template variable for 'station_id'
            lan: URI template variable for 'lan'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/se/smhi/smhi-weather/{lan}/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "weather/se/smhi/smhi-weather/{lan}/{station_id}/info"
        ...
        attributes = {
             "type":"SE.Gov.SMHI.Weather.Station",
             "source":"https://opendata-download-metobs.smhi.se",
    ...
    async def publish_se_gov_smhi_weather_mqtt_weather_observation(self,
        station_id: str,
        lan: str,
        data: smhi_weather_mqtt_producer_data.WeatherObservation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'SE.Gov.SMHI.Weather.mqtt.WeatherObservation' event to an MQTT topic.
        ...
        Args:
            ...
            lan: URI template variable for 'lan'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/se/smhi/smhi-weather/{lan}/{station_id}/observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "weather/se/smhi/smhi-weather/{lan}/{station_id}/observation"
        _topic_template_values: Dict[str, str] = {
            "station_id": str(station_id),
            "lan": str(lan),
        }
        ...
        attributes = {
             "type":"SE.Gov.SMHI.Weather.WeatherObservation",
             "source":"https://opendata-download-metobs.smhi.se",
             "subject":"{station_id}".format(station_id = station_id)
        }
        attributes["datacontenttype"] = content_type
