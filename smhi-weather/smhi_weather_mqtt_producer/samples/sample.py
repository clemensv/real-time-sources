...
import os
import signal
from smhi_weather_mqtt_producer_data import *
from smhi_weather_mqtt_producer_mqtt_client.client import SEGovSMHIWeatherMqttProducer, SEGovSMHIWeatherMqttDispatcher

async def handle_se_gov_smhi_weather_mqtt_station(mqtt_msg,cloud_event, se_gov_smhi_weather_mqtt_station_data):
    """ Handles the SE.Gov.SMHI.Weather.mqtt.Station message """
    print(f"Received SE.Gov.SMHI.Weather.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {se_gov_smhi_weather_mqtt_station_data}")

async def handle_se_gov_smhi_weather_mqtt_weather_observation(mqtt_msg,cloud_event, se_gov_smhi_weather_mqtt_weather_observation_data):
    """ Handles the SE.Gov.SMHI.Weather.mqtt.WeatherObservation message """
    print(f"Received SE.Gov.SMHI.Weather.mqtt.WeatherObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {se_gov_smhi_weather_mqtt_weather_observation_data}")
