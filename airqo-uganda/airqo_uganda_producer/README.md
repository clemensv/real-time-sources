...
# Airqo-uganda-producer Kafka Producer# Airqo-uganda-producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - NetAirqoUgandaEventDispatcher
...
- NetAirqoUgandaProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### NetAirqoUgandaEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`NetAirqoUgandaEventDispatcher` handles events for the net.airqo.uganda message group.
...
from airqo-uganda-producer import NetAirqoUgandaProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = NetAirqoUgandaProducer(
...
await producer.send_net_airqo_uganda_site(```python
...
The NetAirqoUgandaEventDispatcher defines the following event handler hooks.
...
producer = NetAirqoUgandaProducer(
    ...
    security_protocol='SASL_SSL',##### `net_airqo_uganda_site_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'net_airqo_uganda_site_async:  Callable[[ConsumerRecord, CloudEvent, Site],
Awaitable[None]]
...
Asynchronous handler hook for `net.airqo.uganda.Site`: Reference data for an AirQo monitoring site, including its
geographic location, administrative area, and online status.
...
### NetAirqoUgandaProducer- `data`: The event data of type `airqo_uganda_producer_data.Site`.



Producer for `net.airqo.uganda` message group.Example:



#### Constructor```python

async def net_airqo_uganda_site_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Site) -> None:

```python    # Process the event data

NetAirqoUgandaProducer(    await some_processing_function(record, cloud_event, data)
...
net_airqo_uganda_dispatcher.net_airqo_uganda_site_async = net_airqo_uganda_site_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `net_airqo_uganda_measurement_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'net_airqo_uganda_measurement_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `net.airqo.uganda.Measurement`: Near-real-time air quality measurement from an AirQo low-
cost sensor, typically published at hourly intervals.
...
### NetAirqoUgandaProducer- `data`: The event data of type `airqo_uganda_producer_data.Measurement`.
...
async def net_airqo_uganda_measurement_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Measurement) ->
None:
...
net_airqo_uganda_dispatcher.net_airqo_uganda_measurement_async = net_airqo_uganda_measurement_event
...
##### `send_net_airqo_uganda_site`Dispatchers have the following protected methods:



```python### Methods:

async def send_net_airqo_uganda_site(
...
Send a single `net.airqo.uganda.Site` message. Reference data for an AirQo monitoring site, including its geographic
location, administrative area, and online status.Args:
...
await producer.send_net_airqo_uganda_site(
...
Send multiple `net.airqo.uganda.Site` messages in a batch.
...
await producer.send_net_airqo_uganda_site_batch(```
...
##### `send_net_airqo_uganda_measurement`Dispatchers have the following protected methods:



```python### Methods:

async def send_net_airqo_uganda_measurement(
...
Send a single `net.airqo.uganda.Measurement` message. Near-real-time air quality measurement from an AirQo low-cost
sensor, typically published at hourly intervals.Args:
...
await producer.send_net_airqo_uganda_measurement(
...
Send multiple `net.airqo.uganda.Measurement` messages in a batch.
...
await producer.send_net_airqo_uganda_measurement_batch(```
