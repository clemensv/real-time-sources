...
# Snotel_producer Kafka Producer# Snotel_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - GovUsdaNrcsSnotelEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    GovUsdaNrcsSnotelMqttEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    GovUsdaNrcsSnotelAmqpEventDispatcher
...
- GovUsdaNrcsSnotelProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- GovUsdaNrcsSnotelMqttProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- GovUsdaNrcsSnotelAmqpProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### GovUsdaNrcsSnotelEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`GovUsdaNrcsSnotelEventDispatcher` handles events for the gov.usda.nrcs.snotel message group.
...
from snotel_producer import GovUsdaNrcsSnotelProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = GovUsdaNrcsSnotelProducer(
...
await producer.send_gov_usda_nrcs_snotel_station(```python
...
The GovUsdaNrcsSnotelEventDispatcher defines the following event handler hooks.
...
producer = GovUsdaNrcsSnotelProducer(
    ...
    security_protocol='SASL_SSL',##### `gov_usda_nrcs_snotel_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gov_usda_nrcs_snotel_station_async:  Callable[[ConsumerRecord, CloudEvent, Station],
Awaitable[None]]
...
Asynchronous handler hook for `gov.usda.nrcs.snotel.Station`:
...
### GovUsdaNrcsSnotelProducer- `data`: The event data of type `snotel_producer_data.Station`.



Producer for `gov.usda.nrcs.snotel` message group.Example:



#### Constructor```python

async def gov_usda_nrcs_snotel_station_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Station) -> None:

```python    # Process the event data

GovUsdaNrcsSnotelProducer(    await some_processing_function(record, cloud_event, data)
...
gov_usda_nrcs_snotel_dispatcher.gov_usda_nrcs_snotel_station_async = gov_usda_nrcs_snotel_station_event
...
### GovUsdaNrcsSnotelMqttProducer- `data`: The event data of type `snotel_producer_data.Station`.



Producer for `gov.usda.nrcs.snotel.mqtt` message group.Example:
...
GovUsdaNrcsSnotelMqttProducer(    await some_processing_function(record, cloud_event, data)
...
gov_usda_nrcs_snotel_mqtt_dispatcher.gov_usda_nrcs_snotel_station_async = gov_usda_nrcs_snotel_station_event
...
### GovUsdaNrcsSnotelAmqpProducer- `data`: The event data of type `snotel_producer_data.Station`.



Producer for `gov.usda.nrcs.snotel.amqp` message group.Example:
...
GovUsdaNrcsSnotelAmqpProducer(    await some_processing_function(record, cloud_event, data)
...
gov_usda_nrcs_snotel_amqp_dispatcher.gov_usda_nrcs_snotel_station_async = gov_usda_nrcs_snotel_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `gov_usda_nrcs_snotel_snow_observation_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gov_usda_nrcs_snotel_snow_observation_async:  Callable[[ConsumerRecord, CloudEvent,
SnowObservation], Awaitable[None]]

)```

```

Asynchronous handler hook for `gov.usda.nrcs.snotel.SnowObservation`:
...
### GovUsdaNrcsSnotelProducer- `data`: The event data of type `snotel_producer_data.SnowObservation`.
...
async def gov_usda_nrcs_snotel_snow_observation_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
SnowObservation) -> None:
...
gov_usda_nrcs_snotel_dispatcher.gov_usda_nrcs_snotel_snow_observation_async =
gov_usda_nrcs_snotel_snow_observation_event
...
### GovUsdaNrcsSnotelMqttProducer- `data`: The event data of type `snotel_producer_data.SnowObservation`.
...
gov_usda_nrcs_snotel_mqtt_dispatcher.gov_usda_nrcs_snotel_snow_observation_async =
gov_usda_nrcs_snotel_snow_observation_event
...
### GovUsdaNrcsSnotelAmqpProducer- `data`: The event data of type `snotel_producer_data.SnowObservation`.
...
gov_usda_nrcs_snotel_amqp_dispatcher.gov_usda_nrcs_snotel_snow_observation_async =
gov_usda_nrcs_snotel_snow_observation_event
...
##### `send_gov_usda_nrcs_snotel_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_gov_usda_nrcs_snotel_station(
...
Send a single `gov.usda.nrcs.snotel.Station` message.Args:
...
await producer.send_gov_usda_nrcs_snotel_station(
...
Send multiple `gov.usda.nrcs.snotel.Station` messages in a batch.
...
await producer.send_gov_usda_nrcs_snotel_station_batch(```
...
##### `send_gov_usda_nrcs_snotel_snow_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_gov_usda_nrcs_snotel_snow_observation(

    self,##### `_process_event`

    data: SnowObservation,
...
Send a single `gov.usda.nrcs.snotel.SnowObservation` message.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `SnowObservation`
...
await producer.send_gov_usda_nrcs_snotel_snow_observation(

    data=SnowObservation(...),Args:
...
Send multiple `gov.usda.nrcs.snotel.SnowObservation` messages in a batch.
...
await producer.send_gov_usda_nrcs_snotel_snow_observation_batch(```

    messages=[

        SnowObservation(...),Initializes the runner with a Kafka consumer.

        SnowObservation(...),

        SnowObservation(...)Args:
...
- **Scales horizontally** across multiple brokers and partitions### GovUsdaNrcsSnotelMqttEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`GovUsdaNrcsSnotelMqttEventDispatcher` handles events for the gov.usda.nrcs.snotel.mqtt message group.
...
The GovUsdaNrcsSnotelMqttEventDispatcher defines the following event handler hooks.
...
producer = GovUsdaNrcsSnotelProducer(
    ...
    security_protocol='SASL_SSL',##### `gov_usda_nrcs_snotel_mqtt_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gov_usda_nrcs_snotel_mqtt_station_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `gov.usda.nrcs.snotel.mqtt.Station`:
...
async def gov_usda_nrcs_snotel_mqtt_station_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Station) ->
None:
...
gov_usda_nrcs_snotel_dispatcher.gov_usda_nrcs_snotel_mqtt_station_async = gov_usda_nrcs_snotel_mqtt_station_event
...
gov_usda_nrcs_snotel_mqtt_dispatcher.gov_usda_nrcs_snotel_mqtt_station_async = gov_usda_nrcs_snotel_mqtt_station_event
...
gov_usda_nrcs_snotel_amqp_dispatcher.gov_usda_nrcs_snotel_mqtt_station_async = gov_usda_nrcs_snotel_mqtt_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `gov_usda_nrcs_snotel_mqtt_snow_observation_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gov_usda_nrcs_snotel_mqtt_snow_observation_async:  Callable[[ConsumerRecord,
CloudEvent, SnowObservation], Awaitable[None]]

)```

```

Asynchronous handler hook for `gov.usda.nrcs.snotel.mqtt.SnowObservation`:
...
async def gov_usda_nrcs_snotel_mqtt_snow_observation_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
SnowObservation) -> None:
...
gov_usda_nrcs_snotel_dispatcher.gov_usda_nrcs_snotel_mqtt_snow_observation_async =
gov_usda_nrcs_snotel_mqtt_snow_observation_event
...
gov_usda_nrcs_snotel_mqtt_dispatcher.gov_usda_nrcs_snotel_mqtt_snow_observation_async =
gov_usda_nrcs_snotel_mqtt_snow_observation_event
...
gov_usda_nrcs_snotel_amqp_dispatcher.gov_usda_nrcs_snotel_mqtt_snow_observation_async =
gov_usda_nrcs_snotel_mqtt_snow_observation_event
...
##### `send_gov_usda_nrcs_snotel_mqtt_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_gov_usda_nrcs_snotel_mqtt_station(
...
Send a single `gov.usda.nrcs.snotel.mqtt.Station` message.Args:
...
await producer.send_gov_usda_nrcs_snotel_mqtt_station(
...
Send multiple `gov.usda.nrcs.snotel.mqtt.Station` messages in a batch.
...
await producer.send_gov_usda_nrcs_snotel_mqtt_station_batch(```
...
##### `send_gov_usda_nrcs_snotel_mqtt_snow_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_gov_usda_nrcs_snotel_mqtt_snow_observation(
...
Send a single `gov.usda.nrcs.snotel.mqtt.SnowObservation` message.Args:
...
await producer.send_gov_usda_nrcs_snotel_mqtt_snow_observation(
...
Send multiple `gov.usda.nrcs.snotel.mqtt.SnowObservation` messages in a batch.
...
await producer.send_gov_usda_nrcs_snotel_mqtt_snow_observation_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### GovUsdaNrcsSnotelAmqpEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`GovUsdaNrcsSnotelAmqpEventDispatcher` handles events for the gov.usda.nrcs.snotel.amqp message group.
...
The GovUsdaNrcsSnotelAmqpEventDispatcher defines the following event handler hooks.
...
producer = GovUsdaNrcsSnotelProducer(
    ...
    security_protocol='SASL_SSL',##### `gov_usda_nrcs_snotel_amqp_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gov_usda_nrcs_snotel_amqp_station_async:  Callable[[ConsumerRecord, CloudEvent,
Station], Awaitable[None]]
...
Asynchronous handler hook for `gov.usda.nrcs.snotel.amqp.Station`:
...
async def gov_usda_nrcs_snotel_amqp_station_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Station) ->
None:
...
gov_usda_nrcs_snotel_dispatcher.gov_usda_nrcs_snotel_amqp_station_async = gov_usda_nrcs_snotel_amqp_station_event
...
gov_usda_nrcs_snotel_mqtt_dispatcher.gov_usda_nrcs_snotel_amqp_station_async = gov_usda_nrcs_snotel_amqp_station_event
...
gov_usda_nrcs_snotel_amqp_dispatcher.gov_usda_nrcs_snotel_amqp_station_async = gov_usda_nrcs_snotel_amqp_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `gov_usda_nrcs_snotel_amqp_snow_observation_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gov_usda_nrcs_snotel_amqp_snow_observation_async:  Callable[[ConsumerRecord,
CloudEvent, SnowObservation], Awaitable[None]]
...
Asynchronous handler hook for `gov.usda.nrcs.snotel.amqp.SnowObservation`:
...
async def gov_usda_nrcs_snotel_amqp_snow_observation_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
SnowObservation) -> None:
...
gov_usda_nrcs_snotel_dispatcher.gov_usda_nrcs_snotel_amqp_snow_observation_async =
gov_usda_nrcs_snotel_amqp_snow_observation_event
...
gov_usda_nrcs_snotel_mqtt_dispatcher.gov_usda_nrcs_snotel_amqp_snow_observation_async =
gov_usda_nrcs_snotel_amqp_snow_observation_event
...
gov_usda_nrcs_snotel_amqp_dispatcher.gov_usda_nrcs_snotel_amqp_snow_observation_async =
gov_usda_nrcs_snotel_amqp_snow_observation_event
...
##### `send_gov_usda_nrcs_snotel_amqp_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_gov_usda_nrcs_snotel_amqp_station(
...
Send a single `gov.usda.nrcs.snotel.amqp.Station` message.Args:
...
await producer.send_gov_usda_nrcs_snotel_amqp_station(
...
Send multiple `gov.usda.nrcs.snotel.amqp.Station` messages in a batch.
...
await producer.send_gov_usda_nrcs_snotel_amqp_station_batch(```
...
##### `send_gov_usda_nrcs_snotel_amqp_snow_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_gov_usda_nrcs_snotel_amqp_snow_observation(
...
Send a single `gov.usda.nrcs.snotel.amqp.SnowObservation` message.Args:
...
await producer.send_gov_usda_nrcs_snotel_amqp_snow_observation(
...
Send multiple `gov.usda.nrcs.snotel.amqp.SnowObservation` messages in a batch.
...
await producer.send_gov_usda_nrcs_snotel_amqp_snow_observation_batch(```
