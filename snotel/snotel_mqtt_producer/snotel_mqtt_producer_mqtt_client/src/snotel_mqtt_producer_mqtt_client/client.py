...
def get_default_topic_mappings_gov_usda_nrcs_snotel_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the gov.usda.nrcs.snotel.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "gov.usda.nrcs.snotel.mqtt.Station": "hydro/us/usda/snotel/{state}/{station_triplet}/info",
        "gov.usda.nrcs.snotel.mqtt.SnowObservation": "hydro/us/usda/snotel/{state}/{station_triplet}/snow-observation",
    }


def get_subscription_topics_gov_usda_nrcs_snotel_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_gov_usda_nrcs_snotel_mqtt()
    topics = []
...
class GovUsdaNrcsSnotelMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the gov.usda.nrcs.snotel.mqtt message group."""
    ...
    def __init__(
        ...
        self.gov_usda_nrcs_snotel_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, snotel_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.gov_usda_nrcs_snotel_mqtt_snow_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, snotel_mqtt_producer_data.SnowObservation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "gov.usda.nrcs.snotel.Station":
            if self.gov_usda_nrcs_snotel_mqtt_station_async:
                try:
                    ...
                    data = snotel_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "gov.usda.nrcs.snotel.mqtt.Station")
                    await self.gov_usda_nrcs_snotel_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in gov_usda_nrcs_snotel_mqtt_station handler: {e}")
            return
        
        if event_type == "gov.usda.nrcs.snotel.SnowObservation":
            if self.gov_usda_nrcs_snotel_mqtt_snow_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = snotel_mqtt_producer_data.SnowObservation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "gov.usda.nrcs.snotel.mqtt.SnowObservation")
                    await self.gov_usda_nrcs_snotel_mqtt_snow_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in gov_usda_nrcs_snotel_mqtt_snow_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_gov_usda_nrcs_snotel_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_gov_usda_nrcs_snotel_mqtt_station(self,
        station_triplet: str,
        state: str,
        data: snotel_mqtt_producer_data.Station,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'gov.usda.nrcs.snotel.mqtt.Station' event to an MQTT topic.

        Args:
        
            station_triplet: URI template variable for 'station_triplet'
            state: URI template variable for 'state'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/us/usda/snotel/{state}/{station_triplet}/info'
        ...
        """
        target_topic = topic if topic is not None else "hydro/us/usda/snotel/{state}/{station_triplet}/info"
        _topic_template_values: Dict[str, str] = {
            "station_triplet": str(station_triplet),
            "state": str(state),
        }
        ...
        attributes = {
             ...
             "subject":"{station_triplet}".format(station_triplet = station_triplet)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_gov_usda_nrcs_snotel_mqtt_snow_observation(self,
        station_triplet: str,
        state: str,
        data: snotel_mqtt_producer_data.SnowObservation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'gov.usda.nrcs.snotel.mqtt.SnowObservation' event to an MQTT topic.
        ...
        Args:
            ...
            state: URI template variable for 'state'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/us/usda/snotel/{state}/{station_triplet}/snow-observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "hydro/us/usda/snotel/{state}/{station_triplet}/snow-observation"
        _topic_template_values: Dict[str, str] = {
            "station_triplet": str(station_triplet),
            "state": str(state),
        }
