# Snotel_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the snotel_mqtt_producer message definitions.

# Snotel_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - GovUsdaNrcsSnotelMqttEventDispatcher
...
from snotel_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = GovUsdaNrcsSnotelMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### GovUsdaNrcsSnotelMqttEventDispatcher



# Publish message`GovUsdaNrcsSnotelMqttEventDispatcher` handles events for the gov.usda.nrcs.snotel.mqtt message group.
...
from snotel_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = GovUsdaNrcsSnotelMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe GovUsdaNrcsSnotelMqttEventDispatcher defines the following event handler hooks.
...
```##### `gov_usda_nrcs_snotel_mqtt_station_async`



## Test```python

gov_usda_nrcs_snotel_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `snotel_mqtt_producer_data.Station`.

Example:

```python
async def gov_usda_nrcs_snotel_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `gov_usda_nrcs_snotel_mqtt_snow_observation_async`



## Test```python

gov_usda_nrcs_snotel_mqtt_snow_observation_async:  Callable[[PartitionContext, EventData, CloudEvent, SnowObservation],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `snotel_mqtt_producer_data.SnowObservation`.

Example:

```python
async def gov_usda_nrcs_snotel_mqtt_snow_observation_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: SnowObservation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
