...
"""
Tests for carbon_intensity_amqp_producer_amqp_producer
"""
import base64
import json
import os
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../carbon_intensity_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../carbon_intensity_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../carbon_intensity_amqp_producer_amqp_producer/src')))

from carbon_intensity_amqp_producer_amqp_producer import *
from carbon_intensity_amqp_producer_data import Intensity
from test_carbon_intensity_amqp_producer_data_intensity import Test_Intensity
from carbon_intensity_amqp_producer_data import GenerationMix
from test_carbon_intensity_amqp_producer_data_generationmix import Test_GenerationMix
from carbon_intensity_amqp_producer_data import RegionalIntensity
from test_carbon_intensity_amqp_producer_data_regionalintensity import Test_RegionalIntensity
...
class TestUkOrgCarbonintensityAmqpProducer:
    """Test cases for UkOrgCarbonintensityAmqpProducer"""
    ...
    def test_send_intensity(self, artemis_container):
        """Send and receive a Intensity message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Intensity.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "uk.org.carbonintensity.amqp.Intensity"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{period_from}".format(period_from="value")
        finally:
            producer.close()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "uk.org.carbonintensity.amqp.GenerationMix"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
    ...
    def test_send_regional_intensity(self, artemis_container):
        """Send and receive a RegionalIntensity message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_RegionalIntensity.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "uk.org.carbonintensity.amqp.RegionalIntensity"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
