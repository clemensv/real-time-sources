...
class UkOrgCarbonintensityAmqpProducer:
    """
    Producer class to send messages in the `uk.org.carbonintensity.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_intensity(self,
        ...
        """
        Send the `uk.org.carbonintensity.amqp.Intensity` message
        ...
        attributes = {
            ...
            "subject":
            "{period_from}".format(period_from=_period_from),
            "id":
            str(uuid.uuid4()),
        }
        ...
        amqp_msg.subject = "{period_from}".format(period_from=_period_from)
    ...
    def send_intensity_batch(self,
        data_array: typing.List[Intensity],
        _period_from: str,
        _ce_id: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `uk.org.carbonintensity.amqp.Intensity` messages
        
        Args:
            data_array (typing.List[Intensity]): Array of message data objects
            _period_from (str): Value for placeholder period_from in attribute subject
            _ce_id (str): Value for placeholder ce_id in attribute id
    ...
    def send_generation_mix(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `uk.org.carbonintensity.amqp.GenerationMix` message
    ...
    def send_generation_mix_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `uk.org.carbonintensity.amqp.GenerationMix` messages
    ...
    def send_regional_intensity(self,
        data: RegionalIntensity,
        _region_id: str,
        _ce_id: str,
        content_type: str = 'application/json') -> None:
        """
        Send the `uk.org.carbonintensity.amqp.RegionalIntensity` message
        ...
        attributes = {
            ...
            "https://api.carbonintensity.org.uk",
            "subject":
            "{region_id}".format(region_id=_region_id),
            "id":
            str(uuid.uuid4()),
        }
        ...
        if self.content_mode == 'structured':
            ...
            if headers:
                amqp_msg.properties = self._ce_headers_to_amqp_properties(headers)
        # Apply AMQP message properties declared in protocoloptions.properties.
        amqp_msg.subject = "{region_id}".format(region_id=_region_id)
    ...
    def send_regional_intensity_batch(self,
        data_array: typing.List[RegionalIntensity],
        _region_id: str,
        _ce_id: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `uk.org.carbonintensity.amqp.RegionalIntensity` messages
        
        Args:
            data_array (typing.List[RegionalIntensity]): Array of message data objects
            _region_id (str): Value for placeholder region_id in attribute subject
            _ce_id (str): Value for placeholder ce_id in attribute id
            content_type (str): The content type of the message data
        """
        for data in data_array:
            self.send_regional_intensity(
                data=data,
