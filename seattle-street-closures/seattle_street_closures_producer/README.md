...
3. [Quick Start](#quick-start)    - USWASeattleStreetClosuresEventDispatcher
...
- USWASeattleStreetClosuresProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### USWASeattleStreetClosuresEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`USWASeattleStreetClosuresEventDispatcher` handles events for the US.WA.Seattle.StreetClosures message group.
...
producer = USWASeattleStreetClosuresProducer(
...
await producer.send_us_wa_seattle_street_closures_street_closure(```python

    data=StreetClosure(...),add_consumer(self, consumer: KafkaConsumer)
...
The USWASeattleStreetClosuresEventDispatcher defines the following event handler hooks.
...
producer = USWASeattleStreetClosuresProducer(
    ...
    security_protocol='SASL_SSL',##### `us_wa_seattle_street_closures_street_closure_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'us_wa_seattle_street_closures_street_closure_async:  Callable[[ConsumerRecord,
CloudEvent, StreetClosure], Awaitable[None]]

)```

```

Asynchronous handler hook for `US.WA.Seattle.StreetClosures.StreetClosure`:
...
### USWASeattleStreetClosuresProducer- `data`: The event data of type
`seattle_street_closures_producer_data.StreetClosure`.



Producer for `US.WA.Seattle.StreetClosures` message group.Example:



#### Constructor```python

async def us_wa_seattle_street_closures_street_closure_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
StreetClosure) -> None:

```python    # Process the event data

USWASeattleStreetClosuresProducer(    await some_processing_function(record, cloud_event, data)
...
us_wa_seattle_street_closures_dispatcher.us_wa_seattle_street_closures_street_closure_async =
us_wa_seattle_street_closures_street_closure_event
...
##### `send_us_wa_seattle_street_closures_street_closure`Dispatchers have the following protected methods:



```python### Methods:

async def send_us_wa_seattle_street_closures_street_closure(
...
Send a single `US.WA.Seattle.StreetClosures.StreetClosure` message.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `StreetClosure`
...
await producer.send_us_wa_seattle_street_closures_street_closure(

    data=StreetClosure(...),Args:
...
Send multiple `US.WA.Seattle.StreetClosures.StreetClosure` messages in a batch.
...
await producer.send_us_wa_seattle_street_closures_street_closure_batch(```

    messages=[

        StreetClosure(...),Initializes the runner with a Kafka consumer.
