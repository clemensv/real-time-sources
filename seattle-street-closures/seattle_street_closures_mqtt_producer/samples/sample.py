...
import os
import signal
from seattle_street_closures_mqtt_producer_data import *
from seattle_street_closures_mqtt_producer_mqtt_client.client import USWASeattleStreetClosuresMqttProducer, USWASeattleStreetClosuresMqttDispatcher

async def handle_us_wa_seattle_street_closures_street_closure_mqtt(mqtt_msg,cloud_event, us_wa_seattle_street_closures_street_closure_mqtt_data):
    """ Handles the US.WA.Seattle.StreetClosures.StreetClosure.mqtt message """
    print(f"Received US.WA.Seattle.StreetClosures.StreetClosure.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {us_wa_seattle_street_closures_street_closure_mqtt_data}")
