# Seattle_street_closures_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the seattle_street_closures_mqtt_producer message definitions.

# Seattle_street_closures_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - USWASeattleStreetClosuresMqttEventDispatcher
...
from seattle_street_closures_mqtt_producer_mqtt_client import *This module defines an event processing framework for
Azure Event Hubs,
...
client = USWASeattleStreetClosuresMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### USWASeattleStreetClosuresMqttEventDispatcher



# Publish message`USWASeattleStreetClosuresMqttEventDispatcher` handles events for the US.WA.Seattle.StreetClosures.mqtt
message group.
...
from seattle_street_closures_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = USWASeattleStreetClosuresMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of
...
## BuildThe USWASeattleStreetClosuresMqttEventDispatcher defines the following event handler hooks.
...
```##### `us_wa_seattle_street_closures_street_closure_mqtt_async`



## Test```python

us_wa_seattle_street_closures_street_closure_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
StreetClosure], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `US.WA.Seattle.StreetClosures.StreetClosure.mqtt`:
...
- `data`: The event data of type `seattle_street_closures_mqtt_producer_data.StreetClosure`.

Example:

```python
async def us_wa_seattle_street_closures_street_closure_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: StreetClosure) -> None:
...
```python
us_wa_seattle_street_closures_mqtt_dispatcher.us_wa_seattle_street_closures_street_closure_mqtt_async =
us_wa_seattle_street_closures_street_closure_mqtt_event
```
