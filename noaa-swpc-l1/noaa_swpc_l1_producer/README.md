...
# Noaa_swpc_l1_producer Kafka Producer# Noaa_swpc_l1_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - GovNoaaSwpcL1KafkaEventDispatcher
...
- GovNoaaSwpcL1KafkaProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### GovNoaaSwpcL1KafkaEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`GovNoaaSwpcL1KafkaEventDispatcher` handles events for the gov.noaa.swpc.l1.kafka message group.
...
from noaa_swpc_l1_producer import GovNoaaSwpcL1KafkaProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = GovNoaaSwpcL1KafkaProducer(
...
await producer.send_gov_noaa_swpc_l1_kafka_propagated_solar_wind(```python

    data=PropagatedSolarWind(...),add_consumer(self, consumer: KafkaConsumer)
...
The GovNoaaSwpcL1KafkaEventDispatcher defines the following event handler hooks.
...
producer = GovNoaaSwpcL1KafkaProducer(
    ...
    security_protocol='SASL_SSL',##### `gov_noaa_swpc_l1_kafka_propagated_solar_wind_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gov_noaa_swpc_l1_kafka_propagated_solar_wind_async:  Callable[[ConsumerRecord,
CloudEvent, PropagatedSolarWind], Awaitable[None]]

)```

```

Asynchronous handler hook for `gov.noaa.swpc.l1.kafka.PropagatedSolarWind`: One minute-resolution solar-wind observation
at L1 with forward-propagated Earth-arrival timestamp. Fuses plasma (speed/density/temperature), magnetic field
(Bx/By/Bz/Bt in GSM), and bulk velocity vector (Vx/Vy/Vz in GSM) into a single row per `time_tag`. The
`propagated_time_tag` field is the upstream's predicted Earth arrival time for the solar-wind parcel observed at L1 at
`time_tag`. The CloudEvent `time` attribute mirrors `time_tag` (the L1 observation time) — consumers that need the
Earth-arrival time must read `data.propagated_time_tag` instead. Many sub-fields are nullable — instrument gaps on
DSCOVR/ACE are routine, especially for the velocity components which DSCOVR does not measure reliably. Sourced from `GET
https://services.swpc.noaa.gov/products/geospace/propagated-solar-wind.json` — the document is a 7-day rolling window
(~10080 rows) of array-of-arrays form with the column header at index 0.
...
### GovNoaaSwpcL1KafkaProducer- `data`: The event data of type `noaa_swpc_l1_producer_data.PropagatedSolarWind`.



Producer for `gov.noaa.swpc.l1.kafka` message group.Example:



#### Constructor```python

async def gov_noaa_swpc_l1_kafka_propagated_solar_wind_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
PropagatedSolarWind) -> None:

```python    # Process the event data

GovNoaaSwpcL1KafkaProducer(    await some_processing_function(record, cloud_event, data)
...
gov_noaa_swpc_l1_kafka_dispatcher.gov_noaa_swpc_l1_kafka_propagated_solar_wind_async =
gov_noaa_swpc_l1_kafka_propagated_solar_wind_event
...
##### `send_gov_noaa_swpc_l1_kafka_propagated_solar_wind`Dispatchers have the following protected methods:



```python### Methods:

async def send_gov_noaa_swpc_l1_kafka_propagated_solar_wind(

    self,##### `_process_event`

    data: PropagatedSolarWind,
...
Send a single `gov.noaa.swpc.l1.kafka.PropagatedSolarWind` message. One minute-resolution solar-wind observation at L1
with forward-propagated Earth-arrival timestamp. Fuses plasma (speed/density/temperature), magnetic field (Bx/By/Bz/Bt
in GSM), and bulk velocity vector (Vx/Vy/Vz in GSM) into a single row per `time_tag`. The `propagated_time_tag` field is
the upstream's predicted Earth arrival time for the solar-wind parcel observed at L1 at `time_tag`. The CloudEvent
`time` attribute mirrors `time_tag` (the L1 observation time) — consumers that need the Earth-arrival time must read
`data.propagated_time_tag` instead. Many sub-fields are nullable — instrument gaps on DSCOVR/ACE are routine, especially
for the velocity components which DSCOVR does not measure reliably. Sourced from `GET
https://services.swpc.noaa.gov/products/geospace/propagated-solar-wind.json` — the document is a 7-day rolling window
(~10080 rows) of array-of-arrays form with the column header at index 0.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `PropagatedSolarWind`
...
await producer.send_gov_noaa_swpc_l1_kafka_propagated_solar_wind(

    data=PropagatedSolarWind(...),Args:
...
##### `send_gov_noaa_swpc_l1_kafka_propagated_solar_wind_batch`##### `_dispatch_record`



```python```python

async def send_gov_noaa_swpc_l1_kafka_propagated_solar_wind_batch(_dispatch_record(self, record)

    self,```

    messages: List[PropagatedSolarWind],
...
Send multiple `gov.noaa.swpc.l1.kafka.PropagatedSolarWind` messages in a batch.
...
await producer.send_gov_noaa_swpc_l1_kafka_propagated_solar_wind_batch(```

    messages=[

        PropagatedSolarWind(...),Initializes the runner with a Kafka consumer.

        PropagatedSolarWind(...),

        PropagatedSolarWind(...)Args:
