...
import os
import signal
from noaa_swpc_l1_mqtt_producer_data import *
from noaa_swpc_l1_mqtt_producer_mqtt_client.client import GovNoaaSwpcL1MqttProducer, GovNoaaSwpcL1MqttDispatcher

async def handle_gov_noaa_swpc_l1_mqtt_propagated_solar_wind(mqtt_msg,cloud_event, gov_noaa_swpc_l1_mqtt_propagated_solar_wind_data):
    """ Handles the gov.noaa.swpc.l1.mqtt.PropagatedSolarWind message """
    print(f"Received gov.noaa.swpc.l1.mqtt.PropagatedSolarWind on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {gov_noaa_swpc_l1_mqtt_propagated_solar_wind_data}")
