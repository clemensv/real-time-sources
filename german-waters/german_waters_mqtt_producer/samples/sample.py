...
import os
import signal
from german_waters_mqtt_producer_data import *
from german_waters_mqtt_producer_mqtt_client.client import DEWatersHydrologyMqttProducer, DEWatersHydrologyMqttDispatcher

async def handle_de_waters_hydrology_mqtt_station(mqtt_msg,cloud_event, de_waters_hydrology_mqtt_station_data):
    """ Handles the DE.Waters.Hydrology.mqtt.Station message """
    print(f"Received DE.Waters.Hydrology.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {de_waters_hydrology_mqtt_station_data}")

async def handle_de_waters_hydrology_mqtt_water_level_observation(mqtt_msg,cloud_event, de_waters_hydrology_mqtt_water_level_observation_data):
    """ Handles the DE.Waters.Hydrology.mqtt.WaterLevelObservation message """
    print(f"Received DE.Waters.Hydrology.mqtt.WaterLevelObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {de_waters_hydrology_mqtt_water_level_observation_data}")
