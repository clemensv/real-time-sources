# German_waters_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the german_waters_mqtt_producer message definitions.

# German_waters_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - DEWatersHydrologyMqttEventDispatcher
...
from german_waters_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = DEWatersHydrologyMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### DEWatersHydrologyMqttEventDispatcher



# Publish message`DEWatersHydrologyMqttEventDispatcher` handles events for the DE.Waters.Hydrology.mqtt message group.
...
from german_waters_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = DEWatersHydrologyMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe DEWatersHydrologyMqttEventDispatcher defines the following event handler hooks.
...
```##### `de_waters_hydrology_mqtt_station_async`



## Test```python

de_waters_hydrology_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `german_waters_mqtt_producer_data.Station`.

Example:

```python
async def de_waters_hydrology_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `de_waters_hydrology_mqtt_water_level_observation_async`



## Test```python

de_waters_hydrology_mqtt_water_level_observation_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `german_waters_mqtt_producer_data.WaterLevelObservation`.

Example:

```python
async def de_waters_hydrology_mqtt_water_level_observation_event(partition_context: PartitionContext, event: EventData,
