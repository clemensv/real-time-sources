    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from german_waters_amqp_producer_data import Station
from german_waters_amqp_producer_data import WaterLevelObservation

class DEWatersHydrologyAmqpProducer:
    """
    Producer class to send messages in the `DE.Waters.Hydrology.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_station(self,
        ...
        """
        Send the `DE.Waters.Hydrology.amqp.Station` message
        A reference record for one German state water monitoring station published by German state open data portals. It fires when the bridge publishes or refreshes the station catalog so consumers can interpret measurement events.
        ...
        attributes = {
            ...
            "source":
            "https://github.com/clemensv/real-time-sources/german-waters",
            "subject":
    ...
    def send_station_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `DE.Waters.Hydrology.amqp.Station` messages
    ...
    def send_water_level_observation(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `DE.Waters.Hydrology.amqp.WaterLevelObservation` message
        A current measurement from German state open data portals for one monitoring site. It carries water level and discharge observations when the upstream feed reports a new or refreshed value.
    ...
    def send_water_level_observation_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `DE.Waters.Hydrology.amqp.WaterLevelObservation` messages
