...
Deploy with `tools/deploy-fabric/deploy-fabric-aci.ps1 -Source german-waters -Workspace <id> -ResourceGroup <azure-rg> -Location <azure-region>` (the portal button wraps this for you). The script creates the Eventhouse, the KQL database with the [`kql/german_waters.kql`](kql/german_waters.kql) schema and update policies, the Event Stream with a custom endpoint, and the ACI with the connection string wired in.
