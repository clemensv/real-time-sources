# Energy_charts_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the energy_charts_mqtt_producer message definitions.

# Energy_charts_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - InfoEnergyChartsMqttEventDispatcher
...
from energy_charts_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = InfoEnergyChartsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### InfoEnergyChartsMqttEventDispatcher



# Publish message`InfoEnergyChartsMqttEventDispatcher` handles events for the info.energy_charts.mqtt message group.
...
from energy_charts_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = InfoEnergyChartsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe InfoEnergyChartsMqttEventDispatcher defines the following event handler hooks.
...
```##### `info_energy_charts_mqtt_public_power_async`



## Test```python

info_energy_charts_mqtt_public_power_async:  Callable[[PartitionContext, EventData, CloudEvent, PublicPower],
Awaitable[None]]
...
- `data`: The event data of type `energy_charts_mqtt_producer_data.PublicPower`.

Example:

```python
async def info_energy_charts_mqtt_public_power_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: PublicPower) -> None:
...
```##### `info_energy_charts_mqtt_spot_price_async`



## Test```python

info_energy_charts_mqtt_spot_price_async:  Callable[[PartitionContext, EventData, CloudEvent, SpotPrice],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `energy_charts_mqtt_producer_data.SpotPrice`.

Example:

```python
async def info_energy_charts_mqtt_spot_price_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: SpotPrice) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `info_energy_charts_mqtt_grid_signal_async`



## Test```python

info_energy_charts_mqtt_grid_signal_async:  Callable[[PartitionContext, EventData, CloudEvent, GridSignal],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `energy_charts_mqtt_producer_data.GridSignal`.

Example:

```python
async def info_energy_charts_mqtt_grid_signal_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: GridSignal) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `info_energy_charts_mqtt_info_async`



## Test```python

info_energy_charts_mqtt_info_async:  Callable[[PartitionContext, EventData, CloudEvent, Info], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `energy_charts_mqtt_producer_data.Info`.

Example:

```python
async def info_energy_charts_mqtt_info_event(partition_context: PartitionContext, event: EventData, cloud_event:
