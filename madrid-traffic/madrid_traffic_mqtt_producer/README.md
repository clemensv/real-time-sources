# Madrid_traffic_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the madrid_traffic_mqtt_producer message definitions.

# Madrid_traffic_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - EsMadridInformoMqttEventDispatcher
...
from madrid_traffic_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = EsMadridInformoMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### EsMadridInformoMqttEventDispatcher



# Publish message`EsMadridInformoMqttEventDispatcher` handles events for the es.madrid.informo.mqtt message group.
...
from madrid_traffic_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = EsMadridInformoMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe EsMadridInformoMqttEventDispatcher defines the following event handler hooks.
...
```##### `es_madrid_informo_measurement_point_mqtt_async`



## Test```python

es_madrid_informo_measurement_point_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, MeasurementPoint],
Awaitable[None]]
...
```Asynchronous handler hook for `es.madrid.informo.MeasurementPoint.mqtt`: A current transport measurement or status
update from Madrid open traffic sensor feeds. It carries traffic intensity and occupancy measurements when the upstream
...
- `data`: The event data of type `madrid_traffic_mqtt_producer_data.MeasurementPoint`.

Example:

```python
async def es_madrid_informo_measurement_point_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: MeasurementPoint) -> None:
...
```python
es_madrid_informo_mqtt_dispatcher.es_madrid_informo_measurement_point_mqtt_async =
es_madrid_informo_measurement_point_mqtt_event
```
...
```##### `es_madrid_informo_traffic_reading_mqtt_async`



## Test```python

es_madrid_informo_traffic_reading_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, TrafficReading],
Awaitable[None]]
...
```Asynchronous handler hook for `es.madrid.informo.TrafficReading.mqtt`: A current transport measurement or status
update from Madrid open traffic sensor feeds. It carries traffic intensity and occupancy measurements when the upstream
feed reports a new or refreshed value.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `madrid_traffic_mqtt_producer_data.TrafficReading`.

Example:

```python
async def es_madrid_informo_traffic_reading_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: TrafficReading) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
es_madrid_informo_mqtt_dispatcher.es_madrid_informo_traffic_reading_mqtt_async =
es_madrid_informo_traffic_reading_mqtt_event
```
