...
3. [Quick Start](#quick-start)    - EsMadridInformoEventDispatcher
...
- EsMadridInformoProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### EsMadridInformoEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`EsMadridInformoEventDispatcher` handles events for the es.madrid.informo message group.
...
producer = EsMadridInformoProducer(
...
await producer.send_es_madrid_informo_measurement_point(```python

    data=MeasurementPoint(...),add_consumer(self, consumer: KafkaConsumer)
...
The EsMadridInformoEventDispatcher defines the following event handler hooks.
...
producer = EsMadridInformoProducer(
    ...
    security_protocol='SASL_SSL',##### `es_madrid_informo_measurement_point_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'es_madrid_informo_measurement_point_async:  Callable[[ConsumerRecord, CloudEvent,
MeasurementPoint], Awaitable[None]]
...
### EsMadridInformoProducer- `data`: The event data of type `madrid_traffic_producer_data.MeasurementPoint`.



Producer for `es.madrid.informo` message group.Example:



#### Constructor```python

async def es_madrid_informo_measurement_point_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
MeasurementPoint) -> None:

```python    # Process the event data

EsMadridInformoProducer(    await some_processing_function(record, cloud_event, data)
...
es_madrid_informo_dispatcher.es_madrid_informo_measurement_point_async = es_madrid_informo_measurement_point_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `es_madrid_informo_traffic_reading_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'es_madrid_informo_traffic_reading_async:  Callable[[ConsumerRecord, CloudEvent,
TrafficReading], Awaitable[None]]
...
### EsMadridInformoProducer- `data`: The event data of type `madrid_traffic_producer_data.TrafficReading`.
...
async def es_madrid_informo_traffic_reading_event(record: ConsumerRecord, cloud_event: CloudEvent, data: TrafficReading)
-> None:
...
es_madrid_informo_dispatcher.es_madrid_informo_traffic_reading_async = es_madrid_informo_traffic_reading_event
...
##### `send_es_madrid_informo_measurement_point`Dispatchers have the following protected methods:



```python### Methods:

async def send_es_madrid_informo_measurement_point(
...
- `data`: Message data of type `MeasurementPoint`
...
await producer.send_es_madrid_informo_measurement_point(

    data=MeasurementPoint(...),Args:
...
Send multiple `es.madrid.informo.MeasurementPoint` messages in a batch.
...
await producer.send_es_madrid_informo_measurement_point_batch(```

    messages=[

        MeasurementPoint(...),Initializes the runner with a Kafka consumer.

        MeasurementPoint(...),

        MeasurementPoint(...)Args:
...
##### `send_es_madrid_informo_traffic_reading`Dispatchers have the following protected methods:



```python### Methods:

async def send_es_madrid_informo_traffic_reading(
...
- `data`: Message data of type `TrafficReading`
...
await producer.send_es_madrid_informo_traffic_reading(

    data=TrafficReading(...),Args:
...
Send multiple `es.madrid.informo.TrafficReading` messages in a batch.
...
await producer.send_es_madrid_informo_traffic_reading_batch(```

    messages=[

        TrafficReading(...),Initializes the runner with a Kafka consumer.
