...
class AUGovEmergencyWildfiresAmqpProducer:
    """
    Producer class to send messages in the `AU.Gov.Emergency.Wildfires.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_fire_incident(self,
        data: FireIncident,
        _state: str,
        _incident_id: str,
        content_type: str = 'application/json') -> None:
        """
        Send the `AU.Gov.Emergency.Wildfires.amqp.FireIncident` message
        ...
        attributes = {
            ...
            "source":
            "https://github.com/clemensv/real-time-sources/tree/main/australia-wildfires",
            "subject":
            "{state}/{incident_id}".format(state=_state, incident_id=_incident_id),
        }
        ...
        amqp_msg.subject = "{state}/{incident_id}".format(state=_state, incident_id=_incident_id)
    ...
    def send_fire_incident_batch(self,
        data_array: typing.List[FireIncident],
        _state: str,
        _incident_id: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `AU.Gov.Emergency.Wildfires.amqp.FireIncident` messages
        
        Args:
            data_array (typing.List[FireIncident]): Array of message data objects
            _state (str): Value for placeholder state in attribute subject
            _incident_id (str): Value for placeholder incident_id in attribute subject
            content_type (str): The content type of the message data
        """
        for data in data_array:
            self.send_fire_incident(
                data=data,
                _state=_state,
                _incident_id=_incident_id,
