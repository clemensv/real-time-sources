    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from nepal_bipad_hydrology_amqp_producer_data import RiverStation
from nepal_bipad_hydrology_amqp_producer_data import WaterLevelReading

class NpGovBipadHydrologyAmqpProducer:
    """
    Producer class to send messages in the `np.gov.bipad.hydrology.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_river_station(self,
        ...
        """
        Send the `np.gov.bipad.hydrology.amqp.RiverStation` message
        A reference record for one river monitoring stations in Nepal published by Nepal's BIPAD Portal. It fires when the bridge publishes or refreshes the station catalog so consumers can interpret measurement events.
    ...
    def send_river_station_batch(self,
        data_array: typing.List[RiverStation],
        _feedurl: str,
        _station_id: str,
        _basin: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `np.gov.bipad.hydrology.amqp.RiverStation` messages
        
        Args:
            data_array (typing.List[RiverStation]): Array of message data objects
            _feedurl (str): Value for placeholder feedurl in attribute source
            _station_id (str): Value for placeholder station_id in attribute subject
            _basin (str): Value for AMQP protocol option placeholder basin
    ...
    def send_water_level_reading(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send the `np.gov.bipad.hydrology.amqp.WaterLevelReading` message
        A current measurement from Nepal's BIPAD Portal for one monitoring site. It carries river water level observations when the upstream feed reports a new or refreshed value.
    ...
    def send_water_level_reading_batch(self,
        ...
        content_type: str = 'application/json') -> None:
        """
        Send multiple `np.gov.bipad.hydrology.amqp.WaterLevelReading` messages
