...
import os
import signal
from usgs_nwis_wq_mqtt_producer_data import *
from usgs_nwis_wq_mqtt_producer_mqtt_client.client import USGSWaterQualitySitesMqttProducer, USGSWaterQualitySitesMqttDispatcher

async def handle_usgs_water_quality_sites_mqtt_monitoring_site(mqtt_msg,cloud_event, usgs_water_quality_sites_mqtt_monitoring_site_data):
    """ Handles the USGS.WaterQuality.Sites.mqtt.MonitoringSite message """
    print(f"Received USGS.WaterQuality.Sites.mqtt.MonitoringSite on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {usgs_water_quality_sites_mqtt_monitoring_site_data}")
from usgs_nwis_wq_mqtt_producer_mqtt_client.client import USGSWaterQualityReadingsMqttProducer, USGSWaterQualityReadingsMqttDispatcher

async def handle_usgs_water_quality_readings_mqtt_water_quality_reading(mqtt_msg,cloud_event, usgs_water_quality_readings_mqtt_water_quality_reading_data):
    """ Handles the USGS.WaterQuality.Readings.mqtt.WaterQualityReading message """
    print(f"Received USGS.WaterQuality.Readings.mqtt.WaterQualityReading on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {usgs_water_quality_readings_mqtt_water_quality_reading_data}")
