# Nextbus_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the nextbus_mqtt_producer message definitions.

# Nextbus_mqtt_producer Event Dispatcher for Azure Event Hubs
...
from nextbus_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = NextbusMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NextbusMqttEventDispatcher



# Publish message`NextbusMqttEventDispatcher` handles events for the nextbus.mqtt message group.
...
from nextbus_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = NextbusMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe NextbusMqttEventDispatcher defines the following event handler hooks.
...
```##### `nextbus_vehicle_position_mqtt_async`



## Test```python

nextbus_vehicle_position_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, VehiclePosition],
Awaitable[None]]
...
```Asynchronous handler hook for `nextbus.VehiclePosition.mqtt`: Nextbus VehiclePosition event.
...
- `data`: The event data of type `nextbus_mqtt_producer_data.VehiclePosition`.

Example:

```python
async def nextbus_vehicle_position_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: VehiclePosition) -> None:
...
```python
nextbus_mqtt_dispatcher.nextbus_vehicle_position_mqtt_async = nextbus_vehicle_position_mqtt_event
```
...
```##### `nextbus_route_config_mqtt_async`



## Test```python

nextbus_route_config_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RouteConfig], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `nextbus.RouteConfig.mqtt`: Nextbus RouteConfig event.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `nextbus_mqtt_producer_data.RouteConfig`.

Example:

```python
async def nextbus_route_config_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: RouteConfig) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
nextbus_mqtt_dispatcher.nextbus_route_config_mqtt_async = nextbus_route_config_mqtt_event
```
...
```##### `nextbus_schedule_mqtt_async`



## Test```python

nextbus_schedule_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, Schedule], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `nextbus.Schedule.mqtt`: Nextbus Schedule event.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `nextbus_mqtt_producer_data.Schedule`.

Example:

```python
async def nextbus_schedule_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: Schedule) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
nextbus_mqtt_dispatcher.nextbus_schedule_mqtt_async = nextbus_schedule_mqtt_event
```
...
```##### `nextbus_message_mqtt_async`



## Test```python

nextbus_message_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, Message], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `nextbus.Message.mqtt`: Nextbus Message event.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `nextbus_mqtt_producer_data.Message`.

Example:

```python
async def nextbus_message_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent,
data: Message) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
nextbus_mqtt_dispatcher.nextbus_message_mqtt_async = nextbus_message_mqtt_event
```
