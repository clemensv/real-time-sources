# Imgw_hydro_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the imgw_hydro_mqtt_producer message definitions.

# Imgw_hydro_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - PLGovIMGWHydroMqttEventDispatcher
...
from imgw_hydro_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = PLGovIMGWHydroMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### PLGovIMGWHydroMqttEventDispatcher



# Publish message`PLGovIMGWHydroMqttEventDispatcher` handles events for the PL.Gov.IMGW.Hydro.mqtt message group.
...
from imgw_hydro_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = PLGovIMGWHydroMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe PLGovIMGWHydroMqttEventDispatcher defines the following event handler hooks.
...
```##### `pl_gov_imgw_hydro_mqtt_station_async`



## Test```python

pl_gov_imgw_hydro_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `imgw_hydro_mqtt_producer_data.Station`.

Example:

```python
async def pl_gov_imgw_hydro_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `pl_gov_imgw_hydro_mqtt_water_level_observation_async`



## Test```python

pl_gov_imgw_hydro_mqtt_water_level_observation_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `imgw_hydro_mqtt_producer_data.WaterLevelObservation`.

Example:

```python
async def pl_gov_imgw_hydro_mqtt_water_level_observation_event(partition_context: PartitionContext, event: EventData,
