...
# Imgw_hydro_producer Kafka Producer# Imgw_hydro_producer Event Dispatcher for Apache Kafka
...
- **Scales horizontally** across multiple brokers and partitions### PLGovIMGWHydroEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`PLGovIMGWHydroEventDispatcher` handles events for the PL.Gov.IMGW.Hydro message group.
...
The PLGovIMGWHydroEventDispatcher defines the following event handler hooks.
...
producer = PLGovIMGWHydroProducer(
    ...
    security_protocol='SASL_SSL',##### `pl_gov_imgw_hydro_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'pl_gov_imgw_hydro_station_async:  Callable[[ConsumerRecord, CloudEvent, Station],
Awaitable[None]]
...
pl_gov_imgw_hydro_dispatcher.pl_gov_imgw_hydro_station_async = pl_gov_imgw_hydro_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `pl_gov_imgw_hydro_water_level_observation_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'pl_gov_imgw_hydro_water_level_observation_async:  Callable[[ConsumerRecord, CloudEvent,
...
pl_gov_imgw_hydro_dispatcher.pl_gov_imgw_hydro_water_level_observation_async =
...
##### `send_pl_gov_imgw_hydro_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_pl_gov_imgw_hydro_station(
...
await producer.send_pl_gov_imgw_hydro_station(
...
Send multiple `PL.Gov.IMGW.Hydro.Station` messages in a batch.
...
await producer.send_pl_gov_imgw_hydro_station_batch(```
...
##### `send_pl_gov_imgw_hydro_water_level_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_pl_gov_imgw_hydro_water_level_observation(
...
await producer.send_pl_gov_imgw_hydro_water_level_observation(
...
Send multiple `PL.Gov.IMGW.Hydro.WaterLevelObservation` messages in a batch.
