...
import os
import signal
from bafu_hydro_mqtt_producer_data import *
from bafu_hydro_mqtt_producer_mqtt_client.client import CHBAFUHydrologyMqttProducer, CHBAFUHydrologyMqttDispatcher

async def handle_ch_bafu_hydrology_mqtt_station(mqtt_msg,cloud_event, ch_bafu_hydrology_mqtt_station_data):
    """ Handles the CH.BAFU.Hydrology.mqtt.Station message """
    print(f"Received CH.BAFU.Hydrology.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {ch_bafu_hydrology_mqtt_station_data}")

async def handle_ch_bafu_hydrology_mqtt_water_level_observation(mqtt_msg,cloud_event, ch_bafu_hydrology_mqtt_water_level_observation_data):
    """ Handles the CH.BAFU.Hydrology.mqtt.WaterLevelObservation message """
    print(f"Received CH.BAFU.Hydrology.mqtt.WaterLevelObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {ch_bafu_hydrology_mqtt_water_level_observation_data}")
