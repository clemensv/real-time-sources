# Bafu_hydro_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the bafu_hydro_mqtt_producer message definitions.

# Bafu_hydro_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - CHBAFUHydrologyMqttEventDispatcher
...
from bafu_hydro_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = CHBAFUHydrologyMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### CHBAFUHydrologyMqttEventDispatcher



# Publish message`CHBAFUHydrologyMqttEventDispatcher` handles events for the CH.BAFU.Hydrology.mqtt message group.
...
from bafu_hydro_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = CHBAFUHydrologyMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe CHBAFUHydrologyMqttEventDispatcher defines the following event handler hooks.
...
```##### `ch_bafu_hydrology_mqtt_station_async`



## Test```python

ch_bafu_hydrology_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `bafu_hydro_mqtt_producer_data.Station`.

Example:

```python
async def ch_bafu_hydrology_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `ch_bafu_hydrology_mqtt_water_level_observation_async`



## Test```python

ch_bafu_hydrology_mqtt_water_level_observation_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `bafu_hydro_mqtt_producer_data.WaterLevelObservation`.

Example:

```python
async def ch_bafu_hydrology_mqtt_water_level_observation_event(partition_context: PartitionContext, event: EventData,
