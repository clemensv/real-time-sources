# Eurdep_radiation_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the eurdep_radiation_mqtt_producer message definitions.

# Eurdep_radiation_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - EuJrcEurdepMqttEventDispatcher
...
from eurdep_radiation_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = EuJrcEurdepMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### EuJrcEurdepMqttEventDispatcher



# Publish message`EuJrcEurdepMqttEventDispatcher` handles events for the eu.jrc.eurdep.mqtt message group.
...
from eurdep_radiation_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = EuJrcEurdepMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe EuJrcEurdepMqttEventDispatcher defines the following event handler hooks.
...
```##### `eu_jrc_eurdep_mqtt_station_async`



## Test```python

eu_jrc_eurdep_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `eu.jrc.eurdep.mqtt.Station`: Reference metadata for an ambient gamma dose rate
monitoring station in the EURDEP (European Radiological Data Exchange Platform) network. EURDEP aggregates data from
approximately 5,500 stations across 39 European countries. Each station continuously measures ambient gamma dose rate
and reports hourly averaged values. Station metadata includes the geographic position, elevation above sea level,
country of origin, and operational status.
...
- `data`: The event data of type `eurdep_radiation_mqtt_producer_data.Station`.

Example:

```python
async def eu_jrc_eurdep_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```python
eu_jrc_eurdep_mqtt_dispatcher.eu_jrc_eurdep_mqtt_station_async = eu_jrc_eurdep_mqtt_station_event
```
...
```##### `eu_jrc_eurdep_mqtt_dose_rate_reading_async`



## Test```python

eu_jrc_eurdep_mqtt_dose_rate_reading_async:  Callable[[PartitionContext, EventData, CloudEvent, DoseRateReading],
Awaitable[None]]
...
```Asynchronous handler hook for `eu.jrc.eurdep.mqtt.DoseRateReading`: An ambient gamma dose rate reading from a EURDEP
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `eurdep_radiation_mqtt_producer_data.DoseRateReading`.

Example:

```python
async def eu_jrc_eurdep_mqtt_dose_rate_reading_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: DoseRateReading) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
eu_jrc_eurdep_mqtt_dispatcher.eu_jrc_eurdep_mqtt_dose_rate_reading_async = eu_jrc_eurdep_mqtt_dose_rate_reading_event
```
