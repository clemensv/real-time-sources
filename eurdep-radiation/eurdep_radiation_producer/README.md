...
# Eurdep_radiation_producer Kafka Producer# Eurdep_radiation_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - EuJrcEurdepEventDispatcher
...
- EuJrcEurdepProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### EuJrcEurdepEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`EuJrcEurdepEventDispatcher` handles events for the eu.jrc.eurdep message group.
...
from eurdep_radiation_producer import EuJrcEurdepProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = EuJrcEurdepProducer(
...
await producer.send_eu_jrc_eurdep_station(```python
...
The EuJrcEurdepEventDispatcher defines the following event handler hooks.
...
producer = EuJrcEurdepProducer(
    ...
    security_protocol='SASL_SSL',##### `eu_jrc_eurdep_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'eu_jrc_eurdep_station_async:  Callable[[ConsumerRecord, CloudEvent, Station],
Awaitable[None]]
...
Asynchronous handler hook for `eu.jrc.eurdep.Station`: Reference metadata for an ambient gamma dose rate monitoring
station in the EURDEP (European Radiological Data Exchange Platform) network. EURDEP aggregates data from approximately
5,500 stations across 39 European countries. Each station continuously measures ambient gamma dose rate and reports
hourly averaged values. Station metadata includes the geographic position, elevation above sea level, country of origin,
...
### EuJrcEurdepProducer- `data`: The event data of type `eurdep_radiation_producer_data.Station`.



Producer for `eu.jrc.eurdep` message group.Example:



#### Constructor```python

async def eu_jrc_eurdep_station_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Station) -> None:

```python    # Process the event data

EuJrcEurdepProducer(    await some_processing_function(record, cloud_event, data)
...
eu_jrc_eurdep_dispatcher.eu_jrc_eurdep_station_async = eu_jrc_eurdep_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `eu_jrc_eurdep_dose_rate_reading_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'eu_jrc_eurdep_dose_rate_reading_async:  Callable[[ConsumerRecord, CloudEvent,
DoseRateReading], Awaitable[None]]

)```

```

Asynchronous handler hook for `eu.jrc.eurdep.DoseRateReading`: An ambient gamma dose rate reading from a EURDEP
monitoring station. Each reading reports the gross gamma dose rate in microsieverts per hour (µSv/h) averaged over a
one-hour measurement window. Readings include validation status, the nuclide type measured, measurement duration, and
the analysis time range. Normal European background levels range from approximately 0.04 to 0.20 µSv/h depending on
local geology and altitude.
...
### EuJrcEurdepProducer- `data`: The event data of type `eurdep_radiation_producer_data.DoseRateReading`.
...
async def eu_jrc_eurdep_dose_rate_reading_event(record: ConsumerRecord, cloud_event: CloudEvent, data: DoseRateReading)
-> None:
...
eu_jrc_eurdep_dispatcher.eu_jrc_eurdep_dose_rate_reading_async = eu_jrc_eurdep_dose_rate_reading_event
...
##### `send_eu_jrc_eurdep_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_eu_jrc_eurdep_station(
...
Send a single `eu.jrc.eurdep.Station` message. Reference metadata for an ambient gamma dose rate monitoring station in
the EURDEP (European Radiological Data Exchange Platform) network. EURDEP aggregates data from approximately 5,500
stations across 39 European countries. Each station continuously measures ambient gamma dose rate and reports hourly
averaged values. Station metadata includes the geographic position, elevation above sea level, country of origin, and
operational status.Args:
...
await producer.send_eu_jrc_eurdep_station(
...
Send multiple `eu.jrc.eurdep.Station` messages in a batch.
...
await producer.send_eu_jrc_eurdep_station_batch(```
...
##### `send_eu_jrc_eurdep_dose_rate_reading`Dispatchers have the following protected methods:



```python### Methods:

async def send_eu_jrc_eurdep_dose_rate_reading(

    self,##### `_process_event`

    data: DoseRateReading,
...
Send a single `eu.jrc.eurdep.DoseRateReading` message. An ambient gamma dose rate reading from a EURDEP monitoring
station. Each reading reports the gross gamma dose rate in microsieverts per hour (µSv/h) averaged over a one-hour
measurement window. Readings include validation status, the nuclide type measured, measurement duration, and the
analysis time range. Normal European background levels range from approximately 0.04 to 0.20 µSv/h depending on local
geology and altitude.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `DoseRateReading`
...
await producer.send_eu_jrc_eurdep_dose_rate_reading(

    data=DoseRateReading(...),Args:
...
Send multiple `eu.jrc.eurdep.DoseRateReading` messages in a batch.
...
await producer.send_eu_jrc_eurdep_dose_rate_reading_batch(```

    messages=[

        DoseRateReading(...),Initializes the runner with a Kafka consumer.

        DoseRateReading(...),

        DoseRateReading(...)Args:
