"""AMQP companion feeder for ticketmaster. Publishes mock/sample CloudEvents for Docker E2E and auth-blocked upstreams."""
from __future__ import annotations
import argparse, inspect, json, os, pathlib, re
from datetime import datetime, timezone
from urllib.parse import urlparse
from proton import symbol
from ticketmaster_amqp_producer_amqp_producer import producer as producer_mod
import ticketmaster_amqp_producer_data as data_mod
...
def _val(name, typ):
    ...
    if 'country' in n: return 'gb' if 'ticketmaster'=='elexon-bmrs' else ('jp' if 'ticketmaster'=='tepco-denkiyoho' else 'dk' if 'ticketmaster'=='energidataservice-dk' else 'de' if 'ticketmaster'=='energy-charts' else 'nl')
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('feed', nargs='?', default='feed'); ap.add_argument('--broker-url', default=os.getenv('AMQP_BROKER_URL')); ap.add_argument('--host', default=os.getenv('AMQP_HOST')); ap.add_argument('--port', type=int, default=int(os.getenv('AMQP_PORT','0')) or None); ap.add_argument('--address', default=os.getenv('AMQP_ADDRESS','ticketmaster')); ap.add_argument('--username', default=os.getenv('AMQP_USERNAME')); ap.add_argument('--password', default=os.getenv('AMQP_PASSWORD')); ap.add_argument('--tls', action='store_true', default=os.getenv('AMQP_TLS','').lower() in ('1','true','yes')); ap.add_argument('--auth-mode', default=os.getenv('AMQP_AUTH_MODE','password')); args=ap.parse_args()
