# Ticketmaster_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the ticketmaster_mqtt_producer message definitions.

# Ticketmaster_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - TicketmasterEventsMqttEventDispatcher,

    TicketmasterReferenceMqttEventDispatcher
...
from ticketmaster_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = TicketmasterEventsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### TicketmasterEventsMqttEventDispatcher



# Publish message`TicketmasterEventsMqttEventDispatcher` handles events for the Ticketmaster.Events.mqtt message group.
...
from ticketmaster_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = TicketmasterEventsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
client = TicketmasterReferenceMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe TicketmasterEventsMqttEventDispatcher defines the following event handler hooks.
...
```##### `ticketmaster_events_mqtt_event_async`



## Test```python

ticketmaster_events_mqtt_event_async:  Callable[[PartitionContext, EventData, CloudEvent, Event], Awaitable[None]]
...
- `data`: The event data of type `ticketmaster_mqtt_producer_data.Event`.

Example:

```python
async def ticketmaster_events_mqtt_event_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Event) -> None:
...
client = TicketmasterReferenceMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### TicketmasterReferenceMqttEventDispatcher



# Publish message`TicketmasterReferenceMqttEventDispatcher` handles events for the Ticketmaster.Reference.mqtt message
group.
...
## BuildThe TicketmasterReferenceMqttEventDispatcher defines the following event handler hooks.
...
```##### `ticketmaster_reference_mqtt_venue_async`



## Test```python

ticketmaster_reference_mqtt_venue_async:  Callable[[PartitionContext, EventData, CloudEvent, Venue], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `Ticketmaster.Reference.mqtt.Venue`: Reference data for a Ticketmaster venue. Emitted
at bridge startup and refreshed periodically. Carries the stable venue identifier, location, address, timezone, and
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ticketmaster_mqtt_producer_data.Venue`.

Example:

```python
async def ticketmaster_reference_mqtt_venue_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Venue) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `ticketmaster_reference_mqtt_attraction_async`



## Test```python

ticketmaster_reference_mqtt_attraction_async:  Callable[[PartitionContext, EventData, CloudEvent, Attraction],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ticketmaster_mqtt_producer_data.Attraction`.

Example:

```python
async def ticketmaster_reference_mqtt_attraction_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: Attraction) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `ticketmaster_reference_mqtt_classification_async`



## Test```python

ticketmaster_reference_mqtt_classification_async:  Callable[[PartitionContext, EventData, CloudEvent, Classification],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ticketmaster_mqtt_producer_data.Classification`.

Example:

```python
async def ticketmaster_reference_mqtt_classification_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: Classification) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `ticketmaster_reference_mqtt_info_async`



## Test```python

ticketmaster_reference_mqtt_info_async:  Callable[[PartitionContext, EventData, CloudEvent, Info], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `ticketmaster_mqtt_producer_data.Info`.

Example:

```python
async def ticketmaster_reference_mqtt_info_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Info) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
