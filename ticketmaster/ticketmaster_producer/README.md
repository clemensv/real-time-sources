...
- TicketmasterEventsProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### TicketmasterEventsEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`TicketmasterEventsEventDispatcher` handles events for the Ticketmaster.Events message group.
...
The TicketmasterEventsEventDispatcher defines the following event handler hooks.
...
producer = TicketmasterEventsProducer(
    ...
    security_protocol='SASL_SSL',##### `ticketmaster_events_event_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'ticketmaster_events_event_async:  Callable[[ConsumerRecord, CloudEvent, Event],
Awaitable[None]]
...
Asynchronous handler hook for `Ticketmaster.Events.Event`: A Ticketmaster event representing a concert, sports match,
...
ticketmaster_events_dispatcher.ticketmaster_events_event_async = ticketmaster_events_event_event
...
##### `send_ticketmaster_events_event`Dispatchers have the following protected methods:



```python### Methods:

async def send_ticketmaster_events_event(
...
Send a single `Ticketmaster.Events.Event` message. A Ticketmaster event representing a concert, sports match, theater
performance, or other live public event. Emitted when a new event is discovered or when an existing event's status or
schedule changes.Args:
...
await producer.send_ticketmaster_events_event(
...
Send multiple `Ticketmaster.Events.Event` messages in a batch.
...
await producer.send_ticketmaster_events_event_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### TicketmasterReferenceEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`TicketmasterReferenceEventDispatcher` handles events for the Ticketmaster.Reference message group.
...
The TicketmasterReferenceEventDispatcher defines the following event handler hooks.
...
producer = TicketmasterEventsProducer(
    ...
    security_protocol='SASL_SSL',##### `ticketmaster_reference_venue_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'ticketmaster_reference_venue_async:  Callable[[ConsumerRecord, CloudEvent, Venue],
Awaitable[None]]
...
Asynchronous handler hook for `Ticketmaster.Reference.Venue`: Reference data for a Ticketmaster venue. Emitted at bridge
...
ticketmaster_events_dispatcher.ticketmaster_reference_venue_async = ticketmaster_reference_venue_event
...
ticketmaster_reference_dispatcher.ticketmaster_reference_venue_async = ticketmaster_reference_venue_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `ticketmaster_reference_attraction_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'ticketmaster_reference_attraction_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `Ticketmaster.Reference.Attraction`: Reference data for a Ticketmaster attraction
...
ticketmaster_events_dispatcher.ticketmaster_reference_attraction_async = ticketmaster_reference_attraction_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `ticketmaster_reference_classification_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'ticketmaster_reference_classification_async:  Callable[[ConsumerRecord, CloudEvent,
Classification], Awaitable[None]]

)```

```

Asynchronous handler hook for `Ticketmaster.Reference.Classification`: Reference data for a Ticketmaster classification
segment (e.g. Music, Sports, Arts & Theatre). Emitted at bridge startup and refreshed daily. Carries the stable segment
identifier, name, and the primary genre and subgenre under that segment.
...
ticketmaster_events_dispatcher.ticketmaster_reference_classification_async = ticketmaster_reference_classification_event
...
##### `send_ticketmaster_reference_venue`Dispatchers have the following protected methods:



```python### Methods:

async def send_ticketmaster_reference_venue(
...
Send a single `Ticketmaster.Reference.Venue` message. Reference data for a Ticketmaster venue. Emitted at bridge startup
and refreshed periodically. Carries the stable venue identifier, location, address, timezone, and capacity
information.Args:
...
await producer.send_ticketmaster_reference_venue(
...
Send multiple `Ticketmaster.Reference.Venue` messages in a batch.
...
await producer.send_ticketmaster_reference_venue_batch(```
...
##### `send_ticketmaster_reference_attraction`Dispatchers have the following protected methods:



```python### Methods:

async def send_ticketmaster_reference_attraction(
...
Send a single `Ticketmaster.Reference.Attraction` message. Reference data for a Ticketmaster attraction (performer,
...
await producer.send_ticketmaster_reference_attraction(
...
Send multiple `Ticketmaster.Reference.Attraction` messages in a batch.
...
await producer.send_ticketmaster_reference_attraction_batch(```
...
##### `send_ticketmaster_reference_classification`Dispatchers have the following protected methods:



```python### Methods:

async def send_ticketmaster_reference_classification(
...
Send a single `Ticketmaster.Reference.Classification` message. Reference data for a Ticketmaster classification segment
(e.g. Music, Sports, Arts & Theatre). Emitted at bridge startup and refreshed daily. Carries the stable segment
...
await producer.send_ticketmaster_reference_classification(
...
Send multiple `Ticketmaster.Reference.Classification` messages in a batch.
...
await producer.send_ticketmaster_reference_classification_batch(```
