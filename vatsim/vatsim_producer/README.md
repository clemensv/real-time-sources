...
# Vatsim_producer Kafka Producer# Vatsim_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - NetVatsimPilotsEventDispatcher,
...
- NetVatsimPilotsProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- NetVatsimControllersProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### NetVatsimPilotsEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`NetVatsimPilotsEventDispatcher` handles events for the net.vatsim.pilots message group.
...
The NetVatsimPilotsEventDispatcher defines the following event handler hooks.
...
producer = NetVatsimPilotsProducer(
    ...
    security_protocol='SASL_SSL',##### `net_vatsim_pilot_position_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'net_vatsim_pilot_position_async:  Callable[[ConsumerRecord, CloudEvent, PilotPosition],
Awaitable[None]]
...
Asynchronous handler hook for `net.vatsim.PilotPosition`: Current position, flight plan summary, and state of a pilot
connected to the VATSIM virtual aviation network.
...
net_vatsim_pilots_dispatcher.net_vatsim_pilot_position_async = net_vatsim_pilot_position_event
...
net_vatsim_controllers_dispatcher.net_vatsim_pilot_position_async = net_vatsim_pilot_position_event
...
##### `send_net_vatsim_pilot_position`Dispatchers have the following protected methods:



```python### Methods:

async def send_net_vatsim_pilot_position(
...
Send a single `net.vatsim.PilotPosition` message. Current position, flight plan summary, and state of a pilot connected
to the VATSIM virtual aviation network.Args:
...
await producer.send_net_vatsim_pilot_position(
...
Send multiple `net.vatsim.PilotPosition` messages in a batch.
...
await producer.send_net_vatsim_pilot_position_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### NetVatsimControllersEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`NetVatsimControllersEventDispatcher` handles events for the net.vatsim.controllers message group.
...
The NetVatsimControllersEventDispatcher defines the following event handler hooks.
...
producer = NetVatsimPilotsProducer(
    ...
    security_protocol='SASL_SSL',##### `net_vatsim_controller_position_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'net_vatsim_controller_position_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `net.vatsim.ControllerPosition`: Current state and frequency of an air traffic controller
connected to the VATSIM virtual aviation network.
...
net_vatsim_pilots_dispatcher.net_vatsim_controller_position_async = net_vatsim_controller_position_event
...
net_vatsim_controllers_dispatcher.net_vatsim_controller_position_async = net_vatsim_controller_position_event
...
net_vatsim_status_dispatcher.net_vatsim_controller_position_async = net_vatsim_controller_position_event
...
##### `send_net_vatsim_controller_position`Dispatchers have the following protected methods:



```python### Methods:

async def send_net_vatsim_controller_position(
...
Send a single `net.vatsim.ControllerPosition` message. Current state and frequency of an air traffic controller
...
await producer.send_net_vatsim_controller_position(
...
Send multiple `net.vatsim.ControllerPosition` messages in a batch.
...
await producer.send_net_vatsim_controller_position_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### NetVatsimStatusEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`NetVatsimStatusEventDispatcher` handles events for the net.vatsim.status message group.
...
The NetVatsimStatusEventDispatcher defines the following event handler hooks.
...
producer = NetVatsimPilotsProducer(
    ...
    security_protocol='SASL_SSL',##### `net_vatsim_network_status_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'net_vatsim_network_status_async:  Callable[[ConsumerRecord, CloudEvent, NetworkStatus],
Awaitable[None]]
...
Asynchronous handler hook for `net.vatsim.NetworkStatus`: Aggregate network status snapshot from the VATSIM data feed,
emitted once per poll cycle.
...
net_vatsim_pilots_dispatcher.net_vatsim_network_status_async = net_vatsim_network_status_event
...
net_vatsim_controllers_dispatcher.net_vatsim_network_status_async = net_vatsim_network_status_event
...
net_vatsim_status_dispatcher.net_vatsim_network_status_async = net_vatsim_network_status_event
...
##### `send_net_vatsim_network_status`Dispatchers have the following protected methods:



```python### Methods:

async def send_net_vatsim_network_status(
...
Send a single `net.vatsim.NetworkStatus` message. Aggregate network status snapshot from the VATSIM data feed, emitted
once per poll cycle.Args:
...
await producer.send_net_vatsim_network_status(
...
Send multiple `net.vatsim.NetworkStatus` messages in a batch.
