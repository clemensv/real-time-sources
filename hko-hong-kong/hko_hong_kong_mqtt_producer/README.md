# Hko_hong_kong_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the hko_hong_kong_mqtt_producer message definitions.

# Hko_hong_kong_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - HKGovHKOWeatherMqttEventDispatcher
...
from hko_hong_kong_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = HKGovHKOWeatherMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### HKGovHKOWeatherMqttEventDispatcher



# Publish message`HKGovHKOWeatherMqttEventDispatcher` handles events for the HK.Gov.HKO.Weather.mqtt message group.
...
from hko_hong_kong_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = HKGovHKOWeatherMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe HKGovHKOWeatherMqttEventDispatcher defines the following event handler hooks.
...
```##### `hk_gov_hko_weather_mqtt_station_async`



## Test```python

hk_gov_hko_weather_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `hko_hong_kong_mqtt_producer_data.Station`.

Example:

```python
async def hk_gov_hko_weather_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `hk_gov_hko_weather_mqtt_weather_observation_async`



## Test```python

hk_gov_hko_weather_mqtt_weather_observation_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```Asynchronous handler hook for `HK.Gov.HKO.Weather.mqtt.WeatherObservation`: A current environmental measurement from
the Hong Kong Observatory. It carries weather observations when the upstream feed reports a new or refreshed value.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `hko_hong_kong_mqtt_producer_data.WeatherObservation`.

Example:

```python
async def hk_gov_hko_weather_mqtt_weather_observation_event(partition_context: PartitionContext, event: EventData,
