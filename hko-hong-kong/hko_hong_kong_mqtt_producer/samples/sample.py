...
import os
import signal
from hko_hong_kong_mqtt_producer_data import *
from hko_hong_kong_mqtt_producer_mqtt_client.client import HKGovHKOWeatherMqttProducer, HKGovHKOWeatherMqttDispatcher

async def handle_hk_gov_hko_weather_mqtt_station(mqtt_msg,cloud_event, hk_gov_hko_weather_mqtt_station_data):
    """ Handles the HK.Gov.HKO.Weather.mqtt.Station message """
    print(f"Received HK.Gov.HKO.Weather.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {hk_gov_hko_weather_mqtt_station_data}")

async def handle_hk_gov_hko_weather_mqtt_weather_observation(mqtt_msg,cloud_event, hk_gov_hko_weather_mqtt_weather_observation_data):
    """ Handles the HK.Gov.HKO.Weather.mqtt.WeatherObservation message """
    print(f"Received HK.Gov.HKO.Weather.mqtt.WeatherObservation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {hk_gov_hko_weather_mqtt_weather_observation_data}")
