...
import hko_hong_kong_mqtt_producer_data
from hko_hong_kong_mqtt_producer_data import Station
from hko_hong_kong_mqtt_producer_data import WeatherObservation
...
def get_default_topic_mappings_hk_gov_hko_weather_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the HK.Gov.HKO.Weather.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "HK.Gov.HKO.Weather.mqtt.Station": "weather/hk/hko/hko-hong-kong/{district}/{place_id}/info",
        "HK.Gov.HKO.Weather.mqtt.WeatherObservation": "weather/hk/hko/hko-hong-kong/{district}/{place_id}/observation",
    }


def get_subscription_topics_hk_gov_hko_weather_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_hk_gov_hko_weather_mqtt()
    topics = []
...
class HKGovHKOWeatherMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the HK.Gov.HKO.Weather.mqtt message group."""
    ...
    def __init__(
        ...
        self.hk_gov_hko_weather_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, hko_hong_kong_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.hk_gov_hko_weather_mqtt_weather_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, hko_hong_kong_mqtt_producer_data.WeatherObservation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "HK.Gov.HKO.Weather.Station":
            if self.hk_gov_hko_weather_mqtt_station_async:
                try:
                    ...
                    data = hko_hong_kong_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "HK.Gov.HKO.Weather.mqtt.Station")
                    await self.hk_gov_hko_weather_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in hk_gov_hko_weather_mqtt_station handler: {e}")
            return
        
        if event_type == "HK.Gov.HKO.Weather.WeatherObservation":
            if self.hk_gov_hko_weather_mqtt_weather_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = hko_hong_kong_mqtt_producer_data.WeatherObservation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "HK.Gov.HKO.Weather.mqtt.WeatherObservation")
                    await self.hk_gov_hko_weather_mqtt_weather_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in hk_gov_hko_weather_mqtt_weather_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_hk_gov_hko_weather_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_hk_gov_hko_weather_mqtt_station(self,
        place_id: str,
        district: str,
        data: hko_hong_kong_mqtt_producer_data.Station,
        ...
        Args:
            ...
            place_id: URI template variable for 'place_id'
            district: URI template variable for 'district'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/hk/hko/hko-hong-kong/{district}/{place_id}/info'
    ...
    async def publish_hk_gov_hko_weather_mqtt_weather_observation(self,
        place_id: str,
        district: str,
        data: hko_hong_kong_mqtt_producer_data.WeatherObservation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        ...
        Args:
            ...
            district: URI template variable for 'district'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'weather/hk/hko/hko-hong-kong/{district}/{place_id}/observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
