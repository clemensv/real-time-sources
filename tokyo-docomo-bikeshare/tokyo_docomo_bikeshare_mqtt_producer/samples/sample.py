...
import os
import signal
from tokyo_docomo_bikeshare_mqtt_producer_data import *
from tokyo_docomo_bikeshare_mqtt_producer_mqtt_client.client import JPODPTDocomoBikeshareSystemMqttProducer, JPODPTDocomoBikeshareSystemMqttDispatcher

async def handle_jp_odpt_docomo_bikeshare_bikeshare_system_mqtt(mqtt_msg,cloud_event, jp_odpt_docomo_bikeshare_bikeshare_system_mqtt_data):
    """ Handles the JP.ODPT.DocomoBikeshare.BikeshareSystem.mqtt message """
    print(f"Received JP.ODPT.DocomoBikeshare.BikeshareSystem.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_odpt_docomo_bikeshare_bikeshare_system_mqtt_data}")
from tokyo_docomo_bikeshare_mqtt_producer_mqtt_client.client import JPODPTDocomoBikeshareStationsMqttProducer, JPODPTDocomoBikeshareStationsMqttDispatcher

async def handle_jp_odpt_docomo_bikeshare_bikeshare_station_mqtt(mqtt_msg,cloud_event, jp_odpt_docomo_bikeshare_bikeshare_station_mqtt_data):
    """ Handles the JP.ODPT.DocomoBikeshare.BikeshareStation.mqtt message """
    print(f"Received JP.ODPT.DocomoBikeshare.BikeshareStation.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_odpt_docomo_bikeshare_bikeshare_station_mqtt_data}")

async def handle_jp_odpt_docomo_bikeshare_bikeshare_station_status_mqtt(mqtt_msg,cloud_event, jp_odpt_docomo_bikeshare_bikeshare_station_status_mqtt_data):
    """ Handles the JP.ODPT.DocomoBikeshare.BikeshareStationStatus.mqtt message """
    print(f"Received JP.ODPT.DocomoBikeshare.BikeshareStationStatus.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_odpt_docomo_bikeshare_bikeshare_station_status_mqtt_data}")
