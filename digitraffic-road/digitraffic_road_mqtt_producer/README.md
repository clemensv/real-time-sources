# Digitraffic_road_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the digitraffic_road_mqtt_producer message definitions.

# Digitraffic_road_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - FiDigitrafficRoadMqttEventDispatcher
...
from digitraffic_road_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = FiDigitrafficRoadMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### FiDigitrafficRoadMqttEventDispatcher



# Publish message`FiDigitrafficRoadMqttEventDispatcher` handles events for the fi.digitraffic.road.mqtt message group.
...
from digitraffic_road_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = FiDigitrafficRoadMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe FiDigitrafficRoadMqttEventDispatcher defines the following event handler hooks.
...
```##### `fi_digitraffic_road_mqtt_tms_sensor_data_async`



## Test```python

fi_digitraffic_road_mqtt_tms_sensor_data_async:  Callable[[PartitionContext, EventData, CloudEvent, TmsSensorData],
Awaitable[None]]
...
```Asynchronous handler hook for `fi.digitraffic.road.mqtt.TmsSensorData`: A transport update from Fintraffic
...
- `data`: The event data of type `digitraffic_road_mqtt_producer_data.TmsSensorData`.

Example:

```python
async def fi_digitraffic_road_mqtt_tms_sensor_data_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: TmsSensorData) -> None:
...
```##### `fi_digitraffic_road_mqtt_weather_sensor_data_async`



## Test```python

fi_digitraffic_road_mqtt_weather_sensor_data_async:  Callable[[PartitionContext, EventData, CloudEvent,
WeatherSensorData], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `digitraffic_road_mqtt_producer_data.WeatherSensorData`.

Example:

```python
async def fi_digitraffic_road_mqtt_weather_sensor_data_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: WeatherSensorData) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `fi_digitraffic_road_mqtt_traffic_announcement_async`



## Test```python

fi_digitraffic_road_mqtt_traffic_announcement_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```Asynchronous handler hook for `fi.digitraffic.road.mqtt.TrafficAnnouncement`: A current transport measurement or
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `digitraffic_road_mqtt_producer_data.TrafficMessage`.

Example:

```python
async def fi_digitraffic_road_mqtt_traffic_announcement_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: TrafficMessage) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `fi_digitraffic_road_mqtt_road_work_async`



## Test```python

fi_digitraffic_road_mqtt_road_work_async:  Callable[[PartitionContext, EventData, CloudEvent, TrafficMessage],
Awaitable[None]]
...
```python
async def fi_digitraffic_road_mqtt_road_work_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: TrafficMessage) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `fi_digitraffic_road_mqtt_weight_restriction_async`



## Test```python

fi_digitraffic_road_mqtt_weight_restriction_async:  Callable[[PartitionContext, EventData, CloudEvent, TrafficMessage],
Awaitable[None]]
...
```python
async def fi_digitraffic_road_mqtt_weight_restriction_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: TrafficMessage) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `fi_digitraffic_road_mqtt_exempted_transport_async`



## Test```python

fi_digitraffic_road_mqtt_exempted_transport_async:  Callable[[PartitionContext, EventData, CloudEvent, TrafficMessage],
Awaitable[None]]
...
```python
async def fi_digitraffic_road_mqtt_exempted_transport_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: TrafficMessage) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `fi_digitraffic_road_mqtt_maintenance_tracking_async`



## Test```python

fi_digitraffic_road_mqtt_maintenance_tracking_async:  Callable[[PartitionContext, EventData, CloudEvent,
MaintenanceTracking], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `digitraffic_road_mqtt_producer_data.MaintenanceTracking`.

Example:

```python
async def fi_digitraffic_road_mqtt_maintenance_tracking_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: MaintenanceTracking) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `fi_digitraffic_road_mqtt_tms_station_async`



## Test```python

fi_digitraffic_road_mqtt_tms_station_async:  Callable[[PartitionContext, EventData, CloudEvent, TmsStation],
Awaitable[None]]
...
```Asynchronous handler hook for `fi.digitraffic.road.mqtt.TmsStation`: A reference record from Fintraffic Digitraffic
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `digitraffic_road_mqtt_producer_data.TmsStation`.

Example:

```python
async def fi_digitraffic_road_mqtt_tms_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: TmsStation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `fi_digitraffic_road_mqtt_weather_station_async`



## Test```python

fi_digitraffic_road_mqtt_weather_station_async:  Callable[[PartitionContext, EventData, CloudEvent, WeatherStation],
Awaitable[None]]
...
```Asynchronous handler hook for `fi.digitraffic.road.mqtt.WeatherStation`: A reference record from Fintraffic
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `digitraffic_road_mqtt_producer_data.WeatherStation`.

Example:

```python
async def fi_digitraffic_road_mqtt_weather_station_event(partition_context: PartitionContext, event: EventData,
...
```##### `fi_digitraffic_road_mqtt_maintenance_task_type_async`



## Test```python

fi_digitraffic_road_mqtt_maintenance_task_type_async:  Callable[[PartitionContext, EventData, CloudEvent,
MaintenanceTaskType], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `digitraffic_road_mqtt_producer_data.MaintenanceTaskType`.

Example:

```python
async def fi_digitraffic_road_mqtt_maintenance_task_type_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: MaintenanceTaskType) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
