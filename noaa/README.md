...
Deploy with `tools/deploy-fabric/deploy-fabric-aci.ps1 -Source noaa -ResourceGroup <rg> -Location <azure-region> -Workspace <fabric-workspace>` (the portal button wraps this for you). The script creates the Eventhouse, the KQL database with [`kql/noaa.kql`](kql/noaa.kql), the Event Stream with a custom endpoint, and the ACI with the connection string wired in.
