...
import noaa_amqp_producer_data
from noaa_amqp_producer_amqp_producer.producer import *
...
def feed(host, port, address='noaa', username=None, password=None, tls=False, content_mode='binary', auth_mode='password', entra_audience='https://servicebus.azure.net/.default', entra_client_id=None, sas_key_name=None, sas_key=None, once=False):
    ...
    try:
        for method in [getattr(producer,n) for n in dir(producer) if n.startswith('send_') and not n.endswith('_batch')]:
            method(**_required_call_kwargs(method, noaa_amqp_producer_data))
    finally:
