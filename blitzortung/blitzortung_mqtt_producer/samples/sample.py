...
from blitzortung_mqtt_producer_mqtt_client.client import BlitzortungLightningMqttProducer, BlitzortungLightningMqttDispatcher

async def handle_blitzortung_lightning_mqtt_lightning_stroke(mqtt_msg,cloud_event, blitzortung_lightning_mqtt_lightning_stroke_data):
    """ Handles the Blitzortung.Lightning.mqtt.LightningStroke message """
    print(f"Received Blitzortung.Lightning.mqtt.LightningStroke on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {blitzortung_lightning_mqtt_lightning_stroke_data}")
