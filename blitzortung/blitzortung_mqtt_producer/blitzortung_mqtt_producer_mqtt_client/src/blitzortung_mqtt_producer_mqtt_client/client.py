...
def get_default_topic_mappings_blitzortung_lightning_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the Blitzortung.Lightning.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "Blitzortung.Lightning.mqtt.LightningStroke": "weather/intl/blitzortung/blitzortung/{geohash5}/{geohash7}/{stroke_id}/stroke",
    }


def get_subscription_topics_blitzortung_lightning_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_blitzortung_lightning_mqtt()
    topics = []
...
class BlitzortungLightningMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the Blitzortung.Lightning.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_blitzortung_lightning_mqtt()
        ...
        self.blitzortung_lightning_mqtt_lightning_stroke_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, blitzortung_mqtt_producer_data.LightningStroke, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "Blitzortung.Lightning.mqtt.LightningStroke":
            if self.blitzortung_lightning_mqtt_lightning_stroke_async:
                try:
                    ...
                    data = blitzortung_mqtt_producer_data.LightningStroke.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Blitzortung.Lightning.mqtt.LightningStroke")
                    await self.blitzortung_lightning_mqtt_lightning_stroke_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in blitzortung_lightning_mqtt_lightning_stroke handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_blitzortung_lightning_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_blitzortung_lightning_mqtt_lightning_stroke(self,
        ...
        """
        Publish the 'Blitzortung.Lightning.mqtt.LightningStroke' event to an MQTT topic.
        ...
        attributes = {
             "type":"Blitzortung.Lightning.mqtt.LightningStroke",
