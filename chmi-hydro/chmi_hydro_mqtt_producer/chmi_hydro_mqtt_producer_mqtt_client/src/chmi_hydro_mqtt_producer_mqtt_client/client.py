...
import chmi_hydro_mqtt_producer_data
from chmi_hydro_mqtt_producer_data import Station
from chmi_hydro_mqtt_producer_data import WaterLevelObservation
...
def get_default_topic_mappings_cz_gov_chmi_hydro_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the CZ.Gov.CHMI.Hydro.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "CZ.Gov.CHMI.Hydro.mqtt.Station": "hydro/cz/chmi/chmi-hydro/{stream_name}/{station_id}/info",
        "CZ.Gov.CHMI.Hydro.mqtt.WaterLevelObservation": "hydro/cz/chmi/chmi-hydro/{stream_name}/{station_id}/water-level",
    }


def get_subscription_topics_cz_gov_chmi_hydro_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_cz_gov_chmi_hydro_mqtt()
    topics = []
...
class CZGovCHMIHydroMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the CZ.Gov.CHMI.Hydro.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_cz_gov_chmi_hydro_mqtt()
        ...
        self.cz_gov_chmi_hydro_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, chmi_hydro_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.cz_gov_chmi_hydro_mqtt_water_level_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, chmi_hydro_mqtt_producer_data.WaterLevelObservation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "CZ.Gov.CHMI.Hydro.Station":
            if self.cz_gov_chmi_hydro_mqtt_station_async:
                try:
                    ...
                    data = chmi_hydro_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "CZ.Gov.CHMI.Hydro.mqtt.Station")
                    await self.cz_gov_chmi_hydro_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in cz_gov_chmi_hydro_mqtt_station handler: {e}")
            return
        
        if event_type == "CZ.Gov.CHMI.Hydro.WaterLevelObservation":
            if self.cz_gov_chmi_hydro_mqtt_water_level_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = chmi_hydro_mqtt_producer_data.WaterLevelObservation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "CZ.Gov.CHMI.Hydro.mqtt.WaterLevelObservation")
                    await self.cz_gov_chmi_hydro_mqtt_water_level_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in cz_gov_chmi_hydro_mqtt_water_level_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_cz_gov_chmi_hydro_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_cz_gov_chmi_hydro_mqtt_station(self,
        station_id: str,
        stream_name: str,
        data: chmi_hydro_mqtt_producer_data.Station,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'CZ.Gov.CHMI.Hydro.mqtt.Station' event to an MQTT topic.

        Args:
        
            station_id: URI template variable for 'station_id'
            stream_name: URI template variable for 'stream_name'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/cz/chmi/chmi-hydro/{stream_name}/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "hydro/cz/chmi/chmi-hydro/{stream_name}/{station_id}/info"
        ...
        attributes = {
             "type":"CZ.Gov.CHMI.Hydro.Station",
    ...
    async def publish_cz_gov_chmi_hydro_mqtt_water_level_observation(self,
        station_id: str,
        stream_name: str,
        data: chmi_hydro_mqtt_producer_data.WaterLevelObservation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'CZ.Gov.CHMI.Hydro.mqtt.WaterLevelObservation' event to an MQTT topic.
        ...
        Args:
            ...
            stream_name: URI template variable for 'stream_name'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/cz/chmi/chmi-hydro/{stream_name}/{station_id}/water-level'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "hydro/cz/chmi/chmi-hydro/{stream_name}/{station_id}/water-level"
        _topic_template_values: Dict[str, str] = {
            "station_id": str(station_id),
            "stream_name": str(stream_name),
        }
        ...
        attributes = {
             "type":"CZ.Gov.CHMI.Hydro.WaterLevelObservation",
             "source":"https://opendata.chmi.cz",
             "subject":"{station_id}".format(station_id = station_id)
        }
        attributes["datacontenttype"] = content_type
