# Chmi_hydro_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the chmi_hydro_mqtt_producer message definitions.

# Chmi_hydro_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - CZGovCHMIHydroMqttEventDispatcher
...
from chmi_hydro_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = CZGovCHMIHydroMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### CZGovCHMIHydroMqttEventDispatcher



# Publish message`CZGovCHMIHydroMqttEventDispatcher` handles events for the CZ.Gov.CHMI.Hydro.mqtt message group.
...
from chmi_hydro_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = CZGovCHMIHydroMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe CZGovCHMIHydroMqttEventDispatcher defines the following event handler hooks.
...
```##### `cz_gov_chmi_hydro_mqtt_station_async`



## Test```python

cz_gov_chmi_hydro_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `chmi_hydro_mqtt_producer_data.Station`.

Example:

```python
async def cz_gov_chmi_hydro_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `cz_gov_chmi_hydro_mqtt_water_level_observation_async`



## Test```python

cz_gov_chmi_hydro_mqtt_water_level_observation_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `chmi_hydro_mqtt_producer_data.WaterLevelObservation`.

Example:

```python
async def cz_gov_chmi_hydro_mqtt_water_level_observation_event(partition_context: PartitionContext, event: EventData,
