...
"""
Tests for chmi_hydro_amqp_producer_amqp_producer
"""
import base64
import json
import os
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../chmi_hydro_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../chmi_hydro_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../chmi_hydro_amqp_producer_amqp_producer/src')))

from chmi_hydro_amqp_producer_amqp_producer import *
from chmi_hydro_amqp_producer_data import Station
from test_chmi_hydro_amqp_producer_data_station import Test_Station
from chmi_hydro_amqp_producer_data import WaterLevelObservation
from test_chmi_hydro_amqp_producer_data_waterlevelobservation import Test_WaterLevelObservation
...
class TestCZGovCHMIHydroAmqpProducer:
    """Test cases for CZGovCHMIHydroAmqpProducer"""
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "CZ.Gov.CHMI.Hydro.amqp.Station"
                ...
                assert properties.get('stream_name') == "{stream_name}".format(stream_name="value")
        finally:
            producer.close()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "CZ.Gov.CHMI.Hydro.amqp.WaterLevelObservation"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
