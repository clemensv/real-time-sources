...
import ptwc_tsunami_mqtt_producer_data
from ptwc_tsunami_mqtt_producer_data import TsunamiBulletin
...
def get_default_topic_mappings_ptwc_bulletins_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the PTWC.Bulletins.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "PTWC.Bulletins.mqtt.TsunamiBulletin": "alerts/intl/ptwc/ptwc-tsunami/{basin}/{ptwc_level}/{bulletin_id}/bulletin",
    }


def get_subscription_topics_ptwc_bulletins_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_ptwc_bulletins_mqtt()
    topics = []
...
class PTWCBulletinsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the PTWC.Bulletins.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_ptwc_bulletins_mqtt()
        ...
        self.ptwc_bulletins_mqtt_tsunami_bulletin_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, ptwc_tsunami_mqtt_producer_data.TsunamiBulletin, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "PTWC.TsunamiBulletin":
            if self.ptwc_bulletins_mqtt_tsunami_bulletin_async:
                try:
                    ...
                    data = ptwc_tsunami_mqtt_producer_data.TsunamiBulletin.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "PTWC.Bulletins.mqtt.TsunamiBulletin")
                    await self.ptwc_bulletins_mqtt_tsunami_bulletin_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in ptwc_bulletins_mqtt_tsunami_bulletin handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_ptwc_bulletins_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_ptwc_bulletins_mqtt_tsunami_bulletin(self,
        bulletin_id: str,
        basin: str,
        ptwc_level: str,
        data: ptwc_tsunami_mqtt_producer_data.TsunamiBulletin,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'PTWC.Bulletins.mqtt.TsunamiBulletin' event to an MQTT topic.

        Args:
        
            bulletin_id: URI template variable for 'bulletin_id'
            basin: URI template variable for 'basin'
            ptwc_level: URI template variable for 'ptwc_level'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'alerts/intl/ptwc/ptwc-tsunami/{basin}/{ptwc_level}/{bulletin_id}/bulletin'
        ...
        """
        target_topic = topic if topic is not None else "alerts/intl/ptwc/ptwc-tsunami/{basin}/{ptwc_level}/{bulletin_id}/bulletin"
        _topic_template_values: Dict[str, str] = {
            "bulletin_id": str(bulletin_id),
            "basin": str(basin),
            "ptwc_level": str(ptwc_level),
        }
        if _topic_template_values:
            target_topic = _apply_topic_template(target_topic, _topic_template_values)

        attributes = {
             "type":"PTWC.TsunamiBulletin",
             "source":"https://www.tsunami.gov",
             "subject":"{bulletin_id}".format(bulletin_id = bulletin_id)
        }
        attributes["datacontenttype"] = content_type
