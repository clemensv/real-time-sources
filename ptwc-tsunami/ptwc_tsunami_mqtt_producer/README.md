# Ptwc_tsunami_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the ptwc_tsunami_mqtt_producer message definitions.

# Ptwc_tsunami_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - PTWCBulletinsMqttEventDispatcher
...
from ptwc_tsunami_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = PTWCBulletinsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### PTWCBulletinsMqttEventDispatcher



# Publish message`PTWCBulletinsMqttEventDispatcher` handles events for the PTWC.Bulletins.mqtt message group.
...
from ptwc_tsunami_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = PTWCBulletinsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe PTWCBulletinsMqttEventDispatcher defines the following event handler hooks.
...
```##### `ptwc_bulletins_mqtt_tsunami_bulletin_async`



## Test```python

ptwc_bulletins_mqtt_tsunami_bulletin_async:  Callable[[PartitionContext, EventData, CloudEvent, TsunamiBulletin],
Awaitable[None]]
...
- `data`: The event data of type `ptwc_tsunami_mqtt_producer_data.TsunamiBulletin`.

Example:

```python
async def ptwc_bulletins_mqtt_tsunami_bulletin_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: TsunamiBulletin) -> None:
