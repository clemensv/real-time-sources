# Irceline_belgium_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the irceline_belgium_mqtt_producer message definitions.

# Irceline_belgium_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - BeIrcelineStationsMqttEventDispatcher,

    BeIrcelineTimeseriesMqttEventDispatcher
...
from irceline_belgium_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = BeIrcelineStationsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### BeIrcelineStationsMqttEventDispatcher



# Publish message`BeIrcelineStationsMqttEventDispatcher` handles events for the be.irceline.Stations.mqtt message group.
...
from irceline_belgium_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = BeIrcelineStationsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
client = BeIrcelineTimeseriesMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe BeIrcelineStationsMqttEventDispatcher defines the following event handler hooks.
...
```##### `be_irceline_stations_mqtt_station_async`



## Test```python

be_irceline_stations_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `irceline_belgium_mqtt_producer_data.Station`.

Example:

```python
async def be_irceline_stations_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
client = BeIrcelineTimeseriesMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### BeIrcelineTimeseriesMqttEventDispatcher



# Publish message`BeIrcelineTimeseriesMqttEventDispatcher` handles events for the be.irceline.Timeseries.mqtt message
group.
...
## BuildThe BeIrcelineTimeseriesMqttEventDispatcher defines the following event handler hooks.
...
```##### `be_irceline_timeseries_mqtt_timeseries_async`



## Test```python

be_irceline_timeseries_mqtt_timeseries_async:  Callable[[PartitionContext, EventData, CloudEvent, Timeseries],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `irceline_belgium_mqtt_producer_data.Timeseries`.

Example:

```python
async def be_irceline_timeseries_mqtt_timeseries_event(partition_context: PartitionContext, event: EventData,
...
```##### `be_irceline_timeseries_mqtt_observation_async`



## Test```python

be_irceline_timeseries_mqtt_observation_async:  Callable[[PartitionContext, EventData, CloudEvent, Observation],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `irceline_belgium_mqtt_producer_data.Observation`.

Example:

```python
async def be_irceline_timeseries_mqtt_observation_event(partition_context: PartitionContext, event: EventData,
