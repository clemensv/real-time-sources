# Noaa_nws_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the noaa_nws_mqtt_producer message definitions.

# Noaa_nws_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - MicrosoftOpenDataUSNOAANWSAlertsMqttEventDispatcher,
...
from noaa_nws_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = MicrosoftOpenDataUSNOAANWSAlertsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### MicrosoftOpenDataUSNOAANWSAlertsMqttEventDispatcher



# Publish message`MicrosoftOpenDataUSNOAANWSAlertsMqttEventDispatcher` handles events for the
Microsoft.OpenData.US.NOAA.NWS.Alerts.mqtt message group.
...
from noaa_nws_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = MicrosoftOpenDataUSNOAANWSAlertsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The
...
client = MicrosoftOpenDataUSNOAANWSObservationsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`:
...
## BuildThe MicrosoftOpenDataUSNOAANWSAlertsMqttEventDispatcher defines the following event handler hooks.
...
```##### `microsoft_open_data_us_noaa_nws_alerts_mqtt_weather_alert_async`



## Test```python

microsoft_open_data_us_noaa_nws_alerts_mqtt_weather_alert_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `data`: The event data of type `noaa_nws_mqtt_producer_data.WeatherAlert`.

Example:

```python
async def microsoft_open_data_us_noaa_nws_alerts_mqtt_weather_alert_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: WeatherAlert) -> None:
...
client = MicrosoftOpenDataUSNOAANWSObservationsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### MicrosoftOpenDataUSNOAANWSObservationsMqttEventDispatcher



# Publish message`MicrosoftOpenDataUSNOAANWSObservationsMqttEventDispatcher` handles events for the
...
## BuildThe MicrosoftOpenDataUSNOAANWSObservationsMqttEventDispatcher defines the following event handler hooks.
...
```##### `microsoft_open_data_us_noaa_nws_observations_mqtt_observation_station_async`



## Test```python

microsoft_open_data_us_noaa_nws_observations_mqtt_observation_station_async:  Callable[[PartitionContext, EventData,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `noaa_nws_mqtt_producer_data.ObservationStation`.

Example:

```python
async def microsoft_open_data_us_noaa_nws_observations_mqtt_observation_station_event(partition_context:
PartitionContext, event: EventData, cloud_event: CloudEvent, data: ObservationStation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `microsoft_open_data_us_noaa_nws_observations_mqtt_weather_observation_async`



## Test```python

microsoft_open_data_us_noaa_nws_observations_mqtt_weather_observation_async:  Callable[[PartitionContext, EventData,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `noaa_nws_mqtt_producer_data.WeatherObservation`.

Example:

```python
async def microsoft_open_data_us_noaa_nws_observations_mqtt_weather_observation_event(partition_context:
PartitionContext, event: EventData, cloud_event: CloudEvent, data: WeatherObservation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
