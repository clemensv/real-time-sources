...
class MicrosoftOpenDataUSNOAANWSEventProducer:
    ...
    def send_microsoft_open_data_us_noaa_nws_weather_alert(self,data: WeatherAlert, content_type: str = "application/json", flush_producer=True, key_mapper: typing.Callable[[CloudEvent, WeatherAlert], str]=None) -> None:
        """
        ...
        """
        kafka_key = self.__resolve_template("{alert_id}", data)
        attributes = {
             "type":"Microsoft.OpenData.US.NOAA.NWS.WeatherAlert",
             "source":"https://api.weather.gov",

             "subject":self.__resolve_template("{alert_id}", data)
        }
    ...
    def send_microsoft_open_data_us_noaa_nws_zone(self,data: Zone, content_type: str = "application/json", flush_producer=True, key_mapper: typing.Callable[[CloudEvent, Zone], str]=None) -> None:
        ...
        """
        kafka_key = self.__resolve_template("{zone_id}", data)
        attributes = {
             "type":"Microsoft.OpenData.US.NOAA.NWS.Zone",
             "source":"https://api.weather.gov",

             "subject":self.__resolve_template("{zone_id}", data)
        }
        attributes["datacontenttype"] = content_type
        event = CloudEvent.create(attributes, data)
        if self.content_mode == "structured":
    ...
    @classmethod
    def from_connection_string(cls, connection_string: str, topic: typing.Optional[str]=None, content_mode: typing.Literal['structured','binary']='structured') -> 'MicrosoftOpenDataUSNOAANWSEventProducer':
        """
