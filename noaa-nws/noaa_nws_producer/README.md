...
3. [Quick Start](#quick-start)    - MicrosoftOpenDataUSNOAANWSEventDispatcher
...
- MicrosoftOpenDataUSNOAANWSProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### MicrosoftOpenDataUSNOAANWSEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`MicrosoftOpenDataUSNOAANWSEventDispatcher` handles events for the Microsoft.OpenData.US.NOAA.NWS message group.
...
from noaa-nws-producer import MicrosoftOpenDataUSNOAANWSProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = MicrosoftOpenDataUSNOAANWSProducer(
...
The MicrosoftOpenDataUSNOAANWSEventDispatcher defines the following event handler hooks.
...
Producer for `Microsoft.OpenData.US.NOAA.NWS` message group.Example:
...
MicrosoftOpenDataUSNOAANWSProducer(    await some_processing_function(record, cloud_event, data)
...
microsoft_open_data_us_noaa_nws_dispatcher.microsoft_open_data_us_noaa_nws_weather_alert_async =
...
microsoft_open_data_us_noaa_nws_dispatcher.microsoft_open_data_us_noaa_nws_zone_async =
