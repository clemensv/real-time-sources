# Dwd_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the dwd_mqtt_producer message definitions.

# Dwd_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - DEDWDCDCMqttEventDispatcher,

    DEDWDWeatherMqttEventDispatcher,

    DEDWDRadarMqttEventDispatcher,

    DEDWDForecastMqttEventDispatcher
...
from dwd_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = DEDWDCDCMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### DEDWDCDCMqttEventDispatcher



# Publish message`DEDWDCDCMqttEventDispatcher` handles events for the DE.DWD.CDC.mqtt message group.
...
from dwd_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = DEDWDCDCMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
client = DEDWDWeatherMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
Hub.Args:- `consumer_group_name`: The name of the consumer group.- `eventhubs_fully_qualified_namespace`: The fully
qualified namespace of the Event Hub.

client = DEDWDRadarMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
Hub.Args:- `consumer_group_name`: The name of the consumer group.- `eventhubs_fully_qualified_namespace`: The fully
qualified namespace of the Event Hub.

client = DEDWDForecastMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe DEDWDCDCMqttEventDispatcher defines the following event handler hooks.
...
```##### `de_dwd_cdc_mqtt_station_metadata_async`



## Test```python

de_dwd_cdc_mqtt_station_metadata_async:  Callable[[PartitionContext, EventData, CloudEvent, StationMetadata],
Awaitable[None]]
...
- `data`: The event data of type `dwd_mqtt_producer_data.StationMetadata`.

Example:

```python
async def de_dwd_cdc_mqtt_station_metadata_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: StationMetadata) -> None:
...
```##### `de_dwd_cdc_mqtt_air_temperature10_min_async`



## Test```python

de_dwd_cdc_mqtt_air_temperature10_min_async:  Callable[[PartitionContext, EventData, CloudEvent, AirTemperature10Min],
Awaitable[None]]
...
```Asynchronous handler hook for `DE.DWD.CDC.mqtt.AirTemperature10Min`: A current environmental measurement from
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dwd_mqtt_producer_data.AirTemperature10Min`.

Example:

```python
async def de_dwd_cdc_mqtt_air_temperature10_min_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: AirTemperature10Min) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_dwd_cdc_mqtt_precipitation10_min_async`



## Test```python

de_dwd_cdc_mqtt_precipitation10_min_async:  Callable[[PartitionContext, EventData, CloudEvent, Precipitation10Min],
Awaitable[None]]
...
```Asynchronous handler hook for `DE.DWD.CDC.mqtt.Precipitation10Min`: A current environmental measurement from
Germany's Deutscher Wetterdienst (DWD). It carries weather observations, warnings, and forecast product updates when the
upstream feed reports a new or refreshed value.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dwd_mqtt_producer_data.Precipitation10Min`.

Example:

```python
async def de_dwd_cdc_mqtt_precipitation10_min_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Precipitation10Min) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_dwd_cdc_mqtt_wind10_min_async`



## Test```python

de_dwd_cdc_mqtt_wind10_min_async:  Callable[[PartitionContext, EventData, CloudEvent, Wind10Min], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `DE.DWD.CDC.mqtt.Wind10Min`: A current environmental measurement from Germany's
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dwd_mqtt_producer_data.Wind10Min`.

Example:

```python
async def de_dwd_cdc_mqtt_wind10_min_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Wind10Min) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_dwd_cdc_mqtt_solar10_min_async`



## Test```python

de_dwd_cdc_mqtt_solar10_min_async:  Callable[[PartitionContext, EventData, CloudEvent, Solar10Min], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `DE.DWD.CDC.mqtt.Solar10Min`: A current environmental measurement from Germany's
Deutscher Wetterdienst (DWD). It carries weather observations, warnings, and forecast product updates when the upstream
feed reports a new or refreshed value.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dwd_mqtt_producer_data.Solar10Min`.

Example:

```python
async def de_dwd_cdc_mqtt_solar10_min_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Solar10Min) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_dwd_cdc_mqtt_hourly_observation_async`



## Test```python

de_dwd_cdc_mqtt_hourly_observation_async:  Callable[[PartitionContext, EventData, CloudEvent, HourlyObservation],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dwd_mqtt_producer_data.HourlyObservation`.

Example:

```python
async def de_dwd_cdc_mqtt_hourly_observation_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: HourlyObservation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_dwd_cdc_mqtt_extreme_wind10_min_async`



## Test```python

de_dwd_cdc_mqtt_extreme_wind10_min_async:  Callable[[PartitionContext, EventData, CloudEvent, ExtremeWind10Min],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dwd_mqtt_producer_data.ExtremeWind10Min`.

Example:

```python
async def de_dwd_cdc_mqtt_extreme_wind10_min_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: ExtremeWind10Min) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_dwd_cdc_mqtt_extreme_temperature10_min_async`



## Test```python

de_dwd_cdc_mqtt_extreme_temperature10_min_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dwd_mqtt_producer_data.ExtremeTemperature10Min`.

Example:

```python
async def de_dwd_cdc_mqtt_extreme_temperature10_min_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: ExtremeTemperature10Min) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = DEDWDWeatherMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### DEDWDWeatherMqttEventDispatcher



# Publish message`DEDWDWeatherMqttEventDispatcher` handles events for the DE.DWD.Weather.mqtt message group.
...
## BuildThe DEDWDWeatherMqttEventDispatcher defines the following event handler hooks.
...
```##### `de_dwd_weather_mqtt_alert_async`



## Test```python

de_dwd_weather_mqtt_alert_async:  Callable[[PartitionContext, EventData, CloudEvent, Alert], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dwd_mqtt_producer_data.Alert`.

Example:

```python
async def de_dwd_weather_mqtt_alert_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Alert) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = DEDWDRadarMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### DEDWDRadarMqttEventDispatcher



# Publish message`DEDWDRadarMqttEventDispatcher` handles events for the DE.DWD.Radar.mqtt message group.
...
## BuildThe DEDWDRadarMqttEventDispatcher defines the following event handler hooks.
...
```##### `de_dwd_radar_mqtt_radar_product_catalog_async`



## Test```python

de_dwd_radar_mqtt_radar_product_catalog_async:  Callable[[PartitionContext, EventData, CloudEvent, RadarProductCatalog],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dwd_mqtt_producer_data.RadarProductCatalog`.

Example:

```python
async def de_dwd_radar_mqtt_radar_product_catalog_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RadarProductCatalog) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_dwd_radar_mqtt_radar_file_product_async`



## Test```python

de_dwd_radar_mqtt_radar_file_product_async:  Callable[[PartitionContext, EventData, CloudEvent, RadarFileProduct],
Awaitable[None]]
...
```Asynchronous handler hook for `DE.DWD.Radar.mqtt.RadarFileProduct`: A current environmental measurement from
Germany's Deutscher Wetterdienst (DWD). It carries weather observations, warnings, and forecast product updates when the
upstream feed reports a new or refreshed value.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dwd_mqtt_producer_data.RadarFileProduct`.

Example:

```python
async def de_dwd_radar_mqtt_radar_file_product_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: RadarFileProduct) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
client = DEDWDForecastMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### DEDWDForecastMqttEventDispatcher



# Publish message`DEDWDForecastMqttEventDispatcher` handles events for the DE.DWD.Forecast.mqtt message group.
...
## BuildThe DEDWDForecastMqttEventDispatcher defines the following event handler hooks.
...
```##### `de_dwd_forecast_mqtt_forecast_model_catalog_async`



## Test```python

de_dwd_forecast_mqtt_forecast_model_catalog_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dwd_mqtt_producer_data.ForecastModelCatalog`.

Example:

```python
async def de_dwd_forecast_mqtt_forecast_model_catalog_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: ForecastModelCatalog) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_dwd_forecast_mqtt_icon_d2_forecast_file_async`



## Test```python

de_dwd_forecast_mqtt_icon_d2_forecast_file_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `dwd_mqtt_producer_data.IconD2ForecastFile`.

Example:

```python
async def de_dwd_forecast_mqtt_icon_d2_forecast_file_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: IconD2ForecastFile) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
