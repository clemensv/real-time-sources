...
from dwd_producer_data import Grid
...
class DEDWDIconD2EventProducer:
    def __init__(self, producer: Producer, topic: str, content_mode:typing.Literal['structured','binary']='structured'):
        """
        Initializes the Kafka producer
    ...
    def send_de_dwd_icon_d2_grid(self,_run_id : str, _parameter : str, _lead_hour : str, data: Grid, content_type: str = "application/json", flush_producer=True, key_mapper: typing.Callable[[CloudEvent, Grid], str]=None) -> None:
        """
        Sends the 'DE.DWD.IconD2.Grid' event to the Kafka topic

        Args:
            _run_id(str):  Value for placeholder run_id in attribute subject
            _parameter(str):  Value for placeholder parameter in attribute subject
            _lead_hour(str):  Value for placeholder lead_hour in attribute subject
            data: (Grid): The event data to be sent
            content_type (str): The content type that the event data shall be sent with
            flush_producer(bool): Whether to flush the producer after sending the event (default: True)
            key_mapper(Callable[[CloudEvent, Grid], str]): A function to map the CloudEvent contents to a Kafka key (default: None).
                The default key is derived from the xRegistry Kafka key declaration '{run_id}/{parameter}/{lead_hour}'
        """
        kafka_key = "{run_id}/{parameter}/{lead_hour}".format(run_id=_run_id, parameter=_parameter, lead_hour=_lead_hour)
        attributes = {
             "type":"DE.DWD.IconD2.Grid",
             "source":"https://opendata.dwd.de",
             "subject":"{run_id}/{parameter}/{lead_hour}".format(run_id = _run_id,parameter = _parameter,lead_hour = _lead_hour)
        }
        attributes["datacontenttype"] = content_type
        event = CloudEvent.create(attributes, data)
        if self.content_mode == "structured":
    ...
    @classmethod
    def from_connection_string(cls, connection_string: str, topic: typing.Optional[str]=None, content_mode: typing.Literal['structured','binary']='structured') -> 'DEDWDIconD2EventProducer':
        """
        Create a Kafka producer from a connection string and a topic name.
