# Rws_waterwebservices_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the rws_waterwebservices_mqtt_producer message definitions.

# Rws_waterwebservices_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - NLRWSWaterwebservicesMqttEventDispatcher
...
from rws_waterwebservices_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = NLRWSWaterwebservicesMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### NLRWSWaterwebservicesMqttEventDispatcher



# Publish message`NLRWSWaterwebservicesMqttEventDispatcher` handles events for the NL.RWS.Waterwebservices.mqtt message
group.
...
from rws_waterwebservices_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = NLRWSWaterwebservicesMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe NLRWSWaterwebservicesMqttEventDispatcher defines the following event handler hooks.
...
```##### `nl_rws_waterwebservices_mqtt_station_async`



## Test```python

nl_rws_waterwebservices_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station],
Awaitable[None]]
...
- `data`: The event data of type `rws_waterwebservices_mqtt_producer_data.Station`.

Example:

```python
async def nl_rws_waterwebservices_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `nl_rws_waterwebservices_mqtt_water_level_observation_async`



## Test```python

nl_rws_waterwebservices_mqtt_water_level_observation_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `rws_waterwebservices_mqtt_producer_data.WaterLevelObservation`.

Example:

```python
async def nl_rws_waterwebservices_mqtt_water_level_observation_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: WaterLevelObservation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
