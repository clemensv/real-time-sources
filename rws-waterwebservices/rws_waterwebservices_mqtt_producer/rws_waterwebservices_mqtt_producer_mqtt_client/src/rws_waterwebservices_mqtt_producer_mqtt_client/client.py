...
def get_default_topic_mappings_nl_rws_waterwebservices_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the NL.RWS.Waterwebservices.mqtt message group.
...
def get_subscription_topics_nl_rws_waterwebservices_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_nl_rws_waterwebservices_mqtt()
    topics = []
...
class NLRWSWaterwebservicesMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the NL.RWS.Waterwebservices.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_nl_rws_waterwebservices_mqtt()
        ...
        self.nl_rws_waterwebservices_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, rws_waterwebservices_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.nl_rws_waterwebservices_mqtt_water_level_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, rws_waterwebservices_mqtt_producer_data.WaterLevelObservation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "NL.RWS.Waterwebservices.Station":
            if self.nl_rws_waterwebservices_mqtt_station_async:
                try:
                    ...
                    data = rws_waterwebservices_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "NL.RWS.Waterwebservices.mqtt.Station")
                    await self.nl_rws_waterwebservices_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in nl_rws_waterwebservices_mqtt_station handler: {e}")
            return
        
        if event_type == "NL.RWS.Waterwebservices.WaterLevelObservation":
            if self.nl_rws_waterwebservices_mqtt_water_level_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = rws_waterwebservices_mqtt_producer_data.WaterLevelObservation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "NL.RWS.Waterwebservices.mqtt.WaterLevelObservation")
                    await self.nl_rws_waterwebservices_mqtt_water_level_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in nl_rws_waterwebservices_mqtt_water_level_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_nl_rws_waterwebservices_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_nl_rws_waterwebservices_mqtt_station(self,
        ...
        """
        Publish the 'NL.RWS.Waterwebservices.mqtt.Station' event to an MQTT topic.
    ...
    async def publish_nl_rws_waterwebservices_mqtt_water_level_observation(self,
        ...
        content_type: str = "application/json") -> None:
        """
        Publish the 'NL.RWS.Waterwebservices.mqtt.WaterLevelObservation' event to an MQTT topic.
