...
from syke_hydro_mqtt_producer_data import Station
from syke_hydro_mqtt_producer_data import WaterLevelObservation
...
def get_default_topic_mappings_fi_syke_hydrology_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the FI.SYKE.Hydrology.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "FI.SYKE.Hydrology.mqtt.Station": "hydro/fi/syke/syke-hydro/{basin}/{station_id}/info",
        "FI.SYKE.Hydrology.mqtt.WaterLevelObservation": "hydro/fi/syke/syke-hydro/{basin}/{station_id}/water-level",
    }


def get_subscription_topics_fi_syke_hydrology_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_fi_syke_hydrology_mqtt()
    topics = []
...
class FISYKEHydrologyMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the FI.SYKE.Hydrology.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_fi_syke_hydrology_mqtt()
        ...
        self.fi_syke_hydrology_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, syke_hydro_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.fi_syke_hydrology_mqtt_water_level_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, syke_hydro_mqtt_producer_data.WaterLevelObservation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "FI.SYKE.Hydrology.Station":
            if self.fi_syke_hydrology_mqtt_station_async:
                try:
                    ...
                    data = syke_hydro_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "FI.SYKE.Hydrology.mqtt.Station")
                    await self.fi_syke_hydrology_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_syke_hydrology_mqtt_station handler: {e}")
            return
        
        if event_type == "FI.SYKE.Hydrology.WaterLevelObservation":
            if self.fi_syke_hydrology_mqtt_water_level_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = syke_hydro_mqtt_producer_data.WaterLevelObservation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "FI.SYKE.Hydrology.mqtt.WaterLevelObservation")
                    await self.fi_syke_hydrology_mqtt_water_level_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in fi_syke_hydrology_mqtt_water_level_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_fi_syke_hydrology_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_fi_syke_hydrology_mqtt_station(self,
        station_id: str,
        basin: str,
        data: syke_hydro_mqtt_producer_data.Station,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'FI.SYKE.Hydrology.mqtt.Station' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'hydro/fi/syke/syke-hydro/{basin}/{station_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "hydro/fi/syke/syke-hydro/{basin}/{station_id}/info"
    ...
    async def publish_fi_syke_hydrology_mqtt_water_level_observation(self,
        station_id: str,
        basin: str,
        data: syke_hydro_mqtt_producer_data.WaterLevelObservation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'FI.SYKE.Hydrology.mqtt.WaterLevelObservation' event to an MQTT topic.
        ...
        Args:
            ...
            basin: URI template variable for 'basin'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/fi/syke/syke-hydro/{basin}/{station_id}/water-level'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "hydro/fi/syke/syke-hydro/{basin}/{station_id}/water-level"
        _topic_template_values: Dict[str, str] = {
            "station_id": str(station_id),
            "basin": str(basin),
        }
