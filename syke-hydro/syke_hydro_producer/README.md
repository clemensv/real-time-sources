...
# Syke_hydro_producer Kafka Producer# Syke_hydro_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - FISYKEHydrologyEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    FISYKEHydrologyMqttEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    FISYKEHydrologyAmqpEventDispatcher
...
- FISYKEHydrologyProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- FISYKEHydrologyMqttProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- FISYKEHydrologyAmqpProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### FISYKEHydrologyEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`FISYKEHydrologyEventDispatcher` handles events for the FI.SYKE.Hydrology message group.
...
from syke_hydro_producer import FISYKEHydrologyProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = FISYKEHydrologyProducer(
...
await producer.send_fi_syke_hydrology_station(```python
...
The FISYKEHydrologyEventDispatcher defines the following event handler hooks.
...
producer = FISYKEHydrologyProducer(
    ...
    security_protocol='SASL_SSL',##### `fi_syke_hydrology_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'fi_syke_hydrology_station_async:  Callable[[ConsumerRecord, CloudEvent, Station],
Awaitable[None]]
...
Asynchronous handler hook for `FI.SYKE.Hydrology.Station`: A reference record for one Finnish hydrological monitoring
station published by the Finnish Environment Institute (SYKE). It fires when the bridge publishes or refreshes the
...
### FISYKEHydrologyProducer- `data`: The event data of type `syke_hydro_producer_data.Station`.



Producer for `FI.SYKE.Hydrology` message group.Example:



#### Constructor```python

async def fi_syke_hydrology_station_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Station) -> None:

```python    # Process the event data

FISYKEHydrologyProducer(    await some_processing_function(record, cloud_event, data)
...
fi_syke_hydrology_dispatcher.fi_syke_hydrology_station_async = fi_syke_hydrology_station_event
...
### FISYKEHydrologyMqttProducer- `data`: The event data of type `syke_hydro_producer_data.Station`.



Producer for `FI.SYKE.Hydrology.mqtt` message group.Example:
...
FISYKEHydrologyMqttProducer(    await some_processing_function(record, cloud_event, data)
...
fi_syke_hydrology_mqtt_dispatcher.fi_syke_hydrology_station_async = fi_syke_hydrology_station_event
...
### FISYKEHydrologyAmqpProducer- `data`: The event data of type `syke_hydro_producer_data.Station`.



Producer for `FI.SYKE.Hydrology.amqp` message group.Example:
...
FISYKEHydrologyAmqpProducer(    await some_processing_function(record, cloud_event, data)
...
fi_syke_hydrology_amqp_dispatcher.fi_syke_hydrology_station_async = fi_syke_hydrology_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `fi_syke_hydrology_water_level_observation_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'fi_syke_hydrology_water_level_observation_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `FI.SYKE.Hydrology.WaterLevelObservation`: A current measurement from the Finnish
Environment Institute (SYKE) for one monitoring site. It carries water level and discharge observations when the
...
### FISYKEHydrologyProducer- `data`: The event data of type `syke_hydro_producer_data.WaterLevelObservation`.
...
async def fi_syke_hydrology_water_level_observation_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
...
fi_syke_hydrology_dispatcher.fi_syke_hydrology_water_level_observation_async =
fi_syke_hydrology_water_level_observation_event
...
### FISYKEHydrologyMqttProducer- `data`: The event data of type `syke_hydro_producer_data.WaterLevelObservation`.
...
fi_syke_hydrology_mqtt_dispatcher.fi_syke_hydrology_water_level_observation_async =
fi_syke_hydrology_water_level_observation_event
...
### FISYKEHydrologyAmqpProducer- `data`: The event data of type `syke_hydro_producer_data.WaterLevelObservation`.
...
fi_syke_hydrology_amqp_dispatcher.fi_syke_hydrology_water_level_observation_async =
fi_syke_hydrology_water_level_observation_event
...
##### `send_fi_syke_hydrology_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_fi_syke_hydrology_station(
...
Send a single `FI.SYKE.Hydrology.Station` message. A reference record for one Finnish hydrological monitoring station
published by the Finnish Environment Institute (SYKE). It fires when the bridge publishes or refreshes the station
catalog so consumers can interpret measurement events.Args:
...
await producer.send_fi_syke_hydrology_station(
...
Send multiple `FI.SYKE.Hydrology.Station` messages in a batch.
...
await producer.send_fi_syke_hydrology_station_batch(```
...
##### `send_fi_syke_hydrology_water_level_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_fi_syke_hydrology_water_level_observation(
...
Send a single `FI.SYKE.Hydrology.WaterLevelObservation` message. A current measurement from the Finnish Environment
Institute (SYKE) for one monitoring site. It carries water level and discharge observations when the upstream feed
...
await producer.send_fi_syke_hydrology_water_level_observation(
...
Send multiple `FI.SYKE.Hydrology.WaterLevelObservation` messages in a batch.
...
await producer.send_fi_syke_hydrology_water_level_observation_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### FISYKEHydrologyMqttEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`FISYKEHydrologyMqttEventDispatcher` handles events for the FI.SYKE.Hydrology.mqtt message group.
...
The FISYKEHydrologyMqttEventDispatcher defines the following event handler hooks.
...
producer = FISYKEHydrologyProducer(
    ...
    security_protocol='SASL_SSL',##### `fi_syke_hydrology_mqtt_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'fi_syke_hydrology_mqtt_station_async:  Callable[[ConsumerRecord, CloudEvent, Station],
Awaitable[None]]
...
Asynchronous handler hook for `FI.SYKE.Hydrology.mqtt.Station`: A reference record for one Finnish hydrological
monitoring station published by the Finnish Environment Institute (SYKE). It fires when the bridge publishes or
...
async def fi_syke_hydrology_mqtt_station_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Station) -> None:
...
fi_syke_hydrology_dispatcher.fi_syke_hydrology_mqtt_station_async = fi_syke_hydrology_mqtt_station_event
...
fi_syke_hydrology_mqtt_dispatcher.fi_syke_hydrology_mqtt_station_async = fi_syke_hydrology_mqtt_station_event
...
fi_syke_hydrology_amqp_dispatcher.fi_syke_hydrology_mqtt_station_async = fi_syke_hydrology_mqtt_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `fi_syke_hydrology_mqtt_water_level_observation_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'fi_syke_hydrology_mqtt_water_level_observation_async:  Callable[[ConsumerRecord,
...
Asynchronous handler hook for `FI.SYKE.Hydrology.mqtt.WaterLevelObservation`: A current measurement from the Finnish
Environment Institute (SYKE) for one monitoring site. It carries water level and discharge observations when the
upstream feed reports a new or refreshed value.
...
async def fi_syke_hydrology_mqtt_water_level_observation_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
WaterLevelObservation) -> None:
...
fi_syke_hydrology_dispatcher.fi_syke_hydrology_mqtt_water_level_observation_async =
fi_syke_hydrology_mqtt_water_level_observation_event
...
fi_syke_hydrology_mqtt_dispatcher.fi_syke_hydrology_mqtt_water_level_observation_async =
fi_syke_hydrology_mqtt_water_level_observation_event
...
fi_syke_hydrology_amqp_dispatcher.fi_syke_hydrology_mqtt_water_level_observation_async =
fi_syke_hydrology_mqtt_water_level_observation_event
...
##### `send_fi_syke_hydrology_mqtt_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_fi_syke_hydrology_mqtt_station(
...
Send a single `FI.SYKE.Hydrology.mqtt.Station` message. A reference record for one Finnish hydrological monitoring
station published by the Finnish Environment Institute (SYKE). It fires when the bridge publishes or refreshes the
station catalog so consumers can interpret measurement events.Args:
...
await producer.send_fi_syke_hydrology_mqtt_station(
...
Send multiple `FI.SYKE.Hydrology.mqtt.Station` messages in a batch.
...
await producer.send_fi_syke_hydrology_mqtt_station_batch(```
...
##### `send_fi_syke_hydrology_mqtt_water_level_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_fi_syke_hydrology_mqtt_water_level_observation(
...
Send a single `FI.SYKE.Hydrology.mqtt.WaterLevelObservation` message. A current measurement from the Finnish Environment
Institute (SYKE) for one monitoring site. It carries water level and discharge observations when the upstream feed
reports a new or refreshed value.Args:
...
await producer.send_fi_syke_hydrology_mqtt_water_level_observation(
...
Send multiple `FI.SYKE.Hydrology.mqtt.WaterLevelObservation` messages in a batch.
...
await producer.send_fi_syke_hydrology_mqtt_water_level_observation_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### FISYKEHydrologyAmqpEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`FISYKEHydrologyAmqpEventDispatcher` handles events for the FI.SYKE.Hydrology.amqp message group.
...
The FISYKEHydrologyAmqpEventDispatcher defines the following event handler hooks.
...
producer = FISYKEHydrologyProducer(
    ...
    security_protocol='SASL_SSL',##### `fi_syke_hydrology_amqp_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'fi_syke_hydrology_amqp_station_async:  Callable[[ConsumerRecord, CloudEvent, Station],
Awaitable[None]]
...
Asynchronous handler hook for `FI.SYKE.Hydrology.amqp.Station`: A reference record for one Finnish hydrological
monitoring station published by the Finnish Environment Institute (SYKE). It fires when the bridge publishes or
refreshes the station catalog so consumers can interpret measurement events.
...
async def fi_syke_hydrology_amqp_station_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Station) -> None:
...
fi_syke_hydrology_dispatcher.fi_syke_hydrology_amqp_station_async = fi_syke_hydrology_amqp_station_event
...
fi_syke_hydrology_mqtt_dispatcher.fi_syke_hydrology_amqp_station_async = fi_syke_hydrology_amqp_station_event
...
fi_syke_hydrology_amqp_dispatcher.fi_syke_hydrology_amqp_station_async = fi_syke_hydrology_amqp_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `fi_syke_hydrology_amqp_water_level_observation_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'fi_syke_hydrology_amqp_water_level_observation_async:  Callable[[ConsumerRecord,
CloudEvent, WaterLevelObservation], Awaitable[None]]
...
Asynchronous handler hook for `FI.SYKE.Hydrology.amqp.WaterLevelObservation`: A current measurement from the Finnish
Environment Institute (SYKE) for one monitoring site. It carries water level and discharge observations when the
upstream feed reports a new or refreshed value.
...
async def fi_syke_hydrology_amqp_water_level_observation_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
WaterLevelObservation) -> None:
...
fi_syke_hydrology_dispatcher.fi_syke_hydrology_amqp_water_level_observation_async =
fi_syke_hydrology_amqp_water_level_observation_event
...
fi_syke_hydrology_mqtt_dispatcher.fi_syke_hydrology_amqp_water_level_observation_async =
fi_syke_hydrology_amqp_water_level_observation_event
...
fi_syke_hydrology_amqp_dispatcher.fi_syke_hydrology_amqp_water_level_observation_async =
fi_syke_hydrology_amqp_water_level_observation_event
...
##### `send_fi_syke_hydrology_amqp_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_fi_syke_hydrology_amqp_station(
...
Send a single `FI.SYKE.Hydrology.amqp.Station` message. A reference record for one Finnish hydrological monitoring
station published by the Finnish Environment Institute (SYKE). It fires when the bridge publishes or refreshes the
station catalog so consumers can interpret measurement events.Args:
...
await producer.send_fi_syke_hydrology_amqp_station(
...
Send multiple `FI.SYKE.Hydrology.amqp.Station` messages in a batch.
...
await producer.send_fi_syke_hydrology_amqp_station_batch(```
...
##### `send_fi_syke_hydrology_amqp_water_level_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_fi_syke_hydrology_amqp_water_level_observation(
...
Send a single `FI.SYKE.Hydrology.amqp.WaterLevelObservation` message. A current measurement from the Finnish Environment
Institute (SYKE) for one monitoring site. It carries water level and discharge observations when the upstream feed
reports a new or refreshed value.Args:
...
await producer.send_fi_syke_hydrology_amqp_water_level_observation(
...
Send multiple `FI.SYKE.Hydrology.amqp.WaterLevelObservation` messages in a batch.
...
await producer.send_fi_syke_hydrology_amqp_water_level_observation_batch(```
