...
class OrgLigoGracedbAmqpProducer:
    """
    Producer class to send messages in the `org.ligo.gracedb.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_superevent(self,
        ...
        """
        Send the `org.ligo.gracedb.amqp.Superevent` message
        ...
        attributes = {
            ...
            "subject":
            "{superevent_id}".format(superevent_id=_superevent_id),
            "time":
        ...
        amqp_msg.subject = "{superevent_id}".format(superevent_id=_superevent_id)
    ...
    def send_superevent_batch(self,
        data_array: typing.List[Superevent],
        _source_uri: str,
        _superevent_id: str,
        _created: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `org.ligo.gracedb.amqp.Superevent` messages
        
        Args:
            data_array (typing.List[Superevent]): Array of message data objects
            _source_uri (str): Value for placeholder source_uri in attribute source
            _superevent_id (str): Value for placeholder superevent_id in attribute subject
            _created (str): Value for placeholder created in attribute time
        ...
        for data in data_array:
            self.send_superevent(
                data=data,
                _source_uri=_source_uri,
                _superevent_id=_superevent_id,
                _created=_created,
