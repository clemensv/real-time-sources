...
# Ndw-road-traffic-producer Kafka Producer# Ndw-road-traffic-producer Event Dispatcher for Apache Kafka
...
- **Scales horizontally** across multiple brokers and partitions### NLNDWAVGEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`NLNDWAVGEventDispatcher` handles events for the NL.NDW.AVG message group.
...
from ndw-road-traffic-producer import NLNDWAVGProducer```python
...
The NLNDWAVGEventDispatcher defines the following event handler hooks.
...
producer = NLNDWAVGProducer(
    ...
    security_protocol='SASL_SSL',##### `nl_ndw_avg_point_measurement_site_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'nl_ndw_avg_point_measurement_site_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `NL.NDW.AVG.PointMeasurementSite`: Reference record for a point measurement site from the
...
nl_ndw_avg_dispatcher.nl_ndw_avg_point_measurement_site_async = nl_ndw_avg_point_measurement_site_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `nl_ndw_avg_route_measurement_site_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'nl_ndw_avg_route_measurement_site_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `NL.NDW.AVG.RouteMeasurementSite`: Reference record for a route (section) measurement site
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `nl_ndw_avg_traffic_observation_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'nl_ndw_avg_traffic_observation_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `NL.NDW.AVG.TrafficObservation`: Aggregated traffic speed and flow observation from the
Dutch NDW DATEX II trafficspeed feed. Each record represents one measurement site with speed averaged and flow summed
across all reporting lanes.
...
nl_ndw_avg_dispatcher.nl_ndw_avg_traffic_observation_async = nl_ndw_avg_traffic_observation_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `nl_ndw_avg_travel_time_observation_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'nl_ndw_avg_travel_time_observation_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `NL.NDW.AVG.TravelTimeObservation`: Travel time observation for a road segment from the
Dutch NDW DATEX II traveltime feed. Contains the actual measured travel time and the static free-flow reference time for
a route measurement site.
...
##### `send_nl_ndw_avg_point_measurement_site`Dispatchers have the following protected methods:



```python### Methods:

async def send_nl_ndw_avg_point_measurement_site(
...
Send a single `NL.NDW.AVG.PointMeasurementSite` message. Reference record for a point measurement site from the Dutch
NDW DATEX II measurement_current feed. Contains location, sensor technology type, and lane configuration for a fixed
inductive-loop or microwave sensor.Args:
...
await producer.send_nl_ndw_avg_point_measurement_site(
...
Send multiple `NL.NDW.AVG.PointMeasurementSite` messages in a batch.
...
await producer.send_nl_ndw_avg_point_measurement_site_batch(```
...
##### `send_nl_ndw_avg_route_measurement_site`Dispatchers have the following protected methods:



```python### Methods:

async def send_nl_ndw_avg_route_measurement_site(
...
Send a single `NL.NDW.AVG.RouteMeasurementSite` message. Reference record for a route (section) measurement site from
the Dutch NDW DATEX II measurement_current feed. Covers a road segment between two coordinates used for travel time
computation.Args:
...
await producer.send_nl_ndw_avg_route_measurement_site(
...
Send multiple `NL.NDW.AVG.RouteMeasurementSite` messages in a batch.
...
await producer.send_nl_ndw_avg_route_measurement_site_batch(```
...
##### `send_nl_ndw_avg_traffic_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_nl_ndw_avg_traffic_observation(
...
Send a single `NL.NDW.AVG.TrafficObservation` message. Aggregated traffic speed and flow observation from the Dutch NDW
DATEX II trafficspeed feed. Each record represents one measurement site with speed averaged and flow summed across all
reporting lanes.Args:
...
await producer.send_nl_ndw_avg_traffic_observation(
...
Send multiple `NL.NDW.AVG.TrafficObservation` messages in a batch.
...
await producer.send_nl_ndw_avg_traffic_observation_batch(```
...
##### `send_nl_ndw_avg_travel_time_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_nl_ndw_avg_travel_time_observation(
...
Send a single `NL.NDW.AVG.TravelTimeObservation` message. Travel time observation for a road segment from the Dutch NDW
DATEX II traveltime feed. Contains the actual measured travel time and the static free-flow reference time for a route
...
await producer.send_nl_ndw_avg_travel_time_observation(
...
Send multiple `NL.NDW.AVG.TravelTimeObservation` messages in a batch.
...
await producer.send_nl_ndw_avg_travel_time_observation_batch(```
