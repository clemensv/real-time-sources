# Eaws_albina_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the eaws_albina_mqtt_producer message definitions.

# Eaws_albina_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - OrgEAWSALBINAMqttEventDispatcher
...
from eaws_albina_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = OrgEAWSALBINAMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### OrgEAWSALBINAMqttEventDispatcher



# Publish message`OrgEAWSALBINAMqttEventDispatcher` handles events for the org.EAWS.ALBINA.mqtt message group.
...
from eaws_albina_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = OrgEAWSALBINAMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe OrgEAWSALBINAMqttEventDispatcher defines the following event handler hooks.
...
```##### `org_eaws_albina_mqtt_avalanche_bulletin_async`



## Test```python

org_eaws_albina_mqtt_avalanche_bulletin_async:  Callable[[PartitionContext, EventData, CloudEvent, AvalancheBulletin],
Awaitable[None]]
...
- `data`: The event data of type `eaws_albina_mqtt_producer_data.AvalancheBulletin`.

Example:

```python
async def org_eaws_albina_mqtt_avalanche_bulletin_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: AvalancheBulletin) -> None:
