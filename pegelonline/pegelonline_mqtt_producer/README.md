# Pegelonline_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the pegelonline_mqtt_producer message definitions.

# Pegelonline_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - DeWsvPegelonlineMqttEventDispatcher
...
from pegelonline_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = DeWsvPegelonlineMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### DeWsvPegelonlineMqttEventDispatcher



# Publish message`DeWsvPegelonlineMqttEventDispatcher` handles events for the de.wsv.pegelonline.mqtt message group.
...
from pegelonline_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = DeWsvPegelonlineMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe DeWsvPegelonlineMqttEventDispatcher defines the following event handler hooks.
...
```##### `de_wsv_pegelonline_mqtt_station_async`
...
```python
de_wsv_pegelonline_mqtt_dispatcher.de_wsv_pegelonline_mqtt_station_async = de_wsv_pegelonline_mqtt_station_event
```
...
```##### `de_wsv_pegelonline_mqtt_current_measurement_async`
...
```python
de_wsv_pegelonline_mqtt_dispatcher.de_wsv_pegelonline_mqtt_current_measurement_async =
de_wsv_pegelonline_mqtt_current_measurement_event
```
