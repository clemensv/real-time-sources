...
def get_default_topic_mappings_de_wsv_pegelonline_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the de.wsv.pegelonline.mqtt message group.
...
def get_subscription_topics_de_wsv_pegelonline_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_de_wsv_pegelonline_mqtt()
    topics = []
...
class DeWsvPegelonlineMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the de.wsv.pegelonline.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_de_wsv_pegelonline_mqtt()
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_de_wsv_pegelonline_mqtt(self._topic_mappings)
        for topic in topics:
