...
def test_uk_gov_tfl_road_disruptions_ukgovtflroadroaddisruption(kafka_emulator):
    ...
    # Send 5 messages to test message settlement and ordering
    for i in range(5):
        producer_instance.send_uk_gov_tfl_road_road_disruption(_disruption_id = f'test_{i}', data = event_data)
    ...
    for i in range(5):
        received_key = on_event()
        assert received_key is not None, f"Failed to receive message {i+1} of 5"
        expected_key = "disruptions/{disruption_id}".format(disruption_id=f'test_{i}')
        assert received_key == expected_key, f"Expected Kafka key '{expected_key}' but got '{received_key}'"
    consumer.close()
