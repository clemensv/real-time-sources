...
class UkGovTflRoadDisruptionsEventProducer:
    ...
    def send_uk_gov_tfl_road_road_disruption(self,_disruption_id : str, data: RoadDisruption, content_type: str = "application/json", flush_producer=True, key_mapper: typing.Callable[[CloudEvent, RoadDisruption], str]=None) -> None:
        """
        ...
        Args:
            ...
            key_mapper(Callable[[CloudEvent, RoadDisruption], str]): A function to map the CloudEvent contents to a Kafka key (default: None).
                The default key is derived from the xRegistry Kafka key declaration 'disruptions/{disruption_id}'
        """
        kafka_key = "disruptions/{disruption_id}".format(disruption_id=_disruption_id)
        attributes = {
             "type":"uk.gov.tfl.road.RoadDisruption",
             "source":"https://api.tfl.gov.uk/Road/all/Disruption",
             "subject":"disruptions/{disruption_id}".format(disruption_id = _disruption_id)
        }
        attributes["datacontenttype"] = content_type
        event = CloudEvent.create(attributes, data)
        if self.content_mode == "structured":
