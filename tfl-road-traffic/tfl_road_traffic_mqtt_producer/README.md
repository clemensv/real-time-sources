# Tfl_road_traffic_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the tfl_road_traffic_mqtt_producer message definitions.

# Tfl_road_traffic_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - UkGovTflRoadMqttEventDispatcher
...
from tfl_road_traffic_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = UkGovTflRoadMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### UkGovTflRoadMqttEventDispatcher



# Publish message`UkGovTflRoadMqttEventDispatcher` handles events for the uk.gov.tfl.road.mqtt message group.
...
from tfl_road_traffic_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = UkGovTflRoadMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe UkGovTflRoadMqttEventDispatcher defines the following event handler hooks.
...
- `data`: The event data of type `tfl_road_traffic_mqtt_producer_data.RoadStatus`.
...
```##### `uk_gov_tfl_road_mqtt_road_disruption_serious_async`



## Test```python

uk_gov_tfl_road_mqtt_road_disruption_serious_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadDisruption],
Awaitable[None]]
...
```Asynchronous handler hook for `uk.gov.tfl.road.mqtt.RoadDisruptionSerious`: Real-time road disruption event on the
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `tfl_road_traffic_mqtt_producer_data.RoadDisruption`.

Example:

```python
async def uk_gov_tfl_road_mqtt_road_disruption_serious_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RoadDisruption) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `uk_gov_tfl_road_mqtt_road_disruption_severe_async`



## Test```python

uk_gov_tfl_road_mqtt_road_disruption_severe_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadDisruption],
Awaitable[None]]
...
```python
async def uk_gov_tfl_road_mqtt_road_disruption_severe_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RoadDisruption) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `uk_gov_tfl_road_mqtt_road_disruption_moderate_async`



## Test```python

uk_gov_tfl_road_mqtt_road_disruption_moderate_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```Asynchronous handler hook for `uk.gov.tfl.road.mqtt.RoadDisruptionModerate`: Real-time road disruption event on the
TfL road network fetched from GET /Road/all/Disruption.
...
```python
async def uk_gov_tfl_road_mqtt_road_disruption_moderate_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RoadDisruption) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `uk_gov_tfl_road_mqtt_road_disruption_minor_async`



## Test```python

uk_gov_tfl_road_mqtt_road_disruption_minor_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadDisruption],
Awaitable[None]]
...
```python
async def uk_gov_tfl_road_mqtt_road_disruption_minor_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RoadDisruption) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `uk_gov_tfl_road_mqtt_road_disruption_information_async`



## Test```python

uk_gov_tfl_road_mqtt_road_disruption_information_async:  Callable[[PartitionContext, EventData, CloudEvent,
RoadDisruption], Awaitable[None]]
...
```Asynchronous handler hook for `uk.gov.tfl.road.mqtt.RoadDisruptionInformation`: Real-time road disruption event on
the TfL road network fetched from GET /Road/all/Disruption.
...
```python
async def uk_gov_tfl_road_mqtt_road_disruption_information_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RoadDisruption) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `uk_gov_tfl_road_mqtt_road_disruption_closure_async`



## Test```python

uk_gov_tfl_road_mqtt_road_disruption_closure_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadDisruption],
Awaitable[None]]
...
```Asynchronous handler hook for `uk.gov.tfl.road.mqtt.RoadDisruptionClosure`: Real-time road disruption event on the
TfL road network fetched from GET /Road/all/Disruption.
...
```python
async def uk_gov_tfl_road_mqtt_road_disruption_closure_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RoadDisruption) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
