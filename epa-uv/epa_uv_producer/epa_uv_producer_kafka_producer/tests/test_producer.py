...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../epa_uv_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../epa_uv_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../epa_uv_producer_kafka_producer/src')))
...
def test_us_epa_uvindex_usepauvindexhourlyforecast(kafka_emulator):
    """Test the USEPAUVIndexHourlyForecast event from the US.EPA.UVIndex message group"""
    ...
    consumer = Consumer({
        ...
        'group.id': 'test_us_epa_uvindex_usepauvindexhourlyforecast',  # Unique group per test
    ...
    producer_instance = USEPAUVIndexEventProducer(kafka_producer, topic, 'binary')
...
def test_us_epa_uvindex_usepauvindexdailyforecast(kafka_emulator):
    """Test the USEPAUVIndexDailyForecast event from the US.EPA.UVIndex message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_us_epa_uvindex_usepauvindexdailyforecast',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
...
def test_us_epa_uvindex_cross_event_type_kafka_key(kafka_emulator):
    """Test that different event types in US.EPA.UVIndex produce the same Kafka key for the same placeholder values"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_us_epa_uvindex_cross_key',
    ...
    data1 = Test_HourlyForecast.create_instance()
    data2 = Test_DailyForecast.create_instance()

    producer_instance.send_us_epa_uvindex_hourly_forecast(_location_id = shared_key_value, data = data1)
    producer_instance.send_us_epa_uvindex_daily_forecast(_location_id = shared_key_value, data = data2)
    kafka_producer.flush(timeout=5.0)
    ...
    while len(collected_keys) < 2 and time.time() < timeout:
        ...
        if msg is None or msg.error():
            continue
        cloudevent = parse_cloudevent(msg)
        if cloudevent['type'] in ["US.EPA.UVIndex.HourlyForecast", "US.EPA.UVIndex.DailyForecast"]:
    ...
    expected_key = "{location_id}".format(location_id=shared_key_value)
