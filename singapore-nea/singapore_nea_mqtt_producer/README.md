# Singapore_nea_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the singapore_nea_mqtt_producer message definitions.

# Singapore_nea_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - SGGovNEAWeatherMqttEventDispatcher,

    SGGovNEAAirQualityMqttEventDispatcher
...
from singapore_nea_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = SGGovNEAWeatherMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### SGGovNEAWeatherMqttEventDispatcher



# Publish message`SGGovNEAWeatherMqttEventDispatcher` handles events for the SG.Gov.NEA.Weather.mqtt message group.
...
from singapore_nea_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = SGGovNEAWeatherMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
client = SGGovNEAAirQualityMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe SGGovNEAWeatherMqttEventDispatcher defines the following event handler hooks.
...
```##### `sg_gov_nea_weather_station_mqtt_async`



## Test```python

sg_gov_nea_weather_station_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]
...
- `data`: The event data of type `singapore_nea_mqtt_producer_data.Station`.

Example:

```python
async def sg_gov_nea_weather_station_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `sg_gov_nea_weather_weather_observation_mqtt_async`



## Test```python

sg_gov_nea_weather_weather_observation_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `singapore_nea_mqtt_producer_data.WeatherObservation`.

Example:

```python
async def sg_gov_nea_weather_weather_observation_mqtt_event(partition_context: PartitionContext, event: EventData,
...
client = SGGovNEAAirQualityMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### SGGovNEAAirQualityMqttEventDispatcher



# Publish message`SGGovNEAAirQualityMqttEventDispatcher` handles events for the SG.Gov.NEA.AirQuality.mqtt message
group.
...
## BuildThe SGGovNEAAirQualityMqttEventDispatcher defines the following event handler hooks.
...
```##### `sg_gov_nea_air_quality_region_mqtt_async`



## Test```python

sg_gov_nea_air_quality_region_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, Region], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `SG.Gov.NEA.AirQuality.Region.mqtt`: A reference record published by Singapore's
National Environment Agency (NEA). It lets consumers label, group, and route the live measurement or forecast events.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `singapore_nea_mqtt_producer_data.Region`.

Example:

```python
async def sg_gov_nea_air_quality_region_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Region) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `sg_gov_nea_air_quality_psireading_mqtt_async`



## Test```python

sg_gov_nea_air_quality_psireading_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, PSIReading],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `singapore_nea_mqtt_producer_data.PSIReading`.

Example:

```python
async def sg_gov_nea_air_quality_psireading_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: PSIReading) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `sg_gov_nea_air_quality_pm25_reading_mqtt_async`



## Test```python

sg_gov_nea_air_quality_pm25_reading_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, PM25Reading],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `singapore_nea_mqtt_producer_data.PM25Reading`.

Example:

```python
async def sg_gov_nea_air_quality_pm25_reading_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: PM25Reading) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
