...
import os
import signal
from ireland_opw_waterlevel_mqtt_producer_data import *
from ireland_opw_waterlevel_mqtt_producer_mqtt_client.client import IeGovOpwWaterlevelMqttProducer, IeGovOpwWaterlevelMqttDispatcher

async def handle_ie_gov_opw_waterlevel_mqtt_station(mqtt_msg,cloud_event, ie_gov_opw_waterlevel_mqtt_station_data):
    """ Handles the ie.gov.opw.waterlevel.mqtt.Station message """
    print(f"Received ie.gov.opw.waterlevel.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {ie_gov_opw_waterlevel_mqtt_station_data}")

async def handle_ie_gov_opw_waterlevel_mqtt_water_level_reading(mqtt_msg,cloud_event, ie_gov_opw_waterlevel_mqtt_water_level_reading_data):
    """ Handles the ie.gov.opw.waterlevel.mqtt.WaterLevelReading message """
    print(f"Received ie.gov.opw.waterlevel.mqtt.WaterLevelReading on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {ie_gov_opw_waterlevel_mqtt_water_level_reading_data}")
