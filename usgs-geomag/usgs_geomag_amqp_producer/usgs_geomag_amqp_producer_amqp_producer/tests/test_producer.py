...
"""
Tests for usgs_geomag_amqp_producer_amqp_producer
"""
import base64
import json
import os
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../usgs_geomag_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../usgs_geomag_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../usgs_geomag_amqp_producer_amqp_producer/src')))

from usgs_geomag_amqp_producer_amqp_producer import *
from usgs_geomag_amqp_producer_data import Observatory
from test_usgs_geomag_amqp_producer_data_observatory import Test_Observatory
from usgs_geomag_amqp_producer_data import MagneticFieldReading
from test_usgs_geomag_amqp_producer_data_magneticfieldreading import Test_MagneticFieldReading
...
class TestGovUsgsGeomagAmqpProducer:
    """Test cases for GovUsgsGeomagAmqpProducer"""
    ...
    def test_send_observatory(self, artemis_container):
        """Send and receive a Observatory message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_Observatory.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "gov.usgs.geomag.amqp.Observatory"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{iaga_code}".format(iaga_code="value")
        finally:
            producer.close()
    
    def test_send_magnetic_field_reading(self, artemis_container):
        """Send and receive a MagneticFieldReading message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_MagneticFieldReading.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "gov.usgs.geomag.amqp.MagneticFieldReading"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
