...
class GovUsgsGeomagAmqpProducer:
    """
    Producer class to send messages in the `gov.usgs.geomag.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_observatory(self,
        data: Observatory,
        _iaga_code: str,
        content_type: str = 'application/json') -> None:
        """
        Send the `gov.usgs.geomag.amqp.Observatory` message
        ...
        attributes = {
            ...
            "subject":
            "{iaga_code}".format(iaga_code=_iaga_code),
        }
        ...
        amqp_msg.subject = "{iaga_code}".format(iaga_code=_iaga_code)
    ...
    def send_observatory_batch(self,
        data_array: typing.List[Observatory],
        _iaga_code: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `gov.usgs.geomag.amqp.Observatory` messages
        
        Args:
            data_array (typing.List[Observatory]): Array of message data objects
            _iaga_code (str): Value for placeholder iaga_code in attribute subject
            content_type (str): The content type of the message data
        """
        for data in data_array:
            self.send_observatory(
                data=data,
                _iaga_code=_iaga_code,
                content_type=content_type)
    
    
    def send_magnetic_field_reading(self,
        data: MagneticFieldReading,
        _iaga_code: str,
        content_type: str = 'application/json') -> None:
        """
        Send the `gov.usgs.geomag.amqp.MagneticFieldReading` message
    ...
    def send_magnetic_field_reading_batch(self,
        data_array: typing.List[MagneticFieldReading],
        _iaga_code: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `gov.usgs.geomag.amqp.MagneticFieldReading` messages
        
        Args:
            data_array (typing.List[MagneticFieldReading]): Array of message data objects
            _iaga_code (str): Value for placeholder iaga_code in attribute subject
            content_type (str): The content type of the message data
        """
        for data in data_array:
            self.send_magnetic_field_reading(
                data=data,
                _iaga_code=_iaga_code,
                content_type=content_type)
