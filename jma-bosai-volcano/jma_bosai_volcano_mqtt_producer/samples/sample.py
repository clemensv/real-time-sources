...
import os
import signal
from jma_bosai_volcano_mqtt_producer_data import *
from jma_bosai_volcano_mqtt_producer_mqtt_client.client import JPJMAVolcanoMqttProducer, JPJMAVolcanoMqttDispatcher

async def handle_jp_jma_volcano_mqtt_volcano(mqtt_msg,cloud_event, jp_jma_volcano_mqtt_volcano_data):
    """ Handles the JP.JMA.Volcano.mqtt.Volcano message """
    print(f"Received JP.JMA.Volcano.mqtt.Volcano on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_jma_volcano_mqtt_volcano_data}")

async def handle_jp_jma_volcano_mqtt_volcanic_warning(mqtt_msg,cloud_event, jp_jma_volcano_mqtt_volcanic_warning_data):
    """ Handles the JP.JMA.Volcano.mqtt.VolcanicWarning message """
    print(f"Received JP.JMA.Volcano.mqtt.VolcanicWarning on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_jma_volcano_mqtt_volcanic_warning_data}")

async def handle_jp_jma_volcano_mqtt_volcanic_eruption(mqtt_msg,cloud_event, jp_jma_volcano_mqtt_volcanic_eruption_data):
    """ Handles the JP.JMA.Volcano.mqtt.VolcanicEruption message """
    print(f"Received JP.JMA.Volcano.mqtt.VolcanicEruption on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {jp_jma_volcano_mqtt_volcanic_eruption_data}")
