# Jma_bosai_volcano_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the jma_bosai_volcano_mqtt_producer message definitions.

# Jma_bosai_volcano_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - JPJMAVolcanoMqttEventDispatcher
...
from jma_bosai_volcano_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = JPJMAVolcanoMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### JPJMAVolcanoMqttEventDispatcher



# Publish message`JPJMAVolcanoMqttEventDispatcher` handles events for the JP.JMA.Volcano.mqtt message group.
...
from jma_bosai_volcano_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = JPJMAVolcanoMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe JPJMAVolcanoMqttEventDispatcher defines the following event handler hooks.
...
```##### `jp_jma_volcano_mqtt_volcano_async`



## Test```python

jp_jma_volcano_mqtt_volcano_async:  Callable[[PartitionContext, EventData, CloudEvent, Volcano], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `JP.JMA.Volcano.mqtt.Volcano`: JMA Bosai volcano catalog reference data emitted at
startup and on monthly refresh.
...
- `data`: The event data of type `jma_bosai_volcano_mqtt_producer_data.Volcano`.

Example:

```python
async def jp_jma_volcano_mqtt_volcano_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Volcano) -> None:
...
```python
jp_jma_volcano_mqtt_dispatcher.jp_jma_volcano_mqtt_volcano_async = jp_jma_volcano_mqtt_volcano_event
```
...
```##### `jp_jma_volcano_mqtt_volcanic_warning_async`



## Test```python

jp_jma_volcano_mqtt_volcanic_warning_async:  Callable[[PartitionContext, EventData, CloudEvent, VolcanicWarning],
Awaitable[None]]
...
```Asynchronous handler hook for `JP.JMA.Volcano.mqtt.VolcanicWarning`: JMA Bosai active volcanic warning or forecast
for a target volcano.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `jma_bosai_volcano_mqtt_producer_data.VolcanicWarning`.

Example:

```python
async def jp_jma_volcano_mqtt_volcanic_warning_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: VolcanicWarning) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
jp_jma_volcano_mqtt_dispatcher.jp_jma_volcano_mqtt_volcanic_warning_async = jp_jma_volcano_mqtt_volcanic_warning_event
```
...
```##### `jp_jma_volcano_mqtt_volcanic_eruption_async`



## Test```python

jp_jma_volcano_mqtt_volcanic_eruption_async:  Callable[[PartitionContext, EventData, CloudEvent, VolcanicEruption],
Awaitable[None]]
...
```Asynchronous handler hook for `JP.JMA.Volcano.mqtt.VolcanicEruption`: JMA Bosai volcanic eruption observation report
for a target volcano.
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `jma_bosai_volcano_mqtt_producer_data.VolcanicEruption`.

Example:

```python
async def jp_jma_volcano_mqtt_volcanic_eruption_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: VolcanicEruption) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```python
jp_jma_volcano_mqtt_dispatcher.jp_jma_volcano_mqtt_volcanic_eruption_async = jp_jma_volcano_mqtt_volcanic_eruption_event
```
