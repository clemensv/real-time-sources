...
1. **Reference data refresh** — Every hour, all `Event` records are re-emitted so consumers can maintain a current view of public listing metadata, including schedule, venue, address, organizer contact details, images, and purchase URLs.
2. **Sale-status telemetry** — `EventSaleStatus` events are emitted whenever the `sale_status` of an event changes since the last poll. Each telemetry event carries an `observed_at` timestamp generated by the bridge because the public API does not expose an upstream last-updated field for event listings.
