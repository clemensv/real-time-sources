...
c.subscribe(('civic-events/intl/fienta/fienta/+/+/+/+/info', 1))
c.loop_forever()
```
...
Attach a link with `role=receiver` whose **source** is `fienta`. The source terminus is the broker-side node you consume from; source filters such as selectors, Event Hubs offsets, or subscription filters further select which messages flow. The target is your client-side terminus. Generic brokers use their advertised SASL mechanisms (often PLAIN over TLS, EXTERNAL with mTLS, or ANONYMOUS on trusted links). Azure Service Bus and Event Hubs can use SASL PLAIN for SAS credentials on short-lived connections; CBS `put-token` on `$cbs` installs and refreshes Entra ID JWTs or SAS tokens for long-lived AMQP connections.

```python
from proton.handlers import MessagingHandler
from proton.reactor import Container
class H(MessagingHandler):
    def on_start(self,e): e.container.create_receiver('amqps://user:pass@localhost:5671/fienta')
...
| `MQTT/5.0` | topic `civic-events/intl/fienta/fienta/{country}/{city}/{category}/{event_id}/info`, retain `true`, QoS `1` |
| `AMQP/1.0` | source address `amqps://localhost:5671/fienta`, message subject `{event_id}` |
...
| --- | --- |
| `KAFKA` | topic `fienta`, key `{event_id}` |
| `MQTT/5.0` | topic `civic-events/intl/fienta/fienta/{country}/{city}/{category}/{event_id}/sale-status`, retain `true`, QoS `1` |
| `AMQP/1.0` | source address `amqps://localhost:5671/fienta`, message subject `{event_id}` |
