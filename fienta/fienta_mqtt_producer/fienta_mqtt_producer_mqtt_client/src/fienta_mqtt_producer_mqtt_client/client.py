...
def get_default_topic_mappings_com_fienta_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the Com.Fienta.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "Com.Fienta.mqtt.Event": "civic-events/intl/fienta/fienta/{country}/{city}/{category}/{event_id}/info",
        "Com.Fienta.mqtt.EventSaleStatus": "civic-events/intl/fienta/fienta/{country}/{city}/{category}/{event_id}/sale-status",
    }


def get_subscription_topics_com_fienta_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_com_fienta_mqtt()
    topics = []
...
class ComFientaMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the Com.Fienta.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_com_fienta_mqtt()
        ...
        self.com_fienta_mqtt_event_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, fienta_mqtt_producer_data.Event, Dict[str, str]], Awaitable[None]]] = None
        
        self.com_fienta_mqtt_event_sale_status_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, fienta_mqtt_producer_data.EventSaleStatus, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "Com.Fienta.Event":
            if self.com_fienta_mqtt_event_async:
                try:
                    ...
                    data = fienta_mqtt_producer_data.Event.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Com.Fienta.mqtt.Event")
                    await self.com_fienta_mqtt_event_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in com_fienta_mqtt_event handler: {e}")
            return
        
        if event_type == "Com.Fienta.EventSaleStatus":
            if self.com_fienta_mqtt_event_sale_status_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = fienta_mqtt_producer_data.EventSaleStatus.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Com.Fienta.mqtt.EventSaleStatus")
                    await self.com_fienta_mqtt_event_sale_status_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in com_fienta_mqtt_event_sale_status handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_com_fienta_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_com_fienta_mqtt_event(self,
        event_id: str,
        country: str,
        city: str,
        category: str,
        data: fienta_mqtt_producer_data.Event,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Com.Fienta.mqtt.Event' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'civic-events/intl/fienta/fienta/{country}/{city}/{category}/{event_id}/info'
        ...
        """
        target_topic = topic if topic is not None else "civic-events/intl/fienta/fienta/{country}/{city}/{category}/{event_id}/info"
    ...
    async def publish_com_fienta_mqtt_event_sale_status(self,
        event_id: str,
        country: str,
        city: str,
        category: str,
        data: fienta_mqtt_producer_data.EventSaleStatus,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Com.Fienta.mqtt.EventSaleStatus' event to an MQTT topic.
        ...
        Args:
            ...
            category: URI template variable for 'category'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'civic-events/intl/fienta/fienta/{country}/{city}/{category}/{event_id}/sale-status'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "civic-events/intl/fienta/fienta/{country}/{city}/{category}/{event_id}/sale-status"
        _topic_template_values: Dict[str, str] = {
            "event_id": str(event_id),
            "country": str(country),
            "city": str(city),
