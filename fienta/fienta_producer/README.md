...
3. [Quick Start](#quick-start)    - ComFientaEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    ComFientaMqttEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    ComFientaAmqpEventDispatcher
...
- ComFientaProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- ComFientaMqttProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- ComFientaAmqpProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### ComFientaEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`ComFientaEventDispatcher` handles events for the Com.Fienta message group.
...
await producer.send_com_fienta_event(```python
...
The ComFientaEventDispatcher defines the following event handler hooks.
...
producer = ComFientaProducer(
    ...
    security_protocol='SASL_SSL',##### `com_fienta_event_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'com_fienta_event_async:  Callable[[ConsumerRecord, CloudEvent, Event], Awaitable[None]]

)```

```

Asynchronous handler hook for `Com.Fienta.Event`:
...
### ComFientaProducer- `data`: The event data of type `fienta_producer_data.Event`.



Producer for `Com.Fienta` message group.Example:



#### Constructor```python

async def com_fienta_event_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Event) -> None:

```python    # Process the event data

ComFientaProducer(    await some_processing_function(record, cloud_event, data)
...
com_fienta_dispatcher.com_fienta_event_async = com_fienta_event_event
...
### ComFientaMqttProducer- `data`: The event data of type `fienta_producer_data.Event`.



Producer for `Com.Fienta.mqtt` message group.Example:
...
ComFientaMqttProducer(    await some_processing_function(record, cloud_event, data)
...
com_fienta_mqtt_dispatcher.com_fienta_event_async = com_fienta_event_event
...
### ComFientaAmqpProducer- `data`: The event data of type `fienta_producer_data.Event`.



Producer for `Com.Fienta.amqp` message group.Example:
...
ComFientaAmqpProducer(    await some_processing_function(record, cloud_event, data)
...
com_fienta_amqp_dispatcher.com_fienta_event_async = com_fienta_event_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `com_fienta_event_sale_status_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'com_fienta_event_sale_status_async:  Callable[[ConsumerRecord, CloudEvent,
EventSaleStatus], Awaitable[None]]

)```

```

Asynchronous handler hook for `Com.Fienta.EventSaleStatus`:
...
### ComFientaProducer- `data`: The event data of type `fienta_producer_data.EventSaleStatus`.
...
async def com_fienta_event_sale_status_event(record: ConsumerRecord, cloud_event: CloudEvent, data: EventSaleStatus) ->
None:
...
com_fienta_dispatcher.com_fienta_event_sale_status_async = com_fienta_event_sale_status_event
...
### ComFientaMqttProducer- `data`: The event data of type `fienta_producer_data.EventSaleStatus`.
...
com_fienta_mqtt_dispatcher.com_fienta_event_sale_status_async = com_fienta_event_sale_status_event
...
### ComFientaAmqpProducer- `data`: The event data of type `fienta_producer_data.EventSaleStatus`.
...
com_fienta_amqp_dispatcher.com_fienta_event_sale_status_async = com_fienta_event_sale_status_event
...
##### `send_com_fienta_event`Dispatchers have the following protected methods:



```python### Methods:

async def send_com_fienta_event(
...
Send a single `Com.Fienta.Event` message.Args:
...
await producer.send_com_fienta_event(
...
Send multiple `Com.Fienta.Event` messages in a batch.
...
await producer.send_com_fienta_event_batch(```
...
##### `send_com_fienta_event_sale_status`Dispatchers have the following protected methods:



```python### Methods:

async def send_com_fienta_event_sale_status(

    self,##### `_process_event`

    data: EventSaleStatus,
...
Send a single `Com.Fienta.EventSaleStatus` message.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `EventSaleStatus`
...
await producer.send_com_fienta_event_sale_status(

    data=EventSaleStatus(...),Args:
...
Send multiple `Com.Fienta.EventSaleStatus` messages in a batch.
...
await producer.send_com_fienta_event_sale_status_batch(```

    messages=[

        EventSaleStatus(...),Initializes the runner with a Kafka consumer.

        EventSaleStatus(...),

        EventSaleStatus(...)Args:
...
- **Scales horizontally** across multiple brokers and partitions### ComFientaMqttEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`ComFientaMqttEventDispatcher` handles events for the Com.Fienta.mqtt message group.
...
The ComFientaMqttEventDispatcher defines the following event handler hooks.
...
producer = ComFientaProducer(
    ...
    security_protocol='SASL_SSL',##### `com_fienta_mqtt_event_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'com_fienta_mqtt_event_async:  Callable[[ConsumerRecord, CloudEvent, Event],
Awaitable[None]]
...
Asynchronous handler hook for `Com.Fienta.mqtt.Event`:
...
async def com_fienta_mqtt_event_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Event) -> None:
...
com_fienta_dispatcher.com_fienta_mqtt_event_async = com_fienta_mqtt_event_event
...
com_fienta_mqtt_dispatcher.com_fienta_mqtt_event_async = com_fienta_mqtt_event_event
...
com_fienta_amqp_dispatcher.com_fienta_mqtt_event_async = com_fienta_mqtt_event_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `com_fienta_mqtt_event_sale_status_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'com_fienta_mqtt_event_sale_status_async:  Callable[[ConsumerRecord, CloudEvent,
EventSaleStatus], Awaitable[None]]
...
Asynchronous handler hook for `Com.Fienta.mqtt.EventSaleStatus`:
...
async def com_fienta_mqtt_event_sale_status_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
EventSaleStatus) -> None:
...
com_fienta_dispatcher.com_fienta_mqtt_event_sale_status_async = com_fienta_mqtt_event_sale_status_event
...
com_fienta_mqtt_dispatcher.com_fienta_mqtt_event_sale_status_async = com_fienta_mqtt_event_sale_status_event
...
com_fienta_amqp_dispatcher.com_fienta_mqtt_event_sale_status_async = com_fienta_mqtt_event_sale_status_event
...
##### `send_com_fienta_mqtt_event`Dispatchers have the following protected methods:



```python### Methods:

async def send_com_fienta_mqtt_event(
...
Send a single `Com.Fienta.mqtt.Event` message.Args:
...
await producer.send_com_fienta_mqtt_event(
...
Send multiple `Com.Fienta.mqtt.Event` messages in a batch.
...
await producer.send_com_fienta_mqtt_event_batch(```
...
##### `send_com_fienta_mqtt_event_sale_status`Dispatchers have the following protected methods:



```python### Methods:

async def send_com_fienta_mqtt_event_sale_status(
...
Send a single `Com.Fienta.mqtt.EventSaleStatus` message.Args:
...
await producer.send_com_fienta_mqtt_event_sale_status(
...
Send multiple `Com.Fienta.mqtt.EventSaleStatus` messages in a batch.
...
await producer.send_com_fienta_mqtt_event_sale_status_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### ComFientaAmqpEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`ComFientaAmqpEventDispatcher` handles events for the Com.Fienta.amqp message group.
...
The ComFientaAmqpEventDispatcher defines the following event handler hooks.
...
producer = ComFientaProducer(
    ...
    security_protocol='SASL_SSL',##### `com_fienta_amqp_event_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'com_fienta_amqp_event_async:  Callable[[ConsumerRecord, CloudEvent, Event],
Awaitable[None]]
...
Asynchronous handler hook for `Com.Fienta.amqp.Event`:
...
async def com_fienta_amqp_event_event(record: ConsumerRecord, cloud_event: CloudEvent, data: Event) -> None:
...
com_fienta_dispatcher.com_fienta_amqp_event_async = com_fienta_amqp_event_event
...
com_fienta_mqtt_dispatcher.com_fienta_amqp_event_async = com_fienta_amqp_event_event
...
com_fienta_amqp_dispatcher.com_fienta_amqp_event_async = com_fienta_amqp_event_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `com_fienta_amqp_event_sale_status_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'com_fienta_amqp_event_sale_status_async:  Callable[[ConsumerRecord, CloudEvent,
EventSaleStatus], Awaitable[None]]
...
Asynchronous handler hook for `Com.Fienta.amqp.EventSaleStatus`:
...
async def com_fienta_amqp_event_sale_status_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
EventSaleStatus) -> None:
...
com_fienta_dispatcher.com_fienta_amqp_event_sale_status_async = com_fienta_amqp_event_sale_status_event
...
com_fienta_mqtt_dispatcher.com_fienta_amqp_event_sale_status_async = com_fienta_amqp_event_sale_status_event
...
com_fienta_amqp_dispatcher.com_fienta_amqp_event_sale_status_async = com_fienta_amqp_event_sale_status_event
...
##### `send_com_fienta_amqp_event`Dispatchers have the following protected methods:



```python### Methods:

async def send_com_fienta_amqp_event(
...
Send a single `Com.Fienta.amqp.Event` message.Args:
...
await producer.send_com_fienta_amqp_event(
...
Send multiple `Com.Fienta.amqp.Event` messages in a batch.
...
await producer.send_com_fienta_amqp_event_batch(```
...
##### `send_com_fienta_amqp_event_sale_status`Dispatchers have the following protected methods:



```python### Methods:

async def send_com_fienta_amqp_event_sale_status(
...
Send a single `Com.Fienta.amqp.EventSaleStatus` message.Args:
...
await producer.send_com_fienta_amqp_event_sale_status(
...
Send multiple `Com.Fienta.amqp.EventSaleStatus` messages in a batch.
...
await producer.send_com_fienta_amqp_event_sale_status_batch(```
