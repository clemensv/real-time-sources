...
class UkKclLaqnAmqpProducer:
    """
    Producer class to send messages in the `uk.kcl.laqn.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_site(self,
        ...
        app_properties = {}
        app_properties["borough"] = "{borough}".format(borough=_borough)
        app_properties["species_code"] = "{species_code}".format(species_code=_species_code)
        if app_properties:
    ...
    def send_measurement(self,
        ...
        attributes = {
            ...
            "source":
            "http://api.erg.ic.ac.uk/AirQuality/Data/Site",
            "subject":
            "{site_code}".format(site_code=_site_code),
        }
...
class UkKclLaqnSpeciesAmqpProducer:
    """
    Producer class to send messages in the `uk.kcl.laqn.species.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_species(self,
        ...
        app_properties = {}
        app_properties["borough"] = "{borough}".format(borough=_borough)
        app_properties["site_code"] = "{site_code}".format(site_code=_site_code)
        if app_properties:
            if amqp_msg.properties is None:
                amqp_msg.properties = {}
            amqp_msg.properties.update(app_properties)
