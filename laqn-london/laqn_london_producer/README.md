...
# Laqn_london_producer Kafka Producer# Laqn_london_producer Event Dispatcher for Apache Kafka
...
- UkKclLaqnProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### UkKclLaqnEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`UkKclLaqnEventDispatcher` handles events for the uk.kcl.laqn message group.
...
The UkKclLaqnEventDispatcher defines the following event handler hooks.
...
producer = UkKclLaqnProducer(
    ...
    security_protocol='SASL_SSL',##### `uk_kcl_laqn_site_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_kcl_laqn_site_async:  Callable[[ConsumerRecord, CloudEvent, Site], Awaitable[None]]

)```

```

Asynchronous handler hook for `uk.kcl.laqn.Site`: LAQN monitoring site reference data, including stable site identity,
operator information, and WGS84 coordinates.
...
uk_kcl_laqn_dispatcher.uk_kcl_laqn_site_async = uk_kcl_laqn_site_event
...
uk_kcl_laqn_species_dispatcher.uk_kcl_laqn_site_async = uk_kcl_laqn_site_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `uk_kcl_laqn_measurement_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_kcl_laqn_measurement_async:  Callable[[ConsumerRecord, CloudEvent, Measurement],
Awaitable[None]]
...
Asynchronous handler hook for `uk.kcl.laqn.Measurement`: LAQN hourly pollutant measurement for a site and species at a
GMT timestamp.
...
uk_kcl_laqn_dispatcher.uk_kcl_laqn_measurement_async = uk_kcl_laqn_measurement_event
...
uk_kcl_laqn_species_dispatcher.uk_kcl_laqn_measurement_async = uk_kcl_laqn_measurement_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `uk_kcl_laqn_daily_index_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_kcl_laqn_daily_index_async:  Callable[[ConsumerRecord, CloudEvent, DailyIndex],
Awaitable[None]]
...
Asynchronous handler hook for `uk.kcl.laqn.DailyIndex`: LAQN Daily Air Quality Index (DAQI) for a site and pollutant,
published as the latest London-wide bulletin.
...
uk_kcl_laqn_dispatcher.uk_kcl_laqn_daily_index_async = uk_kcl_laqn_daily_index_event
...
##### `send_uk_kcl_laqn_site`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_kcl_laqn_site(
...
Send a single `uk.kcl.laqn.Site` message. LAQN monitoring site reference data, including stable site identity, operator
information, and WGS84 coordinates.Args:
...
await producer.send_uk_kcl_laqn_site(
...
Send multiple `uk.kcl.laqn.Site` messages in a batch.
...
await producer.send_uk_kcl_laqn_site_batch(```
...
##### `send_uk_kcl_laqn_measurement`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_kcl_laqn_measurement(
...
Send a single `uk.kcl.laqn.Measurement` message. LAQN hourly pollutant measurement for a site and species at a GMT
timestamp.Args:
...
await producer.send_uk_kcl_laqn_measurement(
...
Send multiple `uk.kcl.laqn.Measurement` messages in a batch.
...
await producer.send_uk_kcl_laqn_measurement_batch(```
...
##### `send_uk_kcl_laqn_daily_index`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_kcl_laqn_daily_index(
...
Send a single `uk.kcl.laqn.DailyIndex` message. LAQN Daily Air Quality Index (DAQI) for a site and pollutant, published
as the latest London-wide bulletin.Args:
...
await producer.send_uk_kcl_laqn_daily_index(
...
Send multiple `uk.kcl.laqn.DailyIndex` messages in a batch.
...
await producer.send_uk_kcl_laqn_daily_index_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### UkKclLaqnSpeciesEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`UkKclLaqnSpeciesEventDispatcher` handles events for the uk.kcl.laqn.species message group.
...
The UkKclLaqnSpeciesEventDispatcher defines the following event handler hooks.
...
producer = UkKclLaqnProducer(
    ...
    security_protocol='SASL_SSL',##### `uk_kcl_laqn_species_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_kcl_laqn_species_async:  Callable[[ConsumerRecord, CloudEvent, Species],
Awaitable[None]]
...
Asynchronous handler hook for `uk.kcl.laqn.Species`: LAQN pollutant reference data, including descriptive text and
health guidance for a pollutant code.
...
uk_kcl_laqn_dispatcher.uk_kcl_laqn_species_async = uk_kcl_laqn_species_event
...
uk_kcl_laqn_species_dispatcher.uk_kcl_laqn_species_async = uk_kcl_laqn_species_event
...
##### `send_uk_kcl_laqn_species`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_kcl_laqn_species(
...
Send a single `uk.kcl.laqn.Species` message. LAQN pollutant reference data, including descriptive text and health
guidance for a pollutant code.Args:
...
await producer.send_uk_kcl_laqn_species(
...
Send multiple `uk.kcl.laqn.Species` messages in a batch.
