...
import meteoalarm_mqtt_producer_data
from meteoalarm_mqtt_producer_data import WeatherWarning
...
def get_default_topic_mappings_meteoalarm_warnings_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the Meteoalarm.Warnings.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "Meteoalarm.Warnings.mqtt.WeatherWarning": "alerts/intl/meteoalarm/meteoalarm/{country}/{severity}/{awareness_type}/{identifier}/warning",
    }


def get_subscription_topics_meteoalarm_warnings_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_meteoalarm_warnings_mqtt()
    topics = []
...
class MeteoalarmWarningsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the Meteoalarm.Warnings.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_meteoalarm_warnings_mqtt()
        ...
        self.meteoalarm_warnings_mqtt_weather_warning_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, meteoalarm_mqtt_producer_data.WeatherWarning, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "Meteoalarm.WeatherWarning":
            if self.meteoalarm_warnings_mqtt_weather_warning_async:
                try:
                    ...
                    data = meteoalarm_mqtt_producer_data.WeatherWarning.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "Meteoalarm.Warnings.mqtt.WeatherWarning")
                    await self.meteoalarm_warnings_mqtt_weather_warning_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in meteoalarm_warnings_mqtt_weather_warning handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_meteoalarm_warnings_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_meteoalarm_warnings_mqtt_weather_warning(self,
        identifier: str,
        country: str,
        severity: str,
        awareness_type: str,
        data: meteoalarm_mqtt_producer_data.WeatherWarning,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'Meteoalarm.Warnings.mqtt.WeatherWarning' event to an MQTT topic.
        ...
        Args:
            ...
            awareness_type: URI template variable for 'awareness_type'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'alerts/intl/meteoalarm/meteoalarm/{country}/{severity}/{awareness_type}/{identifier}/warning'
        ...
        """
        target_topic = topic if topic is not None else "alerts/intl/meteoalarm/meteoalarm/{country}/{severity}/{awareness_type}/{identifier}/warning"
        _topic_template_values: Dict[str, str] = {
            "identifier": str(identifier),
            "country": str(country),
            "severity": str(severity),
            "awareness_type": str(awareness_type),
        }
        if _topic_template_values:
            target_topic = _apply_topic_template(target_topic, _topic_template_values)

        attributes = {
             "type":"Meteoalarm.WeatherWarning",
