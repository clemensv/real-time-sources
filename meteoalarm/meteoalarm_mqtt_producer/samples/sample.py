...
import os
import signal
from meteoalarm_mqtt_producer_data import *
from meteoalarm_mqtt_producer_mqtt_client.client import MeteoalarmWarningsMqttProducer, MeteoalarmWarningsMqttDispatcher

async def handle_meteoalarm_warnings_mqtt_weather_warning(mqtt_msg,cloud_event, meteoalarm_warnings_mqtt_weather_warning_data):
    """ Handles the Meteoalarm.Warnings.mqtt.WeatherWarning message """
    print(f"Received Meteoalarm.Warnings.mqtt.WeatherWarning on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {meteoalarm_warnings_mqtt_weather_warning_data}")
