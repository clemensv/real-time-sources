...
This source now ships an AMQP 1.0 companion container (`ghcr.io/clemensv/real-time-sources-aisstream-amqp:latest`) alongside the Kafka and MQTT feeders. It publishes the same CloudEvents to one AMQP address (`AMQP_ADDRESS=aisstream` by default) for generic AMQP 1.0 brokers and Azure Service Bus/Event Hubs using CBS authentication.
