...
Deploy with `tools/deploy-fabric/deploy-fabric-aci.ps1 -Source aisstream -Workspace <id> -ResourceGroup <azure-rg> -Location <azure-region>` (the portal button wraps this for you). The script creates the Eventhouse, the KQL database with the [`kql/aisstream.kql`](kql/aisstream.kql) schema and update policies, the Event Stream with a custom endpoint, and the ACI with the connection string and your AISstream.io API key wired in.
