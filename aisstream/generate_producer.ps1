# Generate the AISstream data producer using xrcg

$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$projectRoot = $scriptDir
$xregFile = Join-Path $projectRoot "xreg\aisstream.xreg.json"
$outputDir = Join-Path $projectRoot "aisstream_producer"

Write-Host "Generating AISstream producer from xRegistry definitions..." -ForegroundColor Cyan
...
xrcg generate --style kafkaproducer --language py --projectname aisstream-producer --definitions $xregFile --output $outputDir
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "  3. Run the producer: poetry run python -m aisstream" -ForegroundColor Gray
} else {
