# Autobahn_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the autobahn_mqtt_producer message definitions.

# Autobahn_mqtt_producer Event Dispatcher for Azure Event Hubs
...
from autobahn_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = DEAutobahnMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### DEAutobahnMqttEventDispatcher



# Publish message`DEAutobahnMqttEventDispatcher` handles events for the DE.Autobahn.mqtt message group.
...
from autobahn_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = DEAutobahnMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.-
...
## BuildThe DEAutobahnMqttEventDispatcher defines the following event handler hooks.
...
```##### `de_autobahn_roadwork_appeared_mqtt_async`



## Test```python

de_autobahn_roadwork_appeared_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadEvent],
Awaitable[None]]
...
- `data`: The event data of type `autobahn_mqtt_producer_data.RoadEvent`.

Example:

```python
async def de_autobahn_roadwork_appeared_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: RoadEvent) -> None:
...
```##### `de_autobahn_roadwork_updated_mqtt_async`



## Test```python

de_autobahn_roadwork_updated_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadEvent],
Awaitable[None]]
...
```python
async def de_autobahn_roadwork_updated_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: RoadEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_roadwork_resolved_mqtt_async`



## Test```python

de_autobahn_roadwork_resolved_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadEvent],
Awaitable[None]]
...
```python
async def de_autobahn_roadwork_resolved_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: RoadEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_short_term_roadwork_appeared_mqtt_async`



## Test```python

de_autobahn_short_term_roadwork_appeared_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadEvent],
Awaitable[None]]
...
```python
async def de_autobahn_short_term_roadwork_appeared_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RoadEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_short_term_roadwork_updated_mqtt_async`



## Test```python

de_autobahn_short_term_roadwork_updated_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadEvent],
Awaitable[None]]
...
```python
async def de_autobahn_short_term_roadwork_updated_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RoadEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_short_term_roadwork_resolved_mqtt_async`



## Test```python

de_autobahn_short_term_roadwork_resolved_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadEvent],
Awaitable[None]]
...
```python
async def de_autobahn_short_term_roadwork_resolved_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RoadEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_closure_appeared_mqtt_async`



## Test```python

de_autobahn_closure_appeared_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadEvent],
Awaitable[None]]
...
```python
async def de_autobahn_closure_appeared_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: RoadEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_closure_updated_mqtt_async`



## Test```python

de_autobahn_closure_updated_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadEvent], Awaitable[None]]
...
```python
async def de_autobahn_closure_updated_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: RoadEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_closure_resolved_mqtt_async`



## Test```python

de_autobahn_closure_resolved_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadEvent],
Awaitable[None]]
...
```python
async def de_autobahn_closure_resolved_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: RoadEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_entry_exit_closure_appeared_mqtt_async`



## Test```python

de_autobahn_entry_exit_closure_appeared_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadEvent],
Awaitable[None]]
...
```python
async def de_autobahn_entry_exit_closure_appeared_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RoadEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_entry_exit_closure_updated_mqtt_async`



## Test```python

de_autobahn_entry_exit_closure_updated_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadEvent],
Awaitable[None]]
...
```python
async def de_autobahn_entry_exit_closure_updated_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RoadEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_entry_exit_closure_resolved_mqtt_async`



## Test```python

de_autobahn_entry_exit_closure_resolved_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadEvent],
Awaitable[None]]
...
```python
async def de_autobahn_entry_exit_closure_resolved_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: RoadEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_warning_appeared_mqtt_async`



## Test```python

de_autobahn_warning_appeared_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, WarningEvent],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `autobahn_mqtt_producer_data.WarningEvent`.

Example:

```python
async def de_autobahn_warning_appeared_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: WarningEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_warning_updated_mqtt_async`



## Test```python

de_autobahn_warning_updated_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, WarningEvent],
Awaitable[None]]
...
```python
async def de_autobahn_warning_updated_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: WarningEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_warning_resolved_mqtt_async`



## Test```python

de_autobahn_warning_resolved_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, WarningEvent],
Awaitable[None]]
...
```python
async def de_autobahn_warning_resolved_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: WarningEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_weight_limit35_restriction_appeared_mqtt_async`



## Test```python

de_autobahn_weight_limit35_restriction_appeared_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
...
```python
async def de_autobahn_weight_limit35_restriction_appeared_mqtt_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: RoadEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_weight_limit35_restriction_updated_mqtt_async`



## Test```python

de_autobahn_weight_limit35_restriction_updated_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
RoadEvent], Awaitable[None]]
...
```python
async def de_autobahn_weight_limit35_restriction_updated_mqtt_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: RoadEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_weight_limit35_restriction_resolved_mqtt_async`



## Test```python

de_autobahn_weight_limit35_restriction_resolved_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
RoadEvent], Awaitable[None]]
...
```python
async def de_autobahn_weight_limit35_restriction_resolved_mqtt_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: RoadEvent) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_webcam_appeared_mqtt_async`



## Test```python

de_autobahn_webcam_appeared_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, Webcam], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `autobahn_mqtt_producer_data.Webcam`.

Example:

```python
async def de_autobahn_webcam_appeared_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Webcam) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_webcam_updated_mqtt_async`



## Test```python

de_autobahn_webcam_updated_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, Webcam], Awaitable[None]]
...
```python
async def de_autobahn_webcam_updated_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Webcam) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_webcam_resolved_mqtt_async`



## Test```python

de_autobahn_webcam_resolved_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, Webcam], Awaitable[None]]
...
```python
async def de_autobahn_webcam_resolved_mqtt_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Webcam) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_parking_lorry_appeared_mqtt_async`



## Test```python

de_autobahn_parking_lorry_appeared_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, ParkingLorry],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `autobahn_mqtt_producer_data.ParkingLorry`.

Example:

```python
async def de_autobahn_parking_lorry_appeared_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: ParkingLorry) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_parking_lorry_updated_mqtt_async`



## Test```python

de_autobahn_parking_lorry_updated_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, ParkingLorry],
Awaitable[None]]
...
```python
async def de_autobahn_parking_lorry_updated_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: ParkingLorry) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_parking_lorry_resolved_mqtt_async`



## Test```python

de_autobahn_parking_lorry_resolved_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, ParkingLorry],
Awaitable[None]]
...
```python
async def de_autobahn_parking_lorry_resolved_mqtt_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: ParkingLorry) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_electric_charging_station_appeared_mqtt_async`



## Test```python

de_autobahn_electric_charging_station_appeared_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
ChargingStation], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `autobahn_mqtt_producer_data.ChargingStation`.

Example:

```python
async def de_autobahn_electric_charging_station_appeared_mqtt_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: ChargingStation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_electric_charging_station_updated_mqtt_async`



## Test```python

de_autobahn_electric_charging_station_updated_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
ChargingStation], Awaitable[None]]
...
```python
async def de_autobahn_electric_charging_station_updated_mqtt_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: ChargingStation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_electric_charging_station_resolved_mqtt_async`



## Test```python

de_autobahn_electric_charging_station_resolved_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
ChargingStation], Awaitable[None]]
...
```python
async def de_autobahn_electric_charging_station_resolved_mqtt_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: ChargingStation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_strong_electric_charging_station_appeared_mqtt_async`



## Test```python

de_autobahn_strong_electric_charging_station_appeared_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
ChargingStation], Awaitable[None]]
...
```python
async def de_autobahn_strong_electric_charging_station_appeared_mqtt_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: ChargingStation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_strong_electric_charging_station_updated_mqtt_async`



## Test```python

de_autobahn_strong_electric_charging_station_updated_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
ChargingStation], Awaitable[None]]
...
```python
async def de_autobahn_strong_electric_charging_station_updated_mqtt_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: ChargingStation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `de_autobahn_strong_electric_charging_station_resolved_mqtt_async`



## Test```python

de_autobahn_strong_electric_charging_station_resolved_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
ChargingStation], Awaitable[None]]
...
```python
async def de_autobahn_strong_electric_charging_station_resolved_mqtt_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: ChargingStation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
