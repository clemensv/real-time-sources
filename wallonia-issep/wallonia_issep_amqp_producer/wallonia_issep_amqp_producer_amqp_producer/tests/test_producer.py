...
"""
Tests for wallonia_issep_amqp_producer_amqp_producer
"""
import base64
import json
import os
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../wallonia_issep_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../wallonia_issep_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../wallonia_issep_amqp_producer_amqp_producer/src')))

from wallonia_issep_amqp_producer_amqp_producer import *
from wallonia_issep_amqp_producer_data import SensorConfiguration
from test_wallonia_issep_amqp_producer_data_sensorconfiguration import Test_SensorConfiguration
from wallonia_issep_amqp_producer_data import Observation
from test_wallonia_issep_amqp_producer_data_observation import Test_Observation
...
class TestBeIssepAirqualitySensorsAmqpProducer:
    """Test cases for BeIssepAirqualitySensorsAmqpProducer"""
    ...
    def test_send_sensor_configuration(self, artemis_container):
        """Send and receive a SensorConfiguration message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_SensorConfiguration.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "be.issep.airquality.Sensors.amqp.SensorConfiguration"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
                    assert received.body is not None
                assert received.subject == "{configuration_id}".format(configuration_id="value")
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "be.issep.airquality.Sensors.amqp.Observation"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
