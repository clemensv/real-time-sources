...
def get_default_topic_mappings_be_issep_airquality_sensors_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the be.issep.airquality.Sensors.mqtt message group.
...
def get_subscription_topics_be_issep_airquality_sensors_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_be_issep_airquality_sensors_mqtt()
    topics = []
...
class BeIssepAirqualitySensorsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the be.issep.airquality.Sensors.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_be_issep_airquality_sensors_mqtt()
        ...
        self.be_issep_airquality_sensors_mqtt_sensor_configuration_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, wallonia_issep_mqtt_producer_data.SensorConfiguration, Dict[str, str]], Awaitable[None]]] = None
        
        self.be_issep_airquality_sensors_mqtt_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, wallonia_issep_mqtt_producer_data.Observation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "be.issep.airquality.SensorConfiguration":
            if self.be_issep_airquality_sensors_mqtt_sensor_configuration_async:
                try:
                    ...
                    data = wallonia_issep_mqtt_producer_data.SensorConfiguration.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "be.issep.airquality.Sensors.mqtt.SensorConfiguration")
                    await self.be_issep_airquality_sensors_mqtt_sensor_configuration_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in be_issep_airquality_sensors_mqtt_sensor_configuration handler: {e}")
            return
        
        if event_type == "be.issep.airquality.Observation":
            if self.be_issep_airquality_sensors_mqtt_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = wallonia_issep_mqtt_producer_data.Observation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "be.issep.airquality.Sensors.mqtt.Observation")
                    await self.be_issep_airquality_sensors_mqtt_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in be_issep_airquality_sensors_mqtt_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_be_issep_airquality_sensors_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_be_issep_airquality_sensors_mqtt_sensor_configuration(self,
        ...
        """
        Publish the 'be.issep.airquality.Sensors.mqtt.SensorConfiguration' event to an MQTT topic.
        ...
        attributes = {
             ...
             "subject":"{configuration_id}".format(configuration_id = configuration_id)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_be_issep_airquality_sensors_mqtt_observation(self,
        ...
        content_type: str = "application/json") -> None:
        """
        Publish the 'be.issep.airquality.Sensors.mqtt.Observation' event to an MQTT topic.
