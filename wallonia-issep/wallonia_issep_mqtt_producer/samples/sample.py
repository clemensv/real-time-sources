...
import os
import signal
from wallonia_issep_mqtt_producer_data import *
from wallonia_issep_mqtt_producer_mqtt_client.client import BeIssepAirqualitySensorsMqttProducer, BeIssepAirqualitySensorsMqttDispatcher

async def handle_be_issep_airquality_sensors_mqtt_sensor_configuration(mqtt_msg,cloud_event, be_issep_airquality_sensors_mqtt_sensor_configuration_data):
    """ Handles the be.issep.airquality.Sensors.mqtt.SensorConfiguration message """
    print(f"Received be.issep.airquality.Sensors.mqtt.SensorConfiguration on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {be_issep_airquality_sensors_mqtt_sensor_configuration_data}")

async def handle_be_issep_airquality_sensors_mqtt_observation(mqtt_msg,cloud_event, be_issep_airquality_sensors_mqtt_observation_data):
    """ Handles the be.issep.airquality.Sensors.mqtt.Observation message """
    print(f"Received be.issep.airquality.Sensors.mqtt.Observation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {be_issep_airquality_sensors_mqtt_observation_data}")
