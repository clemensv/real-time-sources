# Wallonia_issep_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the wallonia_issep_mqtt_producer message definitions.

# Wallonia_issep_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - BeIssepAirqualitySensorsMqttEventDispatcher
...
from wallonia_issep_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = BeIssepAirqualitySensorsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### BeIssepAirqualitySensorsMqttEventDispatcher



# Publish message`BeIssepAirqualitySensorsMqttEventDispatcher` handles events for the be.issep.airquality.Sensors.mqtt
message group.
...
from wallonia_issep_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = BeIssepAirqualitySensorsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of
...
## BuildThe BeIssepAirqualitySensorsMqttEventDispatcher defines the following event handler hooks.
...
```##### `be_issep_airquality_sensors_mqtt_sensor_configuration_async`



## Test```python

be_issep_airquality_sensors_mqtt_sensor_configuration_async:  Callable[[PartitionContext, EventData, CloudEvent,
SensorConfiguration], Awaitable[None]]
...
- `data`: The event data of type `wallonia_issep_mqtt_producer_data.SensorConfiguration`.

Example:

```python
async def be_issep_airquality_sensors_mqtt_sensor_configuration_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: SensorConfiguration) -> None:
...
```##### `be_issep_airquality_sensors_mqtt_observation_async`



## Test```python

be_issep_airquality_sensors_mqtt_observation_async:  Callable[[PartitionContext, EventData, CloudEvent, Observation],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `wallonia_issep_mqtt_producer_data.Observation`.

Example:

```python
async def be_issep_airquality_sensors_mqtt_observation_event(partition_context: PartitionContext, event: EventData,
