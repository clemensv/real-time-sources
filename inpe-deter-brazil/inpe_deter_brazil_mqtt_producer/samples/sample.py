...
import os
import signal
from inpe_deter_brazil_mqtt_producer_data import *
from inpe_deter_brazil_mqtt_producer_mqtt_client.client import BRINPEDETERMqttProducer, BRINPEDETERMqttDispatcher

async def handle_br_inpe_deter_mqtt_deforestation_alert(mqtt_msg,cloud_event, br_inpe_deter_mqtt_deforestation_alert_data):
    """ Handles the BR.INPE.DETER.mqtt.DeforestationAlert message """
    print(f"Received BR.INPE.DETER.mqtt.DeforestationAlert on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {br_inpe_deter_mqtt_deforestation_alert_data}")
