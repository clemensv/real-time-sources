...
import inpe_deter_brazil_mqtt_producer_data
from inpe_deter_brazil_mqtt_producer_data import DeforestationAlert
...
def get_default_topic_mappings_br_inpe_deter_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the BR.INPE.DETER.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "BR.INPE.DETER.mqtt.DeforestationAlert": "deforestation/br/inpe/inpe-deter-brazil/{biome}/{state_slug}/{class_slug}/{alert_id}/alert",
    }


def get_subscription_topics_br_inpe_deter_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_br_inpe_deter_mqtt()
    topics = []
...
class BRINPEDETERMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the BR.INPE.DETER.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_br_inpe_deter_mqtt()
        ...
        self.br_inpe_deter_mqtt_deforestation_alert_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, inpe_deter_brazil_mqtt_producer_data.DeforestationAlert, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "BR.INPE.DETER.DeforestationAlert":
            if self.br_inpe_deter_mqtt_deforestation_alert_async:
                try:
                    ...
                    data = inpe_deter_brazil_mqtt_producer_data.DeforestationAlert.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "BR.INPE.DETER.mqtt.DeforestationAlert")
                    await self.br_inpe_deter_mqtt_deforestation_alert_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in br_inpe_deter_mqtt_deforestation_alert handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_br_inpe_deter_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_br_inpe_deter_mqtt_deforestation_alert(self,
        source_uri: str,
        biome: str,
        alert_id: str,
        view_date: str,
        state_slug: str,
        class_slug: str,
        data: inpe_deter_brazil_mqtt_producer_data.DeforestationAlert,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'BR.INPE.DETER.mqtt.DeforestationAlert' event to an MQTT topic.

        Args:
        
            source_uri: URI template variable for 'source_uri'
            biome: URI template variable for 'biome'
            alert_id: URI template variable for 'alert_id'
            view_date: URI template variable for 'view_date'
            state_slug: URI template variable for 'state_slug'
            class_slug: URI template variable for 'class_slug'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'deforestation/br/inpe/inpe-deter-brazil/{biome}/{state_slug}/{class_slug}/{alert_id}/alert'
        ...
        """
        target_topic = topic if topic is not None else "deforestation/br/inpe/inpe-deter-brazil/{biome}/{state_slug}/{class_slug}/{alert_id}/alert"
        ...
        attributes = {
             ...
             "subject":"{biome}/{alert_id}".format(biome = biome,alert_id = alert_id),
             "time":"{view_date}T00:00:00Z".format(view_date = view_date)
        }
        attributes["datacontenttype"] = content_type
        ...
        _ensure_message_expiry(publish_kwargs, 604800)
