...
class DeBfsOdlAmqpProducer:
    """
    Producer class to send messages in the `de.bfs.odl.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_dose_rate_measurement(self,
    ...
    def send_dose_rate_measurement_batch(self,
        ...
        """
        for data in data_array:
            self.send_dose_rate_measurement(
                data=data,
                _feedurl=_feedurl,
                _station_id=_station_id,
                content_type=content_type)
