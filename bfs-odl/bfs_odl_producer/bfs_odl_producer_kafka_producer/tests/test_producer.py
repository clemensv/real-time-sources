...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../bfs_odl_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../bfs_odl_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../bfs_odl_producer_kafka_producer/src')))
...
def test_de_bfs_odl_debfsodlstation(kafka_emulator):
    """Test the DeBfsOdlStation event from the De.Bfs.Odl message group"""
    ...
    consumer = Consumer({
        ...
        'group.id': 'test_de_bfs_odl_debfsodlstation',  # Unique group per test
    ...
    def on_event():
        ...
        while True:
            ...
            if cloudevent['type'] == "de.bfs.odl.Station":
                return msg.key().decode('utf-8') if msg.key() else None

    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = DeBfsOdlEventProducer(kafka_producer, topic, 'binary')
...
def test_de_bfs_odl_debfsodldoseratemeasurement(kafka_emulator):
    """Test the DeBfsOdlDoseRateMeasurement event from the De.Bfs.Odl message group"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_de_bfs_odl_debfsodldoseratemeasurement',  # Unique group per test
        'auto.offset.reset': 'earliest'
    })
    consumer.subscribe([topic])
    ...
    def on_event():
        ...
        while True:
            ...
            if msg.error():
                continue
            cloudevent = parse_cloudevent(msg)
            if cloudevent['type'] == "de.bfs.odl.DoseRateMeasurement":
                return msg.key().decode('utf-8') if msg.key() else None

    kafka_producer = Producer({'bootstrap.servers': bootstrap_servers})
    producer_instance = DeBfsOdlEventProducer(kafka_producer, topic, 'binary')
    # Create valid test data using the test helper
    event_data = Test_DoseRateMeasurement.create_instance()
...
def test_de_bfs_odl_cross_event_type_kafka_key(kafka_emulator):
    """Test that different event types in De.Bfs.Odl produce the same Kafka key for the same placeholder values"""
    ...
    consumer = Consumer({
        'bootstrap.servers': bootstrap_servers,
        'group.id': 'test_de_bfs_odl_cross_key',
    ...
    data2 = Test_DoseRateMeasurement.create_instance()

    producer_instance.send_de_bfs_odl_station(_feedurl = shared_key_value, _station_id = shared_key_value, data = data1)
    producer_instance.send_de_bfs_odl_dose_rate_measurement(_feedurl = shared_key_value, _station_id = shared_key_value, data = data2)
    kafka_producer.flush(timeout=5.0)
    ...
    while len(collected_keys) < 2 and time.time() < timeout:
        ...
        if msg is None or msg.error():
            continue
        cloudevent = parse_cloudevent(msg)
        if cloudevent['type'] in ["de.bfs.odl.Station", "de.bfs.odl.DoseRateMeasurement"]:
