# Cdec_reservoirs_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the cdec_reservoirs_mqtt_producer message definitions.

# Cdec_reservoirs_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - GovCaWaterCdecMqttEventDispatcher
...
from cdec_reservoirs_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = GovCaWaterCdecMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### GovCaWaterCdecMqttEventDispatcher



# Publish message`GovCaWaterCdecMqttEventDispatcher` handles events for the gov.ca.water.cdec.mqtt message group.
...
from cdec_reservoirs_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = GovCaWaterCdecMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe GovCaWaterCdecMqttEventDispatcher defines the following event handler hooks.
...
```##### `gov_ca_water_cdec_mqtt_reservoir_reading_async`



## Test```python

gov_ca_water_cdec_mqtt_reservoir_reading_async:  Callable[[PartitionContext, EventData, CloudEvent, ReservoirReading],
Awaitable[None]]
...
```Asynchronous handler hook for `gov.ca.water.cdec.mqtt.ReservoirReading`: A current reservoir record from the
California Data Exchange Center (CDEC). It reports the latest storage, elevation, capacity, or related reservoir status
available for one reservoir.
...
- `data`: The event data of type `cdec_reservoirs_mqtt_producer_data.ReservoirReading`.

Example:

```python
async def gov_ca_water_cdec_mqtt_reservoir_reading_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: ReservoirReading) -> None:
