...
import os
import signal
from cdec_reservoirs_mqtt_producer_data import *
from cdec_reservoirs_mqtt_producer_mqtt_client.client import GovCaWaterCdecMqttProducer, GovCaWaterCdecMqttDispatcher

async def handle_gov_ca_water_cdec_mqtt_reservoir_reading(mqtt_msg,cloud_event, gov_ca_water_cdec_mqtt_reservoir_reading_data):
    """ Handles the gov.ca.water.cdec.mqtt.ReservoirReading message """
    print(f"Received gov.ca.water.cdec.mqtt.ReservoirReading on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {gov_ca_water_cdec_mqtt_reservoir_reading_data}")
