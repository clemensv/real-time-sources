...
# Cdec_reservoirs_producer Kafka Producer# Cdec_reservoirs_producer Event Dispatcher for Apache Kafka
...
- **Scales horizontally** across multiple brokers and partitions### GovCaWaterCdecEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`GovCaWaterCdecEventDispatcher` handles events for the gov.ca.water.cdec message group.
...
The GovCaWaterCdecEventDispatcher defines the following event handler hooks.
...
producer = GovCaWaterCdecProducer(
    ...
    security_protocol='SASL_SSL',##### `gov_ca_water_cdec_reservoir_reading_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gov_ca_water_cdec_reservoir_reading_async:  Callable[[ConsumerRecord, CloudEvent,
...
gov_ca_water_cdec_dispatcher.gov_ca_water_cdec_reservoir_reading_async = gov_ca_water_cdec_reservoir_reading_event
...
##### `send_gov_ca_water_cdec_reservoir_reading`Dispatchers have the following protected methods:



```python### Methods:

async def send_gov_ca_water_cdec_reservoir_reading(
...
await producer.send_gov_ca_water_cdec_reservoir_reading(
...
Send multiple `gov.ca.water.cdec.ReservoirReading` messages in a batch.
