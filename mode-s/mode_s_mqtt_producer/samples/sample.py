...
import os
import signal
from mode_s_mqtt_producer_data import *
from mode_s_mqtt_producer_mqtt_client.client import ModeSMqttProducer, ModeSMqttDispatcher

async def handle_mode_s_adsb_mqtt(mqtt_msg,cloud_event, mode_s_adsb_mqtt_data):
    """ Handles the Mode_S.ADSB.mqtt message """
    print(f"Received Mode_S.ADSB.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {mode_s_adsb_mqtt_data}")

async def handle_mode_s_altitude_reply_mqtt(mqtt_msg,cloud_event, mode_s_altitude_reply_mqtt_data):
    """ Handles the Mode_S.AltitudeReply.mqtt message """
    print(f"Received Mode_S.AltitudeReply.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {mode_s_altitude_reply_mqtt_data}")

async def handle_mode_s_identity_reply_mqtt(mqtt_msg,cloud_event, mode_s_identity_reply_mqtt_data):
    """ Handles the Mode_S.IdentityReply.mqtt message """
    print(f"Received Mode_S.IdentityReply.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {mode_s_identity_reply_mqtt_data}")

async def handle_mode_s_acquisition_reply_mqtt(mqtt_msg,cloud_event, mode_s_acquisition_reply_mqtt_data):
    """ Handles the Mode_S.AcquisitionReply.mqtt message """
    print(f"Received Mode_S.AcquisitionReply.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {mode_s_acquisition_reply_mqtt_data}")

async def handle_mode_s_comm_baltitude_mqtt(mqtt_msg,cloud_event, mode_s_comm_baltitude_mqtt_data):
    """ Handles the Mode_S.CommBAltitude.mqtt message """
    print(f"Received Mode_S.CommBAltitude.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {mode_s_comm_baltitude_mqtt_data}")

async def handle_mode_s_comm_bidentity_mqtt(mqtt_msg,cloud_event, mode_s_comm_bidentity_mqtt_data):
    """ Handles the Mode_S.CommBIdentity.mqtt message """
    print(f"Received Mode_S.CommBIdentity.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {mode_s_comm_bidentity_mqtt_data}")
