...
import kystverket_ais_mqtt_producer_data
from kystverket_ais_mqtt_producer_data import PositionReport
from kystverket_ais_mqtt_producer_data import ShipStatic
from kystverket_ais_mqtt_producer_data import AidToNavigation
...
def get_default_topic_mappings_no_kystverket_ais_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the NO.Kystverket.AIS.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "NO.Kystverket.AIS.mqtt.PositionReport": "maritime/no/kystverket/kystverket-ais/{flag}/{ship_type}/{geohash5}/{mmsi}/position-report",
        "NO.Kystverket.AIS.mqtt.ShipStatic": "maritime/no/kystverket/kystverket-ais/{flag}/{ship_type}/{geohash5}/{mmsi}/static",
        "NO.Kystverket.AIS.mqtt.AidToNavigation": "maritime/no/kystverket/kystverket-ais/{flag}/{ship_type}/{geohash5}/{mmsi}/aid-to-navigation",
    }


def get_subscription_topics_no_kystverket_ais_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_no_kystverket_ais_mqtt()
    topics = []
...
class NOKystverketAISMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the NO.Kystverket.AIS.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_no_kystverket_ais_mqtt()
        ...
        self.no_kystverket_ais_mqtt_position_report_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, kystverket_ais_mqtt_producer_data.PositionReport, Dict[str, str]], Awaitable[None]]] = None
        
        self.no_kystverket_ais_mqtt_ship_static_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, kystverket_ais_mqtt_producer_data.ShipStatic, Dict[str, str]], Awaitable[None]]] = None
        
        self.no_kystverket_ais_mqtt_aid_to_navigation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, kystverket_ais_mqtt_producer_data.AidToNavigation, Dict[str, str]], Awaitable[None]]] = None
        ...
        if event_type == "NO.Kystverket.AIS.mqtt.PositionReport":
            if self.no_kystverket_ais_mqtt_position_report_async:
                try:
                    ...
                    data = kystverket_ais_mqtt_producer_data.PositionReport.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "NO.Kystverket.AIS.mqtt.PositionReport")
                    await self.no_kystverket_ais_mqtt_position_report_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
        ...
        if event_type == "NO.Kystverket.AIS.mqtt.ShipStatic":
            if self.no_kystverket_ais_mqtt_ship_static_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = kystverket_ais_mqtt_producer_data.ShipStatic.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "NO.Kystverket.AIS.mqtt.ShipStatic")
                    await self.no_kystverket_ais_mqtt_ship_static_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
        ...
        if event_type == "NO.Kystverket.AIS.mqtt.AidToNavigation":
            if self.no_kystverket_ais_mqtt_aid_to_navigation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = kystverket_ais_mqtt_producer_data.AidToNavigation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "NO.Kystverket.AIS.mqtt.AidToNavigation")
                    await self.no_kystverket_ais_mqtt_aid_to_navigation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in no_kystverket_ais_mqtt_aid_to_navigation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_no_kystverket_ais_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_no_kystverket_ais_mqtt_position_report(self,
        mmsi: str,
        flag: str,
        ship_type: str,
        geohash5: str,
        data: kystverket_ais_mqtt_producer_data.PositionReport,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'NO.Kystverket.AIS.mqtt.PositionReport' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'maritime/no/kystverket/kystverket-ais/{flag}/{ship_type}/{geohash5}/{mmsi}/position-report'
        ...
        """
        target_topic = topic if topic is not None else "maritime/no/kystverket/kystverket-ais/{flag}/{ship_type}/{geohash5}/{mmsi}/position-report"
    ...
    async def publish_no_kystverket_ais_mqtt_ship_static(self,
        mmsi: str,
        flag: str,
        ship_type: str,
        geohash5: str,
        data: kystverket_ais_mqtt_producer_data.ShipStatic,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'NO.Kystverket.AIS.mqtt.ShipStatic' event to an MQTT topic.
        ...
        Args:
            ...
            geohash5: URI template variable for 'geohash5'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'maritime/no/kystverket/kystverket-ais/{flag}/{ship_type}/{geohash5}/{mmsi}/static'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (0).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "maritime/no/kystverket/kystverket-ais/{flag}/{ship_type}/{geohash5}/{mmsi}/static"
        _topic_template_values: Dict[str, str] = {
            "mmsi": str(mmsi),
            "flag": str(flag),
            "ship_type": str(ship_type),
    ...
    async def publish_no_kystverket_ais_mqtt_aid_to_navigation(self,
        mmsi: str,
        flag: str,
        ship_type: str,
        geohash5: str,
        data: kystverket_ais_mqtt_producer_data.AidToNavigation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'NO.Kystverket.AIS.mqtt.AidToNavigation' event to an MQTT topic.
        ...
        Args:
            ...
            geohash5: URI template variable for 'geohash5'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'maritime/no/kystverket/kystverket-ais/{flag}/{ship_type}/{geohash5}/{mmsi}/aid-to-navigation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (0).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "maritime/no/kystverket/kystverket-ais/{flag}/{ship_type}/{geohash5}/{mmsi}/aid-to-navigation"
        _topic_template_values: Dict[str, str] = {
            "mmsi": str(mmsi),
            "flag": str(flag),
            "ship_type": str(ship_type),
