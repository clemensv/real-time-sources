...
DATABASE="${3:-kystverket-ais}"
EVENTSTREAM="${4:-kystverket-ais-ingest}"
...
echo "=== Kystverket AIS Fabric Setup ==="
...
# No SQL operator — CloudEvents route directly to _cloudevents_dispatch;
# KQL update policies split events into the 5 typed tables.
ES_DEF=$(jq -n \
...
echo "    Routing:     KQL update policies -> 5 typed AIS tables"
...
echo "  - PositionReportClassA   (AIS types 1,2,3)"
echo "  - StaticVoyageData       (AIS type 5)"
echo "  - PositionReportClassB   (AIS types 18,19)"
echo "  - StaticDataClassB       (AIS type 24)"
echo "  - AidToNavigation        (AIS type 21)"
echo ""
echo "Materialized views: PositionClassALatest, PositionClassBLatest,"
echo "  StaticVoyageDataLatest, StaticDataClassBLatest, AidToNavigationLatest"
echo ""
echo "Functions: VesselPositions(), AISStatistics()"
echo ""
echo "Next steps:"
echo "  1. Open the Event Stream in the Fabric portal to get the Custom Endpoint connection string"
echo "  2. Deploy: az deployment group create --template-file ../azure-template.json --parameters connectionStringSecret='<cs>'"
echo "  3. Or run locally: CONNECTION_STRING='<cs>' python -m kystverket_ais stream"
echo ""
