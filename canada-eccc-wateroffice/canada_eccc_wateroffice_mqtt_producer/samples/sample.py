...
import os
import signal
from canada_eccc_wateroffice_mqtt_producer_data import *
from canada_eccc_wateroffice_mqtt_producer_mqtt_client.client import CAGovECCCHydroMqttProducer, CAGovECCCHydroMqttDispatcher

async def handle_ca_gov_eccc_hydro_mqtt_station(mqtt_msg,cloud_event, ca_gov_eccc_hydro_mqtt_station_data):
    """ Handles the CA.Gov.ECCC.Hydro.mqtt.Station message """
    print(f"Received CA.Gov.ECCC.Hydro.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {ca_gov_eccc_hydro_mqtt_station_data}")

async def handle_ca_gov_eccc_hydro_mqtt_observation(mqtt_msg,cloud_event, ca_gov_eccc_hydro_mqtt_observation_data):
    """ Handles the CA.Gov.ECCC.Hydro.mqtt.Observation message """
    print(f"Received CA.Gov.ECCC.Hydro.mqtt.Observation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {ca_gov_eccc_hydro_mqtt_observation_data}")
