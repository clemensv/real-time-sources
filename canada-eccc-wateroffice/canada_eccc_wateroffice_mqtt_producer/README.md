# Canada_eccc_wateroffice_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the canada_eccc_wateroffice_mqtt_producer message definitions.

# Canada_eccc_wateroffice_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - CAGovECCCHydroMqttEventDispatcher
...
from canada_eccc_wateroffice_mqtt_producer_mqtt_client import *This module defines an event processing framework for
Azure Event Hubs,
...
client = CAGovECCCHydroMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### CAGovECCCHydroMqttEventDispatcher



# Publish message`CAGovECCCHydroMqttEventDispatcher` handles events for the CA.Gov.ECCC.Hydro.mqtt message group.
...
from canada_eccc_wateroffice_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = CAGovECCCHydroMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe CAGovECCCHydroMqttEventDispatcher defines the following event handler hooks.
...
```##### `ca_gov_eccc_hydro_mqtt_station_async`



## Test```python

ca_gov_eccc_hydro_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station], Awaitable[None]]

```bash```

make test

```Asynchronous handler hook for `CA.Gov.ECCC.Hydro.mqtt.Station`: A reference record for one Canadian hydrometric
station published by Environment and Climate Change Canada Water Survey of Canada. It fires when the bridge publishes or
...
- `data`: The event data of type `canada_eccc_wateroffice_mqtt_producer_data.Station`.

Example:

```python
async def ca_gov_eccc_hydro_mqtt_station_event(partition_context: PartitionContext, event: EventData, cloud_event:
...
```##### `ca_gov_eccc_hydro_mqtt_observation_async`



## Test```python

ca_gov_eccc_hydro_mqtt_observation_async:  Callable[[PartitionContext, EventData, CloudEvent, Observation],
Awaitable[None]]
...
```Asynchronous handler hook for `CA.Gov.ECCC.Hydro.mqtt.Observation`: A current measurement from Environment and
Climate Change Canada Water Survey of Canada for one monitoring site. It carries real-time water level and flow
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `canada_eccc_wateroffice_mqtt_producer_data.Observation`.

Example:

```python
async def ca_gov_eccc_hydro_mqtt_observation_event(partition_context: PartitionContext, event: EventData, cloud_event:
