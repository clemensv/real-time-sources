...
def get_default_topic_mappings_ca_gov_eccc_hydro_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the CA.Gov.ECCC.Hydro.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "CA.Gov.ECCC.Hydro.mqtt.Station": "hydro/ca/eccc/canada-eccc-wateroffice/{basin}/{station_number}/info",
        "CA.Gov.ECCC.Hydro.mqtt.Observation": "hydro/ca/eccc/canada-eccc-wateroffice/{basin}/{station_number}/observation",
    }


def get_subscription_topics_ca_gov_eccc_hydro_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_ca_gov_eccc_hydro_mqtt()
    topics = []
...
class CAGovECCCHydroMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the CA.Gov.ECCC.Hydro.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_ca_gov_eccc_hydro_mqtt()
        ...
        self.ca_gov_eccc_hydro_mqtt_station_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, canada_eccc_wateroffice_mqtt_producer_data.Station, Dict[str, str]], Awaitable[None]]] = None
        
        self.ca_gov_eccc_hydro_mqtt_observation_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, canada_eccc_wateroffice_mqtt_producer_data.Observation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "CA.Gov.ECCC.Hydro.Station":
            if self.ca_gov_eccc_hydro_mqtt_station_async:
                try:
                    ...
                    data = canada_eccc_wateroffice_mqtt_producer_data.Station.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "CA.Gov.ECCC.Hydro.mqtt.Station")
                    await self.ca_gov_eccc_hydro_mqtt_station_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in ca_gov_eccc_hydro_mqtt_station handler: {e}")
            return
        
        if event_type == "CA.Gov.ECCC.Hydro.Observation":
            if self.ca_gov_eccc_hydro_mqtt_observation_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = canada_eccc_wateroffice_mqtt_producer_data.Observation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "CA.Gov.ECCC.Hydro.mqtt.Observation")
                    await self.ca_gov_eccc_hydro_mqtt_observation_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in ca_gov_eccc_hydro_mqtt_observation handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_ca_gov_eccc_hydro_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_ca_gov_eccc_hydro_mqtt_station(self,
        station_number: str,
        basin: str,
        data: canada_eccc_wateroffice_mqtt_producer_data.Station,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'CA.Gov.ECCC.Hydro.mqtt.Station' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'hydro/ca/eccc/canada-eccc-wateroffice/{basin}/{station_number}/info'
        ...
        """
        target_topic = topic if topic is not None else "hydro/ca/eccc/canada-eccc-wateroffice/{basin}/{station_number}/info"
        ...
        attributes = {
             "type":"CA.Gov.ECCC.Hydro.Station",
             "source":"https://api.weather.gc.ca/collections/hydrometric-stations",
             "subject":"stations/{station_number}".format(station_number = station_number)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_ca_gov_eccc_hydro_mqtt_observation(self,
        station_number: str,
        basin: str,
        data: canada_eccc_wateroffice_mqtt_producer_data.Observation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'CA.Gov.ECCC.Hydro.mqtt.Observation' event to an MQTT topic.
        ...
        Args:
            ...
            basin: URI template variable for 'basin'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'hydro/ca/eccc/canada-eccc-wateroffice/{basin}/{station_number}/observation'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "hydro/ca/eccc/canada-eccc-wateroffice/{basin}/{station_number}/observation"
        _topic_template_values: Dict[str, str] = {
            "station_number": str(station_number),
            "basin": str(basin),
        }
        ...
        attributes = {
             "type":"CA.Gov.ECCC.Hydro.Observation",
