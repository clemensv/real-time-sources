...
import os
import signal
from wikimedia_eventstreams_mqtt_producer_data import *
from wikimedia_eventstreams_mqtt_producer_mqtt_client.client import WikimediaEventStreamsMqttProducer, WikimediaEventStreamsMqttDispatcher

async def handle_wikimedia_event_streams_recent_change_mqtt(mqtt_msg,cloud_event, wikimedia_event_streams_recent_change_mqtt_data):
    """ Handles the Wikimedia.EventStreams.RecentChange.mqtt message """
    print(f"Received Wikimedia.EventStreams.RecentChange.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {wikimedia_event_streams_recent_change_mqtt_data}")
