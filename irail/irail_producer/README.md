...
# Irail_producer Kafka Producer# Irail_producer Event Dispatcher for Apache Kafka
...
- **Scales horizontally** across multiple brokers and partitions### BeIrailEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`BeIrailEventDispatcher` handles events for the be.irail message group.
...
The BeIrailEventDispatcher defines the following event handler hooks.
...
producer = BeIrailProducer(
    ...
    security_protocol='SASL_SSL',##### `be_irail_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'be_irail_station_async:  Callable[[ConsumerRecord, CloudEvent, Station],
Awaitable[None]]
...
Asynchronous handler hook for `be.irail.Station`: Metadata for a Belgian railway station in the NMBS/SNCB network as
provided by the iRail API. The Belgian rail network comprises approximately 600 passenger stations. Each station has a
unique UIC-derived numeric identifier assigned by the NMBS, geographic coordinates, and multilingual names.
...
be_irail_dispatcher.be_irail_station_async = be_irail_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `be_irail_station_board_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'be_irail_station_board_async:  Callable[[ConsumerRecord, CloudEvent, StationBoard],
Awaitable[None]]
...
Asynchronous handler hook for `be.irail.StationBoard`: A real-time departure board snapshot for a Belgian railway
...
be_irail_dispatcher.be_irail_station_board_async = be_irail_station_board_event
...
##### `send_be_irail_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_be_irail_station(
...
Send a single `be.irail.Station` message. Metadata for a Belgian railway station in the NMBS/SNCB network as provided by
the iRail API. The Belgian rail network comprises approximately 600 passenger stations. Each station has a unique UIC-
derived numeric identifier assigned by the NMBS, geographic coordinates, and multilingual names.Args:
...
await producer.send_be_irail_station(
...
Send multiple `be.irail.Station` messages in a batch.
...
await producer.send_be_irail_station_batch(```
...
##### `send_be_irail_station_board`Dispatchers have the following protected methods:



```python### Methods:

async def send_be_irail_station_board(
...
Send a single `be.irail.StationBoard` message. A real-time departure board snapshot for a Belgian railway station from
the iRail liveboard API. Contains all currently scheduled departures from the station with real-time delay information,
platform assignments, cancellation status, and crowd-sourced occupancy levels. The board is polled periodically and each
event replaces the previous board state for the same station.Args:
...
await producer.send_be_irail_station_board(
...
Send multiple `be.irail.StationBoard` messages in a batch.
...
await producer.send_be_irail_station_board_batch(```
