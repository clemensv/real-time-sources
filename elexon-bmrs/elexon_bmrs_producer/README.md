...
- **Scales horizontally** across multiple brokers and partitions### UKCoElexonBMRSEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`UKCoElexonBMRSEventDispatcher` handles events for the UK.Co.Elexon.BMRS message group.
...
The UKCoElexonBMRSEventDispatcher defines the following event handler hooks.
...
producer = UKCoElexonBMRSProducer(
    ...
    security_protocol='SASL_SSL',##### `uk_co_elexon_bmrs_generation_mix_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_co_elexon_bmrs_generation_mix_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `UK.Co.Elexon.BMRS.GenerationMix`:
...
uk_co_elexon_bmrs_dispatcher.uk_co_elexon_bmrs_generation_mix_async = uk_co_elexon_bmrs_generation_mix_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `uk_co_elexon_bmrs_demand_outturn_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_co_elexon_bmrs_demand_outturn_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `UK.Co.Elexon.BMRS.DemandOutturn`:
...
##### `send_uk_co_elexon_bmrs_generation_mix`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_co_elexon_bmrs_generation_mix(
...
Send a single `UK.Co.Elexon.BMRS.GenerationMix` message.Args:
...
await producer.send_uk_co_elexon_bmrs_generation_mix(
...
Send multiple `UK.Co.Elexon.BMRS.GenerationMix` messages in a batch.
...
await producer.send_uk_co_elexon_bmrs_generation_mix_batch(```
...
##### `send_uk_co_elexon_bmrs_demand_outturn`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_co_elexon_bmrs_demand_outturn(
...
Send a single `UK.Co.Elexon.BMRS.DemandOutturn` message.Args:
...
await producer.send_uk_co_elexon_bmrs_demand_outturn(
...
Send multiple `UK.Co.Elexon.BMRS.DemandOutturn` messages in a batch.
...
await producer.send_uk_co_elexon_bmrs_demand_outturn_batch(```
