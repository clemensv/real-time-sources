...
6. Normalize upstream payloads into the correct generated data classes per family and pass subject or key placeholders explicitly, matching the xreg-declared shape and nullability rules, while consuming the generated producer packages unchanged.
    ...
    - **Parse numeric values with `float()` or `int()`.** If the upstream JSON value is a string representation of a number, convert it before passing to a numeric-typed field.
    - **Handle nullable fields correctly.** If the upstream API can return `null` for a field, the xreg schema MUST declare it nullable (`"type": ["float", "null"]` or `"type": [{"$ref": "..."}, "null"]`). Do not use `# type: ignore` to suppress — fix the schema. If a field has an `enum` constraint and is nullable, it must also be removed from the schema's `required` array (xrcg won't generate `Optional[EnumType]` for required enum fields).
    - **Guard `from_ordinal` calls against None.** Use `EnumType.from_ordinal(val) if val is not None else None` (or a default) — `from_ordinal` does not accept `None`.
