...
2. Identify the real entity or time-series identity for each event family.
3. Decide whether the families can share one endpoint, key model, or schema, and require proof before introducing a shared catch-all shape.
4. Update the checked-in manifest under `xreg/` and model `type`, `source`, and `subject` deliberately.
5. Apply the repo keying rules and validation points in [contract checklist](references/contract-checklist.md).
6. Author each JsonStructure schema with exhaustive schema-level and field-level descriptions, and apply units, alternate names, and validation annotations wherever the upstream contract supports them.
7. Model nullability and alternatives with type unions or `choice`, not `anyOf` or other conditional composition features.
8. Validate any extended JsonStructure features and nullability assumptions before regeneration, because the shared Docker Kafka E2E suite will enforce them against emitted events.
9. Regenerate producer output with the source-local `generate_producer.ps1` script.
10. Keep runtime wiring aligned with the regenerated producer without modifying generated files; if the output cannot be consumed directly, fix the package layout or runtime integration around it instead.
11. Refresh `EVENTS.md` from the manifest and call out any docs-versus-live discrepancies that affected the model.
