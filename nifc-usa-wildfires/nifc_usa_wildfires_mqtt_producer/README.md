# Nifc_usa_wildfires_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the nifc_usa_wildfires_mqtt_producer message definitions.

# Nifc_usa_wildfires_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - GovNIFCWildfiresMqttEventDispatcher
...
from nifc_usa_wildfires_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = GovNIFCWildfiresMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### GovNIFCWildfiresMqttEventDispatcher



# Publish message`GovNIFCWildfiresMqttEventDispatcher` handles events for the Gov.NIFC.Wildfires.mqtt message group.
...
from nifc_usa_wildfires_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = GovNIFCWildfiresMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe GovNIFCWildfiresMqttEventDispatcher defines the following event handler hooks.
...
```##### `gov_nifc_wildfires_mqtt_wildfire_incident_async`



## Test```python

gov_nifc_wildfires_mqtt_wildfire_incident_async:  Callable[[PartitionContext, EventData, CloudEvent, WildfireIncident],
Awaitable[None]]
...
```Asynchronous handler hook for `Gov.NIFC.Wildfires.mqtt.WildfireIncident`: Active wildfire incident data from the
National Interagency Fire Center (NIFC) IRWIN system, published via ArcGIS Feature Service.
...
- `data`: The event data of type `nifc_usa_wildfires_mqtt_producer_data.WildfireIncident`.

Example:

```python
async def gov_nifc_wildfires_mqtt_wildfire_incident_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: WildfireIncident) -> None:
...
```python
gov_nifc_wildfires_mqtt_dispatcher.gov_nifc_wildfires_mqtt_wildfire_incident_async =
gov_nifc_wildfires_mqtt_wildfire_incident_event
```
