...
3. [Quick Start](#quick-start)    - GovNIFCWildfiresEventDispatcher
...
- GovNIFCWildfiresProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### GovNIFCWildfiresEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`GovNIFCWildfiresEventDispatcher` handles events for the Gov.NIFC.Wildfires message group.
...
producer = GovNIFCWildfiresProducer(
...
await producer.send_gov_nifc_wildfires_wildfire_incident(```python

    data=WildfireIncident(...),add_consumer(self, consumer: KafkaConsumer)
...
The GovNIFCWildfiresEventDispatcher defines the following event handler hooks.
...
producer = GovNIFCWildfiresProducer(
    ...
    security_protocol='SASL_SSL',##### `gov_nifc_wildfires_wildfire_incident_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'gov_nifc_wildfires_wildfire_incident_async:  Callable[[ConsumerRecord, CloudEvent,
WildfireIncident], Awaitable[None]]

)```

```

Asynchronous handler hook for `Gov.NIFC.Wildfires.WildfireIncident`: Active wildfire incident data from the National
Interagency Fire Center (NIFC) IRWIN system, published via ArcGIS Feature Service.
...
### GovNIFCWildfiresProducer- `data`: The event data of type `nifc_usa_wildfires_producer_data.WildfireIncident`.



Producer for `Gov.NIFC.Wildfires` message group.Example:



#### Constructor```python

async def gov_nifc_wildfires_wildfire_incident_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
WildfireIncident) -> None:

```python    # Process the event data

GovNIFCWildfiresProducer(    await some_processing_function(record, cloud_event, data)
...
gov_nifc_wildfires_dispatcher.gov_nifc_wildfires_wildfire_incident_async = gov_nifc_wildfires_wildfire_incident_event
...
##### `send_gov_nifc_wildfires_wildfire_incident`Dispatchers have the following protected methods:



```python### Methods:

async def send_gov_nifc_wildfires_wildfire_incident(

    self,##### `_process_event`

    data: WildfireIncident,
...
Send a single `Gov.NIFC.Wildfires.WildfireIncident` message. Active wildfire incident data from the National Interagency
Fire Center (NIFC) IRWIN system, published via ArcGIS Feature Service.Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `WildfireIncident`
...
await producer.send_gov_nifc_wildfires_wildfire_incident(

    data=WildfireIncident(...),Args:
...
Send multiple `Gov.NIFC.Wildfires.WildfireIncident` messages in a batch.
...
await producer.send_gov_nifc_wildfires_wildfire_incident_batch(```

    messages=[

        WildfireIncident(...),Initializes the runner with a Kafka consumer.

        WildfireIncident(...),

        WildfireIncident(...)Args:
