...
# Hongkong_epd_producer Kafka Producer# Hongkong_epd_producer Event Dispatcher for Apache Kafka
...
- **Scales horizontally** across multiple brokers and partitions### HKGovEPDAQHIEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`HKGovEPDAQHIEventDispatcher` handles events for the HK.Gov.EPD.AQHI message group.
...
The HKGovEPDAQHIEventDispatcher defines the following event handler hooks.
...
producer = HKGovEPDAQHIProducer(
    ...
    security_protocol='SASL_SSL',##### `hk_gov_epd_aqhi_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'hk_gov_epd_aqhi_station_async:  Callable[[ConsumerRecord, CloudEvent, Station],
Awaitable[None]]
...
hk_gov_epd_aqhi_dispatcher.hk_gov_epd_aqhi_station_async = hk_gov_epd_aqhi_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `hk_gov_epd_aqhi_aqhireading_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'hk_gov_epd_aqhi_aqhireading_async:  Callable[[ConsumerRecord, CloudEvent, AQHIReading],
Awaitable[None]]
...
hk_gov_epd_aqhi_dispatcher.hk_gov_epd_aqhi_aqhireading_async = hk_gov_epd_aqhi_aqhireading_event
...
##### `send_hk_gov_epd_aqhi_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_hk_gov_epd_aqhi_station(
...
await producer.send_hk_gov_epd_aqhi_station(
...
Send multiple `HK.Gov.EPD.AQHI.Station` messages in a batch.
...
await producer.send_hk_gov_epd_aqhi_station_batch(```
...
##### `send_hk_gov_epd_aqhi_aqhireading`Dispatchers have the following protected methods:



```python### Methods:

async def send_hk_gov_epd_aqhi_aqhireading(
...
await producer.send_hk_gov_epd_aqhi_aqhireading(
...
Send multiple `HK.Gov.EPD.AQHI.AQHIReading` messages in a batch.
