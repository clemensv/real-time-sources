    ...
    def _fail_all(self, exc):
        ...
        for fut in list(self._pending.values()):
            if not fut.done():
                fut.set_exception(exc)
        self._pending.clear()
from rssbridge_amqp_producer_data import FeedItem

class MicrosoftOpenDataRssFeedsAmqpProducer:
    """
    Producer class to send messages in the `Microsoft.OpenData.RssFeeds.amqp` message group via AMQP 1.0 protocol.
    """
    ...
    def send_feed_item(self,
        data: FeedItem,
        _sourceurl: str,
        _item_id: str,
        content_type: str = 'application/json') -> None:
        """
        Send the `Microsoft.OpenData.RssFeeds.amqp.FeedItem` message
        ...
        attributes = {
            ...
            "source":
            "{sourceurl}".format(sourceurl=_sourceurl),
            "subject":
            "{item_id}".format(item_id=_item_id),
        }
        ...
        amqp_msg.subject = "{item_id}".format(item_id=_item_id)
    ...
    def send_feed_item_batch(self,
        data_array: typing.List[FeedItem],
        _sourceurl: str,
        _item_id: str,
        content_type: str = 'application/json') -> None:
        """
        Send multiple `Microsoft.OpenData.RssFeeds.amqp.FeedItem` messages
        
        Args:
            data_array (typing.List[FeedItem]): Array of message data objects
            _sourceurl (str): Value for placeholder sourceurl in attribute source
            _item_id (str): Value for placeholder item_id in attribute subject
