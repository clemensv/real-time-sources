...
import nina_bbk_mqtt_producer_data
from nina_bbk_mqtt_producer_data import CivilWarning
...
def get_default_topic_mappings_nina_warnings_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the NINA.Warnings.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "NINA.Warnings.mqtt.CivilWarning": "alerts/de/nina/nina-bbk/{state}/{severity}/{warning_id}/warning",
    }


def get_subscription_topics_nina_warnings_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_nina_warnings_mqtt()
    topics = []
...
class NINAWarningsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the NINA.Warnings.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_nina_warnings_mqtt()
        ...
        self.nina_warnings_mqtt_civil_warning_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, nina_bbk_mqtt_producer_data.CivilWarning, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "NINA.CivilWarning":
            if self.nina_warnings_mqtt_civil_warning_async:
                try:
                    ...
                    data = nina_bbk_mqtt_producer_data.CivilWarning.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "NINA.Warnings.mqtt.CivilWarning")
                    await self.nina_warnings_mqtt_civil_warning_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in nina_warnings_mqtt_civil_warning handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_nina_warnings_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_nina_warnings_mqtt_civil_warning(self,
        warning_id: str,
        state: str,
        severity: str,
        data: nina_bbk_mqtt_producer_data.CivilWarning,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'NINA.Warnings.mqtt.CivilWarning' event to an MQTT topic.
        ...
        Args:
            ...
            topic: Optional topic override. If not provided, uses default topic 'alerts/de/nina/nina-bbk/{state}/{severity}/{warning_id}/warning'
        ...
        """
        target_topic = topic if topic is not None else "alerts/de/nina/nina-bbk/{state}/{severity}/{warning_id}/warning"
        ...
        attributes = {
             ...
             "subject":"{warning_id}".format(warning_id = warning_id)
        }
        attributes["datacontenttype"] = content_type
