...
class TestTflCyclesAmqpArtemisFlow:
    """End-to-end: tfl-cycles pushes StationInformation + StationStatus to Artemis.

    Polls the live TfL Unified API (GET /BikePoint, anonymous) once; the feeder
    ...
    def test_emits_cloudevents_to_amqp_queue(self, artemis_broker, tfl_cycles_amqp_image):
        client = docker.from_env()
        feeder = client.containers.run(
            tfl_cycles_amqp_image.id,
            detach=True,
            remove=False,
            network=artemis_broker["network"],
            environment={
