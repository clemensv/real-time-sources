        ...
        try:
            ...
            feeder = docker_client.containers.run(
                ...
                remove=False,
                    network_mode='host',
                environment={
                    'MQTT_BROKER_URL': f'mqtt://{broker_ip}:{host_port}',
...
def _generic_mosquitto(network_name: str, container_name: str):
    ...
    while time.time() < deadline:
        ...
        pytest.skip('Mosquitto broker did not start')
    time.sleep(1.0)  # Brief pause to ensure broker accepts connections
    # Feeders run with --network=host and connect via 127.0.0.1:host_port.
    # This avoids inter-container TCP routing issues that cause ~25% of MQTT
    # tests to fail on CI (Docker bridge SYN packets silently dropped).
    return container, None, host_port, '127.0.0.1'
...
class TestUkBodsSiriMqttDockerFlow:
    def test_emits_expected_mqtt_topics(self):
        client = docker.from_env()
        broker, network, host_port, broker_ip = _generic_mosquitto('uk-bods-siri-mqtt-e2e', 'uk-bods-siri-mqtt-e2e-broker')
...
def erddap_mqtt_broker():
    container, network, host_port, broker_ip = _generic_mosquitto('erddap-mqtt-e2e', 'erddap-mqtt-e2e-broker')
    try:
        yield {
            'host_port': host_port,
            'internal_host': gateway_ip,
...
def nws_alerts_mosquitto_container():
    container, network, host_port, broker_ip = _generic_mosquitto('nws-alerts-mqtt-e2e', 'nws-alerts-mqtt-e2e-broker')
    try:
        yield {
            'host_port': host_port,
            'internal_host': gateway_ip,
...
def _run_mqtt_contract_flow(project_dir: str, image, broker: Mapping[str, Any], *, extra_env: Mapping[str, str] | None = None, timeout: int = 420, min_messages: int = 1, allow_empty: bool = False) -> List[Dict[str, Any]]:
    ...
    # Retry feeder start to handle transient connect failures
    logs = ''
    result = {'StatusCode': 1}
    for attempt in range(3):
        feeder = client.containers.run(image.id, detach=True, remove=False, network_mode='host', environment=env)
        try:
...
def mosquitto_inpe_deter_brazil():
    ...
    try:
        yield {'host_port': host_port, 'internal_host': gateway_ip, 'internal_port': host_port}
    finally:
        try: container.kill()
        except docker.errors.APIError: pass
        pass
...
class TestEurdepRadiationMqttDockerFlow:
    def test_emits_retained_uns_topics(self, mosquitto_eurdep_radiation, eurdep_radiation_mqtt_image):
        ...
        feeder = client.containers.run(eurdep_radiation_mqtt_image.id, detach=True, remove=False, network_mode='host', environment={'MQTT_BROKER_URL': broker_url, 'ONCE_MODE': 'true', 'PYTHONUNBUFFERED': '1', 'EURDEP_RADIATION_SAMPLE_MODE': 'true'})
        try:
...
class TestNifcUsaWildfiresMqttDockerFlow:
    def test_emits_incident_topic(self, mosquitto_nifc_usa_wildfires, nifc_usa_wildfires_mqtt_image):
        ...
        feeder=client.containers.run(nifc_usa_wildfires_mqtt_image.id, detach=True, remove=False, network_mode='host', environment={'MQTT_BROKER_URL':broker_url,'ONCE_MODE':'true','PYTHONUNBUFFERED':'1','NIFC_USA_WILDFIRES_SAMPLE_MODE':'true'})
        try:
            result=feeder.wait(timeout=300); logs=feeder.logs().decode('utf-8',errors='replace'); assert result.get('StatusCode')==0, f"Feeder exited non-zero: {result}\n{logs}"
        finally:
            try: feeder.remove(force=True)
...
class TestXceedMqttDockerFlow:
    def test_emits_mqtt_cloudevents(self):
        broker, network, host_port, broker_ip = _generic_mosquitto('xceed-mqtt-e2e', 'xceed-mqtt-e2e-broker')
        client = docker.from_env(); feeder = None
        try:
            image = build_image('xceed', dockerfile='Dockerfile.mqtt', tag='test-xceed-mqtt')
            feeder = client.containers.run(image.id, detach=True, remove=False, network_mode='host', environment={'MQTT_BROKER_URL': f'mqtt://{broker_ip}:{host_port}','ONCE_MODE':'true','PYTHONUNBUFFERED':'1'})
...
class TestElexonBmrsMqttDockerFlow:
    def test_emits_mqtt_cloudevents(self):
        broker, network, host_port, broker_ip = _generic_mosquitto('elexon-bmrs-mqtt-e2e', 'elexon-bmrs-mqtt-e2e-broker')
        client = docker.from_env(); feeder = None
        try:
...
class TestEnergidataserviceDkMqttDockerFlow:
    def test_emits_mqtt_cloudevents(self):
        broker, network, host_port, broker_ip = _generic_mosquitto('energidataservice-dk-mqtt-e2e', 'energidataservice-dk-mqtt-e2e-broker')
        client = docker.from_env(); feeder = None
        try:
...
class TestEnergyChartsMqttDockerFlow:
    def test_emits_mqtt_cloudevents(self):
        broker, network, host_port, broker_ip = _generic_mosquitto('energy-charts-mqtt-e2e', 'energy-charts-mqtt-e2e-broker')
        client = docker.from_env(); feeder = None
        try:
...
class TestBillettoMqttDockerFlow:
    def test_emits_mqtt_cloudevents(self):
        broker, network, host_port, broker_ip = _generic_mosquitto('billetto-mqtt-e2e', 'billetto-mqtt-e2e-broker')
        client = docker.from_env(); feeder = None
        try:
...
class TestFientaMqttDockerFlow:
    def test_emits_mqtt_cloudevents(self):
        broker, network, host_port, broker_ip = _generic_mosquitto('fienta-mqtt-e2e', 'fienta-mqtt-e2e-broker')
        client = docker.from_env(); feeder = None
        try:
...
class TestTicketmasterMqttDockerFlow:
    def test_emits_mqtt_cloudevents(self):
        broker, network, host_port, broker_ip = _generic_mosquitto('ticketmaster-mqtt-e2e', 'ticketmaster-mqtt-e2e-broker')
        client = docker.from_env(); feeder = None
        try:
...
class TestTepcoDenkiyohoMqttDockerFlow:
    def test_emits_mqtt_cloudevents(self):
        broker, network, host_port, broker_ip = _generic_mosquitto('tepco-denkiyoho-mqtt-e2e', 'tepco-denkiyoho-mqtt-e2e-broker')
        client = docker.from_env(); feeder = None
        try:
...
@pytest.fixture(scope='module')
def hubeau_hydrometrie_mqtt_image():
    return build_image('hubeau-hydrometrie', dockerfile='Dockerfile.mqtt', tag='test-hubeau-hydrometrie-mqtt')
...
class _B3SimpleMqttFlow:
    ...
    def test_emits_expected_mqtt_topics(self):
        client = docker.from_env()
        broker, network, host_port, broker_ip = _generic_mosquitto(f'{self.image}-e2e', f'{self.image}-e2e-broker')
        try:
            ...
            for attempt in range(3):
                feeder = client.containers.run(
                    ...
                    remove=False,
                    network_mode='host',
                    environment={'MQTT_BROKER_URL': f'mqtt://{broker_ip}:{host_port}', 'ONCE_MODE': 'true', 'PYTHONUNBUFFERED': '1'},
                )
                result = feeder.wait(timeout=240)
                logs = feeder.logs().decode('utf-8', errors='replace')
                feeder.remove(force=True)
...
def _run_b4_aq_mqtt_flow(source: str, image_name: str, extra_env: dict, topic_filter: str, expected_types: set) -> None:
    ...
    broker, network, host_port, broker_ip = _generic_mosquitto(network_name, broker_name)
    client = docker.from_env()
    feeder = None
    try:
        ...
        for attempt in range(3):
            feeder = client.containers.run(
                image.id, detach=True, remove=False, network_mode='host', environment=env,
            )
            result = feeder.wait(timeout=600)
            logs = feeder.logs().decode('utf-8', errors='replace')
            if result.get('StatusCode') == 0:
...
class TestNoaaSwpcL1MqttDockerFlow:
    ...
    def test_emits_retained_propagated_solar_wind(self, mosquitto_container_swpc, noaa_swpc_l1_mqtt_image):
        ...
        feeder = client.containers.run(
            noaa_swpc_l1_mqtt_image.id, detach=True, remove=False, network_mode='host',
            environment={
                'MQTT_BROKER_URL': broker_url,
                'POLLING_INTERVAL': '60',
                'ONCE_MODE': 'true',
...
class TestSiriMqttDockerFlow:
    def test_emits_expected_mqtt_topics(self):
        client = docker.from_env()
        broker, network, host_port, broker_ip = _generic_mosquitto('siri-mqtt-e2e', 'siri-mqtt-e2e-broker')
        subscriber = mqtt.Client(callback_api_version=CallbackAPIVersion.VERSION2, protocol=MQTTv5)
        messages = []
...
class TestOpenAQMqttDockerFlow:
    def test_emits_retained_uns_topics(self, mosquitto_openaq, openaq_mqtt_image):
        ...
        feeder = client.containers.run(openaq_mqtt_image.id, detach=True, remove=False, network_mode='host', environment={'MQTT_BROKER_URL': broker_url, 'OPENAQ_MOCK': 'true', 'ONCE_MODE': 'true', 'PYTHONUNBUFFERED': '1'})
        try:
            result = feeder.wait(timeout=240)
            logs = feeder.logs().decode('utf-8', errors='replace')
            assert result.get('StatusCode') == 0, logs[-4000:]
...
class TestDatex2MqttDockerFlow:
    def test_emits_binary_cloudevents(self, datex2_mqtt_image):
        client = docker.from_env()
        broker, network, host_port, broker_ip = _generic_mosquitto('datex2-mqtt-e2e', 'datex2-mqtt-e2e-broker')
        try:
            feeder = client.containers.run(
