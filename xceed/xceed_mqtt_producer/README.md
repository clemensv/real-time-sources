# Xceed_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the xceed_mqtt_producer message definitions.

# Xceed_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - XceedMqttEventDispatcher,

    XceedAdmissionsMqttEventDispatcher
...
from xceed_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event Hubs,
...
client = XceedMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### XceedMqttEventDispatcher



# Publish message`XceedMqttEventDispatcher` handles events for the xceed.mqtt message group.
...
from xceed_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = XceedMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event Hub.Args:-
`consumer_group_name`: The name of the consumer group.- `eventhubs_fully_qualified_namespace`: The fully qualified
namespace of the Event Hub.

client = XceedAdmissionsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe XceedMqttEventDispatcher defines the following event handler hooks.
...
```##### `xceed_mqtt_event_async`



## Test```python

xceed_mqtt_event_async:  Callable[[PartitionContext, EventData, CloudEvent, Event], Awaitable[None]]
...
- `data`: The event data of type `xceed_mqtt_producer_data.Event`.

Example:

```python
async def xceed_mqtt_event_event(partition_context: PartitionContext, event: EventData, cloud_event: CloudEvent, data:
Event) -> None:
...
client = XceedAdmissionsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### XceedAdmissionsMqttEventDispatcher



# Publish message`XceedAdmissionsMqttEventDispatcher` handles events for the xceed.admissions.mqtt message group.
...
## BuildThe XceedAdmissionsMqttEventDispatcher defines the following event handler hooks.
...
```##### `xceed_admissions_mqtt_event_admission_async`



## Test```python

xceed_admissions_mqtt_event_admission_async:  Callable[[PartitionContext, EventData, CloudEvent, EventAdmission],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `xceed_mqtt_producer_data.EventAdmission`.

Example:

```python
async def xceed_admissions_mqtt_event_admission_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: EventAdmission) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
