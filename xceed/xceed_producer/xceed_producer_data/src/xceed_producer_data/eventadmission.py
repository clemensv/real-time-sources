...
class EventAdmission:
    """
    Ticket-availability snapshot for a single admission tier of an Xceed event, retrieved from the GET /events/:eventId/admissions endpoint of the Xceed Open Event API v1. An event may offer multiple admission tiers (e.g. early-bird, standard, VIP). Each tier has a stable UUID and carries sales-state signals including whether tickets are sold out or whether sales have been closed, as well as price and remaining quantity.
