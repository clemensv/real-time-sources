...
Asynchronous handler hook for `xceed.EventAdmission`: Ticket-availability snapshot for a single admission tier of an
Xceed event, retrieved from the /events/:eventId/admissions endpoint of the Xceed Open Event API. Captures sales state
signals including isSoldOut, isSalesClosed, current price, and remaining ticket count.
...
Send a single `xceed.EventAdmission` message. Ticket-availability snapshot for a single admission tier of an Xceed
event, retrieved from the /events/:eventId/admissions endpoint of the Xceed Open Event API. Captures sales state signals
including isSoldOut, isSalesClosed, current price, and remaining ticket count.Args:
