# Sensor_community_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the sensor_community_mqtt_producer message definitions.

# Sensor_community_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - IoSensorCommunityMqttEventDispatcher
...
from sensor_community_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = IoSensorCommunityMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### IoSensorCommunityMqttEventDispatcher



# Publish message`IoSensorCommunityMqttEventDispatcher` handles events for the io.sensor.community.mqtt message group.
...
from sensor_community_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = IoSensorCommunityMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the Event
...
## BuildThe IoSensorCommunityMqttEventDispatcher defines the following event handler hooks.
...
```##### `io_sensor_community_mqtt_sensor_info_async`



## Test```python

io_sensor_community_mqtt_sensor_info_async:  Callable[[PartitionContext, EventData, CloudEvent, SensorInfo],
Awaitable[None]]
...
```Asynchronous handler hook for `io.sensor.community.mqtt.SensorInfo`: Reference data for a Sensor.Community sensor
node including its hardware type and last known location metadata.
...
- `data`: The event data of type `sensor_community_mqtt_producer_data.SensorInfo`.

Example:

```python
async def io_sensor_community_mqtt_sensor_info_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: SensorInfo) -> None:
...
```##### `io_sensor_community_mqtt_sensor_reading_async`



## Test```python

io_sensor_community_mqtt_sensor_reading_async:  Callable[[PartitionContext, EventData, CloudEvent, SensorReading],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `sensor_community_mqtt_producer_data.SensorReading`.

Example:

```python
async def io_sensor_community_mqtt_sensor_reading_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: SensorReading) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
