...
docker run --rm -e MQTT_BROKER_URL=mqtt://broker:1883 ghcr.io/clemensv/real-time-sources-gtfs-mqtt:latest
```
...
docker run --rm -e AMQP_HOST=broker -e AMQP_ADDRESS=gtfs ghcr.io/clemensv/real-time-sources-gtfs-amqp:latest
```
...
| `AMQP_ADDRESS` | No | `gtfs` | Queue/topic/address to send to. |
