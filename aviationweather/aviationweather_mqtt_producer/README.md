# Aviationweather_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the aviationweather_mqtt_producer message definitions.

# Aviationweather_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - GovNoaaAviationweatherMqttEventDispatcher
...
from aviationweather_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = GovNoaaAviationweatherMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### GovNoaaAviationweatherMqttEventDispatcher



# Publish message`GovNoaaAviationweatherMqttEventDispatcher` handles events for the gov.noaa.aviationweather.mqtt
message group.
...
from aviationweather_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = GovNoaaAviationweatherMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
## BuildThe GovNoaaAviationweatherMqttEventDispatcher defines the following event handler hooks.
...
```##### `gov_noaa_aviationweather_mqtt_station_async`



## Test```python

gov_noaa_aviationweather_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station],
Awaitable[None]]
...
- `data`: The event data of type `aviationweather_mqtt_producer_data.Station`.

Example:

```python
async def gov_noaa_aviationweather_mqtt_station_event(partition_context: PartitionContext, event: EventData,
...
```##### `gov_noaa_aviationweather_mqtt_metar_async`



## Test```python

gov_noaa_aviationweather_mqtt_metar_async:  Callable[[PartitionContext, EventData, CloudEvent, Metar], Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `aviationweather_mqtt_producer_data.Metar`.

Example:

```python
async def gov_noaa_aviationweather_mqtt_metar_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Metar) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `gov_noaa_aviationweather_mqtt_sigmet_async`



## Test```python

gov_noaa_aviationweather_mqtt_sigmet_async:  Callable[[PartitionContext, EventData, CloudEvent, Sigmet],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `aviationweather_mqtt_producer_data.Sigmet`.

Example:

```python
async def gov_noaa_aviationweather_mqtt_sigmet_event(partition_context: PartitionContext, event: EventData, cloud_event:
CloudEvent, data: Sigmet) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
