...
import os
import signal
from aviationweather_mqtt_producer_data import *
from aviationweather_mqtt_producer_mqtt_client.client import GovNoaaAviationweatherMqttProducer, GovNoaaAviationweatherMqttDispatcher

async def handle_gov_noaa_aviationweather_mqtt_station(mqtt_msg,cloud_event, gov_noaa_aviationweather_mqtt_station_data):
    """ Handles the gov.noaa.aviationweather.mqtt.Station message """
    print(f"Received gov.noaa.aviationweather.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {gov_noaa_aviationweather_mqtt_station_data}")

async def handle_gov_noaa_aviationweather_mqtt_metar(mqtt_msg,cloud_event, gov_noaa_aviationweather_mqtt_metar_data):
    """ Handles the gov.noaa.aviationweather.mqtt.Metar message """
    print(f"Received gov.noaa.aviationweather.mqtt.Metar on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {gov_noaa_aviationweather_mqtt_metar_data}")

async def handle_gov_noaa_aviationweather_mqtt_sigmet(mqtt_msg,cloud_event, gov_noaa_aviationweather_mqtt_sigmet_data):
    """ Handles the gov.noaa.aviationweather.mqtt.Sigmet message """
    print(f"Received gov.noaa.aviationweather.mqtt.Sigmet on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {gov_noaa_aviationweather_mqtt_sigmet_data}")
