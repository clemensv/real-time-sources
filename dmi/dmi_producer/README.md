...
# Dmi_producer Kafka Producer# Dmi_producer Event Dispatcher for Apache Kafka
...
3. [Quick Start](#quick-start)    - DkDmiMetObsKafkaEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    DkDmiOceanObsKafkaEventDispatcher,

4. [Generated Producer Classes](#generated-producer-classes)    DkDmiLightningKafkaEventDispatcher
...
- DkDmiMetObsKafkaProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- DkDmiOceanObsKafkaProducersolution for event-driven applications.

It includes both plain Kafka messages and CloudEvents, offering a versatile

- DkDmiLightningKafkaProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### DkDmiMetObsKafkaEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`DkDmiMetObsKafkaEventDispatcher` handles events for the dk.dmi.metObs.kafka message group.
...
from dmi_producer import DkDmiMetObsKafkaProducer```python

create_processor(self, bootstrap_servers: str, group_id: str, topics: List[str]) -> EventProcessorRunner

# Create producer```

producer = DkDmiMetObsKafkaProducer(
...
The DkDmiMetObsKafkaEventDispatcher defines the following event handler hooks.
...
producer = DkDmiMetObsKafkaProducer(
    ...
    security_protocol='SASL_SSL',##### `dk_dmi_met_obs_kafka_station_async`
...
Asynchronous handler hook for `dk.dmi.metObs.kafka.Station`: Reference data for a DMI meteorological observation
station. Emitted at feeder startup and refreshed daily. Stations cover Denmark, Greenland (DNK/GRL) and the Faroe
Islands (FRO).
...
Producer for `dk.dmi.metObs.kafka` message group.Example:
...
DkDmiMetObsKafkaProducer(    await some_processing_function(record, cloud_event, data)
...
dk_dmi_met_obs_kafka_dispatcher.dk_dmi_met_obs_kafka_station_async = dk_dmi_met_obs_kafka_station_event
...
Producer for `dk.dmi.oceanObs.kafka` message group.Example:
...
DkDmiOceanObsKafkaProducer(    await some_processing_function(record, cloud_event, data)
...
dk_dmi_ocean_obs_kafka_dispatcher.dk_dmi_met_obs_kafka_station_async = dk_dmi_met_obs_kafka_station_event
...
Producer for `dk.dmi.lightning.kafka` message group.Example:
...
DkDmiLightningKafkaProducer(    await some_processing_function(record, cloud_event, data)
...
dk_dmi_lightning_kafka_dispatcher.dk_dmi_met_obs_kafka_station_async = dk_dmi_met_obs_kafka_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `dk_dmi_met_obs_kafka_observation_async`
...
Asynchronous handler hook for `dk.dmi.metObs.kafka.Observation`: A single meteorological observation value reported by a
station for one parameter at a specific observed time. Cadence depends on parameter (10-minute or hourly).
...
dk_dmi_met_obs_kafka_dispatcher.dk_dmi_met_obs_kafka_observation_async = dk_dmi_met_obs_kafka_observation_event
...
dk_dmi_ocean_obs_kafka_dispatcher.dk_dmi_met_obs_kafka_observation_async = dk_dmi_met_obs_kafka_observation_event
...
dk_dmi_lightning_kafka_dispatcher.dk_dmi_met_obs_kafka_observation_async = dk_dmi_met_obs_kafka_observation_event
...
##### `send_dk_dmi_met_obs_kafka_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_dk_dmi_met_obs_kafka_station(
...
Send a single `dk.dmi.metObs.kafka.Station` message. Reference data for a DMI meteorological observation station.
Emitted at feeder startup and refreshed daily. Stations cover Denmark, Greenland (DNK/GRL) and the Faroe Islands
(FRO).Args:
...
##### `send_dk_dmi_met_obs_kafka_station_batch`##### `_dispatch_record`



```python```python

async def send_dk_dmi_met_obs_kafka_station_batch(_dispatch_record(self, record)
...
Send multiple `dk.dmi.metObs.kafka.Station` messages in a batch.
...
await producer.send_dk_dmi_met_obs_kafka_station_batch(```
...
##### `send_dk_dmi_met_obs_kafka_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_dk_dmi_met_obs_kafka_observation(
...
Send a single `dk.dmi.metObs.kafka.Observation` message. A single meteorological observation value reported by a station
for one parameter at a specific observed time. Cadence depends on parameter (10-minute or hourly).Args:
...
##### `send_dk_dmi_met_obs_kafka_observation_batch`##### `_dispatch_record`



```python```python

async def send_dk_dmi_met_obs_kafka_observation_batch(_dispatch_record(self, record)
...
Send multiple `dk.dmi.metObs.kafka.Observation` messages in a batch.
...
await producer.send_dk_dmi_met_obs_kafka_observation_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### DkDmiOceanObsKafkaEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`DkDmiOceanObsKafkaEventDispatcher` handles events for the dk.dmi.oceanObs.kafka message group.
...
The DkDmiOceanObsKafkaEventDispatcher defines the following event handler hooks.
...
producer = DkDmiMetObsKafkaProducer(
    ...
    security_protocol='SASL_SSL',##### `dk_dmi_ocean_obs_kafka_station_async`
...
Asynchronous handler hook for `dk.dmi.oceanObs.kafka.Station`: Reference data for an oceanographic observation station
(tide gauge or other coastal sensor). Owners include DMI and Kystdirektoratet (Danish Coastal Authority).
...
dk_dmi_met_obs_kafka_dispatcher.dk_dmi_ocean_obs_kafka_station_async = dk_dmi_ocean_obs_kafka_station_event
...
dk_dmi_ocean_obs_kafka_dispatcher.dk_dmi_ocean_obs_kafka_station_async = dk_dmi_ocean_obs_kafka_station_event
...
dk_dmi_lightning_kafka_dispatcher.dk_dmi_ocean_obs_kafka_station_async = dk_dmi_ocean_obs_kafka_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `dk_dmi_ocean_obs_kafka_tidewater_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'dk_dmi_ocean_obs_kafka_tidewater_station_async:  Callable[[ConsumerRecord, CloudEvent,
TidewaterStation], Awaitable[None]]

)```

```

Asynchronous handler hook for `dk.dmi.oceanObs.kafka.TidewaterStation`: Reference data for a station/grid-point at which
DMI publishes tidewater (sea-level) predictions. The set partially overlaps physical tide gauges but also includes
prediction-only grid points (e.g. Faroes).
...
### DkDmiMetObsKafkaProducer- `data`: The event data of type `dmi_producer_data.TidewaterStation`.
...
async def dk_dmi_ocean_obs_kafka_tidewater_station_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
TidewaterStation) -> None:
...
dk_dmi_met_obs_kafka_dispatcher.dk_dmi_ocean_obs_kafka_tidewater_station_async =
dk_dmi_ocean_obs_kafka_tidewater_station_event
...
### DkDmiOceanObsKafkaProducer- `data`: The event data of type `dmi_producer_data.TidewaterStation`.
...
dk_dmi_ocean_obs_kafka_dispatcher.dk_dmi_ocean_obs_kafka_tidewater_station_async =
dk_dmi_ocean_obs_kafka_tidewater_station_event
...
### DkDmiLightningKafkaProducer- `data`: The event data of type `dmi_producer_data.TidewaterStation`.
...
dk_dmi_lightning_kafka_dispatcher.dk_dmi_ocean_obs_kafka_tidewater_station_async =
dk_dmi_ocean_obs_kafka_tidewater_station_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `dk_dmi_ocean_obs_kafka_observation_async`
...
Asynchronous handler hook for `dk.dmi.oceanObs.kafka.Observation`: A single oceanographic observation. Parameters are
sealev_dvr (sea level vs DVR90 datum, cm), sealev_ln (sea level vs local zero, cm), sea_reg (registered sea level by
Kystdirektoratet, cm), and tw (water temperature, deg C). 10-minute cadence.
...
dk_dmi_met_obs_kafka_dispatcher.dk_dmi_ocean_obs_kafka_observation_async = dk_dmi_ocean_obs_kafka_observation_event
...
dk_dmi_ocean_obs_kafka_dispatcher.dk_dmi_ocean_obs_kafka_observation_async = dk_dmi_ocean_obs_kafka_observation_event
...
dk_dmi_lightning_kafka_dispatcher.dk_dmi_ocean_obs_kafka_observation_async = dk_dmi_ocean_obs_kafka_observation_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `dk_dmi_ocean_obs_kafka_tidewater_prediction_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'dk_dmi_ocean_obs_kafka_tidewater_prediction_async:  Callable[[ConsumerRecord,
CloudEvent, TidewaterPrediction], Awaitable[None]]

)```

```

Asynchronous handler hook for `dk.dmi.oceanObs.kafka.TidewaterPrediction`: A deterministic tidewater (sea-level)
prediction for one station and one forecast horizon. predictionType is typically '10minutes'; predictions are issued
forward ~30 days.
...
### DkDmiMetObsKafkaProducer- `data`: The event data of type `dmi_producer_data.TidewaterPrediction`.
...
async def dk_dmi_ocean_obs_kafka_tidewater_prediction_event(record: ConsumerRecord, cloud_event: CloudEvent, data:
TidewaterPrediction) -> None:
...
dk_dmi_met_obs_kafka_dispatcher.dk_dmi_ocean_obs_kafka_tidewater_prediction_async =
dk_dmi_ocean_obs_kafka_tidewater_prediction_event
...
### DkDmiOceanObsKafkaProducer- `data`: The event data of type `dmi_producer_data.TidewaterPrediction`.
...
dk_dmi_ocean_obs_kafka_dispatcher.dk_dmi_ocean_obs_kafka_tidewater_prediction_async =
dk_dmi_ocean_obs_kafka_tidewater_prediction_event
...
### DkDmiLightningKafkaProducer- `data`: The event data of type `dmi_producer_data.TidewaterPrediction`.
...
dk_dmi_lightning_kafka_dispatcher.dk_dmi_ocean_obs_kafka_tidewater_prediction_async =
dk_dmi_ocean_obs_kafka_tidewater_prediction_event
...
##### `send_dk_dmi_ocean_obs_kafka_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_dk_dmi_ocean_obs_kafka_station(
...
Send a single `dk.dmi.oceanObs.kafka.Station` message. Reference data for an oceanographic observation station (tide
gauge or other coastal sensor). Owners include DMI and Kystdirektoratet (Danish Coastal Authority).Args:
...
##### `send_dk_dmi_ocean_obs_kafka_station_batch`##### `_dispatch_record`



```python```python

async def send_dk_dmi_ocean_obs_kafka_station_batch(_dispatch_record(self, record)
...
Send multiple `dk.dmi.oceanObs.kafka.Station` messages in a batch.
...
await producer.send_dk_dmi_ocean_obs_kafka_station_batch(```
...
##### `send_dk_dmi_ocean_obs_kafka_tidewater_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_dk_dmi_ocean_obs_kafka_tidewater_station(
...
Send a single `dk.dmi.oceanObs.kafka.TidewaterStation` message. Reference data for a station/grid-point at which DMI
publishes tidewater (sea-level) predictions. The set partially overlaps physical tide gauges but also includes
prediction-only grid points (e.g. Faroes).Args:

- `record`: The Kafka record.

**Parameters:**

- `data`: Message data of type `TidewaterStation`
...
await producer.send_dk_dmi_ocean_obs_kafka_tidewater_station(

    data=TidewaterStation(...),Args:
...
##### `send_dk_dmi_ocean_obs_kafka_tidewater_station_batch`##### `_dispatch_record`



```python```python

async def send_dk_dmi_ocean_obs_kafka_tidewater_station_batch(_dispatch_record(self, record)

    self,```

    messages: List[TidewaterStation],
...
Send multiple `dk.dmi.oceanObs.kafka.TidewaterStation` messages in a batch.
...
await producer.send_dk_dmi_ocean_obs_kafka_tidewater_station_batch(```

    messages=[

        TidewaterStation(...),Initializes the runner with a Kafka consumer.

        TidewaterStation(...),

        TidewaterStation(...)Args:
...
##### `send_dk_dmi_ocean_obs_kafka_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_dk_dmi_ocean_obs_kafka_observation(
...
Send a single `dk.dmi.oceanObs.kafka.Observation` message. A single oceanographic observation. Parameters are sealev_dvr
(sea level vs DVR90 datum, cm), sealev_ln (sea level vs local zero, cm), sea_reg (registered sea level by
Kystdirektoratet, cm), and tw (water temperature, deg C). 10-minute cadence.Args:
...
##### `send_dk_dmi_ocean_obs_kafka_observation_batch`##### `_dispatch_record`



```python```python

async def send_dk_dmi_ocean_obs_kafka_observation_batch(_dispatch_record(self, record)
...
Send multiple `dk.dmi.oceanObs.kafka.Observation` messages in a batch.
...
await producer.send_dk_dmi_ocean_obs_kafka_observation_batch(```
...
##### `send_dk_dmi_ocean_obs_kafka_tidewater_prediction`Dispatchers have the following protected methods:



```python### Methods:

async def send_dk_dmi_ocean_obs_kafka_tidewater_prediction(
...
Send a single `dk.dmi.oceanObs.kafka.TidewaterPrediction` message. A deterministic tidewater (sea-level) prediction for
one station and one forecast horizon. predictionType is typically '10minutes'; predictions are issued forward ~30
days.Args:
...
- `data`: Message data of type `TidewaterPrediction`
...
await producer.send_dk_dmi_ocean_obs_kafka_tidewater_prediction(

    data=TidewaterPrediction(...),Args:
...
##### `send_dk_dmi_ocean_obs_kafka_tidewater_prediction_batch`##### `_dispatch_record`



```python```python

async def send_dk_dmi_ocean_obs_kafka_tidewater_prediction_batch(_dispatch_record(self, record)

    self,```

    messages: List[TidewaterPrediction],
...
Send multiple `dk.dmi.oceanObs.kafka.TidewaterPrediction` messages in a batch.
...
await producer.send_dk_dmi_ocean_obs_kafka_tidewater_prediction_batch(```

    messages=[

        TidewaterPrediction(...),Initializes the runner with a Kafka consumer.

        TidewaterPrediction(...),

        TidewaterPrediction(...)Args:
...
- **Scales horizontally** across multiple brokers and partitions### DkDmiLightningKafkaEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`DkDmiLightningKafkaEventDispatcher` handles events for the dk.dmi.lightning.kafka message group.
...
The DkDmiLightningKafkaEventDispatcher defines the following event handler hooks.
...
producer = DkDmiMetObsKafkaProducer(
    ...
    security_protocol='SASL_SSL',##### `dk_dmi_lightning_kafka_sensor_async`
...
Asynchronous handler hook for `dk.dmi.lightning.kafka.Sensor`: Reference data for one of DMI's lightning detection
sensors. Six DMI-owned sensors cover Denmark; third-party sensor IDs that appear in observation.sensors are not
catalogued here.
...
dk_dmi_met_obs_kafka_dispatcher.dk_dmi_lightning_kafka_sensor_async = dk_dmi_lightning_kafka_sensor_event
...
dk_dmi_ocean_obs_kafka_dispatcher.dk_dmi_lightning_kafka_sensor_async = dk_dmi_lightning_kafka_sensor_event
...
dk_dmi_lightning_kafka_dispatcher.dk_dmi_lightning_kafka_sensor_async = dk_dmi_lightning_kafka_sensor_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `dk_dmi_lightning_kafka_strike_async`
...
Asynchronous handler hook for `dk.dmi.lightning.kafka.Strike`: A single triangulated lightning strike. Type 0 = cloud-
to-ground negative, 1 = cloud-to-ground positive, 2 = cloud-to-cloud. amp is signed peak current in kA. observed
...
dk_dmi_met_obs_kafka_dispatcher.dk_dmi_lightning_kafka_strike_async = dk_dmi_lightning_kafka_strike_event
...
dk_dmi_ocean_obs_kafka_dispatcher.dk_dmi_lightning_kafka_strike_async = dk_dmi_lightning_kafka_strike_event
...
dk_dmi_lightning_kafka_dispatcher.dk_dmi_lightning_kafka_strike_async = dk_dmi_lightning_kafka_strike_event
...
##### `send_dk_dmi_lightning_kafka_sensor`Dispatchers have the following protected methods:



```python### Methods:

async def send_dk_dmi_lightning_kafka_sensor(
...
Send a single `dk.dmi.lightning.kafka.Sensor` message. Reference data for one of DMI's lightning detection sensors. Six
DMI-owned sensors cover Denmark; third-party sensor IDs that appear in observation.sensors are not catalogued here.Args:
...
##### `send_dk_dmi_lightning_kafka_sensor_batch`##### `_dispatch_record`



```python```python

async def send_dk_dmi_lightning_kafka_sensor_batch(_dispatch_record(self, record)
...
Send multiple `dk.dmi.lightning.kafka.Sensor` messages in a batch.
...
await producer.send_dk_dmi_lightning_kafka_sensor_batch(```
...
##### `send_dk_dmi_lightning_kafka_strike`Dispatchers have the following protected methods:



```python### Methods:

async def send_dk_dmi_lightning_kafka_strike(
...
Send a single `dk.dmi.lightning.kafka.Strike` message. A single triangulated lightning strike. Type 0 = cloud-to-ground
negative, 1 = cloud-to-ground positive, 2 = cloud-to-cloud. amp is signed peak current in kA. observed timestamps have
microsecond precision.Args:
...
##### `send_dk_dmi_lightning_kafka_strike_batch`##### `_dispatch_record`



```python```python

async def send_dk_dmi_lightning_kafka_strike_batch(_dispatch_record(self, record)
...
Send multiple `dk.dmi.lightning.kafka.Strike` messages in a batch.
...
await producer.send_dk_dmi_lightning_kafka_strike_batch(```
