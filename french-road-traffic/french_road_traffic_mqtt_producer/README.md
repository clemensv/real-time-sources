# French_road_traffic_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the french_road_traffic_mqtt_producer message definitions.

# French_road_traffic_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - FrGouvTransportBisonFuteTrafficFlowMqttEventDispatcher,
    ...
    FrGouvTransportBisonFuteRoadEventMqttEventDispatcher
...
from french_road_traffic_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure
Event Hubs,
...
client = FrGouvTransportBisonFuteTrafficFlowMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### FrGouvTransportBisonFuteTrafficFlowMqttEventDispatcher



# Publish message`FrGouvTransportBisonFuteTrafficFlowMqttEventDispatcher` handles events for the
...
from french_road_traffic_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = FrGouvTransportBisonFuteTrafficFlowMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The
...
client = FrGouvTransportBisonFuteRoadEventMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The
...
## BuildThe FrGouvTransportBisonFuteTrafficFlowMqttEventDispatcher defines the following event handler hooks.
...
```##### `fr_gouv_transport_bison_fute_traffic_flow_measurement_mqtt_async`



## Test```python

fr_gouv_transport_bison_fute_traffic_flow_measurement_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent,
TrafficFlowMeasurement], Awaitable[None]]
...
- `data`: The event data of type `french_road_traffic_mqtt_producer_data.TrafficFlowMeasurement`.

Example:

```python
async def fr_gouv_transport_bison_fute_traffic_flow_measurement_mqtt_event(partition_context: PartitionContext, event:
EventData, cloud_event: CloudEvent, data: TrafficFlowMeasurement) -> None:
...
client = FrGouvTransportBisonFuteRoadEventMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### FrGouvTransportBisonFuteRoadEventMqttEventDispatcher



# Publish message`FrGouvTransportBisonFuteRoadEventMqttEventDispatcher` handles events for the
...
## BuildThe FrGouvTransportBisonFuteRoadEventMqttEventDispatcher defines the following event handler hooks.
...
```##### `fr_gouv_transport_bison_fute_road_event_mqtt_async`



## Test```python

fr_gouv_transport_bison_fute_road_event_mqtt_async:  Callable[[PartitionContext, EventData, CloudEvent, RoadEvent],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `french_road_traffic_mqtt_producer_data.RoadEvent`.

Example:

```python
async def fr_gouv_transport_bison_fute_road_event_mqtt_event(partition_context: PartitionContext, event: EventData,
