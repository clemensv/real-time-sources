...
docker run --rm -e MQTT_BROKER_URL=mqtt://broker:1883 ghcr.io/clemensv/real-time-sources-french-road-traffic-mqtt:latest
```
...
docker run --rm -e AMQP_HOST=broker -e AMQP_ADDRESS=french-road-traffic ghcr.io/clemensv/real-time-sources-french-road-traffic-amqp:latest
```
...
| `AMQP_ADDRESS` | No | `french-road-traffic` | Queue/topic/address to send to. |
