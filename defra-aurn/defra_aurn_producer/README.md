...
# Defra_aurn_producer Kafka Producer# Defra_aurn_producer Event Dispatcher for Apache Kafka
...
- UkGovDefraAurnStationsProducersolution for event-driven applications.
...
- **Scales horizontally** across multiple brokers and partitions### UkGovDefraAurnStationsEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`UkGovDefraAurnStationsEventDispatcher` handles events for the uk.gov.defra.aurn.Stations message group.
...
The UkGovDefraAurnStationsEventDispatcher defines the following event handler hooks.
...
producer = UkGovDefraAurnStationsProducer(
    ...
    security_protocol='SASL_SSL',##### `uk_gov_defra_aurn_station_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_defra_aurn_station_async:  Callable[[ConsumerRecord, CloudEvent, Station],
Awaitable[None]]
...
Asynchronous handler hook for `uk.gov.defra.aurn.Station`: Reference metadata for a Defra AURN monitoring station entry
as published by the 52°North SOS Timeseries API stations collection.
...
uk_gov_defra_aurn_stations_dispatcher.uk_gov_defra_aurn_station_async = uk_gov_defra_aurn_station_event
...
##### `send_uk_gov_defra_aurn_station`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_defra_aurn_station(
...
Send a single `uk.gov.defra.aurn.Station` message. Reference metadata for a Defra AURN monitoring station entry as
published by the 52°North SOS Timeseries API stations collection.Args:
...
await producer.send_uk_gov_defra_aurn_station(
...
Send multiple `uk.gov.defra.aurn.Station` messages in a batch.
...
await producer.send_uk_gov_defra_aurn_station_batch(```
...
- **Scales horizontally** across multiple brokers and partitions### UkGovDefraAurnTimeseriesEventDispatcher

- **Enables pub/sub messaging** with topic-based routing

`UkGovDefraAurnTimeseriesEventDispatcher` handles events for the uk.gov.defra.aurn.Timeseries message group.
...
The UkGovDefraAurnTimeseriesEventDispatcher defines the following event handler hooks.
...
producer = UkGovDefraAurnStationsProducer(
    ...
    security_protocol='SASL_SSL',##### `uk_gov_defra_aurn_timeseries_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_defra_aurn_timeseries_async:  Callable[[ConsumerRecord, CloudEvent, Timeseries],
Awaitable[None]]
...
Asynchronous handler hook for `uk.gov.defra.aurn.Timeseries`: Reference metadata for a Defra AURN station and pollutant
timeseries combination, including linked station coordinates and pollutant taxonomy identifiers.
...
uk_gov_defra_aurn_stations_dispatcher.uk_gov_defra_aurn_timeseries_async = uk_gov_defra_aurn_timeseries_event
...
uk_gov_defra_aurn_timeseries_dispatcher.uk_gov_defra_aurn_timeseries_async = uk_gov_defra_aurn_timeseries_event
...
- `**kwargs`: Additional Kafka producer configuration
    ...
    security_protocol='SASL_SSL',##### `uk_gov_defra_aurn_observation_async`

    sasl_mechanism='PLAIN',

    sasl_username='your-username',```python

    sasl_password='your-password'uk_gov_defra_aurn_observation_async:  Callable[[ConsumerRecord, CloudEvent,
...
Asynchronous handler hook for `uk.gov.defra.aurn.Observation`: Hourly or near-hourly Defra AURN observation value for a
single timeseries, emitted from the getData endpoint for the most recent two-hour polling window.
...
uk_gov_defra_aurn_stations_dispatcher.uk_gov_defra_aurn_observation_async = uk_gov_defra_aurn_observation_event
...
##### `send_uk_gov_defra_aurn_timeseries`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_defra_aurn_timeseries(
...
Send a single `uk.gov.defra.aurn.Timeseries` message. Reference metadata for a Defra AURN station and pollutant
timeseries combination, including linked station coordinates and pollutant taxonomy identifiers.Args:
...
await producer.send_uk_gov_defra_aurn_timeseries(
...
Send multiple `uk.gov.defra.aurn.Timeseries` messages in a batch.
...
await producer.send_uk_gov_defra_aurn_timeseries_batch(```
...
##### `send_uk_gov_defra_aurn_observation`Dispatchers have the following protected methods:



```python### Methods:

async def send_uk_gov_defra_aurn_observation(
...
Send a single `uk.gov.defra.aurn.Observation` message. Hourly or near-hourly Defra AURN observation value for a single
timeseries, emitted from the getData endpoint for the most recent two-hour polling window.Args:
...
await producer.send_uk_gov_defra_aurn_observation(
...
Send multiple `uk.gov.defra.aurn.Observation` messages in a batch.
