# Defra_aurn_mqtt_producer MQTT Client

This library provides MQTT producer and consumer functionality for the defra_aurn_mqtt_producer message definitions.

# Defra_aurn_mqtt_producer Event Dispatcher for Azure Event Hubs
...
```    - UkGovDefraAurnStationsMqttEventDispatcher,

    UkGovDefraAurnTimeseriesMqttEventDispatcher
...
from defra_aurn_mqtt_producer_mqtt_client import *This module defines an event processing framework for Azure Event
Hubs,
...
client = UkGovDefraAurnStationsMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### UkGovDefraAurnStationsMqttEventDispatcher



# Publish message`UkGovDefraAurnStationsMqttEventDispatcher` handles events for the uk.gov.defra.aurn.Stations.mqtt
message group.
...
from defra_aurn_mqtt_producer_mqtt_client import *create_processor(self, consumer_group_name: str,
...
client = UkGovDefraAurnStationsMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of the
...
client = UkGovDefraAurnTimeseriesMqttMqttClient(mqtt_client, content_mode='structured')- `eventhub_name`: The name of
...
## BuildThe UkGovDefraAurnStationsMqttEventDispatcher defines the following event handler hooks.
...
```##### `uk_gov_defra_aurn_stations_mqtt_station_async`



## Test```python

uk_gov_defra_aurn_stations_mqtt_station_async:  Callable[[PartitionContext, EventData, CloudEvent, Station],
Awaitable[None]]
...
```Asynchronous handler hook for `uk.gov.defra.aurn.Stations.mqtt.Station`: Reference metadata for a Defra AURN
monitoring station entry as published by the 52°North SOS Timeseries API stations collection.
...
- `data`: The event data of type `defra_aurn_mqtt_producer_data.Station`.

Example:

```python
async def uk_gov_defra_aurn_stations_mqtt_station_event(partition_context: PartitionContext, event: EventData,
...
client = UkGovDefraAurnTimeseriesMqttMqttClient(mqtt_client, content_mode='structured')

# Connect to broker

await client.connect("mqtt.example.com", 1883)### UkGovDefraAurnTimeseriesMqttEventDispatcher



# Publish message`UkGovDefraAurnTimeseriesMqttEventDispatcher` handles events for the uk.gov.defra.aurn.Timeseries.mqtt
message group.
...
## BuildThe UkGovDefraAurnTimeseriesMqttEventDispatcher defines the following event handler hooks.
...
```##### `uk_gov_defra_aurn_timeseries_mqtt_timeseries_async`



## Test```python

uk_gov_defra_aurn_timeseries_mqtt_timeseries_async:  Callable[[PartitionContext, EventData, CloudEvent, Timeseries],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `defra_aurn_mqtt_producer_data.Timeseries`.

Example:

```python
async def uk_gov_defra_aurn_timeseries_mqtt_timeseries_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: Timeseries) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
...
```##### `uk_gov_defra_aurn_timeseries_mqtt_observation_async`



## Test```python

uk_gov_defra_aurn_timeseries_mqtt_observation_async:  Callable[[PartitionContext, EventData, CloudEvent, Observation],
Awaitable[None]]
...
- `event`: The event data.
- `cloud_event`: The CloudEvent.
- `data`: The event data of type `defra_aurn_mqtt_producer_data.Observation`.

Example:

```python
async def uk_gov_defra_aurn_timeseries_mqtt_observation_event(partition_context: PartitionContext, event: EventData,
cloud_event: CloudEvent, data: Observation) -> None:
    # Process the event data
    await partition_context.update_checkpoint(event)
```
