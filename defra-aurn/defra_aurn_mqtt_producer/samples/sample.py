...
import os
import signal
from defra_aurn_mqtt_producer_data import *
from defra_aurn_mqtt_producer_mqtt_client.client import UkGovDefraAurnStationsMqttProducer, UkGovDefraAurnStationsMqttDispatcher

async def handle_uk_gov_defra_aurn_stations_mqtt_station(mqtt_msg,cloud_event, uk_gov_defra_aurn_stations_mqtt_station_data):
    """ Handles the uk.gov.defra.aurn.Stations.mqtt.Station message """
    print(f"Received uk.gov.defra.aurn.Stations.mqtt.Station on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_gov_defra_aurn_stations_mqtt_station_data}")
from defra_aurn_mqtt_producer_mqtt_client.client import UkGovDefraAurnTimeseriesMqttProducer, UkGovDefraAurnTimeseriesMqttDispatcher

async def handle_uk_gov_defra_aurn_timeseries_mqtt_timeseries(mqtt_msg,cloud_event, uk_gov_defra_aurn_timeseries_mqtt_timeseries_data):
    """ Handles the uk.gov.defra.aurn.Timeseries.mqtt.Timeseries message """
    print(f"Received uk.gov.defra.aurn.Timeseries.mqtt.Timeseries on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_gov_defra_aurn_timeseries_mqtt_timeseries_data}")

async def handle_uk_gov_defra_aurn_timeseries_mqtt_observation(mqtt_msg,cloud_event, uk_gov_defra_aurn_timeseries_mqtt_observation_data):
    """ Handles the uk.gov.defra.aurn.Timeseries.mqtt.Observation message """
    print(f"Received uk.gov.defra.aurn.Timeseries.mqtt.Observation on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {uk_gov_defra_aurn_timeseries_mqtt_observation_data}")
