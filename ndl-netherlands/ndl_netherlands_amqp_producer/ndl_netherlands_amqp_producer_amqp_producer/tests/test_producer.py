...
"""
Tests for ndl_netherlands_amqp_producer_amqp_producer
"""
import base64
import json
import os
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../ndl_netherlands_amqp_producer_data/src')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../ndl_netherlands_amqp_producer_data/tests')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../ndl_netherlands_amqp_producer_amqp_producer/src')))

from ndl_netherlands_amqp_producer_amqp_producer import *
from ndl_netherlands_amqp_producer_data import TrafficSpeed
from test_ndl_netherlands_amqp_producer_data_trafficspeed import Test_TrafficSpeed
from ndl_netherlands_amqp_producer_data import TravelTime
from test_ndl_netherlands_amqp_producer_data_traveltime import Test_TravelTime
from ndl_netherlands_amqp_producer_data import TrafficSituation
from test_ndl_netherlands_amqp_producer_data_trafficsituation import Test_TrafficSituation
...
class TestNLNDWTrafficMeasurementsAmqpProducer:
    """Test cases for NLNDWTrafficMeasurementsAmqpProducer"""
    ...
    def test_send_amqp(self, artemis_container):
        ...
        payload = Test_TrafficSpeed.create_instance()
        ...
        try:
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    assert cloud_event_payload.get("type") == "NL.NDW.Traffic.TrafficSpeed.amqp"
    ...
    def test_send_amqp(self, artemis_container):
        """Send and receive a Amqp message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_TravelTime.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "NL.NDW.Traffic.TravelTime.amqp"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
...
class TestNLNDWTrafficSituationsAmqpProducer:
    """Test cases for NLNDWTrafficSituationsAmqpProducer"""
    ...
    def test_send_amqp(self, artemis_container):
        """Send and receive a Amqp message via ActiveMQ Artemis."""
        # Create valid test data using the test helper
        payload = Test_TrafficSituation.create_instance()
            ...
            for i in range(5):
                ...
                if True:
                    ...
                    if isinstance(body, memoryview):
                        ...
                        body_text = str(body)
                    cloud_event_payload = json.loads(body_text)
                    assert cloud_event_payload.get("type") == "NL.NDW.Traffic.TrafficSituation.amqp"
                    # Verify data section exists (either as data or data_base64)
                    assert "data" in cloud_event_payload or "data_base64" in cloud_event_payload
                else:
                    # Verify message body is not empty
