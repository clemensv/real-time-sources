...
def get_default_topic_mappings_nl_ndw_traffic_measurements_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the NL.NDW.Traffic.Measurements.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "NL.NDW.Traffic.TrafficSpeed.mqtt": "traffic/nl/ndl/ndl-netherlands/{road}/{site_id}/speed",
        "NL.NDW.Traffic.TravelTime.mqtt": "traffic/nl/ndl/ndl-netherlands/{road}/{site_id}/travel-time",
    }


def get_subscription_topics_nl_ndw_traffic_measurements_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_nl_ndw_traffic_measurements_mqtt()
    topics = []
...
class NLNDWTrafficMeasurementsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the NL.NDW.Traffic.Measurements.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_nl_ndw_traffic_measurements_mqtt()
        ...
        self.nl_ndw_traffic_traffic_speed_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, ndl_netherlands_mqtt_producer_data.TrafficSpeed, Dict[str, str]], Awaitable[None]]] = None
        
        self.nl_ndw_traffic_travel_time_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, ndl_netherlands_mqtt_producer_data.TravelTime, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "NL.NDW.Traffic.TrafficSpeed":
            if self.nl_ndw_traffic_traffic_speed_mqtt_async:
                try:
                    ...
                    data = ndl_netherlands_mqtt_producer_data.TrafficSpeed.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "NL.NDW.Traffic.TrafficSpeed.mqtt")
                    await self.nl_ndw_traffic_traffic_speed_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in nl_ndw_traffic_traffic_speed_mqtt handler: {e}")
            return
        
        if event_type == "NL.NDW.Traffic.TravelTime":
            if self.nl_ndw_traffic_travel_time_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = ndl_netherlands_mqtt_producer_data.TravelTime.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "NL.NDW.Traffic.TravelTime.mqtt")
                    await self.nl_ndw_traffic_travel_time_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in nl_ndw_traffic_travel_time_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_nl_ndw_traffic_measurements_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_nl_ndw_traffic_traffic_speed_mqtt(self,
        site_id: str,
        road: str,
        data: ndl_netherlands_mqtt_producer_data.TrafficSpeed,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'NL.NDW.Traffic.TrafficSpeed.mqtt' event to an MQTT topic.

        Args:
        
            site_id: URI template variable for 'site_id'
            road: URI template variable for 'road'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/nl/ndl/ndl-netherlands/{road}/{site_id}/speed'
        ...
        """
        target_topic = topic if topic is not None else "traffic/nl/ndl/ndl-netherlands/{road}/{site_id}/speed"
        ...
        attributes = {
             "type":"NL.NDW.Traffic.TrafficSpeed",
             "source":"https://opendata.ndw.nu/trafficspeed.xml.gz",
             "subject":"{site_id}".format(site_id = site_id)
        }
        attributes["datacontenttype"] = content_type
    ...
    async def publish_nl_ndw_traffic_travel_time_mqtt(self,
        site_id: str,
        road: str,
        data: ndl_netherlands_mqtt_producer_data.TravelTime,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'NL.NDW.Traffic.TravelTime.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            road: URI template variable for 'road'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/nl/ndl/ndl-netherlands/{road}/{site_id}/travel-time'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (True).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "traffic/nl/ndl/ndl-netherlands/{road}/{site_id}/travel-time"
        _topic_template_values: Dict[str, str] = {
            "site_id": str(site_id),
            "road": str(road),
        }
        ...
        attributes = {
             "type":"NL.NDW.Traffic.TravelTime",
...
def get_default_topic_mappings_nl_ndw_traffic_situations_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the NL.NDW.Traffic.Situations.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "NL.NDW.Traffic.TrafficSituation.mqtt": "traffic/nl/ndl/ndl-netherlands/situations/{road}/{situation_id}/event",
    }


def get_subscription_topics_nl_ndw_traffic_situations_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    Get subscription topics with URI template placeholders replaced by MQTT wildcards (+).
    ...
    Returns:
        List of topic patterns suitable for MQTT subscription.
    """
    mappings = topic_mappings or get_default_topic_mappings_nl_ndw_traffic_situations_mqtt()
    topics = []
    for topic in mappings.values():
        wildcard_topic = _topic_to_mqtt_wildcard(topic)
        if wildcard_topic not in topics:
...
class NLNDWTrafficSituationsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the NL.NDW.Traffic.Situations.mqtt message group."""
    ...
    def __init__(
        ...
        self.content_mode = content_mode
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_nl_ndw_traffic_situations_mqtt()
        self._topic_patterns = {k: _build_topic_regex(v) for k, v in self._topic_mappings.items()}
        
        # Message handler callbacks (Dispatcher pattern)
        
        self.nl_ndw_traffic_traffic_situation_mqtt_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, ndl_netherlands_mqtt_producer_data.TrafficSituation, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "NL.NDW.Traffic.TrafficSituation":
            if self.nl_ndw_traffic_traffic_situation_mqtt_async:
                try:
                    content_type = cloud_event.get_attributes().get('datacontenttype', 'application/json')
                    # CloudEvent.data is now a dict or string, not bytes
                    data = ndl_netherlands_mqtt_producer_data.TrafficSituation.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "NL.NDW.Traffic.TrafficSituation.mqtt")
                    await self.nl_ndw_traffic_traffic_situation_mqtt_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in nl_ndw_traffic_traffic_situation_mqtt handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_nl_ndw_traffic_situations_mqtt(self._topic_mappings)
        for topic in topics:
            self.client.subscribe(topic, qos)
    ...
    async def publish_nl_ndw_traffic_traffic_situation_mqtt(self,
        situation_id: str,
        road: str,
        data: ndl_netherlands_mqtt_producer_data.TrafficSituation,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'NL.NDW.Traffic.TrafficSituation.mqtt' event to an MQTT topic.
        ...
        Args:
            ...
            road: URI template variable for 'road'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'traffic/nl/ndl/ndl-netherlands/situations/{road}/{situation_id}/event'
                with URI template placeholders substituted from the keyword arguments.
            qos: Optional MQTT QoS override. If not provided, uses the message default (1).
            retain: Optional MQTT retain flag override. If not provided, uses the message default (False).
            content_type: The content type for the event data.
        """
        target_topic = topic if topic is not None else "traffic/nl/ndl/ndl-netherlands/situations/{road}/{situation_id}/event"
        _topic_template_values: Dict[str, str] = {
        ...
        attributes = {
             "type":"NL.NDW.Traffic.TrafficSituation",
             "source":"https://opendata.ndw.nu/actueel_beeld.xml.gz",
