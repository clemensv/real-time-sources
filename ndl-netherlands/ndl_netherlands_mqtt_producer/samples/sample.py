...
import os
import signal
from ndl_netherlands_mqtt_producer_data import *
from ndl_netherlands_mqtt_producer_mqtt_client.client import NLNDWTrafficMeasurementsMqttProducer, NLNDWTrafficMeasurementsMqttDispatcher

async def handle_nl_ndw_traffic_traffic_speed_mqtt(mqtt_msg,cloud_event, nl_ndw_traffic_traffic_speed_mqtt_data):
    """ Handles the NL.NDW.Traffic.TrafficSpeed.mqtt message """
    print(f"Received NL.NDW.Traffic.TrafficSpeed.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {nl_ndw_traffic_traffic_speed_mqtt_data}")

async def handle_nl_ndw_traffic_travel_time_mqtt(mqtt_msg,cloud_event, nl_ndw_traffic_travel_time_mqtt_data):
    """ Handles the NL.NDW.Traffic.TravelTime.mqtt message """
    print(f"Received NL.NDW.Traffic.TravelTime.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {nl_ndw_traffic_travel_time_mqtt_data}")
from ndl_netherlands_mqtt_producer_mqtt_client.client import NLNDWTrafficSituationsMqttProducer, NLNDWTrafficSituationsMqttDispatcher

async def handle_nl_ndw_traffic_traffic_situation_mqtt(mqtt_msg,cloud_event, nl_ndw_traffic_traffic_situation_mqtt_data):
    """ Handles the NL.NDW.Traffic.TrafficSituation.mqtt message """
    print(f"Received NL.NDW.Traffic.TrafficSituation.mqtt on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {nl_ndw_traffic_traffic_situation_mqtt_data}")
