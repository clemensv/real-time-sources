...
The notebook drains the DWD Eventstream (bound as a Spark Notebook
destination, preview) and for every file-reference event
(``DE.DWD.Radar.RadarFileProduct`` and ``DE.DWD.Forecast.IconD2ForecastFile``)
fetches ``file_url`` anonymously, lands the raw bytes under
...
CELLS = [
    md(
        "# DWD Ingest — Eventstream → Lakehouse (bronze + silver + COG gold)\n\n"
        "Drains the DWD Fabric Eventstream as a Spark Structured Streaming job. "
        "For every file-reference event (radar composite, ICON-D2 forecast file) the "
        "notebook fetches `file_url` anonymously, writes raw bytes to "
        "`Files/bronze/<channel>/...`, and—when GIS decoders are available—publishes "
        "a Cloud Optimized GeoTIFF to `Files/gold/<channel>_cog/...` and registers it "
        "in the KQL `CogCatalog` table for Fabric Maps imagery layers.\n\n"
        "**Bound automatically by `tools/dwd/fabric/setup.ps1`:**\n"
        "- `LAKEHOUSE_PATH` — abfss URI of the default Lakehouse for this notebook\n"
        "- `KUSTO_URI` / `KUSTO_DATABASE` — DWD KQL database for `CogCatalog` writes\n\n"
        "**Eventstream destination wiring:** add this notebook as a Spark Notebook "
        "destination on the DWD Eventstream (preview). See "
        "[Add a Spark Notebook destination to an Eventstream]"
        "(https://learn.microsoft.com/en-us/fabric/real-time-intelligence/event-streams/add-destination-spark-notebook)."
    ),
    code(
    ...
    md(
        "## Stream binding and `foreachBatch` driver\n\n"
        "Eventstream's Spark Notebook destination binds the stream as a Spark DataFrame "
        "with format `\"eventstream\"`. The driver below classifies each event by "
        "CloudEvents `type`, dispatches to the per-channel handler with a bounded "
        "thread pool, and writes the resulting `CogCatalog` rows in one KQL ingest."
    ),
    code(
        ...
        "    # Materialize the micro-batch as Python dicts (small relative to file payloads).\n"
    ...
    code(
        "# Bind the Eventstream as a streaming DataFrame.\n"
        ...
        "    .option('checkpointLocation', f\"{LAKEHOUSE_PATH}/Files/_checkpoints/dwd_ingest\")\n"
