...
import gdacs_mqtt_producer_data
from gdacs_mqtt_producer_data import DisasterAlert
...
def get_default_topic_mappings_gdacs_alerts_mqtt() -> Dict[str, str]:
    """
    Get the default topic mappings for the GDACS.Alerts.mqtt message group.
    
    Returns:
        Dictionary mapping message identifiers to their default topic patterns.
    """
    return {
        "GDACS.Alerts.mqtt.DisasterAlert": "alerts/intl/gdacs/gdacs/{event_type}/{alert_color}/{country}/{event_id}/alert",
    }


def get_subscription_topics_gdacs_alerts_mqtt(topic_mappings: Optional[Dict[str, str]] = None) -> List[str]:
    """
    ...
    """
    mappings = topic_mappings or get_default_topic_mappings_gdacs_alerts_mqtt()
    topics = []
...
class GDACSAlertsMqttMqttClient(_ClientBase):
    """MQTT Client for producing and consuming messages in the GDACS.Alerts.mqtt message group."""
    ...
    def __init__(
        ...
        self.loop = loop
        self._topic_mappings = topic_mappings or get_default_topic_mappings_gdacs_alerts_mqtt()
        ...
        self.gdacs_alerts_mqtt_disaster_alert_async: Optional[Callable[[mqtt.MQTTMessage, CloudEvent, gdacs_mqtt_producer_data.DisasterAlert, Dict[str, str]], Awaitable[None]]] = None
    ...
    async def _dispatch_cloud_event(self, mqtt_message: mqtt.MQTTMessage, cloud_event: CloudEvent):
        ...
        if event_type == "GDACS.DisasterAlert":
            if self.gdacs_alerts_mqtt_disaster_alert_async:
                try:
                    ...
                    data = gdacs_mqtt_producer_data.DisasterAlert.from_data(cloud_event.data, content_type)
                    topic_params = self._extract_topic_params(mqtt_message.topic, "GDACS.Alerts.mqtt.DisasterAlert")
                    await self.gdacs_alerts_mqtt_disaster_alert_async(mqtt_message, cloud_event, data, topic_params)
                except Exception as e:
                    print(f"Error in gdacs_alerts_mqtt_disaster_alert handler: {e}")
            return
    ...
    async def subscribe(self, topics: Optional[typing.List[str]] = None, qos: int = 0):
        ...
        """
        if topics is None:
            topics = get_subscription_topics_gdacs_alerts_mqtt(self._topic_mappings)
        for topic in topics:
    ...
    async def publish_gdacs_alerts_mqtt_disaster_alert(self,
        event_type: str,
        event_id: str,
        alert_color: str,
        country: str,
        data: gdacs_mqtt_producer_data.DisasterAlert,
        topic: Optional[str] = None,
        qos: Optional[int] = None,
        retain: Optional[bool] = None,
        content_type: str = "application/json") -> None:
        """
        Publish the 'GDACS.Alerts.mqtt.DisasterAlert' event to an MQTT topic.

        Args:
        
            event_type: URI template variable for 'event_type'
            event_id: URI template variable for 'event_id'
            alert_color: URI template variable for 'alert_color'
            country: URI template variable for 'country'
            data: The event data to be published.
            topic: Optional topic override. If not provided, uses default topic 'alerts/intl/gdacs/gdacs/{event_type}/{alert_color}/{country}/{event_id}/alert'
        ...
        """
        target_topic = topic if topic is not None else "alerts/intl/gdacs/gdacs/{event_type}/{alert_color}/{country}/{event_id}/alert"
        _topic_template_values: Dict[str, str] = {
            "event_type": str(event_type),
            "event_id": str(event_id),
            "alert_color": str(alert_color),
            "country": str(country),
        }
        ...
        attributes = {
             ...
             "subject":"{event_type}/{event_id}".format(event_type = event_type,event_id = event_id)
        }
        attributes["datacontenttype"] = content_type
