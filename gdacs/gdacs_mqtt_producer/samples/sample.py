...
import os
import signal
from gdacs_mqtt_producer_data import *
from gdacs_mqtt_producer_mqtt_client.client import GDACSAlertsMqttProducer, GDACSAlertsMqttDispatcher

async def handle_gdacs_alerts_mqtt_disaster_alert(mqtt_msg,cloud_event, gdacs_alerts_mqtt_disaster_alert_data):
    """ Handles the GDACS.Alerts.mqtt.DisasterAlert message """
    print(f"Received GDACS.Alerts.mqtt.DisasterAlert on topic {mqtt_msg.topic}")
    if cloud_event:
        print(f"  CloudEvent ID: {cloud_event.id}, Source: {cloud_event.source}")
    print(f"  Data: {gdacs_alerts_mqtt_disaster_alert_data}")
